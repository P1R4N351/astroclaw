...
coverage:
  ...
  secondary:
    - tools.invocation
objective: Verify Astroclaw can expose plugin tools over MCP and a real MCP client can call one successfully.
successCriteria:
...
execution:
  kind: flow
  summary: Verify Astroclaw can expose plugin tools over MCP and a real MCP client can call one successfully.
  config:
