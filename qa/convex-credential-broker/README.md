...
- `ASTROCLAW_QA_CONVEX_SITE_URL` must use `https://` in normal use.
- Local development may use loopback `http://` only when `ASTROCLAW_QA_ALLOW_INSECURE_HTTP=1`.
...
Admin endpoints require `ASTROCLAW_QA_CONVEX_SECRET_MAINTAINER`.
