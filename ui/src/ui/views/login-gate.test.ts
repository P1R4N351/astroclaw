...
describe("resolveLoginFailureFeedback", () => {
  ...
  it("explains missing auth credentials", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Paste the token from astroclaw dashboard --no-open or enter the configured password.",
      "If no token is configured, run astroclaw doctor --generate-gateway-token on the gateway host.",
  ...
  it("explains rejected stale credentials", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Run astroclaw dashboard --no-open and open the fresh URL or paste its token.",
  ...
  it("preserves pairing request ids in the approval command", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Run astroclaw devices list on the Gateway host.",
      "Approve this request: astroclaw devices approve req-123.",
  ...
  it("keeps generic browser WebSocket constructor failures on the network path", () => {
    ...
    expect(feedback?.steps).toEqual([
      ...
      "Reopen the dashboard with astroclaw dashboard --no-open to recopy the current URL and auth details.",
    ]);
  });
  ...
  it("explains protocol mismatch without requiring a gateway protocol change", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Reopen the served dashboard with astroclaw dashboard so the UI and Gateway come from the same install.",
      "If using pnpm ui:dev, rebuild or restart the dev UI against the current checkout.",
      "Restart the Gateway after updating Astroclaw so it serves the current protocol.",
    ]);
  });
  ...
  it("redacts credential-shaped values from displayed raw errors", () => {
    const feedback = resolveLoginFailureFeedback({
      connected: false,
      lastError:
        "failed ws://host/astroclaw#token=secret-token Authorization: Bearer secret-bearer token=inline-secret",
      lastErrorCode: null,
      hasToken: false,
      hasPassword: false,
    });
    ...
    expect(feedback?.rawError).toBe(
      "failed ws://host/astroclaw#[redacted-credential] Authorization: Bearer [redacted] token=[redacted]",
    );
  });
});
...
describe("renderLoginGate", () => {
  ...
  it("renders an accessible structured failure panel with raw error details", async () => {
    ...
    expect(docsLink?.getAttribute("href")).toBe(
      "https://docs.astroclaw.ai/web/control-ui#debuggingtesting-dev-server--remote-gateway",
    );
  });
});
