...
function waitForChatResult(params: {
  ...
  return new Promise((resolve, reject) => {
    if (params.signal?.aborted) {
      reject(new DOMException("Astroclaw tool call aborted", "AbortError"));
      return;
    }
    const timer = window.setTimeout(() => {
      cleanup();
      reject(new Error("Astroclaw tool call timed out"));
    }, params.timeoutMs);
    const onAbort = () => {
      cleanup();
      reject(new DOMException("Astroclaw tool call aborted", "AbortError"));
...
export async function submitRealtimeTalkConsult(params: {
  ...
  try {
    ...
    if (!runId) {
      throw new Error("Astroclaw realtime tool call did not return a run id");
    }
    if (params.signal?.aborted) {
      abortRun();
      return;
