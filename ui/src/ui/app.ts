...
declare global {
  interface Window {
    __ASTROCLAW_CONTROL_UI_BASE_PATH__?: string;
  }
}
...
export class AstroclawApp extends LitElement {
  ...
  async toggleRealtimeTalk() {
    ...
    const session = new RealtimeTalkSession(
      ...
      this.sessionKey,
      {
        ...
        onTranscript: (entry) => {
          this.realtimeTalkTranscript = `${entry.role === "user" ? "You" : "Astroclaw"}: ${entry.text}`;
        },
      },
...
if (!customElements.get("astroclaw-app")) {
  customElements.define("astroclaw-app", AstroclawApp);
}
