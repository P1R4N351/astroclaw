      ...
      ${renderSettingsSection(
        ...
        permissions.entries.map(({ id, status }) =>
          renderSettingsRow({
            ...
            control: html`
              ${renderSettingsStatus({
                kind: status === "granted" ? "ok" : status === "denied" ? "danger" : "muted",
                label: t(`configPage.deviceSettings.permissionStatuses.${status}`),
              })}
              ${status === "notDetermined"
                ? html`<button
                    type="button"
                    class="btn"
                    @click=${() => capability?.requestPermission(id)}
                  >
                ...
                : status === "denied"
                  ? html`<button
                      type="button"
                      class="btn"
                      @click=${() => capability?.openSystemSettings(id)}
                    >
