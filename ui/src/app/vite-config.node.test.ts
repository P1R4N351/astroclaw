...
describe("Control UI Vite config", () => {
  ...
  it("resolves Control UI dev-server source aliases for internal packages", () => {
    ...
    expect(
      aliases.find((alias) => alias.find === "@astroclaw/normalization-core/agent-id"),
    )?.toEqual({
    ...
    const resultAliasIndex = aliases.findIndex(
      (alias) => alias.find === "@astroclaw/normalization-core/result",
    );
    const rootAliasIndex = aliases.findIndex(
      (alias) => alias.find === "@astroclaw/normalization-core",
    );
  ...
  it("keeps specific tsconfig aliases ahead of broad package aliases", () => {
    ...
    const netPolicyIpIndex = aliases.findIndex(
      (alias) => alias.find === "@astroclaw/net-policy/ip",
    );
