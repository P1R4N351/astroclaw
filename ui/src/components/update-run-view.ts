...
class UpdateRunView extends OpenClawLightDomElement {
  ...
  override render() {
    ...
    >
      ...
      ${!this.connected && !view.terminal
        ? html`<p class="update-run-view__connection">${t("updates.run.reconnecting")}</p>`
        : nothing}
      ...
      <div class="update-run-view__detail-heading">
        ${t("updates.run.details")}${view.detailStep
          ? html`<span>${view.detailStep}</span>`
          : nothing}
      </div>
      <pre
