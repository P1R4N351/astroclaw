...
```bash
astroclaw nodes describe --node <idOrNameOrIp>
astroclaw nodes canvas snapshot --node <idOrNameOrIp>
astroclaw logs --follow
```
...
astroclaw nodes status
astroclaw approvals get --node <idOrNameOrIp>
astroclaw approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`astroclaw approvals get --node ...`), not in the gateway pairing record.
