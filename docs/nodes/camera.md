...
Astroclaw supports **camera capture** for agent workflows:
...
```bash
astroclaw nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
astroclaw nodes camera snap --node <id> --facing front
astroclaw nodes camera clip --node <id> --duration 3000
astroclaw nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`astroclaw.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return "Camera disabled by user".

### CLI helper (node invoke)

Use the main `astroclaw` CLI to invoke camera commands on the macOS node.

Examples:

```bash
astroclaw nodes camera list --node <id>            # list camera ids
astroclaw nodes camera snap --node <id>            # prints MEDIA:<path>
astroclaw nodes camera snap --node <id> --max-width 1280
astroclaw nodes camera snap --node <id> --delay-ms 2000
astroclaw nodes camera snap --node <id> --device-id <id>
astroclaw nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
astroclaw nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
astroclaw nodes camera clip --node <id> --device-id <id>
astroclaw nodes camera clip --node <id> --no-audio
```

Notes:

- `astroclaw nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
astroclaw nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
