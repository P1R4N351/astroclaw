...
## Config (`~/.astroclaw/astroclaw.json`)
...
- `consultThinkingLevel`: optional thinking level override for the full Astroclaw agent run behind realtime `astroclaw_agent_consult` calls.
- `consultFastMode`: optional fast-mode override for realtime `astroclaw_agent_consult` calls.
...
- `realtime.instructions`: appends provider-facing system instructions to Astroclaw's built-in realtime prompt. Use it for voice style and tone; Astroclaw keeps the default `astroclaw_agent_consult` guidance.
...
- Browser realtime Talk uses `talk.client.toolCall` for `astroclaw_agent_consult` instead of exposing `chat.send` to provider-owned browser sessions.
...
- macOS local MLX playback uses the bundled `astroclaw-mlx-tts` helper when present, or an executable on `PATH`. Set `ASTROCLAW_MLX_TTS_BIN` to point at a custom helper binary during development.
