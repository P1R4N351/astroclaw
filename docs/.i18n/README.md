...
- repo: `astroclaw/docs`
- local checkout: `~/Projects/astroclaw-docs`

## Source of truth

- English docs are authored in `astroclaw/astroclaw`.
...
1. Edit English docs in `astroclaw/astroclaw`.
2. Push to `main`.
3. `astroclaw/astroclaw/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `astroclaw/docs`.
...
5. `astroclaw/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `astroclaw/docs`.
...
- Do not treat missing `th` or `fa` entries in generated `docs/docs.json` as a pipeline failure. Verify their generated folders in `astroclaw/docs` instead.
...
- Sync metadata is written to `.astroclaw-sync/source.json` in the publish repo.
- Source repo secret: `ASTROCLAW_DOCS_SYNC_TOKEN`
- Publish repo secret: `ASTROCLAW_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `astroclaw/docs` first.
