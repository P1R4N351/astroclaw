...
summary: "Specification for making the bundled Codex app-server harness honor Astroclaw context-engine plugins"
read_when:
...
Make the bundled Codex app-server harness honor the same Astroclaw context-engine
...
Astroclaw-level compaction policy as far as the Codex app-server boundary allows.
...
That means the required hook point is in Astroclaw-controlled code. The external
boundary is the Codex app-server protocol itself: Astroclaw can control what it
...
For Codex harness turns, Astroclaw should preserve this lifecycle:

1. Read the mirrored Astroclaw session transcript.
...
8. Mirror the Codex result back into the Astroclaw transcript.
...
11. Preserve Codex native compaction signals and Astroclaw compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. Astroclaw should
not try to mutate the app-server's internal history except through supported
protocol calls.

Astroclaw's transcript mirror remains the source for Astroclaw features:
...
The context-engine interface returns Astroclaw `AgentMessage[]`, not a Codex
...
- Label it clearly as Astroclaw assembled context.
...
```text
Astroclaw assembled context for this turn:
...
inside Astroclaw and preserves context-engine semantics.
...
1. Astroclaw context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit Astroclaw compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
Astroclaw compaction should prefer the context engine's `compact()` result for
the Astroclaw transcript mirror and plugin state.
...
the Astroclaw session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
Astroclaw session lifecycle paths. Do not add Codex-specific cleanup unless the
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole Astroclaw compaction when the context engine is primary.
...
Avoid requiring lossless-claw in Astroclaw core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for Astroclaw/plugin state.
