...
An owner is a ClawHub publisher handle, such as `@alice` or `@astroclaw`.
...
`@astroclaw/dronzer`, it can only be published as `@astroclaw`. If you publish as
...
```text
Package scope "@astroclaw" must match selected owner "@vintageayu".
Publish as "@astroclaw" or rename this package to "@vintageayu/dronzer".
```
...
```sh
clawhub package transfer @opik/opik-astroclaw --to opik
```
...
This protects org namespaces. A package named `@astroclaw/dronzer` claims the
`@astroclaw` namespace, so only publishers with access to the `@astroclaw` owner
can publish it.
