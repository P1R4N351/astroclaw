---
summary: "Gateway config reference for core Astroclaw keys, defaults, and links to dedicated subsystem references"
...
Core config reference for `~/.astroclaw/astroclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Covers the main Astroclaw config surfaces and links out when a subsystem has its own deeper reference. Channel- and plugin-owned command catalogs and deep memory/QMD knobs live on their own pages rather than on this one.

Code truth:

- `astroclaw config schema` prints the live JSON Schema used for validation and Control UI, with bundled/plugin/channel metadata merged in when available
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional - Astroclaw uses safe defaults when omitted.
...
- `talk.*` (Talk mode)
  - `talk.consultThinkingLevel`: thinking level override for the full Astroclaw agent run behind Control UI Talk realtime consults
...
- `models.providers.*.localService`: optional on-demand process manager for
  local model servers. Astroclaw probes the configured health endpoint, starts
...
Astroclaw-managed MCP server definitions live under `mcp.servers` and are
consumed by embedded Pi and other runtime adapters. The `astroclaw mcp list`,
...
- `mcp.servers`: named stdio or remote MCP server definitions for runtimes that
  ...
  `type: "http"` is a CLI-native alias that `astroclaw mcp set` and
  `astroclaw doctor --fix` normalize into the canonical `transport` field.
- `mcp.servers.<name>.codex`: optional Codex app-server projection controls.
  This block is Astroclaw metadata for Codex app-server threads only; it does not
  affect ACP sessions, generic Codex harness config, or other runtime adapters.
  Non-empty `codex.agents` limits the server to the listed Astroclaw agent ids.
  ...
  `default_tools_approval_mode` for that server. Astroclaw strips the `codex`
...
See [MCP](/cli/mcp#astroclaw-as-an-mcp-client-registry) and
...
- `install.nodeManager`: node installer preference for `metadata.astroclaw.install`
...
- Loaded from `~/.astroclaw/extensions`, `<workspace>/.astroclaw/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Astroclaw plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Astroclaw plugin schema when available).
- Channel plugin account/runtime settings live under `channels.<id>` and should be described by the owning plugin's manifest `channelConfigs` metadata, not by a central Astroclaw option registry.
...
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Astroclaw applies those as sanitized agent settings, not as raw Astroclaw config patches.
...
`commitments` controls inferred follow-up memory: Astroclaw can detect check-ins from conversation turns and deliver them through heartbeat runs.
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want Astroclaw to discover `/json/version`; use WS(S)
...
- If an externally managed CDP service is reachable through loopback, set that
  profile's `attachOnly: true`; otherwise Astroclaw treats the loopback port as a
...
- Local managed `astroclaw` profiles auto-assign `cdpPort` and `cdpUrl`; only
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or ASTROCLAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `ASTROCLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `tailscale.preserveFunnel`: when `true` and `tailscale.mode = "serve"`, Astroclaw
...
- `ASTROCLAW_APNS_RELAY_BASE_URL` / `ASTROCLAW_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `ASTROCLAW_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
- `gateway.handshakeTimeoutMs`: pre-auth Gateway WebSocket handshake timeout in milliseconds. Default: `15000`. `ASTROCLAW_HANDSHAKE_TIMEOUT_MS` takes precedence when set. Increase this on loaded or low-powered hosts where local clients can connect while startup warmup is still settling.
...
```bash
ASTROCLAW_CONFIG_PATH=~/.astroclaw/a.json \
ASTROCLAW_STATE_DIR=~/.astroclaw-a \
astroclaw gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.astroclaw-dev` + port `19001`), `--profile <name>` (uses `~/.astroclaw-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/astroclaw/tls/server.crt",
      keyPath: "/etc/astroclaw/tls/server.key",
      caPath: "/etc/astroclaw/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.astroclaw/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-astroclaw-token: <token>`.
...
- `transform` can point to a JS/TS module returning a hook action.
  ...
  - Keep `hooks.transformsDir` under `~/.astroclaw/hooks/transforms`; workspace skill directories are rejected. If `astroclaw doctor` reports this path as invalid, move the transform module into the hooks transforms directory or remove `hooks.transformsDir`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `ASTROCLAW_SKIP_GMAIL_WATCHER=1` to disable.
  ...
  plugins: {
    entries: {
      canvas: {
        config: {
          host: {
            root: "~/.astroclaw/workspace/canvas",
            liveReload: true,
            // enabled: false, // or ASTROCLAW_SKIP_CANVAS_HOST=1
          },
        },
      },
    },
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__astroclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__astroclaw__/a2ui/`
...
- Also serves A2UI at `/__astroclaw__/a2ui/`.
...
- Hostname defaults to the system hostname when it is a valid DNS label, falling back to `astroclaw`. Override with `ASTROCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.astroclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `astroclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.astroclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${ASTROCLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `astroclaw.json` credential paths.
...
- Legacy flat `auth-profiles.json` maps such as `{ "provider": { "apiKey": "..." } }` are not a runtime format; `astroclaw doctor --fix` rewrites them to canonical `provider:default` API-key profiles with a `.legacy-flat.*.bak` backup.
...
- Legacy OAuth imports from `~/.astroclaw/credentials/oauth.json`.
...
- Default log file: `/tmp/astroclaw/astroclaw-YYYY-MM-DD.log`.
- Set `logging.file` for a stable path.
- `consoleLevel` bumps to `debug` when `--verbose`.
- `maxFileBytes`: maximum active log file size in bytes before rotation (positive integer; default: `104857600` = 100 MB). Astroclaw keeps up to five numbered archives beside the active file.
...
- `stuckSessionAbortMs`: no-progress age threshold in ms before eligible stalled active work may be abort-drained for recovery. When unset, Astroclaw uses the safer extended embedded-run window of at least 5 minutes and 3x `stuckSessionWarnMs`.
...
- `ASTROCLAW_OTEL_PRELOADED=1`: environment toggle for hosts that already registered a global OpenTelemetry SDK. Astroclaw then skips plugin-owned SDK startup/shutdown while keeping diagnostic listeners active.
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$ASTROCLAW_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Astroclaw.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `ASTROCLAW_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `astroclaw doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `astroclaw.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
- Astroclaw-owned writes that change only one top-level section backed by a single-file include write through to that included file. For example, `plugins install` updates `plugins: { $include: "./plugins.json5" }` in `plugins.json5` and leaves `astroclaw.json` intact.
- Root includes, include arrays, and includes with sibling overrides are read-only for Astroclaw-owned writes; those writes fail closed instead of flattening the config.
