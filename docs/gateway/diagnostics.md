...
Astroclaw can create a local diagnostics zip for bug reports. It combines
...
2. Astroclaw sends the diagnostics preamble and asks for one explicit exec
   approval. The approval runs `astroclaw gateway diagnostics export --json`.
   Do not approve diagnostics through an allow-all rule.
3. After approval, Astroclaw replies with a pasteable report containing the local
   bundle path, manifest summary, privacy notes, and relevant session ids.

In group chats, an owner can still run `/diagnostics`, but Astroclaw does not
...
that the diagnostics flow was sent privately. If Astroclaw cannot find a private
owner route, the command fails closed and asks the owner to run it from a DM.

When the active Astroclaw session is using the native OpenAI Codex harness,
the same exec approval also covers an OpenAI feedback upload for the Codex
runtime threads Astroclaw knows about. That upload is separate from the local
...
the channels, Astroclaw session ids, Codex thread ids, and local resume commands
for the threads that were sent to OpenAI servers. If you deny or ignore the
approval, Astroclaw does not run the export, does not send Codex feedback, and
...
Persisted bundles live under `~/.astroclaw/logs/stability/` when events exist.

## Useful options

```bash
astroclaw gateway diagnostics export \
  --output astroclaw-diagnostics.zip \
  --log-lines 5000 \
  --log-bytes 1000000
```
