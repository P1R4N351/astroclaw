...
Astroclaw has three related (but different) controls:
...
Use the inspector to see what Astroclaw is _actually_ doing:
...
- Astroclaw validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:astroclaw`: all built-in Astroclaw tools (excludes provider plugins)
