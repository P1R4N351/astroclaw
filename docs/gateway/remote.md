...
- Use the macOS app's remote mode (Settings → General → Astroclaw runs).
...
- `astroclaw health` and `astroclaw status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `astroclaw gateway status`, `astroclaw gateway health`, `astroclaw gateway probe`, and `astroclaw gateway call` can also target the forwarded URL via `--url` when needed.

<Note>
Replace `18789` with your configured `gateway.port` (or `--port` or `ASTROCLAW_GATEWAY_PORT`).
</Note>
...
- URL override safety:
  ...
  - Env URL overrides (`ASTROCLAW_GATEWAY_URL`) may use env credentials only (`ASTROCLAW_GATEWAY_TOKEN` / `ASTROCLAW_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `ASTROCLAW_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `ASTROCLAW_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `ASTROCLAW_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `ASTROCLAW_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `ASTROCLAW_GATEWAY_*` only.
...
```bash
astroclaw config set gateway.remote.token "<your-token>"
```

#### Step 4: create the LaunchAgent

Save this as `~/Library/LaunchAgents/ai.astroclaw.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>ai.astroclaw.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/ai.astroclaw.ssh-tunnel.plist
```

The tunnel will start automatically at login, restart on crash, and keep the forwarded port live.

<Note>
If you have a leftover `com.astroclaw.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
</Note>
...
```bash
launchctl kickstart -k gui/$UID/ai.astroclaw.ssh-tunnel
```

Stop the tunnel:

```bash
launchctl bootout gui/$UID/ai.astroclaw.ssh-tunnel
```
