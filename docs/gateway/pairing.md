...
astroclaw nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
changes (for example, display name or client platform hints), Astroclaw treats
...
Pairing state is stored under the Gateway state directory (default `~/.astroclaw`):

- `~/.astroclaw/nodes/paired.json`
- `~/.astroclaw/nodes/pending.json`

If you override `ASTROCLAW_STATE_DIR`, the `nodes/` folder moves with it.
