---
summary: "Start local model servers on demand before Astroclaw model requests"
read_when:
  - You want Astroclaw to start a local model server only when its model is selected
...
`models.providers.<id>.localService` lets Astroclaw start a provider-owned local
model server on demand. It is provider-level config: when the selected model
belongs to that provider, Astroclaw probes the service, starts the process if the
...
2. If that provider has `localService`, Astroclaw probes `healthUrl`.
3. If the probe succeeds, Astroclaw uses the existing server.
4. If the probe fails, Astroclaw starts `command` with `args`.
5. Astroclaw polls readiness until `readyTimeoutMs` expires.
6. The model request is sent through the normal provider transport.
7. If Astroclaw started the process and `idleStopMs` is positive, the process is
   stopped after the last in-flight request has been idle for that long.

Astroclaw does not install launchd, systemd, Docker, or a daemon for this. The
server is a child process of the Astroclaw process that first needed it.
...
- `env`: optional environment variables merged over the Astroclaw process
  environment.
- `healthUrl`: readiness URL. If omitted, Astroclaw appends `/models` to
  `baseUrl`, so `http://127.0.0.1:8000/v1` becomes
  `http://127.0.0.1:8000/v1/models`.
- `readyTimeoutMs`: startup readiness deadline. Default: `120000`.
- `idleStopMs`: idle shutdown delay for Astroclaw-started processes. `0` or
  omitted keeps the process alive until Astroclaw exits.
...
- One Astroclaw process manages the child it started. Another Astroclaw process
...
<CardGroup cols={2}>
  ...
  <Card title="Inferrs" href="/providers/inferrs" icon="cpu">
    Run Astroclaw through the inferrs OpenAI-compatible local server.
  </Card>
</CardGroup>
