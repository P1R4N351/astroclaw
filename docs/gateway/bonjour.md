...
Astroclaw can use Bonjour (mDNS / DNS-SD) to discover an active Gateway (WebSocket endpoint).
...
2. Publish DNS-SD records for `_astroclaw-gw._tcp` under a dedicated zone
   (example: `astroclaw.internal.`).
3. Configure Tailscale **split DNS** so your chosen domain resolves via that
   DNS server for clients (including iOS).

Astroclaw supports any discovery domain; `astroclaw.internal.` is just an example.
...
```bash
astroclaw dns setup --apply
```

This installs CoreDNS and configures it to:

- listen on port 53 only on the gateway's Tailscale interfaces
- serve your chosen domain (example: `astroclaw.internal.`) from `~/.astroclaw/dns/<domain>.db`

Validate from a tailnet-connected machine:

```bash
dns-sd -B _astroclaw-gw._tcp astroclaw.internal.
dig @<TAILNET_IPV4> -p 53 _astroclaw-gw._tcp.astroclaw.internal PTR +short
```
...
`_astroclaw-gw._tcp` in your discovery domain without multicast.
...
- Set `gateway.bind: "tailnet"` in `~/.astroclaw/astroclaw.json`.
- Restart the Gateway (or restart the macOS menubar app).

## What advertises

Only the Gateway advertises `_astroclaw-gw._tcp`. LAN multicast advertising is
provided by the bundled `bonjour` plugin when the plugin is enabled; wide-area
DNS-SD publishing remains Gateway-owned.

## Service types

- `_astroclaw-gw._tcp` - gateway transport beacon (used by macOS/iOS/Android nodes).
...
- Browse instances:
  ...
  ```bash
  dns-sd -B _astroclaw-gw._tcp local.
  ```

- Resolve one instance (replace `<instance>`):

  ```bash
  dns-sd -L "<instance>" _astroclaw-gw._tcp local.
  ```
...
in-progress states. If the service never reaches `announced`, Astroclaw eventually
...
invalid DNS-label character, Astroclaw falls back to `astroclaw.local`. Set
`ASTROCLAW_MDNS_HOSTNAME=<name>` before starting the Gateway when you need an
explicit host label.

## Debugging on iOS node

The iOS node uses `NWBrowser` to discover `_astroclaw-gw._tcp`.
...
```bash
astroclaw plugins enable bonjour
```
...
discovery plugin for that Astroclaw config:

```bash
astroclaw plugins disable bonjour
```

## Docker gotchas

The bundled Bonjour plugin auto-disables LAN multicast advertising in detected
containers when `ASTROCLAW_DISABLE_BONJOUR` is unset. Docker bridge networks
...
- Disabling Bonjour does not change `gateway.bind`; Docker still defaults to
  `ASTROCLAW_GATEWAY_BIND=lan` so the published host port can work.
...
- Reusing the same `ASTROCLAW_CONFIG_DIR` outside Docker does not persist the
  container auto-disable policy.
- Set `ASTROCLAW_DISABLE_BONJOUR=0` only for host networking, macvlan, or another
...
1. Confirm whether the Gateway is running in auto, forced-on, or forced-off mode:
   ...
   ```bash
   docker compose config | grep ASTROCLAW_DISABLE_BONJOUR
   ```
...
4. If you deliberately enabled the Bonjour plugin in Docker and forced advertising
   with `ASTROCLAW_DISABLE_BONJOUR=0`, test multicast from the host:
   ...
   cancellations, restore `ASTROCLAW_DISABLE_BONJOUR=1` and use a direct or
   Tailnet route.
...
- **Advertiser stuck in probing/announcing**: hosts with blocked multicast,
  ...
  non-announced state. Astroclaw retries a few times and then disables Bonjour
...
- **Docker bridge networking**: Bonjour auto-disables in detected containers.
  Set `ASTROCLAW_DISABLE_BONJOUR=0` only for host, macvlan, or another
...
- `astroclaw plugins enable bonjour` enables the bundled LAN discovery plugin on hosts where it is not default-enabled.
- `astroclaw plugins disable bonjour` disables LAN multicast advertising by disabling the bundled plugin.
- `ASTROCLAW_DISABLE_BONJOUR=1` disables LAN multicast advertising without changing plugin config; accepted truthy values are `1`, `true`, `yes`, and `on` (legacy: `ASTROCLAW_DISABLE_BONJOUR`).
- `ASTROCLAW_DISABLE_BONJOUR=0` forces LAN multicast advertising on, including inside detected containers; accepted falsy values are `0`, `false`, `no`, and `off`.
- When the Bonjour plugin is enabled and `ASTROCLAW_DISABLE_BONJOUR` is unset, Bonjour advertises on normal hosts and auto-disables inside detected containers.
- `gateway.bind` in `~/.astroclaw/astroclaw.json` controls the Gateway bind mode.
- `ASTROCLAW_SSH_PORT` overrides the SSH port when `sshPort` is advertised (legacy: `ASTROCLAW_SSH_PORT`).
- `ASTROCLAW_TAILNET_DNS` publishes a MagicDNS hint in TXT when mDNS full mode is enabled (legacy: `ASTROCLAW_TAILNET_DNS`).
- `ASTROCLAW_CLI_PATH` overrides the advertised CLI path (legacy: `ASTROCLAW_CLI_PATH`).
