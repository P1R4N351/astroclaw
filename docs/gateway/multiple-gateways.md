---
summary: "Run multiple Astroclaw Gateways on one host (isolation, ports, and profiles)"
read_when:
...
astroclaw --profile rescue onboard
astroclaw --profile rescue gateway install --port 19789
```

If your main bot is already running, that is usually all you need.

During `astroclaw --profile rescue onboard`:
...
`astroclaw --profile rescue onboard` uses the normal onboarding flow, but it
...
- config file
- state directory
- workspace (by default `~/.astroclaw/workspace-rescue`)
...
# extra gateway
astroclaw --profile ops setup
astroclaw --profile ops gateway --port 19789
```

If you want both Gateways to use named profiles, that also works:

```bash
astroclaw --profile main setup
astroclaw --profile main gateway --port 18789
...
astroclaw --profile ops gateway install --port 19789
```
...
- `ASTROCLAW_CONFIG_PATH` — per-instance config file
- `ASTROCLAW_STATE_DIR` — per-instance sessions, creds, caches
...
Base port = `gateway.port` (or `ASTROCLAW_GATEWAY_PORT` / `--port`).
...
```bash
ASTROCLAW_CONFIG_PATH=~/.astroclaw/main.json \
ASTROCLAW_STATE_DIR=~/.astroclaw \
astroclaw gateway --port 18789

ASTROCLAW_CONFIG_PATH=~/.astroclaw/rescue.json \
ASTROCLAW_STATE_DIR=~/.astroclaw-rescue \
astroclaw gateway --port 19789
```
...
astroclaw --profile rescue gateway status --deep
astroclaw --profile rescue gateway probe
astroclaw status
astroclaw --profile rescue status
astroclaw --profile rescue browser status
```
