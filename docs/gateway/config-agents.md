...
Default: `ASTROCLAW_WORKSPACE_DIR` when set, otherwise `~/.astroclaw/workspace`.
...
`ASTROCLAW_WORKSPACE_DIR`. Use the environment variable to point default agents
at a mounted workspace when you do not want to write that path into config.

### `agents.defaults.repoRoot`

Optional repository root shown in the system prompt's Runtime line. If unset, Astroclaw auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/astroclaw" } },
}
```
...
Astroclaw has multiple high-volume prompt/context budgets, and they are
...
- `imageModel`: accepts either a string (`"provider/model"`) or an object (`{ primary, fallbacks }`).
  ...
  - Prefer explicit `provider/model` refs. Bare IDs are accepted for compatibility; if a bare ID uniquely matches a configured image-capable entry in `models.providers.*.models`, Astroclaw qualifies it to that provider. Ambiguous configured matches require an explicit provider prefix.
...
- `model.primary`: format `provider/model` (e.g. `openai/gpt-5.5` for OpenAI API-key or Codex OAuth access). If you omit the provider, Astroclaw tries an alias first, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider (deprecated compatibility behavior, so prefer explicit `provider/model`). If that provider no longer exposes the configured default model, Astroclaw falls back to the first configured provider/model instead of surfacing a stale removed-provider default.
- `models`: the configured model catalog and allowlist for `/model`. Each entry can include `alias` (shortcut) and `params` (provider-specific, for example `temperature`, `maxTokens`, `cacheRetention`, `context1m`, `responsesServerCompaction`, `responsesCompactThreshold`, `chat_template_kwargs`, `extra_body`/`extraBody`).
  ...
  - Safe edits: use `astroclaw config set agents.defaults.models '<json>' --strict-json --merge` to add entries. `config set` refuses replacements that would remove existing allowlist entries unless you pass `--replace`.
...
- `compat.thinkingFormat`: OpenAI-compatible thinking payload style. Use `"qwen"` for Qwen-style top-level `enable_thinking`, or `"qwen-chat-template"` for `chat_template_kwargs.enable_thinking` on Qwen-family backends that support request-level chat-template kwargs, such as vLLM. Astroclaw maps disabled thinking to `false` and enabled thinking to `true`.
- `compat.supportedReasoningEfforts`: per-model OpenAI-compatible reasoning effort list. Include `"xhigh"` for custom endpoints that truly accept it; Astroclaw then exposes `/think xhigh` in command menus, Gateway session rows, session patch validation, agent CLI validation, and `llm-task` validation for that configured provider/model. Use `compat.reasoningEffortMap` when the backend wants a provider-specific value for a canonical level.
- `params.preserveThinking`: Z.AI-only opt-in for preserved thinking. When enabled and thinking is on, Astroclaw sends `thinking.clear_thinking: false` and replays prior `reasoning_content`; see [Z.AI thinking and preserved thinking](/providers/zai#thinking-and-preserved-thinking).
- `localService`: optional provider-level process manager for local/self-hosted model servers. When the selected model belongs to that provider, Astroclaw probes `healthUrl` (or `baseUrl + "/models"`), starts `command` with `args` if the endpoint is down, waits up to `readyTimeoutMs`, then sends the model request. `command` must be an absolute path. `idleStopMs: 0` keeps the process alive until Astroclaw exits; a positive value stops the Astroclaw-spawned process after that many idle milliseconds. See [Local model services](/gateway/local-model-services).
...
- Whole-agent runtime keys are legacy. `agents.defaults.agentRuntime`, `agents.list[].agentRuntime`, session runtime pins, and `ASTROCLAW_AGENT_RUNTIME` are ignored by runtime selection. Run `astroclaw doctor --fix` to remove stale values.
...
- `reseedFromRawTranscriptWhenUncompacted: true` lets a backend recover safe
  invalidated sessions from a bounded raw Astroclaw transcript tail before the
  first compaction summary exists. Auth profile or credential-epoch changes
  still never raw-reseed.

### `agents.defaults.systemPromptOverride`

Replace the entire Astroclaw-assembled system prompt with a fixed string. Set at the default level (`agents.defaults.systemPromptOverride`) or per agent (`agents.list[].systemPromptOverride`). Per-agent values take precedence; an empty or whitespace-only value is ignored. Useful for controlled prompt experiments.
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Astroclaw aborts it. Default: `900`.
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Astroclaw materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.astroclaw/sandboxes`
...
In `remote` mode, host-local edits made outside Astroclaw are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `astroclaw.json`.
noVNC observer access uses VNC auth by default and Astroclaw emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `astroclaw-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `ASTROCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `ASTROCLAW_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `ASTROCLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        agentDir: "~/.astroclaw/agents/main/agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.astroclaw/workspace-home" },
      { id: "work", workspace: "~/.astroclaw/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, Astroclaw resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
- **`maintenance`**: session-store cleanup + retention controls.
  ...
  - `maxEntries`: maximum number of entries in `sessions.json` (default `500`). Runtime writes batch cleanup with a small high-water buffer for production-sized caps; `astroclaw sessions cleanup --enforce` applies the cap immediately.
  - `rotateBytes`: deprecated and ignored; `astroclaw doctor --fix` removes it from older configs.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.astroclaw/settings/tts.json",
      providers: {
        elevenlabs: {
...
- When `providers.openai.baseUrl` points to a non-OpenAI endpoint, Astroclaw treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
- Legacy flat Talk keys (`talk.voiceId`, `talk.voiceAliases`, `talk.modelId`, `talk.outputFormat`, `talk.apiKey`) are compatibility-only. Run `astroclaw doctor --fix` to rewrite persisted config into `talk.providers.<provider>`.
...
- macOS MLX playback runs through the bundled `astroclaw-mlx-tts` helper when present, or an executable on `PATH`; `ASTROCLAW_MLX_TTS_BIN` overrides the helper path for development.
- `consultThinkingLevel` controls the thinking level for the full Astroclaw agent run behind Control UI Talk realtime `astroclaw_agent_consult` calls. Leave unset to preserve normal session/model behavior.
...
- `realtime.instructions` appends provider-facing system instructions to Astroclaw's built-in realtime prompt, so voice style can be configured without losing default `astroclaw_agent_consult` guidance.
