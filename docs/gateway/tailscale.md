...
Astroclaw can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Astroclaw requires a shared password.
- `off`: Default (no Tailscale automation).

Status and audit output use **Tailscale exposure** for this Astroclaw Serve/Funnel
mode. `off` means Astroclaw is not managing Serve or Funnel; it does not mean the
...
- `token` (default when `ASTROCLAW_GATEWAY_TOKEN` is set)
- `password` (shared secret via `ASTROCLAW_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. Astroclaw verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Astroclaw only treats a request as Serve when it arrives from loopback with
...
Prefer `ASTROCLAW_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
astroclaw gateway --tailscale serve
astroclaw gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Astroclaw to undo `tailscale serve`
...
- Set `gateway.tailscale.preserveFunnel: true` to keep an externally configured
  ...
  gateway runs in `mode: "serve"`, Astroclaw checks `tailscale funnel status`
  before re-applying Serve and skips it when a Funnel route already covers the
  gateway port. The Astroclaw-managed Funnel password-only policy is unchanged.
