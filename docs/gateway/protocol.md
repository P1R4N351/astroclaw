...
Astroclaw. All clients (CLI, web UI, macOS app, iOS/Android nodes, headless
...
<AccordionGroup>
  ...
  <Accordion title="Talk and TTS">
    ...
    - `talk.client.toolCall` lets client-owned realtime transports forward provider tool calls to Gateway policy. The first supported tool is `astroclaw_agent_consult`; clients receive a run id and wait for normal chat lifecycle events before submitting the provider-specific tool result.
...
- Operators may call `skills.install` (`operator.admin`) in three modes:
  ...
  - Gateway installer mode: `{ name, installId, dangerouslyForceUnsafeInstall?, timeoutMs? }`
    runs a declared `metadata.astroclaw.install` action on the gateway host.
...
- Final `agent` results may include `result.deliveryStatus` when delivery was
  ...
  statuses documented for [`astroclaw agent --json --deliver`](/cli/agent#json-delivery-status).
