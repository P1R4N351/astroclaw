...
Astroclaw's Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`astroclaw agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-astroclaw-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-astroclaw-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "astroclaw"`, `model: "astroclaw/default"`, `model: "astroclaw/<agentId>"`, or `x-astroclaw-agent-id`
- use `x-astroclaw-model` when you want to override the selected agent's backend model
- use `x-astroclaw-session-key` for explicit session routing
- use `x-astroclaw-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: Astroclaw reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
Astroclaw normalizes common OpenAI-style aliases before those counters reach
