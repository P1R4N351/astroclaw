...
Astroclaw's Gateway can serve a small OpenAI-compatible Chat Completions endpoint.
...
Under the hood, requests are executed as a normal Gateway agent run (same codepath as `astroclaw agent`), so routing/permissions/config match your Gateway.
...
- When `gateway.auth.mode="token"`, use `gateway.auth.token` (or `ASTROCLAW_GATEWAY_TOKEN`).
- When `gateway.auth.mode="password"`, use `gateway.auth.password` (or `ASTROCLAW_GATEWAY_PASSWORD`).
...
- There is no separate non-owner/per-user tool boundary on this endpoint; once a caller passes Gateway auth here, Astroclaw treats that caller as a trusted operator for this gateway.
- For shared-secret auth modes (`token` and `password`), the endpoint restores the normal full operator defaults even if the caller sends a narrower `x-astroclaw-scopes` header.
- Trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`) honor `x-astroclaw-scopes` when present and otherwise fall back to the normal operator default scope set.
...
- `gateway.auth.mode="token"` or `"password"` + `Authorization: Bearer ...`
  ...
  - ignores narrower `x-astroclaw-scopes`
...
- trusted identity-bearing HTTP modes (for example trusted proxy auth, or `gateway.auth.mode="none"` on private ingress)
  ...
  - honor `x-astroclaw-scopes` when the header is present
...
Astroclaw treats the OpenAI `model` field as an **agent target**, not a raw provider model id.

- `model: "astroclaw"` routes to the configured default agent.
- `model: "astroclaw/default"` also routes to the configured default agent.
- `model: "astroclaw/<agentId>"` routes to a specific agent.

Optional request headers:

- `x-astroclaw-model: <provider/model-or-bare-id>` overrides the backend model for the selected agent.
- `x-astroclaw-agent-id: <agentId>` remains supported as a compatibility override.
- `x-astroclaw-session-key: <sessionKey>` fully controls session routing.
- `x-astroclaw-message-channel: <channel>` sets the synthetic ingress channel context for channel-aware prompts and policies.

Compatibility aliases still accepted:

- `model: "astroclaw:<agentId>"`
- `model: "agent:<agentId>"`
...
<AccordionGroup>
  <Accordion title="What does `/v1/models` return?">
    An Astroclaw agent-target list.

    The returned ids are `astroclaw`, `astroclaw/default`, and `astroclaw/<agentId>` entries.
  ...
  <Accordion title="Why is `astroclaw/default` included?">
    `astroclaw/default` is the stable alias for the configured default agent.
  ...
  <Accordion title="How do I override the backend model?">
    Use `x-astroclaw-model`.

    Examples:
    `x-astroclaw-model: openai/gpt-5.4`
    `x-astroclaw-model: gpt-5.5`
  ...
  <Accordion title="How do embeddings fit this contract?">
    ...
    Use `model: "astroclaw/default"` or `model: "astroclaw/<agentId>"`.
    When you need a specific embedding model, send it in `x-astroclaw-model`.
...
- Model: `astroclaw/default`

Expected behavior:

- `GET /v1/models` should list `astroclaw/default`
- Open WebUI should use `astroclaw/default` as the chat model id
- If you want a specific backend provider/model for that agent, set the agent's normal default model or send `x-astroclaw-model`
...
If that returns `astroclaw/default`, most Open WebUI setups can connect with the same base URL and token.
...
curl -N http://127.0.0.1:18789/v1/chat/completions \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-astroclaw-model: openai/gpt-5.4' \
  -d '{
    "model": "astroclaw/research",
    "stream": true,
    "messages": [{"role":"user","content":"hi"}]
  }'
```
...
```bash
curl -sS http://127.0.0.1:18789/v1/models/astroclaw%2Fdefault \
  -H 'Authorization: Bearer YOUR_TOKEN'
```
...
curl -sS http://127.0.0.1:18789/v1/embeddings \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-astroclaw-model: openai/text-embedding-3-small' \
  -d '{
    "model": "astroclaw/default",
    "input": ["alpha", "beta"]
  }'
...
- `/v1/models` returns Astroclaw agent targets, not raw provider catalogs.
- `astroclaw/default` is always present so one stable id works across environments.
- Backend provider/model overrides belong in `x-astroclaw-model`, not the OpenAI `model` field.
