...
Astroclaw has two distinct problems that look similar on the surface:

1. **Operator remote control**: the macOS menu bar app controlling a gateway running elsewhere.
2. **Node pairing**: iOS/Android (and future nodes) finding a gateway and pairing securely.

The design goal is to keep all network discovery/advertising in the **Node Gateway** (`astroclaw gateway`) and keep clients (mac app, iOS) as consumers.
...
Multicast Bonjour is best-effort and does not cross networks. Astroclaw can also browse the
...
- Service types:
  - `_astroclaw-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  - `transport=gateway`
...
- `astroclaw plugins enable bonjour` enables LAN multicast advertising.
- `ASTROCLAW_DISABLE_BONJOUR=1` disables advertising.
- When the Bonjour plugin is enabled and `ASTROCLAW_DISABLE_BONJOUR` is unset,
...
- `ASTROCLAW_SSH_PORT` overrides the SSH port advertised when `sshPort` is emitted.
- `ASTROCLAW_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS).
- `ASTROCLAW_CLI_PATH` overrides the advertised CLI path.
