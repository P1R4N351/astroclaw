...
- `astroclaw gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `astroclaw doctor` reports no blocking config/service issues.
- `astroclaw channels status --probe` shows live per-account transport status and, where supported, probe/audit results such as `works` or `audit ok`.
...
- `Update restart` in `astroclaw status` / `astroclaw status --all`. Pending or
  failed handoffs include the next command to run.
- `plugin load failed: dependency tree corrupted; run astroclaw doctor --fix`
  under Channels. That means the channel config still exists, but plugin
  registration failed before the channel could load.
- provider 401s after re-auth. `astroclaw doctor --fix` checks for stale
...
Use this when a gateway service unexpectedly stops after an update, or logs show that one `astroclaw` binary is older than the version that last wrote `astroclaw.json`.

Astroclaw stamps config writes with `meta.lastTouchedVersion`. Read-only commands can still inspect a config written by a newer Astroclaw, but process and service mutations refuse to continue from an older binary. Blocked actions include gateway service start, stop, restart, uninstall, forced service reinstall, service-mode gateway startup, and `gateway --force` port cleanup.

```bash
which astroclaw
astroclaw --version
astroclaw gateway status --deep
astroclaw config get meta.lastTouchedVersion
```

<Steps>
  <Step title="Fix PATH">
    Fix `PATH` so `astroclaw` resolves to the newer install, then rerun the action.
  </Step>
  ...
  <Step title="Remove stale wrappers">
    Remove stale system package or old wrapper entries that still point at an old `astroclaw` binary.
  </Step>
</Steps>

<Warning>
For intentional downgrade or emergency recovery only, set `ASTROCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1` for the single command. Leave it unset for normal operation.
</Warning>
...
Astroclaw treats every skill root as a containment boundary. A symlink under
`~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills`, or
`~/.astroclaw/skills` is skipped when its real target resolves outside that root
...
astroclaw config get skills.load
```
...
astroclaw config get agents.defaults.models
```
...
- Astroclaw model runs fail only on normal agent turns
...
astroclaw infer model run --model <provider/model> --prompt "hi" --json
astroclaw logs --follow
```

Look for:

- direct tiny calls succeed, but Astroclaw runs fail only on larger prompts
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0` → the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. Astroclaw retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - direct tiny requests succeed, but Astroclaw agent runs fail with backend/model crashes (for example Gemma on some `inferrs` builds) → Astroclaw transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    3. Set `compat.supportsTools: false` for models/backends that cannot handle Astroclaw's tool schema surface reliably.
    ...
    5. If tiny direct requests keep passing while Astroclaw agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
astroclaw status
astroclaw channels status --probe
astroclaw pairing list --channel <channel> [--account <id>]
astroclaw config get channels
astroclaw logs --follow
```
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | Paste/set token in the client and retry. For dashboard paths: `astroclaw config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity needs approval. Check `error.details.reason` for `not-paired`, `scope-upgrade`, `role-upgrade`, or `metadata-upgrade`, and use `requestId` / `remediationHint` when present. | Approve pending request: `astroclaw devices list` then `astroclaw devices approve <requestId>`. Scope/role upgrades use the same flow after you review the requested access.                                                                                                               |
...
If `astroclaw devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the caller also has `operator.admin`
- `astroclaw devices rotate --scope ...` can only request operator scopes that the caller session already holds
...
astroclaw logs --follow
astroclaw doctor
astroclaw gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `astroclaw onboard --mode local` / `astroclaw setup` to restamp the expected local-mode config. If you are running Astroclaw via Podman, the default config path is `~/.astroclaw/astroclaw.json`.
    ...
    - `System-level Astroclaw gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `ASTROCLAW_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `astroclaw doctor --fix` or `astroclaw gateway install --force`, then restart the gateway service.
...
astroclaw gateway status --deep
astroclaw logs --follow
astroclaw gateway stability --bundle latest
astroclaw gateway diagnostics export
```
...
- `critical memory pressure bundle written` appears shortly before restart → Astroclaw captured a pre-OOM stability bundle. Inspect it with `astroclaw gateway stability --bundle latest`.
- `memory pressure: level=critical ... memoryPressureSnapshot=disabled` appears in gateway logs → Astroclaw detected critical memory pressure, but the pre-OOM stability snapshot is off.
...
- A timestamped `astroclaw.json.rejected.*` file beside the active config
- A timestamped `astroclaw.json.clobbered.*` file if `doctor --fix` repaired a broken direct edit
- Astroclaw keeps the latest 32 `.clobbered.*` files for each config path and rotates older ones

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or an Astroclaw-owned write.
    - Gateway startup fails closed instead of rewriting `astroclaw.json`.
    - Hot reload skips invalid external edits and keeps the current runtime config active.
    - Astroclaw-owned writes reject invalid/destructive payloads before commit and save `.rejected.*`.
    - `astroclaw doctor --fix` owns repair. It can remove non-JSON prefixes or restore the last-known-good copy while preserving the rejected payload as `.clobbered.*`.
    - When many repairs happen for one config path, Astroclaw rotates older `.clobbered.*` files so the newest repaired payload is still available.

  </Accordion>
  <Accordion title="Inspect and repair">
    ```bash
    CONFIG="$(astroclaw config file)"
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an Astroclaw-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → an Astroclaw-owned write was rejected because it lost fields or size compared with the last-known-good backup.
    - `Config last-known-good promotion skipped` → the candidate contained redacted secret placeholders such as `***`.

  </Accordion>
  <Accordion title="Fix options">
    1. Run `astroclaw doctor --fix` to let doctor repair prefixed/clobbered config or restore last-known-good.
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `astroclaw config set` or `config.patch`.
    3. Run `astroclaw config validate` before restarting.
...
Use this when `astroclaw gateway probe` reaches something, but still prints a warning block.

```bash
astroclaw gateway probe
astroclaw gateway probe --json
astroclaw gateway probe --ssh user@gateway-host
```
...
```bash
astroclaw cron status
astroclaw cron list
astroclaw cron runs --id <jobId> --limit 20
astroclaw system heartbeat last
astroclaw logs --follow
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so Astroclaw skips the model call.
...
astroclaw nodes describe --node <idOrNameOrIp>
astroclaw approvals get --node <idOrNameOrIp>
astroclaw logs --follow
astroclaw status
```
...
astroclaw browser start --browser-profile astroclaw
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `astroclaw browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (Astroclaw-managed)](/tools/browser)
...
<AccordionGroup>
  <Accordion title="1. Auth and URL override behavior changed">
    ```bash
    astroclaw gateway status
    astroclaw config get gateway.mode
    astroclaw config get gateway.remote.url
    astroclaw config get gateway.auth.mode
    ```
  ...
  <Accordion title="2. Bind and auth guardrails are stricter">
    ```bash
    astroclaw config get gateway.bind
    astroclaw config get gateway.auth.mode
    astroclaw config get gateway.auth.token
    astroclaw gateway status
    astroclaw logs --follow
    ```
