...
Astroclaw has two log "surfaces":
...
- Default rolling log file is under `/tmp/astroclaw/` (one file per day): `astroclaw-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.astroclaw/astroclaw.json`:
  - `logging.file`
  - `logging.level`
...
Astroclaw can mask sensitive tokens before log or transcript output leaves the
...
`astroclaw gateway` supports a per-gateway style switch:
