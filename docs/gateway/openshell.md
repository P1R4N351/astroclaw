---
summary: "Use OpenShell as a managed sandbox backend for Astroclaw agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for Astroclaw. Instead of running Docker
containers locally, Astroclaw delegates sandbox lifecycle to the `openshell` CLI,
...
- OpenShell plugin installed (`astroclaw plugins install @astroclaw/openshell-sandbox`)
- The `openshell` CLI installed and on `PATH` (or set a custom path via
  `plugins.entries.openshell.config.command`)
- An OpenShell account with sandbox access
- Astroclaw Gateway running on the host
...
```bash
astroclaw plugins install @astroclaw/openshell-sandbox
```
...
2. Restart the Gateway. On the next agent turn, Astroclaw creates an OpenShell
...
- You edit files locally outside Astroclaw and want those changes visible in the
...
- When the sandbox is first created, Astroclaw seeds the remote workspace from
...
- Astroclaw does **not** sync remote changes back into the local workspace.
...
<Warning>
If you edit files on the host outside Astroclaw after the initial seed, the remote sandbox does **not** see those changes. Use `astroclaw sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"astroclaw"`  | Sandbox source for first-time create                  |
...
1. Astroclaw calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Astroclaw calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `astroclaw sandbox` commands
