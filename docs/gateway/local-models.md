---
summary: "Run Astroclaw on local LLMs (LM Studio, vLLM, LiteLLM, custom OpenAI endpoints)"
read_when:
...
| LiteLLM / OAI-proxy / custom OpenAI-compatible proxy | You front another model API and need Astroclaw to treat it as OpenAI         |
...
If `api` is omitted on a custom provider with a `baseUrl`, Astroclaw defaults to
...
- hidden Astroclaw attribution headers (`originator`, `version`, `User-Agent`)
...
- Some local models emit standalone bracketed tool requests as text, such as
  `[tool_name]` followed by JSON and `[END_TOOL_REQUEST]`. Astroclaw promotes
...
- If a model emits JSON, XML, or ReAct-style text that looks like a tool call
  but the provider did not emit a structured invocation, Astroclaw leaves it as
...
- If tools appear as assistant text instead of running, for example raw JSON,
  ...
  It overrides Astroclaw's default proxy value of `tool_choice: "auto"`.
  Replace `local/my-local-model` with the exact provider/model ref shown by
  `astroclaw models list`.

  ```bash
  astroclaw config set agents.defaults.models '{"local/my-local-model":{"params":{"extra_body":{"tool_choice":"required"}}}}' --strict-json --merge
  ```
...
1. **Confirm the local model itself responds.** No tools, no agent context:
   ...
   ```bash
   astroclaw infer model run --local --model <provider/model> --prompt "Reply with exactly: pong" --json
   ```
...
2. **Confirm Gateway routing.** Sends only the supplied prompt — skips transcript, AGENTS bootstrap, context-engine assembly, tools, and bundled MCP servers, but still exercises Gateway routing, auth, and provider selection:
   ...
   ```bash
   astroclaw infer model run --gateway --model <provider/model> --prompt "Reply with exactly: pong" --json
   ```
...
5. **Past that, the bottleneck is upstream.** If the backend still fails only on larger Astroclaw runs after lean mode and `supportsTools: false`, the remaining issue is usually upstream model or server capacity — context window, GPU memory, kv-cache eviction, or a backend bug. It is not Astroclaw's transport layer at that point.
...
- Local server says `terminated`, `ECONNRESET`, or closes the stream mid-turn?
  Astroclaw records a low-cardinality `model.call.error.failureKind` plus the
  Astroclaw process RSS/heap snapshot in diagnostics. For LM Studio/Ollama
...
- Direct tiny `/v1/chat/completions` calls work, but `astroclaw infer model run --local`
