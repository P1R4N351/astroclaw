...
<Warning>
  **Personal assistant trust model.** This guidance assumes one trusted
  ...
  Astroclaw is **not** a hostile multi-tenant security boundary for multiple
...
Astroclaw security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `astroclaw security audit`
...
```bash
astroclaw security audit
astroclaw security audit --deep
astroclaw security audit --fix
astroclaw security audit --json
```
...
Astroclaw is both a product and an experiment: you're wiring frontier-model behavior into real messaging surfaces and real tools. **There is no "perfectly secure" setup.** The goal is to be deliberate about:
...
Astroclaw assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.astroclaw`, including `astroclaw.json`), treat them as a trusted operator.
...
- Astroclaw's product default for trusted single-operator setups is that host exec on `gateway`/`node` is allowed without approval prompts (`security="full"`, `ask="off"` unless you tighten it). That default is intentional UX, not a vulnerability by itself.
...
Astroclaw separates two concepts:
...
If you run `--deep`, Astroclaw also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.astroclaw/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile` (regular file only; symlinks rejected)
- **Discord bot token**: config/env or SecretRef (env/file/exec providers)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**:
  - `~/.astroclaw/credentials/<channel>-allowFrom.json` (default account)
  - `~/.astroclaw/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
- **Model auth profiles**: `~/.astroclaw/agents/<agentId>/agent/auth-profiles.json`
- **Codex runtime state**: `~/.astroclaw/agents/<agentId>/agent/codex-home/`
- **File-backed secrets payload (optional)**: `~/.astroclaw/secrets.json`
- **Legacy OAuth import**: `~/.astroclaw/credentials/oauth.json`
...
`astroclaw security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`astroclaw security audit` raises `config.insecure_or_dangerous_flags` when
...
- Astroclaw gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from Astroclaw responses.
...
Astroclaw stores session transcripts on disk under `~/.astroclaw/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.astroclaw` (see the audit section below). If you need
...
- Approval mode binds exact request context and, when possible, one concrete local script/file operand. If Astroclaw cannot identify exactly one direct local file for an interpreter/runtime command, approval-backed execution is denied rather than promising full semantic coverage.
...
Astroclaw can refresh the skills list mid-session:
...
- If you install or update plugins (`astroclaw plugins install <package>`, `astroclaw plugins update <id>`), treat it like running untrusted code:
  - The install path is the per-plugin directory under the active plugin install root.
  - Astroclaw runs a built-in dangerous-code scan before install/update. `critical` findings block by default.
  - npm and git plugin installs run package-manager dependency convergence only during the explicit install/update flow. Local paths and archives are treated as self-contained plugin packages; Astroclaw copies/references them without running `npm install`.
  ...
  - Gateway-backed skill dependency installs follow the same dangerous/suspicious split: built-in `critical` findings block unless the caller explicitly sets `dangerouslyForceUnsafeInstall`, while suspicious findings still warn only. `astroclaw skills install` remains the separate ClawHub skill download/install flow.
...
```bash
astroclaw pairing list <channel>
...
By default, Astroclaw routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Astroclaw has two separate "who can trigger me?" layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.astroclaw/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- "Paste the full contents of ~/.astroclaw or your logs."

## External content special-token sanitization

Astroclaw strips common self-hosted LLM chat-template special-token literals from wrapped external content and metadata before they reach the model. Covered marker families include Qwen/ChatML, Llama, Gemma, Mistral, Phi, and GPT-OSS role/turn tokens.
...
Astroclaw includes explicit bypass flags that disable external-content safety wrapping:
...
Astroclaw strips common model-family special-token literals from wrapped
...
- `~/.astroclaw/astroclaw.json`: `600` (user read/write only)
- `~/.astroclaw`: `700` (user only)

`astroclaw doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `ASTROCLAW_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__astroclaw__/canvas/` and `/__astroclaw__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
If you run Astroclaw with Docker on a VPS, remember that published container ports
...
When the bundled `bonjour` plugin is enabled, the Gateway broadcasts its presence via mDNS (`_astroclaw-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
5. **Environment variable** (alternative): set `ASTROCLAW_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `astroclaw doctor --generate-gateway-token`.
...
`ASTROCLAW_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
This is intentionally process environment only, not an `astroclaw.json` config
key.
...
- Astroclaw also has a narrow backend/container-local self-connect path for
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `ASTROCLAW_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `ASTROCLAW_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Astroclaw
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. Astroclaw verifies the identity by resolving the
...
- On the OpenAI-compatible HTTP surface, shared-secret bearer auth restores the full default operator scopes (`operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`) and owner semantics for agent turns; narrower `x-astroclaw-scopes` values do not reduce that shared-secret path.
...
- In those identity-bearing modes, omitting `x-astroclaw-scopes` falls back to the normal operator default scope set; send the header explicitly when you want a narrower scope set.
...
- Astroclaw will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.astroclaw/` (or `$ASTROCLAW_STATE_DIR/`) may contain secrets or private data:

- `astroclaw.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
Astroclaw loads workspace-local `.env` files for agents and tools, but never lets those files silently override gateway runtime controls.

- Any key that starts with `ASTROCLAW_*` is blocked from untrusted workspace `.env` files.
...
Why: workspace `.env` files frequently live next to agent code, get committed by accident, or get written by tools. Blocking the whole `ASTROCLAW_*` prefix means adding a new `ASTROCLAW_*` flag later can never regress into silent inheritance from workspace state.
...
- When sharing diagnostics, prefer `astroclaw status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@astroclaw", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.astroclaw`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.astroclaw/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `astroclaw` profile).
...
Astroclaw's browser navigation policy is strict by default: private/internal destinations stay blocked unless you explicitly opt in.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.astroclaw/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.astroclaw/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.astroclaw/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "none",
        },
        // Session tools can reveal sensitive data from transcripts. By default Astroclaw limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `astroclaw gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `ASTROCLAW_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/astroclaw/astroclaw-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.astroclaw/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `astroclaw security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + Astroclaw version
...
Found a vulnerability in Astroclaw? Please report responsibly:

1. Email: [security@astroclaw.ai](mailto:security@astroclaw.ai)
