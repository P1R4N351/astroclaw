...
```bash
ASTROCLAW_DIAGNOSTICS=telegram.http,telegram.payload
```

Disable all flags:

```bash
ASTROCLAW_DIAGNOSTICS=0
```
...
ASTROCLAW_DIAGNOSTICS_TIMELINE_PATH=/tmp/astroclaw-timeline.jsonl \
...
`ASTROCLAW_DIAGNOSTICS_TIMELINE_PATH`. When `timeline` is enabled only from
config, the earliest config-loading spans are not emitted because Astroclaw has
not read config yet; subsequent startup spans use the config flag.

`ASTROCLAW_DIAGNOSTICS=1`, `ASTROCLAW_DIAGNOSTICS=all`, and
`ASTROCLAW_DIAGNOSTICS=*` also enable the timeline because they enable every
diagnostics flag. Prefer `timeline` when you only want the JSONL timing
artifact.

Timeline records use the `astroclaw.diagnostics.v1` envelope. Events can include
...
```bash
ls -t /tmp/astroclaw/astroclaw-*.log | head -n 1
```

Filter for Telegram HTTP diagnostics:

```bash
rg "telegram http error" /tmp/astroclaw/astroclaw-*.log
```

Filter for Brave Search HTTP diagnostics:

```bash
rg "brave http" /tmp/astroclaw/astroclaw-*.log
```

Or tail while reproducing:

```bash
tail -f /tmp/astroclaw/astroclaw-$(date +%F).log | rg "telegram http error"
```

For remote gateways, you can also use `astroclaw logs --follow` (see [/cli/logs](/cli/logs)).
