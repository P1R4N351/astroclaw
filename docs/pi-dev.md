...
A sane workflow for working on the Pi integration in Astroclaw.
...
```bash
ASTROCLAW_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm astroclaw agent --message "Hello" --thinking low`
...
State lives under the Astroclaw state directory. Default is `~/.astroclaw`. If `ASTROCLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `astroclaw.json` for config
