...
<Note>
The official Android app is available on [Google Play](https://play.google.com/store/apps/details?id=ai.astroclaw.app&hl=en_IN). It is a companion node and requires a running Astroclaw Gateway. The source code is also available in the [Astroclaw repository](https://github.com/astroclaw/astroclaw) under `apps/android`; see [apps/android/README.md](https://github.com/astroclaw/astroclaw/blob/main/apps/android/README.md) for build instructions.
</Note>
...
- Install: [Google Play](https://play.google.com/store/apps/details?id=ai.astroclaw.app&hl=en_IN) for the app, [Getting Started](/start/getting-started) for the Gateway, then [Pairing](/channels/pairing).
...
- You can run the CLI (`astroclaw`) on the gateway machine (or via SSH).
...
```bash
astroclaw gateway discover --json
```
...
1. Set up a DNS-SD zone (example `astroclaw.internal.`) on the gateway host and publish `_astroclaw-gw._tcp` records.
...
1. Create `~/.astroclaw/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
astroclaw nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__astroclaw__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__astroclaw__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The A2UI host lives at `http://<gateway-host>:18789/__astroclaw__/a2ui/`.
...
Android supports launching Astroclaw from the system assistant trigger (Google
Assistant). When configured, holding the home button or saying "Hey Google, ask
Astroclaw..." opens the app and hands the prompt into the chat composer.
...
and whether the user has set Astroclaw as the default assistant app.
</Note>
