...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `astroclaw.internal.`), **or**
...
Run `astroclaw devices list` again before approval.
...
astroclaw gateway call node.list --params "{}"
```
...
- `ASTROCLAW_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Astroclaw iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Astroclaw builds.
...
```bash
export ASTROCLAW_APNS_TEAM_ID="TEAMID"
export ASTROCLAW_APNS_KEY_ID="KEYID"
export ASTROCLAW_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.astroclaw/credentials/apns
chmod 700 ~/.astroclaw/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.astroclaw/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.astroclaw/credentials/apns/AuthKey_KEYID.p8
export ASTROCLAW_APNS_PRIVATE_KEY_PATH="$HOME/.astroclaw/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_astroclaw-gw._tcp` on `local.` and, when configured, the same
...
`astroclaw.internal.`) and Tailscale split DNS.
...
```bash
astroclaw nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__astroclaw__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__astroclaw__/canvas/` and `/__astroclaw__/a2ui/`.
...
tools; the iOS app exposes iPhone capabilities through Astroclaw node commands
such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through Astroclaw by invoking node
...
```bash
astroclaw nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__astroclaw; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
astroclaw nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `astroclaw devices list` and approve manually.
