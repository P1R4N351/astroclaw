...
read_when:
  - Installing Astroclaw on Windows
  - Choosing between native Windows and WSL2
  - Looking for Windows companion app status
title: "Windows"
---

Astroclaw supports both **native Windows** and **WSL2**. WSL2 is the more
...
- local CLI use such as `astroclaw --version`, `astroclaw doctor`, and `astroclaw plugins list --json`
- embedded local-agent/provider smoke such as:

```powershell
astroclaw agent --local --agent main --thinking low -m "Reply with exactly WINDOWS-HATCH-OK."
```

Current caveats:

- `astroclaw onboard --non-interactive` still expects a reachable local gateway unless you pass `--skip-health`
- `astroclaw onboard --non-interactive --install-daemon` and `astroclaw gateway install` try Windows Scheduled Tasks first
- if Scheduled Task creation is denied, Astroclaw falls back to a per-user Startup-folder login item and starts the gateway immediately
- if `schtasks` itself wedges or stops responding, Astroclaw now aborts that path quickly and falls back instead of hanging forever
- Scheduled Tasks are still preferred when available because they provide better supervisor status

If you want the native CLI only, without gateway service install, use one of these:

```powershell
astroclaw onboard --non-interactive --skip-health
...
### 2) Install the Astroclaw gateway user service
...
```bash
systemctl --user is-enabled astroclaw-gateway.service
systemctl --user status astroclaw-gateway.service --no-pager
```
...
- Remote nodes must point at a **reachable** Gateway URL (not `127.0.0.1`); use
  `astroclaw status --all` to confirm.
...
### 3) Install Astroclaw (inside WSL)
...
pnpm build
pnpm ui:build
pnpm astroclaw onboard --install-daemon
```
