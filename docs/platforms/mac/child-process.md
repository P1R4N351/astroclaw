...
service via the external `astroclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.astroclaw.gateway`
  (or `ai.astroclaw.<profile>` when using `--profile`/`ASTROCLAW_PROFILE`; legacy `com.astroclaw.*` is supported).
