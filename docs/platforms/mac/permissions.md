...
- Same path: run the app from a fixed location (for Astroclaw, `dist/Astroclaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.astroclaw.mac
sudo tccutil reset ScreenCapture ai.astroclaw.mac
...
Workaround: move files into the Astroclaw workspace (`~/.astroclaw/workspace`) if you want to avoid per-folder grants.
