...
read_when:
  - Hosting PeekabooBridge in Astroclaw.app
...
Astroclaw can host **PeekabooBridge** as a local, permission-aware UI automation
broker. This lets the `peekaboo` CLI drive UI automation while reusing the
macOS app's TCC permissions.

## What this is (and is not)

- **Host**: Astroclaw.app can act as a PeekabooBridge host.
- **Client**: use the `peekaboo` CLI (no separate `astroclaw ui ...` surface).
- **UI**: visual overlays stay in Peekaboo.app; Astroclaw is a thin broker host.

## Relationship to Computer Use

Astroclaw has three desktop-control paths, and they intentionally stay separate:

- **PeekabooBridge host**: Astroclaw.app can host the local PeekabooBridge socket.
  The `peekaboo` CLI remains the client and uses Astroclaw.app's macOS
...
- **Codex Computer Use**: the bundled `codex` plugin prepares Codex app-server,
  ...
  Codex own native desktop-control tool calls during Codex-mode turns. Astroclaw
  does not proxy those actions through PeekabooBridge.
- **Direct `cua-driver` MCP**: Astroclaw can register TryCua's upstream
...
Use Peekaboo when you want the broad macOS automation surface and Astroclaw.app's
...
when you want the CUA driver exposed to any Astroclaw-managed runtime as a normal
MCP server.
...
When enabled, Astroclaw starts a local UNIX socket server. If disabled, the host
...
- The bridge validates **caller code signatures**; an allowlist of TeamIDs is
  enforced (Peekaboo host TeamID + Astroclaw app TeamID).
...
- If `peekaboo` reports "bridge client is not authorized", ensure the client is
  ...
  in **debug** mode only.
- If no hosts are found, open one of the host apps (Peekaboo.app or Astroclaw.app)
