...
2. `npm i -g astroclaw@latest`
3. `astroclaw onboard --install-daemon`
...
Astroclaw installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `astroclaw gateway install` and
`astroclaw onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/astroclaw-gateway[-<profile>].service`:

```
[Unit]
Description=Astroclaw Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. Astroclaw therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, Astroclaw starts the child through a short
...
- Astroclaw-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `ASTROCLAW_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
