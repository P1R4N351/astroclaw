...
read_when:
  - You are moving Astroclaw to a new laptop or server
  - You are coming from another agent system and want to keep state
  - You are upgrading an in-place plugin
title: "Migration guide"
---

Astroclaw supports three migration paths: importing from another agent system, moving an existing install to a new machine, and upgrading a plugin in place.

## Import from another agent system

Use the bundled migration providers to bring instructions, MCP servers, skills, model config, and (opt-in) API keys into Astroclaw. Plans are previewed before any change, secrets are redacted in reports, and apply is backed by a verified backup.
...
The CLI entry point is [`astroclaw migrate`](/cli/migrate). Onboarding can also offer migration when it detects a known source (`astroclaw onboard --flow import`).

## Move Astroclaw to a new machine

Copy the **state directory** (`~/.astroclaw/` by default) and your **workspace** to preserve:

- **Config** — `astroclaw.json` and all gateway settings.
...
<Tip>
Run `astroclaw status` on the old machine to confirm your state directory path. Custom profiles use `~/.astroclaw-<profile>/` or a path set via `ASTROCLAW_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    cd ~
    tar -czf astroclaw-state.tgz .astroclaw
    ```

    If you use multiple profiles (for example `~/.astroclaw-work`), archive each separately.

  </Step>

  <Step title="Install Astroclaw on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine. It is fine if onboarding creates a fresh `~/.astroclaw/`. You will overwrite it next.
  </Step>
...
```bash
awk -F= '/^(TELEGRAM_BOT_TOKEN|DISCORD_BOT_TOKEN)=/ { print $1 "=present" }' ~/.astroclaw/.env
```

`astroclaw doctor` also warns when an enabled default Telegram or Discord account has no configured token and the matching env variable is unavailable to the doctor process.

### Common pitfalls

<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `ASTROCLAW_STATE_DIR` and the new one does not, channels will appear logged out and sessions will be empty. Launch the gateway with the **same** profile or state-dir you migrated, then rerun `astroclaw doctor`.
  </Accordion>

  <Accordion title="Copying only astroclaw.json">
...
- [ ] `astroclaw status` shows the gateway running.
...
- [`astroclaw migrate`](/cli/migrate): CLI reference for cross-system imports.
- [Install overview](/install): all installation methods.
- [Doctor](/gateway/doctor): post-migration health check.
- [Uninstall](/install/uninstall): removing Astroclaw cleanly.
