---
summary: "Install Astroclaw - installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Astroclaw, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://astroclaw.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want Astroclaw and Node kept under a local prefix such as
`~/.astroclaw`, without depending on a system-wide Node install:

```bash
curl -fsSL https://astroclaw.ai/install-cli.sh | bash
```
...
`astroclaw update --channel dev` and `astroclaw update --channel stable`. See
...
<Tabs>
  <Tab title="npm">
    ...
    for the Astroclaw package install. If you install manually with npm, your own
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g astroclaw@latest
```
...
Or skip the link and use `pnpm astroclaw ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:astroclaw/astroclaw#main
```
...
```bash
astroclaw --version      # confirm the CLI is available
astroclaw doctor         # check for config issues
astroclaw gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `astroclaw onboard --install-daemon` or `astroclaw gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Astroclaw on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Astroclaw completely.
  </Card>
</CardGroup>

## Troubleshooting: `astroclaw` not found

If the install succeeded but `astroclaw` is not found in your terminal:
