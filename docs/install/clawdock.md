---
summary: "ClawDock shell helpers for Docker-based Astroclaw installs"
read_when:
  - You run Astroclaw with Docker often and want shorter day-to-day commands
  - You want a helper layer for dashboard, logs, token setup, and pairing flows
title: "ClawDock"
---

ClawDock is a small shell-helper layer for Docker-based Astroclaw installs.
...
| `clawdock-cli <command>`  | Run Astroclaw CLI commands in Docker           |
...
| `clawdock-config`      | Open `~/.astroclaw`                      |
...
- `~/.astroclaw/.env` for env-backed provider keys and bot tokens
- `~/.astroclaw/agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `~/.astroclaw/astroclaw.json` for behavior config

Use `clawdock-show-config` when you want to inspect the `.env` files and `astroclaw.json` quickly. It redacts `.env` values in its printed output.

## Related

<CardGroup cols={2}>
  <Card title="Docker" href="/install/docker" icon="docker">
    Canonical Docker install for Astroclaw.
  </Card>
  ...
  <Card title="Updating" href="/install/updating" icon="arrow-up-right-from-square">
    Updating the Astroclaw package and managed services.
  </Card>
</CardGroup>
