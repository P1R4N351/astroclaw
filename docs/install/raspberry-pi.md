---
summary: "Host Astroclaw on a Raspberry Pi for always-on self-hosting"
read_when:
  - Setting up Astroclaw on a Raspberry Pi
  - Running Astroclaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

Run a persistent, always-on Astroclaw Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well — typical hardware cost is **$35–80 one-time**, no monthly fees.
...
<Steps>
  ...
  <Step title="Verify">
    ```bash
    astroclaw status
    systemctl --user status astroclaw-gateway.service
    journalctl --user -u astroclaw-gateway.service -f
    ```
  </Step>
  ...
  <Step title="Access the Control UI">
    ...
    ```bash
    ssh user@gateway-host 'astroclaw dashboard --no-open'
    ```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/astroclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
...
**systemd drop-in for stable restarts** -- If this Pi is mostly running Astroclaw, add a service drop-in:

```bash
systemctl --user edit astroclaw-gateway.service
```

```ini
[Service]
Environment=ASTROCLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/astroclaw-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```

Then `systemctl --user daemon-reload && systemctl --user restart astroclaw-gateway.service`. On a headless Pi, also enable lingering once so the user service survives logout: `sudo loginctl enable-linger "$(whoami)"`.
...
Most Astroclaw features work on ARM64 without changes (Node.js, Telegram, WhatsApp/Baileys, Chromium). The binaries that occasionally lack ARM builds are typically optional Go/Rust CLI tools shipped by skills. Verify a missing binary's release page for `linux-arm64` / `aarch64` artifacts before falling back to building from source.

## Persistence and backups

Astroclaw state lives under:

- `~/.astroclaw/` — `astroclaw.json`, per-agent `auth-profiles.json`, channel/provider state, sessions.
- `~/.astroclaw/workspace/` — agent workspace (SOUL.md, memory, artifacts).
...
**Service will not start** -- Check logs with `journalctl --user -u astroclaw-gateway.service --no-pager -n 100` and run `astroclaw doctor --non-interactive`. If this is a headless Pi, also verify lingering is enabled: `sudo loginctl enable-linger "$(whoami)"`.
...
- [Updating](/install/updating) -- keep Astroclaw up to date
