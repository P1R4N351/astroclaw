---
summary: "Run Astroclaw Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Astroclaw running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Astroclaw Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

This guide sets up an Azure Linux VM with the Azure CLI, applies Network Security Group (NSG) hardening, configures Azure Bastion for SSH access, and installs Astroclaw.
...
- Install Astroclaw with the installer script
- Verify the Gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    RG="rg-astroclaw"
    LOCATION="westus2"
    VNET_NAME="vnet-astroclaw"
    VNET_PREFIX="10.40.0.0/16"
    VM_SUBNET_NAME="snet-astroclaw-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-astroclaw-vm"
    VM_NAME="vm-astroclaw"
    ADMIN_USERNAME="astroclaw"
    BASTION_NAME="bas-astroclaw"
    BASTION_PIP_NAME="pip-astroclaw-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install Astroclaw (in the VM shell)">
    ```bash
    curl -fsSL https://astroclaw.ai/install.sh -o /tmp/install.sh
    bash /tmp/install.sh
    rm -f /tmp/install.sh
    ```

    The installer installs Node LTS and dependencies if not already present, installs Astroclaw, and launches the onboarding wizard. See [Install](/install) for details.
  ...
  <Step title="Verify the Gateway">
    ...
    Most enterprise Azure teams already have GitHub Copilot licenses. If that is your case, we recommend choosing the GitHub Copilot provider in the Astroclaw onboarding wizard. See [GitHub Copilot provider](/providers/github-copilot).
...
- **Deallocate the VM** when not in use (stops compute billing; disk charges remain). The Astroclaw Gateway will not be reachable while the VM is deallocated — restart it when you need it live again:
...
- For more details on Astroclaw Azure deployment with the GitHub Copilot model provider: [Astroclaw on Azure with GitHub Copilot](https://github.com/johnsonshi/astroclaw-azure-github-copilot)
