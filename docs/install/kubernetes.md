---
summary: "Deploy Astroclaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Astroclaw on a Kubernetes cluster
  - You want to test Astroclaw in a Kubernetes environment
title: "Kubernetes"
---

A minimal starting point for running Astroclaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Astroclaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/astroclaw 18789:18789 -n astroclaw
...
```bash
kubectl get secret astroclaw-secrets -n astroclaw -o jsonpath='{.data.ASTROCLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: astroclaw (configurable via ASTROCLAW_NAMESPACE)
├── Deployment/astroclaw        # Single pod, init container + gateway
├── Service/astroclaw           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/astroclaw-config  # astroclaw.json + AGENTS.md
└── Secret/astroclaw-secrets    # Gateway token + API keys
```
...
Edit `astroclaw.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret astroclaw-secrets -n astroclaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/astroclaw -n astroclaw
```

### Custom namespace

```bash
ASTROCLAW_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/astroclaw/astroclaw:latest # or pin to a specific version from https://github.com/astroclaw/astroclaw/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # astroclaw.json + AGENTS.md
