---
summary: "Host Astroclaw on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Astroclaw on Oracle Cloud
  - Looking for free VPS hosting for Astroclaw
  - Want 24/7 Astroclaw on a small server
title: "Oracle Cloud"
---

Run a persistent Astroclaw Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname astroclaw
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=astroclaw
    ```

    From now on, connect via Tailscale: `ssh ubuntu@astroclaw`.
  ...
  <Step title="Configure the gateway">
    ...
    ```bash
    astroclaw config set gateway.bind loopback
    astroclaw config set gateway.auth.mode token
    astroclaw doctor --generate-gateway-token
    astroclaw config set gateway.tailscale.mode serve
    astroclaw config set gateway.trustedProxies '["127.0.0.1"]'
  ...
  <Step title="Verify">
    ...
    ```
    https://astroclaw.<tailnet-name>.ts.net/
    ```
...
- `chmod 700 ~/.astroclaw` to restrict credential file permissions.
- `astroclaw security audit` for an Astroclaw-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most Astroclaw features work fine; a small number of native binaries need ARM builds:
...
- `~/.astroclaw/` — `astroclaw.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
- `~/.astroclaw/workspace/` — the agent workspace (SOUL.md, memory, artifacts).
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@astroclaw
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=astroclaw --reset` to re-authenticate.

**Gateway will not start** -- Run `astroclaw doctor --non-interactive` and check logs with `journalctl --user -u astroclaw-gateway.service -n 50`.
