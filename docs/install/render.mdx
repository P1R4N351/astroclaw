---
summary: "Deploy Astroclaw on Render with Infrastructure-as-Code"
read_when:
  - Deploying Astroclaw to Render
...
Deploy Astroclaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/astroclaw/astroclaw)
...
repository configures everything needed to run Astroclaw:
...
services:
  - type: web
    ...
    envVars:
      - key: ASTROCLAW_GATEWAY_PORT
        value: "8080"
      - key: ASTROCLAW_STATE_DIR
        value: /data/.astroclaw
      - key: ASTROCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: ASTROCLAW_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means Astroclaw state
...
`ASTROCLAW_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original Astroclaw repository, Render will not auto-deploy your Astroclaw. To update it, run a manual Blueprint sync from the dashboard.
...
For Astroclaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with Astroclaw state plus any configured
...
- Missing `ASTROCLAW_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `ASTROCLAW_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `astroclaw backup create` in the Render shell.
