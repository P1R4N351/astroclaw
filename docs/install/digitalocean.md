---
summary: "Host Astroclaw on a DigitalOcean Droplet"
read_when:
  - Setting up Astroclaw on DigitalOcean
  - Looking for a simple paid VPS for Astroclaw
title: "DigitalOcean"
---

Run a persistent Astroclaw Gateway on a DigitalOcean Droplet (~$6/month for the 1 GB Basic plan).
...
<Steps>
  ...
  <Step title="Connect and install">
    ...
    # Create the non-root user that will own Astroclaw state and services.
    adduser astroclaw
    usermod -aG sudo astroclaw
    loginctl enable-linger astroclaw

    su - astroclaw
    astroclaw --version
    ```

    Use the root shell only for system bootstrap. Run Astroclaw commands as the non-root `astroclaw` user so state lives under `/home/astroclaw/.astroclaw/` and the Gateway installs as that user's systemd service.
  ...
  <Step title="Access the Control UI">
    ...
    **Option C: Tailnet bind (no Serve)**
    ...
    ```bash
    astroclaw config set gateway.bind tailnet
    astroclaw gateway restart
    ```
...
DigitalOcean snapshots back the whole Droplet up; `astroclaw backup create` is portable across hosts.
