---
summary: "Optional Docker-based setup and onboarding for Astroclaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Astroclaw on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export ASTROCLAW_IMAGE="ghcr.io/astroclaw/astroclaw:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/astroclaw/astroclaw/pkgs/container/astroclaw).
  ...
  <Step title="Complete onboarding">
    ...
    `astroclaw-gateway` directly. `astroclaw-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm astroclaw-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm astroclaw-cli channels login

    # Telegram
    docker compose run --rm astroclaw-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm astroclaw-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t astroclaw:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node astroclaw-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node astroclaw-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d astroclaw-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `ASTROCLAW_EXTRA_MOUNTS`
or `ASTROCLAW_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `astroclaw-cli` shares `astroclaw-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d astroclaw-gateway`, run onboarding
and setup-time config writes through `astroclaw-gateway` with
...
| Variable                                   | Purpose                                                         |
| ------------------------------------------ | --------------------------------------------------------------- |
| `ASTROCLAW_IMAGE`                           | Use a remote image instead of building locally                  |
| `ASTROCLAW_DOCKER_APT_PACKAGES`             | Install extra apt packages during build (space-separated)       |
| `ASTROCLAW_EXTENSIONS`                      | Include selected bundled plugin helpers at build time           |
| `ASTROCLAW_EXTRA_MOUNTS`                    | Extra host bind mounts (comma-separated `source:target[:opts]`) |
| `ASTROCLAW_HOME_VOLUME`                     | Persist `/home/node` in a named Docker volume                   |
| `ASTROCLAW_SANDBOX`                         | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)          |
| `ASTROCLAW_SKIP_ONBOARDING`                 | Skip the interactive onboarding step (`1`, `true`, `yes`, `on`) |
| `ASTROCLAW_DOCKER_SOCKET`                   | Override Docker socket path                                     |
| `ASTROCLAW_DISABLE_BONJOUR`                 | Disable Bonjour/mDNS advertising (defaults to `1` for Docker)   |
| `ASTROCLAW_DISABLE_BUNDLED_SOURCE_OVERLAYS` | Disable bundled plugin source bind-mount overlays               |
...
| `ASTROCLAW_OTEL_PRELOADED`                  | Skip starting a second OpenTelemetry SDK when one is preloaded  |
...
`ASTROCLAW_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`.
...
```bash
export ASTROCLAW_EXTENSIONS="diagnostics-otel"
export OTEL_EXPORTER_OTLP_ENDPOINT="http://otel-collector:4318"
export OTEL_SERVICE_NAME="astroclaw-gateway"
./scripts/docker/setup.sh
```

Install the official `@astroclaw/diagnostics-otel` plugin from ClawHub in
packaged Docker installs before enabling export. Custom source-built images can
still include the local plugin source with
`ASTROCLAW_EXTENSIONS=diagnostics-otel`. To enable export, allow and enable the
...
`clawhub:@astroclaw/diagnostics-prometheus`, enable the
...
```bash
docker compose exec astroclaw-gateway node dist/index.js health --token "$ASTROCLAW_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `ASTROCLAW_GATEWAY_BIND=lan` so host access to
...
When Astroclaw runs in Docker, `127.0.0.1` inside the container is the container
...
`ASTROCLAW_DISABLE_BONJOUR=1` so the Gateway does not crash-loop or repeatedly
...
Set `ASTROCLAW_DISABLE_BONJOUR=0` only when running with host networking, macvlan,
...
Docker Compose bind-mounts `ASTROCLAW_CONFIG_DIR` to `/home/node/.astroclaw`,
`ASTROCLAW_WORKSPACE_DIR` to `/home/node/.astroclaw/workspace`, and
`ASTROCLAW_AUTH_PROFILE_SECRET_DIR` to `/home/node/.config/astroclaw`, so those
...
That mounted config directory is where Astroclaw keeps:

- `astroclaw.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `ASTROCLAW_GATEWAY_TOKEN`
...
but separate from `ASTROCLAW_CONFIG_DIR`.

Installed downloadable plugins store their package state under the mounted
Astroclaw home, so plugin install records and package roots survive container
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/astroclaw/astroclaw/main/scripts/clawdock/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export ASTROCLAW_SANDBOX=1
    export ASTROCLAW_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
    ...
    `workspace-write` while the Astroclaw sandbox is active; do not mount the
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm astroclaw-cli gateway probe
    docker compose run -T --rm astroclaw-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `astroclaw-cli` uses `network_mode: "service:astroclaw-gateway"` so CLI
    ...
    `no-new-privileges` on both `astroclaw-gateway` and `astroclaw-cli`.
  </Accordion>

  <Accordion title="Docker Desktop DNS failures in astroclaw-cli">
    Some Docker Desktop setups fail DNS lookups from the shared-network
    `astroclaw-cli` sidecar after `NET_RAW` is dropped, which shows up as
    `EAI_AGAIN` during npm-backed commands such as `astroclaw plugins install`.
    ...
    docker compose -f docker-compose.yml -f docker-compose.cli-no-dropped-caps.local.yml run --rm astroclaw-cli plugins install <package>
    ```

    If you already created a long-running `astroclaw-cli` container, recreate it
  ...
  <Accordion title="Permissions and EACCES">
    ...
    `/home/node/.astroclaw`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/astroclaw-config /path/to/astroclaw-workspace
    ```
    ...
    `/path/to/astroclaw-config/npm` to `root:root` if you intentionally run
    Astroclaw as root long term.
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export ASTROCLAW_HOME_VOLUME="astroclaw_home"`
    2. **Bake system deps**: `export ASTROCLAW_DOCKER_APT_PACKAGES="git curl jq"`
    3. **Bake Playwright Chromium**: `export ASTROCLAW_INSTALL_BROWSER=1`
    4. **Or install Playwright browsers into a persisted volume**:
       ```bash
       docker compose run --rm astroclaw-cli \
         node /app/node_modules/playwright-core/cli.js install chromium
       ```
    5. **Persist browser downloads**: use `ASTROCLAW_HOME_VOLUME` or
       `ASTROCLAW_EXTRA_MOUNTS`. Astroclaw auto-detects the Docker image's
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/astroclaw/astroclaw/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Astroclaw runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm astroclaw-cli dashboard --no-open
    docker compose run --rm astroclaw-cli devices list
    docker compose run --rm astroclaw-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm astroclaw-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm astroclaw-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping Astroclaw up to date
