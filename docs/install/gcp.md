---
summary: "Run Astroclaw Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Astroclaw running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent Astroclaw Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Astroclaw 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Astroclaw Gateway in Docker
- Persist `~/.astroclaw` + `~/.astroclaw/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.astroclaw` state includes `astroclaw.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-astroclaw-project --name="Astroclaw Gateway"
    gcloud config set project my-astroclaw-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create astroclaw-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh astroclaw-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the Astroclaw repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    ASTROCLAW_IMAGE=astroclaw:latest
    ASTROCLAW_GATEWAY_TOKEN=
    ASTROCLAW_GATEWAY_BIND=lan
    ASTROCLAW_GATEWAY_PORT=18789

    ASTROCLAW_CONFIG_DIR=/home/$USER/.astroclaw
    ASTROCLAW_WORKSPACE_DIR=/home/$USER/.astroclaw/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.astroclaw
    ```

    Set `ASTROCLAW_GATEWAY_TOKEN` when you want to manage the stable gateway
    token through `.env`; otherwise configure `gateway.auth.token` before
    relying on clients across restarts. If neither source exists, Astroclaw uses
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `ASTROCLAW_GATEWAY_TOKEN`.
    ...
    services:
      astroclaw-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - ASTROCLAW_GATEWAY_BIND=${ASTROCLAW_GATEWAY_BIND}
          - ASTROCLAW_GATEWAY_PORT=${ASTROCLAW_GATEWAY_PORT}
          - ASTROCLAW_GATEWAY_TOKEN=${ASTROCLAW_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${ASTROCLAW_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`ASTROCLAW_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm astroclaw-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh astroclaw-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop astroclaw-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type astroclaw-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start astroclaw-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create astroclaw-deploy \
     --display-name="Astroclaw Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-astroclaw-project \
     --member="serviceAccount:astroclaw-deploy@my-astroclaw-project.iam.gserviceaccount.com" \
