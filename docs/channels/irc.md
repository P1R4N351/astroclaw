...
title: IRC
read_when:
  - You want to connect Astroclaw to IRC channels or DMs
  - You are configuring IRC allowlists, group policy, or mention gating
---

Use IRC when you want Astroclaw in classic channels (`#room`) and direct messages.
IRC ships as a bundled plugin, but it is configured in the main config under `channels.irc`.

## Quick start

1. Enable IRC config in `~/.astroclaw/astroclaw.json`.
2. Set at least:
...
- IRC uses raw TCP/TLS sockets outside Astroclaw operator-managed forward proxy routing. In deployments that require all egress through that forward proxy, set `channels.irc.enabled=false` unless direct IRC egress is explicitly approved.
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, Astroclaw defaults to **mention-gating** in group contexts.
