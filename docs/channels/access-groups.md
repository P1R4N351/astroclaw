...
Entries are matched with the destination channel's normal `allowFrom` rules. Astroclaw does not translate sender ids between channels. If Alice has a Telegram id and a Discord id, list both ids under the appropriate keys.
...
Channel support depends on whether that channel is wired through the shared Astroclaw sender-authorization helpers. Current bundled support includes Discord, Feishu, Google Chat, iMessage, LINE, Mattermost, Microsoft Teams, Nextcloud Talk, Nostr, QQBot, Signal, WhatsApp, Zalo, and Zalo Personal. Static `message.senders` groups are designed to be channel-agnostic, so new message channels should support them by using the shared plugin SDK helpers instead of custom allowlist expansion.
...
```typescript
import { resolveAccessGroupAllowFromState } from "astroclaw/plugin-sdk/security-runtime";
...
`discord.channelAudience` means "allow Discord DM senders who can currently view this guild channel." Astroclaw resolves the sender through Discord at authorization time and applies Discord `ViewChannel` permission rules.
...
Run `astroclaw doctor` after editing access-control config. It catches many invalid allowlist and policy combinations before runtime.
