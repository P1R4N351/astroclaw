...
read_when:
  - Setting up Matrix in Astroclaw
  - Configuring Matrix E2EE and verification
title: "Matrix"
---

Matrix is a downloadable channel plugin for Astroclaw.
...
```bash
astroclaw plugins install @astroclaw/matrix
```

Bare plugin specs try ClawHub first, then npm fallback. To force the registry source, use `astroclaw plugins install clawhub:@astroclaw/matrix` or `astroclaw plugins install npm:@astroclaw/matrix`.

From a local checkout:

```bash
astroclaw plugins install ./path/to/local/matrix-plugin
```

`plugins install` registers and enables the plugin, so no separate `astroclaw plugins enable matrix` step is needed. The plugin still does nothing until you configure the channel below. See [Plugins](/tools/plugin) for general plugin behavior and install rules.
...
```bash
astroclaw channels add
astroclaw configure --section channels
```
...
If matching `MATRIX_*` env vars already exist and the selected account has no saved auth, the wizard offers an env-var shortcut. To resolve room names before saving an allowlist, run `astroclaw channels resolve --channel matrix "Project Room"`. When E2EE is enabled, the wizard writes the config and runs the same bootstrap as [`astroclaw matrix encryption setup`](#encryption-and-verification).
...
Astroclaw cannot tell at invite time whether an invited room is a DM or a group, so all invites - including DM-style invites - go through `autoJoin` first. `dm.policy` only applies later, after the bot has joined and the room has been classified.
...
Matrix stores cached credentials under `~/.astroclaw/credentials/matrix/`:

- default account: `credentials.json`
- named accounts: `credentials-<account>.json`

When cached credentials exist there, Astroclaw treats Matrix as configured even if the access token is not in the config file - that covers setup, `astroclaw doctor`, and channel-status probes.
...
Matrix reply streaming is opt-in. `streaming` controls how Astroclaw delivers the in-flight assistant reply; `blockStreaming` controls whether each completed block is preserved as its own Matrix message.
...
- If a preview grows past Matrix's per-event size limit, Astroclaw stops preview streaming and falls back to final-only delivery.
- Media replies always send attachments normally. If a stale preview can no longer be reused safely, Astroclaw redacts it before sending the final media reply.
...
Matrix native approval prompts are normal `m.room.message` events with Astroclaw-specific custom event content under `com.astroclaw.approval`. Matrix permits custom event-content keys, so stock clients still render the text body while Astroclaw-aware clients can read the structured approval id, kind, state, available decisions, and exec/plugin details.

When an approval prompt is too long for one Matrix event, Astroclaw chunks the visible text and attaches `com.astroclaw.approval` to the first chunk only. Reactions for allow/deny decisions are bound to that first event, so long prompts keep the same approval target as single-event prompts.
...
By default, Matrix messages from other configured Astroclaw Matrix accounts are ignored.
...
- Astroclaw still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix does not expose a native bot flag here; Astroclaw treats "bot-authored" as "sent by another configured Matrix account on this Astroclaw gateway".
...
All `astroclaw matrix` commands accept `--verbose` (full diagnostics), `--json` (machine-readable output), and `--account <id>` (multi-account setups). Output is concise by default with quiet internal SDK logging. The examples below show the canonical form; add the flags as needed.

### Enable encryption

```bash
astroclaw matrix encryption setup
```
...
astroclaw matrix verify status --include-recovery-key --json
```
...
The literal-key form `astroclaw matrix verify device "<recovery-key>"` is also accepted, but the key ends up in your shell history.
...
If the homeserver requires UIA to upload cross-signing keys, Astroclaw tries no-auth first, then `m.login.dummy`, then `m.login.password` (requires `channels.matrix.password`).
...
```bash
astroclaw matrix verify list
```

Lists pending verification requests for the selected account.

```bash
astroclaw matrix verify request --own-user
astroclaw matrix verify request --user-id @ops:example.org --device-id ABCDEF
```

Sends a verification request from this Astroclaw account. `--own-user` requests self-verification (you accept the prompt in another Matrix client of the same user); `--user-id`/`--device-id`/`--room-id` target someone else. `--own-user` cannot be combined with the other targeting flags.
...
| Command                                    | Purpose                                                             |
| ------------------------------------------ | ------------------------------------------------------------------- |
| `astroclaw matrix verify accept <id>`       | Accept an inbound request                                           |
| `astroclaw matrix verify start <id>`        | Start the SAS flow                                                  |
| `astroclaw matrix verify sas <id>`          | Print the SAS emoji or decimals                                     |
| `astroclaw matrix verify confirm-sas <id>`  | Confirm that the SAS matches what the other client shows            |
| `astroclaw matrix verify mismatch-sas <id>` | Reject the SAS when the emoji or decimals do not match              |
| `astroclaw matrix verify cancel <id>`       | Cancel; takes optional `--reason <text>` and `--code <matrix-code>` |
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    ...
    Startup also runs a conservative crypto bootstrap pass that reuses the current secret storage and cross-signing identity. If bootstrap state is broken, Astroclaw attempts a guarded repair even without `channels.matrix.password`; if the homeserver requires password UIA, startup logs a warning and stays non-fatal. Already-owner-signed devices are preserved.
  ...
  <Accordion title="Verification notices">
    ...
    Incoming requests from another Matrix client are tracked and auto-accepted. For self-verification, Astroclaw starts the SAS flow automatically and confirms its own side once emoji verification is available - you still need to compare and confirm "They match" in your Matrix client.
  ...
  <Accordion title="Deleted or invalid Matrix device">
    If `verify status` says the current device is no longer listed on the homeserver, create a new Astroclaw Matrix device. For password login:
...
astroclaw matrix account add \
  ...
  --password '<password>' \
  --device-name Astroclaw-Gateway
```

    For token auth, create a fresh access token in your Matrix client or admin UI, then update Astroclaw:
...
```
  ...
  <Accordion title="Device hygiene">
    Old Astroclaw-managed devices can accumulate. List and prune:

```bash
astroclaw matrix devices list
astroclaw matrix devices prune-stale
```
  ...
  <Accordion title="Crypto store">
    ...
    Encrypted runtime state lives under `~/.astroclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/` and includes the sync store, crypto store, recovery key, IDB snapshot, thread bindings, and startup verification state. When the token changes but the account identity stays the same, Astroclaw reuses the best existing root so prior state remains visible.
...
```bash
astroclaw matrix profile set --name "Astroclaw Assistant"
astroclaw matrix profile set --avatar-url https://cdn.example.org/avatar.png
```

You can pass both options in one call. Matrix accepts `mxc://` avatar URLs directly; when you pass `http://` or `https://`, Astroclaw uploads the file first and stores the resolved `mxc://` URL into `channels.matrix.avatarUrl` (or the per-account override).
...
`dm.sessionScope` decides how Matrix DM rooms map to Astroclaw sessions:
...
- DM user-target reuse only kicks in when the current session metadata proves the same DM peer on the same Matrix account; otherwise Astroclaw falls back to normal user-scoped routing.
...
When Astroclaw detects a Matrix DM room colliding with another DM room on the same shared session, it posts a one-time `m.notice` in that room pointing to the `/focus` escape hatch and suggesting a `dm.sessionScope` change. The notice only appears when thread bindings are enabled.
...
- `threadBindings.spawnSessions` gates `/acp spawn --thread auto|here`, where Astroclaw needs to create or bind a child Matrix thread.
...
- Matrix room history is pending-only: Astroclaw buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
...
```bash
astroclaw pairing list matrix
astroclaw pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging you before approval, Astroclaw reuses the same pending pairing code and may send a reminder reply after a short cooldown instead of minting a new code.

See [Pairing](/channels/pairing) for the shared DM pairing flow and storage layout.

## Direct room repair

If direct-message state drifts out of sync, Astroclaw can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer:

```bash
astroclaw matrix direct inspect --user-id @alice:example.org
```

Repair it:

```bash
astroclaw matrix direct repair --user-id @alice:example.org
```
...
- If you have multiple accounts and one is literally named `default`, Astroclaw uses it implicitly even when `defaultAccount` is unset.
...
- When Astroclaw promotes a single-account config to multi-account during repair or setup, it preserves the existing named account if one exists or `defaultAccount` already points at one. Only Matrix auth/bootstrap keys move into the promoted account; shared delivery-policy keys stay at the top level.

See [Configuration reference](/gateway/config-channels#multi-account-all-channels) for the shared multi-account pattern.

## Private/LAN homeservers

By default, Astroclaw blocks private/internal Matrix homeservers for SSRF protection unless you
...
Astroclaw uses the same proxy setting for runtime Matrix traffic and account status probes.

## Target resolution

Matrix accepts these target forms anywhere Astroclaw asks you for a room or user target:
...
Astroclaw keeps internal session keys canonical for storage, so those lowercase
