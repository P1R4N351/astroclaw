...
| Request signing              | Not used (auth is the App-Level Token)                                                                                                               | Slack signs every request; Astroclaw verifies with `signingSecret`                                              |
| Recovery on connection drop  | Slack SDK auto-reconnect is enabled; Astroclaw also restarts failed Socket Mode sessions with bounded backoff. Pong-timeout transport tuning applies. | No persistent connection to drop; retries are per-request from Slack                                           |
...
```bash
astroclaw plugins install @astroclaw/slack
```
...
```json Recommended
{
  "display_information": {
    "name": "Astroclaw",
    "description": "Slack connector for Astroclaw"
  },
  "features": {
    "bot_user": { "display_name": "Astroclaw", "always_online": true },
    "app_home": {
...
```
      ...
      <Step title="Configure Astroclaw">
...
JSON5
astroclaw config patch --file ./slack.socket.patch.json5 --dry-run
astroclaw config patch --file ./slack.socket.patch.json5
```
...
JSON5
astroclaw config patch --file ./slack.http.patch.json5 --dry-run
astroclaw config patch --file ./slack.http.patch.json5
```
...
Astroclaw sets the Slack SDK client pong timeout to 15 seconds by default for Socket Mode. Override the transport settings only when you need workspace- or host-specific tuning:
...
- Only `clientPingTimeout` has an Astroclaw default (`15000`). `serverPingTimeout` and `pingPongLoggingEnabled` are passed to the Slack SDK only when configured.
...
The default manifest enables the Slack App Home **Home** tab and subscribes to `app_home_opened`. When a workspace member opens the Home tab, Astroclaw publishes a safe default Home view with `views.publish`; no conversation payload or private configuration is included. The **Messages** tab remains enabled for Slack DMs. The manifest also enables Slack assistant threads with `features.assistant_view`, `assistant:write`, `assistant_thread_started`, and `assistant_thread_context_changed`; assistant threads route to their own Astroclaw thread sessions and keep Slack-provided thread context available to the agent.
...
<Tabs>
  <Tab title="DM policy">
    ...
    Legacy `channels.slack.dm.policy` and `channels.slack.dm.allowFrom` still read for compatibility. `astroclaw doctor --fix` migrates them to `dmPolicy` and `allowFrom` when it can do so without changing access.

    Pairing in DMs uses `astroclaw pairing approve slack <code>`.
...
- Astroclaw seeds an eligible top-level channel root into `agent:<agentId>:slack:channel:<channelId>:thread:<rootTs>` when that root is expected to start a visible Slack thread, so the root and later thread replies share one Astroclaw session. This applies to `app_mention` events, explicit bot or configured mention-pattern matches, and `requireMention: false` channels with non-`off` `replyToMode`.
...
- Top-level Slack DMs stay off-thread by default, so they do not show Slack's thread-style native stream/status preview; Astroclaw posts and edits a draft preview in the DM instead.
...
- If streaming fails mid-reply, Astroclaw falls back to normal delivery for remaining payloads.
...
- Run `astroclaw doctor --fix` to rewrite persisted Slack streaming config to the canonical keys.

## Typing reaction fallback

`typingReaction` adds a temporary reaction to the inbound Slack message while Astroclaw is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
<AccordionGroup>
  <Accordion title="Inbound attachments">
    ...
    Downloads use bounded idle and total timeouts. If Slack file retrieval stalls or fails, Astroclaw keeps processing the message and falls back to the file placeholder.
...
- The interactive callback values are Astroclaw-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Astroclaw falls back to the original text reply instead of sending an invalid blocks payload.

### Plugin-owned modal submissions

Slack plugins that register an interactive handler can also receive modal
`view_submission` and `view_closed` lifecycle events before Astroclaw compacts
the payload for the agent-visible system event. Use one of these routing
patterns when opening a Slack modal:

- Set `callback_id` to `astroclaw:<namespace>:<payload>`.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `astroclaw channels status --probe --json` shows `botTokenStatus` or
...
1. Astroclaw downloads the file from Slack's private URL using the bot token (`xoxb-...`).
...
| Forwarded/shared attachments           | Text and Slack-hosted image/file media are best-effort                       | Re-share directly in the Astroclaw thread                                   |
...
- [PDF tool](/tools/pdf)
- Epic: [#51349](https://github.com/astroclaw/astroclaw/issues/51349) — Slack attachment vision enablement
- Regression tests: [#51353](https://github.com/astroclaw/astroclaw/issues/51353)
- Live verification: [#51354](https://github.com/astroclaw/astroclaw/issues/51354)
