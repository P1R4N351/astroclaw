...
Nextcloud Talk ships as a bundled plugin in current Astroclaw releases, so
...
```bash
astroclaw plugins install @astroclaw/nextcloud-talk
```
...
```bash
astroclaw plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Astroclaw" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure Astroclaw:
   ...
   ```bash
   astroclaw channels add --channel nextcloud-talk \
...
- Approve via:
  - `astroclaw pairing list nextcloud-talk`
  - `astroclaw pairing approve nextcloud-talk <CODE>`
