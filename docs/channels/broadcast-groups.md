...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when Astroclaw would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When Astroclaw would reply in this chat, it will run all three agents.
...
<Tabs>
  <Tab title="Alfred's context">
    ...
    Workspace: /Users/user/astroclaw-alfred/
  ...
  <Tab title="Bärbel's context">
    ...
    Workspace: /Users/user/astroclaw-baerbel/
    Tools: read only
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Agents not responding">
    ...
    **Debug:**
    ...
    ```bash
    tail -f ~/.astroclaw/logs/gateway.log | grep broadcast
    ```
