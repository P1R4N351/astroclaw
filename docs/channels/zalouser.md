...
read_when:
  - Setting up Zalo Personal for Astroclaw
  - Debugging Zalo Personal login or message flow
title: "Zalo personal"
---

Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Astroclaw.
...
Zalo Personal ships as a bundled plugin in current Astroclaw releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install the npm package directly:

- Install via CLI: `astroclaw plugins install @astroclaw/zalouser`
- Pinned version: `astroclaw plugins install @astroclaw/zalouser@2026.5.2`
- Or from a source checkout: `astroclaw plugins install ./path/to/local/zalouser-plugin`
...
- `astroclaw pairing list zalouser`
- `astroclaw pairing approve zalouser <code>`
...
- On startup, Astroclaw resolves group/user names in allowlists to IDs and logs the mapping only when `channels.zalouser.dangerouslyAllowNameMatching: true` is enabled.
...
- When a group message is skipped because mention is required, Astroclaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Astroclaw state. Example:
...
- Astroclaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Astroclaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `astroclaw channels logout --channel zalouser && astroclaw channels login --channel zalouser`
...
- The channel now runs fully in Astroclaw without external CLI binaries.
