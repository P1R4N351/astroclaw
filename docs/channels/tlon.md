...
Tlon is a decentralized messenger built on Urbit. Astroclaw connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Astroclaw releases, so normal packaged
...
```bash
astroclaw plugins install @astroclaw/tlon
```
...
```bash
astroclaw plugins install ./path/to/local/tlon-plugin
```
...
By default, Astroclaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `astroclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Astroclaw replies in-thread.
