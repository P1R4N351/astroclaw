...
read_when:
  - You want to connect Astroclaw to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to Astroclaw via the LINE Messaging API. The plugin runs as a webhook
...
```bash
astroclaw plugins install @astroclaw/line
```

Local checkout (when running from a git repo):

```bash
astroclaw plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Astroclaw applies strict pre-auth body limits and timeout before verification.
- Astroclaw processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
astroclaw pairing list line
astroclaw pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.astroclaw/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. Astroclaw validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
