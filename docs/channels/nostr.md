...
read_when:
  - You want Astroclaw to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Astroclaw to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current Astroclaw releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`astroclaw onboard`) and `astroclaw channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install the npm package directly.

```bash
astroclaw plugins install @astroclaw/nostr
```
...
```bash
astroclaw plugins install --link <path-to-local-nostr-plugin>
```
...
astroclaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
```json5
{
  channels: {
    nostr: {
      ...
      profile: {
        name: "astroclaw",
        displayName: "Astroclaw",
        about: "Personal assistant DM bot",
        picture: "https://example.com/avatar.png",
        banner: "https://example.com/banner.png",
        website: "https://example.com",
        nip05: "astroclaw@example.com",
        lud16: "astroclaw@example.com",
      },
    },
  },
}
