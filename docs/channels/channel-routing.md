...
Astroclaw routes replies **back to the channel where a message came from**. The
...
Astroclaw infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Astroclaw still records inbound session metadata, but it
...
when a guarded path must not create a new Astroclaw session. In that mode,
Astroclaw may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Astroclaw would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.astroclaw/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.astroclaw`):

- `~/.astroclaw/agents/<agentId>/sessions/sessions.json`
