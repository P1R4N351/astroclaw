...
Tencent Yuanbao is Tencent's AI assistant platform. The Astroclaw channel plugin
connects Yuanbao bots to Astroclaw over WebSocket so they can interact with users
...
> **Requires Astroclaw 2026.4.10 or above.** Run `astroclaw --version` to check. Upgrade with `astroclaw update`.

<Steps>
  <Step title="Add the Yuanbao channel with your credentials">
  ```bash
  astroclaw channels add --channel yuanbao --token "appKey:appSecret"
  ```
...
```bash
astroclaw channels login --channel yuanbao
```
...
```bash
astroclaw pairing list yuanbao
astroclaw pairing approve yuanbao <CODE>
```
