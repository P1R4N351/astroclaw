---
summary: "Synology Chat webhook setup and Astroclaw config"
read_when:
  - Setting up Synology Chat with Astroclaw
...
Synology Chat ships as a bundled plugin in current Astroclaw releases, so normal
...
```bash
astroclaw plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   - Current packaged Astroclaw releases already bundle it.
   - Older/custom installs can add it manually from a source checkout with the command above.
   - `astroclaw onboard` now shows Synology Chat in the same channel setup list as `astroclaw channels add`.
   - Non-interactive setup: `astroclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Astroclaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Astroclaw.
   - Guided: `astroclaw onboard`
   - Direct: `astroclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- Astroclaw accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `astroclaw pairing list synology-chat`
  - `astroclaw pairing approve synology-chat <CODE>`
...
```bash
astroclaw message send --channel synology-chat --target 123456 --text "Hello from Astroclaw"
astroclaw message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
astroclaw message send --channel synology-chat --target synology:123456 --text "Short prefix"
```

Media sends are supported by URL-based file delivery.
Outbound file URLs must use `http` or `https`, and private or otherwise blocked network targets are rejected before Astroclaw forwards the URL to the NAS webhook.
...
Give each enabled account a distinct `webhookPath`. Astroclaw now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached Astroclaw
- `Rate limit exceeded`:
