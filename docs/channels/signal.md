...
- Astroclaw installed on your server (Linux flow below tested on Ubuntu 24).
- One of:
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "Astroclaw"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure Astroclaw and restart the gateway.
5. Send a first DM and approve pairing (`astroclaw pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "Astroclaw"` then scan the QR in Signal.
...
4. Configure Astroclaw, restart gateway, verify channel:

```bash
# If you run the gateway as a user systemd service:
systemctl --user restart astroclaw-gateway.service
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `astroclaw pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point Astroclaw at it:
...
This skips auto-spawn and the startup wait inside Astroclaw. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Register or link your Signal account inside the container before connecting Astroclaw.
...
The `apiMode` field controls which protocol Astroclaw uses:
...
When `apiMode` is `"auto"`, Astroclaw caches the detected mode for 30 seconds to avoid repeated probes. Container receive is only selected for streaming after `/v1/receive/{account}` upgrades to WebSocket, which requires `MODE=json-rpc`.

Container mode supports the same Signal channel operations as native mode where the container exposes matching APIs: sends, receives, attachments, typing indicators, read/viewed receipts, reactions, groups, and styled text. Astroclaw translates its native Signal RPC calls into the container's REST payloads, including `group.{base64(internal_id)}` group IDs and `text_mode: "styled"` for formatted text.

Operational notes:

- Use `autoStart: false` with container mode. Astroclaw should not spawn a native daemon when `apiMode: "container"` is selected.
- Use `MODE=json-rpc` for receiving. `MODE=normal` can make `/v1/about` look healthy, but `/v1/receive/{account}` does not WebSocket-upgrade, so Astroclaw will not select container receive streaming in `auto` mode.
...
- Approve via:
  - `astroclaw pairing list signal`
  - `astroclaw pairing approve signal <CODE>`
...
- **Typing indicators**: Astroclaw sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, Astroclaw forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `astroclaw doctor --fix`.
...
astroclaw pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/astroclaw/astroclaw-$(date +%Y-%m-%d).log" | tail -20
```
