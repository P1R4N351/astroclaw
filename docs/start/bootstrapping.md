...
On the first agent run, Astroclaw bootstraps the workspace (default
`~/.astroclaw/workspace`):
...
For embedded/local model runs, Astroclaw keeps `BOOTSTRAP.md` out of the
...
To skip this for a pre-seeded workspace, run `astroclaw onboard --skip-bootstrap`.
...
host (for example, `user@gateway-host:~/.astroclaw/workspace`).
</Note>
