---
summary: "Scripted onboarding and agent setup for the Astroclaw CLI"
read_when:
...
Use `--non-interactive` to automate `astroclaw onboard`.
...
<AccordionGroup>
  ...
  <Accordion title="Custom provider example">
    ...
    Astroclaw marks common vision model IDs as image-capable automatically. Add `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
Anthropic setup-token remains available as a supported onboarding token path, but Astroclaw now prefers Claude CLI reuse when available.
