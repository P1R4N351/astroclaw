---
summary: "Get Astroclaw installed and run your first chat in minutes."
read_when:
...
Install Astroclaw, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.astroclaw/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.astroclaw/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run Astroclaw as a service account or want custom paths:

- `ASTROCLAW_HOME` — home directory for internal path resolution
- `ASTROCLAW_STATE_DIR` — override the state directory
- `ASTROCLAW_CONFIG_PATH` — override the config file path
