...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Astroclaw
...
Running Astroclaw via Node with `tsx` fails at startup with:

```
[astroclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `astroclaw status` and `gateway:watch` worked.
