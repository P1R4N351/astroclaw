---
summary: "Architecture of Astroclaw's embedded Pi agent integration and session lifecycle"
title: "Pi integration architecture"
read_when:
  - Understanding Pi SDK integration design in Astroclaw
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

Astroclaw integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Astroclaw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Astroclaw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Astroclaw's local TUI mode)                                             |
...
├── pi-tools.ts                    # createAstroclawCodingTools()
...
├── astroclaw-tools.ts              # Astroclaw-specific tools
...
Image injection is prompt-local: Astroclaw loads image refs from the current prompt and
...
2. **Custom Replacements**: Astroclaw replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Astroclaw Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
Astroclaw wraps this with `guardSessionManager()` for tool result safety.
...
Astroclaw maintains an auth profile store with multiple API keys per provider:
...
Astroclaw loads custom pi extensions for specialized behavior:
...
Astroclaw also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Astroclaw Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Astroclaw tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.astroclaw/agents/<agentId>/sessions/` (or `$ASTROCLAW_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `ASTROCLAW_LIVE_TEST=1`)
