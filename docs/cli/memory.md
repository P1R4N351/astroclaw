---
summary: "CLI reference for `astroclaw memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
astroclaw memory status
astroclaw memory status --deep
astroclaw memory status --fix
astroclaw memory index --force
astroclaw memory search "meeting notes"
astroclaw memory search --query "deployment" --max-results 20
astroclaw memory promote --limit 10 --min-score 0.75
astroclaw memory promote --apply
astroclaw memory promote --json --min-recall-count 0 --min-unique-queries 0
astroclaw memory promote-explain "router vlan"
astroclaw memory promote-explain "router vlan" --json
astroclaw memory rem-harness
astroclaw memory rem-harness --json
astroclaw memory status --json
astroclaw memory status --deep --index
astroclaw memory status --deep --index --verbose
astroclaw memory status --agent main
astroclaw memory index --agent main --verbose
```
...
```bash
astroclaw memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `astroclaw cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
astroclaw memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
astroclaw memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
