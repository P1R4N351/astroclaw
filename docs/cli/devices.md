---
summary: "CLI reference for `astroclaw devices` (device pairing + token rotation/revocation)"
read_when:
...
### `astroclaw devices remove <deviceId>`
...
```
astroclaw devices remove <deviceId>
astroclaw devices remove <deviceId> --json
```

### `astroclaw devices clear --yes [--pending]`

Clear paired devices in bulk.

```
astroclaw devices clear --yes
astroclaw devices clear --yes --pending
astroclaw devices clear --yes --pending --json
```

### `astroclaw devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, Astroclaw only prints the selected pending
request and exits; rerun approval with the exact request ID after verifying
the details.

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), Astroclaw supersedes the previous pending entry and issues a new `requestId`. Run `astroclaw devices list` right before approval to use the current ID.
</Note>

If the device is already paired and asks for broader scopes or a broader role,
Astroclaw keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `astroclaw devices list`
or use `astroclaw devices approve --latest` to preview the exact upgrade before
approving it.
...
```
astroclaw devices approve
astroclaw devices approve <requestId>
astroclaw devices approve --latest
```
...
### `astroclaw devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
astroclaw devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `astroclaw devices revoke --device <id> --role <role>`
...
```
astroclaw devices revoke --device <deviceId> --role node
```
...
```bash
astroclaw devices rotate --device <deviceId> --role operator
```
