...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Astroclaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`astroclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Astroclaw channel
conversations instead of hosting an ACP harness session, use
[`astroclaw mcp serve`](/cli/mcp) instead.
...
- Astroclaw acts as an ACP server
- an IDE or ACP client connects to Astroclaw
- Astroclaw forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Astroclaw runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Astroclaw: use `astroclaw acp`
- Astroclaw should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include Astroclaw parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                                |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Astroclaw gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
astroclaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
astroclaw acp --url wss://gateway-host:18789 --token-file ~/.astroclaw/gateway.token

# Attach to an existing session key
astroclaw acp --session agent:main:main

# Attach by label (must already exist)
astroclaw acp --session-label "support inbox"

# Reset the session key before the first prompt
astroclaw acp --session agent:main:main --reset-session
```
...
```bash
astroclaw acp client

# Point the spawned bridge at a remote Gateway
astroclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.astroclaw/gateway.token

# Override the server command (default: astroclaw)
astroclaw acp client --server "node" --server-args astroclaw.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Astroclaw through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`astroclaw acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `astroclaw gateway call sessions.list` as the only ACP proof. That
...
it to drive an Astroclaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `astroclaw acp` over stdio.

Example config (persisted):

```bash
astroclaw config set gateway.remote.url wss://gateway-host:18789
astroclaw config set gateway.remote.token <token>
```
...
```bash
astroclaw acp --session agent:main:main
astroclaw acp --session agent:design:main
astroclaw acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Astroclaw plugin tools or selected
...
[Astroclaw tools MCP bridge](/tools/acp-agents-setup#astroclaw-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
Astroclaw bot over ACP, use `acpx` with its built-in `astroclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx astroclaw` at `astroclaw acp`.
3. Target the Astroclaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Astroclaw ACP session
acpx astroclaw exec "Summarize the active Astroclaw session state."

# Persistent named session for follow-up turns
acpx astroclaw sessions ensure --name codex-bridge
acpx astroclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my Astroclaw work agent for recent context relevant to this repo."
```

If you want `acpx astroclaw` to target a specific Gateway and session key every
time, override the `astroclaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "astroclaw": {
      "command": "env ASTROCLAW_HIDE_BANNER=1 ASTROCLAW_SUPPRESS_NOTES=1 astroclaw acp --url ws://127.0.0.1:18789 --token-file ~/.astroclaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Astroclaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env ASTROCLAW_HIDE_BANNER=1 ASTROCLAW_SUPPRESS_NOTES=1 node astroclaw.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Astroclaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select "Astroclaw ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`ASTROCLAW_GATEWAY_TOKEN`, `ASTROCLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`ASTROCLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `ASTROCLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `astroclaw acp client` sets `ASTROCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `astroclaw`).
