---
summary: "CLI reference for `astroclaw configure` (interactive configuration prompts)"
read_when:
...
Use `astroclaw onboard` for the full guided first-run journey, `astroclaw setup` for the baseline config/workspace only, and `astroclaw channels add` when you only need channel account setup.
...
<Tip>
`astroclaw config` without a subcommand opens the same wizard. Use `astroclaw config get|set|unset` for non-interactive edits.
</Tip>

For web search, `astroclaw configure --section web` lets you choose a provider
...
astroclaw configure --section model --section channels
astroclaw configure --section gateway --section daemon
```
