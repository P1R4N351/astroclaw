---
summary: "CLI reference for `astroclaw path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
astroclaw plugins enable oc-path
```

## Why use it

Astroclaw state is spread across human-edited markdown, commented JSONC config,
...
`astroclaw path` gives those callers a stable address instead of a one-off grep,
...
You probably do not need `astroclaw path` for ordinary whole-file edits, rich
...
```bash
astroclaw path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
astroclaw path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
astroclaw path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
astroclaw path resolve 'oc://AGENTS.md/runtime-safety/astroclaw-gateway'
```

Validate a path in CI or a preflight script before the script reads or writes:

```bash
astroclaw path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
`astroclaw path` does four things:
...
# Read a leaf
astroclaw path resolve 'oc://gateway.jsonc/version'

# Wildcard search
astroclaw path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
astroclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
astroclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
astroclaw path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
astroclaw path emit ./AGENTS.md
```
...
astroclaw path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Predicate search over JSONC children
astroclaw path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
astroclaw path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
astroclaw path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
astroclaw path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
astroclaw path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
astroclaw path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
astroclaw path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
astroclaw path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
astroclaw path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
astroclaw path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
astroclaw path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ astroclaw path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ astroclaw path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ astroclaw path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ astroclaw path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ astroclaw path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ astroclaw path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ astroclaw path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: astroclaw.invoke
```

```bash
$ astroclaw path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ astroclaw path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
astroclaw path resolve 'oc://AGENTS.md/tools/gh/risk' --human
astroclaw path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
astroclaw path find 'oc://AGENTS.md/tools/**/risk'
astroclaw path find 'oc://session.jsonl/[event=action]/userId'
astroclaw path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
astroclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
astroclaw path set 'oc://gateway.jsonc/version' '2.0'
astroclaw path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ astroclaw path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
astroclaw path emit ./AGENTS.md
astroclaw path emit ./gateway.jsonc --json
```
...
`astroclaw path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__ASTROCLAW_REDACTED__` (verbatim or as a substring) is refused at write
  time.
