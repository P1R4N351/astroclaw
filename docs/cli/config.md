---
summary: "CLI reference for `astroclaw config` (get/set/patch/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `astroclaw.json`: get/set/patch/unset/file/schema/validate values by path and print the active config file. Run without a subcommand to open the configure wizard (same as `astroclaw configure`).

<Note>
When `ASTROCLAW_NIX_MODE=1`, Astroclaw treats `astroclaw.json` as immutable. Read-only commands such as `config get`, `config file`, `config schema`, and `config validate` still work, but config writers refuse. Agents should edit the Nix source for the install instead; for the first-party nix-astroclaw distribution, use [nix-astroclaw Quick Start](https://github.com/astroclaw/nix-astroclaw#quick-start) and set values under `programs.astroclaw.config` or `instances.<name>.config`.
</Note>

## Root options

<ParamField path="--section <section>" type="string">
  Repeatable guided-setup section filter when you run `astroclaw config` without a subcommand.
</ParamField>
...
astroclaw config --section model
astroclaw config --section gateway --section daemon
astroclaw config schema
astroclaw config get browser.executablePath
...
astroclaw config set agents.defaults.heartbeat.every "2h"
astroclaw config set agents.list[0].tools.exec.node "node-id-or-name"
astroclaw config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
astroclaw config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
astroclaw config set secrets.providers.vaultfile --provider-source file --provider-path /etc/astroclaw/secrets.json --provider-mode json
astroclaw config patch --file ./astroclaw.patch.json5 --dry-run
astroclaw config unset plugins.entries.brave.config.webSearch.apiKey
astroclaw config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
astroclaw config validate
astroclaw config validate --json
```

### `config schema`

Print the generated JSON schema for `astroclaw.json` to stdout as JSON.
...
```bash
astroclaw config schema > astroclaw.schema.json
```
...
```bash
astroclaw config get agents.defaults.workspace
astroclaw config get agents.list[0].id
```

Use the agent list index to target a specific agent:

```bash
astroclaw config get agents.list
astroclaw config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
```bash
astroclaw config set agents.defaults.heartbeat.every "0m"
astroclaw config set gateway.port 19001 --strict-json
astroclaw config set channels.whatsapp.groups '["*"]' --strict-json
```
...
```bash
astroclaw config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
astroclaw config set models.providers.ollama.models '[{"id":"llama3.2","name":"Llama 3.2"}]' --strict-json --merge
```
...
`astroclaw config set` supports four assignment styles:

<Tabs>
  <Tab title="Value mode">
    ```bash
    astroclaw config set <path> <value>
    ```
  </Tab>
  <Tab title="SecretRef builder mode">
    ```bash
    astroclaw config set channels.discord.token \
  ...
  <Tab title="Provider builder mode">
    ...
    ```bash
    astroclaw config set secrets.providers.vault \
      --provider-source exec \
      --provider-command /usr/local/bin/astroclaw-vault \
      --provider-arg read \
  ...
  <Tab title="Batch mode">
    ```bash
    astroclaw config set --batch-json '[
      {
...
```bash
astroclaw config patch --file ./astroclaw.patch.json5 --dry-run
astroclaw config patch --file ./astroclaw.patch.json5
```

You can also pipe a patch over stdin, which is useful for remote setup scripts:

```bash
ssh astroclaw-host 'astroclaw config patch --stdin --dry-run' < ./astroclaw.patch.json5
ssh astroclaw-host 'astroclaw config patch --stdin' < ./astroclaw.patch.json5
```
...
```bash
astroclaw config patch --file ./discord.patch.json5 --replace-path 'channels.discord.guilds["123"].channels'
```
...
astroclaw config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/astroclaw/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `astroclaw.json`.
...
<Tabs>
  <Tab title="Success example">
    ```json
    {
      "ok": true,
      "operations": 1,
      "configPath": "~/.astroclaw/astroclaw.json",
      "inputModes": ["builder"],
      "checks": {
        "schema": false,
        "resolvability": true,
...
`astroclaw config set` and other Astroclaw-owned config writers validate the full post-change config before committing it to disk. If the new payload fails schema validation or looks like a destructive clobber, the active config is left alone and the rejected payload is saved beside it as `astroclaw.json.rejected.*`.

<Warning>
The active config path must be a regular file. Symlinked `astroclaw.json` layouts are unsupported for writes; use `ASTROCLAW_CONFIG_PATH` to point directly at the real file instead.
</Warning>

Prefer CLI writes for small edits:

```bash
astroclaw config set gateway.reload.mode hybrid --dry-run
astroclaw config set gateway.reload.mode hybrid
astroclaw config validate
```
...
Direct editor writes are still allowed, but the running Gateway treats them as untrusted until they validate. Invalid direct edits fail startup or are skipped by hot reload; Gateway does not rewrite `astroclaw.json`. Run `astroclaw doctor --fix` to repair prefixed/clobbered config or restore the last-known-good copy. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-rejected-invalid-config).
...
- `config file`: Print the active config file path (resolved from `ASTROCLAW_CONFIG_PATH` or default location). The path should name a regular file, not a symlink.
...
After `astroclaw config validate` is passing, you can use the local TUI to have an embedded agent compare the active config against the docs while you validate each change from the same terminal:

<Note>
If validation is already failing, start with `astroclaw configure` or `astroclaw doctor --fix`. `astroclaw chat` does not bypass the invalid-config guard.
</Note>
...
```text
!astroclaw config file
!astroclaw docs gateway auth token secretref
!astroclaw config validate
!astroclaw doctor
```
...
<Steps>
  ...
  <Step title="Apply targeted edits">
    Apply targeted edits with `astroclaw config set` or `astroclaw configure`.
  </Step>
  <Step title="Re-validate">
    Rerun `astroclaw config validate` after each change.
  </Step>
  <Step title="Doctor for runtime issues">
    If validation passes but the runtime is still unhealthy, run `astroclaw doctor` or `astroclaw doctor --fix` for migration and repair help.
  </Step>
</Steps>
