---
summary: "CLI reference for `astroclaw migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into Astroclaw
...
```bash
astroclaw migrate list
astroclaw migrate claude --dry-run
astroclaw migrate codex --dry-run
astroclaw migrate codex --skill gog-vault77-google-workspace
astroclaw migrate codex --plugin google-calendar --dry-run
astroclaw migrate codex --plugin google-calendar --verify-plugin-apps --dry-run
astroclaw migrate hermes --dry-run
astroclaw migrate hermes
astroclaw migrate apply codex --yes --skill gog-vault77-google-workspace
astroclaw migrate apply codex --yes --plugin google-calendar
astroclaw migrate apply codex --yes
astroclaw migrate apply claude --yes
astroclaw migrate apply hermes --yes
astroclaw migrate apply hermes --include-secrets --yes
astroclaw onboard --flow import
astroclaw onboard --import-from claude --import-source ~/.claude
astroclaw onboard --import-from hermes --import-source ~/.hermes
```

<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `astroclaw migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local Astroclaw state exists.
</ParamField>
...
`astroclaw migrate` is preview-first.
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `astroclaw migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an Astroclaw backup before applying the migration. If no local Astroclaw state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the Astroclaw agent workspace.
...
- Claude command Markdown files converted into Astroclaw skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. Astroclaw does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the Astroclaw Codex harness and you want to
...
Running `astroclaw migrate codex` in an interactive terminal previews the full
...
start checked unless the target Astroclaw Codex plugin config already has that
...
```bash
astroclaw migrate codex --dry-run --skill gog-vault77-google-workspace
astroclaw migrate apply codex --yes --skill gog-vault77-google-workspace
```
...
```bash
astroclaw migrate codex --dry-run --plugin google-calendar
astroclaw migrate apply codex --yes --plugin google-calendar
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  Astroclaw agent workspace when you want per-agent ownership.
...
- `SOUL.md` and `AGENTS.md` into the Astroclaw agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for Astroclaw file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that Astroclaw cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live Astroclaw config or credentials. This preserves opaque or unsafe state without pretending Astroclaw can execute or trust it automatically:
...
Migration sources are plugins. A plugin declares its provider ids in `astroclaw.plugin.json`:
...
Provider plugins can use `astroclaw/plugin-sdk/migration` for item construction and summary counts, plus `astroclaw/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `astroclaw onboard --flow import` and `astroclaw setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh Astroclaw setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move Astroclaw to a new machine.
