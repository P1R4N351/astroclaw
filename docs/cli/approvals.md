---
summary: "CLI reference for `astroclaw approvals` and `astroclaw exec-policy`"
read_when:
...
Alias: `astroclaw exec-approvals`
...
## `astroclaw exec-policy`

`astroclaw exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.astroclaw/exec-approvals.json`

Examples:

```bash
astroclaw exec-policy show
astroclaw exec-policy show --json

astroclaw exec-policy preset yolo
astroclaw exec-policy preset cautious --json

astroclaw exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `astroclaw exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `astroclaw approvals set --gateway`
or `astroclaw approvals set --node <id|name|ip>`.

## Common commands

```bash
astroclaw approvals get
astroclaw approvals get --node <id|name|ip>
astroclaw approvals get --gateway
```

`astroclaw approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
astroclaw approvals set --file ./exec-approvals.json
astroclaw approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
astroclaw approvals set --node <id|name|ip> --file ./exec-approvals.json
astroclaw approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested Astroclaw policy aligned, also set:
...
- `--node` uses the same resolver as `astroclaw nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.astroclaw/exec-approvals.json`.
