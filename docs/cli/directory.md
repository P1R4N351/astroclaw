---
summary: "CLI reference for `astroclaw directory` (self, peers, groups)"
read_when:
...
- `directory` is meant to help you find IDs you can paste into other commands (especially `astroclaw message send --target ...`).
...
```bash
astroclaw directory peers list --channel slack --query "U0"
astroclaw message send --channel slack --target user:U012ABCDEF --message "hello"
```
...
```bash
astroclaw directory peers list --channel zalouser
astroclaw directory peers list --channel zalouser --query "name"
astroclaw directory peers list --channel zalouser --limit 50
```

## Groups

```bash
astroclaw directory groups list --channel zalouser
