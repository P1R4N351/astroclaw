---
summary: "CLI reference for `astroclaw node` (headless node host)"
read_when:
...
`astroclaw node run` and `astroclaw node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `ASTROCLAW_GATEWAY_TOKEN` / `ASTROCLAW_GATEWAY_PASSWORD` are checked first.
...
trusted private-DNS names, set `ASTROCLAW_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `astroclaw.json` config
key.
`astroclaw node install` persists it into the supervised node service when it is
...
```bash
astroclaw node install --host <gateway-host> --port 18789
```
...
```bash
astroclaw node status
astroclaw node start
astroclaw node stop
astroclaw node restart
astroclaw node uninstall
```

Use `astroclaw node run` for a foreground node host (no service).
...
- `astroclaw approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, Astroclaw prepares a canonical `systemRunPlan`
