---
summary: "CLI reference for `astroclaw tasks` (background task ledger and Task Flow state)"
read_when:
  - You want to inspect, audit, or cancel background task records
  - You are documenting Task Flow commands under `astroclaw tasks flow`
title: "`astroclaw tasks`"
---

Inspect durable background tasks and Task Flow state. With no subcommand,
`astroclaw tasks` is equivalent to `astroclaw tasks list`.
...
```bash
astroclaw tasks list [--runtime <name>] [--status <name>] [--json]
```
...
```bash
astroclaw tasks show <lookup> [--json]
```
...
```bash
astroclaw tasks audit [--severity <warn|error>] [--code <name>] [--limit <n>] [--json]
```
...
```bash
astroclaw tasks flow list [--status <name>] [--json]
