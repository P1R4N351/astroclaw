---
summary: "Expose Astroclaw channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Astroclaw-backed channels
  - Running `astroclaw mcp serve`
  - Managing Astroclaw-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`astroclaw mcp` has two jobs:

- run Astroclaw as an MCP server with `astroclaw mcp serve`
- manage Astroclaw-owned outbound MCP server definitions with `list`, `show`, `set`, and `unset`

In other words:

- `serve` is Astroclaw acting as an MCP server
- `list` / `show` / `set` / `unset` is Astroclaw acting as an MCP client-side registry for other MCP servers its runtimes may consume later

Use [`astroclaw acp`](/cli/acp) when Astroclaw should host a coding harness session itself and route that runtime through ACP.

## Astroclaw as an MCP server

This is the `astroclaw mcp serve` path.

### When to use `serve`

Use `astroclaw mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to Astroclaw-backed channel conversations
- you already have a local or remote Astroclaw Gateway with routed sessions
- you want one MCP server that works across Astroclaw's channel backends instead of running separate per-channel bridges

Use [`astroclaw acp`](/cli/acp) instead when Astroclaw should host the coding runtime itself and keep the agent session inside Astroclaw.

### How it works

`astroclaw mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote Astroclaw Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `astroclaw mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the Astroclaw Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `astroclaw agent` and `astroclaw infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by Astroclaw (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when Astroclaw already has session state with a known route such as:
...
<Tabs>
  ...
  <Tab title="Remote Gateway (token)">
    ```bash
    astroclaw mcp serve --url wss://gateway-host:18789 --token-file ~/.astroclaw/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    astroclaw mcp serve --url wss://gateway-host:18789 --password-file ~/.astroclaw/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    astroclaw mcp serve --verbose
    astroclaw mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the Astroclaw equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying Astroclaw channel configuration
...
Astroclaw ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `astroclaw mcp serve`
...
## Astroclaw as an MCP client registry

This is the `astroclaw mcp list`, `show`, `set`, and `unset` path.

These commands do not expose Astroclaw over MCP. They manage Astroclaw-owned MCP server definitions under `mcp.servers` in Astroclaw config.

Those saved definitions are for runtimes that Astroclaw launches or configures later, such as embedded Pi and other runtime adapters. Astroclaw stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write Astroclaw config
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded Pi consumes Astroclaw `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

Codex app-server also honors an optional `codex` block on each server. This is
Astroclaw projection metadata for Codex app-server threads only; it does not
change ACP sessions, generic Codex harness config, or other runtime adapters.
Use non-empty `codex.agents` to project a server only into specific Astroclaw
...
Astroclaw strips the `codex` metadata before handing the native `mcp_servers`
config to Codex.

### Saved MCP server definitions

Astroclaw also stores a lightweight MCP server registry in config for surfaces that want Astroclaw-managed MCP definitions.

Commands:

- `astroclaw mcp list`
- `astroclaw mcp show [name]`
- `astroclaw mcp set <name> <json>`
- `astroclaw mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `astroclaw mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
astroclaw mcp list
astroclaw mcp show context7 --json
astroclaw mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
astroclaw mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
astroclaw mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, Astroclaw uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |

Astroclaw config uses `transport: "streamable-http"` as the canonical spelling. CLI-native MCP `type: "http"` values are accepted when saved through `astroclaw mcp set` and repaired by `astroclaw doctor --fix` in existing config, but `transport` is what embedded Pi consumes directly.
