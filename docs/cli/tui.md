---
summary: "CLI reference for `astroclaw tui` (Gateway-backed or local embedded terminal UI)"
read_when:
  - You want a terminal UI for the Gateway (remote-friendly)
  - You want to pass url/token/session from scripts
  - You want to run the TUI in local embedded mode without a Gateway
  - You want to use astroclaw chat or astroclaw tui --local
title: "TUI"
---
...
Aliases: `astroclaw chat` and `astroclaw terminal` invoke the same command with `--local` implied.

Notes:

- `chat` and `terminal` are aliases for `astroclaw tui --local`.
...
astroclaw tui --local
astroclaw tui
astroclaw tui --url ws://127.0.0.1:18789 --token <token>
astroclaw tui --session main --deliver
astroclaw chat --message "Compare my config to the docs and tell me what to fix"
# when run inside an agent workspace, infers that agent automatically
astroclaw tui --session bugfix
```
...
If `astroclaw config validate` is already failing, use `astroclaw configure` or
`astroclaw doctor --fix` first. `astroclaw chat` does not bypass the invalid-
config guard.
...
Apply targeted fixes with `astroclaw config set` or `astroclaw configure`, then
rerun `astroclaw config validate`. See [TUI](/web/tui) and [Config](/cli/config).
