---
summary: "CLI reference for `astroclaw models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
astroclaw models set <model-or-alias>
astroclaw models scan
```

`astroclaw models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise Astroclaw falls back to matching OAuth/API-key
...
the command uses `ASTROCLAW_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
configured default agent.
Probe rows can come from auth profiles, env credentials, or `models.json`.
For Codex OAuth troubleshooting, `astroclaw models status`,
`astroclaw models auth list --provider openai-codex`, and
`astroclaw config get agents.defaults.model --json` are the quickest way to
...
- If you omit the provider, Astroclaw resolves the input as an alias first, then
  ...
  If that provider no longer exposes the configured default model, Astroclaw
...
By default Astroclaw tries to probe tool and image support with live model calls.
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `ASTROCLAW_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but Astroclaw could not resolve a probeable
...
```bash
astroclaw models auth add
astroclaw models auth list [--provider <id>] [--json]
astroclaw models auth login --provider <id>
astroclaw models auth setup-token --provider <id>
astroclaw models auth paste-token
```
...
`astroclaw plugins list` to see which providers are installed.
Use `astroclaw models auth --agent <id> <subcommand>` to write auth results to a
...
```bash
astroclaw models auth login --provider openai --set-default
astroclaw models auth login --provider openai --method api-key
astroclaw models auth list --provider openai
```
...
- Anthropic note: Anthropic staff told us Astroclaw-style Claude CLI usage is allowed again, so Astroclaw treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic `setup-token` / `paste-token` remain available as a supported Astroclaw token path, but Astroclaw now prefers Claude CLI reuse and `claude -p` when available.
