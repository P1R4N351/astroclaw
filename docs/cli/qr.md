---
summary: "CLI reference for `astroclaw qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
astroclaw qr
astroclaw qr --setup-code-only
astroclaw qr --json
astroclaw qr --remote
astroclaw qr --url wss://gateway.example/ws
```
...
- With `--remote`, Astroclaw requires either `gateway.remote.url` or
