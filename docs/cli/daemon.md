---
summary: "CLI reference for `astroclaw daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `astroclaw daemon ...` in scripts
...
`astroclaw daemon ...` maps to the same service control surface as `astroclaw gateway ...` service commands.

## Usage

```bash
astroclaw daemon status
astroclaw daemon install
astroclaw daemon start
astroclaw daemon stop
astroclaw daemon restart
astroclaw daemon uninstall
```
...
- `restart --safe --skip-deferral` runs the Astroclaw-aware safe restart but bypasses the active-work deferral gate so the Gateway emits the restart immediately even when blockers are reported. Operator escape hatch when a stuck task run pins the safe restart; requires `--safe`.

## Prefer

Use [`astroclaw gateway`](/cli/gateway) for current docs and examples.
