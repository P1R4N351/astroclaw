---
summary: "CLI reference for `astroclaw logs` (tail gateway logs via RPC)"
read_when:
...
`astroclaw logs` also accepts the standard Gateway client flags:
...
astroclaw logs --follow --interval 2000
astroclaw logs --limit 500 --max-bytes 500000
astroclaw logs --json
astroclaw logs --plain
astroclaw logs --no-color
astroclaw logs --limit 500
astroclaw logs --local-time
astroclaw logs --follow --local-time
astroclaw logs --url ws://127.0.0.1:18789 --token "$ASTROCLAW_GATEWAY_TOKEN"
```

## Notes

- Use `--local-time` to render timestamps in your local timezone.
- If the implicit local loopback Gateway asks for pairing, closes during connect, or times out before `logs.tail` answers, `astroclaw logs` falls back to the configured Gateway file log automatically. Explicit `--url` targets do not use this fallback.
