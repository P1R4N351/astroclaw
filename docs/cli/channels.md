---
summary: "CLI reference for `astroclaw channels` (accounts, status, login/logout, logs)"
read_when:
...
Do not use `astroclaw sessions`, Gateway `sessions.list`, or the agent
...
```bash
astroclaw channels add --channel telegram --token <bot-token>
astroclaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
astroclaw channels remove --channel telegram --delete
```

<Tip>
`astroclaw channels add --help` shows per-channel flags (token, private key, app token, signal-cli paths, etc).
</Tip>
...
If a channel plugin needs to be installed during a flag-driven add command, Astroclaw uses the channel's default install source without opening the interactive plugin install prompt.

When you run `astroclaw channels add` without flags, the interactive wizard can prompt:
...
You can also manage the same routing rules later with `astroclaw agents bindings`, `astroclaw agents bind`, and `astroclaw agents unbind` (see [agents](/cli/agents)).

When you add a non-default account to a channel that is still using single-account top-level settings, Astroclaw promotes account-scoped top-level values into the channel's account map before writing the new account. Most channels land those values in `channels.<channel>.accounts.default`, but bundled channels can preserve an existing matching promoted account instead. Matrix is the current example: if one named account already exists, or `defaultAccount` points at an existing named account, promotion preserves that account instead of creating a new `accounts.default`.
...
- Run `astroclaw status --deep` for a broad probe.
- Use `astroclaw doctor` for guided fixes.
- `astroclaw channels list` no longer prints model provider usage/quota snapshots. For those, use `astroclaw status` (overview) or `astroclaw models list` (per-provider).
- `astroclaw channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
...
```bash
astroclaw channels resolve --channel slack "#general" "@jane"
astroclaw channels resolve --channel discord "My Server/#support" "@someone"
astroclaw channels resolve --channel matrix "Project Room"
```
