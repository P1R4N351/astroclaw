---
summary: "CLI reference for `astroclaw agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
astroclaw agent --to +15555550123 --message "status update" --deliver
astroclaw agent --agent ops --message "Summarize logs"
astroclaw agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
...
astroclaw agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: Astroclaw persists markers from the active source config snapshot, not from resolved runtime secret values.
