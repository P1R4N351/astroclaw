---
summary: "CLI reference for `astroclaw security` (audit and fix common security footguns)"
read_when:
...
astroclaw security audit --deep --password <password>
astroclaw security audit --deep --token <token>
...
It also emits `security.trust_model.multi_user_heuristic` when config suggests likely shared-user ingress (for example open DM/group policy, configured group targets, or wildcard sender rules), and reminds you that Astroclaw is a personal-assistant trust model by default.
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `astroclaw.browserConfigEpoch`) and recommends `astroclaw sandbox recreate --browser --all`.
...
```bash
astroclaw security audit --json | jq '.summary'
astroclaw security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
astroclaw security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
...
- tightens permissions for state/config and common sensitive files
  ...
  `*.jsonl`)
- also tightens config include files referenced from `astroclaw.json`
