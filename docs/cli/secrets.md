---
summary: "CLI reference for `astroclaw secrets` (reload, audit, configure, apply)"
read_when:
...
Use `astroclaw secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
astroclaw secrets reload
astroclaw secrets reload --json
astroclaw secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan Astroclaw state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `astroclaw.json` refs)
...
```bash
astroclaw secrets audit
astroclaw secrets audit --check
astroclaw secrets audit --json
astroclaw secrets audit --allow-exec
```
...
```bash
astroclaw secrets configure
astroclaw secrets configure --plan-out /tmp/astroclaw-secrets-plan.json
astroclaw secrets configure --apply --yes
astroclaw secrets configure --providers-only
astroclaw secrets configure --skip-provider-setup
astroclaw secrets configure --agent ops
astroclaw secrets configure --json
```
...
- `configure` targets secret-bearing fields in `astroclaw.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Astroclaw fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
astroclaw secrets apply --from /tmp/astroclaw-secrets-plan.json --json
```
...
- `astroclaw.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.astroclaw/.env` known secret keys whose values were migrated
