---
summary: "CLI reference for `astroclaw message` (send + channel actions)"
read_when:
...
```
astroclaw message <subcommand> [flags]
```
...
- `astroclaw message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, Astroclaw will attempt a live directory lookup when the provider supports it.
...
- `astroclaw message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Astroclaw does not force a `default` account SecretRef scope
...
```
astroclaw message send --channel discord \
...
```bash
astroclaw message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
astroclaw message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
astroclaw message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
astroclaw message poll --channel msteams \
...
```
astroclaw message react --channel slack \
...
```
astroclaw message react --channel signal \
...
```
astroclaw message send --channel telegram --target @mychat --message "Choose:" \
...
```
astroclaw message send --channel telegram --target 123456789 --message "Open app:" \
...
```bash
astroclaw message send --channel telegram --target @mychat \
