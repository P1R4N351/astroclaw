---
summary: "Bundled `oc-path` plugin: ships the `astroclaw path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`astroclaw path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the Astroclaw repo under
...
- **Local automation**: shell scripts can resolve or update one workspace value
  with `astroclaw path … --json` instead of carrying separate markdown, JSONC,
...
astroclaw path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
astroclaw path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `astroclaw` CLI** on the host where you
...
The plugin metadata lives in `extensions/oc-path/astroclaw.plugin.json`:
...
`astroclaw path …`, so installs that never use the verb pay no cost.
...
state. Bare `astroclaw path` invocations work immediately on the same host —
the CLI loads the plugin on demand.

Disable with:

```bash
astroclaw plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `astroclaw path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
`__ASTROCLAW_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`astroclaw path` CLI reference](/cli/path)
