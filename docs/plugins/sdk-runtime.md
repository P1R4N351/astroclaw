...
`astroclaw/plugin-sdk/config-runtime` compatibility barrel: `config-contracts` for
...
Use `astroclaw/plugin-sdk/pair-loop-guard-runtime` directly only for custom
...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    `runEmbeddedAgent(...)` is the neutral helper for starting a normal Astroclaw agent turn from plugin code. It uses the same provider/model resolution and agent-harness selection as channel-triggered replies.
  ...
  <Accordion title="api.runtime.llm">
    ...
    duplicating Astroclaw model/auth/base URL preparation.
    ...
    The helper uses the same simple-completion preparation path as Astroclaw's
  ...
  <Accordion title="api.runtime.nodes">
    ...
    Inside the Gateway this runtime is in-process. In plugin CLI commands it calls the configured Gateway over RPC, so commands such as `astroclaw googlemeet recover-tab` can inspect paired nodes from the terminal. Node commands still go through normal Gateway node pairing, command allowlists, plugin node-invoke policies, and node-local command handling.
  ...
  <Accordion title="api.runtime.tasks.managedFlows">
    Bind a Task Flow runtime to an existing Astroclaw session key or trusted tool context, then create and manage Task Flows without passing an owner on every call.
    ...
    Use `bindSession({ sessionKey, requesterOrigin })` when you already have a trusted Astroclaw session key from your own binding layer. Do not bind from raw user input.
  ...
  <Accordion title="api.runtime.webSearch">
    ...
    const result = await api.runtime.webSearch.search({
      config: api.config,
      args: { query: "Astroclaw plugin SDK", count: 5 },
    });
    ```
  ...
  <Accordion title="api.runtime.media">
    ...
    const terminalQr = await api.runtime.media.renderQrTerminal("https://astroclaw.ai");
    const pngQr = await api.runtime.media.renderQrPngBase64("https://astroclaw.ai", {
      scale: 6, // 1-12
      marginModules: 4, // 0-16
    });
    const pngQrDataUrl = await api.runtime.media.renderQrPngDataUrl("https://astroclaw.ai");
    const tmpRoot = resolvePreferredAstroclawTmpDir();
    const pngQrFile = await api.runtime.media.writeQrPngTempFile("https://astroclaw.ai", {
      tmpRoot,
...
<ParamField path="api.config" type="AstroclawConfig">
