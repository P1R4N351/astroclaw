...
```bash
astroclaw plugins install @astroclaw/memory-lancedb
```

The plugin is published to npm and is not bundled into the Astroclaw runtime image.
...
Set `dimensions` for non-standard embedding models. Astroclaw knows the
...
`memory_forget` query path, and `astroclaw ltm search`. Auto-recall prefers the
...
```bash
astroclaw ltm list
astroclaw ltm search "project preferences"
astroclaw ltm stats
```

The plugin also extends `astroclaw memory` with a non-vector `query` subcommand
that runs against the LanceDB table directly:

```bash
astroclaw memory query --cols id,text,createdAt --limit 20
astroclaw memory query --filter "category = 'preference'" --order-by createdAt:desc
```
...
By default, LanceDB data lives under `~/.astroclaw/memory/lancedb`. Override the
path with `dbPath`:
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "~/.astroclaw/memory/lancedb",
          embedding: {
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "s3://memory-bucket/astroclaw",
          storageOptions: {
...
Astroclaw treats that package as part of the plugin package. Gateway startup
...
`@lancedb/lancedb` error during plugin load, upgrade Astroclaw and restart the
Gateway.
...
```bash
astroclaw ltm stats
astroclaw ltm search "recent preference"
```
