---
summary: "Set up Codex Computer Use for Codex-mode Astroclaw agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode Astroclaw agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. Astroclaw
...
Use this page when Astroclaw is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## Astroclaw.app and Peekaboo

Astroclaw.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want Astroclaw.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode Astroclaw agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an Astroclaw node and exposes mobile
...
Astroclaw-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through Astroclaw's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the Astroclaw command:

```bash
cua-driver mcp-config --client astroclaw
```

or register the stdio server yourself:

```bash
astroclaw mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
```
...
available as a normal Astroclaw MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. Astroclaw
...
With this config, Astroclaw checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, Astroclaw asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, Astroclaw also tries to
...
plugin command surface is available. These are Astroclaw chat/runtime commands,
not `astroclaw codex ...` CLI subcommands:
...
Astroclaw uses the same app-server API that Codex itself exposes. The
...
During install, Astroclaw polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, Astroclaw prefers
...
`computer-use` is registered, Astroclaw tries to add the standard bundled
...
## What Astroclaw checks

Astroclaw reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If Astroclaw says Computer Use
...
Astroclaw intentionally fails closed when `computerUse.enabled` is true. A
...
fresh Astroclaw session.

**A Computer Use tool says `Native hook relay unavailable`.** The Codex-native
tool hook could not reach an active Astroclaw relay through the local bridge or
Gateway fallback. Start a fresh Astroclaw session with `/new` or `/reset`. If it
