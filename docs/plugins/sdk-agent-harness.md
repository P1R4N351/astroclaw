...
An **agent harness** is the low level executor for one prepared Astroclaw agent
...
runtime and the normal Astroclaw provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the Astroclaw
  session transcript
...
Before a harness is selected, Astroclaw has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the Astroclaw transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an Astroclaw-owned
...
**Import:** `astroclaw/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "astroclaw/plugin-sdk/agent-harness";
...
Astroclaw chooses a harness after provider/model resolution:
...
3. `auto` asks registered harnesses if they support the resolved
   provider/model.
4. If no registered harness matches, Astroclaw uses PI unless PI fallback is
   disabled.
...
provider/model. Once a plugin harness has claimed a run, Astroclaw does not
...
`agents.list[].agentRuntime`, and `ASTROCLAW_AGENT_RUNTIME`. `/status` shows the
...
Astroclaw. The harness then claims that provider in `supports(...)`.
...
- app-server request: Astroclaw sends the bare model id to Codex and lets the
...
Astroclaw requires Codex app-server `0.125.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
Astroclaw only runs against the protocol surface it has been tested with. The
`0.125.0` floor includes the native MCP hook payload support that landed in
Codex `0.124.0`, while pinning Astroclaw to the newer tested stable line.
...
`astroclaw/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so Astroclaw's fallback policy can decide whether to retry on a
...
The bundled `codex` harness is the native Codex mode for embedded Astroclaw
...
`astroclaw doctor --fix`, and legacy `codex/*` model refs remain compatibility
aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. Astroclaw still owns the chat channel,
...
By default, Astroclaw uses `auto` provider/model runtime policy: registered
...
Keep that binding explicitly associated with the Astroclaw session, and keep
mirroring user-visible assistant/tool output into the Astroclaw transcript.

The Astroclaw transcript remains the compatibility layer for:
...
If your harness stores a sidecar binding, implement `reset(...)` so Astroclaw can
clear it when the owning Astroclaw session is reset.

## Tool and media results

Core constructs the Astroclaw tool list and passes it into the prepared attempt.
