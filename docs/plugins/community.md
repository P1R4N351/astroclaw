---
summary: "Find and publish community-maintained Astroclaw plugins"
read_when:
  - You want to find third-party Astroclaw plugins
  - You want to publish or list your own plugin on ClawHub
title: "Community plugins"
doc-schema-version: 1
---

Community plugins are third-party packages that extend Astroclaw with channels,
...
```bash
astroclaw plugins install clawhub:<package-name>
```

npm remains a supported direct-install path during the launch cutover:

```bash
astroclaw plugins install npm:<package-name>
```

Use [Manage plugins](/plugins/manage-plugins) for common install, update,
inspect, and uninstall examples. Use [`astroclaw plugins`](/cli/plugins) for the
full command reference and source-selection rules.

## Publish plugins

Publish public community plugins on ClawHub when you want Astroclaw users to
...
| Requirement          | Why                                                 |
| -------------------- | --------------------------------------------------- |
| Published on ClawHub | Users need `astroclaw plugins install` hints to work |
