...
read_when:
  - You are building an Astroclaw plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page is for the **native Astroclaw plugin manifest** only.
...
Astroclaw auto-detects those bundle layouts too, but they are not validated
against the `astroclaw.plugin.json` schema described here.

For compatible bundles, Astroclaw currently reads bundle metadata plus declared
...
Astroclaw runtime expectations.

Every native Astroclaw plugin **must** ship a `astroclaw.plugin.json` file in the
**plugin root**. Astroclaw uses this manifest to validate configuration
...
`astroclaw.plugin.json` is the metadata Astroclaw reads **before it loads your
...
- QA runner metadata the shared `astroclaw qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`       | Deprecated compatibility env metadata for provider auth/status lookup. Prefer `setup.providers[].envVars` for new plugins; Astroclaw still reads this during the deprecation window.                                                 |
| `providerAuthAliases`                | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                          |
| `channelEnvVars`                     | No       | `Record<string, string[]>`       | Cheap channel env metadata that Astroclaw can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                            |
...
| `qaRunners`                          | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `astroclaw qa` host before plugin runtime loads.                                                                                                                                      |
...
Astroclaw reads these fields before provider runtime loads so core tools can
...
ownership. `toolMetadata` declares cheap availability evidence so Astroclaw can
...
If a tool has no `toolMetadata`, Astroclaw preserves the existing behavior and
...
Astroclaw reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, Astroclaw falls back to `choiceId`.                                        |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. Astroclaw
...
the shared `astroclaw qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `astroclaw qa`, for example `matrix`.    |
...
Astroclaw also includes `setup.providers[].envVars` in generic provider auth and
...
Astroclaw can also derive simple setup choices from `setup.providers[].authMethods`
...
setup surface. Astroclaw treats explicit `false` as a descriptor-only contract
and will not execute `setup-api` or `astroclaw.setupEntry` for setup lookup. If
a descriptor-only plugin still ships one of those setup runtime entries,
Astroclaw reports an additive diagnostic and continues ignoring it. Omitted
...
Use `contracts` only for static capability ownership metadata that Astroclaw can
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for `astroclaw migrate`.                                        |
...
Astroclaw reads manifest metadata to decide which plugin owns that configured
...
`channelConfigs` entries. Without them, Astroclaw can still load the plugin, but
...
the same defaults through `package.json#astroclaw.channel.commands` alongside
...
When `channels.chat` is configured, Astroclaw considers both the channel id and
the preferred plugin id. If the lower-priority plugin was only selected because
it is bundled or enabled by default, Astroclaw disables it in the effective
runtime config so one plugin owns the channel and its tools. Explicit user
selection still wins: if the user explicitly enables both plugins, Astroclaw
...
Use `modelSupport` when Astroclaw should infer your provider plugin from
...
Astroclaw applies this precedence:
...
Use `modelCatalog` when Astroclaw should know provider model metadata before
...
provider-filtered list uses an alias, Astroclaw can read the owning manifest and
...
| `compat`        | `object`                                                       | Optional compatibility flags matching Astroclaw model config compatibility.  |
...
refreshable rows are not authoritative by themselves. Use `runtime` when Astroclaw
...
| Field                      | Type               | What it means                                                                                                        |
| -------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `provider`                 | `string`           | External catalog provider id when it differs from the Astroclaw provider id, for example `z-ai` for a `zai` provider. |
...
### Astroclaw Provider Index

The Astroclaw Provider Index is Astroclaw-owned preview metadata for providers
...
4. Astroclaw Provider Index preview rows.
...
Legacy top-level capability keys are deprecated. Use `astroclaw doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `astroclaw.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `astroclaw` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Astroclaw must know it before loading plugin code, put it in `astroclaw.plugin.json`
...
`astroclaw` block instead of `astroclaw.plugin.json`.
`astroclaw.bundle` and `astroclaw.bundle.json` are not Astroclaw plugin contracts;
native plugins must use `astroclaw.plugin.json` plus the supported
`package.json#astroclaw` fields below.

Important examples:

| Field                                                                                      | What it means                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `astroclaw.extensions`                                                                      | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `astroclaw.runtimeExtensions`                                                               | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `astroclaw.setupEntry`                                                                      | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `astroclaw.runtimeSetupEntry`                                                               | Declares the built JavaScript setup entrypoint for installed packages. Requires `setupEntry`, must exist, and must stay inside the plugin package directory.                         |
| `astroclaw.channel`                                                                         | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `astroclaw.channel.commands`                                                                | Static native command and native skill auto-default metadata used by config, audit, and command-list surfaces before channel runtime loads.                                          |
| `astroclaw.channel.configuredState`                                                         | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `astroclaw.channel.persistedAuthState`                                                      | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `astroclaw.install.clawhubSpec` / `astroclaw.install.npmSpec` / `astroclaw.install.localPath` | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `astroclaw.install.defaultChoice`                                                           | Preferred install path when multiple install sources are available.                                                                                                                  |
| `astroclaw.install.minHostVersion`                                                          | Minimum supported Astroclaw host version, using a semver floor like `>=2026.3.22` or `>=2026.5.1-beta.1`.                                                                             |
| `astroclaw.install.expectedIntegrity`                                                       | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `astroclaw.install.allowInvalidConfigRecovery`                                              | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `astroclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`                          | Lets setup-only channel surfaces load before the full channel plugin during startup.                                                                                                 |

Manifest metadata decides which provider/channel/setup choices appear in
onboarding before runtime loads. `package.json#astroclaw.install` tells
onboarding how to fetch or enable that plugin when the user picks one of those
choices. Do not move install hints into `astroclaw.plugin.json`.

`astroclaw.install.minHostVersion` is enforced during install and manifest
...
`"npmSpec": "@wecom/wecom-astroclaw-plugin@1.2.3"`. Official external catalog
...
Channel plugins should provide `astroclaw.setupEntry` when status, channel list,
...
entrypoint fields. For example, `astroclaw.runtimeExtensions` cannot make an
escaping `astroclaw.extensions` path loadable.

`astroclaw.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
to `astroclaw doctor --fix`.

`astroclaw.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`astroclaw.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
Astroclaw discovers plugins from several roots. For the raw filesystem scan
...
2. **Bundled** — plugins shipped with Astroclaw
3. **Global install** — plugins installed into the global Astroclaw plugin root
...
- When extending or forking a bundled plugin with new config keys, update that plugin's `astroclaw.plugin.json` `configSchema` at the same time. Bundled plugin schemas are strict, so adding `plugins.entries.<id>.config.myNewKey` in user config without adding `myNewKey` to `configSchema.properties` will be rejected before the plugin runtime loads.
...
- The manifest is **required for native Astroclaw plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
...
- Declare exclusive plugin kind in this manifest. Runtime-entry `AstroclawPluginDefinition.kind` is deprecated and remains only as a compatibility fallback for older plugins.
