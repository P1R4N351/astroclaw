...
when Astroclaw loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, Astroclaw will use a
...
**Import:** `astroclaw/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | -                   |
| `configSchema` | `AstroclawPluginConfigSchema \| () => AstroclawPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: AstroclawPluginApi) => void`                               | Yes      | -                   |

- `id` must match your `astroclaw.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- Astroclaw resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `astroclaw/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: AstroclawPluginApi) => void`                               | No       | -                   |
| `registerFull`        | `(api: AstroclawPluginApi) => void`                               | No       | -                   |
...
- Discovery registration is non-activating, not import-free. Astroclaw may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and Astroclaw
...
- For plugin-owned root CLI commands, prefer `api.registerCli(..., { descriptors: [...] })`
  ...
  `api.registerNodeCliFeature(...)` so the command lands under `astroclaw nodes`.
  For other nested plugin commands, add `parentPath` and register commands on
  the `program` object passed to the registrar; Astroclaw resolves it to the
...
Astroclaw loads this instead of the full entry when a channel is disabled,
...
- `astroclaw/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
...
- `astroclaw/plugin-sdk/channel-setup` for optional-install setup surfaces
- `astroclaw/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`astroclaw/plugin-sdk/channel-entry-contract` instead. That contract lets the
...
the plugin entry and the channel plugin object so Astroclaw can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Astroclaw to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; Astroclaw rejects descriptor names outside
...
Astroclaw classifies loaded plugins by their registration behavior:
...
Use `astroclaw plugins inspect <id>` to see a plugin's shape.
