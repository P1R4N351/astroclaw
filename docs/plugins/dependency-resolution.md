---
summary: "How Astroclaw installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged Astroclaw installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

Astroclaw keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the Astroclaw
package directory.
...
- SDK/core imports are peer or supplied Astroclaw imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into Astroclaw-owned package roots

Astroclaw owns only the plugin lifecycle:
...
Astroclaw uses stable per-source roots:

- npm packages install under `~/.astroclaw/npm`
- git packages clone under `~/.astroclaw/git`
...
`astroclaw plugins install npm-pack:<path.tgz>` uses that same managed npm root
for a local npm-pack tarball. Astroclaw reads the tarball's npm metadata, adds it
...
npm may hoist transitive dependencies to `~/.astroclaw/npm/node_modules` beside
the plugin package. Astroclaw scans the managed npm root before trusting the
...
Plugins that import `astroclaw/plugin-sdk/*` declare `astroclaw` as a peer
dependency. Astroclaw does not let npm install a separate registry copy of the
...
resolution/materialization for the shared root and Astroclaw reasserts
plugin-local `node_modules/astroclaw` links for installed packages that declare
...
Local plugins are treated as developer-controlled directories. Astroclaw does not
...
astroclaw plugins install <source>
astroclaw doctor --fix
```

`doctor --fix` can clean legacy Astroclaw-generated dependency state and recover
...
Lightweight and core-critical bundled plugins are shipped as part of Astroclaw.
...
In source checkouts, Astroclaw treats the repository as a pnpm monorepo. After
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g astroclaw`        | Built runtime tree inside the package | Astroclaw package and explicit plugin install/update/doctor flows     |
...
| `astroclaw plugins install ...`   | Managed npm/git/ClawHub plugin root   | The plugin install/update flow                                       |

## Legacy cleanup

Older Astroclaw versions generated bundled-plugin dependency roots at startup or
...
`.astroclaw-runtime-deps*` manifests, generated plugin `node_modules`, install
