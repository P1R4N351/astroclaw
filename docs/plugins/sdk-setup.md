...
read_when:
  ...
  - You are defining plugin config schemas or package.json astroclaw metadata
---

Reference for plugin packaging (`package.json` metadata), manifests (`astroclaw.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `astroclaw` field that tells the plugin system what your plugin provides:

<Tabs>
  <Tab title="Channel plugin">
    ```json
    {
      "name": "@myorg/astroclaw-my-channel",
      "version": "1.0.0",
      "type": "module",
      "astroclaw": {
        "extensions": ["./index.ts"],
  ...
  <Tab title="Provider plugin / ClawHub baseline">
    ```json astroclaw-clawhub-package.json
    {
      "name": "@myorg/astroclaw-my-plugin",
      "version": "1.0.0",
      "type": "module",
      "astroclaw": {
        "extensions": ["./index.ts"],
...
`astroclaw.channel` is cheap package metadata for channel discovery and setup surfaces before runtime loads.
...
`astroclaw.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`                            | Minimum supported Astroclaw version in the form `>=x.y.z` or `>=x.y.z-prerelease`. |
...
When enabled, Astroclaw loads only `setupEntry` during the pre-listen startup phase, even for already-configured channels. The full entry loads after the gateway starts listening.
...
Every native plugin must ship an `astroclaw.plugin.json` in the package root. Astroclaw uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Astroclaw",
  "configSchema": {
    "type": "object",
...
The `setup-entry.ts` file is a lightweight alternative to `index.ts` that Astroclaw loads when it only needs setup surfaces (onboarding, config repair, disabled channel inspection).
...
Bundled workspace channels that keep setup-safe exports in sidecar modules can use `defineBundledChannelSetupEntry(...)` from `astroclaw/plugin-sdk/channel-entry-contract` instead of `defineSetupPluginEntry(...)`. That bundled contract also supports an optional `runtime` export so setup-time runtime wiring can stay lightweight and explicit.

<AccordionGroup>
  <Accordion title="When Astroclaw uses setupEntry instead of the full entry">
...
CLI wizard locale (`ASTROCLAW_LOCALE`, then system locale variables) and falls
...
If you already author the contract as JSON Schema or TypeBox, use the direct helper so Astroclaw can skip Zod-to-JSON-Schema conversion on metadata paths:

```typescript
import { Type } from "typebox";
import { buildJsonChannelConfigSchema } from "astroclaw/plugin-sdk/channel-config-schema";
...
For third-party plugins, the cold-path contract is still the plugin manifest: mirror the generated JSON Schema into `astroclaw.plugin.json#channelConfigs` so config schema, setup, and UI surfaces can inspect `channels.<id>` without loading runtime code.

## Setup wizards

Channel plugins can provide interactive setup wizards for `astroclaw onboard`. The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "astroclaw/plugin-sdk/channel-setup";
...
<AccordionGroup>
  <Accordion title="Shared allowFrom prompts">
    For DM allowlist prompts that only need the standard `note -> prompt -> parse -> merge -> patch` flow, prefer the shared setup helpers from `astroclaw/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`, `createTopLevelChannelParsedAllowFromPrompt(...)`, and `createNestedChannelParsedAllowFromPrompt(...)`.
  </Accordion>
  <Accordion title="Standard channel setup status">
    For channel setup status blocks that only vary by labels, scores, and optional extra lines, prefer `createStandardChannelSetupStatus(...)` from `astroclaw/plugin-sdk/setup` instead of hand-rolling the same `status` object in each plugin.
  </Accordion>
  <Accordion title="Optional channel setup surface">
    For optional setup surfaces that should only appear in certain contexts, use `createOptionalChannelSetupSurface` from `astroclaw/plugin-sdk/channel-setup`:

    ```typescript
    import { createOptionalChannelSetupSurface } from "astroclaw/plugin-sdk/channel-setup";

    const setupSurface = createOptionalChannelSetupSurface({
      channel: "my-channel",
      label: "My Channel",
      npmSpec: "@myorg/astroclaw-my-channel",
      docsPath: "/channels/my-channel",
    });
...
<Tabs>
  <Tab title="npm">
    ```bash
    astroclaw plugins install @myorg/astroclaw-my-plugin
    ```
  ...
  <Tab title="ClawHub only">
    ```bash
    astroclaw plugins install clawhub:@myorg/astroclaw-my-plugin
    ```
  </Tab>
  <Tab title="npm package spec">
    ...
    ```bash
    astroclaw plugins install npm:@myorg/astroclaw-my-plugin
    ```
...
```bash
astroclaw plugins install <package-name>
```

<Info>
For npm-sourced installs, `astroclaw plugins install` installs the package under `~/.astroclaw/npm` with lifecycle scripts disabled. Keep plugin dependency trees pure JS/TS and avoid packages that require `postinstall` builds.
</Info>
...
Bundled package metadata is explicit, not inferred from built JavaScript at gateway startup. Runtime dependencies belong in the plugin package that owns them; packaged Astroclaw startup never repairs or mirrors plugin dependencies.
