...
The plugin is included with Astroclaw, but it is off by default. When disabled, the route is not registered. When enabled, it adds:
...
Use the WebSocket RPC path for Astroclaw clients and interactive tools that can keep a Gateway WebSocket connection open.
...
<Tabs>
  <Tab title="CLI">
    ```bash
    astroclaw plugins enable admin-http-rpc
...
```bash
astroclaw plugins disable admin-http-rpc
astroclaw gateway restart
```
...
- For `token` and `password` auth, narrower `x-astroclaw-scopes` headers are ignored and the normal full operator defaults are restored.
- Trusted identity-bearing HTTP modes honor `x-astroclaw-scopes` when present.
...
The normal Gateway WebSocket RPC path remains the preferred control-plane API for Astroclaw clients. Use admin HTTP RPC only for host tooling that needs a request/response HTTP surface.
