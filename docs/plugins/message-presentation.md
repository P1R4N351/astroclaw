...
Message presentation is Astroclaw's shared contract for rich outbound chat UI.
...
```bash
astroclaw message send --channel slack \
  --target channel:C123 \
...
```bash
astroclaw message send --channel telegram \
...
Use helpers from `astroclaw/plugin-sdk/interactive-runtime` when bridging older
code:
