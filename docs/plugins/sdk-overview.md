...
read_when:
  ...
  - You want a reference for all registration methods on AstroclawPluginApi
...
<Note>
  This page is for plugin authors using `astroclaw/plugin-sdk/*` inside
  Astroclaw. For external apps, scripts, dashboards, CI jobs, and IDE extensions
  that want to run agents through the Gateway, use the
  [Astroclaw App SDK](/concepts/astroclaw-sdk) and the `@astroclaw/sdk` package
  instead.
</Note>
...
prefer `astroclaw/plugin-sdk/channel-core`; keep `astroclaw/plugin-sdk/core` for
...
`astroclaw.plugin.json#channelConfigs`. The `plugin-sdk/channel-config-schema`
subpath is for shared schema primitives and the generic builder. Astroclaw's
...
<Warning>
  ...
  `astroclaw/plugin-sdk/slack`, `.../discord`, `.../signal`, `.../whatsapp`).
...
`astroclaw/plugin-sdk/discord` and `astroclaw/plugin-sdk/telegram-account` are
...
The `register(api)` callback receives an `AstroclawPluginApi` object with these
methods:
...
| `api.registerNodeCliFeature(registrar, opts?)` | Node feature CLI under `astroclaw nodes` |
...
cannot register this middleware; keep normal Astroclaw plugin hooks for work
...
Gateway on a local discovery transport such as mDNS/Bonjour. Astroclaw calls the
...
`astroclaw nodes canvas` explicit plugin-owned node features.
...
- User config still wins. Astroclaw merges `agents.defaults.cliBackends.<id>` over the
...
- Use `resolveExecutionArgs` for request-scoped argv rewrites that belong to
  the CLI dialect, such as mapping Astroclaw thinking levels to a native effort
  flag.
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `astroclaw/plugin-sdk/memory-host-core` instead of reaching into a specific
...
- `cron_changed`: observe gateway-owned cron lifecycle changes. Use `event.job?.state?.nextRunAtMs` and `ctx.getCron?.()` when syncing external wake schedulers, and keep Astroclaw as the source of truth for due checks and execution.
...
| `api.config`             | `AstroclawConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `astroclaw/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when Astroclaw is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through Astroclaw's plugin
...
- **`@astroclaw/openai-provider`**: `api.ts` exports provider builders,
  default-model helpers, and realtime provider builders.
- **`@astroclaw/openrouter-provider`**: `api.ts` exports the provider builder
  plus onboarding/config helpers.

<Warning>
  Extension production code should also avoid `astroclaw/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `astroclaw/plugin-sdk/speech`, `.../provider-model-shared`, or another
