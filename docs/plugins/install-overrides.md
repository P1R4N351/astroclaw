...
[`astroclaw plugins install`](/cli/plugins).
...
```bash
export ASTROCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1
export ASTROCLAW_PLUGIN_INSTALL_OVERRIDES='{
  "codex": "npm-pack:/tmp/astroclaw-codex-2026.5.8.tgz",
  "astroclaw-web-search": "npm:@astroclaw/web-search@2026.5.8"
}'
```
...
Astroclaw uses the override source instead of the catalog, bundled, or default
...
entry normally represents an Astroclaw-owned package, an override is treated as
operator-supplied test input.

Workspace `.env` files cannot enable install overrides. Set these variables in
the trusted shell, CI job, or remote test command that launches Astroclaw.

## Package E2E

Use an isolated state directory so package installs and install records do not
touch your normal Astroclaw state:

```bash
npm pack extensions/codex --pack-destination /tmp

ASTROCLAW_STATE_DIR="$(mktemp -d)" \
ASTROCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1 \
ASTROCLAW_PLUGIN_INSTALL_OVERRIDES='{"codex":"npm-pack:/tmp/astroclaw-codex-2026.5.8.tgz"}' \
pnpm astroclaw onboard --mode local
```

Verify the installed package under the state directory:

```bash
find "$ASTROCLAW_STATE_DIR/npm/node_modules" -maxdepth 3 -name package.json -print
grep -R '"@astroclaw/codex"' "$ASTROCLAW_STATE_DIR/npm/package-lock.json"
```
