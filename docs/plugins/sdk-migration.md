...
read_when:
  - You see the ASTROCLAW_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the ASTROCLAW_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before Astroclaw 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Astroclaw plugin
---

Astroclaw has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`astroclaw/plugin-sdk/compat`** - a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`astroclaw/plugin-sdk/infra-runtime`** - a broad runtime helper barrel that
...
- **`astroclaw/plugin-sdk/config-runtime`** - a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`astroclaw/extension-api`** - a bridge that gave plugins direct access to
...
Astroclaw does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`astroclaw/plugin-sdk/\<subpath\>`)
...
`astroclaw/plugin-sdk/realtime-voice`. The new controller owns the common Talk
...
`talk.providers` should be repaired with `astroclaw doctor --fix`; runtime Talk
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `astroclaw/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `AstroclawConfig` | `astroclaw/plugin-sdk/config-contracts` |
    | Already-loaded config assertions and plugin-entry config lookup | `astroclaw/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `astroclaw/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `astroclaw/plugin-sdk/config-mutation` |
    | Session store helpers | `astroclaw/plugin-sdk/session-store-runtime` |
    | Markdown table config | `astroclaw/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `astroclaw/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `astroclaw/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `astroclaw/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `astroclaw/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `astroclaw/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "astroclaw/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "astroclaw/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "astroclaw/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "astroclaw/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "astroclaw/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedPiAgent } from "astroclaw/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `astroclaw/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `astroclaw/plugin-sdk/system-event-runtime` |
    | Heartbeat wake, event, and visibility helpers | `astroclaw/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `astroclaw/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `astroclaw/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `astroclaw/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `astroclaw/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `astroclaw/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `astroclaw/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `astroclaw/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `astroclaw/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `astroclaw/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `astroclaw/plugin-sdk/error-runtime` |
    | Transport readiness waits | `astroclaw/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `astroclaw/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `astroclaw/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `astroclaw/plugin-sdk/number-runtime` |
    | Process-local async lock | `astroclaw/plugin-sdk/async-lock-runtime` |
    | File locks | `astroclaw/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `astroclaw/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `AstroclawSchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | Astroclaw-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@astroclaw/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`astroclaw/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`astroclaw/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "astroclaw/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "astroclaw/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `astroclaw/plugin-sdk/channel-inbound` or
    `astroclaw/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `astroclaw/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `astroclaw/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `astroclaw/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `astroclaw/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    Astroclaw no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. Astroclaw downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="AstroclawSchemaType alias → AstroclawConfig">
    `AstroclawSchemaType` re-exported from `astroclaw/plugin-sdk` is now a
    one-line alias for `AstroclawConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { AstroclawSchemaType } from "astroclaw/plugin-sdk";
    // After
    import type { AstroclawConfig } from "astroclaw/plugin-sdk/config-schema";
    ```
...
```bash
ASTROCLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 astroclaw gateway run
ASTROCLAW_SUPPRESS_EXTENSION_API_WARNING=1 astroclaw gateway run
```
