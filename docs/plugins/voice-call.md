...
read_when:
  - You want to place an outbound voice call from Astroclaw
...
Voice calls for Astroclaw via a plugin. Supports outbound notifications,
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      <Tab title="From npm">
        ```bash
        astroclaw plugins install @astroclaw/voice-call
        ```
      </Tab>
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    `streaming.*` OpenAI keys are rewritten by `astroclaw doctor --fix`.
    Runtime fallback still accepts the old voice-call keys for now, but
    the rewrite path is `astroclaw doctor --fix` and the compat shim is
    temporary.
...
- Voice Call exposes the shared `astroclaw_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal Astroclaw tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `astroclaw_agent_consult`.
...
configured Astroclaw agent without paying a full agent-consult round trip on
...
`astroclaw_agent_consult` still run the full Astroclaw agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call astroclaw_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `astroclaw doctor --fix`; committed config should use `tts.providers.<provider>`.
...
```bash
astroclaw voicecall call --to "+15555550123" --message "Hello from Astroclaw"
astroclaw voicecall start --to "+15555550123"   # alias for call
astroclaw voicecall continue --call-id <id> --message "Any questions?"
astroclaw voicecall speak --call-id <id> --message "One moment"
astroclaw voicecall dtmf --call-id <id> --digits "ww123456#"
astroclaw voicecall end --call-id <id>
astroclaw voicecall status --call-id <id>
astroclaw voicecall tail
astroclaw voicecall latency                      # summarize turn latency from logs
astroclaw voicecall expose --mode funnel
```
...
```bash
astroclaw voicecall setup
astroclaw voicecall setup --json
```
...
Provider signatures are checked against the public URL Astroclaw reconstructs
...
Use `astroclaw logs --follow` for the live phase trace. A healthy Twilio Meet
...
`astroclaw voicecall tail` still shows persisted call records; it is useful for
...
- `astroclaw logs --follow` shows realtime TwiML served, the realtime bridge
