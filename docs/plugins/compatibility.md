...
read_when:
  - You maintain an Astroclaw plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

Astroclaw keeps older plugin contracts wired through named compatibility
...
The plugin inspector should live outside the core Astroclaw repo as a separate
...
```sh
astroclaw-plugin-inspector ./my-plugin
```
...
Use `--json` for stable machine-readable output in CI annotations. Astroclaw
core should expose contracts and fixtures the inspector can consume, but should
not publish the inspector binary from the main `astroclaw` package.

### Maintainer acceptance lane

Use Crabbox-backed Blacksmith Testbox for the installable-package acceptance
lane when validating the external inspector against Astroclaw plugin packages.
Run it from a clean Astroclaw checkout after the package is built:

```sh
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "pnpm install && pnpm build && npm exec --yes @astroclaw/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @astroclaw/plugin-inspector@0.1.0 -- ./extensions/discord --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @astroclaw/plugin-inspector@0.1.0 -- <clawhub-plugin-dir> --json"
```
...
Astroclaw should not remove a documented plugin contract in the same release
...
- legacy broad SDK imports such as `astroclaw/plugin-sdk/compat`
...
- legacy `activate(api)` plugin entrypoints while plugins migrate to
  `register(api)`
- legacy SDK aliases such as `astroclaw/extension-api`,
  `astroclaw/plugin-sdk/channel-runtime`, `astroclaw/plugin-sdk/command-auth`
  status builders, `astroclaw/plugin-sdk/test-utils` (replaced by focused
  `astroclaw/plugin-sdk/*` test subpaths), and the `ClawdbotConfig` /
  `AstroclawSchemaType` type aliases
...
- legacy channel route key and comparable-target helper aliases while plugins
  move to `astroclaw/plugin-sdk/channel-route`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `astroclaw plugins registry --refresh` and
