...
the native model loop, and Astroclaw adapts its plugin, tool, session, and
diagnostic surfaces around that boundary.

Astroclaw still owns channel routing, session files, visible message delivery,
Astroclaw dynamic tools, approvals, media delivery, and a transcript mirror.
Codex owns the canonical native thread, native model loop, native tool
continuation, and native compaction unless the active Astroclaw context engine
declares that it owns compaction.

## Thread bindings and model changes

When an Astroclaw session is attached to an existing Codex thread, the next turn
...
to the Astroclaw `message` tool if the deployment has not explicitly configured
...
Codex heartbeat turns also get `heartbeat_respond` in the searchable Astroclaw
...
| Layer                                 | Owner                    | Purpose                                                             |
| ------------------------------------- | ------------------------ | ------------------------------------------------------------------- |
| Astroclaw plugin hooks                 | Astroclaw                 | Product/plugin compatibility across PI and Codex harnesses.         |
| Codex app-server extension middleware | Astroclaw bundled plugins | Per-turn adapter behavior around Astroclaw dynamic tools.            |
| Codex native hooks                    | Codex                    | Low-level Codex lifecycle and native tool policy from Codex config. |

Astroclaw does not use project or global Codex `hooks.json` files to route
Astroclaw plugin behavior. For the supported native tool and permission bridge,
Astroclaw injects per-thread Codex config for `PreToolUse`, `PostToolUse`,
...
Codex's app-server reviewer and Astroclaw's approval bridge handle real
...
Codex-level controls. They are not exposed as Astroclaw plugin hooks in the v1
contract.

For Astroclaw dynamic tools, Astroclaw executes the tool after Codex asks for the
call, so Astroclaw fires the plugin and middleware behavior it owns in the
harness adapter. For Codex-native tools, Codex owns the canonical tool record.
Astroclaw can mirror selected events, but it cannot rewrite the native Codex
...
notifications and Astroclaw adapter state, not native Codex hook commands.
Astroclaw's `before_compaction`, `after_compaction`, `llm_input`, and
...
They do not invoke Astroclaw plugin hooks.
...
| Astroclaw channel routing and delivery         | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                           |
| Astroclaw dynamic tools                        | Supported                                                                        | Codex asks Astroclaw to execute these tools, so Astroclaw stays in the execution path.                                                                                                                       |
| Prompt and context plugins                    | Supported                                                                        | Astroclaw builds prompt overlays and projects context into the Codex turn before starting or resuming the thread.                                                                                           |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around Astroclaw-owned dynamic tools.                                                                                                 |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through Astroclaw after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review. |
| App-server trajectory capture                 | Supported                                                                        | Astroclaw records the request it sent to app-server and the app-server notifications it receives.                                                                                                           |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                     | Future path                                                                               |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but Astroclaw does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. Astroclaw owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms Astroclaw-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | Astroclaw observes compaction start and completion, but does not receive a stable kept/dropped list, token delta, or summary payload.            | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | Current Astroclaw compaction hooks are notification-level in Codex mode.                                                                         | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | Astroclaw can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, Astroclaw only returns explicit allow or deny decisions
...
Astroclaw remembers that exact provider/session/tool input/cwd fingerprint for a
...
Codex MCP tool approval elicitations are routed through Astroclaw's plugin
...
default `messages.queue.mode: "steer"`, Astroclaw batches steer-mode chat
...
case, Astroclaw waits for the active run to finish before starting the prompt.
...
harness, Astroclaw also calls Codex app-server `feedback/upload` for relevant
...
error. The completed diagnostics reply lists the channels, Astroclaw session ids,
Codex thread ids, and local `codex resume <thread-id>` commands for the threads
that were sent.

If you deny or ignore the approval, Astroclaw does not print those Codex ids and
...
`ownsCompaction: true`. Owning context engines compact first and cause Astroclaw
to abandon the old Codex backend thread so the next turn can rehydrate a fresh
thread from engine-managed context. Astroclaw keeps a transcript mirror for
channel history, search, `/new`, `/reset`, and future model or harness
switching.

When a context engine requests Codex thread-bootstrap projection, Astroclaw
...
reasoning or plan records when the app-server emits them. Today, Astroclaw only
...
Astroclaw is writing an Astroclaw-owned session transcript tool result.

## Media and delivery

Astroclaw continues to own media delivery and media provider selection. Image,
...
through the normal Astroclaw delivery path. Media generation does not require PI.
When Codex emits a native image-generation item with a `savedPath`, Astroclaw
