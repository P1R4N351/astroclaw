---
summary: "Configure migrated native Codex plugins for Codex-mode Astroclaw agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode Astroclaw agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode Astroclaw agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the Astroclaw turn.

Astroclaw does not translate Codex plugins into synthetic `codex_plugin_*`
Astroclaw dynamic tools. Plugin calls stay in the native Codex transcript, and
...
- The selected Astroclaw agent runtime must be the native Codex harness.
...
```bash
astroclaw migrate codex --dry-run --verify-plugin-apps
```
...
- Enabled: Astroclaw config is willing to make the plugin available to Codex
  harness turns.
...
Migration is the durable install/eligibility step. During planning, Astroclaw
...
Astroclaw reads Codex app inventory through app-server `app/list`, caches it for
one hour, and refreshes stale or missing entries asynchronously. The cache is
in memory only; restarting the CLI or gateway drops it, and Astroclaw rebuilds it
...
A plugin app is exposed only when Astroclaw can map it back to the migrated
...
Astroclaw injects a restrictive `config.apps` patch for the Codex thread:
`_default` is disabled and only apps owned by enabled migrated plugins are
enabled.

Astroclaw sets app-level `destructive_enabled` from the effective global or
...
are emitted with `open_world_enabled: true`; Astroclaw does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for that
  plugin.
- When policy is `false`, Astroclaw returns a deterministic decline.
- When policy is `true`, Astroclaw auto-accepts only safe schemas it can map to
...
empty or stale cache. Astroclaw schedules an async refresh and excludes plugin
...
started with until Astroclaw establishes a new harness session or replaces a
stale binding.
