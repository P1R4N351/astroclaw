...
At startup, Astroclaw does roughly this:
...
The manifest is the control-plane source of truth. Astroclaw uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, Astroclaw
...
Astroclaw does not cache plugin discovery results or direct manifest registry
...
Astroclaw still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Astroclaw calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that Astroclaw no longer calls, such as
...
| --  | _(built-in model lookup)_         | Astroclaw tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
that still runs on Astroclaw's normal inference loop.
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of Astroclaw's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Astroclaw adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  product plugins own their routes and schemas while Astroclaw owns the
...
- Astroclaw only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Astroclaw plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-astroclaw-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-astroclaw-scopes` only when the header is explicitly present
  - if `x-astroclaw-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-astroclaw-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `astroclaw/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `astroclaw/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `astroclaw/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `astroclaw/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `astroclaw/plugin-sdk/config-schema` | Root `astroclaw.json` Zod schema (`AstroclawSchema`) |
...
<Info>
`astroclaw/plugin-sdk/channel-runtime`, `astroclaw/plugin-sdk/config-runtime`,
and `astroclaw/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `astroclaw/plugin-sdk/*` subpaths. Never
...
`astroclaw/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Astroclaw writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Astroclaw's
...
- if both `catalog` and `discovery` are registered, Astroclaw uses `catalog`
...
- Read-only command paths such as `astroclaw status`, `astroclaw status --all`,
  `astroclaw channels status`, `astroclaw channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `astroclaw.extensions`:
...
Security guardrail: every `astroclaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `astroclaw plugins install` installs plugin dependencies with a
...
Optional: `astroclaw.setupEntry` can point at a lightweight setup-only module.
When Astroclaw needs setup surfaces for a disabled channel plugin, or
...
Optional: `astroclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Astroclaw load the
...
Channel plugins can advertise setup/discovery metadata via `astroclaw.channel` and
install hints via `astroclaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@astroclaw/nextcloud-talk",
  "astroclaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@astroclaw/nextcloud-talk",
...
Useful `astroclaw.channel` fields beyond the minimal example:
...
Astroclaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.astroclaw/mpm/plugins.json`
- `~/.astroclaw/mpm/catalog.json`
- `~/.astroclaw/plugins/catalog.json`

Or point `ASTROCLAW_PLUGIN_CATALOG_PATHS` (or `ASTROCLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "astroclaw": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `astroclaw.install` block. The
...
```ts
import { buildMemorySystemPromptAddition } from "astroclaw/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `AstroclawPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Astroclaw stays opinionated without becoming hardcoded to one
