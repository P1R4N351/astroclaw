---
summary: "Install and use Codex, Claude, and Cursor bundles as Astroclaw plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Astroclaw maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

Astroclaw can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
Astroclaw maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Astroclaw plugins. Native plugins run
...
of requiring authors to rewrite them as native Astroclaw plugins, Astroclaw
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    astroclaw plugins install ./my-bundle

    # Archive
    astroclaw plugins install ./my-bundle.tgz
...
## What Astroclaw maps from bundles

Not every bundle feature runs in Astroclaw today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Astroclaw skills                                           | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                  | Claude, Cursor |
| Hook packs    | Astroclaw-style `HOOK.md` + `handler.ts` layouts                                             | Codex          |
...
- bundle skill roots load as normal Astroclaw skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal Astroclaw skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal Astroclaw hook-pack
...
- Astroclaw merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- Astroclaw exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, Astroclaw uses `sse`
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in Astroclaw config. `astroclaw mcp set` and `astroclaw doctor --fix` normalize the common alias.
...
Astroclaw registers bundle MCP tools with provider-safe names in the form
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- Astroclaw sanitizes shell override keys before applying them
...
- Astroclaw loads `.lsp.json` plus any manifest-declared `lspServers` paths
...
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `astroclaw plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but Astroclaw does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Astroclaw best when they use skill roots and Astroclaw-style
...
Astroclaw checks for native plugin format first:

1. `astroclaw.plugin.json` or valid `package.json` with `astroclaw.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, Astroclaw uses the native path. This prevents
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `astroclaw plugins install` and ship everything
  they need in the installed plugin directory.
- Astroclaw-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `astroclaw doctor --fix` removes legacy staged dependency directories and can
...
- Astroclaw does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `astroclaw plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. Astroclaw does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Astroclaw hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
