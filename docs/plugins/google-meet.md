...
read_when:
  - You want an Astroclaw agent to join a Google Meet call
  - You want an Astroclaw agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for Astroclaw — the plugin is explicit by design:
...
- `agent` is the default talk-back mode: realtime transcription listens, the
  configured Astroclaw agent answers, and regular Astroclaw TTS speaks into Meet.
...
provider plus regular Astroclaw TTS. OpenAI is the default transcription
...
```bash
astroclaw googlemeet setup --transport chrome-node --mode transcribe
```
...
before asking an agent to join. Use `astroclaw googlemeet setup --json` for
...
```bash
astroclaw googlemeet setup --transport twilio
```
...
```bash
astroclaw googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
astroclaw googlemeet create --transport chrome-node --mode agent
```
...
```bash
astroclaw googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
- Browser fallback: used when OAuth credentials are absent. Astroclaw uses the
  ...
  the Astroclaw Chrome profile on the node to already be signed in to Google.
...
Astroclaw's microphone/camera permission grant and avoid the Meet **Use
...
Use `astroclaw googlemeet test-listen <meet-url> --transport chrome-node` when
...
Local Chrome joins through the signed-in Astroclaw browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by Astroclaw. For
...
You do **not** need a full Astroclaw Gateway or model API key inside a macOS VM
...
- Gateway host: Astroclaw Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: Astroclaw CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update Astroclaw in the VM, then enable the bundled plugin there:

```bash
astroclaw plugins enable google-meet
```

Start the node host in the VM:

```bash
astroclaw node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
```bash
ASTROCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  astroclaw node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
```bash
ASTROCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  astroclaw node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
astroclaw node restart
```

`ASTROCLAW_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`astroclaw.json` setting. `astroclaw node install` stores it in the LaunchAgent
...
```bash
astroclaw googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, Astroclaw browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, Astroclaw auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `astroclaw node run` in the VM,
  approve pairing, and make sure `astroclaw plugins enable google-meet` and
  `astroclaw plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses Astroclaw
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. Astroclaw
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by Astroclaw; use separate virtual devices or Loopback-style routing
...
Astroclaw does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with Astroclaw, review BlackHole's
...
Chrome transport opens the Meet URL through Astroclaw browser control and joins
as the signed-in Astroclaw browser profile. On macOS, the plugin checks for
...
```bash
astroclaw googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
astroclaw googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local Astroclaw audio
...
meeting; Astroclaw does not discover those from the Meet page.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an Astroclaw agent. Be brief.",
...
```bash
astroclaw config validate
astroclaw plugins list | grep -E 'google-meet|voice-call'
astroclaw googlemeet setup
```
...
`ASTROCLAW_GOOGLE_MEET_*` environment variables.
...
4. Add the scopes Astroclaw requests:
...
`meetings.space.readonly` lets Astroclaw resolve Meet URLs/codes to spaces.
`meetings.space.settings` lets Astroclaw pass `SpaceConfig` settings such as
...
```bash
astroclaw googlemeet auth login --json
```
...
```bash
ASTROCLAW_GOOGLE_MEET_CLIENT_ID="your-client-id" \
ASTROCLAW_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
astroclaw googlemeet auth login --json
```
...
ASTROCLAW_GOOGLE_MEET_CLIENT_ID="your-client-id" \
ASTROCLAW_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
astroclaw googlemeet auth login --json --manual
```
...
support existed, rerun `astroclaw googlemeet auth login --json` so the refresh
...
```bash
astroclaw googlemeet doctor --oauth --json
```
...
```bash
astroclaw googlemeet doctor --oauth --create-space --json
astroclaw googlemeet create --no-join --json
```
...
```bash
astroclaw googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
astroclaw googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `astroclaw googlemeet auth login
...
- `ASTROCLAW_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `ASTROCLAW_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `ASTROCLAW_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `ASTROCLAW_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `ASTROCLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or
  `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `ASTROCLAW_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `ASTROCLAW_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`
...
```bash
astroclaw googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
astroclaw googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
astroclaw googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
astroclaw googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
astroclaw googlemeet latest --today
astroclaw googlemeet calendar-events --today --json
astroclaw googlemeet artifacts --event "Weekly sync"
astroclaw googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
astroclaw googlemeet latest --meeting https://meet.google.com/abc-defg-hij
astroclaw googlemeet artifacts --conference-record conferenceRecords/abc123 --json
astroclaw googlemeet attendance --conference-record conferenceRecords/abc123 --json
```
...
```bash
astroclaw googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```
...
Astroclaw accepts a Meet URL, meeting code, or `spaces/{id}` input and resolves it
to the API space resource before ending the active conference.
It is separate from `googlemeet leave`: `leave` stops Astroclaw's local/session
...
```bash
astroclaw googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
astroclaw googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
astroclaw googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
astroclaw googlemeet export --conference-record conferenceRecords/abc123 \
...
ASTROCLAW_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
...
```bash
astroclaw googlemeet setup --transport chrome-node --mode transcribe
astroclaw googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

Live smoke environment:

- `ASTROCLAW_LIVE_TEST=1` enables guarded live tests.
- `ASTROCLAW_GOOGLE_MEET_LIVE_MEETING` points at a retained Meet URL, code, or
  `spaces/{id}`.
- `ASTROCLAW_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID` provides the OAuth
  client id.
- `ASTROCLAW_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` provides
  the refresh token.
- Optional: `ASTROCLAW_GOOGLE_MEET_CLIENT_SECRET`,
  `ASTROCLAW_GOOGLE_MEET_ACCESS_TOKEN`, and
  `ASTROCLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` use the same fallback names
  without the `ASTROCLAW_` prefix.
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the Astroclaw browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the Astroclaw browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, Astroclaw reports `manualActionRequired: true` or a
...
realtime transcription provider key, and a configured Astroclaw TTS provider.
...
- `chrome.guestName: "Astroclaw Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through Astroclaw browser automation on `chrome-node`
...
- `mode: "agent"`: default talk-back mode. Participant speech is transcribed by
  ...
  Astroclaw agent in a per-meeting sub-agent session, and spoken back through the
  normal Astroclaw TTS runtime.
- `mode: "bidi"`: fallback direct bidirectional realtime model mode. The
  ...
  `astroclaw_agent_consult` for deeper/tool-backed answers.
...
- `realtime.instructions`: brief spoken replies, with
  `astroclaw_agent_consult` for deeper answers
- `realtime.introMessage`: short spoken readiness check when the realtime bridge
  connects; set it to `""` to join silently
- `realtime.agentId`: optional Astroclaw agent id for
  `astroclaw_agent_consult`; defaults to `main`
...
VM. In both cases the model providers and `astroclaw_agent_consult` run on the
Gateway host, so model credentials stay there. With the default `mode: "agent"`,
the realtime transcription provider handles listening, the configured Astroclaw
agent produces the answer, and regular Astroclaw TTS speaks it into Meet. Use
...
- `speechReady` / `speechBlockedReason` / `speechBlockedMessage`: whether
  managed Chrome speech is allowed now. `speechReady: false` means Astroclaw did
...
transcripts are routed through the configured Astroclaw agent, and the answer is
spoken through the normal Astroclaw TTS runtime. Set `mode: "bidi"` when you want
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured Astroclaw agent | Normal Astroclaw TTS runtime            | You want "my agent is in the meeting" behavior        |
...
information, or normal Astroclaw tools, it can call `astroclaw_agent_consult`.

The consult tool runs the regular Astroclaw agent behind the scenes with recent
meeting transcript context and returns a concise spoken answer. In `agent` mode,
Astroclaw sends that answer directly to the TTS runtime; in `bidi` mode, the
...
Meet lane should consult a dedicated Astroclaw agent workspace, model defaults,
...
```bash
astroclaw googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
```bash
astroclaw googlemeet setup
astroclaw nodes status --connected
astroclaw nodes invoke \
  --node parallels-macos \
...
- `astroclaw logs --follow` shows DTMF TwiML served before realtime TwiML, then a
...
```bash
astroclaw plugins list | grep google-meet
astroclaw googlemeet setup
```
...
```bash
astroclaw plugins enable google-meet
astroclaw plugins enable browser
ASTROCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  astroclaw node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; Astroclaw
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, Astroclaw
...
- For API creation: `oauth.clientId` and `oauth.refreshToken` are configured,
  or matching `ASTROCLAW_GOOGLE_MEET_*` environment variables are present.
- For API creation: the refresh token was minted after create support was
  ...
  `astroclaw googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the Astroclaw Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. Astroclaw should click **Use microphone** or, for
...
```bash
astroclaw googlemeet setup
astroclaw googlemeet doctor
```

Use `mode: "agent"` for the normal STT -> Astroclaw agent -> TTS talk-back path,
...
run `astroclaw googlemeet status --json <session-id>` after participants speak
...
utterance but Astroclaw did not see new output bytes reach the Chrome audio
bridge.
...
- Meet microphone and speaker are routed through the virtual audio path used by
  Astroclaw. `doctor` should show `meet output routed: yes` for local Chrome
  realtime joins.
...
```bash
astroclaw googlemeet recover-tab
astroclaw googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
astroclaw googlemeet setup --transport twilio
astroclaw voicecall setup
astroclaw voicecall smoke
```

`voicecall smoke` is readiness-only by default. To dry-run a specific number:

```bash
astroclaw voicecall smoke --to "+15555550123"
```
...
```bash
astroclaw voicecall smoke --to "+15555550123" --yes
```
...
- Run `astroclaw googlemeet doctor <session-id>` to confirm the delegated Twilio
  call ID, whether DTMF was queued, and whether the intro greeting was requested.
- Run `astroclaw voicecall status --call-id <id>` and confirm the call is still
  active.
- Run `astroclaw voicecall tail` and check that Twilio webhooks are arriving at
  the Gateway.
- Run `astroclaw logs --follow` and look for the Twilio Meet sequence: Google
...
- Re-run `astroclaw googlemeet setup --transport twilio`; a green setup check is
...
- If the participant joins but you do not hear the greeting, check
  `astroclaw logs --follow` for the post-DTMF `voicecall.speak` request and
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: Astroclaw owns the
