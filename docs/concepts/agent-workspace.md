...
This is separate from `~/.astroclaw/`, which stores config, credentials, and sessions.
...
When sandboxing is enabled and `workspaceAccess` is not `"rw"`, tools operate inside a sandbox workspace under `~/.astroclaw/sandboxes`, not your host workspace.
</Warning>

## Default location

- Default: `~/.astroclaw/workspace`
- If `ASTROCLAW_PROFILE` is set and not `"default"`, the default becomes `~/.astroclaw/workspace-<profile>`.
- Override in `~/.astroclaw/astroclaw.json`:
...
`astroclaw onboard`, `astroclaw configure`, or `astroclaw setup` will create the workspace and seed the bootstrap files if they are missing.
...
Older installs may have created `~/astroclaw`. Keeping multiple workspace directories around can cause confusing auth or state drift, because only one workspace is active at a time.

<Note>
**Recommendation:** keep a single active workspace. If you no longer use the extra folders, archive or move them to Trash (for example `trash ~/astroclaw`). If you intentionally keep multiple workspaces, make sure `agents.defaults.workspace` points to the active one.

`astroclaw doctor` warns when it detects extra workspace directories.
</Note>

## Workspace file map

These are the standard files Astroclaw expects inside the workspace:
...
<Note>
If any bootstrap file is missing, Astroclaw injects a "missing file" marker into the session and continues. Large bootstrap files are truncated when injected; adjust limits with `agents.defaults.bootstrapMaxChars` (default: 12000) and `agents.defaults.bootstrapTotalMaxChars` (default: 60000). `astroclaw setup` can recreate missing defaults without overwriting existing files.
</Note>

## What is NOT in the workspace

These live under `~/.astroclaw/` and should NOT be committed to the workspace repo:

- `~/.astroclaw/astroclaw.json` (config)
- `~/.astroclaw/agents/<agentId>/agent/auth-profiles.json` (model auth profiles: OAuth + API keys)
- `~/.astroclaw/agents/<agentId>/agent/codex-home/` (per-agent Codex runtime account, config, skills, plugins, and native thread state)
- `~/.astroclaw/credentials/` (channel/provider state plus legacy OAuth import data)
- `~/.astroclaw/agents/<agentId>/sessions/` (session transcripts + metadata)
- `~/.astroclaw/skills/` (managed skills)
  ...
  <Step title="Add a private remote">
    <Tabs>
      ...
      <Tab title="GitHub CLI (gh)">
        ```bash
        gh auth login
        gh repo create astroclaw-workspace --private --source . --remote origin --push
        ```
      </Tab>
...
- Anything under `~/.astroclaw/`.
- Raw dumps of chats or sensitive attachments.

If you must store sensitive references, use placeholders and keep the real secret elsewhere (password manager, environment variables, or `~/.astroclaw/`).
</Warning>
...
<Steps>
  <Step title="Clone the repo">
    Clone the repo to the desired path (default `~/.astroclaw/workspace`).
  </Step>
  <Step title="Update config">
    Set `agents.defaults.workspace` to that path in `~/.astroclaw/astroclaw.json`.
  </Step>
  <Step title="Seed missing files">
    Run `astroclaw setup --workspace <path>` to seed any missing files.
  </Step>
  <Step title="Copy sessions (optional)">
    If you need sessions, copy `~/.astroclaw/agents/<agentId>/sessions/` from the old machine separately.
  </Step>
</Steps>
