...
`node-llama-cpp` runtime package next to Astroclaw, then point `local.modelPath`
at a GGUF file:
...
Astroclaw indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.astroclaw/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `astroclaw memory index --force`
...
**Memory search disabled?** Check `astroclaw memory status`. If no provider is
...
**Stale results?** Run `astroclaw memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** Astroclaw falls back to in-process cosine similarity
automatically. `astroclaw memory status --deep` reports the local vector store
