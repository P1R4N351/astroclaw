...
[Honcho](https://honcho.dev) adds AI-native memory to Astroclaw. It persists
...
```bash
astroclaw plugins install @honcho-ai/astroclaw-honcho
astroclaw honcho setup
...
Settings live under `plugins.entries["astroclaw-honcho"].config`:
    ...
    entries: {
      "astroclaw-honcho": {
        config: {
          ...
          workspaceId: "astroclaw", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `astroclaw honcho setup` detects and
...
```bash
astroclaw honcho setup                        # Configure API key and migrate files
astroclaw honcho status                       # Check connection status
astroclaw honcho ask <question>               # Query Honcho about the user
astroclaw honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/astroclaw-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Astroclaw integration guide](https://docs.honcho.dev/v3/guides/integrations/astroclaw)
- [Memory](/concepts/memory) -- Astroclaw memory overview
