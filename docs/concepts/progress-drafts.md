...
read_when:
  ...
  - Explaining how Astroclaw updates one channel message while work is in progress
...
When progress drafts are enabled, Astroclaw creates one visible work-in-progress
...
That is usually enough. Astroclaw will pick an automatic one-word label, wait
...
Astroclaw sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from Astroclaw's built-in
...
Astroclaw uses the same formatter for progress drafts and `/verbose`:
...
Astroclaw truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, Astroclaw still suppresses the older standalone
...
When the final answer is ready, Astroclaw tries to keep the chat clean:

- If the draft can safely become the final answer, Astroclaw edits it in place.
- If the channel uses native progress streaming, Astroclaw finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, Astroclaw sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, Astroclaw keeps the
