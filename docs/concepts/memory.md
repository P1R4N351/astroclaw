---
summary: "How Astroclaw remembers things across sessions"
...
Astroclaw remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.astroclaw/workspace`).
...
If `MEMORY.md` grows past the bootstrap file budget, Astroclaw keeps the file on
...
`astroclaw doctor` to see raw vs injected sizes and truncation status.
...
for that case. Astroclaw infers them in a hidden background pass, scopes them to
...
<Info>
Astroclaw auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, Astroclaw
...
```bash
astroclaw memory rem-backfill --rollback
astroclaw memory rem-backfill --rollback-short-term
```

## CLI

```bash
astroclaw memory status          # Check index status and provider
astroclaw memory search "query"  # Search from the command line
astroclaw memory index --force   # Rebuild the index
```
