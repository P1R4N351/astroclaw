...
run is started. If the active run cannot accept steering, Astroclaw waits for the
...
- `steer`: inject messages into the active runtime. Pi delivers all pending steering messages **after the current assistant turn finishes executing its tool calls**, before the next LLM call; Codex app-server receives one batched `turn/steer`. If the run is not actively streaming or steering is unavailable, Astroclaw waits until the active run ends before starting the prompt.
...
For mode selection, Astroclaw resolves:
