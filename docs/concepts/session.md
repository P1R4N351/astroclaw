---
summary: "How Astroclaw manages conversation sessions"
read_when:
...
Astroclaw organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `astroclaw security audit`.
...
Astroclaw automatically bounds session storage over time. By default, it runs
...
`astroclaw sessions cleanup --dry-run --fix-dm-scope`. Applying the same flag
retires those old direct-DM rows and keeps their transcripts as deleted
archives.

Preview with `astroclaw sessions cleanup --dry-run`.

## Inspecting sessions

- `astroclaw status` -- session store path and recent activity.
- `astroclaw sessions --json` -- all sessions (filter with `--active <minutes>`).
