...
Astroclaw selects models in this order:
...
<AccordionGroup>
  <Accordion title="Related model surfaces">
    - `agents.defaults.models` is the allowlist/catalog of models Astroclaw can use (plus aliases). Use `provider/*` entries to limit visible providers while keeping provider discovery dynamic.
...
- Auto fallback selections are temporary recovery state. They are stored with `modelOverrideSource: "auto"` so later turns can keep using the fallback chain without probing a known-bad primary every time; Astroclaw periodically probes the original primary again, clears the auto selection when it recovers, and announces fallback/recovery transitions once per state change.
- User session selections are exact. `/model`, the model picker, `session_status(model=...)`, and `sessions.patch` store `modelOverrideSource: "user"`; if that selected provider/model is unreachable, Astroclaw fails visibly instead of falling through to another configured model.
...
<AccordionGroup>
  <Accordion title="Clobber protection rules">
    `astroclaw config set` protects model/provider maps from accidental clobbers. A plain object assignment to `agents.defaults.models`, `models.providers`, or `models.providers.<id>.models` is rejected when it would remove existing entries. Use `--merge` for additive changes; use `--replace` only when the provided value should become the complete target value.

    Interactive provider setup and `astroclaw configure --section model` also merge provider-scoped selections into the existing allowlist, so adding Codex, Ollama, or another provider does not drop unrelated model entries. Configure preserves an existing `agents.defaults.model.primary` when provider auth is re-applied. Explicit default-setting commands such as `astroclaw models auth login --provider <id> --set-default` and `astroclaw models set <model>` still replace `agents.defaults.model.primary`.
...
If `agents.defaults.models` is set, it becomes the **allowlist** for `/model` and for session overrides. When a user selects a model that isn't in that allowlist, Astroclaw returns:
...
exact provider/model shown by `astroclaw models list --provider <provider>`.
...
<AccordionGroup>
  <Accordion title="Picker behavior">
    ...
    - On Telegram, `/models` picker selections are session-scoped; they do not change the agent's persistent default in `astroclaw.json`.
  ...
  <Accordion title="Persistence and live switching">
    ...
    - If a run is already active, Astroclaw marks a live switch as pending and only restarts into the new model at a clean retry point.
  ...
  <Accordion title="Ref parsing">
    ...
    - If you omit the provider, Astroclaw resolves the input in this order:
      1. alias match
      2. unique configured-provider match for that exact unprefixed model id
      3. deprecated fallback to the configured default provider — if that provider no longer exposes the configured default model, Astroclaw instead falls back to the first configured provider/model to avoid surfacing a stale removed-provider default.
  </Accordion>
</AccordionGroup>
...
astroclaw models set <provider/model>
astroclaw models set-image <provider/model>

astroclaw models aliases list
astroclaw models aliases add <alias> <provider/model>
astroclaw models aliases remove <alias>

astroclaw models fallbacks list
astroclaw models fallbacks add <provider/model>
astroclaw models fallbacks remove <provider/model>
astroclaw models fallbacks clear

astroclaw models image-fallbacks list
astroclaw models image-fallbacks add <provider/model>
astroclaw models image-fallbacks remove <provider/model>
astroclaw models image-fallbacks clear
```

`astroclaw models` (no subcommand) is a shortcut for `models status`.
...
`astroclaw models scan` inspects OpenRouter's **free model catalog** and can optionally probe models for tool and image support.
...
When live probes run in a TTY, you can select fallbacks interactively. In non-interactive mode, pass `--yes` to accept defaults. Metadata-only results are informational; `--set-default` and `--set-image` require live probes so Astroclaw does not configure an unusable keyless OpenRouter model.

## Models registry (`models.json`)

Custom providers in `models.providers` are written into `models.json` under the agent directory (default `~/.astroclaw/agents/<agentId>/agent/models.json`). This file is merged by default unless `models.mode` is set to `replace`.
...
<Note>
Marker persistence is source-authoritative: Astroclaw writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values. This applies whenever Astroclaw regenerates `models.json`, including command-driven paths like `astroclaw agent`.
</Note>
