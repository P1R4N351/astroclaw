---
summary: "Delegate architecture: running Astroclaw as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run Astroclaw as a **named delegate** - an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Astroclaw agent that:
...
Astroclaw's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Astroclaw's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.astroclaw/workspace-delegate",
  tools: {
...
- Cron run history: `~/.astroclaw/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.astroclaw/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Astroclaw's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
astroclaw agents add delegate
```

This creates:

- Workspace: `~/.astroclaw/workspace-delegate`
- State: `~/.astroclaw/agents/delegate/agent`
- Sessions: `~/.astroclaw/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.astroclaw/workspace" },
      {
        id: "delegate",
        workspace: "~/.astroclaw/workspace-delegate",
        tools: {
...
~/.astroclaw/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.astroclaw/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.astroclaw/workspace-org",
        agentDir: "~/.astroclaw/agents/org-assistant/agent",
...
recall view. Astroclaw redacts credential/token-like text, truncates long
