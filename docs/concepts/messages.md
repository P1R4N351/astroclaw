...
Astroclaw handles inbound messages through a pipeline of session resolution, queueing, streaming, tool execution, and reasoning visibility. This page maps the path from inbound message to reply.
...
Channels can redeliver the same message after reconnects. Astroclaw keeps a
...
Astroclaw keeps that boundary explicit:
...
Astroclaw separates the **prompt body** from the **command body**:
...
Astroclaw can expose or hide model reasoning:
...
When a turn also has pending tool media, such as generated TTS audio, Astroclaw
strips the silent text but still delivers the media attachment.
Astroclaw resolves that behavior by conversation type:

- Direct conversations never receive `NO_REPLY` prompt guidance. If a direct
  run accidentally returns a bare silent token, Astroclaw suppresses it instead
...
Astroclaw also uses silent replies for internal runner failures that happen
