---
summary: "Mantis is the visual end-to-end verification system for reproducing Astroclaw bugs on live transports, capturing before and after evidence, and attaching artifacts to PRs."
title: "Mantis"
read_when:
  - Building or running live visual QA for Astroclaw bugs
...
Mantis is the Astroclaw end-to-end verification system for bugs that need a real
...
Mantis lives in the Astroclaw QA stack.

- Astroclaw owns the scenario runtime, transport adapters, evidence schema, and
  local CLI under `pnpm astroclaw qa mantis`.
...
- Astroclaw agents drive Mantis through Codex when a scenario needs agentic setup,
  debugging, or stuck-state reporting.

This boundary keeps transport knowledge in Astroclaw, machine scheduling in
...
```bash
pnpm astroclaw qa mantis discord-smoke \
...
thread, calls Astroclaw's `message.thread-reply` action with a repo-local
...
```bash
pnpm astroclaw qa mantis desktop-browser-smoke \
...
`ASTROCLAW_MANTIS_CRABBOX_PROVIDER` when running against another Crabbox fleet.

Useful desktop smoke flags:

- `--lease-id <cbx_...>` or `ASTROCLAW_MANTIS_CRABBOX_LEASE_ID` reuses a warmed desktop.
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome user-data-dir archive from the named environment variable before launching the browser. Use this for logged-in witnesses such as Discord Web. The default env var is `ASTROCLAW_MANTIS_BROWSER_PROFILE_TGZ_B64`.
...
- `--keep-lease` or `ASTROCLAW_MANTIS_KEEP_VM=1` keeps a newly created passing lease open for VNC inspection. Failed runs keep the lease by default when one was created so an operator can reconnect.
...
REST. When `ASTROCLAW_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also
writes a Discord Web URL artifact. When `ASTROCLAW_QA_DISCORD_KEEP_THREADS=1` is
...
the VM, runs `pnpm astroclaw qa slack` inside that VM, opens Slack Web in the VNC
...
shape where the SUT Astroclaw gateway and the browser both live inside the same
Linux desktop VM.

With `--gateway-setup`, the command prepares a persistent disposable Astroclaw
home at `$HOME/.astroclaw-mantis/slack-astroclaw`, patches Slack Socket Mode
configuration for the selected channel, starts `astroclaw gateway run` on port
...
- `ASTROCLAW_LIVE_OPENAI_KEY` for the remote model lane. If only
  `OPENAI_API_KEY` is set locally, Mantis maps it to `ASTROCLAW_LIVE_OPENAI_KEY`
  before invoking Crabbox so Crabbox's `ASTROCLAW_*` env forwarding can carry it
  into the VM.
...
channel id, Socket Mode app token, and bot token as the `ASTROCLAW_MANTIS_SLACK_*`
...
- `--gateway-setup` starts a persistent Astroclaw Slack gateway in the VM instead of only running the bot-to-bot QA lane.
...
- `ASTROCLAW_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome profile inside the VM. The default is `$HOME/.config/astroclaw-mantis/slack-chrome-profile`, so a manual Slack Web login survives reruns on the same lease.
...
Crabbox CLI from
`astroclaw/crabbox` main so it can use the current desktop/browser lease flags
...
runs `pnpm astroclaw qa mantis slack-desktop-smoke --gateway-setup` against that
...
worktree, runs `pnpm astroclaw qa telegram --credential-source convex
...
Astroclaw with the leased Telegram SUT bot token, starts `astroclaw gateway run`
...
token never logs Telegram Desktop in; it only configures Astroclaw. The desktop
...
- `--no-gateway-setup` installs and opens Telegram Desktop without configuring Astroclaw.
...
comment or creates one. The workflows publish to `astroclaw-crabbox-artifacts`
with public URLs under `https://artifacts.astroclaw.ai`. They provide bucket,
...
6. Start a child Astroclaw Gateway with an isolated state directory.
...
- It exercises a real Astroclaw Gateway, Discord bot auth, message dispatch,
...
- `pnpm astroclaw qa discord` already runs a live Discord lane with driver and
  SUT bots.
...
- **Headless automation**: default for CI. Chrome runs with CDP enabled, and
  Playwright or Astroclaw browser control captures screenshots.
...
- Node 22 and pnpm
- Astroclaw checkout and dependency cache
- Playwright Chromium browser cache when Playwright is used
- enough CPU and memory for one Astroclaw Gateway, one browser, and one model run
...
- `ASTROCLAW_QA_DISCORD_MANTIS_BOT_TOKEN`
...
- `ASTROCLAW_QA_DISCORD_NOTIFY_CHANNEL_ID`
- `ASTROCLAW_QA_REDACT_PUBLIC_METADATA=1` for public GitHub artifact uploads
- `ASTROCLAW_QA_CONVEX_SITE_URL`
- `ASTROCLAW_QA_CONVEX_SECRET_CI`
- `ASTROCLAW_QA_MANTIS_CRABBOX_COORDINATOR`
- `ASTROCLAW_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN`
...
`ASTROCLAW_QA_REDACT_PUBLIC_METADATA=1` for this reason.
