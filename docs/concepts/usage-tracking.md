...
- `/usage cost` in chats: local cost summary aggregated from Astroclaw session logs.
- CLI: `astroclaw status --usage` prints a full per-provider breakdown.
- CLI: `astroclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Astroclaw treats
  ...
  quota, so Astroclaw inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Astroclaw prefers the
...
can supply plugin-specific usage auth logic; otherwise Astroclaw falls back to
