---
summary: "Use SOUL.md to give your Astroclaw agent an actual voice instead of generic assistant sludge"
read_when:
...
Astroclaw injects it on normal sessions, so it has real weight. If your agent
...
For Astroclaw, `SOUL.md` is that layer.
...
Path fixed for Astroclaw workspaces: use `SOUL.md`, not `http://SOUL.md`.
...
<CardGroup cols={2}>
  <Card title="Agent workspace" href="/concepts/agent-workspace" icon="folder-open">
    Workspace files Astroclaw injects into the system prompt.
  </Card>
