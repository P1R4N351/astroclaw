...
The private QA stack is meant to exercise Astroclaw in a more realistic,
...
Every QA flow runs under `pnpm astroclaw qa <subcommand>`. Many have `pnpm qa:*`
script aliases; both forms are supported.

| Command                                             | Purpose                                                                                                                                                                                                                                                                 |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qa run`                                            | Bundled QA self-check; writes a Markdown report.                                                                                                                                                                                                                        |
| `qa suite`                                          | Run repo-backed scenarios against the QA gateway lane. Aliases: `pnpm astroclaw qa suite --runner multipass` for a disposable Linux VM.                                                                                                                                  |
...
```bash
pnpm astroclaw qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`astroclaw.run`, `astroclaw.harness.run`, `astroclaw.model.call`,
`astroclaw.context.assembled`, and `astroclaw.message.delivery` must be present;
model calls must not export `StreamAbandoned` on successful turns; raw diagnostic IDs and
`astroclaw.content.*` attributes must stay out of the trace. It writes
...
```bash
pnpm astroclaw qa matrix --profile fast --fail-fast
```
...
CI uses the same command surface in `.github/workflows/qa-live-transports-convex.yml`. Scheduled and default manual runs execute the fast Matrix profile with live frontier credentials, `--fast`, and `ASTROCLAW_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`. Manual `matrix_profile=all` fans out into the five profile shards so the exhaustive catalog can run in parallel while keeping one artifact directory per shard.

For transport-real Telegram, Discord, and Slack smoke lanes:

```bash
pnpm astroclaw qa telegram
pnpm astroclaw qa discord
pnpm astroclaw qa slack
```
...
```bash
pnpm astroclaw qa mantis slack-desktop-smoke \
  --gateway-setup \
...
With `--gateway-setup`, Mantis leaves a persistent Astroclaw Slack gateway
...
```bash
pnpm astroclaw qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `astroclaw infer image describe`
...
```bash
pnpm astroclaw qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds Astroclaw
...
- `ASTROCLAW_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
- `ASTROCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN`
- `ASTROCLAW_QA_TELEGRAM_SUT_BOT_TOKEN`

Optional:

- `ASTROCLAW_QA_TELEGRAM_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts (default redacts).
...
`ASTROCLAW_NPM_TELEGRAM_WARM_SAMPLES` and
`ASTROCLAW_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS`, so `result.json` remains comparable
...
Targets one real private Discord guild channel with two bots: a driver bot controlled by the harness and a SUT bot started by the child Astroclaw gateway through the bundled Discord plugin. Verifies channel mention handling, that the SUT bot has registered the native `/help` command with Discord, and opt-in Mantis evidence scenarios.

Required env when `--credential-source env`:

- `ASTROCLAW_QA_DISCORD_GUILD_ID`
- `ASTROCLAW_QA_DISCORD_CHANNEL_ID`
- `ASTROCLAW_QA_DISCORD_DRIVER_BOT_TOKEN`
- `ASTROCLAW_QA_DISCORD_SUT_BOT_TOKEN`
- `ASTROCLAW_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id returned by Discord (the lane fails fast otherwise).

Optional:

- `ASTROCLAW_QA_DISCORD_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `ASTROCLAW_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for `discord-voice-autojoin`; without it, the scenario picks the first visible voice/stage channel for the SUT bot.
...
- `discord-qa-observed-messages.json` - bodies redacted unless `ASTROCLAW_QA_DISCORD_CAPTURE_CONTENT=1`.
...
Targets one real private Slack channel with two distinct bots: a driver bot controlled by the harness and a SUT bot started by the child Astroclaw gateway through the bundled Slack plugin.

Required env when `--credential-source env`:

- `ASTROCLAW_QA_SLACK_CHANNEL_ID`
- `ASTROCLAW_QA_SLACK_DRIVER_BOT_TOKEN`
- `ASTROCLAW_QA_SLACK_SUT_BOT_TOKEN`
- `ASTROCLAW_QA_SLACK_SUT_APP_TOKEN`

Optional:

- `ASTROCLAW_QA_SLACK_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
...
- `slack-qa-observed-messages.json` - bodies redacted unless `ASTROCLAW_QA_SLACK_CAPTURE_CONTENT=1`.
...
{
  "display_information": {
    "name": "Astroclaw QA Driver",
    "description": "Test driver bot for Astroclaw QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "Astroclaw QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
{
  "display_information": {
    "name": "Astroclaw QA SUT",
    "description": "Astroclaw QA SUT connector for Astroclaw"
  },
  "features": {
    "bot_user": {
      "display_name": "Astroclaw QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#astroclaw-qa`) and invite both bots from inside the channel:

```
/invite @Astroclaw QA Driver
/invite @Astroclaw QA SUT
```
...
Two options. Use env vars for single-machine debugging (set the four `ASTROCLAW_QA_SLACK_*` variables and pass `--credential-source env`), or seed the shared Convex pool so CI and other maintainers can lease them.
...
With `ASTROCLAW_QA_CONVEX_SITE_URL` and `ASTROCLAW_QA_CONVEX_SECRET_MAINTAINER` exported in your shell, register and verify:

```bash
pnpm astroclaw qa credentials add \
  --kind slack \
  --payload-file slack-creds.json \
  --note "QA Slack pool seed"

pnpm astroclaw qa credentials list --kind slack --status all --json
```
...
```bash
pnpm qa:telegram-user:crabbox -- start --tdlib-url http://artifacts.astroclaw.ai/tdlib-v1.8.0-linux-x64.tgz --output-dir .artifacts/qa-e2e/telegram-user-crabbox/pr-review
...
- `mock-openai` is the scenario-aware Astroclaw mock. It remains the default
...
- how `astroclaw qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `astroclaw qa <runner>` instead of registering a competing root command. Runner plugins should declare `qaRunners` in `astroclaw.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`. Keep `runtime-api.ts` light; lazy CLI and runner execution should stay behind separate entrypoints.
...
For the inventory of available scenarios - useful when sizing follow-up work or wiring a new transport - run `pnpm astroclaw qa coverage` (add `--json` for machine-readable output).

For character and style checks, run the same scenario across multiple live model
refs and write a judged Markdown report:

```bash
pnpm astroclaw qa character-eval \
