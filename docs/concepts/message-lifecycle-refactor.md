...
That shape fixes local bugs, but it leaves Astroclaw with too many public
...
That gives Astroclaw at-least-once recovery. Exactly-once behavior exists only
...
- Structured Astroclaw-origin metadata for operational/system output so visible
...
Astroclaw should borrow the vocabulary, not copy the surface.

What Astroclaw needs beyond that model:
...
`thread.post()` style promises are not enough for Astroclaw. They hide the
...
Origin describes who produced a message and how Astroclaw should treat echoes of
that message. It is separate from relation: a message can be a reply to a user
and still be Astroclaw-originated operational output.
...
Core owns the meaning of Astroclaw-originated output. Channels own how that
...
Astroclaw operational output must not be accepted as bot-authored input in shared
...
- **Transport ack:** tells the platform webhook or socket that Astroclaw accepted
...
- **Inbound record ack:** confirms Astroclaw persisted enough inbound metadata to
...
Before bot authorization, receive must apply the shared Astroclaw echo policy
when the channel can decode message origin metadata:

```typescript
function shouldDropAstroclawEcho(params: {
  origin?: MessageOrigin;
  isBotAuthor: boolean;
  isRoomish: boolean;
}): boolean {
  return (
    params.isBotAuthor &&
    params.isRoomish &&
    params.origin?.source === "astroclaw" &&
...
same visible gateway-failure text but without Astroclaw origin metadata still
...
middleware chain, but Astroclaw persists only the safe completed update id after
...
source if we need platform-level redelivery beyond Astroclaw's restart
...
process dies there, Astroclaw cannot know whether the platform message exists
...
Before preflight authorization, core must run the shared Astroclaw echo predicate
whenever `origin.decode` returns Astroclaw-origin metadata. The receive adapter
...
- iMessage monitor delivery records sent messages in an echo cache after a
  ...
  Astroclaw can re-ingest its own final replies as inbound user messages.
...
- Direct-DM helpers can have a reply callback that is the only correct transport
  ...
  that callback.
- Astroclaw gateway failure output must stay visible to humans, but tagged
...
| Slack           | Send adapter handles normal chat posts. Live adapter chooses native stream when thread shape supports it, otherwise draft preview. Receipts preserve thread timestamps. Origin adapter maps Astroclaw gateway failures to Slack `chat.postMessage.metadata` and drops tagged bot-room echoes before `allowBots` authorization.                                  |
...
| Matrix          | Live adapter owns draft event edits, finalization, redaction, encrypted media constraints, and reply-target mismatch fallback. Receive adapter owns encrypted event hydration and dedupe. Origin adapter should encode Astroclaw gateway-failure origin into Matrix event content and drop configured-bot room echoes before `allowBots` handling.              |
...
| Google Chat     | Simple receive plus send adapter with thread relation mapped to spaces and thread ids. Audit `allowBots=true` room behavior for tagged Astroclaw gateway-failure echoes.                                                                                                                                                                                        |
...
- Persist message origin through durable send intents so recovery, replay, and
  chunked sends preserve Astroclaw operational provenance.
...
- Add `astroclaw/plugin-sdk/channel-message`.
...
- Include `MessageOrigin`, origin encode/decode hooks, and the shared
  `shouldDropAstroclawEcho` predicate in the channel-message SDK surface.
...
This is where the model stops being "agent replies" and becomes "Astroclaw sends
messages".
...
- Gateway-failure origin factory and `shouldDropAstroclawEcho` predicate.
...
- Slack tagged Astroclaw gateway failure messages stay visible outbound, tagged
...
- Matrix tagged Astroclaw gateway-failure room echoes from configured bot
...
- Whether Telegram should eventually replace the grammY runner source with a
  ...
  only Astroclaw's persisted restart watermark.
...
- Astroclaw-originated gateway failure output is visible to humans but tagged
