...
An **agent** here is the full per-persona scope: workspace files, auth profiles, model registry, and session store. `agentDir` is the on-disk state directory that holds this per-agent config at `~/.astroclaw/agents/<agentId>/`. A **binding** maps a channel account (e.g. a Slack workspace or a WhatsApp number) to one of those agents.
...
- **Session store** (chat history + routing state) under `~/.astroclaw/agents/<agentId>/sessions`.
...
a local profile, but Astroclaw does not clone OAuth refresh tokens into the
...
Skills are loaded from each agent workspace plus shared roots such as `~/.astroclaw/skills`, then filtered by the effective agent skill allowlist when configured. Use `agents.defaults.skills` for a shared baseline and `agents.list[].skills` for per-agent replacement. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) and [Skills: agent skill allowlists](/tools/skills#agent-skill-allowlists).
...
- State dir: `~/.astroclaw` (or `ASTROCLAW_STATE_DIR`)
- Workspace: `~/.astroclaw/workspace` (or `~/.astroclaw/workspace-<agentId>`)
- Agent dir: `~/.astroclaw/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.astroclaw/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Astroclaw runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.astroclaw/workspace` (or `~/.astroclaw/workspace-<profile>` when `ASTROCLAW_PROFILE` is set).
- State defaults to `~/.astroclaw/agents/main/agent`.
...
```bash
astroclaw agents list --bindings
```
...
<Steps>
  <Step title="Create each agent workspace">
    ...
    ```bash
    astroclaw agents add coding
    astroclaw agents add social
    ```

    Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.astroclaw/agents/<agentId>`.
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.astroclaw/workspace-alex" },
      { id: "mia", workspace: "~/.astroclaw/workspace-mia" },
    ],
  },
  bindings: [
    {
...
<AccordionGroup>
  ...
  <Accordion title="Account-scope detail">
    ...
    - If you later add the same binding for the same agent with an explicit account id, Astroclaw upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
If you want a channel-wide default account when `accountId` is omitted, set `channels.<channel>.defaultAccount` (optional). When unset, Astroclaw falls back to `default` if present, otherwise the first configured account id (sorted).
...
<AccordionGroup>
  <Accordion title="Discord bots per agent">
    ...
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.astroclaw/workspace-main" },
          { id: "coding", workspace: "~/.astroclaw/workspace-coding" },
        ],
      },
      bindings: [
  ...
  <Accordion title="Telegram bots per agent">
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.astroclaw/workspace-main" },
          { id: "alerts", workspace: "~/.astroclaw/workspace-alerts" },
        ],
      },
      bindings: [
  ...
  <Accordion title="WhatsApp numbers per agent">
    ...
    ```bash
    astroclaw channels login --channel whatsapp --account personal
    astroclaw channels login --channel whatsapp --account biz
    ```

    `~/.astroclaw/astroclaw.json` (JSON5):
    ...
    ```js
    {
        ...
        list: [
          {
            ...
            default: true,
            name: "Home",
            workspace: "~/.astroclaw/workspace-home",
            agentDir: "~/.astroclaw/agents/home/agent",
          },
          {
            id: "work",
            name: "Work",
            workspace: "~/.astroclaw/workspace-work",
            agentDir: "~/.astroclaw/agents/work/agent",
          },
        ],
      },
      ...
      channels: {
        whatsapp: {
          accounts: {
            personal: {
              // Optional override. Default: ~/.astroclaw/credentials/whatsapp/personal
              // authDir: "~/.astroclaw/credentials/whatsapp/personal",
            },
            biz: {
              // Optional override. Default: ~/.astroclaw/credentials/whatsapp/biz
        ...
        list: [
          {
            id: "chat",
            name: "Everyday",
            workspace: "~/.astroclaw/workspace-chat",
            model: "anthropic/claude-sonnet-4-6",
          },
          {
            id: "opus",
            name: "Deep Work",
            workspace: "~/.astroclaw/workspace-opus",
