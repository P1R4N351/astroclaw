---
summary: "What experimental flags mean in Astroclaw and which ones are currently documented"
...
Experimental features in Astroclaw are **opt-in preview surfaces**. They are
...
- If you are rolling Astroclaw out broadly, test experimental flags in a smaller
...
| ------------------------ | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Local model runtime      | `agents.defaults.experimental.localModelLean`             | A smaller or stricter local backend chokes on Astroclaw's full default tool surface                             | [Local Models](/gateway/local-models)                                                         |
...
`agents.defaults.experimental.localModelLean: true` is a pressure-release valve for weaker local-model setups. When it is on, Astroclaw drops three default tools — `browser`, `cron`, and `message` — from the agent's tool surface for every turn. Nothing else changes.

### Why these three tools

These three tools have the largest descriptions and the most parameter shapes in the default Astroclaw runtime. On a small-context or stricter OpenAI-compatible backend that is the difference between:
...
Removing them does not silently rewire Astroclaw — it just makes the tool list shorter. The model still has `read`, `write`, `edit`, `exec`, `apply_patch`, web search/fetch (when configured), memory, and session/agent tools available.
...
1. `astroclaw infer model run --gateway --model <ref> --prompt "Reply with exactly: pong"` succeeds.
...
If a feature is experimental, Astroclaw should say so plainly in docs and in the
