...
Paste this into `astroclaw.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, Astroclaw can show:
...
case-insensitive. If `allowedChatIds` is non-empty and Astroclaw cannot resolve a
...
`config.promptOverride` replaces the default Active Memory prompt. Astroclaw
...
5. Verify memory search itself works with `astroclaw memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, Astroclaw auto-detects the first
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `astroclaw memory status --deep` to inspect the memory-search backend
      and index health.
