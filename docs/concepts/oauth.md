---
summary: "OAuth in Astroclaw: token exchange, storage, and multi-account patterns"
read_when:
  - You want to understand Astroclaw OAuth end-to-end
...
Astroclaw also supports **provider plugins** that ship their own OAuth or API-key
flows. Run them via:

```bash
astroclaw models auth login --provider <id>
```
...
- you log in via Astroclaw _and_ via Claude Code / Codex CLI → one of them randomly gets "logged out" later

To reduce that, Astroclaw treats `auth-profiles.json` as a **token sink**:

- the runtime reads credentials from **one place**
- we can keep multiple profiles and route them deterministically
- external CLI reuse is provider-specific: Codex CLI can bootstrap an empty
  `openai-codex:default` profile, but once Astroclaw has a local OAuth profile,
  the local refresh token is canonical. If that local refresh token is rejected,
  Astroclaw can use a usable same-account Codex CLI token as a runtime-only
...
- Auth profiles (OAuth + API keys + optional value-level refs): `~/.astroclaw/agents/<agentId>/agent/auth-profiles.json`
- Legacy compatibility file: `~/.astroclaw/agents/<agentId>/agent/auth.json`
  (static `api_key` entries are scrubbed when discovered)

Legacy import-only file (still supported, but not the main store):

- `~/.astroclaw/credentials/oauth.json` (imported into `auth-profiles.json` on first use)

All of the above also respect `$ASTROCLAW_STATE_DIR` (state dir override). Full reference: [/gateway/configuration](/gateway/configuration-reference#auth-storage)

For static secret refs and runtime snapshot activation behavior, see [Secrets Management](/gateway/secrets).

When a secondary agent has no local auth profile, Astroclaw uses read-through
...
Claude subscription limits, and Anthropic staff told us Astroclaw-style Claude
CLI usage is allowed again. Astroclaw therefore treats Claude CLI reuse and
...
If you want other subscription-style options in Astroclaw, see [OpenAI
Codex](/providers/openai), [Qwen Cloud Coding
Plan](/providers/qwen), [MiniMax Coding Plan](/providers/minimax),
and [Z.AI / GLM Coding Plan](/providers/glm).
</Warning>

Astroclaw also exposes Anthropic setup-token as a supported token-auth path, but it now prefers Claude CLI reuse and `claude -p` when available.

## Anthropic Claude CLI migration

Astroclaw supports Anthropic Claude CLI reuse again. If you already have a local
Claude login on the host, onboarding/configure can reuse it directly.

## OAuth exchange (how login works)

Astroclaw's interactive login flows are implemented in `@earendil-works/pi-ai` and wired into the wizards/commands.

### Anthropic setup-token

Flow shape:

1. start Anthropic setup-token or paste-token from Astroclaw
2. Astroclaw stores the resulting Anthropic credential in an auth profile
3. model selection stays on `anthropic/...`
4. existing Anthropic auth profiles remain available for rollback/order control

### OpenAI Codex (ChatGPT OAuth)

OpenAI Codex OAuth is explicitly supported for use outside the Codex CLI, including Astroclaw workflows.
...
Wizard path is `astroclaw onboard` → auth choice `openai-codex`.
...
- exception: some external CLI credentials stay externally managed; Astroclaw
  ...
  `openai-codex:default` profile, then Astroclaw-owned refreshes keep the local
  profile canonical. If the local Codex refresh fails and Codex CLI has a
  usable token for the same account, Astroclaw may use that token for the current
...
- `astroclaw channels list --json` (shows `auth[]`)
