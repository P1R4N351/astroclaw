...
alongside Astroclaw. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, Astroclaw falls back to the
...
Astroclaw creates a self-contained QMD home under
`~/.astroclaw/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- Astroclaw creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so Astroclaw skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, Astroclaw retries with `qmd query`.
- With QMD releases that advertise multi-collection filters, Astroclaw groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, Astroclaw falls back to the builtin SQLite engine.
  ...
  `astroclaw memory status` and one-shot CLI probes still recheck QMD directly.
...
Astroclaw keeps the QMD search path compatible with both current and older QMD
installs.

On startup, Astroclaw checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, Astroclaw searches all
...
Older QMD builds only accept one collection filter. When Astroclaw detects one
...
process, so you can tune QMD globally without adding new Astroclaw config:
...
collection under `~/.astroclaw/agents/<id>/qmd/sessions/`.
...
When scope denies a search, Astroclaw logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If Astroclaw
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but Astroclaw still reports
...
with `astroclaw memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs Astroclaw uses.

**Many QMD subprocesses during search?** Update QMD if possible. Astroclaw uses
...
`memory.qmd.searchMode = "search"`. Astroclaw treats that mode as lexical-only,
...
the next startup reconciliation. Astroclaw recreates stale managed collections
...
Astroclaw's builtin symlink rules. Keep temporary monorepo checkouts under
