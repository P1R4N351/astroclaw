---
summary: "What the Astroclaw system prompt contains and how it is assembled"
read_when:
...
Astroclaw builds a custom system prompt for every agent run. The prompt is **Astroclaw-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by Astroclaw and injected into each agent run.
...
the full Astroclaw-owned prompt. The provider runtime can:
...
- **Astroclaw Control**: tells the model to prefer the `gateway` tool for
  config/restart work and to avoid inventing CLI commands.
- **Astroclaw Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to Astroclaw docs/source and when to read them.
...
Astroclaw keeps large stable content, including **Project Context**, above the
...
Astroclaw can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Memory Recall**, **Astroclaw
...
For channel auto-reply runs, Astroclaw omits the generic **Silent Replies**
...
Astroclaw keeps committed prompt snapshots for the Codex runtime happy path under
...
Astroclaw developer instructions, turn-scoped collaboration-mode instructions
when Astroclaw provides them, user turn input, and references to the dynamic tool
specs.
...
collaboration-mode instructions inside the Codex runtime after Astroclaw sends
...
through its own project-doc discovery. Astroclaw still resolves the remaining
...
occurs, Astroclaw can inject a concise system-prompt warning notice; control this with
...
When eligible skills exist, Astroclaw injects a compact **available skills list**
...
points to the local Astroclaw docs directory (`docs/` in a Git checkout or the bundled npm
package docs). If local docs are unavailable, it falls back to
[https://docs.astroclaw.ai](https://docs.astroclaw.ai).

The same section also includes the Astroclaw source location. Git checkouts expose the local
...
consult docs first for Astroclaw behavior, commands, configuration, or architecture, and to
run `astroclaw status` itself when possible (asking the user only when it lacks access).
