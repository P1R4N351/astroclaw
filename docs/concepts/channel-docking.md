---
summary: "Move one Astroclaw session's reply route between linked chat channels"
...
Channel docking is call forwarding for one Astroclaw session.
...
Alice can message Astroclaw on Telegram and Discord:
...
Astroclaw keeps the current session context and changes the reply route:
...
4. Keep the same Astroclaw session, but receive future replies in Discord.
