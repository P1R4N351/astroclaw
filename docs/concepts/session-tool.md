...
Astroclaw gives agents tools to work across sessions, inspect status, and
...
After the target responds, Astroclaw can run a **reply-back loop** where the
...
the caller's current session; visible client labels such as `astroclaw-tui` are
not session keys.
...
`subagents` is the control-plane helper for already spawned Astroclaw
...
the completion origin only identifies a channel Astroclaw can still reuse the
