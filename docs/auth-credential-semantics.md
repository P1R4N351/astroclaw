...
Explicit copy flows, such as `astroclaw agents add`, use this portability policy:
...
has such a marker, `astroclaw doctor --fix` moves it to `auth.profiles` and
...
- Probe targets can come from auth profiles, environment credentials, or
  `models.json`.
- If a provider has credentials but Astroclaw cannot resolve a probeable model
