---
summary: "OpenProse: .prose workflows, slash commands, and state in Astroclaw"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In Astroclaw it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
```bash
astroclaw plugins enable open-prose
```

Restart the Gateway after enabling the plugin.

Dev/local checkout: `astroclaw plugins install ./path/to/local/open-prose-plugin`

Related docs: [Plugins](/tools/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses Astroclaw tools under the hood.
...
## Astroclaw runtime mapping

OpenProse programs map to Astroclaw primitives:

| OpenProse concept         | Astroclaw tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use Astroclaw tool allowlists and approval gates to control side effects.
