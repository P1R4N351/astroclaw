...
When a conversation or thread is bound to an ACP session, normal follow-up text routes to that ACP harness. Gateway management commands still stay local: `/acp ...` always reaches the Astroclaw ACP command handler, and `/status` plus `/unfocus` stay local whenever command handling is enabled for the surface.
...
On Discord, native command specs may include `descriptionLocalizations`, which Astroclaw publishes as Discord `description_localizations` and includes in reconcile comparisons.
...
<ParamField path="commands.config" type="boolean" default="false">
  Enables `/config` (reads/writes `astroclaw.json`).
</ParamField>
<ParamField path="commands.mcp" type="boolean" default="false">
  Enables `/mcp` (reads/writes Astroclaw-managed MCP config under `mcp.servers`).
</ParamField>
...
<AccordionGroup>
  <Accordion title="Sessions and runs">
    ...
    - `/export-trajectory [path]` asks for exec approval, then exports a JSONL [trajectory bundle](/tools/trajectory) for the current session. Use it when you need the prompt, tool, and transcript timeline for one Astroclaw session. In group chats, the approval prompt and export result go to the owner privately. Alias: `/trajectory`.
  ...
  <Accordion title="Discovery and status">
    ...
    - `/diagnostics [note]` is the owner-only support-report flow for Gateway bugs and Codex harness runs. It asks for explicit exec approval every time before running `astroclaw gateway diagnostics export --json`; do not approve diagnostics with an allow-all rule. After approval, it sends a pasteable report with the local bundle path, manifest summary, privacy notes, and relevant session ids. In group chats, the approval prompt and report go to the owner privately. When the active session uses the OpenAI Codex harness, the same approval also sends relevant Codex feedback to OpenAI servers and the completed reply lists the Astroclaw session ids, Codex thread ids, and `codex resume <thread-id>` commands. See [Diagnostics Export](/gateway/diagnostics).
  ...
  <Accordion title="Owner-only writes and admin">
    - `/config show|get|set|unset` reads or writes `astroclaw.json`. Owner-only. Requires `commands.config: true`.
    - `/mcp show|get|set|unset` reads or writes Astroclaw-managed MCP server config under `mcp.servers`. Owner-only. Requires `commands.mcp: true`.
    ...
    - `/restart` restarts Astroclaw when enabled. Default: enabled; set `commands.restart: false` to disable it.
...
<AccordionGroup>
  <Accordion title="Argument and parser notes">
    ...
    - For full provider usage breakdown, use `astroclaw status --usage`.
    - `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
    - In multi-account channels, config-targeted `/allowlist --account <id>` and `/config set channels.<provider>.accounts.<id>...` also honor the target account's `configWrites`.
    - `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from Astroclaw session logs.
    - `/restart` is enabled by default; set `commands.restart: false` to disable it.
    - `/plugins install <spec>` accepts the same plugin specs as `astroclaw plugins install`: local path/archive, npm package, `git:<repo>`, or `clawhub:<pkg>`. Managed Gateways restart automatically because plugin source modules changed.
...
- **Provider usage/quota** (example: "Claude 80% left") shows up in `/status` for the current model provider when usage tracking is enabled. Astroclaw normalizes provider windows to `% left`; for MiniMax, remaining-only percent fields are inverted before display, and `model_remains` responses prefer the chat-model entry plus a model-tagged plan label.
...
- **Execution vs runtime:** `/status` reports `Execution` for the effective sandbox path and `Runtime` for who is actually running the session: `Astroclaw Pi Default`, `OpenAI Codex`, a CLI backend, or an ACP backend.
...
```
/debug show
/debug set messages.responsePrefix="[astroclaw]"
...
<Note>
Overrides apply immediately to new config reads, but do **not** write to `astroclaw.json`. Use `/debug reset` to clear all overrides and return to the on-disk config.
</Note>
...
`/config` writes to your on-disk config (`astroclaw.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[astroclaw]"
...
`/mcp` writes Astroclaw-managed MCP server definitions under `mcp.servers`. Owner-only. Disabled by default; enable with `commands.mcp: true`.
...
<Note>
`/mcp` stores config in Astroclaw config, not Pi-owned project settings. Runtime adapters decide which transports are actually executable.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Slack specifics">
    `channels.slack.slashCommand` is still supported for a single `/astroclaw`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
