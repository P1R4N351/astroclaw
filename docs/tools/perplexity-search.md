...
Astroclaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Astroclaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `astroclaw configure --section web`. It stores the key in
`~/.astroclaw/astroclaw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.astroclaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
