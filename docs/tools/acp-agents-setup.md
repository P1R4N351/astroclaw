...
read_when:
  ...
  - Enabling the plugin-tools or Astroclaw-tools MCP bridge
...
Codex has two Astroclaw routes:
...
When Astroclaw uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Astroclaw `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by Astroclaw before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, Astroclaw/acpx cannot force a model selection.
...
Packaged installs use the official `@astroclaw/acpx` runtime plugin for ACP.
...
```bash
astroclaw plugins install ./path/to/local/acpx-plugin
```
...
`ASTROCLAW_ACPX_RUNTIME_STARTUP_PROBE=0` to skip the startup probe and register
...
- `command` accepts an absolute path, relative path (resolved from the Astroclaw workspace), or command name.
...
- `agents.<id>.args` is optional. Each array item is shell-quoted before Astroclaw passes it through the current acpx command-string registry.

See [Plugins](/tools/plugin).

### Automatic dependency install

When you install Astroclaw globally with `npm install -g astroclaw`, the acpx
...
normally and reports the missing dependency through `astroclaw acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose Astroclaw plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
Astroclaw plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
astroclaw config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `astroclaw-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled Astroclaw
  plugins.
...
### Astroclaw tools MCP bridge

By default, ACPX sessions also do **not** expose built-in Astroclaw tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
astroclaw config set plugins.entries.acpx.config.astroClawToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `astroclaw-tools` into ACPX session
  bootstrap.
- Exposes selected built-in Astroclaw tools. The initial server exposes `cron`.
...
```bash
astroclaw config set plugins.entries.acpx.config.timeoutSeconds 180
```
...
```bash
astroclaw config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from Astroclaw exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.
...
```bash
astroclaw config set plugins.entries.acpx.config.permissionMode approve-all
astroclaw config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

<Warning>
Astroclaw defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
