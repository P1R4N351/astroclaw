...
Astroclaw supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), Astroclaw reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.astroclaw/.env`.

If you omit `baseUrl`, Astroclaw defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Astroclaw defaults to `kimi-k2.6`.
...
Astroclaw treats Kimi `web_search` as successful only after Moonshot returns
...
Astroclaw returns a structured `kimi_web_search_ungrounded` error instead of
