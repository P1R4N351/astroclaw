...
read_when:
  - Running Astroclaw Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, Astroclaw Gateway runs inside WSL2, Chrome runs on Windows, and browser control must cross the WSL2 and Windows boundary. The layered failure pattern from [issue #39369](https://github.com/astroclaw/astroclaw/issues/39369) means several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Astroclaw and Chrome are on the same machine
...
- Astroclaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Astroclaw is not the problem yet.
...
Fix that before touching Astroclaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Astroclaw at the address that is reachable from WSL2:
...
- use HTTP(S) when you want Astroclaw to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting Astroclaw to succeed
...
```bash
astroclaw browser open https://example.com --browser-profile remote
astroclaw browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `astroclaw browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `astroclaw browser stop --browser-profile remote`
...
3. Astroclaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Astroclaw config or Control UI auth
