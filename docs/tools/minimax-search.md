...
Astroclaw supports MiniMax as a `web_search` provider through the MiniMax
...
Astroclaw also accepts `MINIMAX_CODING_API_KEY`, `MINIMAX_OAUTH_TOKEN`, and
...
If `plugins.entries.minimax.config.webSearch.region` is unset, Astroclaw resolves
...
| `count`   | integer | 1-10        | Number of results to return. Astroclaw trims the returned list to this size. |
