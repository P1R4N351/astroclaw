...
Astroclaw can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `astroclaw configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `astroclaw doctor --fix`.
...
when you run Firecrawl yourself. Astroclaw accepts `http://` only for loopback,
...
Astroclaw always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, Astroclaw
