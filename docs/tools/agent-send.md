...
`astroclaw agent` runs a single agent turn from the command line without needing
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    astroclaw agent --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    astroclaw agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    astroclaw agent --session-id abc123 --message "Continue the task"
    ```
  ...
  <Step title="Deliver the reply to a channel">
    ...
    astroclaw agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    astroclaw agent --agent ops --message "Generate report" \
...
astroclaw agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
...
<CardGroup cols={2}>
  <Card title="Agent CLI reference" href="/cli/agent" icon="terminal">
    Full `astroclaw agent` flag and option reference.
  </Card>
