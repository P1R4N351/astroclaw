---
summary: "Export redacted trajectory bundles for debugging an Astroclaw agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an Astroclaw session
...
Trajectory capture is Astroclaw's per-session flight recorder. It records a
...
Astroclaw writes the bundle under the workspace:

```text
.astroclaw/trajectory-exports/astroclaw-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.astroclaw/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, Astroclaw sends the
...
```bash
astroclaw sessions export-trajectory --session-key "agent:main:telegram:direct:123" --workspace .
```
...
Trajectory capture is on by default for Astroclaw agent runs.
...
| `metadata.json`       | Astroclaw version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
Astroclaw also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `ASTROCLAW_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export ASTROCLAW_TRAJECTORY_DIR=/var/lib/astroclaw/trajectories
```

When this variable is set, Astroclaw writes one JSONL file per session id in that
directory.
...
Set `ASTROCLAW_TRAJECTORY=0` before starting Astroclaw:

```bash
export ASTROCLAW_TRAJECTORY=0
```
...
Astroclaw flushes runtime trajectory sidecars during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`ASTROCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting Astroclaw:

```bash
export ASTROCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when Astroclaw logs a `pi-trajectory-flush` timeout and continues.
It does not change the trajectory size caps. To tune all agent cleanup steps
that do not pass an explicit timeout, set `ASTROCLAW_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are designed for support and debugging, not public posting.
Astroclaw redacts sensitive values before writing export files:
...
- confirm Astroclaw was started without `ASTROCLAW_TRAJECTORY=0`
- check whether `ASTROCLAW_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.astroclaw/trajectory-exports/`
