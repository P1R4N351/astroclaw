...
Run shell commands in the workspace. `exec` is a mutating shell surface: commands can create, edit, or delete files wherever the selected host or sandbox filesystem permits. Disabling Astroclaw filesystem tools such as `write`, `edit`, or `apply_patch` does not make `exec` read-only.
...
`tools.exec.security` and `~/.astroclaw/exec-approvals.json`; elevated mode can
...
- `gateway`/`node` approvals are controlled by `~/.astroclaw/exec-approvals.json`.
...
- `timeout` applies to foreground, background, `yieldMs`, gateway, sandbox, and node `system.run` execution. If omitted, Astroclaw uses `tools.exec.timeoutSec`; explicit `timeout: 0` disables the exec process timeout for that call.
...
- Astroclaw sets `ASTROCLAW_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
- `astroclaw channels login` is blocked from `exec` because it is an interactive channel-auth flow; run it in a terminal on the gateway host, or use the channel-native login tool from chat when one exists.
...
- No-approval host exec is the default for gateway + node. If you want approvals/allowlist behavior, tighten both `tools.exec.*` and the host `~/.astroclaw/exec-approvals.json`; see [Exec approvals](/tools/exec-approvals#yolo-mode-no-approval).
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Astroclaw prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`astroclaw security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `astroclaw doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`astroclaw security audit` and `astroclaw doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
