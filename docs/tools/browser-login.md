...
When a site requires login, **sign in manually** in the **host** browser profile (the astroclaw browser).
...
Astroclaw controls a **dedicated Chrome profile** (named `astroclaw`, orange-tinted UI). This is separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent should use its isolated `astroclaw` browser.
...
astroclaw browser open https://x.com
```

If you have multiple profiles, pass `--browser-profile <name>` (the default is `astroclaw`).
...
```bash
astroclaw browser open https://x.com --browser-profile astroclaw
```
