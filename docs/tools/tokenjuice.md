...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in Astroclaw
...
Today this applies to PI embedded runs and Astroclaw dynamic tools in the Codex
app-server harness. Tokenjuice hooks Astroclaw's tool-result middleware and
...
```bash
astroclaw config set plugins.entries.tokenjuice.enabled true
```

Equivalent:

```bash
astroclaw plugins enable tokenjuice
```

Astroclaw already ships the plugin. There is no separate `plugins install`
or `tokenjuice install astroclaw` step.
...
```bash
astroclaw config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
astroclaw plugins disable tokenjuice
```
