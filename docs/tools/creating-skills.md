...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.astroclaw/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Astroclaw picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    astroclaw agent --message "give me a greeting"
    ```
...
| `metadata.astroclaw.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.)                    |
| `metadata.astroclaw.requires.bins`   | No       | Required binaries on PATH                                      |
| `metadata.astroclaw.requires.config` | No       | Required config keys                                           |
...
- **Test locally** — use `astroclaw agent --message "..."` to test before sharing
...
| `~/.astroclaw/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Astroclaw) | Low        | Global                |
