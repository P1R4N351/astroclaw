...
Astroclaw supports Grok as a `web_search` provider, using xAI web-grounded
...
key under `plugins.entries.xai.config.webSearch.apiKey`, Astroclaw now reuses it
...
Astroclaw can show a separate follow-up step to enable `x_search` with the same
...
route through an operator proxy or xAI-compatible Responses endpoint. Astroclaw
