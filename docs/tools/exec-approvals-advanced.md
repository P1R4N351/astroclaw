...
- allowlist entries live in host-local `~/.astroclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `astroclaw approvals allowlist ...`).
- `astroclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `astroclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Astroclaw still rejects the `env` builtin in safe-bin
...
After an approved async exec finishes, Astroclaw sends a followup `agent` turn to the same session.
...
the originating channel, Astroclaw keeps the local deterministic `/approve`
prompt visible. If the native runtime is active and attempts delivery but no
target receives the card, Astroclaw sends a same-chat fallback notice with the
...
- Matrix native prompts include `com.astroclaw.approval` custom event content on the first prompt
  event so Astroclaw-aware Matrix clients can read structured approval state while stock clients
...
owner routing for approval prompts and final results. Astroclaw first tries a private route on the
...
topics, Astroclaw preserves the topic for the approval prompt and the post-approval follow-up.
