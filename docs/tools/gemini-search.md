...
Astroclaw supports Gemini models with built-in
...
For a gateway install, put env keys in `~/.astroclaw/.env`.
...
`pd`, `pw`, `pm`, and `py`. Astroclaw converts these values, or an explicit
