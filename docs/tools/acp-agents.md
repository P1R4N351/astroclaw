---
summary: "Run external coding harnesses (Claude Code, Cursor, Gemini CLI, explicit Codex ACP, Astroclaw ACP, OpenCode) through the ACP backend"
read_when:
...
let Astroclaw run external coding harnesses (for example Pi, Claude Code,
Cursor, Copilot, Droid, Astroclaw ACP, OpenCode, Gemini CLI, and other
...
directly to existing Astroclaw channel conversations, use
[`astroclaw mcp serve`](/cli/mcp) instead of ACP.
</Note>
...
| Run Claude Code, Gemini CLI, explicit Codex ACP, or another external harness _through_ Astroclaw | This page                             | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls                                                                                   |
| Expose an Astroclaw Gateway session _as_ an ACP server for an editor or client                   | [`astroclaw acp`](/cli/acp)            | Bridge mode. IDE/client talks ACP to Astroclaw over stdio/WebSocket                                                                                                                            |
| Reuse a local AI CLI as a text-only fallback model                                              | [CLI Backends](/gateway/cli-backends) | Not ACP. No Astroclaw tools, no ACP controls, no harness runtime                                                                                                                               |
...
```bash
astroclaw plugins install @astroclaw/acpx
astroclaw config set plugins.entries.acpx.enabled true
```
...
Astroclaw only teaches agents about ACP spawning when ACP is **truly
...
<AccordionGroup>
  <Accordion title="First-run gotchas">
    ...
    - Codex ACP runs with an isolated `CODEX_HOME`; Astroclaw copies trusted project entries plus safe model/provider routing config from the host Codex config, while auth, notifications, and hooks stay on the host config.
  ...
  <Accordion title="Runtime prerequisites">
    ACP launches a real external harness process. Astroclaw owns routing,
    ...
    Before blaming Astroclaw, verify:
...
Astroclaw plugin tools and built-in Astroclaw tools are **not** exposed to
...
| `astroclaw` | Astroclaw Gateway bridge through `astroclaw acp` | Lets an ACP-aware harness talk back to an Astroclaw Gateway session.                 |
| `pi`       | Pi/embedded Astroclaw runtime                   | Used for Astroclaw-native harness experiments.                                       |
| `qwen`     | Qwen Code / Qwen CLI                           | Requires Qwen-compatible auth on the host.                                          |

Custom acpx agent aliases can be configured in acpx itself, but Astroclaw
...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    - Spawn creates or resumes an ACP runtime session, records ACP metadata in the Astroclaw session store, and may create a background task when the run is parent-owned.
    ...
    - `close` ends the ACP session from Astroclaw's point of view and removes the binding. A harness may still keep its own upstream history if it supports resume.
    - The acpx plugin cleans up Astroclaw-owned wrapper and adapter process trees after `close`, and reaps stale Astroclaw-owned ACPX orphans during Gateway startup.
  ...
  <Accordion title="Native Codex routing rules">
    ...
    Astroclaw dynamic tools still execute through Astroclaw, while
    Codex-native tools such as shell/apply-patch execute inside Codex.
    For Codex-native tool events, Astroclaw injects a per-turn native
    hook relay so plugin hooks can block `before_tool_call`, observe
    `after_tool_call`, and route Codex `PermissionRequest` events
    through Astroclaw approvals. Codex `Stop` hooks are relayed to
    Astroclaw `before_agent_finalize`, where plugins can request one more
  ...
  <Accordion title="ACP-routing natural-language triggers">
    ...
    Astroclaw picks `runtime: "acp"`, resolves the harness `agentId`,
    ...
    Astroclaw config agent id from `agents_list` unless that entry is
    explicitly configured with `agents.list[].runtime.type="acp"`;
    otherwise use the default sub-agent runtime. When an Astroclaw agent
    is configured with `runtime.type="acp"`, Astroclaw uses
...
plugin is enabled. Use **sub-agents** when you want Astroclaw-native
delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Astroclaw native sub-agent runtime  |
...
1. Astroclaw ACP session control plane.
2. Official `@astroclaw/acpx` runtime plugin.
...
- **ACP session** - the durable Codex/Claude/Gemini runtime state Astroclaw routes to.
...
spawned ACP session - no child thread, same chat surface. Astroclaw keeps
...
<AccordionGroup>
  <Accordion title="Binding rules and exclusivity">
    ...
    - `--bind here` only works on channels that advertise current-conversation binding; Astroclaw returns a clear unsupported message otherwise. Bindings persist across gateway restarts.
    - On Discord, `spawnSessions` gates child thread creation for `--thread auto|here` - not `--bind here`.
    - If you spawn to a different ACP agent without `--cwd`, Astroclaw inherits the **target agent's** workspace by default. Missing inherited paths (`ENOENT`/`ENOTDIR`) fall back to the backend default; other access errors (e.g. `EACCES`) surface as spawn errors.
    - Gateway management commands stay local in bound conversations - `/acp ...` commands are handled by Astroclaw even when normal follow-up text routes to the bound ACP session; `/status` and `/unfocus` also stay local whenever command handling is enabled for that surface.

  </Accordion>
  <Accordion title="Thread-bound sessions">
    When thread bindings are enabled for a channel adapter:

    - Astroclaw binds a thread to a target ACP session.
    ...
    adapter does not support thread bindings, Astroclaw returns a clear
...
- Astroclaw ensures the configured ACP session exists before use.
...
- For cross-agent ACP spawns without an explicit `cwd`, Astroclaw inherits the target agent workspace from agent config.
...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    for ACP sessions. If `agentId` is omitted, Astroclaw uses
...
<ParamField path="mode" type='"run" | "session"' default="run">
  ...
  `mode` is omitted, Astroclaw may default to persistent behaviour per
...
<ParamField path="model" type="string">
  ...
  normalize Astroclaw Codex refs such as `openai-codex/gpt-5.4` to Codex
  ...
  `session/set_model`; otherwise Astroclaw/acpx fails clearly instead of
...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    What Astroclaw sends to the harness:
    ...
    - Runtime-generated completion events are materialized per target. Astroclaw agents get Astroclaw's internal runtime-context envelope; external ACP harnesses get a plain prompt with the child result and instruction. The raw `<<<BEGIN_ASTROCLAW_INTERNAL_CONTEXT>>>` envelope should never be sent to external harnesses or persisted as ACP user transcript text.
    - ACP transcript entries use the user-visible trigger text or the plain completion prompt. Internal event metadata stays structured in Astroclaw where possible and is not treated as user-authored chat content.
  ...
  <Accordion title="Parent-owned one-shot ACP sessions">
    ...
    - Completion reports back through the task-completion announce path. Astroclaw converts internal completion metadata into a plain ACP prompt before sending it to an external harness, so harnesses do not see Astroclaw-only runtime context markers.
  ...
  <Accordion title="sessions_send and A2A delivery">
    ...
    peer sessions, Astroclaw uses an agent-to-agent (A2A) follow-up path
    ...
    Astroclaw skips the A2A follow-up only when the requester is the
  ...
  <Accordion title="Resume an existing session">
    ...
    - `resumeSessionId` is a host-local ACP/harness resume id, not an Astroclaw channel session key; Astroclaw still checks ACP spawn policy and target agent policy before dispatch, while the ACP backend or harness owns authorization for loading that upstream id.
    - `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new Astroclaw session you are creating, so `mode: "session"` still requires `thread: true`.
...
- Astroclaw's sandbox policy does **not** wrap ACP harness execution.
- Astroclaw still enforces ACP feature gates, allowed agents, session ownership, channel bindings, and Gateway delivery policy.
- Use `runtime: "subagent"` for sandbox-enforced Astroclaw-native work.
...
If no target resolves, Astroclaw returns a clear error
...
| ---------------------------- | ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/acp model <id>`            | runtime config key `model`           | For Codex ACP, Astroclaw normalizes `openai-codex/<model>` to the adapter model id and maps slash reasoning suffixes such as `openai-codex/gpt-5.4/high` to `reasoning_effort`.                             |
| `/acp set thinking <level>`  | canonical option `thinking`          | Astroclaw sends the backend-advertised equivalent when present, preferring `thinking`, then `effort`, `reasoning_effort`, or `thought_level`. For Codex ACP, the adapter maps values to `reasoning_effort`. |
| `/acp permissions <profile>` | canonical option `permissionProfile` | Astroclaw sends the backend-advertised equivalent when present, such as `approval_policy`, `permission_profile`, `permissions`, or `permission_mode`.                                                       |
| `/acp timeout <seconds>`     | canonical option `timeoutSeconds`    | Astroclaw sends the backend-advertised equivalent when present, such as `timeout` or `timeout_seconds`.                                                                                                     |
...
aliases), the plugin-tools and Astroclaw-tools MCP bridges, and ACP
...
| Vendor auth error from the harness                                          | Astroclaw is healthy, but the target CLI/provider is not logged in.                                                     | Log in or provide the required provider key on the Gateway host environment.                                                                                             |
...
| ACP session stalls indefinitely after completing work                       | Harness process finished but ACP session did not report completion.                                                    | Update Astroclaw; current acpx cleanup reaps Astroclaw-owned stale wrapper and adapter processes on close and Gateway startup.                                             |
| Harness sees `<<<BEGIN_ASTROCLAW_INTERNAL_CONTEXT>>>`                        | Internal event envelope leaked across the ACP boundary.                                                                | Update Astroclaw and rerun the completion flow; external harnesses should receive plain completion prompts only.                                                          |
...
- [`astroclaw acp` (bridge mode)](/cli/acp)
