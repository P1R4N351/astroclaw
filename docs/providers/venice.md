---
summary: "Use Venice AI privacy-focused models in Astroclaw"
read_when:
  - You want privacy-focused inference in Astroclaw
...
<Steps>
  ...
  <Step title="Configure Astroclaw">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        astroclaw onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    astroclaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, Astroclaw shows all available Venice models. Pick based on your needs:
...
```bash
astroclaw models set venice/kimi-k2-5
astroclaw models set venice/claude-opus-4-6
```

List all available models:

```bash
astroclaw models list --all --provider venice
```

You can also run `astroclaw configure`, select **Model/auth**, and choose **Venice AI**.
...
`venice/deepseek-v4-flash`, Astroclaw fills the required DeepSeek V4
...
Astroclaw keeps that provider-specific replay fix separate from the native
...
Astroclaw ships a manifest-backed Venice seed catalog for read-only model listing. Runtime refresh can still discover models from the Venice API, and falls back to the manifest catalog if the API is unreachable.
...
astroclaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
astroclaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
astroclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
astroclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
astroclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  <Accordion title="API key not recognized">
    ```bash
    echo $VENICE_API_KEY
    astroclaw models list | grep venice
    ```
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `astroclaw models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
