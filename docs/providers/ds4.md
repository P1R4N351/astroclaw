---
summary: "Run Astroclaw through ds4, a local DeepSeek V4 Flash OpenAI-compatible server"
read_when:
  - You want to run Astroclaw against antirez/ds4
  - You want a local DeepSeek V4 Flash backend with tool calls
  - You need the Astroclaw config for ds4-server
title: "ds4"
---

[ds4](https://github.com/antirez/ds4) serves DeepSeek V4 Flash from a local
Metal backend with an OpenAI-compatible `/v1` API. Astroclaw connects to ds4
through the generic `openai-completions` provider family.

ds4 is not a bundled Astroclaw provider plugin. Configure it under
...
<Warning>
Astroclaw agent turns include tool schemas and workspace context. A tiny context
...
<Steps>
  ...
  <Step title="Add the Astroclaw provider config">
    ...
    astroclaw infer model run \
      ...
      --thinking off \
      --prompt "Reply with exactly: astroclaw-ds4-ok" \
      --json
    ```
...
aligned with `--tokens` unless you intentionally want Astroclaw to request less
output than the server default.

## On-demand startup

Astroclaw can start ds4 only when a `ds4/...` model is selected. Add
...
If you run that large context, update both the server flags and Astroclaw model
metadata:
...
Then test Astroclaw model routing:
...
<AccordionGroup>
  ...
  <Accordion title="500 prompt exceeds context">
    The configured `--ctx` is too small for the Astroclaw turn. Raise
  ...
  <Accordion title="The first request is slow">
    ...
    `localService.readyTimeoutMs: 300000` when Astroclaw starts the server on
    demand.
  </Accordion>
</AccordionGroup>
