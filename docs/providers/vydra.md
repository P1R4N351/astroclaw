---
summary: "Use Vydra image, video, and speech in Astroclaw"
read_when:
  - You want Vydra media generation in Astroclaw
...
Astroclaw uses the same `VYDRA_API_KEY` for all three capabilities.
...
<Steps>
  <Step title="Run interactive onboarding">
    ```bash
    astroclaw onboard --auth-choice vydra-api-key
    ```
...
<AccordionGroup>
  <Accordion title="Image generation">
    ...
    Current bundled support is text-to-image only. Vydra's hosted edit routes expect remote image URLs, and Astroclaw does not add a Vydra-specific upload bridge in the bundled plugin yet.
  ...
  <Accordion title="Video live tests">
    ...
    ```bash
    export ASTROCLAW_LIVE_VYDRA_KLING_IMAGE_URL="https://example.com/reference.png"
    ```
