---
summary: "Use Mistral models and Voxtral transcription with Astroclaw"
read_when:
  - You want to use Mistral models in Astroclaw
  - You want Voxtral realtime transcription for Voice Call
  - You need Mistral API key onboarding and model refs
title: "Mistral"
---

Astroclaw includes a bundled Mistral plugin that registers four contracts: chat completions, media understanding (Voxtral batch transcription), realtime STT for Voice Call (Voxtral Realtime), and memory embeddings (`mistral-embed`).
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    astroclaw onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    astroclaw onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    astroclaw models list --provider mistral
    ```
  </Step>
</Steps>
...
Astroclaw currently ships this bundled Mistral catalog:
...
```bash
astroclaw infer model run --local \
...
```bash
astroclaw models list --all --provider mistral --plain
```
...
<Note>
Astroclaw defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning">
    ...
    Astroclaw maps the session **thinking** level to Mistral's API:

    | Astroclaw thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
    ...
    answers, turn thinking off/minimal so Astroclaw sends
