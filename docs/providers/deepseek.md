...
title: "DeepSeek"
read_when:
  - You want to use DeepSeek with Astroclaw
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    astroclaw onboard --auth-choice deepseek-api-key
    ```
  ...
  <Step title="Verify models are available">
    ```bash
    astroclaw models list --provider deepseek
    ```
    ...
    ```bash
    astroclaw models list --all --provider deepseek
    ```
...
<Tip>
V4 models support DeepSeek's `thinking` control. Astroclaw also replays
...
`reasoning_content` on follow-up requests. Astroclaw handles this inside the
...
DeepSeek `reasoning_content`. Astroclaw fills that missing field on replayed
assistant messages for DeepSeek V4 thinking requests so the provider can accept
the history without requiring `/new`.

When thinking is disabled in Astroclaw (including the UI **None** selection),
Astroclaw sends DeepSeek `thinking: { type: "disabled" }` and strips replayed
...
```bash
ASTROCLAW_LIVE_PROVIDERS=deepseek \
ASTROCLAW_LIVE_MODELS="deepseek/deepseek-v4-flash,deepseek/deepseek-v4-pro" \
