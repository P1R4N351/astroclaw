...
read_when:
  - You want to use Cloudflare AI Gateway with Astroclaw
  - You need the account ID, gateway ID, or API key env var
---

Cloudflare AI Gateway sits in front of provider APIs and lets you add analytics, caching, and controls. For Anthropic, Astroclaw uses the Anthropic Messages API through your Gateway endpoint.
...
When thinking is enabled for Anthropic Messages models, Astroclaw strips trailing
...
<Steps>
  <Step title="Set the provider API key and Gateway details">
    ...
    ```bash
    astroclaw onboard --auth-choice cloudflare-ai-gateway-api-key
    ```
  ...
  <Step title="Set a default model">
    Add the model to your Astroclaw config:
  ...
  <Step title="Verify the model is available">
    ```bash
    astroclaw models list --provider cloudflare-ai-gateway
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Environment note">
    ...
    <Warning>
    A key exported only in an interactive shell will not help a launchd/systemd daemon unless that environment is imported there as well. Set the key in `~/.astroclaw/.env` or via `env.shellEnv` to ensure the gateway process can read it.
    </Warning>
