...
Astroclaw sources most Go catalog rows from the bundled pi model registry and
supplements current upstream rows while the registry catches up. Run
`astroclaw models list --provider opencode-go` for the current model list.
...
<AccordionGroup>
  <Accordion title="Routing behavior">
    Astroclaw handles per-model routing automatically when the model ref uses
