---
summary: "Use Amazon Bedrock Mantle (OpenAI-compatible) models with Astroclaw"
read_when:
  - You want to use Bedrock Mantle hosted OSS models with Astroclaw
  - You need the Mantle OpenAI-compatible endpoint for GPT-OSS, Qwen, Kimi, or GLM
title: "Amazon Bedrock Mantle"
---

Astroclaw includes a bundled **Amazon Bedrock Mantle** provider that connects to
    ...
    **Best for:** using AWS SDK-compatible credentials (shared config, SSO, web identity, instance or task roles).
    ...
    <Steps>
      ...
      <Step title="Verify models are discovered">
        ...
        Astroclaw generates a Mantle bearer token from the credential chain automatically.
      </Step>
    </Steps>

    <Tip>
    When `AWS_BEARER_TOKEN_BEDROCK` is not set, Astroclaw mints the bearer token for you from the AWS default credential chain, including shared credentials/config profiles, SSO, web identity, and instance or task roles.
    </Tip>
...
When `AWS_BEARER_TOKEN_BEDROCK` is set, Astroclaw uses it directly. Otherwise,
Astroclaw attempts to generate a Mantle bearer token from the AWS default
...
```bash
astroclaw config set plugins.entries.amazon-bedrock-mantle.config.discovery.enabled false
```
...
<AccordionGroup>
  <Accordion title="Reasoning support">
    ...
    `thinking`, `reasoner`, or `gpt-oss-120b`. Astroclaw sets `reasoning: true`
  ...
  <Accordion title="Endpoint unavailability">
    ...
    silently skipped. Astroclaw does not error; other configured providers
  ...
  <Accordion title="Claude Opus 4.7 via the Anthropic Messages route">
    ...
    When you pin an Anthropic Messages model on the Mantle provider, Astroclaw uses the `anthropic-messages` API surface instead of `openai-completions` for that model. Auth still comes from `AWS_BEARER_TOKEN_BEDROCK` (or the minted IAM bearer token).
