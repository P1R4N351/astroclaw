...
read_when:
  - You want to use Vercel AI Gateway with Astroclaw
...
<Tip>
Astroclaw auto-discovers the Gateway `/v1/models` catalog, so
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    astroclaw onboard --auth-choice ai-gateway-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    astroclaw models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
Astroclaw accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
...
configuration. Astroclaw resolves the canonical form automatically.
</Tip>
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the Astroclaw Gateway runs as a daemon (launchd/systemd), make sure
    ...
    the key in `~/.astroclaw/.env` or via `env.shellEnv` to ensure the gateway
    process can read it.
    </Warning>
  ...
  <Accordion title="Thinking levels">
    `/think` options follow trusted upstream model prefixes when Astroclaw knows
