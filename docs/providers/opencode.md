---
summary: "Use OpenCode Zen and Go catalogs with Astroclaw"
read_when:
...
OpenCode exposes two hosted catalogs in Astroclaw:
...
Both catalogs use the same OpenCode API key. Astroclaw keeps the runtime provider ids
    ...
    **Best for:** the curated OpenCode multi-model proxy (Claude, GPT, Gemini).
    ...
    <Steps>
      <Step title="Run onboarding">
        ...
        ```bash
        astroclaw onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
        ```
      </Step>
      <Step title="Set a Zen model as the default">
        ```bash
        astroclaw config set agents.defaults.model.primary "opencode/claude-opus-4-6"
        ```
      </Step>
      <Step title="Verify models are available">
        ```bash
        astroclaw models list --provider opencode
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Go catalog">
    **Best for:** the OpenCode-hosted Kimi, GLM, and MiniMax lineup.
    ...
    <Steps>
      <Step title="Run onboarding">
        ```bash
        astroclaw onboard --auth-choice opencode-go
        ```

        Or pass the key directly:

        ```bash
        astroclaw onboard --opencode-go-api-key "$OPENCODE_API_KEY"
        ```
      </Step>
      <Step title="Set a Go model as the default">
        ```bash
        astroclaw config set agents.defaults.model.primary "opencode-go/kimi-k2.6"
        ```
      </Step>
      <Step title="Verify models are available">
        ```bash
        astroclaw models list --provider opencode-go
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Gemini replay behavior">
    Gemini-backed OpenCode refs stay on the proxy-Gemini path, so Astroclaw keeps
