---
summary: "Sign in to GitHub Copilot from Astroclaw using the device flow or non-interactive token import"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `astroclaw models auth login-github-copilot` flow
title: "GitHub Copilot"
---

GitHub Copilot is GitHub's AI coding assistant. It provides access to Copilot
models for your GitHub account and plan. Astroclaw can use Copilot as a model
provider in two different ways.

## Two ways to use Copilot in Astroclaw

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain a GitHub token, then exchange it for
    Copilot API tokens when Astroclaw runs. This is the **default** and simplest path
    because it does not require VS Code.

    <Steps>
      <Step title="Run the login command">
        ```bash
        astroclaw models auth login-github-copilot
        ```
      ...
      <Step title="Set a default model">
        ```bash
        astroclaw models set github-copilot/claude-opus-4.7
        ```
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. Astroclaw talks to
...
```bash
# Skip confirmation
astroclaw models auth login-github-copilot --yes

# Login and set the default model in one step
astroclaw models auth login --provider github-copilot --method device --set-default
```

## Non-interactive onboarding

If you already have a GitHub OAuth access token for Copilot, import it during
headless setup with `astroclaw onboard --non-interactive`:
...
<AccordionGroup>
  ...
  <Accordion title="Live catalog refresh from the Copilot API">
    ...
    Astroclaw refreshes the model catalog on demand from `${baseUrl}/models`
    ...
    churn. Newly published Copilot models become visible without an Astroclaw
  ...
  <Accordion title="Transport selection">
    ...
    o-series, and Gemini models keep the OpenAI Responses transport. Astroclaw
    selects the correct transport based on the model ref.
  </Accordion>

  <Accordion title="Request compatibility">
    Astroclaw sends Copilot IDE-style request headers on Copilot transports,
  ...
  <Accordion title="Environment variable resolution order">
    Astroclaw resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, Astroclaw uses the highest-priority one.
    The device-login flow (`astroclaw models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    for a Copilot API token when Astroclaw runs. You do not need to manage the
    token manually.
  </Accordion>
</AccordionGroup>
...
have logged in, Astroclaw can use it for embeddings without a separate API key.
...
providers. If a GitHub token is available, Astroclaw discovers available
...
1. Astroclaw resolves your GitHub token (from env vars or auth profile).
...
available, Astroclaw skips Copilot and tries the next provider.
