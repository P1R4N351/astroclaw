---
summary: "Use Kilo Gateway's unified API to access many models in Astroclaw"
title: "Kilo Gateway"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in Astroclaw
---
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    astroclaw onboard --auth-choice kilocode-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    astroclaw models list --provider kilocode
    ```
  </Step>
</Steps>
...
<Note>
Astroclaw treats `kilocode/kilo/auto` as the stable default ref, but does not
...
hard-coded in Astroclaw.
</Note>

## Built-in catalog

Astroclaw dynamically discovers available models from the Kilo Gateway at startup. Use
...
<Tip>
At startup, Astroclaw queries `GET https://api.kilo.ai/api/gateway/models` and merges
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path, so Astroclaw keeps
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails at startup, Astroclaw falls back to the bundled static catalog containing `kilocode/kilo/auto`.
    - Confirm your API key is valid and that your Kilo account has the desired models enabled.
    - When the Gateway runs as a daemon, ensure `KILOCODE_API_KEY` is available to that process (for example in `~/.astroclaw/.env` or via `env.shellEnv`).
