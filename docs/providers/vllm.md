---
summary: "Run Astroclaw with vLLM (OpenAI-compatible local server)"
read_when:
  - You want to run Astroclaw against a local vLLM server
  - You want OpenAI-compatible /v1 endpoints with your own models
title: "vLLM"
---

vLLM can serve open-source (and some custom) models via an **OpenAI-compatible** HTTP API. Astroclaw connects to vLLM using the `openai-completions` API.

Astroclaw can also **auto-discover** available models from vLLM when you opt in with `VLLM_API_KEY` (any value works if your server does not enforce auth). Use `vllm/*` in `agents.defaults.models` to keep discovery dynamic when you also configure a custom vLLM base URL.

Astroclaw treats `vllm` as a local OpenAI-compatible provider that supports
...
<Steps>
  ...
  <Step title="Verify the model is available">
    ```bash
    astroclaw models list --provider vllm
    ```
  </Step>
</Steps>
...
When `VLLM_API_KEY` is set (or an auth profile exists) and you **do not** define `models.providers.vllm`, Astroclaw queries:
...
<Note>
If you set `models.providers.vllm` explicitly, Astroclaw uses your declared models by default. Add `"vllm/*": {}` to `agents.defaults.models` when you want Astroclaw to query that configured provider's `/models` endpoint and include all advertised vLLM models.
</Note>
...
<AccordionGroup>
  <Accordion title="Proxy-style behavior">
    ...
    | Hidden Astroclaw attribution headers | Not injected on custom base URLs |
  ...
  <Accordion title="Qwen thinking controls">
    ...
    server expects Qwen chat-template kwargs. Astroclaw maps `/think off` to:
  ...
  <Accordion title="Nemotron 3 thinking controls">
    ...
    returned as hidden reasoning or visible answer text. When an Astroclaw session
  ...
  <Accordion title="Qwen tool calls appear as text">
    ...
    - vLLM returns an empty `tool_calls` array when Astroclaw sends
      `tool_choice: "auto"`
    ...
    ```bash
    astroclaw config set agents.defaults.models '{"vllm/Qwen-Qwen2.5-Coder-32B-Instruct":{"params":{"extra_body":{"tool_choice":"required"}}}}' --strict-json --merge
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Server not reachable">
    ...
    For explicit loopback, LAN, or Tailscale endpoints, Astroclaw trusts the
  ...
  <Accordion title="Auth errors on requests">
    ...
    <Tip>
    If your vLLM server does not enforce auth, any non-empty value for `VLLM_API_KEY` works as an opt-in signal for Astroclaw.
    </Tip>

  </Accordion>

  <Accordion title="No models discovered">
    Auto-discovery requires `VLLM_API_KEY` to be set. If you have defined `models.providers.vllm`, Astroclaw uses only your declared models unless `agents.defaults.models` includes `"vllm/*": {}`.
  </Accordion>
  ...
  <Accordion title="Tools render as raw text">
    ...
    - confirm the exact model id with `astroclaw models list --provider vllm`
