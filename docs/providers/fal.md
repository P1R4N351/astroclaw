---
summary: "fal image and video generation setup in Astroclaw"
title: "Fal"
read_when:
  - You want to use fal image generation in Astroclaw
  - You need the FAL_KEY auth flow
  - You want fal defaults for image_generate or video_generate
---

Astroclaw ships a bundled `fal` provider for hosted image and video generation.
...
<Steps>
  <Step title="Set the API key">
    ```bash
    astroclaw onboard --auth-choice fal-api-key
    ```
  </Step>
...
explicit transparent-background control in Astroclaw, so `background:
...
<Tip>
Use `astroclaw models list --provider fal` to see the full list of available fal
