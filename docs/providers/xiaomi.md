---
summary: "Use Xiaomi MiMo models with Astroclaw"
read_when:
  - You want Xiaomi MiMo models in Astroclaw
  - You need XIAOMI_API_KEY setup
title: "Xiaomi MiMo"
---

Xiaomi MiMo is the API platform for **MiMo** models. Astroclaw includes a bundled `xiaomi` plugin that registers both an OpenAI-compatible chat provider and a speech (TTS) provider against the same `XIAOMI_API_KEY`.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    astroclaw onboard --auth-choice xiaomi-api-key
    ```

    Or pass the key directly:

    ```bash
    astroclaw onboard --auth-choice xiaomi-api-key --xiaomi-api-key "$XIAOMI_API_KEY"
    ```

  </Step>
  <Step title="Verify the model is available">
    ```bash
    astroclaw models list --provider xiaomi
    ```
  </Step>
</Steps>
...
targets such as Feishu and Telegram, Astroclaw transcodes Xiaomi output to 48kHz
...
<AccordionGroup>
  ...
  <Accordion title="Troubleshooting">
    ...
    - When the Gateway runs as a daemon, ensure the key is available to that process (for example in `~/.astroclaw/.env` or via `env.shellEnv`).

    <Warning>
    Keys set only in your interactive shell are not visible to daemon-managed gateway processes. Use `~/.astroclaw/.env` or `env.shellEnv` config for persistent availability.
    </Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full Astroclaw configuration reference.
  </Card>
