...
read_when:
  - You want to use Volcano Engine or Doubao models with Astroclaw
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    astroclaw onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    astroclaw models list --provider volcengine
    astroclaw models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
Then enable it in `astroclaw.json`:
...
For voice-note targets, Astroclaw asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `astroclaw onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, Astroclaw falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.astroclaw/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running Astroclaw as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about Astroclaw setup.
  </Card>
</CardGroup>
