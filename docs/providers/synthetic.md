---
summary: "Use Synthetic's Anthropic-compatible API in Astroclaw"
read_when:
...
Astroclaw registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    astroclaw onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Astroclaw's Anthropic client appends `/v1` to the base URL automatically, so use
...
`astroclaw models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Astroclaw appends `/v1` automatically.
