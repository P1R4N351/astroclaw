---
summary: "GLM model family overview and how to use it in Astroclaw"
read_when:
  - You want GLM models in Astroclaw
  - You need the model naming convention and setup
title: "GLM (Zhipu)"
---

GLM is a model family (not a company) available through the [Z.AI](https://z.ai) platform. In Astroclaw, GLM models are accessed through the bundled `zai` provider with refs like `zai/glm-5.1`.
...
```bash Auto-detect
astroclaw onboard --auth-choice zai-api-key
```

```bash Coding Plan (global)
astroclaw onboard --auth-choice zai-coding-global
```

```bash Coding Plan (China)
astroclaw onboard --auth-choice zai-coding-cn
```

```bash General API (global)
astroclaw onboard --auth-choice zai-global
```

```bash General API (China)
astroclaw onboard --auth-choice zai-cn
```
  ...
  <Step title="Set GLM as the default model">
    ```bash
    astroclaw config set agents.defaults.model.primary "zai/glm-5.1"
    ```
  </Step>
  <Step title="Verify models are available">
    ```bash
    astroclaw models list --provider zai
    ```
  </Step>
</Steps>
...
<Tip>
  `zai-api-key` lets Astroclaw detect the matching Z.AI endpoint from the key shape and apply the correct base URL automatically. Use the explicit regional choices when you want to pin a specific Coding Plan or general API surface.
</Tip>
...
<Note>
  GLM versions and availability can change. Run `astroclaw models list --provider zai` to see the catalog rows known to your installed version, and check Z.AI's docs for newly added or deprecated models.
</Note>
...
<AccordionGroup>
  <Accordion title="Endpoint auto-detection">
    When you use the `zai-api-key` auth choice, Astroclaw inspects the key shape to determine the correct Z.AI base URL. Explicit regional choices (`zai-coding-global`, `zai-coding-cn`, `zai-global`, `zai-cn`) override auto-detection and pin the endpoint directly.
  </Accordion>
