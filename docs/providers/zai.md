---
summary: "Use Z.AI (GLM models) with Astroclaw"
read_when:
  - You want Z.AI / GLM models in Astroclaw
  - You need a simple ZAI_API_KEY setup
title: "Z.AI"
---

Z.AI is the API platform for **GLM** models. It provides REST APIs for GLM and uses API keys
for authentication. Create your API key in the Z.AI console. Astroclaw uses the `zai` provider
with a Z.AI API key.
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. Astroclaw detects the matching Z.AI endpoint from the key and applies the correct base URL automatically.
    ...
    <Steps>
      ...
      <Step title="Verify the model is listed">
        ```bash
        astroclaw models list --all --provider zai
        ```
      </Step>
    </Steps>
...
Astroclaw ships the bundled `zai` provider catalog in the plugin manifest, so read-only
...
<AccordionGroup>
  ...
  <Accordion title="Thinking and preserved thinking">
    Z.AI thinking follows Astroclaw's `/think` controls. With thinking off,
    Astroclaw sends `thinking: { type: "disabled" }` to avoid responses that
    ...
    When enabled and thinking is on, Astroclaw sends
