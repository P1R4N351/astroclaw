    ...
    **Best for:** Kimi K2 models via the Moonshot Open Platform.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        astroclaw onboard --auth-choice moonshot-api-key
        ```

        Or for the China endpoint:

        ```bash
        astroclaw onboard --auth-choice moonshot-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        astroclaw models list --provider moonshot
        ```
      </Step>
      <Step title="Run a live smoke test">
        ...
        ```bash
        ASTROCLAW_CONFIG_PATH=/tmp/astroclaw-kimi/astroclaw.json \
        ASTROCLAW_STATE_DIR=/tmp/astroclaw-kimi \
  ...
  <Tab title="Kimi Coding">
    **Best for:** code-focused tasks via the Kimi Coding endpoint.
    ...
    <Steps>
      <Step title="Run onboarding">
        ```bash
        astroclaw onboard --auth-choice kimi-code-api-key
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
      ...
      <Step title="Verify the model is available">
        ```bash
        astroclaw models list --provider kimi
        ```
      </Step>
    </Steps>
...
Astroclaw also ships **Kimi** as a `web_search` provider, backed by Moonshot web
search.
...
<AccordionGroup>
  <Accordion title="Native thinking mode">
    ...
    Astroclaw also maps runtime `/think` levels for Moonshot:
    ...
    <Warning>
    When Moonshot thinking is enabled, `tool_choice` must be `auto` or `none`. Astroclaw normalizes incompatible `tool_choice` values to `auto` for compatibility.
    </Warning>
    ...
    default strategy. Astroclaw only forwards `thinking.keep` for
  ...
  <Accordion title="Tool call id sanitization">
    Moonshot Kimi serves tool_call ids shaped like `functions.<name>:<index>`. Astroclaw preserves them unchanged so multi-turn tool calls keep working.
