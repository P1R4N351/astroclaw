---
summary: "Run Astroclaw through inferrs (OpenAI-compatible local server)"
read_when:
  - You want to run Astroclaw against a local inferrs server
  - You are serving Gemma or another model through inferrs
  - You need the exact Astroclaw compat flags for inferrs
title: "Inferrs"
---

[inferrs](https://github.com/ericcurtin/inferrs) can serve local models behind an OpenAI-compatible `/v1` API. Astroclaw works with `inferrs` through the generic `openai-completions` path.

| Property           | Value                                                              |
| ------------------ | ------------------------------------------------------------------ |
| Provider id        | `inferrs` (custom; configure under `models.providers.inferrs`)     |
| Plugin             | none — `inferrs` is not a bundled Astroclaw provider plugin         |
...
<Note>
  `inferrs` is currently best treated as a custom self-hosted OpenAI-compatible backend, not a dedicated Astroclaw provider plugin. You configure it through `models.providers.inferrs` rather than an onboarding choice flag. If you need a true bundled plugin with auto-discovery, see [SGLang](/providers/sglang) or [vLLM](/providers/vllm).
</Note>
...
<Steps>
  ...
  <Step title="Add an Astroclaw provider entry">
...
Inferrs can also be started by Astroclaw only when an `inferrs/...` model is
...
<AccordionGroup>
  <Accordion title="Why requiresStringContent matters">
    ...
    <Warning>
    If Astroclaw runs fail with an error like:
    ...
    Astroclaw will flatten pure text content parts into plain strings before sending
    the request.
  ...
  <Accordion title="Gemma and tool-schema caveat">
    ...
    `/v1/chat/completions` requests but still fail on full Astroclaw agent-runtime
    turns.
    ...
    That disables Astroclaw's tool schema surface for the model and can reduce prompt
    pressure on stricter local backends.

    If tiny direct requests still work but normal Astroclaw agent turns continue to
    crash inside `inferrs`, the remaining issue is usually upstream model/server
    behavior rather than Astroclaw's transport layer.
...
<AccordionGroup>
  ...
  <Accordion title="Direct /v1/chat/completions calls pass but astroclaw infer model run fails">
  ...
  <Accordion title="inferrs still crashes on larger agent turns">
    If Astroclaw no longer gets schema errors but `inferrs` still crashes on larger
...
<CardGroup cols={2}>
  <Card title="Local models" href="/gateway/local-models" icon="server">
    Running Astroclaw against local model servers.
  </Card>
