...
<AccordionGroup>
  <Accordion title='What is the "default model"?'>
    Astroclaw's default model is whatever you set as:

    ```
    agents.defaults.model.primary
    ```

    Models are referenced as `provider/model` (example: `openai/gpt-5.5` or `anthropic/claude-sonnet-4-6`). If you omit the provider, Astroclaw first tries an alias, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider as a deprecated compatibility path. If that provider no longer exposes the configured default model, Astroclaw falls back to the first configured provider/model instead of surfacing a stale removed-provider default. You should still **explicitly** set `provider/model`.
  ...
  <Accordion title="How do I switch models without wiping my config?">
    ...
    - `astroclaw models set ...` (updates just model config)
    - `astroclaw configure --section model` (interactive)
    - edit `agents.defaults.model` in `~/.astroclaw/astroclaw.json`
    ...
    for partial updates.
    If you did overwrite config, restore from backup or re-run `astroclaw doctor` to repair.
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    4. Run `astroclaw onboard` and choose `Ollama`
    ...
    - for manual switching, use `astroclaw models list` and `astroclaw models set ollama/<model>`
  ...
  <Accordion title="What do Astroclaw, Flawd, and Krill use for models?">
    - These deployments can differ and may change over time; there is no fixed provider recommendation.
    - Check the current runtime setting on each gateway with `astroclaw models status`.
  ...
  <Accordion title="If two providers expose the same model id, which one does /model use?">
    ...
    For example, `qianfan/deepseek-v4-flash` and `deepseek/deepseek-v4-flash` are different model refs even though both contain `deepseek-v4-flash`. Astroclaw should not silently switch from one provider to the other just because the bare model id matches.
    ...
    If a run that started from a non-session override is allowed to use fallback, Astroclaw tries the requested provider/model first, then configured fallbacks, and only then the configured primary. That prevents duplicate bare model ids from jumping directly back to the default provider.
  ...
  <Accordion title="Can I use GPT 5.5 for daily tasks and Codex 5.5 for coding?">
    ...
    - **Native Codex coding agent:** set `agents.defaults.model.primary` to `openai/gpt-5.5`. Sign in with `astroclaw models auth login --provider openai-codex` when you want ChatGPT/Codex subscription auth.
  ...
  <Accordion title='Why do I see "Model ... is not allowed" and then no reply?'>
    ...
    Add it with: astroclaw config set agents.defaults.models '{"provider/model":{}}' --strict-json --merge
    ```
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    1. Upgrade to a current Astroclaw release (or run from source `main`), then restart the gateway.
  ...
  <Accordion title="Are opus / sonnet / gpt built-in shortcuts?">
    Yes. Astroclaw ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
  ...
  <Accordion title="How do I add models from other providers like OpenRouter or Z.AI?">
    ...
    **No API key found for provider after adding a new agent**
    ...
    ```
    ~/.astroclaw/agents/<agentId>/agent/auth-profiles.json
    ```

    Fix options:

    - Run `astroclaw agents add <id>` and configure auth during the wizard.
    - Or copy only portable static `api_key` / `token` profiles from the main agent's auth store into the new agent's auth store.
    - For OAuth profiles, sign in from the new agent when it needs its own account; otherwise Astroclaw can read through to the default/main agent without cloning refresh tokens.
...
<AccordionGroup>
  <Accordion title="How does failover work?">
    ...
    Cooldowns apply to failing profiles (exponential backoff), so Astroclaw can keep responding even when a provider is rate-limited or temporarily failing.

    The rate-limit bucket includes more than plain `429` responses. Astroclaw
    ...
    explicit billing text on `401` or `403`, Astroclaw can still keep that in
    ...
    `organization spending limit exceeded`), Astroclaw treats it as
    ...
    unknown/error in it". Astroclaw does treat provider-scoped transient shapes
  ...
  <Accordion title='What does "No credentials found for profile anthropic:default" mean?'>
    ...
    **Fix checklist:**
    ...
    - **Confirm where auth profiles live** (new vs legacy paths)
      - Current: `~/.astroclaw/agents/<agentId>/agent/auth-profiles.json`
      - Legacy: `~/.astroclaw/agent/*` (migrated by `astroclaw doctor`)
    - **Confirm your env var is loaded by the Gateway**
      - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.astroclaw/.env` or enable `env.shellEnv`.
    - **Make sure you're editing the correct agent**
      - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
    - **Sanity-check model/auth status**
      - Use `astroclaw models status` to see configured models and whether providers are authenticated.
    ...
    **Fix checklist for "No credentials found for profile anthropic"**
    ...
    - **Use Claude CLI**
      - Run `astroclaw models auth login --provider anthropic --method cli --set-default` on the gateway host.
    - **If you want to use an API key instead**
      - Put `ANTHROPIC_API_KEY` in `~/.astroclaw/.env` on the **gateway host**.
      - Clear any pinned order that forces a missing profile:

        ```bash
        astroclaw models auth order clear --provider anthropic
        ```
  ...
  <Accordion title="Why did it also try Google Gemini and fail?">
    If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), Astroclaw will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
    ...
    **LLM request rejected: thinking signature required (Google Antigravity)**
    ...
    Fix: Astroclaw now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
<AccordionGroup>
  <Accordion title="What is an auth profile?">
    ...
    To inspect saved profiles without dumping secrets, run `astroclaw models auth list` (optionally `--provider <id>` or `--json`). See [Models CLI](/cli/models#auth-profiles) for details.

  </Accordion>

  <Accordion title="What are typical profile IDs?">
    Astroclaw uses provider-prefixed IDs like:
  ...
  <Accordion title="Can I control which auth profile is tried first?">
    ...
    Astroclaw may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `astroclaw models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
    ...
    astroclaw models auth order get --provider anthropic

    # Lock rotation to a single profile (only try this one)
    astroclaw models auth order set --provider anthropic anthropic:default

    # Or set an explicit order (fallback within provider)
    astroclaw models auth order set --provider anthropic anthropic:work anthropic:default
    ...
    ```bash
    astroclaw models auth order set --provider anthropic --agent main anthropic:default
    ```

    To verify what will actually be tried, use:

    ```bash
    astroclaw models status --probe
    ```
