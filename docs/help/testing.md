...
Astroclaw has three Vitest suites (unit/integration, e2e, live) and a small set
...
- [Matrix QA](/concepts/qa-matrix) - reference for `pnpm astroclaw qa matrix`.
...
- Linux VM-backed QA lane: `pnpm astroclaw qa suite --runner multipass --scenario channel-chat-baseline`
...
- Runtime performance reports: dispatch `Astroclaw Performance` with
  ...
  `astroclaw/clawgrit-reports` when `CLAWGRIT_REPORTS_TOKEN` is configured. The
...
- Docker live model sweep: `pnpm test:docker:live-models`
  - Each selected model now runs a text turn plus a small file-read-style probe.
    ...
    Disable the extra probes with `ASTROCLAW_LIVE_MODEL_FILE_PROBE=0` or
    `ASTROCLAW_LIVE_MODEL_IMAGE_PROBE=0` when isolating provider failures.
  - CI coverage: daily `Astroclaw Scheduled Live And E2E Checks` and manual
    `Astroclaw Release Checks` both call the reusable live/E2E workflow with
    `include_live_suites: true`, which includes separate Docker live model
    matrix jobs sharded by provider.
  - For focused CI reruns, dispatch `Astroclaw Live And E2E Checks (Reusable)`
  ...
  - Add new high-signal provider secrets to `scripts/ci-hydrate-live-auth.sh`
    plus `.github/workflows/astroclaw-live-and-e2e-checks-reusable.yml` and its
...
- Codex app-server harness smoke: `pnpm test:docker:live-codex-harness`
  - Runs gateway agent turns through the plugin-owned Codex app-server harness,
    ...
    `ASTROCLAW_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=0` when isolating other Codex
    app-server failures. For a focused sub-agent check, disable the other probes:
    `ASTROCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE=0 ASTROCLAW_LIVE_CODEX_HARNESS_MCP_PROBE=0 ASTROCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0 ASTROCLAW_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=1 pnpm test:docker:live-codex-harness`.
    This exits after the sub-agent probe unless
    `ASTROCLAW_LIVE_CODEX_HARNESS_SUBAGENT_ONLY=0` is set.
- Codex on-demand install smoke: `pnpm test:docker:codex-on-demand`
  - Installs the packaged Astroclaw tarball in Docker, runs OpenAI API-key
...
- Crestodian first-run Docker smoke: `pnpm test:docker:crestodian-first-run`
  - Starts from an empty Astroclaw state dir, routes bare `astroclaw` to
    ...
    `pnpm astroclaw qa suite --scenario crestodian-ring-zero-setup`.
- Moonshot/Kimi cost smoke: with `MOONSHOT_API_KEY` set, run
  `astroclaw models list --provider moonshot --json`, then run an isolated
  `astroclaw agent --local --session-id live-kimi-cost --message 'Reply exactly: KIMI_LIVE_OK' --thinking off --json`
...
`media`, `e2ee-smoke`, `e2ee-deep`, and `e2ee-cli` jobs. `Astroclaw Release
...
`ghcr.io/astroclaw/astroclaw-live-media-runner:ubuntu-24.04`, which already has
`ffmpeg` and `ffprobe`. Docker live model/backend shards use the shared
`ghcr.io/astroclaw/astroclaw-live-test:<sha>` image built once per selected
commit, then pull it with `ASTROCLAW_SKIP_DOCKER_BUILD=1` instead of rebuilding
inside every shard.

- `pnpm astroclaw qa suite`
...
- `pnpm test:plugins:kitchen-sink-live`
  - Runs the live OpenAI Kitchen Sink plugin gauntlet through QA Lab. It
    ...
    `astroclaw-testbox-env` helper is present.
...
- `pnpm astroclaw qa suite --runner multipass`
...
- `pnpm test:docker:npm-onboard-channel-agent`
  ...
  - Use `ASTROCLAW_NPM_ONBOARD_CHANNEL=discord` to run the same packaged-install
    lane with Discord.
- `pnpm test:docker:session-runtime-context`
  - Runs a deterministic built-app Docker smoke for embedded runtime context
    transcripts. It verifies hidden Astroclaw runtime context is persisted as a
    ...
    `astroclaw doctor --fix` rewrites it to the active branch with a backup.
- `pnpm test:docker:npm-telegram-live`
  - Installs an Astroclaw package candidate in Docker, runs installed-package
  ...
  - The wrapper mounts only the `qa-lab` harness source from the checkout; the
    installed package owns `dist`, `astroclaw/plugin-sdk`, and bundled plugin
    runtime so the lane does not mix current checkout plugins into the package
    under test.
  - Defaults to `ASTROCLAW_NPM_TELEGRAM_PACKAGE_SPEC=astroclaw@beta`; set
    `ASTROCLAW_NPM_TELEGRAM_PACKAGE_TGZ=/path/to/astroclaw-current.tgz` or
    `ASTROCLAW_CURRENT_PACKAGE_TGZ` to test a resolved local tarball instead of
    installing from the registry.
  - Uses the same Telegram env credentials or Convex credential source as
    `pnpm astroclaw qa telegram`. For CI/release automation, set
    `ASTROCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex` plus
    `ASTROCLAW_QA_CONVEX_SITE_URL` and the role secret. If
    `ASTROCLAW_QA_CONVEX_SITE_URL` and a Convex role secret are present in CI,
  ...
  - The wrapper validates Telegram or Convex credential env on the host before
    Docker build/install work. Set `ASTROCLAW_NPM_TELEGRAM_SKIP_CREDENTIAL_PREFLIGHT=1`
    only when deliberately debugging pre-credential setup.
  - `ASTROCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE=ci|maintainer` overrides the shared
    `ASTROCLAW_QA_CREDENTIAL_ROLE` for this lane only.
...
- GitHub Actions also exposes `Package Acceptance` for side-run product proof
  ...
  the normalized `astroclaw-current.tgz` as `package-under-test`, then runs the
...
gh workflow run package-acceptance.yml --ref main \
  -f source=url \
  -f package_url=https://registry.npmjs.org/astroclaw/-/astroclaw-VERSION.tgz \
...
- `pnpm test:docker:plugins`
  - Packs and installs the current Astroclaw build in Docker, starts the Gateway
  ...
  - Also installs a known older npm baseline, enables Telegram before running
    `astroclaw update --tag <candidate>`, and verifies the candidate's
...
- `pnpm test:parallels:npm-update`
  - Runs the native packaged-install update smoke across Parallels guests. Each
    ...
    the installed `astroclaw update` command in the same guest and verifies the
  ...
  - The OpenAI lane uses `openai/gpt-5.5` for the live agent-turn proof by
    ...
    `ASTROCLAW_PARALLELS_OPENAI_MODEL` when deliberately validating another
    OpenAI model.
  ...
  - The script writes nested lane logs under `/tmp/astroclaw-parallels-npm-update.*`.
...
- `pnpm astroclaw qa telegram`
  - Runs the Telegram live QA lane against a real private group using the driver and SUT bot tokens from env.
  - Requires `ASTROCLAW_QA_TELEGRAM_GROUP_ID`, `ASTROCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and `ASTROCLAW_QA_TELEGRAM_SUT_BOT_TOKEN`. The group id must be the numeric Telegram chat id.
  - Supports `--credential-source convex` for shared pooled credentials. Use env mode by default, or set `ASTROCLAW_QA_CREDENTIAL_SOURCE=convex` to opt into pooled leases.
...
- `pnpm astroclaw qa mantis telegram-desktop-builder`
  - Leases or reuses a Crabbox Linux desktop, installs native Telegram Desktop, configures Astroclaw with a leased Telegram SUT bot token, starts the gateway, and records screenshot/MP4 evidence from the visible VNC desktop.
  - Defaults to `--credential-source convex` so workflows only need the Convex broker secret. Use `--credential-source env` with the same `ASTROCLAW_QA_TELEGRAM_*` variables as `pnpm astroclaw qa telegram`.
  - Telegram Desktop still needs a user login/profile. The bot token configures Astroclaw only. Use `--telegram-profile-archive-env <name>` for a base64 `.tgz` profile archive, or use `--keep-lease` and log in manually through VNC once.
...
When `--credential-source convex` (or `ASTROCLAW_QA_CREDENTIAL_SOURCE=convex`) is enabled for
...
- `ASTROCLAW_QA_CONVEX_SITE_URL` (for example `https://your-deployment.convex.site`)
- One secret for the selected role:
  - `ASTROCLAW_QA_CONVEX_SECRET_MAINTAINER` for `maintainer`
  - `ASTROCLAW_QA_CONVEX_SECRET_CI` for `ci`
- Credential role selection:
  - CLI: `--credential-role maintainer|ci`
  - Env default: `ASTROCLAW_QA_CREDENTIAL_ROLE` (defaults to `ci` in CI, `maintainer` otherwise)

Optional env vars:

- `ASTROCLAW_QA_CREDENTIAL_LEASE_TTL_MS` (default `1200000`)
- `ASTROCLAW_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS` (default `30000`)
- `ASTROCLAW_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS` (default `90000`)
- `ASTROCLAW_QA_CREDENTIAL_HTTP_TIMEOUT_MS` (default `15000`)
- `ASTROCLAW_QA_CONVEX_ENDPOINT_PREFIX` (default `/qa-credentials/v1`)
- `ASTROCLAW_QA_CREDENTIAL_OWNER_ID` (optional trace id)
- `ASTROCLAW_QA_ALLOW_INSECURE_HTTP=1` allows loopback `http://` Convex URLs for local-only development.

`ASTROCLAW_QA_CONVEX_SITE_URL` should use `https://` in normal operation.

Maintainer admin commands (pool add/remove/list) require
`ASTROCLAW_QA_CONVEX_SECRET_MAINTAINER` specifically.

CLI helpers for maintainers:

```bash
pnpm astroclaw qa credentials doctor
pnpm astroclaw qa credentials add --kind telegram --payload-file qa/telegram-credential.json
pnpm astroclaw qa credentials list --kind telegram
pnpm astroclaw qa credentials remove --credential-id <credential-id>
```
...
Default endpoint contract (`ASTROCLAW_QA_CONVEX_SITE_URL` + `/qa-credentials/v1`):
...
```bash
tmp=$(mktemp -d /tmp/astroclaw-telegram-user.XXXXXX)
...
pnpm qa:telegram-user:crabbox -- start \
  --tdlib-url http://artifacts.astroclaw.ai/tdlib-v1.8.0-linux-x64.tgz \
...
- `run --session <file> -- <remote command>` runs an arbitrary command on the Crabbox and saves its output, for example `bash -lc 'source /tmp/astroclaw-telegram-user-crabbox/env.sh && python3 /tmp/astroclaw-telegram-user-crabbox/user-driver.py transcript --limit 20 --json'`.
...
<AccordionGroup>
  <Accordion title="Projects, shards, and scoped lanes">
    ...
    - `pnpm test:changed` expands changed git paths into cheap scoped lanes by default: direct test edits, sibling `*.test.ts` files, explicit source mappings, and local import-graph dependents. Config/setup/package edits do not broad-run tests unless you explicitly use `ASTROCLAW_TEST_CHANGED_BROAD=1 pnpm test:changed`.
    ...
    - Import-light unit tests from agents, commands, plugins, auto-reply helpers, `plugin-sdk`, and similar pure utility areas route through the `unit-fast` lane, which skips `test/setup-astroclaw-runtime.ts`; stateful/runtime-heavy files stay on the existing lanes.
  ...
  <Accordion title="Vitest pool and isolation defaults">
    ...
    - `scripts/run-vitest.mjs` adds `--no-maglev` for Vitest child Node
      ...
      Set `ASTROCLAW_VITEST_ENABLE_MAGLEV=1` to compare against stock V8
      behavior.
  ...
  <Accordion title="Fast local iteration">
    ...
    - `pnpm test:changed` routes through cheap scoped lanes by default. Use
      `ASTROCLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` only when the agent
    ...
    - The base Vitest config marks the projects/config files as
      ...
      wiring changes.
    - The config keeps `ASTROCLAW_VITEST_FS_MODULE_CACHE` enabled on supported
      hosts; set `ASTROCLAW_VITEST_FS_MODULE_CACHE_PATH=/abs/path` if you want
...
- Useful overrides:
  - `ASTROCLAW_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `ASTROCLAW_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Scope:
  ...
  - Exercises Astroclaw's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `ASTROCLAW_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `ASTROCLAW_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- Default: **enabled** by `pnpm test:live` (sets `ASTROCLAW_LIVE_TEST=1`)
- Scope:
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.astroclaw`.
- Set `ASTROCLAW_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` defaults to a quieter mode: it keeps `[live] ...` progress output and mutes gateway bootstrap logs/Bonjour chatter. Set `ASTROCLAW_LIVE_TEST_QUIET=0` if you want the full startup logs back.
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `ASTROCLAW_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `ASTROCLAW_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `ASTROCLAW_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Docker live runners default to a smaller smoke cap so a full Docker sweep stays practical:
  `test:docker:live-models` defaults to `ASTROCLAW_LIVE_MAX_MODELS=12`, and
  `test:docker:live-gateway` defaults to `ASTROCLAW_LIVE_GATEWAY_SMOKE=1`,
  `ASTROCLAW_LIVE_GATEWAY_MAX_MODELS=8`,
  `ASTROCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS=45000`, and
  `ASTROCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`. Override those env vars when you
  explicitly want the larger exhaustive scan.
- `test:docker:all` builds the live Docker image once via `test:docker:live-build`, packs Astroclaw once as an npm tarball through `scripts/package-astroclaw-for-docker.mjs`, then builds/reuses two `scripts/e2e/Dockerfile` images. The bare image is only the Node/Git runner for install/update/plugin-dependency lanes; those lanes mount the prebuilt tarball. The functional image installs the same tarball into `/app` for built-app functionality lanes. Docker lane definitions live in `scripts/lib/docker-e2e-scenarios.mjs`; planner logic lives in `scripts/lib/docker-e2e-plan.mjs`; `scripts/test-docker-all.mjs` executes the selected plan. The aggregate uses a weighted local scheduler: `ASTROCLAW_DOCKER_ALL_PARALLELISM` controls process slots, while resource caps keep heavy live, npm-install, and multi-service lanes from all starting at once. If a single lane is heavier than the active caps, the scheduler can still start it when the pool is empty and then keeps it running alone until capacity is available again. Defaults are 10 slots, `ASTROCLAW_DOCKER_ALL_LIVE_LIMIT=9`, `ASTROCLAW_DOCKER_ALL_NPM_LIMIT=10`, and `ASTROCLAW_DOCKER_ALL_SERVICE_LIMIT=7`; tune `ASTROCLAW_DOCKER_ALL_WEIGHT_LIMIT` or `ASTROCLAW_DOCKER_ALL_DOCKER_LIMIT` only when the Docker host has more headroom. The runner performs a Docker preflight by default, removes stale Astroclaw E2E containers, prints status every 30 seconds, stores successful lane timings in `.artifacts/docker-tests/lane-timings.json`, and uses those timings to start longer lanes first on later runs. Use `ASTROCLAW_DOCKER_ALL_DRY_RUN=1` to print the weighted lane manifest without building or running Docker, or `node scripts/test-docker-all.mjs --plan-json` to print the CI plan for selected lanes, package/image needs, and credentials.
...
- Npm tarball onboarding/channel/agent smoke: `pnpm test:docker:npm-onboard-channel-agent` installs the packed Astroclaw tarball globally in Docker, configures OpenAI via env-ref onboarding plus Telegram by default, runs doctor, and runs one mocked OpenAI agent turn. Reuse a prebuilt tarball with `ASTROCLAW_CURRENT_PACKAGE_TGZ=/path/to/astroclaw-*.tgz`, skip the host rebuild with `ASTROCLAW_NPM_ONBOARD_HOST_BUILD=0`, or switch channel with `ASTROCLAW_NPM_ONBOARD_CHANNEL=discord` or `ASTROCLAW_NPM_ONBOARD_CHANNEL=slack`.

- Release user journey smoke: `pnpm test:docker:release-user-journey` installs the packed Astroclaw tarball globally in a clean Docker home, runs onboarding, configures a mocked OpenAI provider, runs an agent turn, installs/uninstalls external plugins, configures ClickClack against a local fixture, verifies outbound/inbound messaging, restarts Gateway, and runs doctor.
- Release typed onboarding smoke: `pnpm test:docker:release-typed-onboarding` installs the packed tarball, drives `astroclaw onboard` through a real TTY, configures OpenAI as an env-ref provider, verifies no raw key persistence, and runs a mocked agent turn.
...
- Release upgrade user journey smoke: `pnpm test:docker:release-upgrade-user-journey` installs `astroclaw@latest` by default, configures provider/plugin/ClickClack state on the published package, upgrades to the candidate tarball, then reruns the core agent/plugin/channel journey. Override the baseline with `ASTROCLAW_RELEASE_UPGRADE_BASELINE_SPEC=astroclaw@<version>`.
...
- Skill install smoke: `pnpm test:docker:skill-install` installs the packed Astroclaw tarball globally in Docker, disables uploaded archive installs in config, resolves the current live ClawHub skill slug from search, installs it with `astroclaw skills install`, and verifies the installed skill plus `.clawhub` origin/lock metadata.
- Update channel switch smoke: `pnpm test:docker:update-channel-switch` installs the packed Astroclaw tarball globally in Docker, switches from package `stable` to git `dev`, verifies the persisted channel and plugin post-update work, then switches back to package `stable` and checks update status.
- Upgrade survivor smoke: `pnpm test:docker:upgrade-survivor` installs the packed Astroclaw tarball over a dirty old-user fixture with agents, channel config, plugin allowlists, stale plugin dependency state, and existing workspace/session files. It runs package update plus non-interactive doctor without live provider or channel keys, then starts a loopback Gateway and checks config/state preservation plus startup/status budgets.
- Published upgrade survivor smoke: `pnpm test:docker:published-upgrade-survivor` installs `astroclaw@latest` by default, seeds realistic existing-user files, configures that baseline with a baked command recipe, validates the resulting config, updates that published install to the candidate tarball, runs non-interactive doctor, writes `.artifacts/upgrade-survivor/summary.json`, then starts a loopback Gateway and checks configured intents, state preservation, startup, `/healthz`, `/readyz`, and RPC status budgets. Override one baseline with `ASTROCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC`, ask the aggregate scheduler to expand exact local baselines with `ASTROCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS` such as `astroclaw@2026.5.2 astroclaw@2026.4.23 astroclaw@2026.4.15`, and expand issue-shaped fixtures with `ASTROCLAW_UPGRADE_SURVIVOR_SCENARIOS` such as `reported-issues`; the reported-issues set includes `configured-plugin-installs` for automatic external Astroclaw plugin install repair. Package Acceptance exposes those as `published_upgrade_survivor_baseline`, `published_upgrade_survivor_baselines`, and `published_upgrade_survivor_scenarios`, resolves meta baseline tokens such as `last-stable-4` or `all-since-2026.4.23`, and Full Release Validation expands the release-soak package gate to `last-stable-4 2026.4.23 2026.5.2 2026.4.15` plus `reported-issues`.
...
- Bun global install smoke: `bash scripts/e2e/bun-global-install-smoke.sh` packs the current tree, installs it with `bun install -g` in an isolated home, and verifies `astroclaw infer image providers --json` returns bundled image providers instead of hanging. Reuse a prebuilt tarball with `ASTROCLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ=/path/to/astroclaw-*.tgz`, skip the host build with `ASTROCLAW_BUN_GLOBAL_SMOKE_HOST_BUILD=0`, or copy `dist/` from a built Docker image with `ASTROCLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE=astroclaw-dockerfile-smoke:local`.
- Installer Docker smoke: `bash scripts/test-install-sh-docker.sh` shares one npm cache across its root, update, and direct-npm containers. Update smoke defaults to npm `latest` as the stable baseline before upgrading to the candidate tarball. Override with `ASTROCLAW_INSTALL_SMOKE_UPDATE_BASELINE=2026.4.22` locally, or with the Install Smoke workflow's `update_baseline_version` input on GitHub. Non-root installer checks keep an isolated npm cache so root-owned cache entries do not mask user-local install behavior. Set `ASTROCLAW_INSTALL_SMOKE_NPM_CACHE_DIR=/path/to/cache` to reuse the root/update/direct-npm cache across local reruns.
- Install Smoke CI skips the duplicate direct-npm global update with `ASTROCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1`; run the script locally without that env when direct `npm install -g` coverage is needed.
- Agents delete shared workspace CLI smoke: `pnpm test:docker:agents-delete-shared-workspace` (script: `scripts/e2e/agents-delete-shared-workspace-docker.sh`) builds the root Dockerfile image by default, seeds two agents with one workspace in an isolated container home, runs `agents delete --json`, and verifies valid JSON plus retained workspace behavior. Reuse the install-smoke image with `ASTROCLAW_AGENTS_DELETE_SHARED_WORKSPACE_E2E_IMAGE=astroclaw-dockerfile-smoke:local ASTROCLAW_AGENTS_DELETE_SHARED_WORKSPACE_E2E_SKIP_BUILD=1`.
...
- Plugins (install/update smoke for local path, `file:`, npm registry with hoisted dependencies, malformed npm package metadata, git moving refs, ClawHub kitchen-sink, marketplace updates, and Claude-bundle enable/inspect): `pnpm test:docker:plugins` (script: `scripts/e2e/plugins-docker.sh`)
  Set `ASTROCLAW_PLUGINS_E2E_CLAWHUB=0` to skip the ClawHub block, or override the default kitchen-sink package/runtime pair with `ASTROCLAW_PLUGINS_E2E_CLAWHUB_SPEC` and `ASTROCLAW_PLUGINS_E2E_CLAWHUB_ID`. Without `ASTROCLAW_CLAWHUB_URL`/`CLAWHUB_URL`, the test uses a hermetic local ClawHub fixture server.
- Plugin update unchanged smoke: `pnpm test:docker:plugin-update` (script: `scripts/e2e/plugin-update-unchanged-docker.sh`)
- Plugin lifecycle matrix smoke: `pnpm test:docker:plugin-lifecycle-matrix` installs the packed Astroclaw tarball in a bare container, installs an npm plugin, toggles enable/disable, upgrades and downgrades it through a local npm registry, deletes the installed code, then verifies uninstall still removes stale state while logging RSS/CPU metrics for each lifecycle phase.
...
```bash
ASTROCLAW_DOCKER_E2E_IMAGE=astroclaw-docker-e2e-functional:local pnpm test:docker:e2e-build
ASTROCLAW_DOCKER_E2E_IMAGE=astroclaw-docker-e2e-functional:local ASTROCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels
```

Suite-specific image overrides such as `ASTROCLAW_GATEWAY_NETWORK_E2E_IMAGE` still win when set. When `ASTROCLAW_SKIP_DOCKER_BUILD=1` points at a remote shared image, the scripts pull it if it is not already local. The QR and installer Docker tests keep their own Dockerfiles because they validate package/install behavior rather than the shared built-app runtime.
...
`.pnpm-store`, `.worktrees`, `__astroclaw_vitest__`, and app-local `.build` or
Gradle output directories so Docker live runs do not spend minutes copying
machine-specific artifacts.
They also set `ASTROCLAW_SKIP_CHANNELS=1` so gateway live probes do not start
...
`ASTROCLAW_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
live coverage from that Docker lane.
`test:docker:openwebui` is a higher-level compatibility smoke: it starts an
Astroclaw gateway container with the OpenAI-compatible HTTP endpoints enabled,
starts a pinned Open WebUI container against that gateway, signs in through
Open WebUI, verifies `/api/models` exposes `astroclaw/default`, then sends a
...
environment, staged auth profiles, or an explicit `ASTROCLAW_PROFILE_FILE`.
...
container, starts a second container that spawns `astroclaw mcp serve`, then
...
- `ASTROCLAW_CONFIG_DIR=...` (default: `~/.astroclaw`) mounted to `/home/node/.astroclaw`
- `ASTROCLAW_WORKSPACE_DIR=...` (default: `~/.astroclaw/workspace`) mounted to `/home/node/.astroclaw/workspace`
- `ASTROCLAW_PROFILE_FILE=...` mounted and sourced before running tests
- `ASTROCLAW_DOCKER_PROFILE_ENV_ONLY=1` to verify only env vars sourced from `ASTROCLAW_PROFILE_FILE`, using temporary config/workspace dirs and no external CLI auth mounts
- `ASTROCLAW_DOCKER_CLI_TOOLS_DIR=...` (default: `~/.cache/astroclaw/docker-cli-tools`) mounted to `/home/node/.npm-global` for cached CLI installs inside Docker
- External CLI auth dirs/files under `$HOME` are mounted read-only under `/host-auth...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs/files inferred from `ASTROCLAW_LIVE_PROVIDERS` / `ASTROCLAW_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `ASTROCLAW_DOCKER_AUTH_DIRS=all`, `ASTROCLAW_DOCKER_AUTH_DIRS=none`, or a comma list like `ASTROCLAW_DOCKER_AUTH_DIRS=.claude,.codex`
- `ASTROCLAW_LIVE_GATEWAY_MODELS=...` / `ASTROCLAW_LIVE_MODELS=...` to narrow the run
- `ASTROCLAW_LIVE_GATEWAY_PROVIDERS=...` / `ASTROCLAW_LIVE_PROVIDERS=...` to filter providers in-container
- `ASTROCLAW_SKIP_DOCKER_BUILD=1` to reuse an existing `astroclaw:local-live` image for reruns that do not need a rebuild
- `ASTROCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `ASTROCLAW_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
- `ASTROCLAW_OPENWEBUI_PROMPT=...` to override the nonce-check prompt used by the Open WebUI smoke
