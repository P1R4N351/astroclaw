---
summary: "Frequently asked questions about Astroclaw setup, configuration, and usage"
read_when:
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/astroclaw/astroclaw-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   astroclaw health --json
   astroclaw health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  ...
  <Accordion title="What are the top five everyday use cases for Astroclaw?">
  ...
  <Accordion title="Can Astroclaw help with lead gen, outreach, ads, and blogs for a SaaS?">
    Yes for **research, qualification, and drafting**. It can scan sites, build shortlists,
    summarize prospects, and write outreach or ad copy drafts.

    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and
    platform policies, and review anything before it is sent. The safest pattern is to let
    Astroclaw draft and you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    Astroclaw is a **personal assistant** and coordination layer, not an IDE replacement. Use
    Claude Code or Codex for the fastest direct coding loop inside a repo. Use Astroclaw when you
    ...
    Showcase: [https://astroclaw.ai/showcase](https://astroclaw.ai/showcase)
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.astroclaw/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.astroclaw/astroclaw.json`). Precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.astroclaw/skills` → bundled → `skills.load.extraDirs`, so managed overrides still win over bundled skills without touching git. If you need the skill installed globally but only visible to some agents, keep the shared copy in `~/.astroclaw/skills` and control visibility with `agents.defaults.skills` and `agents.list[].skills`. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.astroclaw/astroclaw.json` (lowest precedence). Default precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.astroclaw/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Astroclaw treats as `<workspace>/skills` on the next session. If the skill should only be visible to certain agents, pair that with `agents.defaults.skills` or `agents.list[].skills`.
  </Accordion>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, Astroclaw falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is the exact silent token `NO_REPLY` / `no_reply`, or exactly `ANNOUNCE_SKIP`, Astroclaw intentionally suppresses the announce instead of posting stale earlier progress.
    ...
    ```bash
    astroclaw tasks show <runId-or-sessionKey>
    ```
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    - Confirm cron is enabled (`cron.enabled`) and `ASTROCLAW_SKIP_CRON` is not set.
    ...
    ```bash
    astroclaw cron run <jobId>
    astroclaw cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `astroclaw skills` commands or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
    Browse skills at [https://clawhub.ai](https://clawhub.ai).

    ```bash
    astroclaw skills search "calendar"
    astroclaw skills search --limit 20
    astroclaw skills install <skill-slug>
    astroclaw skills install <skill-slug> --version <version>
    astroclaw skills install <skill-slug> --force
    astroclaw skills update --all
    astroclaw skills list --eligible
    astroclaw skills check
    ```

    Native `astroclaw skills install` writes into the active workspace `skills/`
    ...
    `~/.astroclaw/skills` and use `agents.defaults.skills` or
    `agents.list[].skills` if you want to narrow which agents can see it.

  </Accordion>

  <Accordion title="Can Astroclaw run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.astroclaw.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH).**
    Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. Astroclaw can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.astroclaw/skills`) to allow Linux:

       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "astroclaw": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.astroclaw/skills/<name>/SKILL.md`. If only some agents should see a shared install, configure `agents.defaults.skills` or `agents.list[].skills`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills), [Skills config](/tools/skills-config), and [ClawHub](/clawhub).

  </Accordion>

  <Accordion title="How do I use my existing signed-in Chrome with Astroclaw?">
    ...
    astroclaw browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Docker feels limited - how do I enable full features?">
    ...
    - Persist `/home/node` with `ASTROCLAW_HOME_VOLUME` so caches survive.
    - Bake system deps into the image with `ASTROCLAW_DOCKER_APT_PACKAGES`.
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    Astroclaw validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor. That means symlink-parent escapes still fail closed even when the last path segment does not exist yet, and allowed-root checks still apply after symlink resolution.
  ...
  <Accordion title="How does memory work?">
    Astroclaw memory is just Markdown files in the agent workspace:

    - Daily notes in `memory/YYYY-MM-DD.md`
    - Curated long-term notes in `MEMORY.md` (main/private sessions only)

    Astroclaw also runs a **silent pre-compaction memory flush** to remind the model
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    If you don't set a provider explicitly, Astroclaw auto-selects a provider when it
    ...
    configured and present, Astroclaw
...
<AccordionGroup>
  <Accordion title="Is all data used with Astroclaw saved locally?">
    No - **Astroclaw's state is local**, but **external services still see what you send them**.

    - **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
      (`~/.astroclaw` + your workspace directory).
  ...
  <Accordion title="Where does Astroclaw store its data?">
    Everything lives under `$ASTROCLAW_STATE_DIR` (default: `~/.astroclaw`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$ASTROCLAW_STATE_DIR/astroclaw.json`                             | Main config (JSON5)                                                |
    | `$ASTROCLAW_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
    | `$ASTROCLAW_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
    | `$ASTROCLAW_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
    | `$ASTROCLAW_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
    | `$ASTROCLAW_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
    | `$ASTROCLAW_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
    | `$ASTROCLAW_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
    | `$ASTROCLAW_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

    Legacy single-agent path: `~/.astroclaw/agent/*` (migrated by `astroclaw doctor`).

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.astroclaw/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These files live in the **agent workspace**, not `~/.astroclaw`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md`, `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
      Lowercase root `memory.md` is legacy repair input only; `astroclaw doctor --fix`
      can merge it into `MEMORY.md` when both files exist.
    - **State dir (`~/.astroclaw`)**: config, channel/provider state, auth profiles, sessions, logs,
      and shared skills (`~/.astroclaw/skills`).

    Default workspace is `~/.astroclaw/workspace`, configurable via:

    ```json5
    {
      agents: { defaults: { workspace: "~/.astroclaw/workspace" } },
    }
    ```
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.astroclaw` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall Astroclaw?">
  ...
  <Accordion title="Can agents work outside the workspace?">
    ...
    `workspace` to the repo root. The Astroclaw repo is just source code; keep the
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    Astroclaw reads an optional **JSON5** config from `$ASTROCLAW_CONFIG_PATH` (default: `~/.astroclaw/astroclaw.json`):

    ```
    $ASTROCLAW_CONFIG_PATH
    ```

    If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.astroclaw/workspace`).
  ...
  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    ...
    - For password auth, set `gateway.auth.mode: "password"` plus `gateway.auth.password` (or `ASTROCLAW_GATEWAY_PASSWORD`) instead.
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Astroclaw enforces gateway auth by default, including loopback. In the normal default path that means token auth: if no explicit auth path is configured, gateway startup resolves to token mode and generates a runtime-only token for that startup, so **local WS clients must authenticate**. Configure `gateway.auth.token`, `gateway.auth.password`, `ASTROCLAW_GATEWAY_TOKEN`, or `ASTROCLAW_GATEWAY_PASSWORD` explicitly when clients need a stable secret across restarts. This blocks other local processes from calling the Gateway.

    If you prefer a different auth path, you can explicitly choose password mode (or, for identity-aware reverse proxies, `trusted-proxy`). If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `astroclaw doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one Astroclaw.` every time.
    - `random`: rotating funny/seasonal taglines (default behavior).
    - If you want no banner at all, set env `ASTROCLAW_HIDE_BANNER=1`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `astroclaw configure --section web` and choose a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, Astroclaw auto-detects the first ready fetch fallback provider from available credentials. Today the bundled provider is Firecrawl.
    - Daemons read env vars from `~/.astroclaw/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Current Astroclaw protects many accidental clobbers:

    - Astroclaw-owned config writes validate the full post-change config before writing.
    - Invalid or destructive Astroclaw-owned writes are rejected and saved as `astroclaw.json.rejected.*`.
    - If a direct edit breaks startup or hot reload, Gateway fails closed or skips the reload; it does not rewrite `astroclaw.json`.
    - `astroclaw doctor --fix` owns repair and can restore last-known-good while saving the rejected file as `astroclaw.json.clobbered.*`.

    Recover:

    - Check `astroclaw logs --follow` for `Invalid config at`, `Config write rejected:`, or `config reload skipped (invalid config)`.
    - Inspect the newest `astroclaw.json.clobbered.*` or `astroclaw.json.rejected.*` beside the active config.
    - Run `astroclaw config validate` and `astroclaw doctor --fix`.
    - Copy only the intended keys back with `astroclaw config set` or `config.patch`.
    - If you have no last-known-good or rejected payload, restore from backup, or re-run `astroclaw doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `astroclaw config set` for small changes.
    - Use `astroclaw configure` for interactive edits.
  ...
  <Accordion title="Can the Astroclaw browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="How can my agent access my computer if the Gateway is hosted remotely?">
    ...
    5. Approve the node on the Gateway:
       ...
       astroclaw devices approve <requestId>
       ```
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `astroclaw gateway status`
    - Gateway health: `astroclaw status`
    - Channel health: `astroclaw channels status`
  ...
  <Accordion title="Can two Astroclaw instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `astroclaw agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    astroclaw agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
  ...
  <Accordion title="How do I set up Tailscale on a VPS and connect from my Mac?">
    ...
    ```bash
    astroclaw gateway --tailscale serve
    ```
...
<AccordionGroup>
  <Accordion title="How does Astroclaw load environment variables?">
    Astroclaw reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.astroclaw/.env` (aka `$ASTROCLAW_STATE_DIR/.env`)
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.astroclaw/.env` so they're picked up even when the service doesn't inherit your shell env.
    ...
    `ASTROCLAW_LOAD_SHELL_ENV=1`, `ASTROCLAW_SHELL_ENV_TIMEOUT_MS=15000`.

  </Accordion>

  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `astroclaw models status` reports whether **shell env import** is enabled. "Shell env: off"
    does **not** mean your env vars are missing - it just means Astroclaw won't load
    your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
    environment. Fix by doing one of these:

    1. Put the token in `~/.astroclaw/.env`:
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of Astroclaw instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset Astroclaw but keep it installed?">
    ...
    - If you used profiles (`--profile` / `ASTROCLAW_PROFILE`), reset each state dir (defaults are `~/.astroclaw-<profile>`).
    - Dev reset: `astroclaw gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    headers like `# Heading`), Astroclaw skips the heartbeat run to save API calls.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. Astroclaw runs on **your own account**, so if you're in the group, Astroclaw can see it.
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    astroclaw directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does Astroclaw not reply in a group?">
    Two common causes:
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth:** sessions + transcripts live under `~/.astroclaw/agents/<agentId>/sessions/`.
    ...
    - Use `astroclaw doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```
    --port > ASTROCLAW_GATEWAY_PORT > gateway.port > default 18789
    ```

  </Accordion>

  <Accordion title='Why does astroclaw gateway status say "Runtime: running" but "Connectivity probe: failed"?'>
    ...
    Use `astroclaw gateway status` and trust these lines:
  ...
  <Accordion title='Why does astroclaw gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `ASTROCLAW_STATE_DIR` mismatch).
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    Astroclaw enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

    Fix: stop the other instance, free the port, or run with `astroclaw gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run Astroclaw in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `astroclaw gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `astroclaw dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
    - If you don't have a token yet: `astroclaw doctor --generate-gateway-token`.
    - If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
    - Shared-secret mode: set `gateway.auth.token` / `ASTROCLAW_GATEWAY_TOKEN` or `gateway.auth.password` / `ASTROCLAW_GATEWAY_PASSWORD`, then paste the matching secret in Control UI settings.
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `astroclaw devices list`
      - `astroclaw devices rotate --device <id> --role operator`
    ...
    - Still stuck? Run `astroclaw status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - `ASTROCLAW_CONFIG_PATH` (per-instance config)
    - `ASTROCLAW_STATE_DIR` (per-instance state)
    - `agents.defaults.workspace` (workspace isolation)
    - `gateway.port` (unique ports)

    Quick setup (recommended):

    - Use `astroclaw --profile <name> ...` per instance (auto-creates `~/.astroclaw-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `astroclaw --profile <name> gateway install`.

    Profiles also suffix service names (`ai.astroclaw.<profile>`; legacy `com.astroclaw.*`, `astroclaw-gateway-<profile>.service`, `Astroclaw Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    astroclaw tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    ```
    /tmp/astroclaw/astroclaw-YYYY-MM-DD.log
    ```
    ...
    - macOS: `$ASTROCLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.astroclaw/logs/...`; profiles use `~/.astroclaw-<profile>/logs/...`)
    - Linux: `journalctl --user -u astroclaw-gateway[-<profile>].service -n 200 --no-pager`
    - Windows: `schtasks /Query /TN "Astroclaw Gateway (<profile>)" /V /FO LIST`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `astroclaw gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart Astroclaw?">
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    1. Is the Gateway running? `astroclaw gateway status`
    2. Is the Gateway healthy? `astroclaw status`
    3. Does the UI have the right token? `astroclaw dashboard`
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    astroclaw channels status
    astroclaw channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Astroclaw already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: astroclaw gateway restart vs astroclaw gateway">
    - `astroclaw gateway restart`: restarts the **background service** (launchd/systemd).
    - `astroclaw gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `astroclaw gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Astroclaw assistant setup](/start/astroclaw) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    astroclaw message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose Astroclaw to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `astroclaw pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `astroclaw pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `astroclaw doctor` to surface risky DM policies.
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Astroclaw only replies to chats it receives or to explicit sends you trigger.
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    Astroclaw blocks **cross-provider** messaging by default. If a tool call is bound
