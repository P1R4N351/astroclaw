...
This is handy when you need to toggle obscure settings without editing `astroclaw.json`.
...
Use `ASTROCLAW_PLUGIN_LIFECYCLE_TRACE=1` when plugin lifecycle commands feel slow
...
```bash
ASTROCLAW_PLUGIN_LIFECYCLE_TRACE=1 astroclaw plugins install tokenjuice --force
```
...
runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm astroclaw ...`
...
`ASTROCLAW_RUN_NODE_CPU_PROF_DIR`:

```bash
ASTROCLAW_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm astroclaw status
```
...
```bash
ASTROCLAW_TRACE_SYNC_IO=1 pnpm astroclaw gateway --force
```

`pnpm gateway:watch` leaves this flag disabled by default for the watched
Gateway child. Set `ASTROCLAW_TRACE_SYNC_IO=1` when you explicitly want Node
...
`astroclaw-gateway-watch-main` (or a profile/port-specific variant such as
`astroclaw-gateway-watch-dev-19001`) and auto-attaches from interactive terminals.
...
```bash
tmux attach -t astroclaw-gateway-watch-main
```
...
# or
ASTROCLAW_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Disable auto-attach while keeping tmux management:

```bash
ASTROCLAW_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
`ASTROCLAW_TRACE_SYNC_IO=1` with `--benchmark` when you explicitly want both CPU
...
`ASTROCLAW_PROFILE`, `ASTROCLAW_CONFIG_PATH`, `ASTROCLAW_STATE_DIR`,
`ASTROCLAW_GATEWAY_PORT`, and `ASTROCLAW_SKIP_CHANNELS` into the pane. Put
...
`astroclaw doctor --fix --non-interactive` once and restarts the Gateway child.
Use `ASTROCLAW_GATEWAY_WATCH_AUTO_DOCTOR=0` when you want the original startup
...
extension `package.json` and `astroclaw.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.astroclaw-dev` and
...
```bash
pnpm gateway:dev
ASTROCLAW_PROFILE=dev astroclaw tui
```

If you don't have a global install yet, run the CLI via `pnpm astroclaw ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `ASTROCLAW_PROFILE=dev`
   - `ASTROCLAW_STATE_DIR=~/.astroclaw-dev`
   - `ASTROCLAW_CONFIG_PATH=~/.astroclaw-dev/astroclaw.json`
   - `ASTROCLAW_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`ASTROCLAW_SKIP_CHANNELS=1`).
...
```bash
ASTROCLAW_PROFILE=dev astroclaw gateway --dev --reset
```
...
## Raw stream logging (Astroclaw)

Astroclaw can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.astroclaw/logs/raw-stream.jsonl
```

Equivalent env vars:

```bash
ASTROCLAW_RAW_STREAM=1
ASTROCLAW_RAW_STREAM_PATH=~/.astroclaw/logs/raw-stream.jsonl
```

Default file:

`~/.astroclaw/logs/raw-stream.jsonl`
...
- If you need to use the built Astroclaw CLI for other tasks (i.e. `dashboard --no-open` if your debug session spawns a new auth token), you can execute it in another terminal as `node ./astroclaw.mjs` or create a shell alias like `alias astroclaw-build="node $(pwd)/astroclaw.mjs"`
