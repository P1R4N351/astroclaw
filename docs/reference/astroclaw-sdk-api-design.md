---
summary: "Reference design for the public Astroclaw App SDK API, event taxonomy, artifacts, approvals, and package structure"
title: "Astroclaw App SDK API design"
sidebarTitle: "App SDK API design"
read_when:
  - You are implementing the proposed public Astroclaw app SDK
  - You need the draft namespace, event, result, artifact, approval, or security contract for the app SDK
  - You are comparing Gateway protocol resources with the high-level Astroclaw App SDK wrapper
---

This page is the detailed API reference design for the public
[Astroclaw App SDK](/concepts/astroclaw-sdk). It is intentionally separate from
the [Plugin SDK](/plugins/sdk-overview).

<Note>
  `@astroclaw/sdk` is the external app/client package for talking to the
  Gateway. `astroclaw/plugin-sdk/*` is the in-process plugin authoring contract.
...
2. A high-level ergonomic wrapper with `Astroclaw`, `Agent`, `Session`, `Run`,
...
- source: Astroclaw, MCP, plugin, channel, runtime, or app
- schema summary
- approval policy
...
| Package                 | Purpose                                                       |
| ----------------------- | ------------------------------------------------------------- |
| `@astroclaw/sdk`         | Public high-level SDK and generated low-level Gateway client. |
| `@astroclaw/sdk-react`   | Optional React hooks for dashboards and app builders.         |
| `@astroclaw/sdk-testing` | Test helpers and fake Gateway server for app integrations.    |

The repo already has `astroclaw/plugin-sdk/*` for plugins. Keep that namespace
...
4. High-level `Astroclaw`, `Agent`, `Session`, `Run`, `Task`, and `Artifact`
   wrappers.
...
- [Astroclaw App SDK](/concepts/astroclaw-sdk)
