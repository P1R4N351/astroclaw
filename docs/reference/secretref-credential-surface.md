...
- In scope: strictly user-supplied credentials that Astroclaw does not mint or rotate.
- Out of scope: runtime-minted or rotating credentials, OAuth refresh material, and session-like artifacts.

## Supported credentials

### `astroclaw.json` targets (`secrets configure` + `secrets apply` + `secrets audit`)
...
- In `astroclaw.json`, SecretRefs must use structured objects such as `{"source":"env","provider":"default","id":"DISCORD_BOT_TOKEN"}`. Legacy `secretref-env:<ENV_VAR>` marker strings are rejected on SecretRef credential paths; run `astroclaw doctor --fix` to migrate valid markers.
...
- Marker persistence is source-authoritative: Astroclaw writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
- For web search:
