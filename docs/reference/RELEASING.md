...
Astroclaw has three public release lanes:
...
- Every stable Astroclaw release ships the npm package and macOS app together;
...
6. Run `Astroclaw NPM Release` with `preflight_only=true`. Before a tag exists,
...
9. For beta, tag `vYYYY.M.D-beta.N`, then run `Astroclaw Release Publish` from
   ...
   ClawHub in parallel, and then promotes the prepared Astroclaw npm preflight
   artifact with the matching dist-tag as soon as plugin npm publish succeeds.
   After the Astroclaw npm publish child succeeds, it creates or updates the
   ...
   `astroclaw-<version>-dependency-evidence.zip` for post-release incident
   response.
   ClawHub publishing may still be running while Astroclaw npm publishes, but the
   release publish workflow prints the child run IDs immediately. By default it
   does not wait for ClawHub after dispatching it, so Astroclaw npm availability
   ...
   `pnpm release:verify-beta -- YYYY.M.D-beta.N --astroclaw-npm-run <run-id> --plugin-npm-run <run-id> --plugin-clawhub-run <run-id>`
   ...
   `astroclaw@YYYY.M.D-beta.N` or
   `astroclaw@beta` package. If a pushed or published prerelease needs a fix,
...
10. For stable, continue only after the vetted beta or release candidate has the
    ...
    `Astroclaw Release Publish`, reusing the successful preflight artifact via
...
- Run the manual `Full Release Validation` workflow before release approval to
  ...
  `Astroclaw Release Checks` for install smoke, package acceptance, cross-OS
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof
  ...
  `astroclaw@beta`, `astroclaw@latest`, or an exact release version; `source=ref`
  ...
  Example: `gh workflow run package-acceptance.yml --ref main -f workflow_ref=main -f source=npm -f package_spec=astroclaw@beta -f suite_profile=product -f published_upgrade_survivor_baseline=astroclaw@2026.4.26 -f telegram_mode=mock-openai`
  Common profiles:
...
- `Astroclaw NPM Release` preflight generates dependency release evidence before
...
- The preflight uploads dependency evidence as
  `astroclaw-release-dependency-evidence-<tag>` and also embeds it under
  ...
  to the GitHub release as `astroclaw-<version>-dependency-evidence.zip`.
- Run `Astroclaw Release Publish` for the mutating publish sequence after the
  tag exists. Dispatch it from `release/YYYY.M.D` (or `main` when publishing a
  main-reachable tag), pass the release tag and successful Astroclaw npm
  ...
  workflow serializes plugin npm publish, plugin ClawHub publish, and Astroclaw
  npm publish so the core package is not published before its externalized
  plugins.
- Release checks now run in a separate manual workflow:
  `Astroclaw Release Checks`
- `Astroclaw Release Checks` also runs the QA Lab mock parity lane plus the fast
...
- Cross-OS install and upgrade runtime validation is part of public
  `Astroclaw Release Checks` and `Full Release Validation`, which call the
  reusable workflow
  `.github/workflows/astroclaw-cross-os-release-checks-reusable.yml` directly
...
- `Astroclaw Release Checks` accepts a branch, tag, or full commit SHA as long
  as the resolved commit is reachable from an Astroclaw branch or release tag
- `Astroclaw NPM Release` validation-only preflight also accepts the current
...
- That workflow runs
  `ASTROCLAW_LIVE_TEST=1 ASTROCLAW_LIVE_CACHE_TEST=1 pnpm test:live:cache`
...
- Before tagging a release candidate locally, run
  ...
  UI build, and `release:astroclaw:npm:check` in the order that catches common
  approval-blocking mistakes before the GitHub publish workflow starts.
- Run `RELEASE_TAG=vYYYY.M.D node --import tsx scripts/astroclaw-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/astroclaw-npm-postpublish-verify.ts YYYY.M.D`
  (or the matching beta/correction version) to verify the published registry
  install path in a fresh temp prefix
- After a beta publish, run `ASTROCLAW_NPM_TELEGRAM_PACKAGE_SPEC=astroclaw@YYYY.M.D-beta.N ASTROCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex ASTROCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE=ci pnpm test:docker:npm-telegram-live`
  ...
  `ASTROCLAW_QA_TELEGRAM_*` env credentials directly.
...
- Maintainer release automation now uses preflight-then-promote:
  ...
  - token-based npm dist-tag mutation now lives in
    `astroclaw/releases-private/.github/workflows/astroclaw-npm-dist-tags.yml`
...
gh workflow run full-release-validation.yml \
  ...
  -f evidence_package_spec=astroclaw@YYYY.M.D-beta.N
```

The workflow resolves the target ref, dispatches manual `CI` with
`target_ref=<release-ref>`, dispatches `Astroclaw Release Checks`, prepares a
...
`npm_telegram_package_spec` is set. `Astroclaw Release
...
`Astroclaw Release Checks` uses the trusted workflow ref to resolve the target
...
After a beta is already on npm, set `release_package_spec=astroclaw@YYYY.M.D-beta.N`
...
The cross-OS OpenAI install smoke uses `ASTROCLAW_CROSS_OS_OPENAI_MODEL` when the
...
gh workflow run full-release-validation.yml \
  ...
  -f release_package_spec=astroclaw@YYYY.M.D-beta.N \
  -f evidence_package_spec=astroclaw@YYYY.M.D-beta.N \
...
The Docker box lives in `Astroclaw Release Checks` through
`astroclaw-live-and-e2e-checks-reusable.yml`, plus the release-mode
...
The QA Lab box is also part of `Astroclaw Release Checks`. It is the agentic
...
`scripts/resolve-astroclaw-package-candidate.mjs`. The resolver normalizes a
...
- `source=npm`: `astroclaw@beta`, `astroclaw@latest`, or an exact Astroclaw release
  version
...
`Astroclaw Release Checks` runs Package Acceptance with `source=artifact`, the
...
gh workflow run package-acceptance.yml \
  ...
  -f published_upgrade_survivor_baseline=astroclaw@2026.4.26
```
...
`Astroclaw Release Publish` is the normal mutating publish entrypoint. It
...
6. Dispatch `Astroclaw NPM Release` with the release tag, npm dist-tag, and
...
`plugin_publish_scope=selected` and `plugins=@astroclaw/name` to
`Astroclaw Release Publish`, or dispatch the child workflow directly when the
Astroclaw package must not be published.

## NPM workflow inputs

`Astroclaw NPM Release` accepts these operator-controlled inputs:
...
`Astroclaw Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `Astroclaw NPM Release` preflight run id;
  required when `publish_astroclaw_npm=true`
- `npm_dist_tag`: npm target tag for the Astroclaw package
- `plugin_publish_scope`: defaults to `all-publishable`; use `selected` only
  for focused repair work
- `plugins`: comma-separated `@astroclaw/*` package names when
  `plugin_publish_scope=selected`
- `publish_astroclaw_npm`: defaults to `true`; set `false` only when using the
...
`Astroclaw Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks
  require the resolved commit to be reachable from an Astroclaw branch or
  release tag.
...
- For `Astroclaw NPM Release`, full commit SHA input is allowed only when
  `preflight_only=true`
- `Astroclaw Release Checks` and `Full Release Validation` are always
  validation-only
...
1. Run `Astroclaw NPM Release` with `preflight_only=true`
...
6. Run `Astroclaw Release Publish` with the same `tag`, the same `npm_dist_tag`,
   and the saved `preflight_run_id`; it publishes externalized plugins to npm
   and ClawHub before promoting the Astroclaw npm package
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/astroclaw/astroclaw/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/astroclaw/astroclaw/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/astroclaw-npm-release.yml`](https://github.com/astroclaw/astroclaw/blob/main/.github/workflows/astroclaw-npm-release.yml)
- [`.github/workflows/astroclaw-release-checks.yml`](https://github.com/astroclaw/astroclaw/blob/main/.github/workflows/astroclaw-release-checks.yml)
- [`.github/workflows/astroclaw-cross-os-release-checks-reusable.yml`](https://github.com/astroclaw/astroclaw/blob/main/.github/workflows/astroclaw-cross-os-release-checks-reusable.yml)
- [`scripts/resolve-astroclaw-package-candidate.mjs`](https://github.com/astroclaw/astroclaw/blob/main/scripts/resolve-astroclaw-package-candidate.mjs)
- [`scripts/astroclaw-npm-release-check.ts`](https://github.com/astroclaw/astroclaw/blob/main/scripts/astroclaw-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/astroclaw/astroclaw/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/astroclaw/astroclaw/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in
[`astroclaw/maintainers/release/README.md`](https://github.com/astroclaw/maintainers/blob/main/release/README.md)
