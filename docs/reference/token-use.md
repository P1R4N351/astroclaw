---
summary: "How Astroclaw builds prompt context and reports token usage + costs"
read_when:
...
Astroclaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Astroclaw assembles its own system prompt on every run. It includes:
...
For images, Astroclaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Astroclaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `astroclaw status --usage` and `astroclaw channels list` show
...
available; otherwise Astroclaw falls back to matching OAuth/API-key credentials
...
Astroclaw keeps provider usage accounting separate from the current context
...
`cacheWrite`. If pricing is missing, Astroclaw shows tokens only. OAuth tokens
never show dollar cost.

After sidecars and channels reach the Gateway ready path, Astroclaw starts an
...
Provider prompt caching only applies within the cache TTL window. Astroclaw can
...
Anthropic's 1M context window is currently beta-gated. Astroclaw can inject the
...
Astroclaw skips the `context-1m-*` beta header because Anthropic currently
