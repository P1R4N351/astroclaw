...
Astroclaw features that can generate provider usage or paid API calls.
...
- Gemini CLI note: when the CLI returns JSON output, Astroclaw reads usage from
  `stats`, normalizes `stats.cached` into `cacheRead`, and derives input tokens
  from `stats.input_tokens - stats.cached` when needed.

Anthropic note: Anthropic staff told us Astroclaw-style Claude CLI usage is
...
Anthropic still does not expose a per-message dollar estimate that Astroclaw can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `astroclaw status --usage` and `astroclaw channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so Astroclaw inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, Astroclaw prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise Astroclaw falls back to matching OAuth/API-key
...
Astroclaw can pick up credentials from:
...
Astroclaw's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's Astroclaw Claude-login path with **Extra Usage** enabled.
...
`astroclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
