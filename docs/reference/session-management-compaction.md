...
Astroclaw manages sessions end-to-end across these areas:
...
Astroclaw is designed around a single **Gateway process** that owns session state.
...
Astroclaw persists sessions in two layers:
...
- Transcripts: `~/.astroclaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Astroclaw resolves these via `src/config/sessions.ts`.
...
Astroclaw no longer creates automatic `sessions.json.bak.*` rotation backups during Gateway writes. The legacy `session.maintenance.rotateBytes` key is ignored and `astroclaw doctor --fix` removes it from older configs.
...
In `mode: "warn"`, Astroclaw reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
astroclaw sessions cleanup --dry-run
astroclaw sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.astroclaw/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
- **Parent fork policy** uses PI's active branch when creating a thread or subagent fork. If that branch is too large, Astroclaw starts the child with isolated context instead of failing or inheriting unusable history. The sizing policy is automatic; legacy `session.parentForkMaxTokens` config is removed by `astroclaw doctor --fix`.
...
Astroclaw intentionally does **not** "fix up" transcripts; the Gateway uses `SessionManager` to read/write them.
...
When Astroclaw splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, Astroclaw
...
- If a trailing tool-result block would otherwise push the chunk over target,
  Astroclaw preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (Astroclaw consumes the events, but Pi decides when to compact).

Astroclaw can also trigger a preflight local compaction before opening the next
...
reopen cost, not raw archival: Astroclaw still runs normal semantic compaction,
...
next model call, Astroclaw estimates the prompt pressure using the same preflight
...
Astroclaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Astroclaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it's already higher, Astroclaw leaves it alone.
...
- When `agents.defaults.compaction.truncateAfterCompaction` is enabled,
  Astroclaw rotates the active transcript to a compacted successor JSONL after
...
- If the provider fails or returns an empty result, Astroclaw falls back to built-in LLM summarization automatically.
...
- `astroclaw status` (CLI)
- `astroclaw sessions` / `sessions --json`
- Gateway logs (`pnpm gateway:watch` or `astroclaw logs --follow`): `embedded run auto-compaction start` + `complete`
...
Astroclaw supports "silent" turns for background tasks where the user should not see intermediate output.
...
- Astroclaw strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, Astroclaw also suppresses **draft/typing streaming** when a
...
Astroclaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Astroclaw's
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `astroclaw status`.
