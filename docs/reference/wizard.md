...
This is the full reference for `astroclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.astroclaw/astroclaw.json` exists, choose **Keep current values**, **Review and update**, or **Reset before setup**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `astroclaw doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though Astroclaw now prefers Claude CLI reuse when available.
    ...
    - Auth profiles live in `~/.astroclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.astroclaw/credentials/oauth.json` is legacy import-only.
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.astroclaw/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$ASTROCLAW_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
    is only a legacy import source.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.astroclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `astroclaw pairing approve <channel> <code>` or use allowlists.
  ...
  <Step title="Web search">
    ...
    - Configure later: `astroclaw configure --section web`.
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `astroclaw health`.
    - Tip: `astroclaw status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
...
- Stores it under `~/.astroclaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.astroclaw/astroclaw.json`:
...
`astroclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.astroclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.astroclaw/agents/<agentId>/sessions/`.
