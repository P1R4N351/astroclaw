...
- Re-open anytime: `astroclaw dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
- If clipboard and browser delivery fail, `astroclaw dashboard` still prints the
  clean URL and tells you to use the token from `ASTROCLAW_GATEWAY_TOKEN` or
...
- **Shared-secret token source**: `gateway.auth.token` (or
  `ASTROCLAW_GATEWAY_TOKEN`); `astroclaw dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `astroclaw dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `astroclaw dashboard` still prints a non-tokenized URL plus
  actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or
  `ASTROCLAW_GATEWAY_PASSWORD`). The dashboard does not persist passwords across
  reloads.
...
- Ensure the gateway is reachable (local: `astroclaw status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `astroclaw config get gateway.auth.token`
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `ASTROCLAW_GATEWAY_TOKEN` in this shell, then rerun `astroclaw dashboard`
  - No shared secret configured: `astroclaw doctor --generate-gateway-token`
