...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/astroclaw`)
...
If the page fails to load, start the Gateway first: `astroclaw gateway`.
...
If the browser retries pairing with changed auth details (role/scopes/public key), the previous pending request is superseded and a new `requestId` is created. Re-run `astroclaw devices list` before approval.

If the browser is already paired and you change it from read access to write/admin access, this is treated as an approval upgrade, not a silent reconnect. Astroclaw keeps the old approval active, blocks the broader reconnect, and asks you to approve the new scope set explicitly.

Once approved, the device is remembered and won't require re-approval unless you revoke it with `astroclaw devices revoke --device <id> --role <role>`. See [Devices CLI](/cli/devices) for token rotation and revocation.
...
<AccordionGroup>
  ...
  <Accordion title="Config">
    - View/edit `~/.astroclaw/astroclaw.json` (`config.get`, `config.set`).
...
<AccordionGroup>
  ...
  <Accordion title="Stop and abort">
    ...
    - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop astroclaw`, `please stop`) to abort out-of-band.
...
If the page shows **Protocol mismatch** right after an Astroclaw update, first reopen the dashboard with `astroclaw dashboard` and hard-refresh the page. If it still fails, clear site data for the dashboard origin or test in a private browser window; an old tab or browser service-worker cache can keep running a pre-update Control UI bundle against the newer Gateway.
...
| `push/vapid-keys.json` (under the Astroclaw state dir) | Auto-generated VAPID keypair used to sign Web Push payloads.       |
...
- `ASTROCLAW_VAPID_PUBLIC_KEY`
- `ASTROCLAW_VAPID_PRIVATE_KEY`
- `ASTROCLAW_VAPID_SUBJECT` (defaults to `mailto:astroclaw@localhost`)
...
<Tabs>
  ...
  <Tab title="Bind to tailnet + token">
    ```bash
    astroclaw gateway --bind tailnet --token "$(openssl rand -hex 32)"
    ```
...
If you open the dashboard over plain HTTP (`http://<lan-ip>` or `http://<tailscale-ip>`), the browser runs in a **non-secure context** and blocks WebCrypto. By default, Astroclaw **blocks** Control UI connections without device identity.
...
- `GET /__astroclaw__/assistant-media?meta=1&source=<path>` requires the normal Control UI operator auth. The browser sends the gateway token as a bearer header when checking availability.
...
```bash
ASTROCLAW_CONTROL_UI_BASE_PATH=/astroclaw/ pnpm ui:build
```
...
If the browser loads a blank dashboard and DevTools shows no useful error, an extension or early content script may have prevented the JavaScript module app from evaluating. The static page includes a plain HTML recovery panel that appears when `<astroclaw-app>` is not registered after startup.
