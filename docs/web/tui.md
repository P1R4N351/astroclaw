...
```bash
astroclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- `astroclaw chat` and `astroclaw terminal` are aliases for `astroclaw tui --local`.
- `--local` cannot be combined with `--url`, `--token`, or `--password`.
- Local mode uses the embedded agent runtime directly. Most local tools work, but Gateway-only features are unavailable.
- `astroclaw` and `astroclaw crestodian` also use this TUI shell, with Crestodian as the local setup and repair chat backend.
...
- Turn delivery on:
  ...
  - or start with `astroclaw tui --deliver`
...
- Local shell commands receive `ASTROCLAW_SHELL=tui-local` in their environment.
...
If `astroclaw config validate` is already failing, start with `astroclaw configure`
or `astroclaw doctor --fix` first. `astroclaw chat` does not bypass the invalid-
config guard.
...
4. Apply narrow changes with `astroclaw config set` or `astroclaw configure`, then rerun `!astroclaw config validate`.
5. If Doctor recommends an automatic migration or repair, review it and run `!astroclaw doctor --fix`.

Tips:

- Prefer `astroclaw config set` or `astroclaw configure` over hand-editing `astroclaw.json`.
- `astroclaw docs "<query>"` searches the live docs index from the same machine.
- `astroclaw config validate --json` is useful when you want structured schema and SecretRef/resolvability errors.
...
- If your terminal uses a light background and auto-detection is wrong, set `ASTROCLAW_THEME=light` before launching `astroclaw tui`.
- To force the original dark palette instead, set `ASTROCLAW_THEME=dark`.
...
- Check the Gateway logs: `astroclaw logs --follow`.
- Confirm the agent can run: `astroclaw status` and `astroclaw models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `astroclaw agents list` and your routing config.
- Empty session picker: you might be in global scope or have no sessions yet.

## Related

- [Control UI](/web/control-ui) — web-based control interface
- [Config](/cli/config) — inspect, validate, and edit `astroclaw.json`
