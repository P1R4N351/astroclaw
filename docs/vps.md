---
summary: "Run Astroclaw on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for Astroclaw
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the Astroclaw Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install Astroclaw on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep Astroclaw bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/astroclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `ASTROCLAW_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for a stable startup path:
  ...
  - `NODE_COMPILE_CACHE=/var/tmp/astroclaw-compile-cache`
...
For the standard `astroclaw onboard --install-daemon` path, edit the user unit:
...
`astroclaw-gateway.service` via `sudo systemctl edit astroclaw-gateway.service`.
