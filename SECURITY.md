...
Security work is shared across a number of Astroclaw maintainers, including engineers and security researchers from organizations such as NVIDIA and Tencent. See the [maintainer list](CONTRIBUTING.md#maintainers).

## Report a Security Issue

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [astroclaw/astroclaw](https://github.com/astroclaw/astroclaw)
- **macOS desktop app** — [astroclaw/astroclaw](https://github.com/astroclaw/astroclaw) (apps/macos)
- **iOS app** — [astroclaw/astroclaw](https://github.com/astroclaw/astroclaw) (apps/ios)
- **Android app** — [astroclaw/astroclaw](https://github.com/astroclaw/astroclaw) (apps/android)
- **ClawHub** — [astroclaw/clawhub](https://github.com/astroclaw/clawhub)
- **Trust and threat model** — [astroclaw/trust](https://github.com/astroclaw/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@astroclaw.ai](mailto:security@astroclaw.ai)** and we'll route it.

For Astroclaw core issues, submit through a private [GitHub Security Advisory](https://github.com/astroclaw/astroclaw/security/advisories/new). Do not open a public issue or PR that discloses an unpatched vulnerability, exploit path, secret, or security-sensitive proof of concept.
...
For full reporting instructions see our [Trust page](https://trust.astroclaw.ai).
For maintainer response workflow, see the [incident response plan](docs/security/incident-response.md).

Astroclaw does not currently run a paid bug bounty program. Please still disclose responsibly so we can fix real issues quickly. The best way to help the project right now is to send high-signal reports and, when practical, focused PRs.
...
- The actual impact, including which Astroclaw trust boundary is crossed.
- Any remediation advice or focused patch you can provide.

Reports without reproduction steps, demonstrated impact, and remediation advice are deprioritized. We receive a high volume of AI-generated scanner findings, so we prioritize vetted reports from researchers who can show how the issue crosses an Astroclaw security boundary.
...
- Scanner-only, dependency-only, or stale-path reports without a working repro and demonstrated Astroclaw impact.
...
- Tested version details (Astroclaw version and/or commit SHA).
...
- For dependency CVE reports, evidence that the shipped dependency version is actually affected, plus a PoC that reproduces impact through Astroclaw. Showing that Astroclaw can reach a native parser is not enough by itself.
- Demonstrated impact tied to Astroclaw's documented trust boundaries.
- For exposed-secret reports: proof the credential is Astroclaw-owned (or grants access to Astroclaw-operated infrastructure/services).
...
- Reports that only show untrusted media bytes reaching a maintained native decoder dependency (for example Sharp/libvips/libheif) without proving the shipped dependency version is vulnerable and demonstrating crash, memory corruption, data exposure, or a boundary bypass through Astroclaw. JavaScript header sniffing and image dimension fast-paths are preflight/UX checks, not the security boundary for native decoder correctness.
- Reports whose only impact is transient extra memory, CPU, or allocation work from decoding, base64 expansion, media transcoding, serialization, or other format conversion after the input was already accepted under Astroclaw's configured size/trust limits, including base64 decode-before-size-estimate findings. These are performance issues, not vulnerabilities, unless the report demonstrates unauthenticated amplification, bypass of configured limits, crash/process termination, persistent resource exhaustion, data exposure, or another documented boundary bypass.
...
Astroclaw does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Concretely, on the OpenAI-compatible HTTP surface:
  ...
  - narrower `x-astroclaw-scopes` headers are ignored for that shared-secret path
...
- Astroclaw can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Astroclaw, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Astroclaw's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

### Trusted Plugins

Plugins/extensions are part of Astroclaw's trusted computing base for a gateway.
...
- Using Astroclaw in ways that the docs recommend not to
- Test-only code and maintainer harnesses, including QA Lab, QE Lab, E2E fixtures, benchmark rigs, smoke-test containers, and local debugging proxies, unless the report demonstrates that the same vulnerable behavior is reachable from shipped Astroclaw production code or a published package artifact intended for users.
...
- Reports that require write access to trusted local state (`~/.astroclaw`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not Astroclaw-owned and not granting access to Astroclaw-operated infrastructure/services) without demonstrated Astroclaw impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.
- SSRF reports limited to the operator-managed HTTP/WebSocket proxy-routing feature where the demonstrated mitigation is to enable/configure `proxy.enabled` with a filtering `proxy.proxyUrl`/`ASTROCLAW_PROXY_URL`, or where impact depends on a permissive/misconfigured operator proxy. This only covers normal process-local HTTP(S)/WebSocket egress (`fetch`, Node HTTP(S), and similar JavaScript clients); non-HTTP egress and other features are assessed separately. See [Network proxy](https://docs.astroclaw.ai/security/network-proxy).

### Deployment Assumptions

Astroclaw security guidance assumes:

- The host where Astroclaw runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.astroclaw` state/config (including `astroclaw.json`) is effectively a trusted operator.
...
Astroclaw's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
Astroclaw separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when Astroclaw can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the Astroclaw process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

### Temp Folder Boundary

Astroclaw uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/astroclaw` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/astroclaw` (or `astroclaw-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the Astroclaw-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use Astroclaw temp helpers (`resolvePreferredAstroclawTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-astroclaw-dir.ts`
...
For threat model + hardening guidance (including `astroclaw security audit --deep` and `--fix`), see:
...
Astroclaw's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `astroclaw gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - Astroclaw keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `astroclaw security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__astroclaw__/canvas/`, `/__astroclaw__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__astroclaw__/canvas/`, `/__astroclaw__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

Astroclaw requires **Node.js 22.16.0 or later** (LTS). This version includes important security patches:
...
When running Astroclaw in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v astroclaw-data:/app/data \
  astroclaw/astroclaw:latest
```

## Security Scanning

Astroclaw uses several security and release-validation layers. No single scanner is treated as the boundary.

### Secret Detection

Astroclaw runs the pre-commit `detect-private-key` hook in CI and keeps secret-resolution behavior covered by the dedicated secrets test surface.
