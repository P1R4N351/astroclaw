---
name: discord-clawd
description: Use to talk to the Discord-backed Astroclaw agent/session; not for archive search.
---
...
Use the Astroclaw relay helper:

```bash
cd ~/Projects/agent-scripts
python3 skills/astroclaw-relay/scripts/astroclaw_relay.py targets
python3 skills/astroclaw-relay/scripts/astroclaw_relay.py resolve --target maintainers
```

If the target alias exists, prefer a private ask first:

```bash
python3 skills/astroclaw-relay/scripts/astroclaw_relay.py ask \
  --target maintainers \
