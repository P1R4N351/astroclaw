...
```bash
export ASTROCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.astroclaw/astroclaw.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env ASTROCLAW_PARALLELS_DISCORD_TOKEN \
...
- Use the `astroclaw` wrapper for guest `message send/read`; `node astroclaw.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/astroclaw-parallels-smoke.*`
...
- host inbound nonce appears in `astroclaw message read` output
