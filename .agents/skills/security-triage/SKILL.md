...
description: Triage Astroclaw security advisories, drafts, and GHSA reports with shipped-tag and trust-model proof.
---

# Security Triage

Use when reviewing Astroclaw security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/astroclaw/astroclaw/security-advisories/<GHSA>`.
3. Inspect the exact implicated code paths.
4. Verify shipped state:
   - `git tag --sort=-creatordate | head`
   - `npm view astroclaw version --userconfig "$(mktemp)"`
...
```bash
gh api /repos/astroclaw/astroclaw/security-advisories/<GHSA>
gh api /repos/astroclaw/astroclaw/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view astroclaw version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo astroclaw/astroclaw --match title,body,comments -- "<terms>"
gh search prs --repo astroclaw/astroclaw --match title,body,comments -- "<terms>"
```
