---
name: astroclaw-test-performance
description: Benchmark, diagnose, and optimize Astroclaw test and plugin-suite runtime, import hotspots, CPU/RSS, heap growth, and slow coverage paths.
---

# Astroclaw Test Performance
...
test:extensions:batch <plugin[,plugin...]>` or plugin-inspector command
   ...
   - For import-heavy suspicion add:
     `ASTROCLAW_VITEST_IMPORT_DURATIONS=1 ASTROCLAW_VITEST_PRINT_IMPORT_BREAKDOWN=1`.
...
4. For broad or package-heavy plugin proof, use Crabbox-backed Blacksmith
   ...
   - `pnpm crabbox:run -- --provider blacksmith-testbox --timing-json -- ASTROCLAW_TESTBOX=1 pnpm test:extensions:batch <ids>`
...
5. If plugin performance is package-artifact sensitive, switch to
   `astroclaw-pre-release-plugin-testing` and Package Acceptance rather than
...
| import duration | broad barrel/runtime loads         | `ASTROCLAW_VITEST_IMPORT_DURATIONS=1`                                        |
...
| heap snapshots  | real leak vs retained module graph | `astroclaw-test-heap-leaks` workflow                                         |
...
```bash
ASTROCLAW_TEST_MEMORY_TRACE=1 \
ASTROCLAW_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 \
ASTROCLAW_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap \
ASTROCLAW_TEST_WORKERS=2 \
ASTROCLAW_TEST_MAX_OLD_SPACE_SIZE_MB=6144 \
pnpm test
```

Use `astroclaw-test-heap-leaks` when RSS keeps growing across intervals, workers
...
- Parallel Vitest runs sharing `node_modules/.experimental-vitest-cache` without
  distinct `ASTROCLAW_VITEST_FS_MODULE_CACHE_PATH` values.
...
timeout 240 /usr/bin/time -l env \
  ASTROCLAW_VITEST_IMPORT_DURATIONS=1 \
