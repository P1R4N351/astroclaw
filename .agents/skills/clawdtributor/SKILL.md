---
name: clawdtributor
description: "Use for Astroclaw clawtributors PR/issue triage: Discrawl discovery, live-open rechecks, deep review, topic grouping, and compact @handle/LOC/type/blast/verification summaries."
---

# Clawdtributor

Use for the `#clawtributors` queue: Discord-discovered Astroclaw PRs/issues that need live GitHub status plus maintainer-quality review.

## Compose with other skills

- `$discrawl`: local Discord archive sync/search.
- `$astroclaw-pr-maintainer`: live GitHub PR/issue review, duplicate search, close/land rules.
- `$gitcrawl`: related issue/PR and current-main/stale-proof search.
- `$astroclaw-testing` / `$crabbox`: proof choice when a candidate needs real validation.
...
perl -nE 'while(m{github\.com/astroclaw/astroclaw/(pull|issues)/(\d+)}g){say "$1\t$2\t$_"}'
```
...
left join members mm on mm.guild_id=m.guild_id and mm.user_id=m.author_id
where m.channel_id='1458141495701012561'
  and m.content like '%github.com/astroclaw/astroclaw/<pull-or-issues>/<number>%'
order by m.created_at desc
limit 1;"
```
...
gh api repos/astroclaw/astroclaw/pulls/<number> \
...
```bash
GITHUB_TOKEN= GITHUB_TOKEN_NODIFF= GH_TOKEN= \
gh api repos/astroclaw/astroclaw/issues/<number> \
