---
name: clawsweeper
description: "Use for all ClawSweeper work: Astroclaw issue/PR sweep reports, commit-review reports, repair jobs, cloud fix PRs, @clawsweeper maintainer mention commands, trusted ClawSweeper-reviewed autofix/automerge, GitHub Actions monitoring, permissions, gates, and manual backfills."
---

# ClawSweeper

ClawSweeper lives at `~/Projects/clawsweeper`. It is the one Astroclaw
...
- Target app permissions: read target scan context; write issues and pull
  ...
  inputs; Actions write on `astroclaw/clawsweeper` for comment-router
...
pnpm commit-reports -- --repo astroclaw/astroclaw --author steipete --since 7d
...
```bash
gh workflow run commit-review.yml --repo astroclaw/clawsweeper \
  -f target_repo=astroclaw/astroclaw \
...
```bash
pnpm run repair:create-job -- \
  --from-report ../clawsweeper/records/astroclaw-astroclaw/items/123.md
```
...
```bash
pnpm run repair:validate-job -- jobs/astroclaw/inbox/clawsweeper-astroclaw-astroclaw-123.md
pnpm run repair:dispatch -- jobs/astroclaw/inbox/clawsweeper-astroclaw-astroclaw-123.md \
  --mode autonomous \
...
```bash
gh variable set CLAWSWEEPER_ALLOW_EXECUTE --repo astroclaw/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_FIX_PR --repo astroclaw/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_MERGE --repo astroclaw/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_AUTOMERGE --repo astroclaw/clawsweeper --body 1
```
...
@astroclaw-clawsweeper fix ci
@astroclaw-clawsweeper[bot] fix ci
```
...
```bash
pnpm run repair:comment-router -- --repo astroclaw/astroclaw --lookback-minutes 180
pnpm run repair:comment-router -- --repo astroclaw/astroclaw --execute --wait-for-capacity
```
...
leave the local Astroclaw checkout on the PR branch. After the PR is created,
...
escalation, and sensitive data exposure to central Astroclaw security handling.
...
```bash
gh run list --repo astroclaw/clawsweeper --workflow "ClawSweeper Commit Review" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo astroclaw/clawsweeper --workflow "repair cluster worker" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo astroclaw/clawsweeper --workflow "repair comment router" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
```
...
```bash
gh run list --repo astroclaw/astroclaw --workflow "ClawSweeper Dispatch" \
...
```bash
gh api "repos/astroclaw/astroclaw/commits/<sha>/check-runs?per_page=100" \
