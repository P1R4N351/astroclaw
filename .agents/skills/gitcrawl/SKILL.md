---
name: gitcrawl
description: Use gitcrawl for Astroclaw issue and PR archive search, duplicate discovery, related-thread clustering, and local GitHub mirror freshness checks.
metadata:
  astroclaw:
    requires:
      bins:
...
Use this skill before live GitHub search when triaging Astroclaw issues or PRs.
...
gitcrawl search astroclaw/astroclaw --query "<scope or title keywords>" --mode hybrid --limit 20 --json
```
...
```bash
gitcrawl init --portable-store git@github.com:astroclaw/gitcrawl-store.git --json
```
