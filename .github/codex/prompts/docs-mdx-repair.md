# Astroclaw Docs MDX Repair Agent

You are repairing generated Astroclaw documentation after a fast MDX validation failure.
...
1. Read `.astroclaw-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".astroclaw-sync/mdx/${LOCALE}.json"`.
