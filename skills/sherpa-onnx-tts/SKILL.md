...
1. Download the runtime for your OS (extracts into `$ASTROCLAW_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.astroclaw/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$ASTROCLAW_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.astroclaw/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Astroclaw config file (`$ASTROCLAW_CONFIG_PATH`, default `~/.astroclaw/astroclaw.json`):
