---
name: node-connect
description: Diagnose Astroclaw Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Astroclaw is advertising that route, then fix pairing/auth.
...
- whether `astroclaw devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `astroclaw qr --json`. It uses the same setup-code payload Android scans.

```bash
astroclaw config get gateway.mode
astroclaw config get gateway.bind
astroclaw config get gateway.tailscale.mode
astroclaw config get gateway.remote.url
astroclaw config get gateway.auth.mode
astroclaw config get gateway.auth.allowTailscale
astroclaw config get plugins.entries.device-pair.config.publicUrl
astroclaw qr --json
astroclaw devices list
astroclaw nodes status
```

If this Astroclaw instance is pointed at a remote gateway, also run:

```bash
astroclaw qr --remote --json
```
...
`astroclaw qr --json` success means:
...
If `astroclaw qr --json` says `Gateway is only bound to loopback`:
...
- `astroclaw devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run astroclaw qr again, rescan, then approve the pending device pairing.`
