---
name: healthcheck
description: Audit and harden hosts running Astroclaw for SSH, firewall, updates, exposure, cron checks, and risk posture.
---

# Astroclaw Host Hardening

## Overview

Assess and harden the host running Astroclaw, then align it to a user-defined risk tolerance without breaking access. Use Astroclaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Astroclaw changes the host firewall, SSH, or OS updates; it does not.
...
5. Astroclaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Astroclaw can access sensitive data.
...
### 2) Run Astroclaw security audits (read-only)

As part of the default read-only checks, run `astroclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `astroclaw security audit` (faster, non-probing)
2. `astroclaw security audit --json` (structured output)

Offer to apply Astroclaw safe defaults (numbered):

1. `astroclaw security audit --fix`

Be explicit that `--fix` only tightens Astroclaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Astroclaw version/update status (read-only)

As part of the default read-only checks, run `astroclaw update status`.
...
- Credential hygiene notes (location of Astroclaw creds, prefer disk encryption)
...
- Astroclaw security audit (re-run)
...
After Astroclaw install or first hardening pass, run at least one baseline audit and version check:

- `astroclaw security audit`
- `astroclaw security audit --deep`
- `astroclaw update status`

Ongoing monitoring is recommended. Use the Astroclaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `astroclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `astroclaw update status`
...
Before creating, `astroclaw cron list` and match on exact `name`. If found, `astroclaw cron edit <id> ...`.
If not found, `astroclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `astroclaw update status` (preferred for source checkouts and channels)
2. `npm view astroclaw version` (published npm version)

## Astroclaw command accuracy

Use only supported commands and flags:

- `astroclaw security audit [--deep] [--fix] [--json]`
- `astroclaw status` / `astroclaw status --deep`
- `astroclaw health --json`
- `astroclaw update status`
- `astroclaw cron add|list|runs|run`

Do not invent CLI flags or imply Astroclaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Astroclaw compaction:
