...
export type AstroclawVitestPool = "forks" | "threads";
...
export function resolveLocalVitestMaxWorkers(
  ...
  pool: AstroclawVitestPool = resolveDefaultVitestPool(env),
): number {
...
function hasWorkerOverride(env: Record<string, string | undefined>): boolean {
  return Boolean((env.ASTROCLAW_VITEST_MAX_WORKERS ?? env.ASTROCLAW_TEST_WORKERS)?.trim());
}
