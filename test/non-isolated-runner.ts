...
function getSharedTestHome(): string | undefined {
  ...
  return globalState[SHARED_TEST_SETUP]?.tempHome ?? process.env.ASTROCLAW_TEST_HOME;
}
...
function restoreSharedTestHomeAfterEnvUnstub(testHomeRaw: string | undefined): void {
  ...
  process.env.ASTROCLAW_TEST_HOME = testHome;
...
export default class AstroclawNonIsolatedRunner extends TestRunner {
  override onCollectStart(file: { filepath: string }) {
    super.onCollectStart(file);
    restoreSharedTestHomeAfterEnvUnstub(getSharedTestHome());
    const orderLogPath = process.env.ASTROCLAW_VITEST_FILE_ORDER_LOG?.trim();
    if (orderLogPath) {
