...
function createTempHome(): string {
  return makeTempDir(tempDirs, "astroclaw-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-astroclaw.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.astroclaw/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.astroclaw/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".astroclaw", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".astroclaw", "external-plugins", "glueclaw", "astroclaw.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".astroclaw", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.ASTROCLAW_LIVE_TEST_QUIET = "1";
    process.env.ASTROCLAW_CONFIG_PATH = "~/custom-astroclaw.json5";
    process.env.ASTROCLAW_TEST_HOME = priorIsolatedHome;
    process.env.ASTROCLAW_STATE_DIR = path.join(priorIsolatedHome, ".astroclaw");
    ...
    expect(process.env.ASTROCLAW_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".astroclaw", "astroclaw.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".astroclaw", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".astroclaw", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.ASTROCLAW_LIVE_TEST = "1";
    process.env.ASTROCLAW_LIVE_USE_REAL_HOME = "1";
    process.env.ASTROCLAW_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.ASTROCLAW_LIVE_TEST;
    delete process.env.ASTROCLAW_LIVE_GATEWAY;
    delete process.env.ASTROCLAW_LIVE_USE_REAL_HOME;
    delete process.env.ASTROCLAW_LIVE_TEST_QUIET;
