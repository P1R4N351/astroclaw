...
import { createAstroclawCodingTools } from "../../../src/plugin-sdk/agent-harness.js";
...
const WORKSPACE_DIR = "/tmp/astroclaw-happy-path/workspace";
const AGENT_DIR = "/tmp/astroclaw-happy-path/agent";
const SESSION_FILE = "/tmp/astroclaw-happy-path/session.jsonl";
...
const dynamicToolsConfig: AstroclawConfig = {
  ...baseConfig,
  plugins: {
    enabled: true,
    slots: {
...
function createScenarios(): PromptScenario[] {
  ...
  const discordGroupCtx: TemplateContext = {
    ...
    SenderName: "Pash",
    SenderUsername: "pash",
    GroupSubject: "Astroclaw maintainers",
    GroupChannel: "#agent-sandbox",
    GroupSpace: "Astroclaw",
    ConversationLabel: "Astroclaw/#agent-sandbox",
    WasMentioned: true,
    InboundHistory: [
      {
        sender: "Peter",
      ...
      {
        sender: "Pash",
        body: "@Astroclaw please verify the Codex happy path too.",
      },
    ],
    Body: "@Astroclaw can you audit whether this prompt path has conflicting silence instructions?",
...
function renderModelBoundPromptLayers(params: {
  ...
  const codexConfigInstructions =
    ...
    typeof params.codexSnapshot.threadStartParams.config.instructions === "string"
      ...
      : "";
  const astroClawDeveloperInstructions = params.codexSnapshot.developerInstructions;
  ...
  return [
    ...
    markdownFence(
      ...
      stableJson({
        ...
        astroClawDeveloperInstructions: textStats(astroClawDeveloperInstructions),
    ...
    "",
    markdownFence("text", astroClawDeveloperInstructions),
    "",
...
function renderReadme(scenarios: PromptScenario[]): string {
  return [
    ...
    "",
    "The Markdown files show selected app-server thread/turn params plus a reconstructed model-bound prompt layer stack: Codex `gpt-5.5` model instructions from a pinned Codex model catalog fixture, Codex permission developer instructions for the happy-path yolo profile, simulated Astroclaw workspace bootstrap config instructions, Astroclaw developer instructions, user turn input, and references to the complete dynamic tool catalog.",
    "",
    "The workspace bootstrap simulation includes dummy `SOUL.md`, `TOOLS.md`, and `HEARTBEAT.md` contents so prompt reviewers can see how those Astroclaw project/user context files are forwarded to Codex. `AGENTS.md` is intentionally not repeated here because Codex loads it natively.",
    "",
    "The tool catalog is pinned to the canonical happy-path Astroclaw tools so optional locally installed plugin tools do not create fixture churn.",
    "",
    ...
    "",
    "These snapshots are still not a byte-for-byte raw OpenAI request capture. Codex-owned native `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions can be added inside the Codex runtime after Astroclaw sends thread and turn params.",
    "",
    "Regenerate with:",
    "",
