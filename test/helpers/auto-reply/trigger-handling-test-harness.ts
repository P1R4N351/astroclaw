...
const piEmbeddedMocks = getSharedMocks("astroclaw.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("astroclaw.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("astroclaw.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("astroclaw.trigger-handling.web-session-mocks", () => ({
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.ASTROCLAW_STATE_DIR = join(home, ".astroclaw");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "astroclaw-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".astroclaw", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): AstroclawConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-6" },
        workspace: join(home, "astroclaw"),
