...
async function withPluginAssetFixture(run: (rootDir: string) => Promise<void>) {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-plugin-assets-"));
  try {
    ...
    fs.writeFileSync(
      path.join(rootDir, "extensions", "canvas", "astroclaw.plugin.json"),
  ...
  it("discovers plugin-owned asset scripts by manifest id", async () => {
    await withPluginAssetFixture(async (rootDir) => {
      ...
      expect(hooks).toEqual([
        {
          aliases: ["@astroclaw/canvas-plugin", "canvas", "canvas-plugin"],
          command: "node scripts/bundle-a2ui.mjs",
          packageName: "@astroclaw/canvas-plugin",
          phase: "build",
