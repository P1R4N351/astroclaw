...
describe("write-cli-startup-metadata", () => {
  ...
  it("writes startup metadata with populated root help text when dist falls back to source rendering", async () => {
    const tempRoot = createTempDir("astroclaw-startup-metadata-");
    ...
    await writeCliStartupMetadata({
      ...
      renderSourceRootHelpText: () => "Usage: astroclaw\n",
      renderSourceBrowserHelpText: () => "Usage: astroclaw browser\n",
    });
    ...
    expect(written.browserHelpText).toContain("astroclaw browser");
    expect(written.rootHelpText).toContain("Usage:");
    expect(written.rootHelpText).toContain("astroclaw");
  });
});
