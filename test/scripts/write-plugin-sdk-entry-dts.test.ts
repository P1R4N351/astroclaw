...
function createFixture() {
  ...
  write(
    ...
    JSON.stringify({
      compilerOptions: {
        ...
        types: [],
        paths: {
          "@astroclaw/llm-core": ["./src/shared.ts"],
          "@astroclaw/llm-core/contract": ["./contracts/current.ts"],
        },
      },
      include: ["src/**/*.ts"],
    }),
  ...
  for (const source of Object.values(qa.selected)) {
    write(
      source,
      [
        'export { Shared } from "@astroclaw/llm-core";',
        'export type { TransitiveAlias } from "@astroclaw/llm-core/contract";',
