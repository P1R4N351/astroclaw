...
function proofBody(evidence: string, overrides: Record<string, string> = {}) {
  const fields = {
    ...
    environment: "macOS 15.4, Node 24, local Astroclaw gateway with a redacted Discord token.",
    steps: "pnpm astroclaw gateway restart, then pnpm astroclaw gateway status",
    evidence,
...
describe("real-behavior-proof-policy", () => {
  it.each([
    ...
    "Linked artifact: https://github.com/astroclaw/astroclaw/actions/runs/123456789/artifacts/987654321",
    "Redacted runtime log: gateway connected Discord channel and delivered the reply.",
    ["Terminal transcript:", "```text", "$ astroclaw gateway status", "discord ready", "```"].join(
      "\n",
    ),
