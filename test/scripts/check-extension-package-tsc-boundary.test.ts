...
describe("check-extension-package-tsc-boundary", () => {
  it("reruns the real compiler after an inherited paths change in the CLI", () => {
    ...
    const config = {
      ...
      compilerOptions: {
        paths: { "astroclaw/plugin-sdk/*": ["../packages/plugin-sdk/dist/src/plugin-sdk/*.d.ts"] },
      },
    };
    ...
    write(
      ...
      'import type { DemoContract } from "astroclaw/plugin-sdk/core";\nexport const demo: DemoContract = { ok: true };\n',
    );
    ...
    config.compilerOptions.paths["astroclaw/plugin-sdk/*"] = ["../missing-sdk/*.d.ts"];
    write(pathsConfig, JSON.stringify(config));
    const changed = run();
