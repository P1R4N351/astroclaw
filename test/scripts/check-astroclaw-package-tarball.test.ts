...
const CHECK_SCRIPT = "scripts/check-astroclaw-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "astroclaw-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "astroclaw", version }));
    writeFileSync(
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><astroclaw-app></astroclaw-app>",
    ...
    const tarball = join(root, "astroclaw.tgz");
...
describe("check-astroclaw-package-tarball", () => {
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("Astroclaw package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
