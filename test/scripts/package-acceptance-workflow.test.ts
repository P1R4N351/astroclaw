...
const LIVE_E2E_WORKFLOW = ".github/workflows/astroclaw-live-and-e2e-checks-reusable.yml";
const NPM_TELEGRAM_WORKFLOW = ".github/workflows/npm-telegram-beta-e2e.yml";
const PACKAGE_JSON = "package.json";
const RELEASE_CHECKS_WORKFLOW = ".github/workflows/astroclaw-release-checks.yml";
const RELEASE_PUBLISH_WORKFLOW = ".github/workflows/astroclaw-release-publish.yml";
...
describe("package acceptance workflow", () => {
  it("resolves candidate package sources before reusing Docker E2E lanes", () => {
    ...
    expect(workflow).toContain("scripts/resolve-astroclaw-package-candidate.mjs");
    ...
    expect(workflow).toContain(
      "uses: ./.github/workflows/astroclaw-live-and-e2e-checks-reusable.yml",
    );
    expect(workflow).toContain(
  ...
  it("offers bounded product profiles and can run Telegram against the resolved artifact", () => {
    ...
    expect(workflow).toContain(
      "package_label: astroclaw@${{ needs.resolve_package.outputs.package_version }}",
    );
  ...
  it("requires pinned full release child workflows to run at the resolved target SHA", () => {
    ...
    expect(releaseChecksWorkflow).toContain(
      "package_spec: ${{ needs.resolve_target.outputs.package_acceptance_package_spec || needs.resolve_target.outputs.release_package_spec || 'astroclaw@beta' }}",
    );
  });
...
describe("package artifact reuse", () => {
  it("lets reusable Docker E2E consume an already resolved package artifact", () => {
    ...
    expect(workflow).toContain("ASTROCLAW_DOCKER_E2E_SELECTED_SHA:");
    expect(workflow).toContain(
      "ASTROCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC: ${{ inputs.published_upgrade_survivor_baseline }}",
    );
    expect(workflow).toContain(
      "ASTROCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ matrix.group.published_upgrade_survivor_baselines || inputs.published_upgrade_survivor_baselines }}",
    );
    expect(workflow).toContain(
      "ASTROCLAW_UPGRADE_SURVIVOR_SCENARIOS: ${{ inputs.published_upgrade_survivor_scenarios }}",
    );
    expect(workflow).toContain("Download current-run Astroclaw Docker E2E package");
    expect(workflow).toContain("Download previous-run Astroclaw Docker E2E package");
    ...
    expect(workflow).toContain("ASTROCLAW_DOCKER_E2E_REPO_ROOT:");
    ...
    expect(workflowStep(prepareDockerImage, "Plan Docker E2E images").env).toEqual({
      ...
      ASTROCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC: "${{ inputs.published_upgrade_survivor_baseline }}",
      ASTROCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS:
    ...
    expect(workflow).toContain(
      "ASTROCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ inputs.published_upgrade_survivor_baselines }}",
    );
    ...
    expect(scheduler).toContain('["ASTROCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS",');
    expect(scheduler).toContain('["ASTROCLAW_UPGRADE_SURVIVOR_SCENARIOS",');
    expect(packageJson).toContain("ASTROCLAW_UPGRADE_SURVIVOR_PUBLISHED_BASELINE=1");
    expect(packageJson).toContain("test:docker:update-restart-auth");
    expect(packageJson).toContain("ASTROCLAW_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE=auto-auth");
    expect(publishedUpgradeSurvivor).toContain("validate_baseline_package_spec");
    expect(publishedUpgradeSurvivor).toContain("ASTROCLAW_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE");
    ...
    expect(publishedUpgradeSurvivor).toContain(
      "env -u ASTROCLAW_GATEWAY_TOKEN -u ASTROCLAW_GATEWAY_PASSWORD astroclaw",
    );
    expect(publishedUpgradeSurvivor).toContain("phase prepare-update-restart-probe");
    expect(publishedUpgradeSurvivor).toContain("astroclaw@(alpha|beta|latest|");
    ...
    expect(publishedUpgradeSurvivor).toContain('"id": "opik-astroclaw"');
  ...
  it("bounds shared Docker image pulls so package acceptance cannot stall forever", () => {
    ...
    expect(pullHelper).toContain("ASTROCLAW_DOCKER_PULL_ATTEMPTS");
    expect(pullHelper).toContain("ASTROCLAW_DOCKER_PULL_TIMEOUT_SECONDS");
    expect(pullHelper).toContain('timeout_seconds="${ASTROCLAW_DOCKER_PULL_TIMEOUT_SECONDS:-180}"');
    expect(pullHelper).toContain(
      'retry_delay_seconds="${ASTROCLAW_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"',
    );
    expect(pullHelper).toContain(
  ...
  it("shards broad native live tests instead of one serial live-all job", () => {
    ...
    expect(workflow).toContain("ASTROCLAW_LIVE_COMMAND: ${{ matrix.command }}");
    ...
    expect(workflow).toContain("ASTROCLAW_LIVE_GATEWAY_PROVIDERS=deepseek,fireworks");
    expect(workflow).toContain("ASTROCLAW_LIVE_GATEWAY_PROVIDERS=opencode-go,openrouter");
    expect(workflow).toContain("ASTROCLAW_LIVE_GATEWAY_PROVIDERS=xai,zai");
    expect(workflow).toContain("inputs.live_suite_filter == 'live-gateway-advisory-docker'");
    expect(workflow).toContain("ASTROCLAW_LIVE_CLI_BACKEND_MODEL=claude-cli/claude-sonnet-4-6");
    expect(workflow).toContain("ASTROCLAW_LIVE_CLI_BACKEND_AUTH=api-key");
    expect(workflow).not.toContain("ASTROCLAW_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG=1");
    expect(workflow).not.toContain('service_tier=\\"fast\\"');
    expect(workflow).not.toContain("ASTROCLAW_LIVE_CLI_BACKEND_ARGS=");
    expect(workflow).not.toContain("ASTROCLAW_LIVE_CLI_BACKEND_RESUME_ARGS=");
    expect(workflow).not.toContain(
      'ASTROCLAW_LIVE_CLI_BACKEND_ARGS=["exec","--json","--color","never","--sandbox","danger-full-access","--skip-git-repo-check"]',
    );
    ...
    expect(workflow).toContain(
      "command: ASTROCLAW_LIVE_APNS_REACHABILITY=1 node .release-harness/scripts/test-live-shard.mjs native-live-src-infra",
    );
    ...
    expect(workflow).not.toContain(
      "ASTROCLAW_LIVE_GATEWAY_PROVIDERS=deepseek,opencode-go,openrouter,xai,zai",
    );
    expect(workflow).toContain("suite_id: live-gateway-anthropic-docker");
    expect(workflow).toContain("ASTROCLAW_LIVE_GATEWAY_MAX_MODELS=2");
    ...
    expect(workflow).toContain("ASTROCLAW_LIVE_SUITE_ADVISORY: ${{ matrix.advisory }}");
    ...
    expect(workflow).toMatch(
      /validate_live_media_provider_suites:[\s\S]*?ASTROCLAW_LIVE_SUITE_ADVISORY: \$\{\{ matrix\.advisory \}\}/u,
    );
    expect(workflow).toMatch(
    ...
    expect(workflow).toContain("image: ghcr.io/astroclaw/astroclaw-live-media-runner:ubuntu-24.04");
    ...
    expect(workflow).toContain("ASTROCLAW_LIVE_VIDEO_GENERATION_PROVIDERS=google,minimax");
    expect(workflow).toContain("ASTROCLAW_LIVE_VIDEO_GENERATION_PROVIDERS=openai,openrouter,xai");
    ...
    expect(retryHelper).toContain("ASTROCLAW_LIVE_COMMAND_ATTEMPTS:-2");
  ...
  it("runs Docker live harnesses from trusted helper scripts", () => {
    ...
    expect(workflow).toContain(
      'run: ASTROCLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-models-docker.sh',
    );
    expect(workflow).toContain(
      "command: ASTROCLAW_LIVE_GATEWAY_PROVIDERS=openai ASTROCLAW_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toContain(
      'command: ASTROCLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-cli-backend-docker.sh',
    );
    expect(workflow).toContain(
      'command: ASTROCLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-acp-bind-docker.sh',
    );
    expect(workflow).toContain(
      'command: ASTROCLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-codex-harness-docker.sh',
    );
    expect(workflow).toContain(
      'command: ASTROCLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 20m bash .release-harness/scripts/test-live-subagent-announce-docker.sh',
    );
    ...
    expect(build).toContain('ROOT_DIR="${ASTROCLAW_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"');
    expect(build).toContain('source "$SCRIPT_ROOT_DIR/scripts/lib/docker-build.sh"');
    expect(stage).toContain(
      'local scripts_dir="${ASTROCLAW_LIVE_DOCKER_SCRIPTS_DIR:-/src/scripts}"',
    );
  ...
  it("allows the Telegram lane to run from reusable package acceptance artifacts", () => {
    ...
    expect(workflow).toContain("ASTROCLAW_NPM_TELEGRAM_PACKAGE_TGZ");
  ...
  it("includes package acceptance in release checks", () => {
    ...
    expect(workflow).toContain(
      "ASTROCLAW_QA_CONVEX_SITE_URL: ${{ secrets.ASTROCLAW_QA_CONVEX_SITE_URL }}",
    );
    expect(workflow).toContain(
      "ASTROCLAW_QA_CONVEX_SECRET_CI: ${{ secrets.ASTROCLAW_QA_CONVEX_SECRET_CI }}",
    );
  ...
  it("detects Matrix fail-fast support for older release refs", () => {
    ...
    expect(releaseWorkflow).toContain(
      'pnpm astroclaw qa matrix --help 2>/dev/null | grep -F -q -- "--fail-fast"',
    );
    expect(releaseWorkflow).toContain("matrix_args+=(--fail-fast)");
    expect(releaseWorkflow).toContain(
      'pnpm astroclaw qa matrix --output-dir "${attempt_output_dir}" "${matrix_args[@]}"',
    );
    expect(releaseWorkflow).toContain(
  ...
  it("runs live transport lanes nightly while release checks stay gated", () => {
    ...
    for (const channel of ["DISCORD", "WHATSAPP", "SLACK"]) {
      ...
      expect(releaseWorkflow).toContain(
        `RELEASE_QA_${channel}_LIVE_CI_ENABLED: \${{ vars.ASTROCLAW_RELEASE_QA_${channel}_LIVE_CI_ENABLED || 'false' }}`,
      );
      expect(releaseWorkflow).toContain(`qa_live_${lower}_enabled="$qa_live_${lower}_ci_enabled"`);
      expect(releaseWorkflow).toContain(
        `vars.ASTROCLAW_RELEASE_QA_${channel}_LIVE_CI_ENABLED == 'true'`,
      );
      expect(qaWorkflow).not.toContain(`ASTROCLAW_QA_${channel}_LIVE_CI_ENABLED`);
    }
  });
  ...
  it("runs full release children from the trusted workflow ref", () => {
    ...
    expectTextToIncludeAll(
      ...
      [
        "scripts/resolve-astroclaw-package-candidate.mjs",
        "--source ref",
    ...
    expectTextToIncludeAll(dispatchStep.run, [
      ...
      'args=(-f package_spec="${PACKAGE_SPEC:-astroclaw@beta}"',
  ...
  it("lets npm Telegram consume current-run or release-run package artifacts", () => {
    ...
    expectTextToIncludeAll(runStep.run, [
      'export ASTROCLAW_NPM_TELEGRAM_PACKAGE_TGZ="${package_tgzs[0]}"',
    ]);
  });
  ...
  it("keeps release publish creation compatible with gh api and prerelease notes", () => {
    ...
    const npmWorkflow = readFileSync(".github/workflows/astroclaw-npm-release.yml", "utf8");

    expect(workflow).toContain("timeout-minutes: 60");
    expect(workflow).toContain("Download Astroclaw npm preflight manifest");
    expect(workflow).toContain("Validate Astroclaw npm preflight manifest");
  ...
  it("keeps beta release verification and ClawHub publish repair hooks wired", () => {
    ...
    expect(releaseWorkflow).toContain("Astroclaw npm run ID");
  ...
  it("keeps release workflow setup and timeout budgets bounded", () => {
    ...
    const crossOs = readWorkflow(".github/workflows/astroclaw-cross-os-release-checks-reusable.yml");
    ...
    const releaseWorkflowPaths = [
      ...
      ".github/workflows/astroclaw-release-publish.yml",
      ".github/workflows/astroclaw-npm-release.yml",
