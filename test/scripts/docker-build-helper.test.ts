...
describe("docker build helper", () => {
  it("forces BuildKit for centralized Docker builds", () => {
    ...
    expect(helper).toContain("ASTROCLAW_DOCKER_BUILD_RETRIES");
  ...
  it("lets Testbox fall back to building when a reused Docker image is missing", () => {
    ...
    expect(helper).toContain("ASTROCLAW_DOCKER_BUILD_ON_MISSING");
    expect(helper).toContain("ASTROCLAW_TESTBOX");
    ...
    expect(liveCliBackend).not.toContain(
      'echo "==> Reuse live-test image: $LIVE_IMAGE_NAME (ASTROCLAW_SKIP_DOCKER_BUILD=1)"',
    );
  });
  ...
  it("preserves pnpm lookup paths for scheduled Docker child lanes", () => {
    ...
    expect(scheduler).toContain("ASTROCLAW_DOCKER_ALL_PNPM_COMMAND");
  });
  ...
  it("runs release installer E2E against the npm beta tag", () => {
    ...
    expect(openWebUiRunner).toContain(
      'SMOKE_MODE="${OPENWEBUI_SMOKE_MODE:-${ASTROCLAW_OPENWEBUI_SMOKE_MODE:-chat}}"',
    );
  ...
  it("times and parallelizes release installer E2E agent turns after gateway startup", () => {
    ...
    expect(runner).toContain(
      'AGENT_TURNS_PARALLEL="${ASTROCLAW_INSTALL_E2E_AGENT_TURNS_PARALLEL:-1}"',
    );
    ...
    expect(wrapper).toContain("ASTROCLAW_INSTALL_E2E_AGENT_TURNS_PARALLEL");
    expect(wrapper).toContain("ASTROCLAW_INSTALL_E2E_AGENT_TOOL_SMOKE");
    expect(wrapper).toContain("ASTROCLAW_INSTALL_E2E_OPENAI_MODEL");
    expect(wrapper).toContain("ASTROCLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS");
    expect(wrapper).toContain("ASTROCLAW_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS:-300");
    expect(runner).toContain("ASTROCLAW_INSTALL_E2E_OPENAI_MODEL");
    expect(runner).toContain("ASTROCLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS");
    expect(runner).toContain(
      'AGENT_TURN_TIMEOUT_SECONDS="${ASTROCLAW_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS:-300}"',
    );
  });
  ...
  it("keeps package acceptance plugin coverage offline-capable", () => {
    ...
    expect(scenarios).toContain("ASTROCLAW_PLUGINS_E2E_CLAWHUB=0");
  });
  ...
  it("caps package acceptance legacy compatibility at 2026.4.25", () => {
    ...
    expect(scripts.join("\n")).toContain("ASTROCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT");
    expect(scripts.join("\n")).toContain(
  ...
  it("prepares pnpm workspace package fixtures without package dependencies", () => {
    const root = mkdtempSync(join(tmpdir(), "astroclaw-update-channel-fixture-"));
    try {
      mkdirSync(join(root, "patches"));
      writeFileSync(
        join(root, "package.json"),
        `${JSON.stringify({ name: "astroclaw", version: "2026.5.6", scripts: {} }, null, 2)}\n`,
        "utf8",
      );
      writeFileSync(
  ...
  it("keeps bundled plugin install/uninstall sweep chunkable", () => {
    ...
    expect(runner).toContain("ASTROCLAW_BUNDLED_PLUGIN_SWEEP_TOTAL");
    expect(runner).toContain("ASTROCLAW_BUNDLED_PLUGIN_SWEEP_INDEX");
    expect(runner).toContain("ASTROCLAW_BUNDLED_PLUGIN_RUNTIME_READY_MS");
    expect(runner).toContain("scripts/e2e/lib/bundled-plugin-install-uninstall/sweep.sh");
    expect(probe).toContain('"astroclaw.plugin.json"');
    expect(runtimeSmoke).toContain("process.env.ASTROCLAW_BUNDLED_PLUGIN_RUNTIME_READY_MS");
    expect(runtimeSmoke).toContain("900000");
    expect(sweep).toContain("read -r plugin_id plugin_dir requires_config");
    expect(sweep).toContain('node "$ASTROCLAW_ENTRY" plugins install "$plugin_id"');
    expect(sweep).toContain('node "$ASTROCLAW_ENTRY" plugins uninstall "$plugin_id" --force');
  ...
  it("passes installer tag env to bash, not curl", () => {
    ...
    expect(runner).toContain('curl -fsSL "$INSTALL_URL" | ASTROCLAW_BETA=1 bash');
    expect(runner).toContain('curl -fsSL "$INSTALL_URL" | ASTROCLAW_VERSION="$INSTALL_TAG" bash');
    expect(runner).not.toContain('ASTROCLAW_BETA=1 curl -fsSL "$INSTALL_URL" | bash');
    expect(runner).not.toContain(
      'ASTROCLAW_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash',
    );
  });
  ...
  it("keeps ClawHub plugin Docker smoke hermetic by default", () => {
    ...
    expect(runner).toContain("ASTROCLAW_PLUGINS_E2E_LIVE_CLAWHUB");
    expect(sweep).toContain("scripts/e2e/lib/plugins/clawhub.sh");
    expect(clawhub).toContain("start_clawhub_fixture_server()");
    expect(clawhub).toContain('ASTROCLAW_CLAWHUB_URL="http://127.0.0.1:');
    expect(clawhub).toContain("ASTROCLAW_PLUGINS_E2E_LIVE_CLAWHUB");
    expect(clawhub).toContain("ASTROCLAW_PLUGINS_E2E_LIVE_NPM_REGISTRY");
    expect(clawhub).toContain("live ClawHub can rate-limit CI");
    expect(clawhub).toContain('[[ -n "${ASTROCLAW_CLAWHUB_URL:-}" || -n "${CLAWHUB_URL:-}" ]]');
    expect(clawhub).toContain("Ignoring ambient ClawHub URL for fixture-mode plugin E2E");
    expect(clawhub).toContain("unset ASTROCLAW_CLAWHUB_URL CLAWHUB_URL");
  });
  ...
  it("covers plugin install/update sources in the Docker plugin sweep", () => {
    ...
    expect(sweep).toContain('plugins install "npm:@astroclaw/demo-plugin-npm@0.0.1"');
    ...
    expect(clawhub).toContain("clawhub:@astroclaw/kitchen-sink");
