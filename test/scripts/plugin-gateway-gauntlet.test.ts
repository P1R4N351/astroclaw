...
describe("plugin gateway gauntlet helpers", () => {
  ...
  it("discovers bundled plugin manifests into lifecycle matrix rows", async () => {
    ...
    expect(matrix[0]).toEqual({
      ...
      id: "alpha",
      manifestPath: path.join("extensions", "alpha", "astroclaw.plugin.json"),
      name: "alpha",
  ...
  it("skips source-only plugin dirs that are excluded from the built runtime", async () => {
    await writeManifest("qqbot", "astroclaw.plugin.json", JSON.stringify({ id: "qqbot" }));
    await writeManifest("telegram", "astroclaw.plugin.json", JSON.stringify({ id: "telegram" }));
