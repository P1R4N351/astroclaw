...
describe("scripts/test-extension.mjs", () => {
  ...
  it("can fail safe to all extensions when the base revision is unavailable", () => {
    const extensionIds = listChangedExtensionIds({
      base: "refs/heads/astroclaw-test-missing-base",
  ...
  it("runs extension batch config groups concurrently when requested", async () => {
    ...
    const runPromise = runExtensionBatchPlan(
      ...
      {
        env: { ASTROCLAW_EXTENSION_BATCH_PARALLEL: "2" },
        runGroup,
    ...
    expect(firstRunGroupParams).toEqual({
      ...
      config: "heavy",
      env: {
        ASTROCLAW_EXTENSION_BATCH_PARALLEL: "2",
  ...
  it("keeps extension batch parallelism bounded by group count", () => {
    expect(resolveExtensionBatchParallelism(3, { ASTROCLAW_EXTENSION_BATCH_PARALLEL: "2" })).toBe(2);
    expect(resolveExtensionBatchParallelism(1, { ASTROCLAW_EXTENSION_BATCH_PARALLEL: "4" })).toBe(1);
    expect(resolveExtensionBatchParallelism(3, { ASTROCLAW_EXTENSION_BATCH_PARALLEL: "nope" })).toBe(
      1,
    );
  });
