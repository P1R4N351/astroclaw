...
describe("install.sh", () => {
  ...
  it("loads nvm before checking Node.js so stale system Node does not win", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "astroclaw-install-nvm-"));
  ...
  it("promotes a supported Linux Node binary over stale PATH entries", () => {
    const tmp = mkdtempSync(join(tmpdir(), "astroclaw-install-node-promote-"));
  ...
  it("persists a supported Linux Node path before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "astroclaw-install-linux-node-path-"));
    const home = join(tmp, "home");
  ...
  it("warns before redirecting an unwritable npm prefix", () => {
    const tmp = mkdtempSync(join(tmpdir(), "astroclaw-install-npm-prefix-"));
    const home = join(tmp, "home");
    ...
    expect(result?.stdout).toContain("npm i -g astroclaw@latest");
  ...
  it("persists npm prefix PATH before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "astroclaw-install-npm-prefix-shell-"));
    const home = join(tmp, "home");
    mkdirSync(home, { recursive: true });
    writeFileSync(
      join(home, ".bashrc"),
  ...
  it("uses a quoted absolute astroclaw path in follow-up commands when npm bin is not on the original PATH", () => {
    const tmp = mkdtempSync(join(tmpdir(), "astroclaw-install-command-"));
    const npmBin = join(tmp, "npm bin");
    const visibleBin = join(tmp, "visible-bin");
    mkdirSync(npmBin, { recursive: true });
    mkdirSync(visibleBin, { recursive: true });
    const astroclawBin = join(npmBin, "astroclaw");
    writeFileSync(astroclawBin, "#!/bin/sh\nexit 0\n");
    chmodSync(astroclawBin, 0o755);
    ...
    try {
      result = runInstallShell(`
        ...
        printf 'missing=%s\\n' "$(astroclaw_command_for_user "${astroclawBin}")"
    ...
    expect(result?.status).toBe(0);
    expect(result?.stdout).toContain(`missing=${astroclawBin.replace(/ /g, "\\ ")}`);
    expect(result?.stdout).toContain("present=astroclaw");
  });
...
describe("install.sh macOS Homebrew Node behavior", () => {
  ...
  it("reruns spinner-wrapped commands when gum reports ioctl failure", () => {
    const dir = mkdtempSync(join(tmpdir(), "astroclaw-install-sh-gum-"));
    try {
...
describe("install.sh duplicate Astroclaw install detection", () => {
  it("warns with concrete package paths and versions for duplicate npm roots", () => {
    const result = runInstallShell(`
      ...
      mkdir -p "$root/brew/astroclaw" "$root/fnm/astroclaw"
      printf '{"version":"2026.3.7"}\\n' > "$root/brew/astroclaw/package.json"
      printf '{"version":"2026.3.1"}\\n' > "$root/fnm/astroclaw/package.json"
      collect_astroclaw_npm_root_candidates() { printf '%s\\n' "$root/brew" "$root/fnm"; }
      ASTROCLAW_BIN="$root/fnm/.bin/astroclaw"
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_astroclaw_global_installs
    `);

    expect(result.status).toBe(0);
    expect(result.stdout).toContain("Multiple Astroclaw global installs detected");
    expect(result.stdout).toContain("2026.3.7");
    expect(result.stdout).toContain("2026.3.1");
    expect(result.stdout).toContain("/brew/astroclaw");
    expect(result.stdout).toContain("/fnm/astroclaw");
    expect(result.stdout).toContain("Active astroclaw:");
    expect(result.stdout).toContain("npm uninstall -g astroclaw");
  });

  it("stays quiet when only one Astroclaw npm root exists", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      root="$(mktemp -d)"
      trap 'rm -rf "$root"' EXIT
      mkdir -p "$root/only/astroclaw"
      printf '{"version":"2026.3.7"}\\n' > "$root/only/astroclaw/package.json"
      collect_astroclaw_npm_root_candidates() { printf '%s\\n' "$root/only"; }
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_astroclaw_global_installs
    `);
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).not.toContain("Multiple Astroclaw global installs detected");
  });
});
