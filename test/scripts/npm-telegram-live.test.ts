...
describe("package Telegram live Docker E2E", () => {
  it("supports npm-specific Convex credential aliases", () => {
    ...
    expect(script).toContain("ASTROCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE");
    expect(script).toContain("ASTROCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE");
    expect(script).toContain('docker_env+=(-e ASTROCLAW_QA_CREDENTIAL_SOURCE="$credential_source")');
    expect(script).toContain('docker_env+=(-e ASTROCLAW_QA_CREDENTIAL_ROLE="$credential_role")');
  });
  ...
  it("defaults CI runs to Convex when broker credentials are present", () => {
    ...
    expect(script).toContain("ASTROCLAW_QA_CONVEX_SECRET_CI");
    expect(script).toContain("ASTROCLAW_QA_CONVEX_SECRET_MAINTAINER");
  ...
  it("can install a resolved package tarball instead of a registry spec", () => {
    ...
    expect(script).toContain("ASTROCLAW_NPM_TELEGRAM_PACKAGE_TGZ");
    expect(script).toContain("ASTROCLAW_CURRENT_PACKAGE_TGZ");
    expect(script).toContain(
      'package_mount_args=(-v "$resolved_package_tgz:$package_install_source:ro")',
    );
    expect(script).toContain('validate_astroclaw_package_spec "$PACKAGE_SPEC"');
    expect(script.indexOf('if [ -n "$resolved_package_tgz" ]; then')).toBeLessThan(
      script.indexOf('validate_astroclaw_package_spec "$PACKAGE_SPEC"'),
    );
  });
  ...
  it("keeps private QA harness imports local while using the installed package dist", () => {
    ...
    expect(script).toContain('ln -sfnT "$astroclaw_package_dir/dist" /app/dist');
    expect(script).toContain('cp "$astroclaw_package_dir/package.json" /app/package.json');
    expect(script).toContain('-v "$ROOT_DIR/extensions/qa-lab:/app/extensions/qa-lab:ro"');
    expect(script).not.toContain('ln -sfnT /app/extensions "$astroclaw_package_dir/extensions"');
    expect(script).toContain("node scripts/e2e/lib/npm-telegram-live/prepare-package.mjs");
    expect(script).toContain("/app/node_modules/astroclaw/package.json");
    ...
    expect(gatewayRpcClient).toContain('from "astroclaw/plugin-sdk/gateway-runtime"');
    expect(qaRuntimeApi).toContain('from "astroclaw/plugin-sdk/gateway-runtime"');
  });
  ...
  it("lets npm-specific credential aliases override shared QA env", () => {
    expect(
      __testing.resolveCredentialSource({
        ASTROCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE: "convex",
        ASTROCLAW_QA_CREDENTIAL_SOURCE: "env",
      }),
    ).toBe("convex");
    expect(
      __testing.resolveCredentialRole({
        ASTROCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE: "ci",
