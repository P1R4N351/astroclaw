    ...
    expect(
      findPluginSdkWildcardReexports(
        [
          ...
          'export * as sdk from "astroclaw/plugin-sdk/foo";',
          'export type * from "astroclaw/plugin-sdk/bar";',
          'export type * as sdkTypes from "astroclaw/plugin-sdk/bar";',
          'export { named } from "astroclaw/plugin-sdk/foo";',
        ].join("\n"),
      ),
    ).toEqual([
      { line: 1, text: 'export * from "astroclaw/plugin-sdk/foo";' },
      { line: 2, text: 'export * as sdk from "astroclaw/plugin-sdk/foo";' },
      { line: 3, text: 'export type * from "astroclaw/plugin-sdk/bar";' },
      { line: 4, text: 'export type * as sdkTypes from "astroclaw/plugin-sdk/bar";' },
    ]);
  });
    ...
    expect(JSON.parse(result.stdout)).toEqual([
      {
        ...
        line: 1,
        text: 'export * from "astroclaw/plugin-sdk/foo";',
      },
    ]);
  });
});
