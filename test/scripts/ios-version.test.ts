...
describe("renderIosVersionXcconfig", () => {
  it("renders checked-in defaults from the pinned iOS version", () => {
    ...
    expect(renderIosVersionXcconfig(version)).toContain("ASTROCLAW_IOS_VERSION = 2026.4.8");
    expect(renderIosVersionXcconfig(version)).toContain("ASTROCLAW_MARKETING_VERSION = 2026.4.8");
    expect(renderIosVersionXcconfig(version)).toContain("ASTROCLAW_BUILD_VERSION = 1");
  });
});
