...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.ASTROCLAW_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "ASTROCLAW_TEST_FAST", value: process.env.ASTROCLAW_TEST_FAST },
    {
      key: "ASTROCLAW_STRICT_FAST_REPLY_CONFIG",
      value: process.env.ASTROCLAW_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "ASTROCLAW_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.ASTROCLAW_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "ASTROCLAW_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.ASTROCLAW_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "ASTROCLAW_STATE_DIR", value: process.env.ASTROCLAW_STATE_DIR },
    { key: "ASTROCLAW_CONFIG_PATH", value: process.env.ASTROCLAW_CONFIG_PATH },
    { key: "ASTROCLAW_GATEWAY_PORT", value: process.env.ASTROCLAW_GATEWAY_PORT },
    { key: "ASTROCLAW_BRIDGE_ENABLED", value: process.env.ASTROCLAW_BRIDGE_ENABLED },
    { key: "ASTROCLAW_BRIDGE_HOST", value: process.env.ASTROCLAW_BRIDGE_HOST },
    { key: "ASTROCLAW_BRIDGE_PORT", value: process.env.ASTROCLAW_BRIDGE_PORT },
    { key: "ASTROCLAW_CANVAS_HOST_PORT", value: process.env.ASTROCLAW_CANVAS_HOST_PORT },
    { key: "ASTROCLAW_TEST_HOME", value: process.env.ASTROCLAW_TEST_HOME },
    { key: "ASTROCLAW_AGENT_DIR", value: process.env.ASTROCLAW_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.ASTROCLAW_TEST_HOME = tempHome;
  process.env.ASTROCLAW_TEST_FAST = "1";
  process.env.ASTROCLAW_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.ASTROCLAW_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.ASTROCLAW_BRIDGE_ENABLED;
  delete process.env.ASTROCLAW_BRIDGE_HOST;
  delete process.env.ASTROCLAW_BRIDGE_PORT;
  delete process.env.ASTROCLAW_CANVAS_HOST_PORT;
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.ASTROCLAW_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.ASTROCLAW_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".astroclaw");
  const priorIsolatedHome = params.env.ASTROCLAW_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".astroclaw")
  ) {
    realStateDir = path.join(params.realHome, ".astroclaw");
  }
  const tempStateDir = path.join(params.tempHome, ".astroclaw");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.ASTROCLAW_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.ASTROCLAW_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "astroclaw.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "astroclaw.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    ...
    process.env.ASTROCLAW_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.ASTROCLAW_LIVE_USE_REAL_HOME);
