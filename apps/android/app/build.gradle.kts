...
val androidStoreFile = providers.gradleProperty("ASTROCLAW_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("ASTROCLAW_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("ASTROCLAW_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("ASTROCLAW_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
  error(
    "Missing Android release signing properties. Set ASTROCLAW_ANDROID_STORE_FILE, " +
      "ASTROCLAW_ANDROID_STORE_PASSWORD, ASTROCLAW_ANDROID_KEY_ALIAS, and " +
      "ASTROCLAW_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
  )
}
...
android {
  ...
  sourceSets {
    getByName("main") {
      assets.directories.add("../../shared/AstroclawKit/Sources/AstroclawKit/Resources")
    }
  }

  defaultConfig {
    applicationId = "ai.astroclaw.app"
    minSdk = 31
    targetSdk = 36
  ...
  onVariants { variant ->
    variant.outputs
      ...
      .forEach { output ->
        ...
        val outputFileName =
          if (flavorName == null) {
            "astroclaw-$versionName-$buildType.apk"
          } else {
            "astroclaw-$versionName-$flavorName-$buildType.apk"
          }
