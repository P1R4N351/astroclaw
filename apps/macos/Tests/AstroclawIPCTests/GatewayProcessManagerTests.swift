...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["ASTROCLAW_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "astroclaw-gateway",
                executablePath: "/tmp/astroclaw-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 astroclaw-gateway @ /tmp/astroclaw-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error
