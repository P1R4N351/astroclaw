...
struct GatewayDiscoveryModelTests {
    ...
    @Test func `local gateway matches display name`() {
        ...
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: "Peter's Mac Studio (Astroclaw)",
            serviceName: nil,
            local: local))
    }
    ...
    @Test func `service name does not false positive on substring host token`() {
        ...
        #expect(!GatewayDiscoveryModel.isLocalGateway(
            ...
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipetacstudio (Astroclaw)",
            local: local))
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipete (Astroclaw)",
            local: local))
    }
    ...
    @Test func `parses gateway TXT fields`() {
        ...
        #expect(parsed.cliPath == "/opt/astroclaw")
    }
