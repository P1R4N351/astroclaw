...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["astroclaw"]
        let text = "hey astroclaw"
    ...
    @Test func `trigger only rejects trailing wake word mentions in ordinary speech`() {
        let triggers = ["astroclaw"]
        let text = "tell me about astroclaw"
        #expect(!VoiceWakeRuntime._testIsTriggerOnly(text, triggers: triggers))
    }
    ...
    @Test func `matched trigger finds trigger not at transcript start`() {
        let triggers = ["astroclaw"]
        let text = "uh astroclaw"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "astroclaw")
    }
    ...
    @Test func `matched trigger prefers most specific overlapping phrase`() {
        let triggers = ["astroclaw", "hey astroclaw"]
        let text = "hey astroclaw"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "hey astroclaw")
    }
    ...
    @Test func `text only fallback populates matched trigger`() {
        let transcript = "hey astroclaw do thing"
        let config = WakeWordGateConfig(triggers: ["astroclaw"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["astroclaw"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "astroclaw")
    }

    @Test func `text only fallback keeps the first trigger phrase when later words match another trigger`() {
        let transcript = "astroclaw tell me about computer vision"
        let config = WakeWordGateConfig(triggers: ["astroclaw", "computer"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["astroclaw", "computer"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "astroclaw")
    }
    ...
    @Test func `gate requires gap between trigger and command`() {
        ...
        let config = WakeWordGateConfig(triggers: ["astroclaw"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "astroclaw", start: 0.2, duration: 0.1, range: transcript.range(of: "astroclaw")),
