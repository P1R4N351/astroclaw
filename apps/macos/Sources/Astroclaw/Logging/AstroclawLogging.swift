...
enum AstroclawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = AstroclawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = AstroclawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        AstroclawLogging.bootstrapIfNeeded()
        let label = AstroclawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct AstroclawOSLogHandler: AppLogLevelBackedHandler {
...
struct AstroclawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = AstroclawLogging.parseLabel(self.label)
