...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.astroclaw", category: "gateway-endpoint")
    ...
    {
        let raw = env["ASTROCLAW_GATEWAY_PASSWORD"] ?? ""
        ...
        if !trimmed.isEmpty {
               ...
               !configPassword.isEmpty
            {
                self.warnEnvOverrideOnce(
                    kind: .password,
                    envVar: "ASTROCLAW_GATEWAY_PASSWORD",
    ...
    private static func resolveGatewayToken(
        ...
        launchdSnapshot: LaunchAgentPlistSnapshot?) -> String?
    {
        let raw = env["ASTROCLAW_GATEWAY_TOKEN"] ?? ""
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
               ...
               configToken != trimmed
            {
                self.warnEnvOverrideOnce(
                    kind: .token,
                    envVar: "ASTROCLAW_GATEWAY_TOKEN",
    ...
    private let logger = Logger(subsystem: "ai.astroclaw", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "astroclaw.onboardingSeen")
        ...
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            ...
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: AstroclawConfigFile.loadDict())
    ...
    {
        if let envBind = env["ASTROCLAW_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["ASTROCLAW_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
