...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: AstroclawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.astroclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AstroclawKit.AnyCodable]()) {
                $0[$1.key] = AstroclawKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking Astroclaw Local Network access.
                    Allow Astroclaw in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AstroclawProtocol.AnyCodable?) -> [String: AstroclawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AstroclawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AstroclawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AstroclawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("astroclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("astroclaw.control.agent")
}
