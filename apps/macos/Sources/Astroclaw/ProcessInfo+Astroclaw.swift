...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.astroclaw.mac`) even if the shipped
    ...
    {
        if environment["ASTROCLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "astroclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "astroclaw.nixMode") { return true }
