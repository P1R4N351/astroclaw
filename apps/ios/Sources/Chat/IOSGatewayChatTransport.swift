...
struct IOSGatewayChatTransport: AstroclawChatTransport {
    private static let logger = Logger(subsystem: "ai.astroclaw", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> AstroclawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(AstroclawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> AstroclawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(AstroclawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [AstroclawChatAttachmentPayload]) async throws -> AstroclawChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [AstroclawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(AstroclawChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(AstroclawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AstroclawGatewayHealthOK.self))?.ok ?? true
