...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: AstroclawWatchNotifyParams) -> AstroclawWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [AstroclawWatchAction]?,
        kind: String?,
        promptId: String?) -> [AstroclawWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> AstroclawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AstroclawWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                AstroclawWatchAction(id: "approve", label: "Approve"),
                AstroclawWatchAction(id: "decline", label: "Decline", style: "destructive"),
                AstroclawWatchAction(id: "open_phone", label: "Open iPhone"),
                AstroclawWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            AstroclawWatchAction(id: "done", label: "Done"),
            AstroclawWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            AstroclawWatchAction(id: "open_phone", label: "Open iPhone"),
            AstroclawWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: AstroclawWatchRisk?,
        priority: AstroclawNotificationPriority?) -> AstroclawWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: AstroclawNotificationPriority?,
        risk: AstroclawWatchRisk?) -> AstroclawNotificationPriority?
    {
