...
struct OnboardingWizardView: View {
    ...
    @State private var statusLine: String = "In your Astroclaw chat, run /pair qr, then scan the code here."
    ...
    private var introStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Welcome to Astroclaw")
                .font(.largeTitle.weight(.bold))
                .multilineTextAlignment(.center)
                .padding(.bottom, 10)

            Text("Turn this iPhone into a secure Astroclaw node for chat, voice, camera, and device tools.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                Label("Use Astroclaw from your phone", systemImage: "message.fill")
            }
            ...
            HStack(alignment: .top, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Security notice")
                        .font(.headline)
                    Text(
                        "The connected Astroclaw agent can use device capabilities you enable, "
    ...
    private var welcomeStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Scan a QR code from your Astroclaw gateway or continue with manual setup.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .padding(.horizontal, 32)
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text("How to pair")
                    .font(.headline)
                Text("In your Astroclaw chat, run")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
                Text("/pair qr")
            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `astroclaw devices list`."
                    }()
                    let commandLine = self.currentProblem?.actionCommand ?? "astroclaw devices approve <requestId>"
                    Text(
                        "Approve this device on the gateway.\n"
                            + "1) `\(commandLine)`\n"
                            + "2) `/pair approve` in your Astroclaw chat\n"
                            + "\(requestLine)\n"
                            + "Astroclaw will also retry automatically when you return to this app.")
                }
            }
    ...
    private func advanceFromIntro() {
        ...
        self.statusLine = "In your Astroclaw chat, run /pair qr, then scan the code here."
        self.step = .welcome
    }
    ...
    private func initializeState() {
        if self.manualHost.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            if let last = GatewaySettingsStore.loadLastGatewayConnection() {
                ...
                case .discovered:
                    self.manualHost = "astroclaw.local"
        ...
        if self.selectedMode == .developerLocal, self.manualHost == "astroclaw.local" {
        ...
        if !hasSavedGateway, !hasToken, !hasPassword {
            self.statusLine = "No saved pairing found. In your Astroclaw chat, run /pair qr, then scan the code here."
        }
    }
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "astroclaw.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "astroclaw.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "astroclaw.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
