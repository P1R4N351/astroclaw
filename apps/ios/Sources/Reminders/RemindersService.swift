...
final class RemindersService: RemindersServicing {
    func list(params: AstroclawRemindersListParams) async throws -> AstroclawRemindersListPayload {
        ...
        let payload: [AstroclawReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return AstroclawReminderPayload(
        ...
        return AstroclawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: AstroclawRemindersAddParams) async throws -> AstroclawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = AstroclawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return AstroclawRemindersAddPayload(reminder: payload)
    }
