...
enum WatchMessagingPayloadCodec {
    ...
    static func encodeNotificationPayload(
        id: String,
        params: AstroclawWatchNotifyParams) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": AstroclawWatchPayloadType.notify.rawValue,
            "id": id,
            "title": params.title,
            "body": params.body,
            "priority": params.priority?.rawValue ?? AstroclawNotificationPriority.active.rawValue,
            "sentAtMs": self.nowMs(),
        ]
    ...
    static func encodeExecApprovalItem(_ item: AstroclawWatchExecApprovalItem) -> [String: Any] {
        var payload: [String: Any] = [
            "id": item.id,
    ...
    static func encodeExecApprovalPromptPayload(
        _ message: AstroclawWatchExecApprovalPromptMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": AstroclawWatchPayloadType.execApprovalPrompt.rawValue,
    ...
    static func encodeExecApprovalResolvedPayload(
        _ message: AstroclawWatchExecApprovalResolvedMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": AstroclawWatchPayloadType.execApprovalResolved.rawValue,
    ...
    static func encodeExecApprovalExpiredPayload(
        _ message: AstroclawWatchExecApprovalExpiredMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": AstroclawWatchPayloadType.execApprovalExpired.rawValue,
            "approvalId": message.approvalId,
    ...
    static func encodeExecApprovalSnapshotPayload(
        _ message: AstroclawWatchExecApprovalSnapshotMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": AstroclawWatchPayloadType.execApprovalSnapshot.rawValue,
    ...
    {
        guard (payload["type"] as? String) == AstroclawWatchPayloadType.reply.rawValue else {
            return nil
        }
    ...
    {
        guard (payload["type"] as? String) == AstroclawWatchPayloadType.execApprovalResolve.rawValue else {
            return nil
        }
        guard let approvalId = nonEmpty(payload["approvalId"] as? String),
              let rawDecision = nonEmpty(payload["decision"] as? String),
              let decision = AstroclawWatchExecApprovalDecision(rawValue: rawDecision)
        else {
            return nil
        }
        let replyId = self.nonEmpty(payload["replyId"] as? String) ?? UUID().uuidString
    ...
    {
        guard (payload["type"] as? String) == AstroclawWatchPayloadType.execApprovalSnapshotRequest.rawValue else {
            return nil
        }
