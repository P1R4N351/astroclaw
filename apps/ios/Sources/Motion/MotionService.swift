...
final class MotionService: MotionServicing {
    func activities(params: AstroclawMotionActivityParams) async throws -> AstroclawMotionActivityPayload {
        ...
        let mapped: [AstroclawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return AstroclawMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = AstroclawPedometerPayload(
