...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> AstroclawDeviceStatusPayload {
        ...
        return AstroclawDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> AstroclawDeviceInfoPayload {
        ...
        return AstroclawDeviceInfoPayload(
    ...
    private func batteryStatus() -> AstroclawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: AstroclawBatteryState = switch device.batteryState {
        ...
        return AstroclawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> AstroclawThermalStatusPayload {
        let state: AstroclawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return AstroclawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> AstroclawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return AstroclawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
