...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__ASTROCLAW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<AstroclawTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.astroclaw", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = AstroclawTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> AstroclawTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return AstroclawTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> AstroclawTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> AstroclawTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return AstroclawTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> AstroclawTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = AstroclawTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: AstroclawTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: AstroclawAgentEventPayload.self)
            else {
                continue
            }
