...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "astroclaw.exec-approval"
    static let reviewActionIdentifier = "astroclaw.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.astroClawPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.astroClawPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func astroClawPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
