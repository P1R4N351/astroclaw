...
final class ContactsService: ContactsServicing {
    ...
    func search(params: AstroclawContactsSearchParams) async throws -> AstroclawContactsSearchPayload {
        ...
        return AstroclawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: AstroclawContactsAddParams) async throws -> AstroclawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return AstroclawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return AstroclawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> AstroclawContactPayload {
        AstroclawContactPayload(
