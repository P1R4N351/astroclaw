...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.astroclaw.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Astroclaw", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Astroclaw gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Astroclaw.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "AstroclawShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Astroclaw is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Astroclaw Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("astroclaw-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
