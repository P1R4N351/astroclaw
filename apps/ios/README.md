# Astroclaw iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an Astroclaw Gateway as a `role: node`.
...
- Beta release uses canonical `ai.astroclaw.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `AstroclawPushTransport=relay`, `AstroclawPushDistribution=official`, and `AstroclawPushAPNsEnvironment=production`.
...
- `ASTROCLAW_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
```bash
export ASTROCLAW_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/Astroclaw-<version>.ipa`
   - `apps/ios/build/beta/Astroclaw-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/Astroclaw.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `AstroclawPushTransport=direct` and `AstroclawPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `ASTROCLAW_APNS_TEAM_ID`, `ASTROCLAW_APNS_KEY_ID`, and either `ASTROCLAW_APNS_PRIVATE_KEY_P8` or `ASTROCLAW_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.astroclaw/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `ASTROCLAW_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `AstroclawPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `astroclaw.json`. `ASTROCLAW_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Astroclaw official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as Astroclaw node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
