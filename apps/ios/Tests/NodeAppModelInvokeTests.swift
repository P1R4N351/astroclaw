...
private final class MockWatchMessagingService: @preconcurrency WatchMessagingServicing, @unchecked Sendable {
    ...
    var sendError: Error?
    var lastSent: (id: String, params: AstroclawWatchNotifyParams)?
    var lastSentExecApprovalPrompt: AstroclawWatchExecApprovalPromptMessage?
    var lastSentExecApprovalResolved: AstroclawWatchExecApprovalResolvedMessage?
    var lastSentExecApprovalExpired: AstroclawWatchExecApprovalExpiredMessage?
    var lastSentExecApprovalSnapshot: AstroclawWatchExecApprovalSnapshotMessage?
    ...
    func sendNotification(id: String, params: AstroclawWatchNotifyParams) async throws -> WatchNotificationSendResult {
...
@Suite(.serialized) struct NodeAppModelInvokeTests {
    @Test @MainActor func decodeParamsFailsWithoutJSON() {
        #expect(throws: Error.self) {
            _ = try NodeAppModel._test_decodeParams(AstroclawCanvasNavigateParams.self, from: nil)
        }
    }
    ...
    @Test @MainActor func handleInvokeRejectsBackgroundCommands() async {
        ...
        let req = BridgeInvokeRequest(id: "bg", command: AstroclawCanvasCommand.present.rawValue)
    ...
    @Test @MainActor func handleInvokeRejectsCameraWhenDisabled() async {
        let appModel = NodeAppModel()
        let req = BridgeInvokeRequest(id: "cam", command: AstroclawCameraCommand.snap.rawValue)
    ...
    @Test @MainActor func handleInvokeRejectsInvalidScreenFormat() async {
        let appModel = NodeAppModel()
        let params = AstroclawScreenRecordParams(format: "gif")
        let data = try? JSONEncoder().encode(params)
        let json = data.flatMap { String(data: $0, encoding: .utf8) }

        let req = BridgeInvokeRequest(
            id: "screen",
            command: AstroclawScreenCommand.record.rawValue,
            paramsJSON: json)
    ...
    @Test @MainActor func handleInvokeCanvasCommandsUpdateScreen() async throws {
        ...
        let present = BridgeInvokeRequest(id: "present", command: AstroclawCanvasCommand.present.rawValue)
        ...
        let navigateParams = AstroclawCanvasNavigateParams(url: "http://example.com/")
        let navData = try JSONEncoder().encode(navigateParams)
        let navJSON = String(decoding: navData, as: UTF8.self)
        let navigate = BridgeInvokeRequest(
            id: "nav",
            command: AstroclawCanvasCommand.navigate.rawValue,
            paramsJSON: navJSON)
        let navRes = await appModel._test_handleInvoke(navigate)
        #expect(navRes.ok == true)
        #expect(appModel.screen.urlString == "http://example.com/")

        let evalParams = AstroclawCanvasEvalParams(javaScript: "1+1")
        ...
        let eval = BridgeInvokeRequest(
            id: "eval",
            command: AstroclawCanvasCommand.evalJS.rawValue,
            paramsJSON: evalJSON)
    ...
    @Test @MainActor func handleInvokeA2UICommandsFailWhenHostMissing() async throws {
        ...
        let reset = BridgeInvokeRequest(id: "reset", command: AstroclawCanvasA2UICommand.reset.rawValue)
        ...
        let pushParams = AstroclawCanvasA2UIPushJSONLParams(jsonl: jsonl)
        ...
        let push = BridgeInvokeRequest(
            id: "push",
            command: AstroclawCanvasA2UICommand.pushJSONL.rawValue,
            paramsJSON: pushJSON)
    ...
    @Test @MainActor func handleInvokeWatchStatusReturnsServiceSnapshot() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let req = BridgeInvokeRequest(id: "watch-status", command: AstroclawWatchCommand.status.rawValue)

        let res = await appModel._test_handleInvoke(req)
        #expect(res.ok == true)

        let payloadData = try #require(res.payloadJSON?.data(using: .utf8))
        let payload = try JSONDecoder().decode(AstroclawWatchStatusPayload.self, from: payloadData)
    ...
    @Test @MainActor func handleInvokeWatchNotifyRoutesToWatchService() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = AstroclawWatchNotifyParams(
            title: "Astroclaw",
        ...
        let req = BridgeInvokeRequest(
            id: "watch-notify",
            command: AstroclawWatchCommand.notify.rawValue,
            paramsJSON: paramsJSON)

        let res = await appModel._test_handleInvoke(req)
        #expect(res.ok == true)
        #expect(watchService.lastSent?.params.title == "Astroclaw")
        ...
        let payloadData = try #require(res.payloadJSON?.data(using: .utf8))
        let payload = try JSONDecoder().decode(AstroclawWatchNotifyPayload.self, from: payloadData)
    ...
    @Test @MainActor func handleInvokeWatchNotifyRejectsEmptyMessage() async throws {
        let watchService = MockWatchMessagingService()
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = AstroclawWatchNotifyParams(title: "   ", body: "\n")
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleInvokeWatchNotifyDerivesPriorityFromRiskAndCapsActions() async throws {
        ...
        let params = AstroclawWatchNotifyParams(
            ...
            risk: .high,
            actions: [
                AstroclawWatchAction(id: "a1", label: "A1"),
                AstroclawWatchAction(id: "a2", label: "A2"),
                AstroclawWatchAction(id: "a3", label: "A3"),
                AstroclawWatchAction(id: "a4", label: "A4"),
                AstroclawWatchAction(id: "a5", label: "A5"),
            ])
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleInvokeWatchNotifyReturnsUnavailableOnDeliveryFailure() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = AstroclawWatchNotifyParams(title: "Astroclaw", body: "Delivery check")
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleDeepLinkSetsErrorWhenNotConnected() async {
        let appModel = NodeAppModel()
        let url = URL(string: "astroclaw://agent?message=hello")!
    ...
    @Test @MainActor func handleDeepLinkRejectsOversizedMessage() async {
        ...
        let url = URL(string: "astroclaw://agent?message=\(msg)")!
        await appModel.handleDeepLink(url: url)
