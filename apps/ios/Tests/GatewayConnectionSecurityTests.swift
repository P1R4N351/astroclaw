...
@Suite(.serialized) struct GatewayConnectionSecurityTests {
    ...
    {
        let endpoint: NWEndpoint = .service(name: "Test", type: "_astroclaw-gw._tcp", domain: "local.", interface: nil)
    ...
    @Test @MainActor func manualConnectionsAllowPrivateLanPlaintext() async {
        ...
        #expect(controller._test_resolveManualUseTLS(host: "astroclaw.local", useTLS: false) == false)
