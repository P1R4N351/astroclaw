...
ASC_KEYCHAIN_SERVICE=astroclaw-asc-key
...
```bash
ASC_APP_IDENTIFIER=ai.astroclaw.client
# or
...
Tip: run `scripts/ios-team-id.sh` from repo root to print a Team ID for `.env`. The helper prefers the canonical Astroclaw team (`Y5PE65HELJ`) when present locally; otherwise it prefers the first non-personal team from your Xcode account (then personal team if needed). Fastlane uses this helper automatically if `IOS_DEVELOPMENT_TEAM` is missing.

For local/manual iOS builds that stay on direct APNs, configure the gateway host separately with `ASTROCLAW_APNS_TEAM_ID`, `ASTROCLAW_APNS_KEY_ID`, and either `ASTROCLAW_APNS_PRIVATE_KEY_P8` or `ASTROCLAW_APNS_PRIVATE_KEY_PATH`. Those gateway runtime env vars are separate from Fastlane's `.env`.
...
- confirm `apps/ios/build/beta/Astroclaw-<version>.ipa` exists
...
- The beta flow regenerates `apps/ios/Astroclaw.xcodeproj` from `apps/ios/project.yml` before archiving
