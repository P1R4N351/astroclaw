...
describe("nextcloud-talk doctor", () => {
  ...
  it("warns when the configured bot is missing the response feature", async () => {
    hoisted.probeNextcloudTalkBotResponseFeature.mockResolvedValueOnce({
      ...
      message:
        'Nextcloud Talk bot "Astroclaw" (1) is missing the response feature (features=9); outbound replies will fail.',
    });
    ...
    await expect(
      ...
      '- channels.nextcloud-talk.default: Nextcloud Talk bot "Astroclaw" (1) is missing the response feature (features=9); outbound replies will fail.',
    ]);
  });
});
