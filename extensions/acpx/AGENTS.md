...
The ACPX extension is a thin Astroclaw wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `astroclaw/acpx`, not in this extension.
...
Use this flow when Astroclaw needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `astroclaw/acpx` repo first.
2. In Astroclaw, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Astroclaw and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Astroclaw ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Astroclaw-specific glue, move it to `astroclaw/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
