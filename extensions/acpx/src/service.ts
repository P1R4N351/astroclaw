  ...
  async function resolveRuntime(): Promise<AcpxRuntimeLike> {
    ...
    runtimePromise ??= loadRuntimeModule().then((module) => {
      runtime = new module.AcpxRuntime({
        ...
        astroclawGatewayInstanceId: params.gatewayInstanceId,
        astroclawProcessLeaseStore: params.processLeaseStore,
        astroclawWrapperRoot: params.wrapperRoot,
...
function resolveAllowedAgentsProbeAgent(ctx: AstroclawPluginServiceContext): string | undefined {
...
async function reapOpenAcpxProcessLeases(params: {
  ...
  for (const lease of leases) {
    if (lease.rootPid <= 0) {
      ...
      if (!result) {
        result = await reapStaleAstroclawOwnedAcpxOrphans({
...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: AstroclawPluginServiceContext): Promise<void> {
      ...
      if (startupReap.terminatedPids.length > 0) {
        ctx.logger.info(
          `reaped ${startupReap.terminatedPids.length} stale Astroclaw-owned ACPX process${startupReap.terminatedPids.length === 1 ? "" : "es"}`,
        );
      }
    ...
    async stop(_ctx: AstroclawPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
