...
const DOCUMENTED_ASTROCLAW_BRIDGE_COMMAND =
  "env ASTROCLAW_HIDE_BANNER=1 ASTROCLAW_SUPPRESS_NOTES=1 astroclaw acp --url ws://127.0.0.1:18789 --token-file ~/.astroclaw/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @zed-industries/codex-acp@0.13.0";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/astroclaw/acpx/codex-acp-wrapper.mjs"`;
const CODEX_ACP_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_ACP_WRAPPER_COMMAND} ${ASTROCLAW_ACPX_LEASE_ID_ARG} lease-close ${ASTROCLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  const runtime = new AcpxRuntime(
    {
      ...
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "astroclaw" ? "astroclaw acp" : agentName),
        list: () => ["codex", "astroclaw"],
      },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("normalizes Astroclaw Codex model ids for ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("adds Codex wrapper stderr tail to generic session initialization failures", async () => {
    const wrapperRoot = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-acpx-runtime-"));
    ...
    const { runtime, delegate } = makeRuntime(baseStore, {
      astroclawGatewayInstanceId: "gateway-test",
      astroclawProcessLeaseStore: leaseStore.store,
      astroclawWrapperRoot: wrapperRoot,
      agentRegistry: {
  ...
  it("adds Codex wrapper stderr tail to generic terminal turn error events", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        astroclawLeaseId: "lease-turn-event",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(__testing.isCodexAcpCommand("astroclaw acp")).toBe(false);
  });
  ...
  it("recognizes claude-agent-acp commands", () => {
    ...
    expect(
      __testing.isClaudeAcpCommand(`node "/tmp/astroclaw/acpx/claude-agent-acp-wrapper.mjs"`),
    ).toBe(true);
    expect(
      __testing.isClaudeAcpCommand(
        `node.exe "C:/Users/runner/AppData/Local/Temp/astroclaw/acpx/claude-agent-acp-wrapper.mjs"`,
      ),
    ).toBe(true);
    expect(
      __testing.isClaudeAcpCommand(
    ...
    expect(__testing.isClaudeAcpCommand("astroclaw acp")).toBe(false);
  ...
  it("cleans up Astroclaw-owned ACPX process trees after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/astroclaw/acpx/codex-acp-wrapper.mjs"',
        pid: 900,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate } = makeRuntime(
      baseStore,
      {
        astroclawWrapperRoot: "/tmp/astroclaw/acpx",
      },
      {
        astroclawProcessCleanup: {
          listProcesses: vi.fn(async () => [
            ...
            },
            {
              ...
              ppid: 900,
              command:
                "node /tmp/astroclaw/plugin-runtime-deps/node_modules/@zed-industries/codex-acp/bin/codex-acp.js",
            },
          ]),
  ...
  it("records ACPX process leases without persisting lease-specific agent commands", async () => {
    ...
    expect(lease?.wrapperPath).toBe("/tmp/astroclaw/acpx/codex-acp-wrapper.mjs");
    expect(launchCommands[0]).toContain("ASTROCLAW_ACPX_LEASE_ID=");
    expect(launchCommands[0]).toContain("ASTROCLAW_GATEWAY_INSTANCE_ID=gateway-test");
    expect(savedRecords[0]?.agentCommand).toBe(CODEX_ACP_WRAPPER_COMMAND);
    expect(savedRecords[0]?.astroclawGatewayInstanceId).toBe("gateway-test");
    expect(savedRecords[0]?.astroclawLeaseId).toBe(lease?.leaseId);
  });
  ...
  it("merges sidecar lease ids into loaded ACPX session records", async () => {
    ...
    expect(loadedRecord?.astroclawGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.astroclawLeaseId).toBe("lease-loaded");
  });
  ...
  it("merges the lease for the current ACPX session process when old leases exist", async () => {
    ...
    const loadedRecord = await wrappedStore.load("agent:codex:acp:binding:test");
    expect(loadedRecord?.astroclawGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.astroclawLeaseId).toBe("lease-current");
  });
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 930,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${ASTROCLAW_ACPX_LEASE_ID_ARG} lease-old ${ASTROCLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
            {
              pid: 940,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${ASTROCLAW_ACPX_LEASE_ID_ARG} lease-current ${ASTROCLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
  ...
  it("routes astroclaw ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const bridgeEnsure = vi.spyOn(bridgeSafeDelegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:astroclaw:acp:test",
      backend: "acpx",
  ...
  it("routes non-astroclaw sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for astroclaw sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:astroclaw:acp:test",
      backend: "acpx",
      runtimeSessionName: "astroclaw-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the astroclaw agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "astroclaw" ? "codex" : agentName),
        list: () => ["codex", "astroclaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the astroclaw bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "astroclaw acp" : agentName),
        list: () => ["codex", "astroclaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped astroclaw bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "astroclaw" ? DOCUMENTED_ASTROCLAW_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "astroclaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node astroclaw entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "astroclaw" ? "env ASTROCLAW_HIDE_BANNER=1 node astroclaw.mjs acp" : agentName,
        list: () => ["codex", "astroclaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:astroclaw:acp:test",
        agentCommand: DOCUMENTED_ASTROCLAW_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:astroclaw:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:astroclaw:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to astroclaw bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
