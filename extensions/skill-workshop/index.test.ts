...
import type { AnyAgentTool } from "astroclaw/plugin-sdk/agent-runtime";
import type { PluginTrustedToolPolicyRegistration } from "astroclaw/plugin-sdk/core";
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-skill-workshop-test-"));
  tempDirs.push(dir);
  return dir;
}
