...
export function getGoogleMeetSetupStatus(
  ...
  checks.push({
    ...
    message: config.chrome.browserProfile
      ? "Local Chrome uses the Astroclaw browser profile; chrome.browserProfile is passed to chrome-node hosts"
      : "Local Chrome uses the Astroclaw browser profile; configure browser.defaultProfile to choose another profile",
  });
