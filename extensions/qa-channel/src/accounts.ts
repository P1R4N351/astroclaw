...
import { resolveMergedAccountConfig } from "astroclaw/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "astroclaw";
  const botDisplayName = merged.botDisplayName?.trim() || "Astroclaw QA";
  return {
    accountId,
    enabled,
