...
const CODEX_DIAGNOSTICS_SOURCE = "astroclaw-diagnostics";
...
async function bindConversation(
  ...
  if (!ctx.sessionFile) {
    return {
      text: "Cannot bind Codex because this command did not include an Astroclaw session file.",
    };
  }
...
async function resumeThread(
  ...
  if (!ctx.sessionFile) {
    return "Cannot attach a Codex thread because this command did not include an Astroclaw session file.";
  }
  ...
  return `Attached this Astroclaw session to Codex thread ${formatCodexDisplayText(
    effectiveThreadId,
  )}.`;
}
...
async function stopConversationTurn(
  ...
  if (!sessionFile) {
    return "Cannot stop Codex because this command did not include an Astroclaw session file.";
  }
...
async function steerConversationTurn(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return "Cannot steer Codex because this command did not include an Astroclaw session file.";
  }
  return (
...
async function setConversationModel(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return "Cannot set Codex model because this command did not include an Astroclaw session file.";
  }
  const [model = ""] = args;
...
async function setConversationFastMode(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return "Cannot set Codex fast mode because this command did not include an Astroclaw session file.";
  }
  const value = args[0];
...
async function setConversationPermissions(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return "Cannot set Codex permissions because this command did not include an Astroclaw session file.";
  }
  const value = args[0];
...
async function requestCodexDiagnosticsFeedbackApproval(
  ...
  if (!(await hasAnyCodexDiagnosticsSessionFile(ctx))) {
    return {
      text: "Cannot send Codex diagnostics because this command did not include an Astroclaw session file.",
    };
  }
...
async function previewCodexDiagnosticsFeedbackApproval(
  ...
  if (!(await hasAnyCodexDiagnosticsSessionFile(ctx))) {
    return "Cannot send Codex diagnostics because this command did not include an Astroclaw session file.";
  }
  const targets = await resolveCodexDiagnosticsTargets(deps, ctx);
  if (targets.length === 0) {
    return [
...
function formatCodexDiagnosticsTargetBlock(
  ...
  if (target.sessionKey) {
    lines.push(`Astroclaw session key: ${formatCodexCopyableValueForDisplay(target.sessionKey)}`);
  }
  if (target.sessionId) {
    lines.push(`Astroclaw session id: ${formatCodexCopyableValueForDisplay(target.sessionId)}`);
  }
...
function formatCodexDiagnosticsTargetLine(target: CodexDiagnosticsTarget): string {
  ...
  if (sessionLabel) {
    parts.push(`Astroclaw session ${formatCodexValueForDisplay(sessionLabel)}`);
  }
...
async function startThreadAction(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return `Cannot start Codex ${label} because this command did not include an Astroclaw session file.`;
  }
  const binding = await deps.readCodexAppServerBinding(sessionFile);
  if (!binding?.threadId) {
    return `No Codex thread is attached to this Astroclaw session yet.`;
  }
