...
const CODEX_CONVERSATION_CONTROL_STATE = Symbol.for("astroclaw.codex.conversationControl");
...
async function requireThreadBinding(sessionFile: string) {
  ...
  if (!binding?.threadId) {
    throw new Error("No Codex thread is attached to this Astroclaw session yet.");
  }
  return binding;
}
