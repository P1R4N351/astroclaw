import { formatErrorMessage } from "astroclaw/plugin-sdk/agent-harness-runtime";
import type {
...
const CODEX_CONVERSATION_GLOBAL_STATE = Symbol.for("astroclaw.codex.conversationBinding");
  ...
  const response: CodexThreadStartResponse = await client.request(
    "thread/start",
    {
      ...
      developerInstructions:
        "This Codex thread is bound to an Astroclaw conversation. Answer normally; Astroclaw will deliver your final response back to the conversation.",
    ...
    async (request): Promise<JsonValue | undefined> => {
      if (request.method === "item/tool/call") {
        return {
          contentItems: [
            {
              type: "inputText",
              text: "Astroclaw native Codex conversation binding does not expose dynamic Astroclaw tools yet.",
            },
          ],
          success: false,
        };
      ...
      if (
        ...
        return {
          decision: "decline",
          reason:
            "Astroclaw native Codex conversation binding cannot route interactive approvals yet; use the Codex harness or explicit /acp spawn codex for that workflow.",
        };
      }
