...
async function findManagedCodexAppServerCommandPath(params: {
  ...
  throw new Error(
    [
      ...
      "Reinstall or update Astroclaw, or run pnpm install in a source checkout.",
      "Set plugins.entries.codex.config.appServer.command or ASTROCLAW_CODEX_APP_SERVER_BIN to use a custom Codex binary.",
    ].join(" "),
  );
}
