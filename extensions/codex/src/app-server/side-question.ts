...
async function createCodexSideToolBridge(input: {
  ...
  if (supportsModelTools(runtimeModel)) {
    const createAstroclawCodingTools = (await import("astroclaw/plugin-sdk/agent-harness"))
...
async function handleSideDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    return failedSideDynamicToolResponse("Astroclaw dynamic tool call aborted before execution.");
  }
  ...
  const timeoutPromise = new Promise<CodexDynamicToolCallResponse>((resolve) => {
    ...
    timeout = setTimeout(() => {
      controller.abort(new Error(`Astroclaw dynamic tool call timed out after ${timeoutMs}ms.`));
      resolve(
        failedSideDynamicToolResponse(`Astroclaw dynamic tool call timed out after ${timeoutMs}ms.`),
      );
    }, timeoutMs);
    timeout.unref?.();
  });
