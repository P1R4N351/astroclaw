...
export function resolveCodexAppServerEnvApiKeyCacheKey(params: {
  ...
  hash.update("astroclaw:codex:app-server-env-api-key:v1");
  hash.update("\0");
...
function fingerprintApiKeyAuthProfileCacheKey(apiKey: string): string {
  const hash = createHash("sha256");
  hash.update("astroclaw:codex:app-server-auth-profile-api-key:v1");
  hash.update("\0");
  hash.update(apiKey);
...
function fingerprintTokenAuthProfileCacheKey(accessToken: string): string {
  const hash = createHash("sha256");
  hash.update("astroclaw:codex:app-server-auth-profile-token:v1");
  hash.update("\0");
