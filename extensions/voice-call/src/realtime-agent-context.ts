...
import { buildRealtimeVoiceAgentConsultPolicyInstructions } from "astroclaw/plugin-sdk/realtime-voice";
...
export async function buildRealtimeVoiceInstructions(params: {
  ...
  const capsule: string[] = [
    "Astroclaw agent voice context:",
    `- Agent id: ${agentId}`,
    "- Use this context to match the Astroclaw agent's personality and standing preferences on fast voice turns.",
    "- Treat this as compact context only; call astroclaw_agent_consult when the caller needs the full agent brain, tools, memory, or workspace state.",
  ];

  if (contextConfig.includeIdentity) {
    const identity = params.agentRuntime.resolveAgentIdentity(
      params.coreConfig as AstroclawConfig,
      agentId,
