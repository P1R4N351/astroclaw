...
type Registered = {
  ...
  tools: unknown[];
  service?: Parameters<AstroclawPluginApi["registerService"]>[0];
};
type MockCallSource = {
  mock: {
...
function setup(config: Record<string, unknown>): Registered {
  ...
  const api = createTestPluginApi({
    ...
    config: {},
    pluginConfig: config,
    runtime: { tts: { textToSpeechTelephony: vi.fn() } } as unknown as AstroclawPluginApi["runtime"],
    logger: noopLogger,
...
describe("voice-call plugin", () => {
  ...
  afterEach(() => {
    ...
    vi.restoreAllMocks();
    vi.unstubAllEnvs();
    delete (globalThis as Record<PropertyKey, unknown>)[Symbol.for("astroclaw.voice-call.runtime")];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("astroclaw.voice-call.runtimePromise")
    ];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("astroclaw.voice-call.runtimeStopPromise")
    ];
  });
  ...
  it("still starts the webhook runtime for gateway CLI processes", async () => {
    ...
    vi.stubEnv("ASTROCLAW_CLI", "1");
    process.argv = ["node", "astroclaw", "gateway", "run"];
    const { service } = setup({ provider: "mock" });
  ...
  it("normalizes legacy config through runtime creation and warns to run doctor", async () => {
    ...
    expectWarningIncludes('Run "astroclaw doctor --fix"');
  });
  ...
  it("CLI smoke can place a live notify call with --yes", async () => {
    ...
    try {
      ...
      expect(runtimeStub.manager.initiateCall).toHaveBeenCalledWith("+15550009999", undefined, {
        message: "Astroclaw voice call smoke test.",
        mode: "notify",
      });
