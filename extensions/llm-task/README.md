...
`astroclaw.invoke --each`) without adding new Astroclaw code per workflow.
...
This extension depends on Astroclaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Astroclaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.astroclaw/extensions` as a standalone plugin directory.
