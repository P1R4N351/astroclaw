...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'astroclaw matrix verify status --verbose --account ${accountId}' and 'astroclaw matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Astroclaw runtime is using the same account. " +
    `Check 'astroclaw matrix verify status --verbose --account ${accountId}' and 'astroclaw matrix devices list --account ${accountId}'.`
  );
}
