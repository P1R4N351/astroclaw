...
import * as jsonStore from "astroclaw/plugin-sdk/json-store";
...
describe("FileBackedMatrixSyncStore", () => {
  ...
  function createStoragePath(): string {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-matrix-sync-store-"));
