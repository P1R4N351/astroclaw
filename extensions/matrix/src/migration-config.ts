  ...
  if (!channel) {
    return {
      warning:
        ...
        'Configure Matrix, then rerun "astroclaw doctor --fix" or restart the gateway.',
    };
  }
  if (requiresExplicitMatrixDefaultAccount(params.cfg)) {
    return {
      warning:
        ...
        'Set "channels.matrix.defaultAccount" to the intended target account before rerunning "astroclaw doctor --fix" or restarting the gateway.',
    };
  }
  ...
  if (!target) {
    ...
    return {
      warning:
        ...
        'Start the gateway once with a working Matrix login, or rerun "astroclaw doctor --fix" after cached credentials are available.',
    };
  }
