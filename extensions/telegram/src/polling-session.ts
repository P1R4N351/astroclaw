...
export class TelegramPollingSession {
  ...
  async runUntilAbort(): Promise<void> {
    ...
    try {
      ...
      // leak to api.telegram.org; see astroclaw#68128.
  ...
  async #runPollingCycle(bot: TelegramBot): Promise<"continue" | "exit"> {
    ...
    try {
      ...
      const conflictHint = isConflict
        ? " Another Astroclaw gateway, script, or Telegram poller may be using this bot token; stop the duplicate poller or switch this account to webhook mode."
        : "";
      this.opts.log(
