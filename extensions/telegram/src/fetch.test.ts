...
describe("resolveTelegramFetch", () => {
  ...
  it("uses the Astroclaw debug proxy URL when no explicit proxy fetch is provided", async () => {
    vi.stubEnv("ASTROCLAW_DEBUG_PROXY_ENABLED", "1");
    vi.stubEnv("ASTROCLAW_DEBUG_PROXY_URL", "http://127.0.0.1:7777");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("uses ASTROCLAW_PROXY_URL as a Telegram explicit proxy when proxy env is absent", async () => {
    vi.stubEnv("ASTROCLAW_PROXY_URL", "http://127.0.0.1:7788");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
    ...
    expect(ProxyAgentCtor).toHaveBeenCalledTimes(1);
    const proxyOptions = constructorOptions(ProxyAgentCtor, "Astroclaw proxy") as {
      allowH2?: boolean;
      uri?: string;
  ...
  it("preserves caller-provided custom fetch when ASTROCLAW_PROXY_URL is present", async () => {
    vi.stubEnv("ASTROCLAW_PROXY_URL", "http://127.0.0.1:7788");
    const proxyFetch = vi.fn(async () => ({ ok: true }) as Response) as unknown as typeof fetch;
  ...
  it("prefers standard proxy env over ASTROCLAW_PROXY_URL for Telegram", async () => {
    vi.stubEnv("ASTROCLAW_PROXY_URL", "http://127.0.0.1:7788");
    vi.stubEnv("https_proxy", "http://127.0.0.1:7890");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("keeps resolver-scoped transport policy for Astroclaw proxy fetches", async () => {
