...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): AstroclawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as AstroclawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): AstroclawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as AstroclawConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: AstroclawConfig, accountId = "media") {
    await handleTelegramAction(
