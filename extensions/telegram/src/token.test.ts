...
describe("resolveTelegramToken", () => {
  ...
  function createTempDir(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-telegram-token-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  function createUnknownAccountConfig(): AstroclawConfig {
    return {
      channels: {
        telegram: {
  ...
  function expectNoTokenForUnknownAccount(cfg: AstroclawConfig) {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked tokenFile paths", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile: tokenLink } } } as AstroclawConfig;
  ...
  // Regression: https://github.com/astroclaw/astroclaw/issues/53876
...
describe("telegram update offset store", () => {
  it("persists and reloads the last update id", async () => {
    await withStateDirEnv("astroclaw-telegram-", async () => {
