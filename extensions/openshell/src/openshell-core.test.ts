...
describe("openshell backend manager", () => {
  ...
  it("checks runtime status with config override from Astroclaw config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "astroclaw-session-1234",
        backendId: "openshell",
        runtimeLabel: "astroclaw-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "astroclaw-session-1234",
        config: expectedConfig,
      },
      args: ["sandbox", "get", "astroclaw-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "astroclaw-session-5678",
        backendId: "openshell",
        runtimeLabel: "astroclaw-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "astroclaw",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "astroclaw-session-5678",
        config: expectedConfig,
      },
      args: ["sandbox", "delete", "astroclaw-session-5678"],
    });
  });
});
...
describe("openshell fs bridges", () => {
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("astroclaw-openshell-fs-");
  ...
  it("rejects symlink-parent writes instead of escaping the local mount root", async () => {
    const workspaceDir = await makeTempDir("astroclaw-openshell-fs-");
    const outsideDir = await makeTempDir("astroclaw-openshell-outside-");
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("astroclaw-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("astroclaw-openshell-agent-");
