...
- Extension production code should import from `astroclaw/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `astroclaw.plugin.json` and the package
  `astroclaw` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `astroclaw/plugin-sdk/*`.
