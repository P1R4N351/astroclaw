...
describe("diagnostics-prometheus service", () => {
  it("records trusted run metrics without raw diagnostic identifiers", () => {
    ...
    expect(rendered).toContain("# TYPE astroclaw_run_completed_total counter");
    expect(rendered).toContain(
      'astroclaw_run_completed_total{channel="discord",model="gpt-5.4",outcome="completed",provider="openai",trigger="message"} 1',
    );
    expect(rendered).toContain(
      'astroclaw_run_duration_seconds_sum{channel="discord",model="gpt-5.4",outcome="completed",provider="openai",trigger="message"} 1.5',
    );
  ...
  it("records hook-blocked run metrics with safe blocker originator only", () => {
    ...
    expect(rendered).toContain(
      'astroclaw_run_completed_total{blocked_by="policy-plugin",channel="slack",model="gpt-5.4",outcome="blocked",provider="openai",trigger="message"} 1',
    );
    expect(rendered).not.toContain("run-should-not-export");
    expect(rendered).not.toContain("session-should-not-export");
  ...
  it("redacts and bounds label values", () => {
    ...
    expect(rendered).toContain(
      'astroclaw_tool_execution_total{error_category="other",outcome="error",params_kind="unknown",tool="tool"} 1',
    );
  ...
  it("bounds messaging labels without exporting raw chat identifiers", () => {
    ...
    expect(rendered).toContain(
      'astroclaw_message_delivery_started_total{channel="matrix",delivery_kind="text"} 1',
    );
    expect(rendered).toContain(
      'astroclaw_message_processed_total{channel="unknown",outcome="completed",reason="none"} 1',
    );
    expect(rendered).toContain(
      'astroclaw_message_delivery_total{channel="unknown",delivery_kind="other",error_category="TimeoutError",outcome="error"} 1',
    );
  ...
  it("records session recovery and talk metrics without exporting raw ids or content", () => {
    ...
    expect(rendered).toContain(
      'astroclaw_session_recovery_total{action="abort-active-run",active_work_kind="tool_call",state="processing",status="released"} 1',
    );
    expect(rendered).toContain(
      'astroclaw_session_recovery_age_seconds_sum{action="abort-active-run",active_work_kind="tool_call",state="processing",status="released"} 12',
    );
    expect(rendered).toContain(
      'astroclaw_talk_event_total{brain="agent-consult",event_type="input.audio.delta",mode="realtime",provider="openai",transport="gateway-relay"} 1',
    );
    expect(rendered).toContain(
      'astroclaw_talk_audio_bytes_sum{brain="agent-consult",event_type="input.audio.delta",mode="realtime",provider="openai",transport="gateway-relay"} 320',
    );
    expect(rendered).not.toContain("session-should-not-export");
  ...
  it("caps metric series growth and reports dropped series", () => {
    ...
    expect(rendered).toContain("# TYPE astroclaw_prometheus_series_dropped_total counter");
    expect(rendered).toContain("astroclaw_prometheus_series_dropped_total ");
  });
  ...
  it("subscribes to internal diagnostics and renders scrape text", () => {
    ...
    exporter.service.start({
      config: {} as never,
      stateDir: "/tmp/astroclaw-prometheus-test",
      logger: {
        info: vi.fn(),
        warn: vi.fn(),
        error: vi.fn(),
    ...
    expect(exporter.render()).toContain(
      'astroclaw_model_tokens_total{agent="unknown",channel="unknown",model="gpt-5.4",provider="openai",token_type="input"} 12',
    );
