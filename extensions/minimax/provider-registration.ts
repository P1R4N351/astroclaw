...
import { MINIMAX_FAST_MODE_STREAM_HOOKS } from "astroclaw/plugin-sdk/provider-stream-family";
import { fetchMinimaxUsage } from "astroclaw/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: AstroclawConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: AstroclawConfig): string | undefined {
...
export function registerMinimaxProviders(api: AstroclawPluginApi) {
