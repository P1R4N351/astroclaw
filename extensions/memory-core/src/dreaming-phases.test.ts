...
const LIGHT_DREAMING_TEST_CONFIG: AstroclawConfig = {
  plugins: {
    entries: {
      "memory-core": {
        config: {
...
async function createDreamingWorkspace(): Promise<string> {
  const workspaceDir = await createTempWorkspace("astroclaw-dreaming-phases-");
...
async function triggerLightDreaming(
  ...
  await beforeAgentReply(
    { cleanedBody: "__astroclaw_memory_core_light_sleep__" },
...
describe("memory-core dreaming phases", () => {
      ...
      await beforeAgentReply(
        {
          cleanedBody: [
            "System: rotate logs",
            "System: __astroclaw_memory_core_light_sleep__",
            "",
            "A scheduled reminder has been triggered. The reminder content is:",
            "",
            "rotate logs",
            "__astroclaw_memory_core_light_sleep__",
            "",
  ...
  it("checkpoints session transcript ingestion and skips unchanged transcripts", async () => {
    ...
    vi.stubEnv("ASTROCLAW_STATE_DIR", path.join(workspaceDir, ".state"));
  ...
  it("skips dreaming-generated narrative transcripts during session ingestion", async () => {
    ...
    await fs.writeFile(
      ...
      [
        JSON.stringify({
          type: "custom",
          customType: "astroclaw:bootstrap-context:full",
          data: {
  ...
  it("records light/rem signals that reinforce deep promotion ranking", async () => {
    ...
    await withDreamingTestClock(async () => {
      ...
      await beforeAgentReply(
        { cleanedBody: "__astroclaw_memory_core_rem_sleep__" },
        { trigger: "heartbeat", workspaceDir },
      );
    });
  ...
  it("increments dailyCount when the same daily file is re-ingested on a later day", async () => {
    ...
    const configForTest: AstroclawConfig = {
      plugins: {
        entries: {
          "memory-core": {
            config: {
