...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("astroclaw.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("astroclaw.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("astroclaw.qmdEmbedQueueTail");
...
vi.mock("astroclaw/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("astroclaw/plugin-sdk/file-lock")>(
    "astroclaw/plugin-sdk/file-lock",
  );
  return {
    ...actual,
