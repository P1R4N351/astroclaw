...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "astroclaw/plugin-sdk/memory-core-host-engine-qmd";
...
function createBuiltinCfg(agentId: string): AstroclawConfig {
  return {
    agents: {
      defaults: {
...
async function expectPendingQmdReplacement(params: {
  agentId: string;
  firstCfg: AstroclawConfig;
  secondCfg: AstroclawConfig;
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("astroclaw.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-qmd-workspace-"));
