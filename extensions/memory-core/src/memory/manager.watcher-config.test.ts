...
const { createdWatchers, memoryLoggerWarn, watchMock } = vi.hoisted(() => {
  ...
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("astroclaw.test.memoryWatchFactory")] =
    result.watchMock;
  return result;
});
...
vi.mock("astroclaw/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("astroclaw/plugin-sdk/memory-core-host-engine-foundation")>();
  return {
    ...actual,
...
describe("memory watcher config", () => {
  ...
  afterAll(() => {
    Reflect.deleteProperty(globalThis, Symbol.for("astroclaw.test.memoryWatchFactory"));
  });
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): AstroclawConfig {
    const defaults: NonNullable<NonNullable<AstroclawConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: AstroclawConfig) {
