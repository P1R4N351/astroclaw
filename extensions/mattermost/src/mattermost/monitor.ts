...
import { isReasoningReplyPayload } from "astroclaw/plugin-sdk/reply-payload";
...
export async function monitorMattermostProvider(opts: MonitorMattermostOpts = {}): Promise<void> {
  ...
  const cfg = (opts.config ?? core.config.current()) as AstroclawConfig;
      ...
      authorizeButtonClick: async ({ payload, post }) => {
        ...
        return {
          ...
          response: {
            ...
            ephemeral_text: `Astroclaw ignored this action for ${decision.roomLabel}.`,
          },
        };
      },
