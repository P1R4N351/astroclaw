...
function fakeApi(): AstroclawPluginApi {
  return {
    config: {},
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.astroclaw.ai", "", "astroclaw.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.astroclaw.ai", "astroclaw.ai"],
  ...
  it("late-binds dedicated tools to the resolved runtime config snapshot", async () => {
    ...
    const registeredTools: Array<Parameters<AstroclawPluginApi["registerTool"]>[0]> = [];
    const registeredOptions: Array<Parameters<AstroclawPluginApi["registerTool"]>[1]> = [];
    ...
    await searchTool.execute("search-call", { query: "astroclaw" });
    ...
    expect(searchParams.query).toBe("astroclaw");
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as AstroclawConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
