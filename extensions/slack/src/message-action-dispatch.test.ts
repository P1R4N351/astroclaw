...
describe("handleSlackMessageAction", () => {
  ...
  it("keeps generated Slack control ids unique when presentation and interactive controls are merged", async () => {
    ...
    expect(firstButtons.block_id).toBe("astroclaw_reply_buttons_1");
    expect(elementAt(firstButtons, 0).action_id).toBe("astroclaw:reply_button:1:1");
    const secondButtons = blockAt(action, 1);
    expect(secondButtons.block_id).toBe("astroclaw_reply_buttons_2");
    expect(elementAt(secondButtons, 0).action_id).toBe("astroclaw:reply_button:2:1");
  });
