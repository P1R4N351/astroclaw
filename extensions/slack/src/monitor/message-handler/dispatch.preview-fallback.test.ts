...
vi.mock("astroclaw/plugin-sdk/channel-streaming", () => ({
...
vi.mock("astroclaw/plugin-sdk/outbound-runtime", () => ({
...
vi.mock("../config.runtime.js", () => ({
  resolveStorePath: () => "/tmp/astroclaw-store.json",
...
describe("dispatchPreparedSlackMessage preview fallback", () => {
  ...
  it("updates non-main DM last-route metadata on the prepared direct session", async () => {
    ...
    expect(updateLastRouteMock).toHaveBeenCalledWith({
      storePath: "/tmp/astroclaw-store.json",
      sessionKey: "agent:main:slack:direct:u1",
      deliveryContext: {
        channel: "slack",
        to: "user:U1",
