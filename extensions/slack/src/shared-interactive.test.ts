...
describe("buildSlackInteractiveBlocks", () => {
  it("renders shared interactive blocks in authored order", () => {
    expect(
      ...
      {
        type: "actions",
        block_id: "astroclaw_reply_select_1",
        elements: [
          {
            type: "static_select",
            action_id: "astroclaw:reply_select:1",
            placeholder: {
              type: "plain_text",
              text: "Pick one",
              emoji: true,
      ...
      },
      {
        ...
        elements: [
          {
            type: "button",
            action_id: "astroclaw:reply_button:1:1",
            text: {
              type: "plain_text",
              text: "Retry",
              emoji: true,
  ...
  it("preserves original callback payloads for round-tripping", () => {
    ...
    expect(buttonBlock.elements?.[0]?.action_id).toBe("astroclaw:reply_button:1:1");
    expect(buttonBlock.elements?.[0]?.value).toBe("pluginbind:approval-123:o");
    expect(selectBlock.elements?.[0]?.action_id).toBe("astroclaw:reply_select:1");
  ...
  it("drops value-only Slack buttons with values beyond Block Kit limits", () => {
    ...
    expect(buttonBlock.elements?.[1]).toEqual({
      type: "button",
      action_id: "astroclaw:reply_button:1:3",
      text: {
        type: "plain_text",
        text: "Docs",
        emoji: true,
