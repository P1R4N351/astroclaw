...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of Astroclaw")).toBe("friends-of-astroclaw");
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-astroclaw" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Astroclaw"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-astroclaw");
  });
  ...
  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-astroclaw": { slug: "friends-of-astroclaw" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Astroclaw"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["astroclaw-dm"],
        channelId: "1",
        channelName: "Astroclaw DM",
        channelSlug: "astroclaw-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
...
function makeReactionListenerParams(overrides?: {
  ...
  return {
    cfg: {} as import("astroclaw/plugin-sdk/config-contracts").AstroclawConfig,
    accountId: "acc-1",
