...
describe("maybeCreateDiscordAutoThread autoThreadName", () => {
  it("renames created thread when generated mode is enabled", async () => {
    ...
    const cfg = { agents: { defaults: { model: "anthropic/claude-opus-4-6" } } } as AstroclawConfig;
    const result = await maybeCreateDiscordAutoThread(
      createBaseParams({
        baseText: "Need help with deploy rollout",
        combinedBody: "Need help with deploy rollout",
        channelName: "astroclaw",
        channelDescription: "Astroclaw development coordination and release planning",
    ...
    expectGeneratedTitleField("channelName", "astroclaw");
    expectGeneratedTitleField(
      "channelDescription",
      "Astroclaw development coordination and release planning",
    );
