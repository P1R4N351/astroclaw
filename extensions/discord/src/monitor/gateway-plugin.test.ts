...
describe("createDiscordGatewayPlugin", () => {
  ...
  it("resolves gateway metadata timeout from config, env, then default", () => {
    ...
    expect(
      resolveDiscordGatewayInfoTimeoutMs({
        env: { ASTROCLAW_DISCORD_GATEWAY_INFO_TIMEOUT_MS: "25000" },
      }),
    ).toBe(25_000);
  ...
  it("leaves autoInteractions disabled so Astroclaw owns interaction handoff", () => {
  ...
  it("keeps Astroclaw metadata timeout out of gateway options", () => {
