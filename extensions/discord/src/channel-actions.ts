import { createUnionActionGate } from "astroclaw/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "astroclaw/plugin-sdk/channel-contract";
import type { DiscordActionConfig, AstroclawConfig } from "astroclaw/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: AstroclawConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: AstroclawConfig) {
