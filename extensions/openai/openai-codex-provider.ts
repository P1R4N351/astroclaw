...
import { fetchCodexUsage } from "astroclaw/plugin-sdk/provider-usage";
import {
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.astroclaw.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `astroclaw models auth login --provider openai-codex` or `astroclaw configure`.";
}
