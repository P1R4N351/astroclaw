...
describe("buildOpenAIRealtimeVoiceProvider", () => {
  ...
  it("adds Astroclaw attribution headers to native realtime websocket requests", () => {
  ...
  it("returns browser-safe Astroclaw attribution headers for native WebRTC offers", async () => {
    vi.stubEnv("ASTROCLAW_VERSION", "2026.3.22");
    fetchWithSsrFGuardMock.mockResolvedValueOnce({
      response: createJsonResponse({
  ...
  it("resolves keychain OPENAI_API_KEY refs before creating browser sessions", async () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:astroclaw:OPENAI_REALTIME_BROWSER_TEST");
  ...
  it("resolves and caches keychain OPENAI_API_KEY refs before creating bridges", () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:astroclaw:OPENAI_REALTIME_BRIDGE_TEST");
  ...
  it("does not resolve keychain refs during configured checks", () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:astroclaw:OPENAI_REALTIME_CONFIGURED_TEST");
    const provider = buildOpenAIRealtimeVoiceProvider();
  ...
  it("fails closed when keychain refs cannot be resolved", async () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:astroclaw:OPENAI_REALTIME_MISSING_TEST");
  ...
  it("emits tool calls from realtime conversation item done events", async () => {
    ...
    expect(onEvent).toHaveBeenCalledWith({
      direction: "server",
      type: "conversation.item.done",
      detail: "function_call name=astroclaw_agent_consult",
    });
  });
