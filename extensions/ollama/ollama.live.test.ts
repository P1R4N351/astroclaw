...
const LIVE = process.env.ASTROCLAW_LIVE_TEST === "1" && process.env.ASTROCLAW_LIVE_OLLAMA === "1";
const OLLAMA_BASE_URL =
  process.env.ASTROCLAW_LIVE_OLLAMA_BASE_URL?.trim() || "http://127.0.0.1:11434";
const CHAT_MODEL = process.env.ASTROCLAW_LIVE_OLLAMA_MODEL?.trim() || "llama3.2:latest";
const EMBEDDING_MODEL =
  process.env.ASTROCLAW_LIVE_OLLAMA_EMBED_MODEL?.trim() || "embeddinggemma:latest";
const PROVIDER_ID = process.env.ASTROCLAW_LIVE_OLLAMA_PROVIDER_ID?.trim() || "ollama-live-custom";
const RUN_WEB_SEARCH = process.env.ASTROCLAW_LIVE_OLLAMA_WEB_SEARCH !== "0";
...
async function withTempAstroclawState<T>(run: (paths: { root: string }) => Promise<T>): Promise<T> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-ollama-cli-live-"));
  try {
    await fs.writeFile(
      path.join(root, "astroclaw.json"),
      JSON.stringify(
        {
          models: {
            providers: {
...
async function runAstroclawCli(args: string[], env: NodeJS.ProcessEnv) {
  const outputRoot = fsSync.mkdtempSync(path.join(os.tmpdir(), "astroclaw-ollama-cli-output-"));
  ...
  try {
    const result = spawnSync(process.execPath, ["astroclaw.mjs", ...args], {
      cwd: process.cwd(),
      env,
      timeout: 90_000,
...
function buildCliEnv(root: string): NodeJS.ProcessEnv {
  return {
    ...
    ASTROCLAW_LIVE_OLLAMA: "1",
    ASTROCLAW_LIVE_OLLAMA_WEB_SEARCH: "0",
    ASTROCLAW_STATE_DIR: path.join(root, "state"),
    ASTROCLAW_CONFIG_PATH: path.join(root, "astroclaw.json"),
...
describe.skipIf(!LIVE)("ollama live", () => {
  it("runs infer model run through the local CLI path without PI model discovery", async () => {
    await withTempAstroclawState(async ({ root }) => {
      const result = await runAstroclawCli(
        [
          "infer",
          "model",
          "run",
