...
function normalizeDenyList(cfg: AstroclawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: AstroclawPluginApi["config"]): string[] {
...
async function disarmNow(params: {
  ...
  const cfg = api.runtime.config.current() as AstroclawConfig;
...
export default definePluginEntry({
  ...
  register(api: AstroclawPluginApi) {
    ...
    const timerService: AstroclawPluginService = {
