...
describe("qqbot local media path remapping", () => {
  ...
  function createAstroclawTestRoot() {
    const actualHome = getHomeDir();
    const astroclawDir = path.join(actualHome, ".astroclaw");
    fs.mkdirSync(astroclawDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(astroclawDir, "qqbot-platform-test-"));
  ...
  function createQqbotMediaFile(fileName: string) {
    const { actualHome, testRootName } = createAstroclawTestRoot();
  ...
  it("allows structured payload files inside sibling Astroclaw media subdirectories", () => {
    ...
    // trust the shared `~/.astroclaw/media` root so auto-routed sends can access
    // those files without the path-outside-storage guard firing.
    const actualHome = getHomeDir();
    const outboundDir = path.join(actualHome, ".astroclaw", "media", "outbound");
