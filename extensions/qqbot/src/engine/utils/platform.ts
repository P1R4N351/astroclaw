 ...
 * silk-wasm availability checks — all without importing `astroclaw/plugin-sdk`.
...
/** Return a path under `~/.astroclaw/qqbot` without creating it. */
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".astroclaw", "qqbot", ...subPaths);
}

/** Return a path under `~/.astroclaw/qqbot`, creating it on demand. */
...
/**
 * Return a path under `~/.astroclaw/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under Astroclaw's core media allowlist so
...
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".astroclaw", "media", "qqbot", ...subPaths);
}

/** Return a path under `~/.astroclaw/media/qqbot`, creating it on demand. */
...
/**
 * Return `~/.astroclaw/media`, Astroclaw's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `astroclaw/src/media/local-roots.ts`). Using it as a
 ...
 */
function getAstroclawMediaDir(): string {
  return path.join(getHomeDir(), ".astroclaw", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".astroclaw", "workspace", "qqbot");
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and Astroclaw's shared `~/.astroclaw/media`
  ...
  const allowedRoots = [getAstroclawMediaDir(), getQQBotMediaPath()];
