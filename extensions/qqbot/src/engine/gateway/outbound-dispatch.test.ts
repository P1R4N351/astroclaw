...
function makeRuntime(params: {
  ...
  return {
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/astroclaw/qqbot-sessions.json"),
    ...
    tts: {
      textToSpeech: vi.fn(async () => ({
        success: true,
        audioPath: "/tmp/astroclaw-qqbot/tts.wav",
        provider: "test-tts",
        outputFormat: "wav",
      })),
    },
...
describe("dispatchOutbound", () => {
  ...
  it("synthesizes plain audioAsVoice text as a QQ voice reply", async () => {
    ...
    expect(audioFileToSilkBase64Mock).toHaveBeenCalledWith("/tmp/astroclaw-qqbot/tts.wav");
