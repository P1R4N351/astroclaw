...
metadata: { "astroclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Astroclaw `sessions_spawn`
- **File I/O** == Astroclaw `read`/`write`
- **Remote fetch** == Astroclaw `web_fetch` (or `exec` with curl when POST is required)
