...
import { mediaKindFromMime } from "astroclaw/plugin-sdk/media-mime";
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS, runFfmpeg } from "astroclaw/plugin-sdk/media-runtime";
import { saveMediaBuffer, saveMediaStream, type SavedMedia } from "astroclaw/plugin-sdk/media-store";
import { readByteStreamWithLimit } from "astroclaw/plugin-sdk/response-limit-runtime";
import { readRegularFile, writeExternalFileWithinRoot } from "astroclaw/plugin-sdk/security-runtime";
...
export async function downloadImageFeishu(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "astroclaw-feishu-img-",
...
async function downloadMessageResourceWithType(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "astroclaw-feishu-resource-",
