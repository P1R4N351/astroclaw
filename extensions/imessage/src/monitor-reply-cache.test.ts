...
// Isolate from any live ~/.astroclaw/imessage/reply-cache.jsonl that the
...
beforeAll(() => {
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-imsg-reply-cache-"));
  priorStateDir = process.env.ASTROCLAW_STATE_DIR;
...
afterAll(() => {
  if (priorStateDir === undefined) {
    ...
    process.env.ASTROCLAW_STATE_DIR = priorStateDir;
  }
...
beforeEach(() => {
  ...
  // _reset helper does this when ASTROCLAW_STATE_DIR is set, but explicitly
...
describe("findLatestIMessageEntryForChat", () => {
  ...
  it("never crosses account boundaries", () => {
    ...
    expect(process.env.ASTROCLAW_STATE_DIR).toBe(tempStateDir);
...
describe("hydrate-on-resolve (post-restart short-id persistence)", () => {
  it("hydrates the on-disk JSONL before resolving a short id whose mapping predates this run", () => {
    ...
    // ASTROCLAW_STATE_DIR is set, so we have to keep the file ourselves
