...
const {
  ...
  resolveMatrixQaAstroclawCliEntryPath,
  runMatrixQaAstroclawCli,
  startMatrixQaAstroclawCli,
} = vi.hoisted(() => ({
  formatMatrixQaCliCommand: (args: string[]) => `astroclaw ${args.join(" ")}`,
  redactMatrixQaCliOutput: (text: string) => text,
  resolveMatrixQaAstroclawCliEntryPath: (cwd: string) => `${cwd}/dist/index.js`,
  runMatrixQaAstroclawCli: vi.fn(),
  startMatrixQaAstroclawCli: vi.fn(),
}));
...
describe("matrix live qa scenarios", () => {
  beforeEach(() => {
    ...
    runMatrixQaAstroclawCli.mockReset();
    startMatrixQaAstroclawCli.mockReset();
  ...
  it("merges default and scenario-requested Matrix topology once per run", () => {
    expect(
      scenarioTesting.buildMatrixQaTopologyForScenarios({
        defaultRoomName: "Astroclaw Matrix QA run",
        scenarios: [
  ...
  it("runs the Matrix E2EE state_after regression scenario through the gateway fault proxy", async () => {
    ...
    try {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverDeviceId: "DRIVERDEVICE",
        gatewayRuntimeEnv: {
          ASTROCLAW_CONFIG_PATH: gatewayConfigPath,
  ...
  it("runs Matrix self-verification through the interactive CLI command", async () => {
    ...
    try {
      ...
      startMatrixQaAstroclawCli.mockReturnValue({
        args: ["matrix", "verify", "self", "--account", "cli"],
        endStdin,
        kill,
      ...
      runMatrixQaAstroclawCli.mockImplementation(async ({ args, env, stdin }) => {
        if (!cliAccountConfigDuringRun && env.ASTROCLAW_CONFIG_PATH) {
          const cliConfig = JSON.parse(
            await readFile(String(env.ASTROCLAW_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
              matrix?: {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverPassword: "driver-password",
        gatewayRuntimeEnv: {
          ASTROCLAW_CONFIG_PATH: "/tmp/gateway-config.json",
          ASTROCLAW_STATE_DIR: "/tmp/gateway-state",
          PATH: process.env.PATH,
        },
        outputDir,
      });
      ...
      expect(startMatrixQaAstroclawCli).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(startMatrixQaAstroclawCli, "startMatrixQaAstroclawCli").args).toEqual([
        "matrix",
        "verify",
        "self",
        "--account",
      ...
      expect(mockObjectArg(startMatrixQaAstroclawCli, "startMatrixQaAstroclawCli").timeoutMs).toBe(
        16_000,
      );
      ...
      expect(registrationRequest?.deviceName).toBe(
        "Astroclaw Matrix QA CLI Self Verification Owner",
      );
      if (
      ...
      expect(loginWithPassword).toHaveBeenCalledWith({
        deviceName: "Astroclaw Matrix QA CLI Self Verification Device",
        password: "cli-owner-password",
        userId: "@cli-owner:matrix-qa.test",
      });
      const e2eeClientOptions = mockObjectArg(
      ...
      expect(runMatrixQaAstroclawCli).toHaveBeenCalledTimes(2);
      expect(runMatrixQaAstroclawCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          "matrix",
          "verify",
          "backup",
      ...
      expect(mockObjectArg(runMatrixQaAstroclawCli, "runMatrixQaAstroclawCli").stdin).toBe(
        "encoded-recovery-key\n",
      );
      const cliEnv = mockObjectArg(startMatrixQaAstroclawCli, "startMatrixQaAstroclawCli")
        .env as Record<string, unknown>;
      expect(cliEnv?.ASTROCLAW_STATE_DIR).toContain("astroclaw-matrix-cli-qa-");
      expect(cliEnv?.ASTROCLAW_CONFIG_PATH).toContain("astroclaw-matrix-cli-qa-");
      const configPath = String(cliEnv?.ASTROCLAW_CONFIG_PATH);
      ...
      await expectPathMissing(String(cliEnv?.ASTROCLAW_STATE_DIR));
  ...
  it("runs Matrix account add --enable-e2ee through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaAstroclawCli.mockImplementation(async ({ args, env }) => {
        if (env.ASTROCLAW_CONFIG_PATH) {
      ...
      expect(runMatrixQaAstroclawCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          ...
          "cli-add-password",
          "--device-name",
          "Astroclaw Matrix QA CLI Account Add E2EE",
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Astroclaw Matrix QA CLI Account Add Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaAstroclawCli.mockImplementation(async ({ args, env }) => {
        if (!initialAccountConfig && env.ASTROCLAW_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.ASTROCLAW_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Astroclaw Matrix QA CLI Encryption Setup Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup idempotency through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Astroclaw Matrix QA CLI Encryption Idempotent Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup bootstrap failure through the CLI QA scenario", async () => {
    ...
    try {
      ...
      const wait = vi
        .fn()
        .mockRejectedValue(new Error("astroclaw matrix encryption setup exited 1"));
      const kill = vi.fn();
      startMatrixQaAstroclawCli.mockReturnValue({
      ...
      expect(
        (
          mockObjectArg(startMatrixQaAstroclawCli, "startMatrixQaAstroclawCli").env as Record<
            string,
            unknown
          >
        ).ASTROCLAW_CONFIG_PATH,
      ).toContain("astroclaw-matrix-e2ee-setup-qa-");
      expect(output).toHaveBeenCalledTimes(1);
      expect(wait).toHaveBeenCalledTimes(1);
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Astroclaw Matrix QA CLI Encryption Failure Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Astroclaw Matrix QA CLI Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix invalid recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Astroclaw Matrix QA CLI Invalid Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix multi-account encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaAstroclawCli.mockImplementation(async ({ args, env }) => {
        const configPath = String(env.ASTROCLAW_CONFIG_PATH);
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Astroclaw Matrix QA CLI Multi Account Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
    ...
    try {
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "Astroclaw Matrix QA CLI Setup Gateway" &&
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "Astroclaw Matrix QA CLI Setup Driver" &&
            request.registrationToken === "registration-token",
        ),
      ).toBe(true);
      expect(createPrivateRoom).toHaveBeenCalledWith({
