...
import { sleep } from "astroclaw/plugin-sdk/text-utility-runtime";
...
vi.mock("astroclaw/plugin-sdk/text-utility-runtime", async () => {
  const actual = await vi.importActual<typeof import("astroclaw/plugin-sdk/text-utility-runtime")>(
    "astroclaw/plugin-sdk/text-utility-runtime",
  );
  return {
    ...actual,
    sleep: vi.fn(async () => {}),
