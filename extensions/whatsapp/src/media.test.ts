...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredAstroclawTmpDir(), "astroclaw-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub ASTROCLAW_STATE_DIR.
    // Also keep it outside the Astroclaw temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["ASTROCLAW_STATE_DIR"]);
    process.env.ASTROCLAW_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "astroclaw-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredAstroclawTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/astroclaw` root that
    // actually holds `tinyPngFile` on this Linux test runner (#60713).
    const realTmpRoot = resolvePreferredAstroclawTmpDir();
  ...
  it("allows default Astroclaw state workspace and sandbox roots", async () => {
  ...
  it("rejects default Astroclaw state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
