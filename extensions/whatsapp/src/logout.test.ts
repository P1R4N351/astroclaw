...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "astroclaw-test-web-logout-"));
    previousOAuthDir = process.env.ASTROCLAW_OAUTH_DIR;
    process.env.ASTROCLAW_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  afterAll(async () => {
    if (previousOAuthDir === undefined) {
      ...
      process.env.ASTROCLAW_OAUTH_DIR = previousOAuthDir;
    }
  ...
  it("does not delete through symlinked auth dirs inside the Astroclaw auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the Astroclaw auth root", async () => {
