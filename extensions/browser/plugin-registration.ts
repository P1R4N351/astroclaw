...
function createLazyBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via Astroclaw's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      "Browser choice: omit profile by default for the isolated Astroclaw-managed browser (`astroclaw`).",
...
export const browserPluginNodeHostCommands: AstroclawPluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: AstroclawPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): AstroclawPluginService {
  let service: AstroclawPluginService | null = null;
...
export function registerBrowserPlugin(api: AstroclawPluginApi) {
  api.registerTool(((ctx: AstroclawPluginToolContext) =>
    createLazyBrowserTool({
      sandboxBridgeUrl: ctx.browser?.sandboxBridgeUrl,
      allowHostControl: ctx.browser?.allowHostControl,
      agentSessionKey: ctx.sessionKey,
    })) as AstroclawPluginToolFactory);
  api.registerCli(
    async ({ program }) => {
