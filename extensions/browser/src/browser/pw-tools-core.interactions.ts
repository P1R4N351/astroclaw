...
// P10-RELAX(rule 4): Public interaction wiring keeps click guard setup and cleanup in one function to preserve behavior.
...
// P10-RELAX(rule 4): Public typing behavior has two guarded paths that must share one cleanup scope.
...
export async function fillFormViaPlaywright(opts: {
  ...
  const reconcileRemoteDialog = () => reconcileRemoteDialogAfterActionSettled(page, opts.signal);
  try {
    // P10-RELAX(rule 2): fillForm must preserve caller-provided field cardinality for compatibility.
...
type BrowserEvaluateRunOptions = {
  abortPromise?: Promise<never>;
  cdpUrl: string;
  page: Page;
  previousUrl: string;
  reconcileRemoteDialog: () => void;
  ssrfPolicy?: SsrFPolicy;
  targetId?: string;
  timeoutMs: number;
};

function resolveBrowserEvaluateTimeoutMs(timeoutMs: number | undefined): number {
  const outerTimeout = normalizeTimeoutMs(timeoutMs, 20_000);
  if (!Number.isFinite(outerTimeout)) {
    throw new Error("evaluate timeout must be finite");
  }
  if (outerTimeout <= 0) {
    throw new Error("evaluate timeout must be positive");
  }
  return Math.min(Math.max(1000, Math.min(120_000, outerTimeout - 500)), outerTimeout);
}

function createEvaluateTimeoutPromise(timeoutMs: number): {
...
async function awaitBrowserEvaluateWithTimeout<T>(
  evalPromise: Promise<T>,
  opts: BrowserEvaluateRunOptions,
): Promise<T> {
  if (!evalPromise || typeof evalPromise.then !== "function") {
    throw new Error("evaluate promise is required");
  }
  if (!Number.isFinite(opts.timeoutMs) || opts.timeoutMs <= 0) {
    throw new Error("evaluate timeout must be positive");
  }
  const timeout = createEvaluateTimeoutPromise(opts.timeoutMs);
  try {
    return await awaitActionWithAbort(
      Promise.race([evalPromise, timeout.promise]),
      opts.abortPromise,
      opts.reconcileRemoteDialog,
    );
  } finally {
    timeout.cleanup();
...
async function assertEvaluateNavigationStartAllowed(opts: {
  cdpUrl: string;
  page: Page;
  ssrfPolicy?: SsrFPolicy;
  targetId?: string;
...
async function evaluateRefFunctionViaPlaywright(
  opts: BrowserEvaluateRunOptions & {
    fnSource: string;
    ref: string;
  },
  ...
  const locator = refLocator(opts.page, opts.ref);
  const evalPromise = locator.evaluate(opts.fnSource);
  return await assertInteractionNavigationCompletedSafely({
    action: () => awaitBrowserEvaluateWithTimeout(evalPromise, opts),
    cdpUrl: opts.cdpUrl,
    page: opts.page,
    previousUrl: opts.previousUrl,
    ssrfPolicy: opts.ssrfPolicy,
...
async function evaluatePageFunctionViaPlaywright(
  opts: BrowserEvaluateRunOptions & {
    fnSource: string;
  },
): Promise<unknown> {
  ...
  const evalPromise = opts.page.evaluate(opts.fnSource);
  return await assertInteractionNavigationCompletedSafely({
    action: () => awaitBrowserEvaluateWithTimeout(evalPromise, opts),
    cdpUrl: opts.cdpUrl,
    page: opts.page,
...
export async function evaluateViaPlaywright(opts: {
  ...
  const page = await getRestoredPageForTarget(opts);
  const evaluateTimeout = resolveBrowserEvaluateTimeoutMs(opts.timeoutMs);
  ...
  try {
    const previousUrl = page.url();
    await assertEvaluateNavigationStartAllowed({
      cdpUrl: opts.cdpUrl,
      page,
      ssrfPolicy: opts.ssrfPolicy,
      targetId: opts.targetId,
    ...
    const runOpts = {
      ...
      page,
      previousUrl,
      reconcileRemoteDialog: () => reconcileRemoteDialogAfterActionSettled(page, signal),
      ssrfPolicy: opts.ssrfPolicy,
      targetId: opts.targetId,
      timeoutMs: evaluateTimeout,
    };
    return opts.ref
      ? await evaluateRefFunctionViaPlaywright({ ...runOpts, fnSource, ref: opts.ref })
      : await evaluatePageFunctionViaPlaywright({ ...runOpts, fnSource });
  } finally {
    cleanup();
  }
}
...
export async function waitForViaPlaywright(opts: {
  ...
  try {
    ...
    if (opts.fn) {
      ...
      if (fn) {
        // P10-RELAX(rule 1): Playwright wait predicates are caller-supplied browser functions by public API contract.
...
// P10-RELAX(rule 4): Screenshot labeling keeps capture planning and overlay cleanup in one public operation.
export async function screenshotWithLabelsViaPlaywright(opts: {
  ...
  let bboxFailures = 0;
  // P10-RELAX(rule 2): label screenshots must preserve caller-provided ref cardinality for annotations.
...
// P10-RELAX(rule 4): Exhaustive action dispatch stays centralized to preserve batch action semantics.
