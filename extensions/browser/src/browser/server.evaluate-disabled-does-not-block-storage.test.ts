...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        astroclaw: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ASTROCLAW_GATEWAY_PORT;
    process.env.ASTROCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ASTROCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ASTROCLAW_GATEWAY_PASSWORD;
