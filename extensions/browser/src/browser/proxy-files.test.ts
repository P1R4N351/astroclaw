...
import { MEDIA_MAX_BYTES } from "astroclaw/plugin-sdk/media-runtime";
...
describe("persistBrowserProxyFiles", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("astroclaw-browser-proxy-files-");
  });
  ...
  it("persists browser proxy files under the shared media store", async () => {
    ...
    expect(path.normalize(savedPath ?? "")).toContain(
      `${path.sep}.astroclaw${path.sep}media${path.sep}browser${path.sep}`,
    );
  ...
  it("rejects browser proxy files that exceed the shared media size limit", async () => {
    ...
    await expect(
      fs.stat(path.join(tempHome.home, ".astroclaw", "media", "browser")),
