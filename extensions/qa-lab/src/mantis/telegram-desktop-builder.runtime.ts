...
const TELEGRAM_PROFILE_ARCHIVE_ENV = "ASTROCLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64";
const TELEGRAM_PROFILE_ARCHIVE_ENV_NAME_ENV =
  "ASTROCLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_ARCHIVE_ENV";
const TELEGRAM_PROFILE_DIR_ENV = "ASTROCLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_DIR";
...
function buildCrabboxEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  if (!trimToValue(next.ASTROCLAW_MANTIS_TELEGRAM_GROUP_ID)) {
    next.ASTROCLAW_MANTIS_TELEGRAM_GROUP_ID = trimToValue(next.ASTROCLAW_QA_TELEGRAM_GROUP_ID);
  }
  if (!trimToValue(next.ASTROCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN)) {
    next.ASTROCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN = trimToValue(
      next.ASTROCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN,
    );
  }
  if (!trimToValue(next.ASTROCLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN)) {
    next.ASTROCLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN = trimToValue(
      next.ASTROCLAW_QA_TELEGRAM_SUT_BOT_TOKEN,
    );
  }
  return next;
}
...
function resolveTelegramGatewayEnvPayload(
  ...
  const groupId = trimToValue(env.ASTROCLAW_QA_TELEGRAM_GROUP_ID);
  const driverToken = trimToValue(env.ASTROCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN);
  const sutToken = trimToValue(env.ASTROCLAW_QA_TELEGRAM_SUT_BOT_TOKEN);
  if (!groupId || !driverToken || !sutToken) {
    throw new Error(
      "Telegram desktop builder requires ASTROCLAW_QA_TELEGRAM_GROUP_ID, ASTROCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN, and ASTROCLAW_QA_TELEGRAM_SUT_BOT_TOKEN when using --credential-source env.",
    );
  }
...
async function prepareGatewayCredentialEnv(params: {
  ...
  if (
    trimToValue(params.env.ASTROCLAW_MANTIS_TELEGRAM_GROUP_ID) &&
    trimToValue(params.env.ASTROCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN) &&
    trimToValue(params.env.ASTROCLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN)
  ) {
    return {};
  }
  ...
  params.env.ASTROCLAW_MANTIS_TELEGRAM_GROUP_ID = payload.groupId;
  params.env.ASTROCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN = payload.driverToken;
  params.env.ASTROCLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN = payload.sutToken;
  params.env.ASTROCLAW_QA_TELEGRAM_GROUP_ID =
    trimToValue(params.env.ASTROCLAW_QA_TELEGRAM_GROUP_ID) ?? payload.groupId;
  params.env.ASTROCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN =
    trimToValue(params.env.ASTROCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN) ?? payload.driverToken;
  params.env.ASTROCLAW_QA_TELEGRAM_SUT_BOT_TOKEN =
    trimToValue(params.env.ASTROCLAW_QA_TELEGRAM_SUT_BOT_TOKEN) ?? payload.sutToken;
  return {
    credentialLease,
    leaseHeartbeat,
  };
...
telegram_root="$HOME/.local/share/astroclaw-mantis/telegram-desktop-bin"
...
if [ -n "\${ASTROCLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64:-}" ]; then
  printf '%s' "$ASTROCLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64" | base64 -d >"$out/telegram-profile.tgz"
...
qa_status=0
{
  ...
  if [ "$setup_gateway" = "1" ]; then
    export TELEGRAM_BOT_TOKEN="\${ASTROCLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN:-\${TELEGRAM_BOT_TOKEN:-}}"
    telegram_group_id="\${ASTROCLAW_MANTIS_TELEGRAM_GROUP_ID:-}"
    driver_token="\${ASTROCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN:-}"
    if [ -z "$TELEGRAM_BOT_TOKEN" ] || [ -z "$telegram_group_id" ] || [ -z "$driver_token" ]; then
      echo "Gateway setup requires ASTROCLAW_MANTIS_TELEGRAM_GROUP_ID, ASTROCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN, and ASTROCLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN." >&2
      exit 2
    fi
...
const token = process.env.ASTROCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN;
...
node --input-type=module -e 'import fs from "node:fs"; const value = JSON.parse(fs.readFileSync(process.argv[1], "utf8")); process.stdout.write(String(value.id || ""));' "$out/telegram-driver-getme.json")"
    export ASTROCLAW_HOME="$HOME/.astroclaw-mantis/telegram-astroclaw"
...
MANTIS_TELEGRAM_PATCH
    pnpm astroclaw config patch --file "$out/telegram.patch.json5" --dry-run
    pnpm astroclaw config patch --file "$out/telegram.patch.json5"
    node --input-type=module >"$out/telegram-ready-message.json" 2>"$out/telegram-ready-message.err" <<'MANTIS_TELEGRAM_READY'
const token = process.env.ASTROCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN;
const chatId = process.env.ASTROCLAW_MANTIS_TELEGRAM_GROUP_ID;
...
MANTIS_TELEGRAM_READY
    nohup pnpm astroclaw gateway run --dev --allow-unconfigured --port 38974 --cli-backend-logs </dev/null >"$out/astroclaw-gateway.log" 2>&1 &
    gateway_pid="$!"
...
function renderReport(summary: MantisTelegramDesktopBuilderSummary) {
  const lines = [
    ...
    "- Astroclaw gateway log: `astroclaw-gateway.log`",
...
export async function runMantisTelegramDesktopBuilder(
  ...
  const remoteOutputDir = `/tmp/astroclaw-mantis-telegram-desktop-${startedAt
    .toISOString()
  ...
  try {
    ...
    if (gatewaySetup && !gatewaySetupCompleted) {
      throw new Error("Telegram desktop builder did not report a live Astroclaw gateway.");
    }
    summary = {
      artifacts: {
        reportPath,
