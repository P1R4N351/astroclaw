...
describe("mantis Slack desktop smoke runtime", () => {
  ...
  it("leases a desktop box, runs Slack QA inside it, copies artifacts, and stops on pass", async () => {
    ...
    const runtimeEnv = {
      ...
      ASTROCLAW_QA_SLACK_CHANNEL_ID: "C123",
      ASTROCLAW_QA_SLACK_DRIVER_BOT_TOKEN: "driver-token",
      ASTROCLAW_QA_SLACK_SUT_APP_TOKEN: "app-token",
      ASTROCLAW_QA_SLACK_SUT_BOT_TOKEN: "sut-token",
    };
    const runner = vi.fn(
    ...
    expect(
      commands.every((entry) => entry.env?.ASTROCLAW_LIVE_OPENAI_KEY === "openai-runtime-key"),
    ).toBe(true);
    ...
    expect(remoteScript).toContain("astroclaw qa slack");
    expect(remoteScript).toContain("--scenario 'slack-canary'");
    expect(remoteScript).toContain("ASTROCLAW_MANTIS_SLACK_BROWSER_PROFILE_DIR");
    ...
    expect(rsyncArgs).toContain(
      "crabbox@203.0.113.10:/tmp/astroclaw-mantis-slack-desktop-2026-05-04T13-00-00-000Z/",
    );
    expect(rsyncArgs).toContain(
      "crabbox@203.0.113.10:/tmp/astroclaw-mantis-slack-desktop-2026-05-04T13-00-00-000Z/slack-qa/",
    );
  ...
  it("leases Convex Slack credentials for gateway setup and maps them into the VM env", async () => {
    ...
    expect(runCommand?.env?.ASTROCLAW_MANTIS_SLACK_APP_TOKEN).toBe("xapp-leased");
    expect(runCommand?.env?.ASTROCLAW_MANTIS_SLACK_BOT_TOKEN).toBe("xoxb-leased");
    expect(runCommand?.env?.ASTROCLAW_MANTIS_SLACK_CHANNEL_ID).toBe("CLEASED");
    expect(runCommand?.env?.ASTROCLAW_QA_SLACK_CHANNEL_ID).toBe("CLEASED");
    expect(runCommand?.env?.ASTROCLAW_QA_SLACK_SUT_APP_TOKEN).toBe("xapp-leased");
    expect(runCommand?.env?.ASTROCLAW_QA_SLACK_SUT_BOT_TOKEN).toBe("xoxb-leased");
    ...
    expect(remoteScript).toContain('</dev/null >"$out/astroclaw-gateway.log"');
  ...
  it("passes gateway setup when Crabbox returns non-zero after remote metadata proves success", async () => {
    ...
    const result = await runMantisSlackDesktopSmoke({
      ...
      env: {
        OPENAI_API_KEY: "openai-runtime-key",
        ASTROCLAW_MANTIS_SLACK_APP_TOKEN: "xapp-direct",
        ASTROCLAW_MANTIS_SLACK_BOT_TOKEN: "xoxb-direct",
        PATH: process.env.PATH,
      },
      gatewaySetup: true,
