...
const DEFAULT_BROWSER_URL = "https://astroclaw.ai";
...
const BROWSER_PROFILE_ARCHIVE_ENV = "ASTROCLAW_MANTIS_BROWSER_PROFILE_TGZ_B64";
const BROWSER_PROFILE_DIR_ENV = "ASTROCLAW_MANTIS_BROWSER_PROFILE_DIR";
...
if [ -n "$profile_archive_b64" ]; then
  profile_archive="$profile/astroclaw-mantis-browser-profile.tgz"
...
export async function runMantisDesktopBrowserSmoke(
  ...
  const browserProfileArchiveEnv =
    ...
    trimToValue(env.ASTROCLAW_MANTIS_BROWSER_PROFILE_ARCHIVE_ENV) ??
  ...
  const remoteOutputDir = `/tmp/astroclaw-mantis-desktop-${startedAt
    .toISOString()
