...
import { createSlackWebClient, createSlackWriteClient } from "@astroclaw/slack/api.js";
...
const SLACK_QA_CAPTURE_CONTENT_ENV = "ASTROCLAW_QA_SLACK_CAPTURE_CONTENT";
...
const SLACK_QA_SCENARIOS: SlackQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    timeoutMs: 8_000,
    configOverrides: { users: ["U_ASTROCLAW_QA_NEVER_ALLOWED"] },
    buildRun: (sutUserId) => {
...
function resolveSlackQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): SlackQaRuntimeEnv {
  const runtimeEnv = {
    channelId: resolveEnvValue(env, "ASTROCLAW_QA_SLACK_CHANNEL_ID"),
    driverBotToken: resolveEnvValue(env, "ASTROCLAW_QA_SLACK_DRIVER_BOT_TOKEN"),
    sutBotToken: resolveEnvValue(env, "ASTROCLAW_QA_SLACK_SUT_BOT_TOKEN"),
    sutAppToken: resolveEnvValue(env, "ASTROCLAW_QA_SLACK_SUT_APP_TOKEN"),
  };
  return validateSlackQaRuntimeEnv(runtimeEnv, "ASTROCLAW_QA_SLACK");
}
