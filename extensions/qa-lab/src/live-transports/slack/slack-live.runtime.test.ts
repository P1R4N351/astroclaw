...
describe("Slack live QA runtime helpers", () => {
  it("resolves env credential payloads", () => {
    expect(
      __testing.resolveSlackQaRuntimeEnv({
        ASTROCLAW_QA_SLACK_CHANNEL_ID: "C123456789",
        ASTROCLAW_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        ASTROCLAW_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        ASTROCLAW_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toEqual({
  ...
  it("rejects malformed Slack channel ids", () => {
    expect(() =>
      __testing.resolveSlackQaRuntimeEnv({
        ASTROCLAW_QA_SLACK_CHANNEL_ID: "qa-channel",
        ASTROCLAW_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        ASTROCLAW_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        ASTROCLAW_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toThrow("ASTROCLAW_QA_SLACK channelId must be a Slack id like C123 or U123.");
  });
  ...
  it("writes artifacts when Convex credential acquisition fails", async () => {
    const outputDir = await fs.mkdtemp(path.join(tmpdir(), "astroclaw-slack-qa-"));
    ...
    expect(result.scenarios[0]?.details).toContain("Missing ASTROCLAW_QA_CONVEX_SITE_URL");
