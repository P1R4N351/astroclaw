...
describe("WhatsApp QA live runtime", () => {
  ...
  it("unpacks auth archives into a caller-provided temp directory", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-wa-qa-test-"));
    try {
  ...
  it("uses automatic visible replies for WhatsApp group mention gating", () => {
    ...
    expect(scenarioRun.input).toContain("astroclawqa reply with only this exact marker");
    ...
    const cfg = __testing.buildWhatsAppQaConfig(
      {},
      {
        ...
        authDir: "/tmp/astroclaw-whatsapp-qa-auth",
        dmPolicy: "allowlist",
    ...
    expect(cfg.messages?.groupChat?.mentionPatterns).toContain("\\bastroclawqa\\b");
  });
