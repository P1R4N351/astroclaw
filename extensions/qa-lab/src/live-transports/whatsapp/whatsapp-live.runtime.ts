...
import { startWhatsAppQaDriverSession } from "@astroclaw/whatsapp/api.js";
import { normalizeE164 } from "astroclaw/plugin-sdk/account-resolution";
...
const WHATSAPP_QA_CAPTURE_CONTENT_ENV = "ASTROCLAW_QA_WHATSAPP_CAPTURE_CONTENT";
const QA_REDACT_PUBLIC_METADATA_ENV = "ASTROCLAW_QA_REDACT_PUBLIC_METADATA";
const WHATSAPP_QA_ENV_KEYS = [
  "ASTROCLAW_QA_WHATSAPP_DRIVER_PHONE_E164",
  "ASTROCLAW_QA_WHATSAPP_SUT_PHONE_E164",
  "ASTROCLAW_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64",
  "ASTROCLAW_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64",
] as const;
...
const WHATSAPP_QA_SCENARIOS: WhatsAppQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    buildRun: () => ({
      ...
      matchText: /Astroclaw: access not configured|Pairing code:/iu,
      target: "dm",
    }),
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: true,
        input: `astroclawqa reply with only this exact marker: ${replyToken}`,
...
function inferWhatsAppCredentialSource(
  ...
  const normalized =
    value?.trim().toLowerCase() || env.ASTROCLAW_QA_CREDENTIAL_SOURCE?.trim().toLowerCase();
...
function resolveWhatsAppQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): WhatsAppQaRuntimeEnv {
  return validateWhatsAppQaRuntimeEnv(
    {
      driverPhoneE164: resolveEnvValue(env, "ASTROCLAW_QA_WHATSAPP_DRIVER_PHONE_E164"),
      sutPhoneE164: resolveEnvValue(env, "ASTROCLAW_QA_WHATSAPP_SUT_PHONE_E164"),
      driverAuthArchiveBase64: resolveEnvValue(
        env,
        "ASTROCLAW_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64",
      ),
      sutAuthArchiveBase64: resolveEnvValue(env, "ASTROCLAW_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64"),
      groupJid: env.ASTROCLAW_QA_WHATSAPP_GROUP_JID?.trim() || undefined,
    },
    "ASTROCLAW_QA_WHATSAPP",
  );
}
...
export async function runWhatsAppQaLive(params: {
  ...
  try {
    ...
    tempAuthRoot = await fs.mkdtemp(
      path.join(resolvePreferredAstroclawTmpDir(), "astroclaw-whatsapp-qa-"),
    );
