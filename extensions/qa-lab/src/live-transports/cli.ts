import { listQaRunnerCliContributions } from "astroclaw/plugin-sdk/qa-runner-runtime";
  ...
  return {
    ...
    register(qa) {
      qa.command(params.commandName)
        ...
        .action(() => {
          throw new Error(
            `QA runner "${params.commandName}" is installed but not active. Enable or allow plugin "${params.pluginId}" in your Astroclaw config, then try again.`,
          );
        });
    },
  };
