# @astroclaw/memory-wiki

Persistent wiki compiler and Obsidian-friendly knowledge vault for **Astroclaw**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.astroclaw-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
- The compiled agent digests in `.astroclaw-wiki/cache/agent-digest.json` and `.astroclaw-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
