...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-astroclaw-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-astroclaw-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.astroclawDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.astroclawDiffsError = "true";
