...
function ensureAstroclawExtensionAlias(params) {
  const pluginSdkDir = path.join(params.repoRoot, "dist", "plugin-sdk");
  if (!fs.existsSync(pluginSdkDir)) {
    return;
  }

  const aliasDir = path.join(params.distExtensionsRoot, "node_modules", "astroclaw");
...
function shouldCopyRuntimeFile(relativePath) {
  return (
    ...
    isPathOrNestedPath(relativePath, "astroclaw.plugin.json") ||
...
export function stageBundledPluginRuntime(params = {}) {
  ...
  ensureAstroclawExtensionAlias({ repoRoot, distExtensionsRoot });
