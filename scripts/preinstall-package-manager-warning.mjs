...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[astroclaw] warning: detected ${packageManager} for install lifecycle.`,
    "[astroclaw] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[astroclaw] prefer: corepack pnpm install",
  ].join("\n");
}
