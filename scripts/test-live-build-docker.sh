...
DOCKER_BUILD_EXTENSIONS="${ASTROCLAW_DOCKER_BUILD_EXTENSIONS:-${ASTROCLAW_EXTENSIONS:-}}"
...
if [[ -n "${DOCKER_BUILD_EXTENSIONS}" ]]; then
  DOCKER_BUILD_ARGS+=(--build-arg "ASTROCLAW_EXTENSIONS=${DOCKER_BUILD_EXTENSIONS}")
fi

if [[ "${ASTROCLAW_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  ...
  echo "==> Live-test image not available; building because ASTROCLAW_DOCKER_BUILD_ON_MISSING/ASTROCLAW_TESTBOX allows fallback."
fi
