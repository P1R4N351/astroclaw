...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.ASTROCLAW_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[astroclaw] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[astroclaw] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[astroclaw] This usually means a file in node_modules is empty or truncated.");
  console.error("[astroclaw] Recommended recovery:");
  console.error("[astroclaw]   rm -rf node_modules");
  console.error("[astroclaw]   pnpm store prune");
  console.error("[astroclaw]   pnpm install");
  console.error("");
  console.error("[astroclaw] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.ASTROCLAW_WATCH_MODE = "1";
  childEnv.ASTROCLAW_WATCH_SESSION = watchSession;
  ...
  childEnv.ASTROCLAW_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.ASTROCLAW_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `astroclaw doctor --fix --non-interactive` once.",
        deps,
      );
