...
start_gateway() {
  GATEWAY_PID="$(astroclaw_e2e_start_gateway "$ASTROCLAW_ENTRY" 18789 /tmp/gateway-e2e.log)"
}

wait_for_gateway() {
  for _ in $(seq 1 20); do
    if astroclaw_e2e_probe_tcp 127.0.0.1 18789 500 >/dev/null 2>&1; then
      return 0
    fi
...
run_wizard_cmd() {
  ...
  set_isolated_astroclaw_env "$state_ref"

  input_fifo="$(mktemp -u "/tmp/astroclaw-onboard-${case_name}.XXXXXX")"
  mkfifo "$input_fifo"
  local log_path="/tmp/astroclaw-onboard-${case_name}.log"
...
run_wizard() {
  ...
  run_wizard_cmd "$case_name" "$state_ref" "node \"$ASTROCLAW_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
}

assert_onboard_config() {
  local scenario="$1"
  shift
  astroclaw_e2e_assert_file "$ASTROCLAW_CONFIG_PATH"
  node scripts/e2e/lib/onboard/assert-config.mjs "$scenario" "$ASTROCLAW_CONFIG_PATH" "$@"
}

set_isolated_astroclaw_env() {
  local state_ref="$1"
  astroclaw_test_state_create "$state_ref" empty
}
...
run_case_local_basic() {
  set_isolated_astroclaw_env local-basic
  astroclaw_e2e_run_logged local-basic node "$ASTROCLAW_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
  ...
  workspace_dir="$ASTROCLAW_STATE_DIR/workspace"
  sessions_dir="$ASTROCLAW_STATE_DIR/agents/main/sessions"

  astroclaw_e2e_assert_dir "$sessions_dir"
  for file in AGENTS.md BOOTSTRAP.md IDENTITY.md SOUL.md TOOLS.md USER.md; do
    astroclaw_e2e_assert_file "$workspace_dir/$file"
  done
...
run_case_remote_non_interactive() {
  set_isolated_astroclaw_env remote-non-interactive
  # Smoke test non-interactive remote config write.
  astroclaw_e2e_run_logged remote-non-interactive node "$ASTROCLAW_ENTRY" onboard --non-interactive --accept-risk \
    --mode remote \
...
run_case_reset() {
  set_isolated_astroclaw_env reset-config
  node scripts/e2e/lib/onboard/write-config.mjs reset "$ASTROCLAW_CONFIG_PATH"

  astroclaw_e2e_run_logged reset-config node "$ASTROCLAW_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
...
run_case_channels() {
  ...
  run_wizard_cmd channels channels "node \"$ASTROCLAW_ENTRY\" configure --section channels" send_channels_flow
...
run_case_skills() {
  local home_dir
  set_isolated_astroclaw_env skills
  home_dir="$HOME"
  node scripts/e2e/lib/onboard/write-config.mjs skills "$ASTROCLAW_CONFIG_PATH"

  run_wizard_cmd skills "$home_dir" "node \"$ASTROCLAW_ENTRY\" configure --section skills" send_skills_flow
...
validate_local_basic_log() {
  local log_path="$1"
  astroclaw_e2e_assert_log_not_contains "$log_path" "systemctl --user unavailable"
}
