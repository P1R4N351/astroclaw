...
function scanLogs() {
  const roots = ["/tmp", path.join(process.env.HOME, ".astroclaw")];
  const files = [];
  const visit = (entry) => {
    ...
    if (/\.(?:log|jsonl)$/u.test(entry) || /astroclaw-kitchen-sink-/u.test(path.basename(entry))) {
...
function assertClawHubExternalInstallContract(installPath) {
  const astroclawPeerPath = path.join(installPath, "node_modules", "astroclaw");
  if (!fs.existsSync(astroclawPeerPath)) {
    throw new Error(`missing kitchen-sink astroclaw peer symlink: ${astroclawPeerPath}`);
  }
  if (!fs.lstatSync(astroclawPeerPath).isSymbolicLink()) {
    throw new Error(`kitchen-sink astroclaw peer is not a symlink: ${astroclawPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(astroclawPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected kitchen-sink astroclaw peer ${linkedHostRoot} to target ${hostRoot}`);
  }
