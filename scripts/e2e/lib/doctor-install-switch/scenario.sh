...
cp scripts/e2e/lib/doctor-install-switch/shims/systemctl /tmp/astroclaw-bin/systemctl
cp scripts/e2e/lib/doctor-install-switch/shims/loginctl /tmp/astroclaw-bin/loginctl
chmod +x /tmp/astroclaw-bin/systemctl /tmp/astroclaw-bin/loginctl

package_tgz="${ASTROCLAW_CURRENT_PACKAGE_TGZ:?missing ASTROCLAW_CURRENT_PACKAGE_TGZ}"
git_root="/tmp/astroclaw-git"
mkdir -p "$git_root"
...
(
  cd "$git_root"
  npm install --omit=optional --no-fund --no-audit >/tmp/astroclaw-git-install.log 2>&1
  git init -q
  git config user.email "docker-e2e@astroclaw.local"
  git config user.name "Astroclaw Docker E2E"
  git add -A
  git commit -qm "test fixture"
)
npm_log="/tmp/astroclaw-doctor-switch-npm-install.log"
if ! npm install -g --prefix /tmp/npm-prefix --omit=optional "$package_tgz" >"$npm_log" 2>&1; then
  cat "$npm_log"
  exit 1
fi

npm_bin="/tmp/npm-prefix/bin/astroclaw"
npm_root="/tmp/npm-prefix/lib/node_modules/astroclaw"
...
git_cli="$git_root/astroclaw.mjs"
...
run_flow() {
  ...
  local install_log="/tmp/astroclaw-doctor-switch-${name}-install.log"
  local doctor_log="/tmp/astroclaw-doctor-switch-${name}-doctor.log"
  local command_timeout="${ASTROCLAW_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"

  echo "== Flow: $name =="
  astroclaw_test_state_create "switch-${name}" empty
  ...
  unit_path="$HOME/.config/systemd/user/astroclaw-gateway.service"
  if [ ! -f "$unit_path" ]; then
...
run_flow \
  ...
  "$npm_entry" \
  "ASTROCLAW_UPDATE_IN_PROGRESS=1 node $git_cli doctor --repair --force --yes --non-interactive" \
  "$git_entry"
...
run_flow \
  ...
  "$git_entry" \
  "ASTROCLAW_UPDATE_IN_PROGRESS=1 $npm_bin doctor --repair --force --yes --non-interactive" \
  "$npm_entry"
...
run_proxy_env_flow() {
  ...
  if ! timeout "$command_timeout" env ASTROCLAW_UPDATE_IN_PROGRESS=1 \
...
run_wrapper_flow() {
  ...
  local install_log="/tmp/astroclaw-doctor-switch-${name}-install.log"
  local reinstall_log="/tmp/astroclaw-doctor-switch-${name}-reinstall.log"
  local env_repair_log="/tmp/astroclaw-doctor-switch-${name}-env-repair.log"
  local doctor_log="/tmp/astroclaw-doctor-switch-${name}-doctor.log"
  local clear_log="/tmp/astroclaw-doctor-switch-${name}-clear.log"
  local command_timeout="${ASTROCLAW_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"
  ...
  local wrapper="$HOME/.local/bin/astroclaw-wrapper"
  node scripts/e2e/lib/doctor-install-switch/write-wrapper.mjs \
    "$wrapper" \
    "$npm_bin" \
    "$HOME/astroclaw-wrapper-argv.log"

  local unit_path="$HOME/.config/systemd/user/astroclaw-gateway.service"
  ...
  assert_env_value "$unit_path" "ASTROCLAW_WRAPPER" "$wrapper"
  ...
  sed -i "/^Environment=ASTROCLAW_WRAPPER=/d" "$unit_path"
  ...
  sed -i "s#^Environment=ASTROCLAW_WRAPPER=.*#Environment=ASTROCLAW_WRAPPER=/tmp/stale-astroclaw-wrapper#" "$unit_path"
  if ! timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" >"$env_repair_log" 2>&1; then
    cat "$env_repair_log"
    exit 1
  fi
  ...
  if ! grep -Fq "Gateway service invokes ASTROCLAW_WRAPPER:" "$doctor_log"; then
  ...
  if ! timeout "$command_timeout" env ASTROCLAW_WRAPPER= "$npm_bin" gateway install --force >"$clear_log" 2>&1; then
    cat "$clear_log"
    exit 1
  fi
  assert_no_env_key "$unit_path" "ASTROCLAW_WRAPPER"
