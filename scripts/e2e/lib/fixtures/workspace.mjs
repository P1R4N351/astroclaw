...
function writeOpenWebUiWorkspace() {
  const workspace =
    process.env.ASTROCLAW_WORKSPACE_DIR || path.join(process.env.HOME, ".astroclaw", "workspace");
  write(
...
function writeAgentsDeleteConfig() {
  const stateDir = requireArg(process.env.ASTROCLAW_STATE_DIR, "ASTROCLAW_STATE_DIR");
  ...
  writeJson(path.join(stateDir, "astroclaw.json"), {
    agents: {
      list: [
...
function assertAgentsDeleteResult([outputPath]) {
  ...
  const remaining =
    readJson(path.join(process.env.ASTROCLAW_STATE_DIR, "astroclaw.json"))?.agents?.list ?? [];
