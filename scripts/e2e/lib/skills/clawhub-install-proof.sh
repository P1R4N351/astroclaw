...
source "$ROOT_DIR/scripts/lib/astroclaw-e2e-instance.sh"

ASTROCLAW_TEST_STATE_SCRIPT_B64="${ASTROCLAW_TEST_STATE_SCRIPT_B64:-}"
if [ -n "$ASTROCLAW_TEST_STATE_SCRIPT_B64" ]; then
  astroclaw_e2e_eval_test_state_from_b64 "$ASTROCLAW_TEST_STATE_SCRIPT_B64"
else
  export HOME="$(mktemp -d "${TMPDIR:-/tmp}/astroclaw-skill-install-home.XXXXXX")"
  export USERPROFILE="$HOME"
  export ASTROCLAW_HOME="$HOME"
  export ASTROCLAW_STATE_DIR="$HOME/.astroclaw"
...
if [ -n "${ASTROCLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
  ...
  astroclaw_e2e_install_package /tmp/astroclaw-skill-install-npm.log
fi

if [ -n "${ASTROCLAW_CURRENT_PACKAGE_TGZ:-}" ] && command -v astroclaw >/dev/null 2>&1; then
  ASTROCLAW_CMD=(astroclaw)
elif command -v pnpm >/dev/null 2>&1 && [ -f package.json ]; then
  if [ "${ASTROCLAW_SKILL_INSTALL_E2E_BUILD_SOURCE:-0}" = "1" ]; then
    pnpm build >/tmp/astroclaw-skill-install-build.log 2>&1
  fi
  ASTROCLAW_CMD=(pnpm --silent astroclaw)
elif command -v astroclaw >/dev/null 2>&1; then
  ASTROCLAW_CMD=(astroclaw)
else
  echo "astroclaw command not found; install package first or run from repo with pnpm" >&2
  exit 1
fi

mkdir -p "$(dirname "$ASTROCLAW_CONFIG_PATH")"
node --input-type=module - "$ASTROCLAW_CONFIG_PATH" <<'NODE'
...
query="${ASTROCLAW_SKILL_INSTALL_E2E_QUERY:-homeassistant}"
requested_slug="${ASTROCLAW_SKILL_INSTALL_E2E_SLUG:-}"
preferred_slug="${ASTROCLAW_SKILL_INSTALL_E2E_PREFERRED_SLUG:-homeassistant-skill}"
search_json="/tmp/astroclaw-skill-install-search.json"
resolve_json="/tmp/astroclaw-skill-install-resolved.json"
install_log="/tmp/astroclaw-skill-install.log"
info_json="/tmp/astroclaw-skill-install-info.json"

echo "Searching live ClawHub skills for: $query"
"${ASTROCLAW_CMD[@]}" skills search "$query" --limit 8 --json >"$search_json"
...
if ! "${ASTROCLAW_CMD[@]}" skills install "$slug" --force >"$install_log" 2>&1; then
  echo "Skill install failed" >&2
  astroclaw_e2e_dump_logs /tmp/astroclaw-skill-install-npm.log "$search_json" "$resolve_json" "$install_log"
  exit 1
fi

workspace_dir="$HOME/.astroclaw/workspace"
skill_dir="$workspace_dir/skills/$slug"
origin_json="$skill_dir/.clawhub/origin.json"
lock_json="$workspace_dir/.clawhub/lock.json"

astroclaw_e2e_assert_file "$skill_dir/SKILL.md"
astroclaw_e2e_assert_file "$origin_json"
astroclaw_e2e_assert_file "$lock_json"

"${ASTROCLAW_CMD[@]}" skills info "$slug" --json >"$info_json"

node --input-type=module - "$ASTROCLAW_CONFIG_PATH" "$skill_dir" "$origin_json" "$lock_json" "$info_json" "$slug" <<'NODE'
import fs from "node:fs";
