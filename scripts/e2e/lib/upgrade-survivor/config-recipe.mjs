    ...
    [
      {
        id: "logging-file",
        intent: "logging",
        argv: ["config", "set", "logging.file", "~/astroclaw-upgrade-survivor/gateway.jsonl"],
      },
    ],
  ],
  [
...
function selectedScenario() {
  return process.env.ASTROCLAW_UPGRADE_SURVIVOR_SCENARIO || "base";
}
...
function runAstroclaw(step) {
  const result = spawnSync("astroclaw", step.argv, {
    encoding: "utf8",
    env: process.env,
  });
  return {
    id: step.id,
    intent: step.intent,
    command: ["astroclaw", ...step.argv].join(" "),
...
function applyRecipe() {
  ...
  for (const step of [...recipe.slice(0, -1), ...scenarioSteps, recipe.at(-1)]) {
    ...
    const outcome = runAstroclaw(adaptedStep);
