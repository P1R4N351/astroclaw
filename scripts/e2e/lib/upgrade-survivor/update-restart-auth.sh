...
write_update_restart_service_auth_env() {
  ...
  printf 'GATEWAY_AUTH_TOKEN_REF=%s\n' "$GATEWAY_AUTH_TOKEN_REF" >"$ASTROCLAW_STATE_DIR/gateway.systemd.env"
}
...
prepare_update_restart_probe_current_install() {
  ...
  env -u ASTROCLAW_GATEWAY_TOKEN -u ASTROCLAW_GATEWAY_PASSWORD astroclaw gateway --port "$port" --bind loopback --allow-unconfigured >"$log_file" 2>&1 &
  gateway_pid="$!"
  printf '%s\n' "$gateway_pid" >"$ASTROCLAW_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE"
  astroclaw_e2e_wait_gateway_ready "$gateway_pid" "$log_file" 360
  ready_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  start_seconds=$(((ready_epoch - start_epoch + 999) / 1000))
  write_update_restart_service_auth_env
  if ! env -u ASTROCLAW_GATEWAY_TOKEN -u ASTROCLAW_GATEWAY_PASSWORD astroclaw gateway install --force --json >"$ASTROCLAW_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_JSON" 2>"$ASTROCLAW_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_ERR"; then
    echo "gateway service install failed" >&2
    cat "$ASTROCLAW_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_ERR" >&2 || true
    cat "$ASTROCLAW_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_JSON" >&2 || true
    return 1
  fi
}
