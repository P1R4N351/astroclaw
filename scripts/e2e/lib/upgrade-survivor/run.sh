...
export ASTROCLAW_SKIP_PROVIDERS=1
export ASTROCLAW_SKIP_CHANNELS=1
export ASTROCLAW_DISABLE_BONJOUR=1
export GATEWAY_AUTH_TOKEN_REF="upgrade-survivor-token"
export OPENAI_API_KEY="sk-astroclaw-upgrade-survivor"
...
ARTIFACT_ROOT="$(dirname "${ASTROCLAW_UPGRADE_SURVIVOR_SUMMARY_JSON:-/tmp/astroclaw-upgrade-survivor-artifacts/summary.json}")"
...
SUMMARY_JSON="${ASTROCLAW_UPGRADE_SURVIVOR_SUMMARY_JSON:-$ARTIFACT_ROOT/summary.json}"
PHASE_LOG="$ARTIFACT_ROOT/phases.jsonl"
BASELINE_RAW="${ASTROCLAW_UPGRADE_SURVIVOR_BASELINE:?missing ASTROCLAW_UPGRADE_SURVIVOR_BASELINE}"
CANDIDATE_KIND="${ASTROCLAW_UPGRADE_SURVIVOR_CANDIDATE_KIND:-tarball}"
CANDIDATE_SPEC="${ASTROCLAW_UPGRADE_SURVIVOR_CANDIDATE_SPEC:-${ASTROCLAW_CURRENT_PACKAGE_TGZ:-}}"
SCENARIO="${ASTROCLAW_UPGRADE_SURVIVOR_SCENARIO:-base}"
UPDATE_RESTART_MODE="${ASTROCLAW_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE:-manual}"
ROOT_MANAGED_VPS="${ASTROCLAW_UPGRADE_SURVIVOR_ROOT_MANAGED_VPS:-0}"
CURRENT_PHASE="setup"
FAILURE_PHASE=""
FAILURE_MESSAGE=""
gateway_pid=""
...
export ASTROCLAW_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON="$CONFIG_COVERAGE_JSON"
rm -f "$SUMMARY_JSON" "$CONFIG_COVERAGE_JSON"
: >"$PHASE_LOG"

validate_baseline_package_spec() {
  local spec="$1"
  if [[ "$spec" =~ ^astroclaw@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$ ]]; then
    return 0
  fi
  echo "ASTROCLAW_UPGRADE_SURVIVOR_BASELINE must be astroclaw@latest, astroclaw@beta, astroclaw@alpha, an exact Astroclaw release version, or a bare release version; got: $spec" >&2
  return 1
}
...
normalize_baseline() {
  ...
  if [ -z "$raw" ]; then
    echo "ASTROCLAW_UPGRADE_SURVIVOR_BASELINE cannot be empty" >&2
    return 1
  fi
  case "$raw" in
    astroclaw@*)
      baseline_spec="$raw"
      baseline_version="${raw#astroclaw@}"
      ;;
    *@*)
      echo "ASTROCLAW_UPGRADE_SURVIVOR_BASELINE must be astroclaw@<version> or a bare version" >&2
      return 1
      ;;
    *)
      baseline_version="$raw"
      baseline_spec="astroclaw@$raw"
      ;;
  esac
  case "$baseline_version" in
    latest | beta | alpha)
      ...
      ;;
    dev | main | "")
      echo "ASTROCLAW_UPGRADE_SURVIVOR_BASELINE must be astroclaw@latest, astroclaw@beta, astroclaw@alpha, astroclaw@<version>, or a bare version" >&2
      return 1
      ;;
    *)
...
validate_update_restart_mode() {
  case "$UPDATE_RESTART_MODE" in
    manual | auto-auth)
      ;;
    *)
      echo "ASTROCLAW_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE must be manual or auto-auth; got: $UPDATE_RESTART_MODE" >&2
      return 1
      ;;
  esac
}
...
const summary = {
  ...
  candidate: {
    kind: process.env.ASTROCLAW_UPGRADE_SURVIVOR_CANDIDATE_KIND || null,
    spec: process.env.ASTROCLAW_UPGRADE_SURVIVOR_CANDIDATE_SPEC || process.env.ASTROCLAW_CURRENT_PACKAGE_TGZ || null,
...
cleanup() {
  ...
  if [ -s "$SYSTEMCTL_SHIM_PID_FILE" ]; then
    ...
    if [[ "$shim_pid" =~ ^[0-9]+$ ]] && [ "$shim_pid" -gt 1 ]; then
      astroclaw_e2e_terminate_gateways "$shim_pid"
    fi
  fi
}
...
legacy_runtime_deps_symlink_plugin() {
  local plugin="${ASTROCLAW_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK:-}"
  if [ -z "$plugin" ]; then
    return 1
  fi
  case "$plugin" in
    *[!A-Za-z0-9._-]*)
      echo "ASTROCLAW_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK must be a plugin id, got: $plugin" >&2
      return 2
      ;;
  esac
  printf '%s\n' "$plugin"
...
legacy_runtime_deps_symlink_target() {
  local plugin="$1"
  printf '%s/@astroclaw-upgrade-survivor/%s-runtime-dep\n' "$(dirname "$(package_root)")" "$plugin"
}
...
plugin_deps_cleanup_plugins() {
  printf '%s\n' "${ASTROCLAW_UPGRADE_SURVIVOR_PLUGIN_DEPS_CLEANUP_PLUGINS:-discord telegram}"
}
...
seed_source_only_plugin_shadow() {
  ...
  local shadow_root="$ASTROCLAW_STATE_DIR/extensions/opik-astroclaw"
  mkdir -p "$shadow_root/src"
  cat >"$shadow_root/package.json" <<'JSON'
{
  "name": "@opik/opik-astroclaw",
...
JSON
  cat >"$shadow_root/astroclaw.plugin.json" <<'JSON'
{
  "id": "opik-astroclaw",
  "activation": {
    "onStartup": false
...
configure_configured_plugin_install_fixture_registry() {
  ...
  local tarball="$fixture_root/astroclaw-brave-plugin-2026.5.2.tgz"
...
fs.writeFileSync(
  path.join(root, "astroclaw.plugin.json"),
  `${JSON.stringify(
    {
      id: "brave",
...
legacy_plugin_dependency_probe_paths() {
  ...
  while IFS= read -r plugin_dir; do
    printf '%s\n' \
      ...
      "$plugin_dir/.astroclaw-runtime-deps.json" \
      "$plugin_dir/.astroclaw-runtime-deps-stamp.json" \
      "$plugin_dir/.astroclaw-runtime-deps-copy-upgrade-survivor" \
      "$plugin_dir/.astroclaw-install-stage-upgrade-survivor" \
      "$plugin_dir/.astroclaw-pnpm-store"
  done < <(plugin_deps_cleanup_plugin_dirs "$plugin")
  printf '%s\n' \
    "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$ASTROCLAW_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$ASTROCLAW_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor"
}
...
seed_legacy_plugin_dependency_debris() {
  ...
  for plugin in $(plugin_deps_cleanup_plugins); do
    ...
    found=1
    mkdir -p \
      "$plugin_dir/node_modules/astroclaw-upgrade-survivor-dep" \
      "$plugin_dir/.astroclaw-runtime-deps-copy-upgrade-survivor/node_modules/astroclaw-upgrade-survivor-dep" \
      "$plugin_dir/.astroclaw-install-stage-upgrade-survivor" \
      "$plugin_dir/.astroclaw-pnpm-store" \
      "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/astroclaw-upgrade-survivor-dep" \
      "$ASTROCLAW_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/astroclaw-upgrade-survivor-dep" \
      "$ASTROCLAW_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/astroclaw-upgrade-survivor-dep"
    printf '{"name":"astroclaw-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/node_modules/astroclaw-upgrade-survivor-dep/package.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup"}\n' "$plugin" \
      >"$plugin_dir/.astroclaw-runtime-deps.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup","stale":true}\n' "$plugin" \
      >"$plugin_dir/.astroclaw-runtime-deps-stamp.json"
    printf '{"name":"astroclaw-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/.astroclaw-runtime-deps-copy-upgrade-survivor/node_modules/astroclaw-upgrade-survivor-dep/package.json"
    printf '{"name":"astroclaw-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/astroclaw-upgrade-survivor-dep/package.json"
    printf '{"name":"astroclaw-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$ASTROCLAW_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/astroclaw-upgrade-survivor-dep/package.json"
    printf '{"name":"astroclaw-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$ASTROCLAW_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/astroclaw-upgrade-survivor-dep/package.json"
...
seed_legacy_runtime_deps_symlink() {
  ...
  printf '{"name":"astroclaw-upgrade-survivor-legacy-runtime-deps","version":"0.0.0"}\n' \
...
install_baseline() {
  ...
  if ! command -v astroclaw >/dev/null; then
    echo "baseline install did not expose astroclaw on PATH" >&2
    echo "PATH=$PATH" >&2
  ...
  if ! version_output="$(astroclaw --version 2>&1)"; then
    echo "baseline astroclaw --version failed" >&2
    echo "$version_output" >&2
    return 1
  fi
  if [[ "$version_output" != *"$baseline_version"* ]]; then
    echo "baseline astroclaw --version mismatch: expected output to include $baseline_version" >&2
    echo "$version_output" >&2
    return 1
  fi
}
...
seed_state() {
  astroclaw_e2e_eval_test_state_from_b64 "${ASTROCLAW_TEST_STATE_FUNCTION_B64:?missing ASTROCLAW_TEST_STATE_FUNCTION_B64}"
  if [ "$ROOT_MANAGED_VPS" = "1" ]; then
    if [ "$(id -u)" -ne 0 ]; then
      echo "root-managed VPS survivor mode must run as uid 0" >&2
      return 1
    fi
    rm -rf /root/.astroclaw /root/workspace
    astroclaw_test_state_create /root minimal
  else
    astroclaw_test_state_create "$ARTIFACT_ROOT/state-home" minimal
  fi
  export ASTROCLAW_UPGRADE_SURVIVOR_BASELINE_VERSION="$baseline_version"
...
validate_baseline_config() {
  if ! astroclaw config validate >"$BASELINE_CONFIG_VALIDATE_LOG" 2>&1; then
...
log_file="${ASTROCLAW_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_LOG:-/tmp/astroclaw-systemctl-shim.log}"
pid_file="${ASTROCLAW_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE:-/tmp/astroclaw-systemctl-shim.pid}"
daemon_log="${ASTROCLAW_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_DAEMON_LOG:-/tmp/astroclaw-systemctl-shim-gateway.log}"
printf '%s\n' "$*" >>"$log_file"
...
unit_path() {
  printf '%s/.config/systemd/user/astroclaw-gateway.service\n' "${HOME:?missing HOME}"
}
...
SHIM
  ...
  export ASTROCLAW_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_LOG="$SYSTEMCTL_SHIM_LOG"
  export ASTROCLAW_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE="$SYSTEMCTL_SHIM_PID_FILE"
  export ASTROCLAW_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_DAEMON_LOG="$SYSTEMCTL_SHIM_DAEMON_LOG"
  export PATH="$shim_dir:$PATH"
}

install_update_restart_service_unit() {
  if ! env -u ASTROCLAW_GATEWAY_TOKEN -u ASTROCLAW_GATEWAY_PASSWORD astroclaw gateway install --force --json >"$BASELINE_SERVICE_INSTALL_JSON" 2>"$BASELINE_SERVICE_INSTALL_ERR"; then
...
if (!stateDir) {
  throw new Error("missing ASTROCLAW_STATE_DIR");
}
...
write_update_restart_service_secretref_env() {
  mkdir -p "$ASTROCLAW_STATE_DIR"
  local dotenv_path="$ASTROCLAW_STATE_DIR/.env"
...
write_update_restart_service_auth_env() {
  ...
  printf 'GATEWAY_AUTH_TOKEN_REF=%s\n' "$GATEWAY_AUTH_TOKEN_REF" >>"$tmp_path"
  mv "$tmp_path" "$dotenv_path"
  local systemd_env_path="$ASTROCLAW_STATE_DIR/gateway.systemd.env"
...
assert_baseline_state() {
  ASTROCLAW_UPGRADE_SURVIVOR_ASSERT_STAGE=baseline \
...
resolve_candidate_version() {
  if [ -z "$CANDIDATE_SPEC" ]; then
    echo "missing ASTROCLAW_UPGRADE_SURVIVOR_CANDIDATE_SPEC" >&2
    return 1
  fi
  ...
  ASTROCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT="$(
    node scripts/e2e/lib/package-compat.mjs "$candidate_version"
  )"
  export ASTROCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT
}
...
update_candidate() {
  ...
  local update_env=(
    env
    -u ASTROCLAW_GATEWAY_TOKEN
    -u ASTROCLAW_GATEWAY_PASSWORD
    -u ASTROCLAW_ALLOW_ROOT
  )
  ...
  if [ "$ROOT_MANAGED_VPS" != "1" ]; then
    update_env+=(ASTROCLAW_ALLOW_ROOT=1)
  fi
  if ! "${update_env[@]}" astroclaw "${update_args[@]}" >"$UPDATE_JSON" 2>"$UPDATE_ERR"; then
    echo "astroclaw update failed" >&2
...
assert_root_managed_vps_cli_usable() {
  ...
  "${root_cli_env[@]}" astroclaw config file >"$ARTIFACT_ROOT/root-vps-config-file.out" 2>"$ARTIFACT_ROOT/root-vps-config-file.err"
  "${root_cli_env[@]}" astroclaw plugins >"$ARTIFACT_ROOT/root-vps-plugins.out" 2>"$ARTIFACT_ROOT/root-vps-plugins.err"
}

run_doctor() {
  if ! astroclaw doctor --fix --non-interactive >"$DOCTOR_LOG" 2>&1; then
    echo "astroclaw doctor failed" >&2
...
validate_post_doctor_config() {
  if ! astroclaw config validate >>"$DOCTOR_LOG" 2>&1; then
...
probe_gateway_endpoint() {
  ...
  if [ -n "${ASTROCLAW_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING:-}" ]; then
    args+=(--allow-failing "$ASTROCLAW_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING")
  fi
  args+=(--out "$out_file")
...
start_gateway() {
  local port=18789
  local budget="${ASTROCLAW_UPGRADE_SURVIVOR_START_BUDGET_SECONDS:-90}"
  local start_epoch
  local ready_epoch
  start_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  env -u ASTROCLAW_GATEWAY_TOKEN -u ASTROCLAW_GATEWAY_PASSWORD astroclaw gateway --port "$port" --bind loopback --allow-unconfigured >"$GATEWAY_LOG" 2>&1 &
  gateway_pid="$!"
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
    printf '%s\n' "$gateway_pid" >"$SYSTEMCTL_SHIM_PID_FILE"
  fi
  astroclaw_e2e_wait_gateway_ready "$gateway_pid" "$GATEWAY_LOG" 360
...
check_gateway_probes() {
  ...
  export ASTROCLAW_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING="discord,telegram,whatsapp,feishu,matrix"
  readyz_seconds="$(probe_gateway_endpoint /readyz ready "$READYZ_JSON")"
  unset ASTROCLAW_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING
}

check_gateway_status() {
  local port=18789
  local budget="${ASTROCLAW_UPGRADE_SURVIVOR_STATUS_BUDGET_SECONDS:-30}"
  local status_start
  local status_end
  status_start="$(node -e "process.stdout.write(String(Date.now()))")"
  if ! astroclaw gateway status --url "ws://127.0.0.1:$port" --token "$GATEWAY_AUTH_TOKEN_REF" --require-rpc --timeout 30000 --json >"$STATUS_JSON" 2>"$STATUS_ERR"; then
