...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/astroclaw-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/astroclaw-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${ASTROCLAW_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${ASTROCLAW_SECRET_ENV_NAME:-}" ] && [ -n "${ASTROCLAW_SECRET_FILE:-}" ] && [ -f "$ASTROCLAW_SECRET_FILE" ]; then
  secret_value="$(cat "$ASTROCLAW_SECRET_FILE")"
  rm -f "$ASTROCLAW_SECRET_FILE"
  export "${ASTROCLAW_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "ASTROCLAW_SECRET_ENV_NAME=$api_key_env" \
      "ASTROCLAW_SECRET_FILE=$secret_path" \
      "ASTROCLAW_WRAPPER_FILE=$wrapper_path" \
