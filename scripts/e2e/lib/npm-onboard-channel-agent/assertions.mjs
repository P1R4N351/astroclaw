...
function assertOnboardState() {
  ...
  if (!fs.existsSync(configPath)) {
    throw new Error("onboard did not write astroclaw.json");
  }
  ...
  if (authRaw.includes("sk-astroclaw-npm-onboard-e2e")) {
...
function assertAgentTurn() {
  ...
  const output = fs.readFileSync("/tmp/astroclaw-agent.combined", "utf8");
