...
function astroclawPath(...parts) {
  return path.join(home, ".astroclaw", ...parts);
}
...
function records() {
  const index = readJson(astroclawPath("plugins", "installs.json"));
...
function config() {
  return readJson(process.env.ASTROCLAW_CONFIG_PATH ?? astroclawPath("astroclaw.json"));
}
