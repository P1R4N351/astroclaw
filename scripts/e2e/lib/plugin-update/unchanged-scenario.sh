...
astroclaw_e2e_install_package /tmp/astroclaw-install.log "mounted Astroclaw package" /tmp/npm-prefix
...
ASTROCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT="$(node "$probe" legacy-compat "$package_version")"
...
node scripts/e2e/lib/plugin-update/registry-server.mjs >/tmp/astroclaw-e2e-registry.log 2>&1 &
registry_pid=$!
...
if ! node "$probe" wait-registry; then
  ...
  cat /tmp/astroclaw-e2e-registry.log || true
  exit 1
fi

before_config_hash=""
if [ "$ASTROCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT" != "1" ]; then
  before_config_hash="$(sha256sum "$ASTROCLAW_CONFIG_PATH" | awk '{print $1}')"
fi
plugin_update_timeout_seconds="${ASTROCLAW_PLUGIN_UPDATE_TIMEOUT_SECONDS:-180}"
...
if [ -n "$before_config_hash" ]; then
  after_config_hash="$(sha256sum "$ASTROCLAW_CONFIG_PATH" | awk '{print $1}')"
