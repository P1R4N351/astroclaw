...
const allowBetaCompatDiagnostics =
  process.env.ASTROCLAW_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS === "1";

function stateDir() {
  return process.env.ASTROCLAW_STATE_DIR || path.join(process.env.HOME, ".astroclaw");
}
...
function assertPlugin() {
  const spec = process.argv[3] || "npm:@astroclaw/codex";
  const list = readJson("/tmp/astroclaw-codex-plugins-list.json");
  const inspect = readJson("/tmp/astroclaw-codex-plugin-inspect.json");
...
function assertNpmDeps() {
  ...
  if (!fs.existsSync(pluginPackageJson)) {
    throw new Error(`missing npm-installed @astroclaw/codex package.json: ${pluginPackageJson}`);
  }
  ...
  if (pluginPackage.name !== "@astroclaw/codex") {
  ...
  if (!openAiCodexPackageJson) {
    throw new Error("missing @openai/codex dependency under .astroclaw/npm");
  }
...
function assertPreflight() {
  ...
  const output = fs.readFileSync("/tmp/astroclaw-codex-preflight.log", "utf8");
...
function assertAgentTurn() {
  ...
  const stdout = fs.readFileSync("/tmp/astroclaw-codex-agent.json", "utf8");
  const stderr = fs.existsSync("/tmp/astroclaw-codex-agent.err")
    ? fs.readFileSync("/tmp/astroclaw-codex-agent.err", "utf8")
    : "";
  const response = JSON.parse(stdout);
  const text = (response.payloads || []).map((payload) => payload?.text || "").join("\n");
  if (!text.includes(marker)) {
    throw new Error(
      `Astroclaw agent reply did not contain ${marker}:\nstdout=${stdout}\nstderr=${stderr}`,
    );
  }
  ...
  if (typeof entry.sessionFile !== "string" || !fs.existsSync(entry.sessionFile)) {
    throw new Error(`missing Astroclaw session file: ${entry.sessionFile}`);
  }
...
function assertUninstalled() {
  ...
  const list = readJson("/tmp/astroclaw-codex-plugins-list-after-uninstall.json");
  const plugin = (list.plugins || []).find((entry) => entry.id === "codex");
  if (plugin?.status === "loaded" || plugin?.enabled === true) {
...
function assertAgentError() {
  ...
  if (!Number.isInteger(status) || status === 0) {
    throw new Error(
      `expected Astroclaw agent to fail after Codex uninstall, got status ${process.argv[3]}`,
    );
  }
  const stdout = fs.existsSync("/tmp/astroclaw-codex-agent-after-uninstall.json")
    ? fs.readFileSync("/tmp/astroclaw-codex-agent-after-uninstall.json", "utf8")
    : "";
  const stderr = fs.existsSync("/tmp/astroclaw-codex-agent-after-uninstall.err")
    ? fs.readFileSync("/tmp/astroclaw-codex-agent-after-uninstall.err", "utf8")
    : "";
