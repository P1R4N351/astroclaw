...
function getInstallRecords() {
  ...
  const allowLegacyCompat = process.env.ASTROCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function assertPluginRemoved(params) {
  ...
  const config = readAstroclawConfig();
...
function recordFixturePluginTrust() {
  ...
  const ledgerPath = path.join(process.env.HOME, ".astroclaw", "plugins", "installs.json");
  const ledger = fs.existsSync(ledgerPath)
    ? readJson(ledgerPath)
    : {
        version: 1,
        warning:
          "DO NOT EDIT. This file is generated by Astroclaw plugin install/update/uninstall commands. Use `astroclaw plugins install/update/uninstall` instead.",
        records: {},
      };
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".astroclaw", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(astroclawPeerPath)) {
    throw new Error(`missing ClawHub astroclaw peer symlink: ${astroclawPeerPath}`);
  }
  if (!fs.lstatSync(astroclawPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub astroclaw peer is not a symlink: ${astroclawPeerPath}`);
  }
  ...
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub astroclaw peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@astroclaw/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@astroclaw/demo-plugin-npm") {
...
function assertInvalidAstroclawExtensionsRejected() {
  const pluginId = "demo-plugin-invalid-metadata";
  const output = fs.readFileSync("/tmp/plugins-invalid-astroclaw-extensions.log", "utf8");
  for (const expected of ["astroclaw.extensions[1]", "non-empty string"]) {
  ...
  const list = readJson("/tmp/plugins-invalid-astroclaw-extensions-list.json");
  ...
  const managedInstallPath = path.join(process.env.HOME, ".astroclaw", "extensions", pluginId);
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.ASTROCLAW_CLAWHUB_URL ||
    process.env.CLAWHUB_URL ||
    "https://clawhub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.ASTROCLAW_CLAWHUB_TOKEN ||
...
function assertClawHubInstalled() {
  ...
  const installPath = record.installPath.replace(/^~(?=$|\/)/u, process.env.HOME);
  const extensionsRoot = path.join(process.env.HOME, ".astroclaw", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".astroclaw", "astroclaw.json");
...
const commands = {
  ...
  "invalid-astroclaw-extensions": assertInvalidAstroclawExtensionsRejected,
