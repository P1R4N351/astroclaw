#!/usr/bin/env bash
# Runs Open WebUI against a Dockerized Astroclaw Gateway and verifies the proxied
...
IMAGE_NAME="$(docker_e2e_resolve_image "astroclaw-openwebui-e2e" ASTROCLAW_OPENWEBUI_E2E_IMAGE)"
...
MODEL="${ASTROCLAW_OPENWEBUI_MODEL:-openai/gpt-5.5}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${ASTROCLAW_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${ASTROCLAW_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${ASTROCLAW_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${ASTROCLAW_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${ASTROCLAW_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="astroclaw-openwebui-e2e-$$"
GW_NAME="astroclaw-openwebui-gateway-$$"
OW_NAME="astroclaw-openwebui-$$"
PROVIDER_TIMEOUT_SECONDS="${ASTROCLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS:-900}"
PROBE_FETCH_TIMEOUT_MS="${ASTROCLAW_OPENWEBUI_FETCH_TIMEOUT_MS:-$((PROVIDER_TIMEOUT_SECONDS * 1000 + 60000))}"
DOCKER_COMMAND_TIMEOUT="${ASTROCLAW_OPENWEBUI_DOCKER_COMMAND_TIMEOUT:-$((PROVIDER_TIMEOUT_SECONDS + 90))s}"
DOCKER_PULL_TIMEOUT="${ASTROCLAW_OPENWEBUI_DOCKER_PULL_TIMEOUT:-600s}"
...
PROFILE_FILE="${ASTROCLAW_TESTBOX_PROFILE_FILE:-$HOME/.astroclaw-testbox-live.profile}"
...
docker_e2e_docker_cmd run -d \
  ...
  -e "ASTROCLAW_OPENWEBUI_MODEL=$MODEL" \
  -e "ASTROCLAW_SKIP_CHANNELS=1" \
  -e "ASTROCLAW_SKIP_GMAIL_WATCHER=1" \
  -e "ASTROCLAW_SKIP_CRON=1" \
  -e "ASTROCLAW_SKIP_CANVAS_HOST=1" \
  -e "ASTROCLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS=$PROVIDER_TIMEOUT_SECONDS" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    entry="$(astroclaw_e2e_resolve_entrypoint)"

    openai_api_key="${OPENAI_API_KEY:?OPENAI_API_KEY required}"
    batch_file="$(mktemp /tmp/astroclaw-openwebui-config.XXXXXX.json)"
    ASTROCLAW_CONFIG_BATCH_PATH="$batch_file" node scripts/e2e/lib/fixture.mjs openwebui-config "$openai_api_key"
    node "$entry" config set --batch-file "$batch_file" >/dev/null
    rm -f "$batch_file"
    node scripts/e2e/lib/fixture.mjs openwebui-workspace

    astroclaw_e2e_exec_gateway "$entry" '"$PORT"' lan /tmp/openwebui-gateway.log
  ' >/dev/null

echo "Waiting for gateway HTTP surface..."
if ! docker_e2e_wait_container_bash "$GW_NAME" 240 1 "ASTROCLAW_HTTP_PROBE_BEARER='$TOKEN' node scripts/e2e/lib/openwebui/http-probe.mjs 'http://127.0.0.1:$PORT/v1/models' 200"; then
...
docker_e2e_docker_cmd run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Astroclaw E2E" \
  -e WEBUI_SECRET_KEY="astroclaw-openwebui-e2e-secret-key-v1" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Astroclaw E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="astroclaw/default" \
...
if ! docker_e2e_wait_container_bash "$GW_NAME" 90 5 "ASTROCLAW_HTTP_PROBE_BEARER='$TOKEN' ASTROCLAW_HTTP_PROBE_TIMEOUT_MS=8000 node scripts/e2e/lib/openwebui/http-probe.mjs 'http://$GW_NAME:$PORT/v1/models' 200"; then
...
echo "Running Open WebUI -> Astroclaw smoke..."
