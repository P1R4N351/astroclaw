...
const windowsStalePostSwapImportRegex = String.raw`node_modules\\astroclaw\\dist\\[^\\]+-[A-Za-z0-9_-]+\.js`;
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  ASTROCLAW_ALLOW_ROOT="\${ASTROCLAW_ALLOW_ROOT:-}" ${input.auth.apiKeyEnv}=${shellQuote(input.auth.apiKeyValue)} ${command} agent --local --agent main --session-id "$session_id" --message 'Reply with exact ASCII text OK only.' --thinking minimal --json >"$output_file" 2>&1
  rc=$?
  set -e
  cat "$output_file"
  if [ "$rc" -ne 0 ]; then
...
function windowsUpdateWithBundledPluginsDisabled(input: NpmUpdateScriptInput): string {
  return `$script:AstroclawUpdateExit = 0
$updateOutput = Invoke-WithScopedEnv @{ ASTROCLAW_DISABLE_BUNDLED_PLUGINS = '1'; ASTROCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1' } {
  Invoke-Astroclaw update --tag ${psSingleQuote(input.updateTarget)} --yes --json --no-restart 2>&1
  $script:AstroclawUpdateExit = $LASTEXITCODE
}
$updateExit = $script:AstroclawUpdateExit
$updateOutput`;
}

function windowsGatewayReadyScript(): string {
  return `function Wait-AstroclawGateway {
  $deadline = (Get-Date).AddSeconds(180)
  $attempt = 0
  while ((Get-Date) -lt $deadline) {
    Invoke-Astroclaw gateway status --deep --require-rpc --timeout 15000
    if ($LASTEXITCODE -eq 0) { return }
    $attempt += 1
    if ($attempt -eq 4) {
      Invoke-Astroclaw gateway start *>&1 | Out-Host
    }
    Start-Sleep -Seconds 5
  }
  throw "gateway did not become ready after update"
}
Invoke-Astroclaw gateway restart *>&1 | Out-Host
if ($LASTEXITCODE -ne 0) {
  "gateway restart exited with code $LASTEXITCODE; probing readiness before failing" | Out-Host
}
Wait-AstroclawGateway`;
}
...
$sessionPath = Join-Path $env:USERPROFILE '.astroclaw\\agents\\main\\sessions\\parallels-npm-update-windows.jsonl'
...
for ($attempt = 1; $attempt -le 2; $attempt++) {
  ...
  Remove-Item $sessionPath -Force -ErrorAction SilentlyContinue
  $output = Invoke-Astroclaw agent --local --agent main --session-id $sessionId --model ${psSingleQuote(input.auth.modelId)} --message 'Reply with exact ASCII text OK only.' --thinking minimal --timeout ${resolveParallelsModelTimeoutSeconds("windows")} --json 2>&1
...
path = Path.home() / ".astroclaw" / "astroclaw.json"
if not path.exists():
    raise SystemExit(0)
try:
...
stop_astroclaw_gateway_processes() {
  ASTROCLAW_DISABLE_BUNDLED_PLUGINS=1 /opt/homebrew/bin/astroclaw gateway stop || true
  pkill -f 'astroclaw.*gateway' >/dev/null 2>&1 || true
...
start_astroclaw_gateway() {
  stop_astroclaw_gateway_processes
  rm -f /tmp/astroclaw-parallels-macos-gateway.log
  trap '' HUP
  /usr/bin/env ASTROCLAW_HOME="$HOME" ASTROCLAW_STATE_DIR="$HOME/.astroclaw" ASTROCLAW_CONFIG_PATH="$HOME/.astroclaw/astroclaw.json" ${input.auth.apiKeyEnv}=${shellQuote(
    input.auth.apiKeyValue,
  )} /opt/homebrew/bin/astroclaw gateway run --bind loopback --port 18789 --force >/tmp/astroclaw-parallels-macos-gateway.log 2>&1 </dev/null &
  sleep 1
}
wait_for_gateway() {
  deadline=$((SECONDS + 240))
  while [ "$SECONDS" -lt "$deadline" ]; do
    if /opt/homebrew/bin/astroclaw gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    sleep 2
  done
  cat /tmp/astroclaw-parallels-macos-gateway.log >&2 || true
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_astroclaw_gateway_processes
ASTROCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 ASTROCLAW_DISABLE_BUNDLED_PLUGINS=1 /opt/homebrew/bin/astroclaw update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck("/opt/homebrew/bin/astroclaw", input.expectedNeedle)}
start_astroclaw_gateway
wait_for_gateway
/opt/homebrew/bin/astroclaw models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands("/opt/homebrew/bin/astroclaw", input.auth.modelId, "macos")}
/opt/homebrew/bin/astroclaw config set agents.defaults.skipBootstrap true --strict-json
/opt/homebrew/bin/astroclaw config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript("/opt/homebrew/bin/astroclaw", input, "parallels-npm-update-macos")}`;
}
...
function Stop-AstroclawGatewayProcesses {
  Invoke-Astroclaw gateway stop *>&1 | Out-Host
  Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
    Where-Object { $_.CommandLine -match 'astroclaw.*gateway' } |
...
Stop-AstroclawGatewayProcesses
...
if ($updateExit -ne 0) {
  ...
  if (-not $stalePostSwapImport) { throw "astroclaw update failed with exit code $updateExit" }
  Write-Host "astroclaw update returned a stale post-swap module import; continuing to post-update health checks"
}
...
export ASTROCLAW_ALLOW_ROOT=1
scrub_future_plugin_entries() {
  node - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || "/root", ".astroclaw", "astroclaw.json");
...
stop_astroclaw_gateway_processes() {
  ASTROCLAW_DISABLE_BUNDLED_PLUGINS=1 ASTROCLAW_ALLOW_ROOT=1 astroclaw gateway stop || true
  pkill -f 'astroclaw.*gateway' >/dev/null 2>&1 || true
}
start_astroclaw_gateway() {
  pkill -f "astroclaw gateway run" >/dev/null 2>&1 || true
  rm -f /tmp/astroclaw-parallels-linux-gateway.log
  setsid sh -lc ${shellQuote(
    `exec env ASTROCLAW_HOME=/root ASTROCLAW_STATE_DIR=/root/.astroclaw ASTROCLAW_CONFIG_PATH=/root/.astroclaw/astroclaw.json ASTROCLAW_DISABLE_BONJOUR=1 ASTROCLAW_ALLOW_ROOT=1 ${input.auth.apiKeyEnv}=${shellQuote(
      input.auth.apiKeyValue,
...
wait_for_gateway() {
  deadline=$((SECONDS + 240))
  while [ "$SECONDS" -lt "$deadline" ]; do
    if astroclaw gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    sleep 2
  done
  cat /tmp/astroclaw-parallels-linux-gateway.log >&2 || true
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_astroclaw_gateway_processes
ASTROCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 ASTROCLAW_DISABLE_BUNDLED_PLUGINS=1 astroclaw update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck("astroclaw", input.expectedNeedle)}
start_astroclaw_gateway
wait_for_gateway
astroclaw models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands("astroclaw", input.auth.modelId, "linux")}
astroclaw config set agents.defaults.skipBootstrap true --strict-json
astroclaw config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript("astroclaw", input, "parallels-npm-update-linux")}`;
}
...
while ($true) {
  $version = Invoke-Astroclaw --version
  $version
  if ($LASTEXITCODE -eq 0) { break }
  if ((Get-Date) -ge $versionDeadline) { throw "astroclaw --version failed with exit code $LASTEXITCODE" }
  Start-Sleep -Seconds 2
}`;
  }
