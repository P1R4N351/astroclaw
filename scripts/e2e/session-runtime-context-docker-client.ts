...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".astroclaw", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(customEntry.customType === "astroclaw.runtime-context", "unexpected custom message type");
  ...
  assert(
    !userText.includes("ASTROCLAW_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-session-runtime-context-"));
  process.env.HOME = root;
  process.env.ASTROCLAW_STATE_DIR = path.join(root, ".astroclaw");
  process.env.ASTROCLAW_CONFIG_PATH = path.join(process.env.ASTROCLAW_STATE_DIR, "astroclaw.json");
  try {
    ...
    if (process.env.ASTROCLAW_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
