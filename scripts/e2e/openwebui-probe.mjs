...
const smokeMode =
  process.env.OPENWEBUI_SMOKE_MODE ?? process.env.ASTROCLAW_OPENWEBUI_SMOKE_MODE ?? "chat";
...
for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
  ...
  if (modelsRes?.ok) {
    ...
    targetModel =
      modelIds.find((id) => id === "astroclaw/default") ?? modelIds.find((id) => id === "astroclaw");
    if (targetModel) {
      break;
    }
    lastModelsError = `missing astroclaw model: ${JSON.stringify(modelIds)}`;
  } else if (modelsRes) {
...
if (!targetModel) {
  throw new Error(
    `astroclaw model missing from Open WebUI model list after retry: ${JSON.stringify(modelIds)} (${lastModelsError})`,
  );
}
if (smokeMode === "models") {
