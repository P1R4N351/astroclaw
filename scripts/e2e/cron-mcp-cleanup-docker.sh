...
IMAGE_NAME="$(docker_e2e_resolve_image "astroclaw-cron-mcp-cleanup-e2e" ASTROCLAW_IMAGE)"
PORT="18789"
TOKEN="cron-mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="astroclaw-cron-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t astroclaw-cron-mcp-client-log.XXXXXX)"
...
ASTROCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 cron-mcp-cleanup empty)"
...
docker_e2e_run_with_harness \
  ...
  bash -lc "set -euo pipefail
    ...
    export SUCCESS_MARKER=ASTROCLAW_CRON_MCP_CLEANUP_OK
    export MOCK_REQUEST_LOG=/tmp/astroclaw-cron-mock-openai-requests.jsonl
    export ASTROCLAW_DOCKER_OPENAI_BASE_URL=\"http://127.0.0.1:\$MOCK_PORT/v1\"
    mock_pid=\"\$(astroclaw_e2e_start_mock_openai \"\$MOCK_PORT\" /tmp/cron-mcp-cleanup-mock-openai.log)\"
    gateway_pid=
    cleanup_inner() {
      astroclaw_e2e_stop_process \"\${gateway_pid:-}\"
    ...
    gateway_pid=\"\$(astroclaw_e2e_start_gateway \"\$entry\" $PORT /tmp/cron-mcp-cleanup-gateway.log)\"
    astroclaw_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/cron-mcp-cleanup-gateway.log 300
