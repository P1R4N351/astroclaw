...
IMAGE_NAME="$(docker_e2e_resolve_image "astroclaw-update-channel-switch-e2e" ASTROCLAW_UPDATE_CHANNEL_SWITCH_E2E_IMAGE)"
SKIP_BUILD="${ASTROCLAW_UPDATE_CHANNEL_SWITCH_E2E_SKIP_BUILD:-0}"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz update-channel-switch "${ASTROCLAW_CURRENT_PACKAGE_TGZ:-}")"
# Bare lanes mount the package artifact instead of baking app sources into the image.
docker_e2e_package_mount_args "$PACKAGE_TGZ"
ASTROCLAW_TEST_STATE_SCRIPT_B64="$(
  node "$ROOT_DIR/scripts/lib/astroclaw-test-state.mjs" shell \
...
(
  cd "$git_root"
  if ! npm install --omit=optional --no-fund --no-audit >/tmp/astroclaw-git-install.log 2>&1; then
    cat /tmp/astroclaw-git-install.log >&2 || true
    exit 1
  fi
)
node scripts/e2e/lib/update-channel-switch/assertions.mjs write-control-ui "$git_root"

git config --global user.email "docker-e2e@astroclaw.local"
git config --global user.name "Astroclaw Docker E2E"
...
package_version="$(node -p "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/npm-prefix/lib/node_modules/astroclaw/package.json\", \"utf8\")).version")"
...
export ASTROCLAW_GIT_DIR="$git_root"
export ASTROCLAW_UPDATE_DEV_TARGET_REF="$fixture_sha"

echo "==> package -> git dev channel"
set +e
dev_json="$(astroclaw update --channel dev --yes --json --no-restart)"
dev_status=$?
set -e
printf "%s\n" "$dev_json"
...
status_json="$(astroclaw update status --json)"
printf "%s\n" "$status_json"
...
set +e
stable_json="$(astroclaw update --channel stable --tag "$pkg_tgz_path" --yes --json --no-restart)"
stable_status=$?
set -e
printf "%s\n" "$stable_json"
