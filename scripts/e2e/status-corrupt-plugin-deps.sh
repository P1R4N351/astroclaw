...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/astroclaw-status-corrupt-plugin-deps.XXXXXX")"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
const stageDir = process.env.ASTROCLAW_PLUGIN_STAGE_DIR || "";
const renameResidue = path.join(stageDir, "node_modules", "ansi-escapes", ".astroclaw-rename-tmp");
...
run_astroclaw() {
  HOME="$HOME_DIR" \
  ASTROCLAW_HOME="$STATE_DIR" \
  ASTROCLAW_STATE_DIR="$STATE_DIR" \
  ASTROCLAW_CONFIG_PATH="$CONFIG_PATH" \
  ASTROCLAW_PLUGIN_STAGE_DIR="$STAGE_DIR" \
  ASTROCLAW_DISABLE_BUNDLED_PLUGINS=1 \
  ASTROCLAW_NO_ONBOARD=1 \
  ASTROCLAW_NO_PROMPT=1 \
  ASTROCLAW_SKIP_CHANNELS=1 \
  ASTROCLAW_SKIP_PROVIDERS=1 \
...
run_astroclaw status --all --timeout 1 > "$BEFORE"
grep -F "e2e-corrupt-chat" "$BEFORE" >/dev/null
grep -F "plugin load failed: dependency tree corrupted; run astroclaw doctor --fix" "$BEFORE" >/dev/null

run_astroclaw doctor --fix --non-interactive --yes > "$DOCTOR"
if [[ -e "$STAGE_DIR" ]]; then
  echo "doctor --fix did not remove corrupt plugin stage dir: $STAGE_DIR" >&2
  exit 1
fi

run_astroclaw status --all --timeout 1 > "$AFTER"
