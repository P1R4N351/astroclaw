...
const PLUGIN_SPEC =
  process.env.ASTROCLAW_KITCHEN_SINK_NPM_SPEC || "npm:@astroclaw/kitchen-sink@latest";
const PLUGIN_ID = process.env.ASTROCLAW_KITCHEN_SINK_PLUGIN_ID || "astroclaw-kitchen-sink-fixture";
...
const READY_TIMEOUT_MS = readPositiveInt(process.env.ASTROCLAW_KITCHEN_SINK_RPC_READY_MS, 240000);
const COMMAND_TIMEOUT_MS = readPositiveInt(
  process.env.ASTROCLAW_KITCHEN_SINK_RPC_COMMAND_MS,
  180000,
);
const RPC_TIMEOUT_MS = readPositiveInt(process.env.ASTROCLAW_KITCHEN_SINK_RPC_CALL_MS, 60000);
const MAX_RSS_MIB = readPositiveInt(process.env.ASTROCLAW_KITCHEN_SINK_MAX_RSS_MIB, 2048);
...
function resolveAstroclawRunner() {
  if (process.env.ASTROCLAW_ENTRY) {
    return {
      command: "node",
      baseArgs: [process.env.ASTROCLAW_ENTRY],
      label: process.env.ASTROCLAW_ENTRY,
    };
  }
  ...
  return { command: "pnpm", baseArgs: ["astroclaw"], label: "pnpm astroclaw" };
}

function makeEnv() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-kitchen-sink-rpc-"));
  ...
  return {
    ...
    env: {
      ...
      ASTROCLAW_SKIP_PROVIDERS: "0",
      ASTROCLAW_KITCHEN_SINK_PERSONALITY:
        process.env.ASTROCLAW_KITCHEN_SINK_PERSONALITY || "conformance",
    },
  };
}
...
async function runAstroclaw(runner, args, env, options = {}) {
...
async function rpcCall(method, params, options) {
  ...
  const payload = await callGateway({
    config: readJson(options.env.ASTROCLAW_CONFIG_PATH),
    configPath: options.env.ASTROCLAW_CONFIG_PATH,
    ...
    entries: {
      ...
      [PLUGIN_ID]: {
        ...
        config: {
          ...
          personality: env.ASTROCLAW_KITCHEN_SINK_PERSONALITY,
        },
        hooks: {
...
async function main() {
  const runner = resolveAstroclawRunner();
  const port = readPositiveInt(process.env.ASTROCLAW_KITCHEN_SINK_RPC_PORT, 19173);
  const { root, env } = makeEnv();
  const logPath = path.join(root, "gateway.log");

  console.log(`Kitchen Sink RPC walk using ${PLUGIN_SPEC} via ${runner.label}`);
  await runAstroclaw(runner, ["plugins", "install", PLUGIN_SPEC], env, { timeoutMs: 240000 });
  configureKitchenSink(env, port);
  await runAstroclaw(runner, ["plugins", "enable", PLUGIN_ID], env, { timeoutMs: 60000 });
  const inspect = parseJsonOutput(
    (await runAstroclaw(runner, ["plugins", "inspect", PLUGIN_ID, "--runtime", "--json"], env))
      .stdout,
  );
