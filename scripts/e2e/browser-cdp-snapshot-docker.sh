...
BASE_IMAGE="$(docker_e2e_resolve_image "astroclaw-browser-cdp-base-e2e" ASTROCLAW_BROWSER_CDP_BASE_E2E_IMAGE)"
IMAGE_NAME="$(docker_e2e_resolve_image "astroclaw-browser-cdp-snapshot-e2e" ASTROCLAW_BROWSER_CDP_SNAPSHOT_E2E_IMAGE)"
SKIP_BUILD="${ASTROCLAW_BROWSER_CDP_SNAPSHOT_E2E_SKIP_BUILD:-0}"
PORT="18789"
CDP_PORT="19222"
FIXTURE_PORT="18080"
TOKEN="browser-cdp-e2e-token"
CONTAINER_NAME="astroclaw-browser-cdp-e2e-$$"
DOCKER_COMMAND_TIMEOUT="${ASTROCLAW_BROWSER_CDP_SNAPSHOT_DOCKER_COMMAND_TIMEOUT:-900s}"
...
if [ "${ASTROCLAW_SKIP_DOCKER_BUILD:-0}" = "1" ] || [ "$SKIP_BUILD" = "1" ]; then
  ...
  build_dir="$(mktemp -d "${TMPDIR:-/tmp}/astroclaw-browser-cdp-build.XXXXXX")"
...
ASTROCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 browser-cdp-snapshot empty)"
...
mkdir -p /tmp/astroclaw-browser-cdp/chrome
find dist -maxdepth 1 -type f -name 'pw-ai-*.js' ! -name 'pw-ai-state-*' -exec mv {} /tmp/astroclaw-browser-cdp/ \;
PORT=$PORT CDP_PORT=$CDP_PORT node scripts/e2e/lib/fixture.mjs browser-cdp
chromium --headless=new --no-sandbox --disable-gpu --disable-dev-shm-usage \\
  --remote-debugging-address=127.0.0.1 \\
  --remote-debugging-port=$CDP_PORT \\
  --user-data-dir=/tmp/astroclaw-browser-cdp/chrome \\
...
astroclaw_e2e_exec_gateway \"\$entry\" $PORT loopback /tmp/browser-cdp-gateway.log" >/dev/null

echo "Waiting for Chromium and Gateway..."
if ! docker_e2e_wait_container_bash "$CONTAINER_NAME" 180 0.5 "
    source scripts/lib/astroclaw-e2e-instance.sh
    astroclaw_e2e_probe_http_status http://127.0.0.1:$CDP_PORT/json/version
    astroclaw_e2e_probe_tcp 127.0.0.1 $PORT
"; then
...
set -euo pipefail
source /tmp/astroclaw-test-state-env
source scripts/lib/astroclaw-e2e-instance.sh
entry=\"\$(astroclaw_e2e_resolve_entrypoint)\"
