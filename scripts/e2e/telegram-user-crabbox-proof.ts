...
const REMOTE_ROOT = "/tmp/astroclaw-telegram-user-crabbox";
...
function usageText() {
  return [
    "Usage:",
    "  node --import tsx scripts/e2e/telegram-user-crabbox-proof.ts [probe] [--text /status] [--expect Astroclaw]",
    ...
    "  --repo <owner/name>           GitHub repo for publish. Default: astroclaw/astroclaw.",
...
function parseArgs(argv: string[]): Options {
  ...
  const opts: Options = {
    ...
    command,
    crabboxBin: trimToValue(process.env.ASTROCLAW_TELEGRAM_USER_CRABBOX_BIN) ?? "crabbox",
    desktopChatTitle:
      trimToValue(process.env.ASTROCLAW_TELEGRAM_USER_DESKTOP_CHAT_TITLE) ?? "Astroclaw Testing",
    dryRun: false,
    expect: ["Astroclaw"],
    gatewayPort: 19_879,
    idleTimeout: "60m",
    keepBox: false,
    mockResponseText: "ASTROCLAW_E2E_OK",
    mockPort: 19_882,
    outputDir: path.join(DEFAULT_OUTPUT_ROOT, stamp),
    previewCropWidth: TELEGRAM_PROOF_VIEW.cropWidth,
    previewFps: 24,
    previewWidth: 1920,
    provider: process.env.ASTROCLAW_TELEGRAM_USER_CRABBOX_PROVIDER?.trim() || "aws",
    publishFullArtifacts: false,
    publishRepo: "astroclaw/astroclaw",
    recordFps: 24,
    recordSeconds: 35,
    remoteCommand: [],
    target: "linux",
    ...
    ttl: "120m",
    userDriverScript:
      trimToValue(process.env.ASTROCLAW_TELEGRAM_USER_DRIVER_SCRIPT) ?? DEFAULT_USER_DRIVER,
  };
...
function repoRoot() {
  ...
  if (
    ...
    throw new Error("Run from the Astroclaw repo root.");
  }
  return cwd;
}
...
function childProcessBaseEnv() {
  const keys = [
    ...
    "LC_ALL",
    "NODE_OPTIONS",
    "ASTROCLAW_BUILD_PRIVATE_QA",
    "ASTROCLAW_ENABLE_PRIVATE_QA_CLI",
    "PATH",
    "PNPM_HOME",
    "SHELL",
    "TEMP",
...
function gatewayEnv(params: { configPath: string; stateDir: string; sutToken: string }) {
  return {
    ...childProcessBaseEnv(),
    OPENAI_API_KEY: "sk-astroclaw-e2e-mock",
...
function writeSutConfig(params: {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-tg-crabbox-sut-"));
...
async function startLocalSut(params: {
  ...
  const gateway = spawnLogged(
    "pnpm",
    ["astroclaw", "gateway", "--port", String(params.gatewayPort)],
    {
      cwd: params.repoRoot,
...
async function startLocalSutDaemon(params: {
  ...
  const gatewayPid = spawnDaemon({
    command: "pnpm",
    args: ["astroclaw", "gateway", "--port", String(params.gatewayPort)],
    cwd: params.repoRoot,
    env: gatewayEnv({ ...config, sutToken: params.sutToken }),
    logPath: gatewayLog,
  });
...
async function startSession(root: string, opts: Options, outputDir: string) {
  ...
  const hasConvexEnv =
    trimToValue(process.env.ASTROCLAW_QA_CONVEX_SITE_URL) &&
    trimToValue(process.env.ASTROCLAW_QA_CONVEX_SECRET_CI);
...
async function main() {
  ...
  const localRoot = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-telegram-crabbox-"));
  const summary: JsonObject = {
    artifacts: {},
