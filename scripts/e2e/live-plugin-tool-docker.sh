...
IMAGE_NAME="$(docker_e2e_resolve_image "astroclaw-live-plugin-tool-e2e" ASTROCLAW_LIVE_PLUGIN_TOOL_E2E_IMAGE)"
DOCKER_TARGET="${ASTROCLAW_LIVE_PLUGIN_TOOL_DOCKER_TARGET:-bare}"
HOST_BUILD="${ASTROCLAW_LIVE_PLUGIN_TOOL_HOST_BUILD:-1}"
PACKAGE_TGZ="${ASTROCLAW_CURRENT_PACKAGE_TGZ:-}"
AGENT_TURN_TIMEOUT_SECONDS="${ASTROCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}"
PROFILE_FILE="${ASTROCLAW_LIVE_PLUGIN_TOOL_PROFILE_FILE:-${ASTROCLAW_TESTBOX_PROFILE_FILE:-$HOME/.astroclaw-testbox-live.profile}}"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${ASTROCLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "ASTROCLAW_LIVE_PLUGIN_TOOL_HOST_BUILD=0 requires ASTROCLAW_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
ASTROCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 live-plugin-tool empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e ASTROCLAW_LIVE_PLUGIN_TOOL_MODEL="${ASTROCLAW_LIVE_PLUGIN_TOOL_MODEL:-openai/gpt-5.5}" \
  -e "ASTROCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
...
MODEL_REF="${ASTROCLAW_LIVE_PLUGIN_TOOL_MODEL:?missing ASTROCLAW_LIVE_PLUGIN_TOOL_MODEL}"
PLUGIN_ID="e2e-slug-tool"
PLUGIN_NAME="@astroclaw/e2e-slug-tool"
PLUGIN_VERSION="0.0.0-e2e.1"
TOOL_NAME="e2e_slug_probe"
SEED="Astroclaw E2E Plugin Tool $(date +%s)-$RANDOM"
...
dump_debug_logs() {
  ...
  astroclaw_e2e_dump_logs \
    ...
    /tmp/astroclaw-plugin-install.log \
    /tmp/astroclaw-plugin-enable.log \
    /tmp/astroclaw-plugins-list.json \
    /tmp/astroclaw-plugin-inspect.json \
...
fixture_dir="$(mktemp -d /tmp/astroclaw-live-plugin-tool.XXXXXX)"
...
astroclaw plugins install "npm-pack:$plugin_tgz" --force >/tmp/astroclaw-plugin-install.log 2>&1
node scripts/e2e/lib/live-plugin-tool/assertions.mjs configure
astroclaw plugins enable "$PLUGIN_ID" >/tmp/astroclaw-plugin-enable.log 2>&1
astroclaw plugins list --json >/tmp/astroclaw-plugins-list.json
astroclaw plugins inspect "$PLUGIN_ID" --runtime --json >/tmp/astroclaw-plugin-inspect.json
...
astroclaw agent --local \
  ...
  --thinking off \
  --timeout "${ASTROCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}" \
  --json >/tmp/astroclaw-agent.json 2>/tmp/astroclaw-agent.err
