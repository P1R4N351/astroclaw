...
// against the Astroclaw npm tarball installed in the functional image.
...
export async function connectMcpClient(params: {
  ...
  const tokenDir = "/tmp/astroclaw-mcp-client";
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      ASTROCLAW_STATE_DIR: "/tmp/astroclaw-mcp-client",
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[astroclaw mcp] ${String(chunk)}`);
  });
