#!/usr/bin/env bash
# Installs an Astroclaw package candidate in Docker, performs Telegram
...
IMAGE_NAME="$(docker_e2e_resolve_image "astroclaw-npm-telegram-live-e2e" ASTROCLAW_NPM_TELEGRAM_LIVE_E2E_IMAGE)"
...
OUTPUT_DIR="${ASTROCLAW_NPM_TELEGRAM_OUTPUT_DIR:-.artifacts/qa-e2e/npm-telegram-live}"
...
validate_astroclaw_package_spec() {
  ...
  echo "ASTROCLAW_NPM_TELEGRAM_PACKAGE_SPEC must be astroclaw@alpha, astroclaw@beta, astroclaw@latest, or an exact Astroclaw release version; got: $spec" >&2
  exit 1
}
...
validate_credential_preflight() {
  if [ "${ASTROCLAW_NPM_TELEGRAM_SKIP_CREDENTIAL_PREFLIGHT:-0}" = "1" ]; then
    return 0
  fi
  ...
  if [ "${#missing[@]}" -gt 0 ]; then
    {
      ...
      echo "Use one of:"
      echo "  direct Telegram env: ASTROCLAW_QA_TELEGRAM_GROUP_ID, ASTROCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN, ASTROCLAW_QA_TELEGRAM_SUT_BOT_TOKEN"
      echo "  Convex env: ASTROCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex plus ASTROCLAW_QA_CONVEX_SITE_URL and a role secret"
    } >&2
    exit 1
  fi
}
...
run_log="$(mktemp "${TMPDIR:-/tmp}/astroclaw-npm-telegram-live.XXXXXX")"
...
docker_env=(
  ...
  -e ASTROCLAW_NPM_TELEGRAM_PACKAGE_SPEC="$PACKAGE_SPEC"
  -e ASTROCLAW_NPM_TELEGRAM_PACKAGE_LABEL="$PACKAGE_LABEL"
  -e ASTROCLAW_NPM_TELEGRAM_OUTPUT_DIR="$OUTPUT_DIR"
  -e ASTROCLAW_NPM_TELEGRAM_FAST="${ASTROCLAW_NPM_TELEGRAM_FAST:-1}"
)
...
if [ -n "$credential_source" ]; then
  docker_env+=(-e ASTROCLAW_QA_CREDENTIAL_SOURCE="$credential_source")
fi
if [ -n "$credential_role" ]; then
  docker_env+=(-e ASTROCLAW_QA_CREDENTIAL_ROLE="$credential_role")
fi
...
for key in \
  ...
  ASTROCLAW_QA_ALLOW_INSECURE_HTTP \
  ASTROCLAW_QA_REDACT_PUBLIC_METADATA \
  ASTROCLAW_QA_TELEGRAM_CAPTURE_CONTENT \
  ASTROCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS \
  ASTROCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS \
  ASTROCLAW_QA_SUITE_PROGRESS \
  ASTROCLAW_NPM_TELEGRAM_PROVIDER_MODE \
  ASTROCLAW_NPM_TELEGRAM_MODEL \
  ASTROCLAW_NPM_TELEGRAM_ALT_MODEL \
  ASTROCLAW_NPM_TELEGRAM_SCENARIOS \
  ASTROCLAW_NPM_TELEGRAM_SKIP_HOTPATH \
  ASTROCLAW_NPM_TELEGRAM_SUT_ACCOUNT \
  ASTROCLAW_NPM_TELEGRAM_ALLOW_FAILURES; do
...
export HOME="$(mktemp -d "/tmp/astroclaw-npm-telegram-install.XXXXXX")"
...
export HOME="$(mktemp -d "/tmp/astroclaw-npm-telegram-runtime.XXXXXX")"
export NPM_CONFIG_PREFIX="/npm-global"
export PATH="$NPM_CONFIG_PREFIX/bin:$PATH"
export ASTROCLAW_NPM_TELEGRAM_REPO_ROOT="/app"

dump_hotpath_logs() {
  local status="$1"
  echo "installed-package onboarding recovery hot path failed with exit code $status" >&2
  for file in \
    /tmp/astroclaw-npm-telegram-onboard.json \
    /tmp/astroclaw-npm-telegram-channel-add.log \
    /tmp/astroclaw-npm-telegram-doctor-fix.log \
    /tmp/astroclaw-npm-telegram-doctor-check.log; do
    if [ -f "$file" ]; then
      echo "--- $file ---" >&2
...
astroclaw_package_dir="/npm-global/lib/node_modules/astroclaw"
# The mounted QA harness imports astroclaw/plugin-sdk and package dependencies;
# point those imports at the installed package without copying source plugins into the test image.
rm -rf /app/node_modules/astroclaw
ln -sfnT "$astroclaw_package_dir" /app/node_modules/astroclaw
rm -rf /app/dist
ln -sfnT "$astroclaw_package_dir/dist" /app/dist
cp "$astroclaw_package_dir/package.json" /app/package.json
node scripts/e2e/lib/npm-telegram-live/prepare-package.mjs \
  /app/package.json \
  /app/node_modules/astroclaw/package.json
for deps_dir in "$astroclaw_package_dir/node_modules" /npm-global/lib/node_modules; do
  [ -d "$deps_dir" ] || continue
...
link_installed_package_dependency() {
  local name="$1"
  local source="/npm-global/lib/node_modules/astroclaw/node_modules/$name"
...
if [ "${ASTROCLAW_NPM_TELEGRAM_SKIP_HOTPATH:-0}" != "1" ]; then
  echo "Running installed-package onboarding recovery hot path..."
  OPENAI_API_KEY="${OPENAI_API_KEY:-sk-astroclaw-npm-telegram-hotpath}" astroclaw onboard --non-interactive --accept-risk \
    --mode local \
    ...
    --skip-skills \
    --skip-health \
    --json >/tmp/astroclaw-npm-telegram-onboard.json </dev/null

  astroclaw channels add --channel telegram --token "123456:astroclaw-npm-telegram-hotpath" >/tmp/astroclaw-npm-telegram-channel-add.log 2>&1 </dev/null
  astroclaw doctor --fix --non-interactive >/tmp/astroclaw-npm-telegram-doctor-fix.log 2>&1 </dev/null
  astroclaw doctor --non-interactive >/tmp/astroclaw-npm-telegram-doctor-check.log 2>&1 </dev/null
fi

export ASTROCLAW_NPM_TELEGRAM_SUT_COMMAND="$(command -v astroclaw)"
trap - ERR
