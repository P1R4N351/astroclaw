#!/usr/bin/env bash
# ClawDock - Docker helpers for Astroclaw
# Inspired by Simon Willison's "Running Astroclaw in Docker"
# https://til.simonwillison.net/llms/astroclaw-docker
#
# Installation:
...
# Common paths to check for Astroclaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/astroclaw"
  "${HOME}/workspace/astroclaw"
  "${HOME}/projects/astroclaw"
  "${HOME}/dev/astroclaw"
  "${HOME}/code/astroclaw"
  "${HOME}/src/astroclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Astroclaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/astroclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Astroclaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/astroclaw/astroclaw.git ~/astroclaw"
    echo "  cd ~/astroclaw && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^ASTROCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d astroclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart astroclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f astroclaw-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.astroclaw"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show astroclaw.json
  if [[ -f "${config_dir}/astroclaw.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/astroclaw.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/astroclaw.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No astroclaw.json found${_CLR_RESET}"
  fi
  echo ""
...
clawdock-workspace() {
  cd ~/.astroclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec astroclaw-gateway \
    bash -c 'echo "alias astroclaw=\"./astroclaw.mjs\"" > /tmp/.bashrc_astroclaw && bash --rcfile /tmp/.bashrc_astroclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm astroclaw-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build astroclaw-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d astroclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build astroclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "ASTROCLAW_GATEWAY_TOKEN=$token" astroclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" astroclaw-gateway \
    bash -c './astroclaw.mjs config set gateway.remote.token "$TOKEN" && ./astroclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec astroclaw-gateway \
    bash -c "./astroclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart astroclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm astroclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec astroclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'astroclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Astroclaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (astroclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to astroclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.astroclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'astroclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'astroclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.astroclaw.ai${_CLR_RESET}"
  echo ""
}
