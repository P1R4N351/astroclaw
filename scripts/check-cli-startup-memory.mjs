...
const tmpHome = mkdtempSync(path.join(os.tmpdir(), "astroclaw-startup-memory-"));
...
const MAX_RSS_MARKER = "__ASTROCLAW_MAX_RSS_KB__=";
...
const cases = [
  {
    id: "help",
    label: "--help",
    args: ["astroclaw.mjs", "--help"],
    limitMb: Number(process.env.ASTROCLAW_STARTUP_MEMORY_HELP_MB ?? DEFAULT_LIMITS_MB.help),
  },
  {
    id: "statusJson",
    label: "status --json",
    args: ["astroclaw.mjs", "status", "--json"],
    limitMb: Number(
      process.env.ASTROCLAW_STARTUP_MEMORY_STATUS_JSON_MB ?? DEFAULT_LIMITS_MB.statusJson,
    ),
  },
  {
    id: "gatewayStatus",
    label: "gateway status",
    args: ["astroclaw.mjs", "gateway", "status"],
    limitMb: Number(
      process.env.ASTROCLAW_STARTUP_MEMORY_GATEWAY_STATUS_MB ?? DEFAULT_LIMITS_MB.gatewayStatus,
    ),
  },
];
...
function formatFixGuidance(testCase, details) {
  ...
  const guidance = [
    ...
    "LLM prompt:",
    `"Astroclaw startup-memory CI failed for '${testCase.label}'. Analyze this failure, identify the first runtime/import side effect that makes startup heavier or broken, and propose the smallest safe patch. Failure output:\n${details}"`,
  ];
