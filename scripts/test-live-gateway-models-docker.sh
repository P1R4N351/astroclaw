...
elif [[ -n "${ASTROCLAW_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(astroclaw_live_collect_auth_dirs_from_csv "${ASTROCLAW_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(astroclaw_live_collect_auth_files_from_csv "${ASTROCLAW_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
DOCKER_RUN_ARGS=(docker run --rm -t \
  ...
  -e ASTROCLAW_LIVE_GATEWAY_MODELS="${ASTROCLAW_LIVE_GATEWAY_MODELS:-modern}" \
  -e ASTROCLAW_LIVE_GATEWAY_PROVIDERS="${ASTROCLAW_LIVE_GATEWAY_PROVIDERS:-}" \
  -e ASTROCLAW_LIVE_GATEWAY_SMOKE="${ASTROCLAW_LIVE_GATEWAY_SMOKE:-1}" \
  -e ASTROCLAW_LIVE_GATEWAY_MAX_MODELS="${ASTROCLAW_LIVE_GATEWAY_MAX_MODELS:-8}" \
  -e ASTROCLAW_LIVE_GATEWAY_HEARTBEAT_MS="${ASTROCLAW_LIVE_GATEWAY_HEARTBEAT_MS:-}" \
  -e ASTROCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS="${ASTROCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS:-45000}" \
  -e ASTROCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${ASTROCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-90000}" \
