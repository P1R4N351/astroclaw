...
const EXPECTED_REPOSITORY_URL = "https://github.com/astroclaw/astroclaw";
const OPTIONAL_LOCAL_EMBEDDING_RUNTIME_PACKAGE = "node-llama-cpp";
const FS_SAFE_PACKAGE = "@astroclaw/fs-safe";
...
const skipPackValidationEnv = "ASTROCLAW_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "astroclaw") {
    errors.push(`package.json name must be "astroclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.astroclaw !== "astroclaw.mjs") {
    errors.push(
      `package.json bin.astroclaw must be "astroclaw.mjs"; found "${pkg.bin?.astroclaw ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`astroclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `astroclaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
