...
function parseArgs(argv) {
  const options = {
    ...
    pluginIds: [],
    shardTotal: readOptionalPositiveIntEnv("ASTROCLAW_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("ASTROCLAW_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    limit: undefined,
    skipPrebuild: false,
    skipLifecycle: false,
    skipQa: false,
  ...
  const envIds = normalizeCsv(process.env.ASTROCLAW_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
function astroclawCommand(repoRoot, args) {
  return {
...
function sourceAstroclawCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function createIsolatedEnv(repoRoot, runRoot) {
  ...
  return {
    ...
    ASTROCLAW_LOG_DIR: path.join(runRoot, "logs"),
    ASTROCLAW_QA_SUITE_PROGRESS: process.env.ASTROCLAW_QA_SUITE_PROGRESS ?? "1",
    ...
    for (const { phase, args } of commands) {
      ...
      params.rows.push(
        runMeasuredCommand({
          ...
          ...astroclawCommand(params.repoRoot, ["plugins", ...args]),
    ...
    for (const alias of plugin.cliCommandAliases) {
      ...
      params.rows.push(
        runMeasuredCommand({
          ...
          ...astroclawCommand(params.repoRoot, [command, "--help"]),
...
function runQaChunks(params) {
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    const row = runMeasuredCommand({
      ...
      ...sourceAstroclawCommand(params.repoRoot, [
        "qa",
        "suite",
        "--provider-mode",
        "mock-openai",
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-plugin-gauntlet-"));
