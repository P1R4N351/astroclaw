...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/astroclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Astroclaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/astroclaw/scripts/mobile-reauth.sh'" --id astroclaw-auth
        ;;
    *)
