...
# Astroclaw Auth Widget for Termux
...
SERVER="${ASTROCLAW_SERVER:-l36}"

# Check auth status
termux-toast "Checking Astroclaw auth..."

STATUS=$(ssh "$SERVER" '$HOME/astroclaw/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/astroclaw/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|ASTROCLAW_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/astroclaw/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|ASTROCLAW_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Astroclaw won't work until re-auth"
                ;;
        esac
        ;;
