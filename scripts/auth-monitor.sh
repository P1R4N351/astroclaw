...
#
# Suggested cron: */30 * * * * /home/admin/astroclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Astroclaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., astroclaw-alerts)
...
STATE_FILE="$HOME/.astroclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Astroclaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use astroclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Astroclaw to $NOTIFY_PHONE..."
            astroclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Astroclaw is down. Run: ssh l36 '~/astroclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
