...
# Checks both Claude Code and Astroclaw auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
ASTROCLAW_AUTH="$HOME/.astroclaw/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    astroclaw models status --json 2>/dev/null || true
}
...
check_astroclaw_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$ASTROCLAW_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    astroclaw_status=$(check_astroclaw_auth 2>/dev/null || true)

    claude_expires=0
    astroclaw_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        astroclaw_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        astroclaw_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$ASTROCLAW_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$astroclaw_status" \
        --arg be "$astroclaw_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            astroclaw: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$astroclaw_status" == EXPIRED* ]] || [[ "$astroclaw_status" == MISSING* ]]; then
        echo "ASTROCLAW_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$astroclaw_status" == EXPIRING* ]]; then
        echo "ASTROCLAW_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "Astroclaw Auth (~/.astroclaw/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$ASTROCLAW_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$ASTROCLAW_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'astroclaw doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active astroclaw >/dev/null 2>&1; then
    echo -e "Astroclaw service: ${GREEN}running${NC}"
else
    echo -e "Astroclaw service: ${RED}NOT running${NC}"
fi
