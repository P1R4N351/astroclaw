...
function prepareMirroredDocsDir(sourceDir = DOCS_DIR) {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-docs-link-audit-"));
...
export function prepareAnchorAuditDocsDir(sourceDir = DOCS_DIR) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-docs-anchor-audit-"));
