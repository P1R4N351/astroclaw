...
DEFAULT_PROVIDER="${ASTROCLAW_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
CLI_MODEL="${ASTROCLAW_LIVE_CLI_BACKEND_MODEL:-}"
CLI_PROVIDER="${CLI_MODEL%%/*}"
CLI_DISABLE_MCP_CONFIG="${ASTROCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG:-}"
CLI_AUTH_MODE="${ASTROCLAW_LIVE_CLI_BACKEND_AUTH:-auto}"
CLI_SETUP_TIMEOUT_SECONDS="${ASTROCLAW_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:-180}"
TEMP_DIRS=()
...
case "$CLI_AUTH_MODE" in
  auto | api-key | subscription)
    ;;
  *)
    echo "ERROR: ASTROCLAW_LIVE_CLI_BACKEND_AUTH must be one of: auto, api-key, subscription." >&2
    exit 1
    ;;
esac

if [[ "$CLI_AUTH_MODE" == "subscription" && "$CLI_PROVIDER" != "claude-cli" ]]; then
  echo "ERROR: ASTROCLAW_LIVE_CLI_BACKEND_AUTH=subscription is only supported for claude-cli." >&2
  exit 1
fi
...
export ASTROCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${ASTROCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
export ASTROCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE="${ASTROCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
export ASTROCLAW_LIVE_CLI_BACKEND_MCP_PROBE="${ASTROCLAW_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
...
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  ...
  if [[ -z "${ASTROCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]]; then
    if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" ]]; then
      export ASTROCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV='["CLAUDE_CODE_OAUTH_TOKEN"]'
    else
      export ASTROCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="[]"
    fi
  fi
  if [[ "$ASTROCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV" == *ANTHROPIC_API_KEY* ]]; then
    echo "ERROR: subscription auth smoke must not preserve Anthropic API-key env vars." >&2
    exit 1
  fi
  if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" && "$ASTROCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV" != *CLAUDE_CODE_OAUTH_TOKEN* ]]; then
  ...
  export ASTROCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${ASTROCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
  export ASTROCLAW_LIVE_CLI_BACKEND_RESUME_PROBE="${ASTROCLAW_LIVE_CLI_BACKEND_RESUME_PROBE:-1}"
  export ASTROCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE="${ASTROCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
  export ASTROCLAW_LIVE_CLI_BACKEND_MCP_PROBE="${ASTROCLAW_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
fi
...
if [[ -n "${ASTROCLAW_DOCKER_AUTH_DIRS:-}" ]]; then
  ...
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(astroclaw_live_collect_auth_dirs_from_csv "$CLI_PROVIDER")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(astroclaw_live_collect_auth_files_from_csv "$CLI_PROVIDER")
fi
AUTH_DIRS_CSV=""
if ((${#AUTH_DIRS[@]} > 0)); then
...
run_setup_command() {
  timeout --foreground "${ASTROCLAW_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:-180}s" "$@"
}
...
provider="${ASTROCLAW_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
default_command="${ASTROCLAW_DOCKER_CLI_BACKEND_COMMAND_DEFAULT:-}"
docker_package="${ASTROCLAW_DOCKER_CLI_BACKEND_NPM_PACKAGE:-}"
binary_name="${ASTROCLAW_DOCKER_CLI_BACKEND_BINARY_NAME:-}"
if [ -z "$binary_name" ] && [ -n "$default_command" ]; then
  binary_name="$(basename "$default_command")"
fi
if [ -z "${ASTROCLAW_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -n "$binary_name" ]; then
  export ASTROCLAW_LIVE_CLI_BACKEND_COMMAND="$NPM_CONFIG_PREFIX/bin/$binary_name"
fi
...
if [ -n "${ASTROCLAW_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ ! -x "${ASTROCLAW_LIVE_CLI_BACKEND_COMMAND}" ] && [ -n "$docker_package" ]; then
  run_setup_command npm install -g "$docker_package"
elif [ -n "$docker_package" ] && package_has_explicit_version "$docker_package"; then
  run_setup_command npm install -g "$docker_package"
fi
if [ -n "${ASTROCLAW_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -x "${ASTROCLAW_LIVE_CLI_BACKEND_COMMAND}" ]; then
  echo "==> CLI backend binary: ${ASTROCLAW_LIVE_CLI_BACKEND_COMMAND}"
  "${ASTROCLAW_LIVE_CLI_BACKEND_COMMAND}" -V || "${ASTROCLAW_LIVE_CLI_BACKEND_COMMAND}" --version || true
fi
if [ "$provider" = "claude-cli" ]; then
  auth_mode="${ASTROCLAW_LIVE_CLI_BACKEND_AUTH:-auto}"
...
if [ -n "\${ASTROCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${ASTROCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${ASTROCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${ASTROCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD}"
fi
...
WRAP
  ...
  if [ -z "${ASTROCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]; then
    export ASTROCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV='["ANTHROPIC_API_KEY","ANTHROPIC_API_KEY_OLD"]'
  fi
  if [ "$auth_mode" = "subscription" ]; then
    claude --version
    direct_token="ASTROCLAW-CLAUDE-SUBSCRIPTION-DIRECT"
    direct_output="$(
      claude \
...
DOCKER_AUTH_ENV=(
  -e ASTROCLAW_LIVE_CLI_BACKEND_AUTH="$CLI_AUTH_MODE"
)
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  DOCKER_AUTH_ENV+=(
    -e CLAUDE_CODE_OAUTH_TOKEN="${CLAUDE_CODE_OAUTH_TOKEN:-}"
    -e ASTROCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="$ASTROCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV"
  )
else
  DOCKER_AUTH_ENV+=(
    -e ANTHROPIC_API_KEY
    -e ANTHROPIC_API_KEY_OLD
    -e ASTROCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
    -e ASTROCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}"
    -e ASTROCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="${ASTROCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}"
  )
fi
...
DOCKER_RUN_ARGS=(docker run --rm -t \
  ...
  -e ASTROCLAW_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS="$CLI_SETUP_TIMEOUT_SECONDS" \
  -e ASTROCLAW_DOCKER_CLI_BACKEND_PROVIDER="$CLI_PROVIDER" \
  -e ASTROCLAW_DOCKER_CLI_BACKEND_COMMAND_DEFAULT="$CLI_DEFAULT_COMMAND" \
  -e ASTROCLAW_DOCKER_CLI_BACKEND_NPM_PACKAGE="$CLI_DOCKER_NPM_PACKAGE" \
  -e ASTROCLAW_DOCKER_CLI_BACKEND_BINARY_NAME="$CLI_DOCKER_BINARY_NAME" \
  -e ASTROCLAW_LIVE_TEST=1 \
  -e ASTROCLAW_LIVE_CLI_BACKEND=1 \
  -e ASTROCLAW_LIVE_CLI_BACKEND_DEBUG="${ASTROCLAW_LIVE_CLI_BACKEND_DEBUG:-}" \
  -e ASTROCLAW_CLI_BACKEND_LOG_OUTPUT="${ASTROCLAW_CLI_BACKEND_LOG_OUTPUT:-}" \
  -e ASTROCLAW_TEST_CONSOLE="${ASTROCLAW_TEST_CONSOLE:-}" \
  -e ASTROCLAW_LIVE_CLI_BACKEND_MODEL="$CLI_MODEL" \
  -e ASTROCLAW_LIVE_CLI_BACKEND_COMMAND="${ASTROCLAW_LIVE_CLI_BACKEND_COMMAND:-}" \
  -e ASTROCLAW_LIVE_CLI_BACKEND_ARGS="${ASTROCLAW_LIVE_CLI_BACKEND_ARGS:-}" \
  -e ASTROCLAW_LIVE_CLI_BACKEND_RESUME_ARGS="${ASTROCLAW_LIVE_CLI_BACKEND_RESUME_ARGS:-}" \
  -e ASTROCLAW_LIVE_CLI_BACKEND_CLEAR_ENV="${ASTROCLAW_LIVE_CLI_BACKEND_CLEAR_ENV:-}" \
  -e ASTROCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG="$CLI_DISABLE_MCP_CONFIG" \
  -e ASTROCLAW_LIVE_CLI_BACKEND_RESUME_PROBE="${ASTROCLAW_LIVE_CLI_BACKEND_RESUME_PROBE:-}" \
  -e ASTROCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${ASTROCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-}" \
  -e ASTROCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE="${ASTROCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE:-}" \
  -e ASTROCLAW_LIVE_CLI_BACKEND_MCP_PROBE="${ASTROCLAW_LIVE_CLI_BACKEND_MCP_PROBE:-}" \
  -e ASTROCLAW_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE="${ASTROCLAW_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE:-}" \
  -e ASTROCLAW_LIVE_CLI_BACKEND_IMAGE_ARG="${ASTROCLAW_LIVE_CLI_BACKEND_IMAGE_ARG:-}" \
  -e ASTROCLAW_LIVE_CLI_BACKEND_IMAGE_MODE="${ASTROCLAW_LIVE_CLI_BACKEND_IMAGE_MODE:-}")
