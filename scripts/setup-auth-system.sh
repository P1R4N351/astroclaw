#!/bin/bash
# Setup Astroclaw Auth Management System
...
echo "=== Astroclaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'astroclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Astroclaw message: Send warning via Astroclaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/astroclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/astroclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/astroclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now astroclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status astroclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
