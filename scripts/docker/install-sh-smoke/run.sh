...
SMOKE_MODE="${ASTROCLAW_INSTALL_SMOKE_MODE:-install}"
SMOKE_PREVIOUS_VERSION="${ASTROCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ASTROCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="astroclaw"
PACKAGE_NAME="${ASTROCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
FRESH_VERSION="${ASTROCLAW_INSTALL_FRESH_VERSION:-}"
FRESH_TAG_URL="${ASTROCLAW_INSTALL_FRESH_TAG_URL:-}"
UPDATE_BASELINE_VERSION="${ASTROCLAW_INSTALL_UPDATE_BASELINE:-latest}"
UPDATE_BASELINE_TAG_URL="${ASTROCLAW_INSTALL_UPDATE_BASELINE_TAG_URL:-}"
UPDATE_EXPECT_VERSION="${ASTROCLAW_INSTALL_UPDATE_EXPECT_VERSION:-}"
UPDATE_TAG_URL="${ASTROCLAW_INSTALL_UPDATE_TAG_URL:-}"
HEARTBEAT_INTERVAL="${ASTROCLAW_INSTALL_SMOKE_HEARTBEAT_INTERVAL:-60}"
INSTALL_COMMAND_TIMEOUT="${ASTROCLAW_INSTALL_SMOKE_COMMAND_TIMEOUT:-900}"
...
is_self_swapped_package_process_exit() {
  local stderr="$1"
  [[ "$stderr" == *"[astroclaw] Failed to start CLI:"* ]] &&
    [[ "$stderr" == *"ERR_MODULE_NOT_FOUND"* ]] &&
    [[ "$stderr" == *"/node_modules/astroclaw/dist/"* ]]
}
...
run_install_smoke() {
  if [[ -n "$FRESH_VERSION" && -n "$FRESH_TAG_URL" ]]; then
    ...
    if [[ -n "${ASTROCLAW_INSTALL_LATEST_OUT:-}" ]]; then
      ...
      if [[ -n "$latest_npm_version" ]]; then
        printf "%s" "$latest_npm_version" > "${ASTROCLAW_INSTALL_LATEST_OUT:-}"
      else
        printf "%s" "$FRESH_VERSION" > "${ASTROCLAW_INSTALL_LATEST_OUT:-}"
      fi
    fi
...
NODE
  ...
  if [[ "$SKIP_PREVIOUS" == "1" ]]; then
    echo "==> Skip preinstall previous (ASTROCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
  else
  ...
  echo "==> Verify installed version"
  if [[ -n "${ASTROCLAW_INSTALL_LATEST_OUT:-}" ]]; then
    printf "%s" "$LATEST_VERSION" > "${ASTROCLAW_INSTALL_LATEST_OUT:-}"
  fi
...
run_update_smoke() {
  if [[ -z "$UPDATE_EXPECT_VERSION" ]]; then
    echo "ERROR: ASTROCLAW_INSTALL_UPDATE_EXPECT_VERSION is required for update mode" >&2
    return 1
  fi
  if [[ -z "$UPDATE_TAG_URL" ]]; then
    echo "ERROR: ASTROCLAW_INSTALL_UPDATE_TAG_URL is required for update mode" >&2
    return 1
  fi
  ...
  echo "==> Run astroclaw update from host-served tgz"
  local update_status
  ...
  set +e
  UPDATE_JSON="$(
    run_with_heartbeat "astroclaw update" \
      env npm_config_omit=optional NPM_CONFIG_OMIT=optional ASTROCLAW_ALLOW_ROOT=1 \
      astroclaw update --tag "$UPDATE_TAG_URL" --yes --json 2>"$update_stderr_file"
  )"
  update_status=$?
  set -e
  ...
  if [[ "$update_status" -ne 0 ]]; then
    if is_self_swapped_package_process_exit "$update_stderr"; then
      ...
      echo "ERROR: astroclaw update failed with exit code $update_status" >&2
...
run_npm_global_smoke() {
  if [[ -z "$UPDATE_EXPECT_VERSION" ]]; then
    echo "ERROR: ASTROCLAW_INSTALL_UPDATE_EXPECT_VERSION is required for npm-global mode" >&2
    return 1
  fi
  if [[ -z "$UPDATE_TAG_URL" ]]; then
    echo "ERROR: ASTROCLAW_INSTALL_UPDATE_TAG_URL is required for npm-global mode" >&2
    return 1
  fi
...
case "$SMOKE_MODE" in
  ...
  npm-global)
    ...
    ;;
  *)
    echo "ERROR: unsupported ASTROCLAW_INSTALL_SMOKE_MODE=$SMOKE_MODE" >&2
    exit 1
    ;;
esac
