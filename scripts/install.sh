...
# Astroclaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://astroclaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Astroclaw."
...
GUM_VERSION="${ASTROCLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Astroclaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://astroclaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$ASTROCLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.astroclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_astroclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.astroclaw-* "$npm_root"/astroclaw 2>/dev/null || true
}

extract_astroclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_astroclaw_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/astroclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/astroclaw"|"/usr/local/bin/astroclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/astroclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale astroclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing astroclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Astroclaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_astroclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Astroclaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*astroclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_astroclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_astroclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Astroclaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an astroclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${ASTROCLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${ASTROCLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((ASTROCLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${ASTROCLAW_NO_ONBOARD:-0}
NO_PROMPT=${ASTROCLAW_NO_PROMPT:-0}
DRY_RUN=${ASTROCLAW_DRY_RUN:-0}
INSTALL_METHOD=${ASTROCLAW_INSTALL_METHOD:-}
ASTROCLAW_VERSION=${ASTROCLAW_VERSION:-latest}
USE_BETA=${ASTROCLAW_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/astroclaw"
GIT_DIR=${ASTROCLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${ASTROCLAW_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${ASTROCLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${ASTROCLAW_VERBOSE:-0}"
VERIFY_INSTALL="${ASTROCLAW_VERIFY_INSTALL:-0}"
ASTROCLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Astroclaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://astroclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/astroclaw)
...
Environment variables:
  ASTROCLAW_INSTALL_METHOD=git|npm
  ASTROCLAW_VERSION=latest|next|main|<semver>|<spec>
  ASTROCLAW_BETA=0|1
  ASTROCLAW_GIT_DIR=...
  ASTROCLAW_GIT_UPDATE=0|1
  ASTROCLAW_NO_PROMPT=1
  ASTROCLAW_VERIFY_INSTALL=1
  ASTROCLAW_DRY_RUN=1
  ASTROCLAW_NO_ONBOARD=1
  ASTROCLAW_VERBOSE=1
  ASTROCLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://astroclaw.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://astroclaw.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://astroclaw.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://astroclaw.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://astroclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Astroclaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Astroclaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_astroclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"astroclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://astroclaw.ai/install.sh | bash"
}
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future Astroclaw updates; use npm i -g astroclaw@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_astroclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/astroclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/astroclaw" ]]; then
        ln -sf "$npm_root/astroclaw/dist/entry.js" "${npm_bin}/astroclaw"
        ui_info "Created astroclaw bin link at ${npm_bin}/astroclaw"
    fi
    return 0
}

# Check for existing Astroclaw installation
check_existing_astroclaw() {
    if [[ -n "$(type -P astroclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing Astroclaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
resolve_git_astroclaw_ref() {
    local requested="${ASTROCLAW_VERSION:-latest}"
    local resolved_version=""

    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "astroclaw" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_astroclaw_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${ASTROCLAW_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_astroclaw_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/astroclaw"
        ...
        local version=""
        version="$(astroclaw_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_astroclaw_npm_root_candidates)
}

warn_duplicate_astroclaw_global_installs() {
    local installs=()
    local line=""
    while IFS= read -r line; do
        [[ -n "$line" ]] && installs+=("$line")
    done < <(find_astroclaw_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple Astroclaw global installs detected"
    echo "  Different Node/npm environments can run different Astroclaw versions."

    local active_node active_npm active_astroclaw
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_astroclaw="${ASTROCLAW_BIN:-}"
    if [[ -z "$active_astroclaw" ]]; then
        active_astroclaw="$(type -P astroclaw 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active astroclaw: ${INFO}${active_astroclaw:-none}${NC}"
    echo ""
    echo "  Found installs:"
    ...
    echo "    npm uninstall -g astroclaw"
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make astroclaw show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

astroclaw_command_for_user() {
    local claw="${1:-}"
    if [[ -z "$claw" ]]; then
        echo "astroclaw"
        return 0
...
warn_astroclaw_not_found() {
    ui_warn "Installed, but astroclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t astroclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named astroclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_astroclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P astroclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/astroclaw" ]]; then
        echo "${npm_bin}/astroclaw"
        return 0
    fi
...
install_astroclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/astroclaw/astroclaw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Astroclaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing Astroclaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Astroclaw" git clone "$repo_url" "$repo_dir"
    fi

    local git_ref
    git_ref="$(resolve_git_astroclaw_ref)"
    if [[ -z "$(git -C "$repo_dir" status --porcelain 2>/dev/null || true)" ]]; then
        ui_info "Using git ref: ${git_ref}"
        checkout_git_astroclaw_ref "$repo_dir" "$git_ref"
    else
    ...
    run_quiet_step "Building Astroclaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/astroclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/astroclaw"
    ui_success "Astroclaw wrapper installed to \$HOME/.local/bin/astroclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view astroclaw dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:astroclaw/astroclaw#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_astroclaw() {
    local package_name="astroclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            ASTROCLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="astroclaw"
        else
            ASTROCLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${ASTROCLAW_VERSION}" ]]; then
        ASTROCLAW_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${ASTROCLAW_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${ASTROCLAW_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Astroclaw v${resolved_version}"
    else
        ui_info "Installing Astroclaw (${ASTROCLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${ASTROCLAW_VERSION}")"

    if ! install_astroclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_astroclaw_paths
        install_astroclaw_npm "${install_spec}"
    fi

    if [[ "${ASTROCLAW_VERSION}" == "latest" && "${package_name}" == "astroclaw" ]]; then
        if ! resolve_astroclaw_bin &> /dev/null; then
            ui_warn "npm install astroclaw@latest failed; retrying astroclaw@next"
            cleanup_npm_astroclaw_paths
            install_astroclaw_npm "astroclaw@next"
        fi
    fi

    ensure_astroclaw_bin_link || true

    ui_success "Astroclaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${ASTROCLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_astroclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (astroclaw not on PATH yet)"
        warn_astroclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${ASTROCLAW_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.astroclaw/workspace-${profile}"
    else
        echo "${HOME}/.astroclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${ASTROCLAW_CONFIG_PATH:-$HOME/.astroclaw/astroclaw.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        local user_claw
        user_claw="$(astroclaw_command_for_user "${ASTROCLAW_BIN:-}")"
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but astroclaw not on PATH; skipping onboarding"
        warn_astroclaw_not_found
        return
    fi
    ...
    "$claw" onboard || {
        local user_claw
        user_claw="$(astroclaw_command_for_user "$claw")"
...
if ! declare -F extract_astroclaw_semver >/dev/null 2>&1; then
...
resolve_astroclaw_version() {
    ...
    local claw="${ASTROCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v astroclaw &> /dev/null; then
        claw="$(command -v astroclaw)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_astroclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/astroclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/astroclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: astroclaw not on PATH yet"
        warn_astroclaw_not_found
        return 1
    fi

    run_quiet_step "Checking Astroclaw version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: astroclaw gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_astroclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Astroclaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set ASTROCLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_astroclaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Astroclaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g astroclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g astroclaw 2>/dev/null || true
        ...
        install_astroclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/astroclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/astroclaw"
    ...
    ASTROCLAW_BIN="$(resolve_astroclaw_bin || true)"
    warn_duplicate_astroclaw_global_installs || true
    ...
    installed_version=$(resolve_astroclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Astroclaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Astroclaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/astroclaw"
        ui_kv "Update command" "astroclaw update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://astroclaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running astroclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                ASTROCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                ASTROCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                ASTROCLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
        ...
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (astroclaw not on PATH yet)"
                    warn_astroclaw_not_found
                    return 0
                fi
                exec </dev/tty
    ...
    if command -v astroclaw &> /dev/null; then
        local claw="${ASTROCLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_astroclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (astroclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if ASTROCLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: astroclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${ASTROCLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
