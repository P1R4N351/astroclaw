...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  ASTROCLAW_SKIP_ACPX_RUNTIME: "1",
  ASTROCLAW_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$ASTROCLAW_WATCH_PID_FILE"',
    'mkdir -p "$ASTROCLAW_HOME/.astroclaw"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$ASTROCLAW_HOME/.astroclaw/astroclaw.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    ASTROCLAW_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    ASTROCLAW_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-gateway-watch-"));
