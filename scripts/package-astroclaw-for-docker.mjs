#!/usr/bin/env node
// Builds the Astroclaw package artifact used by Docker E2E.
...
async function newestAstroclawTarball(outputDir, packOutput) {
  let fromOutput = "";
  ...
  const packed = entries
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed Astroclaw tarball in ${outputDir}`);
  }
...
async function main() {
  ...
  if (!options.skipBuild) {
    console.error("==> Building Astroclaw package artifacts");
    await run("pnpm", ["build"], sourceDir);
    console.error("==> Building Astroclaw Control UI artifacts");
    await run("pnpm", ["ui:build"], sourceDir);
  }

  console.error("==> Writing Astroclaw package inventory");
  await run(
    "node",
    [
      "--import",
  ...
  console.error("==> Packing Astroclaw package");
  const packOutput = await runCapture(
    "npm",
    ["pack", "--silent", "--ignore-scripts", "--pack-destination", outputDir],
    sourceDir,
  );
  let tarball = await newestAstroclawTarball(outputDir, packOutput);
  ...
  console.error("==> Checking Astroclaw package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-astroclaw-package-tarball.mjs"), tarball],
    sourceDir,
    { timeoutMs: 5 * 60 * 1000 },
  );
  console.error(
