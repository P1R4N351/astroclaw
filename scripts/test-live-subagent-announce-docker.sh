...
if [[ -n "${OPENAI_API_KEY:-}" || -n "${OPENAI_BASE_URL:-}" || -n "${GEMINI_API_KEY:-}" || -n "${GOOGLE_API_KEY:-}" ]]; then
  docker_env_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/astroclaw-subagent-live-env.XXXXXX")"
  TEMP_DIRS+=("$docker_env_dir")
  docker_env_file="$docker_env_dir/provider.env"
  {
    if [[ -n "${OPENAI_API_KEY:-}" ]]; then
      printf 'ASTROCLAW_DOCKER_LIVE_OPENAI_API_KEY=%s\n' "${OPENAI_API_KEY}"
    fi
    if [[ -n "${OPENAI_BASE_URL:-}" ]]; then
      printf 'ASTROCLAW_DOCKER_LIVE_OPENAI_BASE_URL=%s\n' "${OPENAI_BASE_URL}"
    fi
    if [[ -n "${GEMINI_API_KEY:-}" ]]; then
      printf 'ASTROCLAW_DOCKER_LIVE_GEMINI_API_KEY=%s\n' "${GEMINI_API_KEY}"
    fi
    if [[ -n "${GOOGLE_API_KEY:-}" ]]; then
      printf 'ASTROCLAW_DOCKER_LIVE_GOOGLE_API_KEY=%s\n' "${GOOGLE_API_KEY}"
    fi
  } >"$docker_env_file"
  DOCKER_EXTRA_ENV_FILES+=(--env-file "$docker_env_file")
fi

CONTAINER_NODE_OPTIONS="${ASTROCLAW_DOCKER_NODE_OPTIONS:-${NODE_OPTIONS:-}}"
if [[ -z "$(astroclaw_live_trim "$CONTAINER_NODE_OPTIONS")" ]]; then
...
if [ -n "${ASTROCLAW_DOCKER_LIVE_OPENAI_API_KEY:-}" ]; then
  export OPENAI_API_KEY="$ASTROCLAW_DOCKER_LIVE_OPENAI_API_KEY"
  unset ASTROCLAW_DOCKER_LIVE_OPENAI_API_KEY
fi
if [ -n "${ASTROCLAW_DOCKER_LIVE_OPENAI_BASE_URL:-}" ]; then
  export OPENAI_BASE_URL="$ASTROCLAW_DOCKER_LIVE_OPENAI_BASE_URL"
  unset ASTROCLAW_DOCKER_LIVE_OPENAI_BASE_URL
fi
if [ -n "${ASTROCLAW_DOCKER_LIVE_GEMINI_API_KEY:-}" ]; then
  export GEMINI_API_KEY="$ASTROCLAW_DOCKER_LIVE_GEMINI_API_KEY"
  unset ASTROCLAW_DOCKER_LIVE_GEMINI_API_KEY
fi
if [ -n "${ASTROCLAW_DOCKER_LIVE_GOOGLE_API_KEY:-}" ]; then
  export GOOGLE_API_KEY="$ASTROCLAW_DOCKER_LIVE_GOOGLE_API_KEY"
  unset ASTROCLAW_DOCKER_LIVE_GOOGLE_API_KEY
fi
...
ASTROCLAW_LIVE_SUBAGENT_E2E=1 \
ASTROCLAW_VITEST_MAX_WORKERS="${ASTROCLAW_VITEST_MAX_WORKERS:-1}" \
...
echo "==> Model: ${ASTROCLAW_LIVE_SUBAGENT_E2E_MODEL:-openai/gpt-5.5}"
...
DOCKER_RUN_ARGS=(docker run --rm -t \
  ...
  -e ASTROCLAW_LIVE_TEST_QUIET="${ASTROCLAW_LIVE_TEST_QUIET:-}" \
  -e ASTROCLAW_LIVE_WRAPPER_HEARTBEAT_MS="${ASTROCLAW_LIVE_WRAPPER_HEARTBEAT_MS:-}" \
  -e ASTROCLAW_LIVE_SUBAGENT_E2E=1 \
  -e ASTROCLAW_LIVE_SUBAGENT_E2E_MODEL="${ASTROCLAW_LIVE_SUBAGENT_E2E_MODEL:-}" \
  -e ASTROCLAW_VITEST_FS_MODULE_CACHE=0 \
  -e ASTROCLAW_VITEST_MAX_WORKERS="${ASTROCLAW_VITEST_MAX_WORKERS:-1}")
