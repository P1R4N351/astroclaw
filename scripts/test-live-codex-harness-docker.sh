...
TRUSTED_HARNESS_DIR="${ASTROCLAW_LIVE_DOCKER_TRUSTED_HARNESS_DIR:-${ASTROCLAW_LIVE_CODEX_TRUSTED_HARNESS_DIR:-$SCRIPT_ROOT_DIR}}"
...
CODEX_HARNESS_AUTH_MODE="${ASTROCLAW_LIVE_CODEX_HARNESS_AUTH:-codex-auth}"
TEMP_DIRS=()
DOCKER_USER="${ASTROCLAW_DOCKER_USER:-node}"
DOCKER_HOME_MOUNT=()
DOCKER_TRUSTED_HARNESS_MOUNT=()
DOCKER_TRUSTED_HARNESS_CONTAINER_DIR=""
DOCKER_CACHE_CONTAINER_DIR="/tmp/astroclaw-cache"
DOCKER_CLI_TOOLS_CONTAINER_DIR="/tmp/astroclaw-npm-global"
DOCKER_EXTRA_ENV_FILES=()
DOCKER_AUTH_PRESTAGED=0

astroclaw_live_codex_harness_is_ci() {
  [[ -n "${CI:-}" && "${CI:-}" != "false" ]] || [[ -n "${GITHUB_ACTIONS:-}" && "${GITHUB_ACTIONS:-}" != "false" ]]
}

astroclaw_live_codex_harness_append_build_extension() {
  local extension="${1:?extension required}"
  local current="${ASTROCLAW_DOCKER_BUILD_EXTENSIONS:-${ASTROCLAW_EXTENSIONS:-}}"
  case " $current " in
    *" $extension "*)
      ;;
    *)
      export ASTROCLAW_DOCKER_BUILD_EXTENSIONS="${current:+$current }$extension"
      ;;
  esac
}
...
case "$CODEX_HARNESS_AUTH_MODE" in
  codex-auth | api-key)
    ;;
  *)
    echo "ERROR: ASTROCLAW_LIVE_CODEX_HARNESS_AUTH must be one of: codex-auth, api-key." >&2
    exit 1
    ;;
esac
...
if [[ "$CODEX_HARNESS_AUTH_MODE" == "api-key" && -z "${OPENAI_API_KEY:-}" ]]; then
  echo "ERROR: ASTROCLAW_LIVE_CODEX_HARNESS_AUTH=api-key requires OPENAI_API_KEY." >&2
  exit 1
fi
if [[ "$CODEX_HARNESS_AUTH_MODE" != "api-key" && ! -s "$HOME/.codex/auth.json" ]]; then
  echo "ERROR: ASTROCLAW_LIVE_CODEX_HARNESS_AUTH=codex-auth requires ~/.codex/auth.json before building the live Docker image." >&2
  if [[ -n "${OPENAI_API_KEY:-}" ]]; then
    echo "If this is a Testbox/API-key run, set ASTROCLAW_LIVE_CODEX_HARNESS_AUTH=api-key and run through astroclaw-testbox-env." >&2
  fi
  exit 1
fi
...
if [[ -n "${ASTROCLAW_DOCKER_CLI_TOOLS_DIR:-}" ]]; then
  CLI_TOOLS_DIR="${ASTROCLAW_DOCKER_CLI_TOOLS_DIR}"
elif astroclaw_live_codex_harness_is_ci; then
  CLI_TOOLS_DIR="$(mktemp -d "${RUNNER_TEMP:-/tmp}/astroclaw-docker-cli-tools.XXXXXX")"
  TEMP_DIRS+=("$CLI_TOOLS_DIR")
else
  CLI_TOOLS_DIR="$HOME/.cache/astroclaw/docker-cli-tools"
fi
...
if astroclaw_live_codex_harness_is_ci; then
...
if [[ "$CODEX_HARNESS_AUTH_MODE" != "api-key" ]]; then
  ...
  done < <(astroclaw_live_collect_auth_files_from_csv "openai-codex")
fi
...
if [[ -n "${DOCKER_HOME_DIR:-}" ]]; then
  astroclaw_live_stage_auth_into_home "$DOCKER_HOME_DIR" --files "${AUTH_FILES[@]}"
...
DOCKER_AUTH_ENV=()
if [[ "$CODEX_HARNESS_AUTH_MODE" == "api-key" ]]; then
  docker_env_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/astroclaw-codex-harness-env.XXXXXX")"
...
if [ "${ASTROCLAW_LIVE_CODEX_HARNESS_DEBUG:-}" = "1" ]; then
  id
  mount | grep -E 'astroclaw-cache|astroclaw-npm|/home/node' || true
...
if [ "${ASTROCLAW_LIVE_CODEX_HARNESS_AUTH:-codex-auth}" != "api-key" ]; then
...
if [ "${ASTROCLAW_LIVE_CODEX_HARNESS_AUTH:-codex-auth}" != "api-key" ] && [ ! -s "$HOME/.codex/auth.json" ]; then
...
if [ -d /app/dist-runtime/extensions/codex ]; then
  export ASTROCLAW_BUNDLED_PLUGINS_DIR=/app/dist-runtime/extensions
elif [ -d /app/dist/extensions/codex ]; then
  export ASTROCLAW_BUNDLED_PLUGINS_DIR=/app/dist/extensions
elif [ -f "$tmp_dir/extensions/codex/astroclaw.plugin.json" ]; then
  export ASTROCLAW_BUNDLED_PLUGINS_DIR="$tmp_dir/extensions"
else
  echo "ERROR: staged Codex plugin not found for live harness." >&2
  exit 1
fi
astroclaw_live_stage_state_dir "$tmp_dir/.astroclaw-state"
if [ -n "${ASTROCLAW_LIVE_CODEX_TRUSTED_HARNESS_DIR:-}" ] && [ -d "$ASTROCLAW_LIVE_CODEX_TRUSTED_HARNESS_DIR" ]; then
  for harness_file in src/gateway/gateway-codex-harness.live-helpers.ts; do
    if [ -f "$ASTROCLAW_LIVE_CODEX_TRUSTED_HARNESS_DIR/$harness_file" ]; then
      mkdir -p "$(dirname "$tmp_dir/$harness_file")"
      cp "$ASTROCLAW_LIVE_CODEX_TRUSTED_HARNESS_DIR/$harness_file" "$tmp_dir/$harness_file"
    fi
  done
fi
astroclaw_live_prepare_staged_config
cd "$tmp_dir"
if [ "${ASTROCLAW_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG:-1}" = "1" ]; then
...
node scripts/test-live.mjs -- ${ASTROCLAW_LIVE_CODEX_TEST_FILES:-src/gateway/gateway-codex-harness.live.test.ts}
EOF

astroclaw_live_codex_harness_append_build_extension codex
...
ASTROCLAW_SKIP_DOCKER_BUILD=0
export ASTROCLAW_SKIP_DOCKER_BUILD
...
echo "==> Model: ${ASTROCLAW_LIVE_CODEX_HARNESS_MODEL:-codex/gpt-5.5}"
echo "==> Image probe: ${ASTROCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE:-1}"
echo "==> MCP probe: ${ASTROCLAW_LIVE_CODEX_HARNESS_MCP_PROBE:-1}"
echo "==> Subagent probe: ${ASTROCLAW_LIVE_CODEX_HARNESS_SUBAGENT_PROBE:-1}"
echo "==> Subagent-only fast path: ${ASTROCLAW_LIVE_CODEX_HARNESS_SUBAGENT_ONLY:-auto}"
echo "==> Guardian probe: ${ASTROCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE:-1}"
echo "==> Auth mode: $CODEX_HARNESS_AUTH_MODE"
echo "==> Profile file: $PROFILE_STATUS"
echo "==> CI-safe Codex config: ${ASTROCLAW_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG:-1}"
echo "==> Test files: ${ASTROCLAW_LIVE_CODEX_TEST_FILES:-src/gateway/gateway-codex-harness.live.test.ts}"
...
DOCKER_RUN_ARGS=(docker run --rm -t \
  ...
  -e ASTROCLAW_AGENT_HARNESS_FALLBACK=none \
  -e ASTROCLAW_DOCKER_AUTH_PRESTAGED="$DOCKER_AUTH_PRESTAGED" \
  -e ASTROCLAW_CODEX_APP_SERVER_BIN="${ASTROCLAW_CODEX_APP_SERVER_BIN:-codex}" \
  ...
  -e ASTROCLAW_LIVE_CODEX_HARNESS_AUTH="$CODEX_HARNESS_AUTH_MODE" \
  -e ASTROCLAW_LIVE_CODEX_HARNESS=1 \
  -e ASTROCLAW_LIVE_CODEX_HARNESS_DEBUG="${ASTROCLAW_LIVE_CODEX_HARNESS_DEBUG:-}" \
  -e ASTROCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE="${ASTROCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE:-1}" \
  -e ASTROCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE="${ASTROCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE:-1}" \
  -e ASTROCLAW_LIVE_CODEX_HARNESS_MCP_PROBE="${ASTROCLAW_LIVE_CODEX_HARNESS_MCP_PROBE:-1}" \
  -e ASTROCLAW_LIVE_CODEX_HARNESS_MODEL="${ASTROCLAW_LIVE_CODEX_HARNESS_MODEL:-codex/gpt-5.5}" \
  -e ASTROCLAW_LIVE_CODEX_HARNESS_REQUIRE_GUARDIAN_EVENTS="${ASTROCLAW_LIVE_CODEX_HARNESS_REQUIRE_GUARDIAN_EVENTS:-1}" \
  -e ASTROCLAW_LIVE_CODEX_HARNESS_REQUEST_TIMEOUT_MS="${ASTROCLAW_LIVE_CODEX_HARNESS_REQUEST_TIMEOUT_MS:-}" \
  -e ASTROCLAW_LIVE_CODEX_HARNESS_SUBAGENT_ONLY="${ASTROCLAW_LIVE_CODEX_HARNESS_SUBAGENT_ONLY:-}" \
  -e ASTROCLAW_LIVE_CODEX_HARNESS_SUBAGENT_PROBE="${ASTROCLAW_LIVE_CODEX_HARNESS_SUBAGENT_PROBE:-1}" \
  -e ASTROCLAW_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG="${ASTROCLAW_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG:-1}" \
  -e ASTROCLAW_CLI_BACKEND_LOG_OUTPUT="${ASTROCLAW_CLI_BACKEND_LOG_OUTPUT:-}" \
  -e ASTROCLAW_TEST_CONSOLE="${ASTROCLAW_TEST_CONSOLE:-}" \
  -e ASTROCLAW_LIVE_DOCKER_SCRIPTS_DIR="${DOCKER_TRUSTED_HARNESS_CONTAINER_DIR}/scripts" \
  -e ASTROCLAW_LIVE_DOCKER_TRUSTED_HARNESS_DIR="$DOCKER_TRUSTED_HARNESS_CONTAINER_DIR" \
  -e ASTROCLAW_LIVE_CODEX_TRUSTED_HARNESS_DIR="$DOCKER_TRUSTED_HARNESS_CONTAINER_DIR" \
  -e ASTROCLAW_LIVE_CODEX_BIND="${ASTROCLAW_LIVE_CODEX_BIND:-}" \
  -e ASTROCLAW_LIVE_CODEX_BIND_MODEL="${ASTROCLAW_LIVE_CODEX_BIND_MODEL:-}" \
  -e ASTROCLAW_LIVE_CODEX_BIND_PROVIDER="${ASTROCLAW_LIVE_CODEX_BIND_PROVIDER:-}" \
  -e ASTROCLAW_LIVE_CODEX_TEST_FILES="${ASTROCLAW_LIVE_CODEX_TEST_FILES:-}" \
  -e ASTROCLAW_LIVE_TEST=1 \
  -e ASTROCLAW_VITEST_FS_MODULE_CACHE=0)
astroclaw_live_append_array DOCKER_RUN_ARGS DOCKER_AUTH_ENV
astroclaw_live_append_array DOCKER_RUN_ARGS DOCKER_EXTRA_ENV_FILES
