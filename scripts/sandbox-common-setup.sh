...
BASE_IMAGE="${BASE_IMAGE:-astroclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-astroclaw-sandbox-common:bookworm-slim}"
...
ASTROCLAW_DOCKER_BUILD_USE_BUILDX="${ASTROCLAW_DOCKER_BUILD_USE_BUILDX:-0}"
ASTROCLAW_DOCKER_BUILD_CACHE_FROM="${ASTROCLAW_DOCKER_BUILD_CACHE_FROM:-}"
ASTROCLAW_DOCKER_BUILD_CACHE_TO="${ASTROCLAW_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=astroclaw.sandbox=1)
NOTE
