...
function usage() {
  return "Usage: node scripts/check-astroclaw-package-tarball.mjs <astroclaw.tgz>";
}
...
if (!fs.existsSync(tarball)) {
  fail(`Astroclaw package tarball does not exist: ${tarball}`);
}

const phaseTimingsEnabled = process.env.ASTROCLAW_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-astroclaw-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-package-tarball-"));
try {
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`Astroclaw package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`Astroclaw package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("Astroclaw package tarball integrity passed.");
