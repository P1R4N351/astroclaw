...
// Usage:
// ASTROCLAW_PROMPT_TRANSPORT=direct|gateway
// ASTROCLAW_PROMPT_MODE=extra|override
// ASTROCLAW_PROMPT_TEXT='...'
// ASTROCLAW_PROMPT_CAPTURE=1
...
const TRANSPORT = process.env.ASTROCLAW_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE =
  process.env.ASTROCLAW_PROMPT_MODE?.trim() === "override" ? "override" : "extra";
const PROMPT_TEXT = process.env.ASTROCLAW_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.ASTROCLAW_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.ASTROCLAW_USER_PROMPT?.trim() || "is clawd here?";
const ENABLE_CAPTURE = process.env.ASTROCLAW_PROMPT_CAPTURE === "1";
const INCLUDE_RAW = process.env.ASTROCLAW_PROMPT_INCLUDE_RAW === "1";
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.ASTROCLAW_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = Number(process.env.ASTROCLAW_PROMPT_TIMEOUT_MS ?? "45000");
const GATEWAY_TIMEOUT_MS = Number(process.env.ASTROCLAW_PROMPT_GATEWAY_TIMEOUT_MS ?? "120000");
...
if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
  throw new Error("missing ASTROCLAW_PROMPT_TEXT or ASTROCLAW_PROMPT_LIST_JSON");
}
...
function resolveSetupTokenSource(): TokenSource {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set ASTROCLAW_LIVE_SETUP_TOKEN_VALUE or ASTROCLAW_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(prompt: string): Promise<PromptResult> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["astroclaw.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        ASTROCLAW_CONFIG_PATH: params.configPath,
        ASTROCLAW_STATE_DIR: params.stateDir,
        ASTROCLAW_AGENT_DIR: params.agentDir,
        ASTROCLAW_GATEWAY_TOKEN: params.gatewayToken,
        ASTROCLAW_SKIP_CHANNELS: "1",
        ...
        ASTROCLAW_SKIP_CRON: "1",
        ASTROCLAW_TEST_MINIMAL_GATEWAY: "1",
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-gateway-prompt-probe-"));
  const stateDir = path.join(tmpDir, "state");
  const agentDir = path.join(stateDir, "agents", "main", "agent");
  const bundledPluginsDir = path.join(tmpDir, "bundled-plugins-empty");
  const configPath = path.join(tmpDir, "astroclaw.json");
