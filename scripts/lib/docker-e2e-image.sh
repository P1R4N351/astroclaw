...
# build the runner/functional images with the prepared Astroclaw package tarball.
...
docker_e2e_resolve_image() {
  ...
  if [ -n "${ASTROCLAW_DOCKER_E2E_IMAGE:-}" ]; then
    printf '%s\n' "$ASTROCLAW_DOCKER_E2E_IMAGE"
    return 0
  fi
...
docker_e2e_build_or_reuse() {
  ...
  if [ "${ASTROCLAW_SKIP_DOCKER_BUILD:-0}" = "1" ] || [ "$skip_build" = "1" ]; then
    ...
    if ! docker image inspect "$image_name" >/dev/null 2>&1; then
      ...
      if docker_build_on_missing_enabled; then
        echo "Docker image not available; building because ASTROCLAW_DOCKER_BUILD_ON_MISSING/ASTROCLAW_TESTBOX allows fallback."
      else
        echo "Docker image not found: $image_name" >&2
        echo "Build it first or unset ASTROCLAW_SKIP_DOCKER_BUILD." >&2
        return 1
      fi
    else
      return 0
  ...
  if [ "$target" = "functional" ]; then
    ...
    build_args+=(--build-context "astroclaw_package=$package_context")
  fi
...
docker_e2e_test_state_function_b64() {
  node "$ROOT_DIR/scripts/lib/astroclaw-test-state.mjs" shell-function |
    base64 |
    tr -d '\n'
}
