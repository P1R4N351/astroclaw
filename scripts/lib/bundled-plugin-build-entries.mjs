...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@astroclaw/${params.dirName}`) {
    return false;
  }
...
function shouldBuildBundledDistEntry(packageJson) {
  return packageJson?.astroclaw?.build?.bundledDist !== false;
}

export function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.astroclaw?.extensions)
    ? packageJson.astroclaw.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.astroclaw?.setupEntry === "string" &&
    packageJson.astroclaw.setupEntry.trim().length > 0
...
export function collectBundledPluginBuildEntries(params = {}) {
  ...
  for (const candidate of collectBundledPluginCandidates(cwd, extensionsRoot)) {
    ...
    const hasManifest = relativeFiles?.includes("astroclaw.plugin.json") ?? fs.existsSync(manifestPath);
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "astroclaw.plugin.json"));
    }
    if (hasPackageJson) {
