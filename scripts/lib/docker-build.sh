...
docker_build_on_missing_enabled() {
  case "${ASTROCLAW_DOCKER_BUILD_ON_MISSING:-}" in
    1 | true | TRUE | yes | YES)
      return 0
      ;;
    0 | false | FALSE | no | NO)
  ...
  [ "${ASTROCLAW_TESTBOX:-0}" = "1" ]
}

docker_build_command() {
  local build_cmd=(docker build)
  if [ "${ASTROCLAW_DOCKER_BUILD_USE_BUILDX:-0}" = "1" ]; then
    build_cmd=(docker buildx build --load)
    if [ -n "${ASTROCLAW_DOCKER_BUILD_CACHE_FROM:-}" ]; then
      build_cmd+=(--cache-from "${ASTROCLAW_DOCKER_BUILD_CACHE_FROM}")
    fi
    if [ -n "${ASTROCLAW_DOCKER_BUILD_CACHE_TO:-}" ]; then
      build_cmd+=(--cache-to "${ASTROCLAW_DOCKER_BUILD_CACHE_TO}")
    fi
  fi
...
docker_build_retry_count() {
  local configured="${ASTROCLAW_DOCKER_BUILD_RETRIES:-2}"
