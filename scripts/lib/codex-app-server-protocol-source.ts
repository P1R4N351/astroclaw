...
export async function resolveCodexAppServerProtocolSource(repoRoot: string): Promise<{
  ...
  throw new Error(
    [
      ...
      "Set ASTROCLAW_CODEX_REPO to a checkout of openai/codex, or keep a sibling `codex` checkout next to the primary Astroclaw checkout.",
...
async function collectCodexRepoCandidates(repoRoot: string): Promise<string[]> {
  const candidates = [
    process.env.ASTROCLAW_CODEX_REPO,
