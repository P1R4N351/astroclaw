...
function isSystemThrottleDisabled(env) {
  const normalized = env.ASTROCLAW_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestEnv(env = process.env) {
  const normalizedLocalCheck = env.ASTROCLAW_LOCAL_CHECK?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.ASTROCLAW_VITEST_MAX_WORKERS ?? env.ASTROCLAW_TEST_WORKERS);
  if (override !== null) {
