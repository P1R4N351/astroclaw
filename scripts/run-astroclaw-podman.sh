#!/usr/bin/env bash
# Rootless Astroclaw in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-astroclaw-podman.sh launch        # Start gateway
#   ./scripts/run-astroclaw-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   astroclaw --container astroclaw dashboard --no-open
#   astroclaw --container astroclaw channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      ASTROCLAW_GATEWAY_TOKEN|ASTROCLAW_PODMAN_CONTAINER|ASTROCLAW_PODMAN_IMAGE|ASTROCLAW_IMAGE|ASTROCLAW_PODMAN_PULL|ASTROCLAW_PODMAN_GATEWAY_HOST_PORT|ASTROCLAW_GATEWAY_PORT|ASTROCLAW_PODMAN_BRIDGE_HOST_PORT|ASTROCLAW_BRIDGE_PORT|ASTROCLAW_GATEWAY_BIND|ASTROCLAW_PODMAN_USERNS|ASTROCLAW_BIND_MOUNT_OPTIONS|ASTROCLAW_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-astroclaw-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${ASTROCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.astroclaw}"
ENV_FILE="${ASTROCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${ASTROCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.astroclaw}"
ENV_FILE="${ASTROCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${ASTROCLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${ASTROCLAW_PODMAN_CONTAINER:-astroclaw}"
ASTROCLAW_IMAGE="${ASTROCLAW_PODMAN_IMAGE:-${ASTROCLAW_IMAGE:-astroclaw:local}}"
PODMAN_PULL="${ASTROCLAW_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${ASTROCLAW_PODMAN_GATEWAY_HOST_PORT:-${ASTROCLAW_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${ASTROCLAW_PODMAN_BRIDGE_HOST_PORT:-${ASTROCLAW_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${ASTROCLAW_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$ASTROCLAW_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  ASTROCLAW_CONTAINER="" ASTROCLAW_CONFIG_DIR="$config_dir" \
    astroclaw config get gateway.bind 2>/dev/null || true
}
...
# ASTROCLAW_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${ASTROCLAW_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'ASTROCLAW_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v astroclaw >/dev/null 2>&1; then
    echo "Warning: astroclaw not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    ASTROCLAW_CONTAINER="" ASTROCLAW_CONFIG_DIR="$config_dir" \
      astroclaw config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    ASTROCLAW_CONTAINER="" ASTROCLAW_CONFIG_DIR="$config_dir" \
      astroclaw config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  ASTROCLAW_CONTAINER="" ASTROCLAW_CONFIG_DIR="$config_dir" \
    astroclaw config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${ASTROCLAW_GATEWAY_TOKEN:-}" ]]; then
  export ASTROCLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "ASTROCLAW_GATEWAY_TOKEN" "$ASTROCLAW_GATEWAY_TOKEN"
  echo "Generated ASTROCLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/astroclaw.json"
...
PODMAN_USERNS="${ASTROCLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported ASTROCLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (ASTROCLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${ASTROCLAW_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${ASTROCLAW_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$ASTROCLAW_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.astroclaw/.npm \
    -e ASTROCLAW_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.astroclaw:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.astroclaw/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$ASTROCLAW_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME astroclaw dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME astroclaw devices approve --latest  # if pairing required"
