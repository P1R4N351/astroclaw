...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.astroclaw?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.astroclaw?.channel?.order;
      const labelRaw = parsed.astroclaw?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".astroclaw-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "astroclaw-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "astroclaw-build",
...
function renderSourceBrowserHelpText(
  ...
  const result = spawnSync(
    ...
    ["--import", "tsx", "--input-type=module", "--eval", inlineModule],
    {
      ...
      env: {
        ...renderContext.env,
        ASTROCLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
      },
