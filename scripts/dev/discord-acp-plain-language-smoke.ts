...
type DriverMode = "token" | "webhook" | "astroclaw";
...
function resolveStateDir(): string {
  const override = process.env.ASTROCLAW_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.ASTROCLAW_HOME?.trim() || process.env.HOME || "";
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver astroclaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Astroclaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|astroclaw> Driver transport mode (default: token)\n" +
    ...
    "  --astroclaw-bin <path>        Astroclaw CLI binary for driver=astroclaw (default: astroclaw)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  ASTROCLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  ASTROCLAW_DISCORD_SMOKE_DRIVER\n" +
    "  ASTROCLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  ASTROCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  ASTROCLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  ASTROCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  ASTROCLAW_DISCORD_SMOKE_AGENT\n" +
    "  ASTROCLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  ASTROCLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  ASTROCLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  ASTROCLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  ASTROCLAW_DISCORD_SMOKE_ASTROCLAW_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.ASTROCLAW_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.ASTROCLAW_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "astroclaw"
        ? "astroclaw"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.ASTROCLAW_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.ASTROCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.ASTROCLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.ASTROCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.ASTROCLAW_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.ASTROCLAW_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.ASTROCLAW_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.ASTROCLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.ASTROCLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.ASTROCLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const astroclawBin =
    resolveArg("--astroclaw-bin") || process.env.ASTROCLAW_DISCORD_SMOKE_ASTROCLAW_BIN || "astroclaw";
...
async function astroclawCliJson<T>(params: { astroclawBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.astroclawBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`astroclaw ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithAstroclaw(params: {
  astroclawBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await astroclawCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    astroclawBin: params.astroclawBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "astroclaw") {
    return await readMessagesWithAstroclaw({
      astroclawBin: params.args.astroclawBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `astroclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await astroclawCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        astroclawBin: args.astroclawBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("astroclaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "astroclaw"
            ? await readMessagesWithAstroclaw({
                astroclawBin: args.astroclawBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Astroclaw.`,
        diagnostics: {
          bindingCandidates: [
            {
