...
export const CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS = [
  "astroclaw/plugin-sdk/agent-runtime-test-contracts",
  "astroclaw/plugin-sdk/plugin-test-contracts",
  "astroclaw/plugin-sdk/provider-test-contracts",
] as const;
...
function resolveInstalledBinaryPath(prefixDir: string): string {
  return process.platform === "win32"
    ? join(prefixDir, "astroclaw.cmd")
    : join(prefixDir, "bin", "astroclaw");
}
...
export function collectPackedInstalledPackageVerificationErrors(params: {
  ...
  if (
    ...
    errors.push(
      `installed astroclaw binary version mismatch: expected ${params.expectedVersion}, found ${params.installedBinaryVersion || "<missing>"}.`,
    );
  }
  return errors;
}
...
export function writePackedBundledPluginActivationConfig(homeDir: string): void {
  const configPath = join(homeDir, ".astroclaw", "astroclaw.json");
  mkdirSync(join(homeDir, ".astroclaw"), { recursive: true });
  writeFileSync(
    configPath,
    `${JSON.stringify(
      {
        ...
        models: {
          providers: {
            openai: {
              apiKey: "sk-astroclaw-release-check",
...
function runPackedBundledPluginActivationSmoke(packageRoot: string, tmpRoot: string): void {
  ...
  const env = createPackedCliSmokeEnv(process.env, {
    HOME: homeDir,
    OPENAI_API_KEY: "sk-astroclaw-release-check",
  });

  writePackedBundledPluginActivationConfig(homeDir);
  execFileSync(
    process.execPath,
    [join(packageRoot, "astroclaw.mjs"), ...PACKED_BUNDLED_RUNTIME_DEPS_REPAIR_ARGS],
    {
      cwd: packageRoot,
      stdio: "inherit",
      env,
  ...
  execFileSync(process.execPath, [join(packageRoot, "astroclaw.mjs"), "plugins", "doctor"], {
    cwd: packageRoot,
    stdio: "inherit",
    env,
  });
...
function runPackedBundledChannelEntrySmoke(): void {
  const tmpRoot = mkdtempSync(join(tmpdir(), "astroclaw-release-pack-smoke-"));
  try {
    ...
    const packageRoot = join(resolveGlobalRoot(prefixDir, tmpRoot), "astroclaw");
    ...
    execFileSync(
      process.execPath,
      [join(packageRoot, "astroclaw.mjs"), ...PACKED_COMPLETION_SMOKE_ARGS],
      {
        cwd: packageRoot,
        stdio: "inherit",
  ...
  return [...paths]
    .filter(
      (path) =>
        ...
        /(^|\/)\.astroclaw-runtime-deps-[^/]+(\/|$)/u.test(path) ||
        path.endsWith("/.astroclaw-runtime-deps-stamp.json") ||
...
// Critical functions that channel extension plugins import from astroclaw/plugin-sdk.
