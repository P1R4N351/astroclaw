...
- ASTROCLAW_IOS_VERSION = exact canonical iOS version
- ASTROCLAW_MARKETING_VERSION = short iOS/App Store version
- ASTROCLAW_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    ASTROCLAW_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    ASTROCLAW_MARKETING_VERSION)
...
ASTROCLAW_IOS_VERSION = ${IOS_VERSION}
ASTROCLAW_MARKETING_VERSION = ${MARKETING_VERSION}
ASTROCLAW_BUILD_VERSION = ${BUILD_NUMBER}
EOF
