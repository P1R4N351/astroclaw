...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/astroclaw/astroclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Astroclaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AstroclawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AstroclawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Astroclaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Astroclaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Astroclaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Astroclaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Astroclaw/Resources/Astroclaw.icns" "$APP_ROOT/Contents/Resources/Astroclaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Astroclaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying AstroclawKit resources"
ASTROCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AstroclawKit_AstroclawKit.bundle"
if [ -d "$ASTROCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AstroclawKit_AstroclawKit.bundle"
  cp -R "$ASTROCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AstroclawKit_AstroclawKit.bundle"
else
  echo "WARN: AstroclawKit resource bundle not found at $ASTROCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Astroclaw"
killall -q Astroclaw 2>/dev/null || true
