...
function resolveCliShardSpec(args, env) {
  ...
  return env.ASTROCLAW_ADDITIONAL_BOUNDARY_SHARD ?? "";
}

if (import.meta.url === `file://${process.argv[1]}`) {
  const concurrency = resolveConcurrency(
    process.env.ASTROCLAW_ADDITIONAL_BOUNDARY_CONCURRENCY ??
      process.env.ASTROCLAW_EXTENSION_BOUNDARY_CONCURRENCY,
  );
