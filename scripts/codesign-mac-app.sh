...
APP_BUNDLE="${1:-dist/Astroclaw.app}"
IDENTITY="${SIGN_IDENTITY:-}"
TIMESTAMP_MODE="${CODESIGN_TIMESTAMP:-auto}"
DISABLE_LIBRARY_VALIDATION="${DISABLE_LIBRARY_VALIDATION:-0}"
SKIP_TEAM_ID_CHECK="${SKIP_TEAM_ID_CHECK:-0}"
ENT_TMP_BASE=$(mktemp -t astroclaw-entitlements-base.XXXXXX)
ENT_TMP_APP_BASE=$(mktemp -t astroclaw-entitlements-app-base.XXXXXX)
ENT_TMP_RUNTIME=$(mktemp -t astroclaw-entitlements-runtime.XXXXXX)
...
MLX_TTS_HELPER="$APP_BUNDLE/Contents/MacOS/astroclaw-mlx-tts"
if [ -f "$MLX_TTS_HELPER" ]; then
  echo "Signing MLX TTS helper"; sign_item "$MLX_TTS_HELPER" "$APP_ENTITLEMENTS"
fi

# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/Astroclaw" ]; then
  echo "Signing main binary"; sign_item "$APP_BUNDLE/Contents/MacOS/Astroclaw" "$APP_ENTITLEMENTS"
fi
