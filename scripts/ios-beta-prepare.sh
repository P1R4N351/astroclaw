...
- regenerates apps/ios/Astroclaw.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${ASTROCLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid ASTROCLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid ASTROCLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid ASTROCLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid ASTROCLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing ASTROCLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(ASTROCLAW_URL_SLASH)$(ASTROCLAW_URL_SLASH)#g'
)"
...
ASTROCLAW_CODE_SIGN_STYLE = Automatic
ASTROCLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
ASTROCLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
ASTROCLAW_APP_BUNDLE_ID = ai.astroclaw.client
ASTROCLAW_SHARE_BUNDLE_ID = ai.astroclaw.client.share
ASTROCLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.astroclaw.client.activitywidget
ASTROCLAW_WATCH_APP_BUNDLE_ID = ai.astroclaw.client.watchkitapp
ASTROCLAW_WATCH_EXTENSION_BUNDLE_ID = ai.astroclaw.client.watchkitapp.extension
ASTROCLAW_APP_PROFILE =
ASTROCLAW_SHARE_PROFILE =
ASTROCLAW_PUSH_TRANSPORT = relay
ASTROCLAW_PUSH_DISTRIBUTION = official
ASTROCLAW_URL_SLASH = /
ASTROCLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
ASTROCLAW_PUSH_APNS_ENVIRONMENT = production
EOF
