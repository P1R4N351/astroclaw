...
Options:
  ...
  --repo <owner/repo>          GitHub repo. Default: astroclaw/astroclaw
...
function resolveBetaVersion(beta: string): string {
  const value = beta.trim().replace(/^astroclaw@/, "");
  if (/^\d{4}\.\d+\.\d+-beta\.\d+$/u.test(value)) {
    return value;
  }
  if (value === "beta") {
    return run("npm", ["view", "astroclaw@beta", "version"], { capture: true }).trim();
  }
  const betaMatch = /^(?:beta)?(\d+)$/u.exec(value);
  if (!betaMatch) {
    return run("npm", ["view", `astroclaw@${value}`, "version"], { capture: true }).trim();
  }
  const suffix = `-beta.${betaMatch[1]}`;
  const versions = JSON.parse(
    run("npm", ["view", "astroclaw", "versions", "--json"], { capture: true }),
  ) as string[];
  const match = versions
    ...
    .at(-1);
  if (!match) {
    throw new Error(`no astroclaw registry version found for ${beta}`);
  }
  return match;
}
...
async function main(): Promise<void> {
  ...
  const packageSpec = `astroclaw@${version}`;
