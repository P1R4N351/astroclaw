...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${ASTROCLAW_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$ASTROCLAW_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.ASTROCLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `ASTROCLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SMOKE_IMAGE="${ASTROCLAW_INSTALL_SMOKE_IMAGE:-astroclaw-install-smoke:local}"
NONROOT_IMAGE="${ASTROCLAW_INSTALL_NONROOT_IMAGE:-astroclaw-install-nonroot:local}"
SMOKE_PLATFORM="$(resolve_default_smoke_platform)"
NONROOT_PLATFORM="${ASTROCLAW_INSTALL_NONROOT_PLATFORM:-$SMOKE_PLATFORM}"
INSTALL_URL="${ASTROCLAW_INSTALL_URL:-https://astroclaw.bot/install.sh}"
CLI_INSTALL_URL="${ASTROCLAW_INSTALL_CLI_URL:-https://astroclaw.bot/install-cli.sh}"
PACKAGE_NAME="${ASTROCLAW_INSTALL_PACKAGE:-astroclaw}"
SKIP_NONROOT="${ASTROCLAW_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${ASTROCLAW_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${ASTROCLAW_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${ASTROCLAW_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${ASTROCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
UPDATE_BASELINE_VERSION="${ASTROCLAW_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"
UPDATE_PACKAGE_SPEC="${ASTROCLAW_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${ASTROCLAW_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${ASTROCLAW_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${ASTROCLAW_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${ASTROCLAW_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${ASTROCLAW_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
NPM_CACHE_DIR="${ASTROCLAW_INSTALL_SMOKE_NPM_CACHE_DIR:-}"
NPM_CACHE_OWNED=0
...
' "$pack_json_file"
  ...
  if [[ -z "$UPDATE_PACKAGE_SPEC" ]]; then
    node scripts/check-astroclaw-package-tarball.mjs "${UPDATE_DIR}/${UPDATE_TGZ_FILE}"
  fi
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (ASTROCLAW_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  prepare_npm_cache
  ...
  docker run --rm -t \
    ...
    -e ASTROCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e ASTROCLAW_INSTALL_METHOD=npm \
    -e ASTROCLAW_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e ASTROCLAW_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e ASTROCLAW_INSTALL_LATEST_OUT="/out/latest" \
    -e ASTROCLAW_NO_ONBOARD=1 \
    -e ASTROCLAW_NO_PROMPT=1 \
  ...
  docker run --rm -t \
    ...
    "${NPM_CACHE_DOCKER_ARGS[@]}" \
    -e ASTROCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e ASTROCLAW_INSTALL_SMOKE_MODE=update \
    -e ASTROCLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e ASTROCLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e ASTROCLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e ASTROCLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e ASTROCLAW_NO_ONBOARD=1 \
    -e ASTROCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (ASTROCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    docker run --rm -t \
      ...
      "${NPM_CACHE_DOCKER_ARGS[@]}" \
      -e ASTROCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e ASTROCLAW_INSTALL_SMOKE_MODE=npm-global \
      -e ASTROCLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e ASTROCLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e ASTROCLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e ASTROCLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (ASTROCLAW_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  docker run --rm -t \
    ...
    -e ASTROCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e ASTROCLAW_INSTALL_METHOD=npm \
    -e ASTROCLAW_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e ASTROCLAW_NO_ONBOARD=1 \
    -e ASTROCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${ASTROCLAW_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (ASTROCLAW_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
docker run --rm -t \
  ...
  --entrypoint /bin/bash \
  -e ASTROCLAW_INSTALL_URL="$INSTALL_URL" \
  -e ASTROCLAW_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e ASTROCLAW_NO_ONBOARD=1 \
  -e ASTROCLAW_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
