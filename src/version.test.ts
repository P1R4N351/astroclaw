...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "astroclaw-version-" });
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "astroclaw", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "astroclaw", version: "2.3.4" });
  ...
  it("ignores non-astroclaw package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers ASTROCLAW_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        ASTROCLAW_VERSION: "9.9.9",
        ASTROCLAW_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over stale ASTROCLAW_VERSION for compatibility checks", () => {
    const previous = process.env.ASTROCLAW_VERSION;
    const previousService = process.env.ASTROCLAW_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.ASTROCLAW_VERSION = "2026.3.25";
      process.env.ASTROCLAW_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      restoreEnvValue("ASTROCLAW_VERSION", previous);
      restoreEnvValue("ASTROCLAW_SERVICE_VERSION", previousService);
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        ASTROCLAW_VERSION: "2026.3.99",
        ASTROCLAW_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        ASTROCLAW_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        ASTROCLAW_VERSION: "2026.3.99",
        ASTROCLAW_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
  ...
  it("prefers runtime VERSION over service/package markers and ignores unusable env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        ASTROCLAW_VERSION: "   ",
        ASTROCLAW_SERVICE_VERSION: "  2.0.0  ",
    ...
    expect(
      resolveRuntimeServiceVersion({
        ...
        ASTROCLAW_SERVICE_VERSION: "null",
        npm_package_version: "1.0.0-package",
      }),
    ).toBe(VERSION);
  });
