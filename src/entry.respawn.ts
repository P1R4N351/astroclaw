...
export const ASTROCLAW_NODE_OPTIONS_READY = "ASTROCLAW_NODE_OPTIONS_READY";
export const ASTROCLAW_NODE_EXTRA_CA_CERTS_READY = "ASTROCLAW_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (
    ...
    isTruthyEnvValue(env.ASTROCLAW_NO_RESPAWN)
  ) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[ASTROCLAW_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[ASTROCLAW_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !shouldSkipRespawnForArgv(argv) &&
    !isTruthyEnvValue(env[ASTROCLAW_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[ASTROCLAW_NODE_OPTIONS_READY] = "1";
...
export function runCliRespawnPlan(
  ...
  child.once("error", (error) => {
    clearSignalTimers();
    runtime.writeError(
      "[astroclaw] Failed to respawn CLI:",
