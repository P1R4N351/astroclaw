...
import { BUNDLED_PLUGIN_ROOT_DIR } from "astroclaw/plugin-sdk/test-fixtures";
...
describe("Dockerfile", () => {
  ...
  it("uses full bookworm for build stages and slim bookworm for runtime", async () => {
    ...
    expect(dockerfile).toContain("FROM ${ASTROCLAW_NODE_BOOKWORM_IMAGE} AS workspace-deps");
    expect(dockerfile).toContain("FROM ${ASTROCLAW_NODE_BOOKWORM_IMAGE} AS build");
    expect(dockerfile).toContain("FROM ${ASTROCLAW_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime");
    ...
    expect(dockerfile).not.toContain("ASTROCLAW_VARIANT");
  });
  ...
  it("installs optional browser dependencies after pnpm install", async () => {
    ...
    const browserArgIndex = dockerfile.indexOf("ARG ASTROCLAW_INSTALL_BROWSER");
  ...
  it("prunes runtime dependencies after the build stage", async () => {
    ...
    expect(dockerfile).toContain("ARG ASTROCLAW_EXTENSIONS");
    expect(dockerfile).toContain("ARG ASTROCLAW_BUNDLED_PLUGIN_DIR");
    expect(dockerfile).toContain(
  ...
  it("does not override bundled plugin discovery in runtime images", async () => {
    const dockerfile = collapseDockerContinuations(await readFile(dockerfilePath, "utf8"));
    expect(dockerfile).toContain(`ARG ASTROCLAW_BUNDLED_PLUGIN_DIR=${BUNDLED_PLUGIN_ROOT_DIR}`);
    expect(dockerfile).not.toMatch(/^\s*ENV\b[^\n]*\bASTROCLAW_BUNDLED_PLUGINS_DIR\b/m);
  });
  ...
  it("Docker GPG fingerprint awk uses correct quoting for ASTROCLAW_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
  ...
  it("pre-creates the Astroclaw home before switching to the node user", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
    ...
    expect(dockerfile).not.toContain("mkdir -p /home/node/.astroclaw");
    expect(dockerfile).toContain(
