...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ASTROCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("ASTROCLAW_HOME", "/srv/astroclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/astroclaw-home", "cron", "jobs.json"));
  });
});
