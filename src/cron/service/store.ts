...
function warnInvalidPersistedCronJob(params: {
  ...
  params.state.deps.log.warn(
    ...
    "cron: skipped invalid persisted job; run astroclaw doctor --fix to repair",
  );
}
  ...
  for (const [index, job] of loadedJobs.entries()) {
    ...
    try {
      ...
      state.deps.log.warn(
        ...
        "cron: job has invalid persisted sessionTarget; run astroclaw doctor --fix to repair",
      );
    }
    const hydrated =
    ...
    if (legacyJobIdIssue) {
      ...
      state.deps.log.warn(
        ...
        "cron: job used legacy jobId field; normalized id in memory (run astroclaw doctor --fix to persist canonical shape)",
      );
    }
