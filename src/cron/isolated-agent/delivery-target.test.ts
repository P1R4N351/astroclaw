      ...
      },
      {
        ...
        plugin: {
          ...
          config: {
            listAccountIds: () => [],
            resolveAccount: () => ({}),
            resolveAllowFrom: ({ cfg }: { cfg: AstroclawConfig }) =>
...
function makeCfg(overrides?: Partial<AstroclawConfig>): AstroclawConfig {
  return {
    bindings: [],
    channels: {},
    ...overrides,
...
function makeForumBoundCfg(accountId = "account-b"): AstroclawConfig {
  return makeCfg({
    bindings: [
      {
        agentId: AGENT_ID,
...
async function resolveLastTarget(cfg: AstroclawConfig) {
