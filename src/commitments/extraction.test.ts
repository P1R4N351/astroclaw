...
describe("commitment extraction", () => {
  ...
  async function createConfig(): Promise<AstroclawConfig> {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-commitments-"));
    tmpDirs.push(tmpDir);
  ...
  it("rejects disabled, low-confidence, and non-future candidates", () => {
    const cfg: AstroclawConfig = { commitments: { enabled: true } };
