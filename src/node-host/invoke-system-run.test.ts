...
describe("handleSystemRunInvoke mac app exec host routing", () => {
  ...
  let sharedAstroclawHome = "";
  let sharedRuntimeBinDir = "";
  let sharedFixtureId = 0;
  let previousAstroclawHome: string | undefined;
  const sharedRuntimeBins = new Set<string>();

  beforeAll(() => {
    sharedFixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-node-host-fixtures-"));
    sharedAstroclawHome = path.join(sharedFixtureRoot, "astroclaw-home");
    sharedRuntimeBinDir = path.join(sharedFixtureRoot, "bin");
    fs.mkdirSync(sharedAstroclawHome, { recursive: true });
  ...
  beforeEach(() => {
    ...
    process.env.ASTROCLAW_HOME = sharedAstroclawHome;
  ...
  async function withTempApprovalsHome<T>(params: {
    ...
    const tempHome = sharedAstroclawHome;
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      for (const testCase of approvedEnvShellWrapperCases) {
        const tmp = createFixtureDir("astroclaw-approved-wrapper-");
  ...
  it("denies shell payload carriers in allowlist mode without explicit approval", async () => {
    ...
    > = [
      ...
      },
      {
        ...
        command:
          process.platform === "win32"
            ? ["cmd.exe", "/d", "/s", "/c", "astroclaw status; id"]
            : ["/bin/sh", "-lc", "astroclaw status; id"],
        approvalRequired: true,
      },
      {
        ...
        command:
          process.platform === "win32"
            ? ["cmd.exe", "/d", "/s", "/c", "astroclaw status; cat /etc/passwd"]
            : ["/bin/sh", "-lc", "astroclaw status; cat /etc/passwd"],
        approvalRequired: true,
      },
      {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const { runCommand, sendInvokeResult } = await runSystemInvoke({
        ...
        security: "allowlist",
        ask: "off",
        command: ["/bin/sh", "-lc", "head -c${IFS}16${IFS}${ASTROCLAW_CONFIG_PATH}"],
        rawCommand: "head -c${IFS}16${IFS}${ASTROCLAW_CONFIG_PATH}",
      });
  ...
  it("denies abbreviated PowerShell encoded payloads even when the wrapper is allowlisted", async () => {
    const binDir = createFixtureDir("astroclaw-pwsh-allowlist-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withPathTokenCommand({
        tmpPrefix: "astroclaw-approval-path-pin-",
        run: async ({ expected }) => {
          const { runCommand, sendInvokeResult } = await runSystemInvoke({
            preferMacAppExecHost: false,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const sendInvokeResult = vi.fn(async () => {});
      await withPathTokenCommand({
        tmpPrefix: "astroclaw-allowlist-path-pin-",
      ...
      for (const testCase of [
        {
          label: "cwd symlink",
          setup: () => {
            const tmp = createFixtureDir("astroclaw-approval-cwd-link-");
        ...
        },
        {
          ...
          setup: () => {
            const tmp = createFixtureDir("astroclaw-approval-cwd-parent-link-");
  ...
  it("uses canonical executable path for approval-based relative command execution", async () => {
    const tmp = createFixtureDir("astroclaw-approval-cwd-real-");
  ...
  it("denies approval-based execution when cwd identity drifts before execution", async () => {
    const tmp = createFixtureDir("astroclaw-approval-cwd-drift-");
    const fallback = createFixtureDir("astroclaw-approval-cwd-drift-alt-");
    const script = path.join(tmp, "run.sh");
    fs.writeFileSync(script, "#!/bin/sh\necho SAFE\n");
    fs.chmodSync(script, 0o755);
    const canonicalCwd = fs.realpathSync(tmp);
  ...
  it("validates approved script operand bindings at dispatch", async () => {
    for (const mutate of [true, false]) {
      const tmp = createFixtureDir(
        mutate ? "astroclaw-approval-script-drift-" : "astroclaw-approval-script-stable-",
      );
  ...
  it("validates approved runtime script operand bindings at dispatch", async () => {
    await withFakeRuntimeOnPath({
      runtime: "tsx",
      run: async () => {
        const tmp = createFixtureDir("astroclaw-approval-tsx-script-drift-");
        ...
        const missingBindingTmp = createFixtureDir("astroclaw-approval-tsx-missing-binding-");
  ...
  it("denies ./sh wrapper spoof in allowlist on-miss mode before execution", async () => {
    const marker = path.join(os.tmpdir(), `astroclaw-wrapper-spoof-${process.pid}-${Date.now()}`);
    const runCommand = vi.fn(async () => {
  ...
  it("does not persist allow-always approvals for strict inline-eval carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("astroclaw-inline-eval-bin-");
  ...
  it("persists benign awk allow-always approvals in strict inline-eval mode without reopening inline carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("astroclaw-inline-eval-awk-");
          const executablePath = createTempExecutable({
            dir: tempDir,
            name: "awk",
          });
  ...
  it("does not persist allow-always approvals for strict inline-eval make carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("astroclaw-inline-eval-make-");
          const executablePath = createTempExecutable({
            dir: tempDir,
            name: "make",
          });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = createFixtureDir("astroclaw-shell-wrapper-inner-");
  ...
  it("keeps cmd.exe transport wrappers approval-gated on Windows", async () => {
    ...
    try {
      for (const testCase of [
        ...
        const tempDir = createFixtureDir("astroclaw-cmd-wrapper-allow-");
  ...
  it("reuses exact-command durable trust for shell-wrapper reruns", async () => {
    ...
    const tempDir = createFixtureDir("astroclaw-shell-wrapper-allow-");
    const prepared = buildSystemRunApprovalPlan({
      command: ["/bin/sh", "-c", "cd ."],
      cwd: tempDir,
    });
