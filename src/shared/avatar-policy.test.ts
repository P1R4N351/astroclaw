...
describe("avatar policy", () => {
  it("classifies avatar URI and path helpers directly", () => {
    ...
    expect(isWindowsAbsolutePath("C:\\\\avatars\\\\astroclaw.png")).toBe(true);
  });

  it("accepts workspace-relative avatar paths and rejects URI schemes", () => {
    expect(isWorkspaceRelativeAvatarPath("avatars/astroclaw.png")).toBe(true);
    expect(isWorkspaceRelativeAvatarPath("C:\\\\avatars\\\\astroclaw.png")).toBe(true);
  ...
  it("detects avatar-like path strings", () => {
    expect(looksLikeAvatarPath("avatars/astroclaw.svg")).toBe(true);
    expect(looksLikeAvatarPath("astroclaw.webp")).toBe(true);
