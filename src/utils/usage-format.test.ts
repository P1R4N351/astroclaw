...
describe("usage-format", () => {
  const originalAgentDir = process.env.ASTROCLAW_AGENT_DIR;
  const originalStateDir = process.env.ASTROCLAW_STATE_DIR;
  let agentDir: string;
  let stateDir: string;

  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-usage-format-"));
  ...
  afterEach(async () => {
    if (originalAgentDir === undefined) {
      ...
      process.env.ASTROCLAW_AGENT_DIR = originalAgentDir;
    }
  ...
  it("prefers models.json pricing over astroclaw config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          "demo-preferred": {
  ...
  it("falls back to astroclaw config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
