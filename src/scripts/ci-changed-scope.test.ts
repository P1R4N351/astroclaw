...
describe("detectChangedScope", () => {
  ...
  it("keeps node lane off for native-only changes", () => {
    ...
    expect(
      detectChangedScope(["apps/macos-mlx-tts/Sources/AstroclawMLXTTSHelper/main.swift"]),
    ).toEqual({
      runNode: false,
      runMacos: true,
      runAndroid: false,
    ...
    expect(detectChangedScope(["apps/shared/AstroclawKit/Sources/Foo.swift"])).toEqual({
      runNode: false,
      runMacos: true,
      runAndroid: true,
      runWindows: false,
  ...
  it("does not force macOS for generated protocol model-only changes", () => {
    expect(
      detectChangedScope(["apps/shared/AstroclawKit/Sources/AstroclawProtocol/GatewayModels.swift"]),
    ).toEqual({
      runNode: false,
      runMacos: false,
      runAndroid: false,
  ...
  it("treats base and head as literal git args", () => {
    const markerPath = path.join(
      os.tmpdir(),
      `astroclaw-ci-changed-scope-${Date.now()}-${Math.random().toString(16).slice(2)}.tmp`,
    );
  ...
  it("keeps direct CLI preflight empty diffs as no-op scope", () => {
    const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-ci-scope-empty-"));
