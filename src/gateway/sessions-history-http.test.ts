...
const READ_SCOPE_HEADER = { "x-astroclaw-scopes": "operator.read" };
...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-session-history-"));
...
function expectAstroclawMetadata(
...
async function expectMessageEventMatch(
  ...
  if (params.id !== undefined) {
    expectAstroclawMetadata(
      (event.data as { message?: { __astroclaw?: { id?: string; seq?: number } } }).message
        ?.__astroclaw,
      {
        id: params.id,
        seq: params.seq,
...
describe("session history HTTP endpoints", () => {
  test("returns session history over direct REST", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expectAstroclawMetadata(
        (
          body.messages?.[0] as {
            __astroclaw?: { id?: string; seq?: number };
          }
        )?.__astroclaw,
        {
          seq: 1,
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      const firstBody = (await firstPage.json()) as {
        sessionKey?: string;
        items?: Array<{ content?: Array<{ text?: string }>; __astroclaw?: { seq?: number } }>;
        messages?: Array<{ content?: Array<{ text?: string }>; __astroclaw?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(firstBody.messages?.map((message) => message.__astroclaw?.seq)).toEqual([2, 3]);
      ...
      const secondBody = (await secondPage.json()) as {
        items?: Array<{ content?: Array<{ text?: string }>; __astroclaw?: { seq?: number } }>;
        messages?: Array<{ __astroclaw?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(secondBody.messages?.map((message) => message.__astroclaw?.seq)).toEqual([1]);
  ...
  test("streams bounded history windows over SSE", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expectAstroclawMetadata(nextData.messages?.[0]?.__astroclaw, {
        id: thirdMessageId,
        seq: 3,
      });
  ...
  test("seeds bounded SSE windows from visible history when transcript refreshes are silent", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(refreshData.messages?.[0]?.__astroclaw?.seq).toBe(2);
  ...
  test("sanitizes phased assistant history entries before returning them", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expectAstroclawMetadata(body.messages?.[0]?.__astroclaw, {
        id: visibleMessageId,
        seq: 2,
      });
    });
