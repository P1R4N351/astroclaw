...
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("astroclaw.gatewayTestHelpers.configRoot");
...
const gatewayTestHoisted = vi.hoisted(() => {
  const key = Symbol.for("astroclaw.gatewayTestHelpers.hoisted");
...
export const testConfigRoot = resolveGlobalSingleton(GATEWAY_TEST_CONFIG_ROOT_KEY, () => ({
  value: path.join(os.tmpdir(), `astroclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
}));

export function setTestConfigRoot(root: string): void {
  testConfigRoot.value = root;
  process.env.ASTROCLAW_CONFIG_PATH = path.join(root, "astroclaw.json");
}
