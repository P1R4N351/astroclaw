...
import type { loadAstroclawPlugins } from "../plugins/loader.js";
...
const SKIP_STARTUP_MODEL_PREWARM_ENV = "ASTROCLAW_SKIP_STARTUP_MODEL_PREWARM";
...
function resolveGatewayMemoryStartupPolicy(cfg: AstroclawConfig): GatewayMemoryStartupPolicy {
...
async function resolveRestartSentinelPathFast(
  ...
  const resolveHome = () => {
    const explicitHome = normalizePathEnv(env.ASTROCLAW_HOME);
    if (explicitHome) {
  ...
  const override = normalizePathEnv(env.ASTROCLAW_STATE_DIR);
  if (override) {
    return path.join(resolveUserPath(override), RESTART_SENTINEL_FILENAME);
  }
  const home = resolveHome();
  const newStateDir = path.join(home, ".astroclaw");
  if (env.ASTROCLAW_TEST_FAST === "1" || (await pathExists(newStateDir))) {
...
function hasGatewayStartHooks(pluginRegistry: ReturnType<typeof loadAstroclawPlugins>): boolean {
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  const { ensureAstroclawModelsJson } = await import("../agents/models-config.js");
  const agentDir = resolveDefaultAgentDir(params.cfg);
  const workspaceDir =
    params.workspaceDir ?? resolveAgentWorkspaceDir(params.cfg, resolveDefaultAgentId(params.cfg));
  try {
    await ensureAstroclawModelsJson(params.cfg, agentDir, {
      workspaceDir,
...
export async function startGatewaySidecars(params: {
  cfg: AstroclawConfig;
  pluginRegistry: ReturnType<typeof loadAstroclawPlugins>;
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: AstroclawConfig;
    pluginRegistry: ReturnType<typeof loadAstroclawPlugins>;
    defaultWorkspaceDir: string;
    deps: CliDeps;
    startChannels: () => Promise<void>;
