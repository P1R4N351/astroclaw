...
const hoisted = vi.hoisted(() => {
  ...
  const resolveAstroclawPackageRootSync = vi.fn((_params: unknown) => "/package");
...
vi.mock("../infra/astroclaw-root.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../infra/astroclaw-root.js")>();
  return {
    ...actual,
    resolveAstroclawPackageRootSync: hoisted.resolveAstroclawPackageRootSync,
  };
});
...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    prevSkipChannels = process.env.ASTROCLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.ASTROCLAW_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.ASTROCLAW_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.ASTROCLAW_SKIP_CHANNELS = "0";
    ...
    hoisted.resolveAstroclawPackageRootSync.mockClear();
    hoisted.resolveAstroclawPackageRootSync.mockReturnValue("/package");
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.ASTROCLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.ASTROCLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ASTROCLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.ASTROCLAW_SKIP_PROVIDERS;
    } else {
      process.env.ASTROCLAW_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    ...
    if (!configPath) {
      throw new Error("ASTROCLAW_CONFIG_PATH is not set");
    }
  ...
  async function withNonMinimalGatewayServer(
    ...
    return await withEnvAsync({ ASTROCLAW_TEST_MINIMAL_GATEWAY: undefined }, async () =>
...
`,
    ...
    const previousGatewayTokenEnv = process.env.ASTROCLAW_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.ASTROCLAW_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
