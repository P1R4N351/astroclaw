...
type GatewayReloadHandlerParams = {
  ...
  createHealthMonitor: (config: AstroclawConfig) => ChannelHealthMonitor | null;
...
> & {
  ...
  initialConfig: AstroclawConfig;
  initialCompareConfig?: AstroclawConfig;
  ...
  resolveSharedGatewaySessionGenerationForConfig: (config: AstroclawConfig) => string | undefined;
...
export function createGatewayReloadHandlers(params: GatewayReloadHandlerParams) {
  ...
  const markActiveMainSessionsForRestart = async (nextConfig: AstroclawConfig, reason: string) => {
  ...
  const applyHotReload = async (plan: GatewayReloadPlan, nextConfig: AstroclawConfig) => {
            ...
            await startGmailWatcherWithLogs({
              ...
              onSkipped: () =>
                params.logHooks.info(
                  "skipping gmail watcher restart (ASTROCLAW_SKIP_GMAIL_WATCHER=1)",
                ),
            });
          }
        }
    ...
    if (channelsToRestart.size > 0) {
      if (shouldSkipChannelRestart()) {
        params.logChannels.info(
          "skipping channel reload (ASTROCLAW_SKIP_CHANNELS=1 or ASTROCLAW_SKIP_PROVIDERS=1)",
        );
      } else {
        if (!plan.reloadPlugins) {
          await waitForActiveWorkBeforeChannelReload(channelsToRestart, nextConfig);
  ...
  const requestGatewayRestart = (plan: GatewayReloadPlan, nextConfig: AstroclawConfig): boolean => {
    setGatewaySigusr1RestartPolicy({ allowExternal: isRestartEnabled(nextConfig) });
