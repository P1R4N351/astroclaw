...
function createGatewayStartupTrace() {
  const logEnabled = isTruthyEnvValue(process.env.ASTROCLAW_GATEWAY_STARTUP_TRACE);
  let timelineConfig: AstroclawConfig | undefined;
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.ASTROCLAW_DIAGNOSTICS_EVENT_LOOP);
  ...
  return {
    setConfig(config: AstroclawConfig) {
...
function formatRuntimeGatewayAuthTokenWarning(): string {
  ...
  if (!isNixMode) {
    return `${base} Persist one with \`astroclaw config set gateway.auth.mode token\` and \`astroclaw config set gateway.auth.token <token>\`.`;
  }
  return [
    base,
    "In Nix mode, set gateway.auth.token in your Nix-managed Astroclaw config and rebuild.",
    "For the first-party Nix flow, see https://github.com/astroclaw/nix-astroclaw#quick-start and https://docs.astroclaw.ai/install/nix.",
  ].join(" ");
}
...
export type GatewayServerOptions = {
   ...
   * reparsing astroclaw.json during server startup.
   */
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.ASTROCLAW_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.ASTROCLAW_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "ASTROCLAW_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "ASTROCLAW_RAW_STREAM_PATH",
  ...
  let cfgAtStart: AstroclawConfig;
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: AstroclawConfig) =>
  ...
  const getReadiness = createReadinessChecker({
    ...
    shouldSkipChannelReadiness: () =>
      isTruthyEnvValue(process.env.ASTROCLAW_SKIP_CHANNELS) ||
      isTruthyEnvValue(process.env.ASTROCLAW_SKIP_PROVIDERS),
  });
  ...
  try {
    ...
    const gatewayRequestContext = createGatewayRequestContext({
      ...
      clients,
      enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: AstroclawConfig) => {
        enforceSharedGatewaySessionGenerationForConfigWrite({
