export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current Astroclaw session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsAstroclawStatus =
    normalized.includes("astroclaw is running on") ||
    /astroclaw\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("astroclaw status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsAstroclawStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from astroclaw status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live astroclaw config shows") ||
