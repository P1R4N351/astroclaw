...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.ASTROCLAW_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.ASTROCLAW_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
function resolveScopedSessionKey(cfg: AstroclawConfig, rawSessionKey: string | undefined): string {
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-astroclaw-message-channel")) ?? undefined,
    accountId: normalizeOptionalString(getHeader(req, "x-astroclaw-account-id")),
    inboundEventKind: normalizeMcpInboundEventKind(getHeader(req, "x-astroclaw-inbound-event-kind")),
