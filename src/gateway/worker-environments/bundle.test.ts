  ...
  it("installs a source bundle when the AI workspace import is bundled", async () => {
    await withTestDir({ prefix: "openclaw-worker-bundle-npm-install-" }, async (root) => {
      ...
      const dependencyFields = (["dependencies", "devDependencies"] as const).filter(
        (field) => aiManifest[field]?.["@astroclaw/normalization-core"] !== undefined,
      );
      ...
      const normalizationCoreSpec = aiManifest[dependencyField]?.["@astroclaw/normalization-core"];
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify({
          ...
          type: "module",
          main: "./dist/index.js",
          [dependencyField]: { "@astroclaw/normalization-core": normalizationCoreSpec },
        })}\n`,
        "utf8",
      );
