...
describe("gateway server models + voicewake", () => {
  const listModels = async (params?: { view?: "default" | "configured" | "all" }) =>
    withEnvAsync({ ASTROCLAW_DISABLE_BUNDLED_PLUGINS: "1" }, async () =>
      params
  ...
  const withModelsConfig = async <T>(config: unknown, run: () => Promise<T>): Promise<T> => {
    ...
    if (!configPath) {
      throw new Error("Missing ASTROCLAW_CONFIG_PATH");
    }
  ...
  const withTempHome = async <T>(fn: (homeDir: string) => Promise<T>): Promise<T> => {
    const tempHome = await createTempHomeEnv("astroclaw-home-");
    try {
  ...
  test(
    ...
    async () => {
      await withTempHome(async (homeDir) => {
        ...
        expect(initial.payload?.triggers).toEqual(["astroclaw", "claude", "computer"]);
        ...
        const onDisk = JSON.parse(
          await fs.readFile(path.join(homeDir, ".astroclaw", "settings", "voicewake.json"), "utf8"),
  ...
  test("voicewake.routing.get/set persists and broadcasts", { timeout: 60_000 }, async () => {
    await withTempHome(async (homeDir) => {
      ...
      const onDisk = JSON.parse(
        await fs.readFile(
          path.join(homeDir, ".astroclaw", "settings", "voicewake-routing.json"),
          "utf8",
        ),
      ) as { routes?: unknown };
