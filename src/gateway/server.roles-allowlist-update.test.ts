...
const connectNodeClient = async (params: {
  ...
  const token = process.env.ASTROCLAW_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("ASTROCLAW_GATEWAY_TOKEN is required for node test clients");
  }
...
function getGatewayTestConfigPath(): string {
  ...
  if (!configPath) {
    throw new Error("ASTROCLAW_CONFIG_PATH is required in the gateway test environment");
  }
  return configPath;
}
...
describe("gateway node command allowlist", () => {
  test("enforces command allowlists across node clients", async () => {
    ...
    try {
      const systemDeviceIdentity = loadOrCreateDeviceIdentity(
        path.join(os.tmpdir(), `astroclaw-node-system-run-${Date.now()}-${Math.random()}.json`),
      );
      const emptyDeviceIdentity = loadOrCreateDeviceIdentity(
        path.join(os.tmpdir(), `astroclaw-node-empty-${Date.now()}-${Math.random()}.json`),
      );
      const allowedDeviceIdentity = loadOrCreateDeviceIdentity(
        path.join(os.tmpdir(), `astroclaw-node-allowed-${Date.now()}-${Math.random()}.json`),
      );
  ...
  test("rechecks current allowlist before exposing approved live commands", async () => {
    ...
    try {
      const deviceIdentity = loadOrCreateDeviceIdentity(
        path.join(
          os.tmpdir(),
          `astroclaw-node-current-allowlist-${Date.now()}-${Math.random()}.json`,
        ),
      );
      nodeClient = await connectNodeClientWithPairing({
        port,
  ...
  test("records only allowlisted commands in pending node pairing requests", async () => {
    const deviceIdentityPath = path.join(
      os.tmpdir(),
      `astroclaw-allowlisted-pending-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
  ...
  test("rejects reconnect metadata spoof for paired node devices", async () => {
    const deviceIdentityPath = path.join(
      os.tmpdir(),
      `astroclaw-spoof-test-device-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
    const deviceIdentity = loadOrCreateDeviceIdentity(deviceIdentityPath);
  ...
  test("does not promote paired desktop client id changes into host command defaults", async () => {
    const deviceIdentityPath = path.join(
      os.tmpdir(),
      `astroclaw-client-id-promotion-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
    const deviceIdentity = loadOrCreateDeviceIdentity(deviceIdentityPath);
  ...
  test("allows canonical node-host reconnect for legacy pinned platform metadata", async () => {
    const deviceIdentityPath = path.join(
      os.tmpdir(),
      `astroclaw-node-host-platform-upgrade-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
    const deviceIdentity = loadOrCreateDeviceIdentity(deviceIdentityPath);
  ...
  test("filters system.run for confusable iOS metadata at connect time", async () => {
    const deviceIdentityPath = path.join(
      os.tmpdir(),
      `astroclaw-confusable-node-greek-omicron-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
    const deviceIdentity = loadOrCreateDeviceIdentity(deviceIdentityPath);
