...
const GATEWAY_ENV_TOKEN = "ASTROCLAW_GATEWAY_TOKEN";
...
export function resolveGatewayAuthTokenSourceConflict(params: {
  ...
  const envToken = normalizeOptionalString(params.env.ASTROCLAW_GATEWAY_TOKEN);
  if (!envToken) {
    return null;
  }

  if (params.env.ASTROCLAW_SERVICE_KIND?.trim() === GATEWAY_SERVICE_KIND) {
    return null;
  }
  ...
  const remediation =
    `Remove ${GATEWAY_ENV_TOKEN} from the shell, ~/.astroclaw/.env, or launchctl env if gateway.auth.token is intended, ` +
