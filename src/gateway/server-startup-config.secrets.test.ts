...
function gatewayTokenConfig(config: AstroclawConfig): AstroclawConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function buildSnapshot(config: AstroclawConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/astroclaw-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: AstroclawConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.ASTROCLAW_SKIP_CHANNELS;
  const previousSkipProviders = process.env.ASTROCLAW_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.ASTROCLAW_SKIP_CHANNELS;
    } else {
      process.env.ASTROCLAW_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.ASTROCLAW_SKIP_PROVIDERS;
    } else {
      process.env.ASTROCLAW_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
