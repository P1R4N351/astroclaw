...
function loadGatewayConfig(): AstroclawConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: AstroclawConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallTimeout(
  ...
  const hasEnvHandshakeTimeout =
    Boolean(process.env.ASTROCLAW_HANDSHAKE_TIMEOUT_MS) ||
    Boolean(process.env.VITEST && process.env.ASTROCLAW_TEST_HANDSHAKE_TIMEOUT_MS);
...
function resolveGatewayCallContext(opts: CallGatewayBaseOptions): ResolvedGatewayCallContext {
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.ASTROCLAW_GATEWAY_URL);
  ...
  const config = opts.config ?? (canSkipConfigLoad ? ({} as AstroclawConfig) : loadGatewayConfig());
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `astroclaw doctor` for diagnostics.";
  }
  return message;
}
