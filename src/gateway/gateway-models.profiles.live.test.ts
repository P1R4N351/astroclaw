...
import { ensureAstroclawModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { STREAM_ERROR_FALLBACK_TEXT } from "../agents/stream-message-shared.js";
import { clearRuntimeConfigSnapshot, getRuntimeConfig } from "../config/io.js";
import type { ModelsConfig, ModelProviderConfig, AstroclawConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ASTROCLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.ASTROCLAW_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.ASTROCLAW_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.ASTROCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_SETUP_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.ASTROCLAW_LIVE_GATEWAY_SETUP_TIMEOUT_MS, 60_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.ASTROCLAW_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.ASTROCLAW_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["ASTROCLAW_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.ASTROCLAW_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.ASTROCLAW_LIVE_GATEWAY_MODELS?.trim();
  const useExplicitModels = Boolean(rawModels) && rawModels !== "modern" && rawModels !== "all";
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.ASTROCLAW_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.ASTROCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.ASTROCLAW_LIVE_MODEL_TIMEOUT_MS,
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.ASTROCLAW_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.ASTROCLAW_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.ASTROCLAW_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("ASTROCLAW_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("ASTROCLAW_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("ASTROCLAW_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.ASTROCLAW_LIVE_GATEWAY_MODELS;
    delete process.env.ASTROCLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.ASTROCLAW_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.ASTROCLAW_LIVE_GATEWAY_MODELS = "openai/gpt-5.5,anthropic/claude-opus-4-6";
    delete process.env.ASTROCLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.ASTROCLAW_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.ASTROCLAW_LIVE_GATEWAY_MAX_MODELS = "2";
...
async function sanitizeAuthConfig(params: {
  cfg: AstroclawConfig;
  agentDir: string;
}): Promise<AstroclawConfig["auth"] | undefined> {
  ...
  let profiles: NonNullable<AstroclawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    ...
    disableBonjour: process.env.ASTROCLAW_DISABLE_BONJOUR,
    logLevel: process.env.ASTROCLAW_LOG_LEVEL,
    agentDir: process.env.ASTROCLAW_AGENT_DIR,
  ...
  if (QUIET_LIVE_LOGS) {
    process.env.ASTROCLAW_DISABLE_BONJOUR = "1";
    process.env.ASTROCLAW_LOG_LEVEL = "silent";
  }
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-live-state-"));
  process.env.ASTROCLAW_STATE_DIR = tempStateDir;
  ...
  process.env.ASTROCLAW_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.astroclaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveDefaultAgentDir(params.cfg);
  const sanitizedCfg: AstroclawConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-live-"));
  const tempConfigPath = path.join(tempDir, "astroclaw.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.ASTROCLAW_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            (async () => {
              ...
              for (
                ...
                toolText = await requestGatewayAgentText({
                  ...
                  modelKey,
                  message: strictReply
                    ? "Astroclaw live tool probe (local, safe): " +
    ...
    process.env.ASTROCLAW_CONFIG_PATH = previous.configPath;
    process.env.ASTROCLAW_GATEWAY_TOKEN = previous.token;
    process.env.ASTROCLAW_SKIP_CHANNELS = previous.skipChannels;
    process.env.ASTROCLAW_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.ASTROCLAW_SKIP_CRON = previous.skipCron;
    process.env.ASTROCLAW_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.ASTROCLAW_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.ASTROCLAW_LOG_LEVEL = previous.logLevel;
    process.env.ASTROCLAW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ASTROCLAW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        await withGatewayLiveSetupTimeout(
          ensureAstroclawModelsJson(cfg, undefined, {
            workspaceDir,
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via ASTROCLAW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    await ensureAstroclawModelsJson(cfg);
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.astroclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
