...
export function resolveHooksConfig(cfg: AstroclawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: AstroclawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-astroclaw-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-astroclaw-idempotency-key"]) ||
