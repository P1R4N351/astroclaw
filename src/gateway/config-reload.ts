...
import type { ConfigFileSnapshot, AstroclawConfig } from "../config/types.astroclaw.js";
...
function asPluginInstallConfig(records: PluginInstallRecords): AstroclawConfig {
  return {
    plugins: {
      installs: records,
    },
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: AstroclawConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: AstroclawConfig) => void | Promise<void>;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: AstroclawConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (
    ...
    nextCompareConfig: AstroclawConfig,
    afterWrite?: ConfigWriteNotification["afterWrite"],
  ) => {
