...
test("sessions.compact without maxLines runs embedded manual compaction for checkpoint-capable flows", async () => {
  ...
  expect(compactionCall.workspaceDir).toBe(path.join(os.tmpdir(), "astroclaw-gateway-test"));
  ...
  expect(callConfig.agents?.defaults?.workspace).toBe(
    path.join(os.tmpdir(), "astroclaw-gateway-test"),
  );
...
test("sessions.patch preserves nested model ids under provider overrides", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-gw-sessions-nested-"));
  ...
  await withEnvAsync({ ASTROCLAW_CONFIG_PATH: undefined }, async () => {
    ...
    await withEnvAsync({ ASTROCLAW_CONFIG_PATH: configPath }, async () => {
