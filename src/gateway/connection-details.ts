...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: AstroclawConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride =
    cliUrlOverride || options.ignoreEnvUrlOverride
      ? undefined
      : normalizeOptionalString(process.env.ASTROCLAW_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.ASTROCLAW_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set ASTROCLAW_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: astroclaw doctor --fix",
