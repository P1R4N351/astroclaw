...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.ASTROCLAW_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: AstroclawConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: AstroclawConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: AstroclawConfig, requestedAgentId: string) => {
