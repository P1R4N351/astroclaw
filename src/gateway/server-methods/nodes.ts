...
function resolveRelayNodePushConfig(cfg: AstroclawConfig) {
...
export async function maybeWakeNodeWithApns(
  nodeId: string,
  opts?: { force?: boolean; wakeReason?: string; cfg?: AstroclawConfig },
  ...
  try {
    ...
    if (registration.transport === "relay") {
      ...
      result = await sendApnsAlert({
        registration,
        nodeId,
        title: "Astroclaw needs a quick reopen",
        body: "Tap to reopen Astroclaw and restore the node connection.",
        relayConfig: relay.relayConfig,
      });
    } else {
      const auth = await resolveDirectNodePushConfig();
