...
async function runSessionsUsage(
  ...
  config: AstroclawConfig = TEST_RUNTIME_CONFIG,
) {
  const respond = vi.fn();
...
describe("sessions.usage", () => {
  ...
  it("uses the requested agent for legacy specific session keys", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-usage-test-"));

    try {
      await withEnvAsync({ ASTROCLAW_STATE_DIR: stateDir }, async () => {
