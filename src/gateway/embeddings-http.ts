...
function resolveAutoExplicitProviders(cfg: AstroclawConfig): Set<string> {
  return new Set(
...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== ASTROCLAW_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-astroclaw-model")) ||
