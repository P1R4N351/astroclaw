...
describe("gateway network runtime", () => {
  ...
  it("bootstraps env proxy dispatching when the gateway starts directly", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-gw-proxy-home-"));
    ...
    try {
      ...
      process.env.ASTROCLAW_BUNDLED_PLUGINS_DIR = path.join(tempHome, "empty-bundled-plugins");
      await fs.mkdir(process.env.ASTROCLAW_BUNDLED_PLUGINS_DIR, { recursive: true });
