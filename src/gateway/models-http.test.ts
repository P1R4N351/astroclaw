...
describe("OpenAI-compatible models HTTP API (e2e)", () => {
  it("serves /v1/models when compatibility endpoints are enabled", async () => {
    ...
    expect(json.data?.map((entry) => entry.id)).toContain("astroclaw");
    expect(json.data?.map((entry) => entry.id)).toContain("astroclaw/default");
    expect(
      json.data?.every((entry) => typeof entry.id === "string" && entry.id?.startsWith("astroclaw")),
    ).toBe(true);
  });
  ...
  it("rejects operator scopes that lack read access", async () => {
    const res = await getModels("/v1/models", { "x-astroclaw-scopes": "operator.approvals" });
    await expectMissingReadScope(res);
  });

  it("rejects requests with no declared operator scopes", async () => {
    const res = await getModels("/v1/models", { "x-astroclaw-scopes": "" });
    await expectMissingReadScope(res);
  });
