...
export function formatUnknownChannelMessage(params: {
  ...
  const listCommand = params.listCommand ?? "astroclaw channels list --all";
...
export function formatUnsupportedChannelActionMessage(params: {
  ...
  const inspectCommand =
    params.inspectCommand ?? `astroclaw channels capabilities --channel ${params.channel}`;
...
export function formatStrictJsonParseFailure(params: { value: string; cause: unknown }): string {
  ...
  return [
    ...
    `Use valid JSON, for example ${formatInlineCliCommand(
      "astroclaw config set gateway.port 18789 --strict-json",
    )}.`,
...
export function formatGatewayCommandFailure(params: {
  ...
  const message = raw
    .replace(/\s*Run [`"]?astroclaw doctor[`"]? for diagnostics\.?/gi, "")
    .replace(/\s+Gateway target:\s+.*$/isu, "")
    .replace(/\s+/g, " ")
    .trim()
    .replace(/[.。]+$/u, "");
  const inspectCommand = params.inspectCommand ?? "astroclaw gateway status --deep";
...
export function formatMissingPluginMessage(params: {
  ...
  const listCommand = params.listCommand ?? "astroclaw plugins list";
  const searchHint = params.includeSearch
    ? `, or ${formatInlineCliCommand("astroclaw plugins search " + params.id)} to look for installable plugins`
    : "";
