...
export const COMPLETION_SKIP_PLUGIN_COMMANDS_ENV = "ASTROCLAW_COMPLETION_SKIP_PLUGIN_COMMANDS";
...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Astroclaw Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# Astroclaw Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(astroclaw completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "astroclaw") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
