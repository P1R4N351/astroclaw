...
describe("capability cli", () => {
  ...
  it("passes image files to local model probes", async () => {
    const tempInput = path.join(os.tmpdir(), `astroclaw-model-run-image-${Date.now()}.png`);
  ...
  it("passes image files to gateway model probes as attachments", async () => {
    const tempInput = path.join(os.tmpdir(), `astroclaw-model-run-gateway-image-${Date.now()}.png`);
    await fs.writeFile(tempInput, Buffer.from(PNG_1X1_BASE64, "base64"));
  ...
  it("normalizes HEIC files to JPEG before local model probes", async () => {
    const tempInput = path.join(os.tmpdir(), `astroclaw-model-run-image-${Date.now()}.heic`);
  ...
  it("rejects non-image files for model probes", async () => {
    const tempInput = path.join(os.tmpdir(), `astroclaw-model-run-audio-${Date.now()}.mp3`);
  ...
  it("rewrites mismatched explicit image output extensions to the detected file type", async () => {
    ...
    const tempOutput = path.join(os.tmpdir(), `astroclaw-image-mismatch-${Date.now()}.png`);
  ...
  it("passes image output format and OpenAI background hints through to edit runtime", async () => {
    ...
    const inputPath = path.join(os.tmpdir(), `astroclaw-image-edit-${Date.now()}.png`);
  ...
  it("forwards size, aspect ratio, and resolution overrides for image edit", async () => {
    ...
    const tempInput = path.join(os.tmpdir(), `astroclaw-image-edit-input-${Date.now()}.png`);
    const tempOutput = path.join(os.tmpdir(), `astroclaw-image-edit-output-${Date.now()}.png`);
  ...
  it("streams url-only generated videos to --output paths", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-video-generate-"));
