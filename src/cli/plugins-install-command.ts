...
async function installBundledPluginSource(params: {
  ...
  const configWarning = shouldEnable
    ? ""
    : `Installed bundled plugin "${params.bundledSource.pluginId}" without enabling it because it requires configuration first. Configure it, then run \`astroclaw plugins enable ${params.bundledSource.pluginId}\`.`;
...
async function loadConfigFromSnapshotForInstall(
  ...
  if (
    ...
    throw buildInvalidPluginInstallConfigError(
      `Config invalid outside the plugin recovery path for ${pluginLabel}; run \`astroclaw doctor --fix\` before reinstalling it.`,
    );
  }
...
export async function runPluginInstallCommand(params: {
  ...
  if (opts.marketplace) {
    if (opts.link) {
      runtime.error(
        `--link is not supported with --marketplace. Remove --link, or install a local path with ${formatCliCommand("astroclaw plugins install --link <path>")}.`,
      );
      return runtime.exit(1);
    }
    if (opts.pin) {
      runtime.error(
        `--pin is not supported with --marketplace. Use ${formatCliCommand("astroclaw plugins install <plugin> --marketplace <name>")} without --pin.`,
      );
      return runtime.exit(1);
    }
  }
  ...
  if (gitPrefix && !gitSpec) {
    runtime.error(
      `Unsupported git plugin spec: ${raw}. Use ${formatCliCommand("astroclaw plugins install git:<repo>@<ref>")}.`,
    );
    return runtime.exit(1);
  }
  if (gitSpec && opts.link) {
    runtime.error(
      `--link is not supported with git: installs. Use ${formatCliCommand("astroclaw plugins install git:<repo>@<ref>")} for Git installs or ${formatCliCommand("astroclaw plugins install --link <path>")} for local paths.`,
    );
    return runtime.exit(1);
  }
  if (gitSpec && opts.pin) {
    runtime.error(
      `--pin is not supported with git: installs. Pin the ref in the spec instead, for example ${formatCliCommand("astroclaw plugins install git:<repo>@<ref>")}.`,
    );
    return runtime.exit(1);
  }
  if (opts.link && opts.force) {
    runtime.error(
      `--force is not supported with --link. Linked plugins point at the source path directly; remove --force and re-run ${formatCliCommand("astroclaw plugins install --link <path>")}.`,
    );
    return runtime.exit(1);
  }
  ...
  if (opts.link) {
    runtime.error(
      `--link requires a local path. Run ${formatCliCommand("astroclaw plugins install --link <path>")}.`,
    );
    return runtime.exit(1);
  }
  ...
  if (npmPrefixSpec !== null) {
    if (!npmPrefixSpec) {
      runtime.error(
        `Unsupported npm plugin spec: missing package. Use ${formatCliCommand("astroclaw plugins install npm:<package>")}.`,
      );
      return runtime.exit(1);
    }
  ...
  if (npmPackPath !== null) {
    if (!npmPackPath) {
      runtime.error(
        `Unsupported npm-pack plugin spec: missing archive path. Use ${formatCliCommand("astroclaw plugins install npm-pack:<path-to.tgz>")}.`,
      );
      return runtime.exit(1);
    }
  ...
  if (
    ...
    runtime.error(
      `Plugin path not found: ${resolved}. Check the path, or install from npm with ${formatCliCommand("astroclaw plugins install npm:<package>")}.`,
    );
    return runtime.exit(1);
  }
