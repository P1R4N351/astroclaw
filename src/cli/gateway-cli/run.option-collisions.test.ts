...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/astroclaw-stability.json",
  path: "/tmp/astroclaw-stability.json",
}));
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/astroclaw-control-ui" as string | null,
}));
...
vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/astroclaw-test-missing-config.json",
...
vi.mock("../../config/paths.js", () => ({
  CONFIG_PATH: "/tmp/astroclaw-test-missing-config.json",
  resolveStateDir: () => "/tmp",
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/astroclaw-control-ui";
  ...
  it("blocks service-mode startup from an older binary with newer config", async () => {
    ...
    const previousMarker = process.env.ASTROCLAW_SERVICE_MARKER;
    process.env.ASTROCLAW_SERVICE_MARKER = "gateway";
    try {
      ...
      if (previousMarker === undefined) {
        delete process.env.ASTROCLAW_SERVICE_MARKER;
      } else {
        process.env.ASTROCLAW_SERVICE_MARKER = previousMarker;
      }
    }
  ...
  it.each([
    ...
    delete process.env.ASTROCLAW_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.ASTROCLAW_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `astroclaw onboard --mode local` or `astroclaw setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("blocks invalid startup config without automatic recovery", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/astroclaw-test-missing-config.json",
      config: {},
      parsed: null,
