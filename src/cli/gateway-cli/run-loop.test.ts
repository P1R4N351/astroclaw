...
>(() => ({ mode: "disabled", detail: "ASTROCLAW_NO_RESPAWN" }));
...
describe("runGatewayLoop", () => {
  ...
  it("releases the lock before exiting on spawned restart", async () => {
    vi.clearAllMocks();
    peekGatewaySigusr1RestartReason.mockReturnValue(undefined);
    const originalTraceEnv = process.env.ASTROCLAW_GATEWAY_RESTART_TRACE;
    process.env.ASTROCLAW_GATEWAY_RESTART_TRACE = "1";
    ...
    try {
      await withIsolatedSignals(async ({ captureSignal }) => {
        ...
        expect(respawnOpts?.env?.ASTROCLAW_GATEWAY_RESTART_TRACE_STARTED_AT_MS).toMatch(/^\d/u);
        expect(respawnOpts?.env?.ASTROCLAW_GATEWAY_RESTART_TRACE_LAST_AT_MS).toMatch(/^\d/u);
        expect(writeGatewayRestartHandoffSync).not.toHaveBeenCalled();
      });
    } finally {
      if (originalTraceEnv === undefined) {
        delete process.env.ASTROCLAW_GATEWAY_RESTART_TRACE;
      } else {
        process.env.ASTROCLAW_GATEWAY_RESTART_TRACE = originalTraceEnv;
      }
    }
  });
