...
const STEP_LABELS: Record<string, string> = {
  ...
  "astroclaw doctor entry": "Checking doctor entrypoint",
  "astroclaw doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g astroclaw@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g astroclaw@latest --omit=optional");
  }
