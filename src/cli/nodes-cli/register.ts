    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["astroclaw nodes status", "List known nodes with live status."],
          ["astroclaw nodes pairing pending", "Show pending node pairing requests."],
          ["astroclaw nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'astroclaw nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["astroclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.astroclaw.ai/cli/nodes")}\n`,
    );
