...
describe("tryRouteCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    originalDisableRouteFirst = process.env.ASTROCLAW_DISABLE_ROUTE_FIRST;
    originalHideBanner = process.env.ASTROCLAW_HIDE_BANNER;
    delete process.env.ASTROCLAW_DISABLE_ROUTE_FIRST;
  ...
  afterEach(() => {
    ...
    if (originalDisableRouteFirst === undefined) {
      ...
      process.env.ASTROCLAW_DISABLE_ROUTE_FIRST = originalDisableRouteFirst;
    }
  ...
  it("skips config guard for routed status --json commands", async () => {
    await expect(tryRouteCli(["node", "astroclaw", "status", "--json"])).resolves.toBe(true);
  ...
  it("does not pass suppressDoctorStdout for routed non-json commands", async () => {
    await expect(tryRouteCli(["node", "astroclaw", "status"])).resolves.toBe(true);
  ...
  it("keeps logs routed to stderr for routed --json commands", async () => {
    ...
    await tryRouteCli(["node", "astroclaw", "agents", "--json"]);
  ...
  it("routes command-run logs to stderr for config-guard-skipping --json routes", async () => {
    ...
    await expect(tryRouteCli(["node", "astroclaw", "models", "status", "--json"])).resolves.toBe(
      true,
    );
  ...
  it("does not route logs to stderr during plugin loading without --json", async () => {
    ...
    await tryRouteCli(["node", "astroclaw", "agents"]);
  ...
  it("routes status when root options precede the command", async () => {
    await expect(tryRouteCli(["node", "astroclaw", "--log-level", "debug", "status"])).resolves.toBe(
      true,
    );

    expect(findRoutedCommandMock).toHaveBeenCalledWith(
      ["status"],
      ["node", "astroclaw", "--log-level", "debug", "status"],
    );
    expect(ensureConfigReadyMock).toHaveBeenCalledTimes(1);
    const configReadyCall = firstConfigReadyCall();
    expect(typeof configReadyCall?.runtime).toBe("object");
  ...
  it("respects ASTROCLAW_HIDE_BANNER for routed commands", async () => {
  ...
  it("falls back before bootstrap when the route cannot parse the argv", async () => {
    ...
    await expect(tryRouteCli(["node", "astroclaw", "tasks", "list"])).resolves.toBe(false);
