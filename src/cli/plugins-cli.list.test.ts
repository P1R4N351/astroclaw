...
describe("plugins cli list", () => {
  ...
  it("reports stale plugin config in doctor output without claiming full plugin health", async () => {
    ...
    expect(output).toContain(
      'Run "astroclaw doctor --fix" to remove stale plugin ids and dangling channel references.',
    );
    expect(output).toContain(
  ...
  it("reports config-selected plugin source shadowing in doctor output", async () => {
      ...
      diagnostics: [
        {
          level: "warn",
          pluginId: "discord",
          source: "/tmp/astroclaw/npm/node_modules/@astroclaw/discord/index.ts",
          message:
            "duplicate plugin id resolved by explicit config-selected plugin; global plugin will be overridden by config plugin (/tmp/astroclaw-upstream/extensions/discord/index.ts)",
        },
      ],
    });
    ...
    expect(output).toContain("active: /tmp/astroclaw-upstream/extensions/discord/index.ts");
    expect(output).toContain("shadowed: /tmp/astroclaw/npm/node_modules/@astroclaw/discord/index.ts");
    expect(output).toContain("astroclaw plugins registry --refresh");
  });
  ...
  it("reports persisted plugin registry state without refreshing", async () => {
    ...
    expect(runtimeLogs.join("\n")).toContain("astroclaw plugins registry --refresh");
  });
  ...
  it("keeps inspect on the static snapshot by default", async () => {
    setInstalledPluginIndexInstallRecords({
      "astroclaw-mem0": {
        source: "clawhub",
        spec: "clawhub:astroclaw-mem0",
        installPath: "/plugins/astroclaw-mem0",
        version: "2026.5.1",
        clawhubPackage: "astroclaw-mem0",
        clawhubChannel: "official",
        artifactKind: "npm-pack",
        artifactFormat: "tgz",
        npmIntegrity: "sha512-clawpack",
        npmShasum: "1".repeat(40),
        npmTarballName: "astroclaw-mem0-2026.5.1.tgz",
    ...
    buildPluginSnapshotReport.mockReturnValue({
      plugins: [createPluginRecord({ id: "astroclaw-mem0", name: "Mem0" })],
      diagnostics: [],
    });
    buildPluginInspectReport.mockReturnValue({
      workspaceDir: "/workspace",
      plugin: createPluginRecord({ id: "astroclaw-mem0", name: "Mem0" }),
      shape: "hook-only",
    ...
    await runPluginsCommand(["plugins", "inspect", "astroclaw-mem0"]);

    expect(buildPluginDiagnosticsReport).not.toHaveBeenCalled();
    expect(runtimeLogs.join("\n")).toContain("Policy");
    expect(runtimeLogs.join("\n")).toContain("allowConversationAccess: true");
    expect(runtimeLogs.join("\n")).toContain("ClawHub package: astroclaw-mem0");
  ...
  it("runtime-inspects without repairing deps", async () => {
    ...
    await runPluginsCommand(["plugins", "inspect", "astroclaw-mem0", "--runtime"]);

    expect(buildPluginDiagnosticsReport).toHaveBeenCalledWith({
      config: {},
      onlyPluginIds: ["astroclaw-mem0"],
    });
  });
