...
const createInstallPlanFixture = vi.hoisted(() => {
  return async (params?: {
    ...
    if (params?.wrapperPath || params?.env?.ASTROCLAW_WRAPPER) {
      environment.ASTROCLAW_WRAPPER = params.wrapperPath ?? params.env?.ASTROCLAW_WRAPPER;
    }
    return {
...
vi.mock("../../config/io.js", () => ({
  ...
  readConfigFileSnapshotForWrite: vi.fn(async () => ({
    ...
    writeOptions: { expectedConfigPath: "/tmp/astroclaw.json" },
  })),
}));
...
function mockResolvedGatewayTokenSecretRef() {
  ...
  resolveSecretRefValuesMock.mockResolvedValue(
    new Map([["env:default:ASTROCLAW_GATEWAY_TOKEN", "resolved-from-secretref"]]),
  );
}
...
describe("runDaemonInstall", () => {
  ...
  it("does not treat env-template gateway.auth.token as plaintext during install", async () => {
    loadConfigMock.mockReturnValue({
      gateway: { auth: { mode: "token", token: "${ASTROCLAW_GATEWAY_TOKEN}" } },
    });
    mockResolvedGatewayTokenSecretRef();
  ...
  it("reinstalls when the loaded service still embeds ASTROCLAW_GATEWAY_TOKEN", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["astroclaw", "gateway", "run"],
      environment: {
        ASTROCLAW_GATEWAY_TOKEN: "stale-service-token",
      },
    } as never);
    ...
    expect(installDaemonServiceAndEmitMock).toHaveBeenCalledTimes(1);
    expect(actionState.warnings).toContain(
      "Gateway service ASTROCLAW_GATEWAY_TOKEN differs from the current install plan; refreshing the install.",
    );
  });
  ...
  it("returns already-installed when the embedded gateway token matches the install plan", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["astroclaw", "gateway", "run"],
      environment: {
        ASTROCLAW_GATEWAY_TOKEN: "durable-token",
      },
    } as never);
    buildGatewayInstallPlanMock.mockResolvedValueOnce({
      programArguments: ["astroclaw", "gateway", "run"],
  ...
  it("preserves wrapper env from an installed but unloaded service during forced reinstall", async () => {
    service.isLoaded.mockResolvedValue(false);
    service.readCommand.mockResolvedValue({
      programArguments: ["/usr/local/bin/astroclaw-doppler", "gateway", "run"],
      environment: {
    ...
    expectFields(installPlanArg, { wrapperPath: "/usr/local/bin/astroclaw-doppler" });
  ...
  it("reinstalls when wrapper command matches but wrapper env is missing", async () => {
    ...
    await runDaemonInstall({
      json: true,
      wrapper: "/usr/local/bin/astroclaw-doppler",
    });

    expect(installDaemonServiceAndEmitMock).toHaveBeenCalledTimes(1);
    expect(actionState.warnings).toContain(
      "Gateway service ASTROCLAW_WRAPPER differs from the current wrapper install plan; refreshing the install.",
    );
  });
  ...
  it("does not reinstall when ASTROCLAW_GATEWAY_TOKEN comes from an env file", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["astroclaw", "gateway", "run"],
      environment: {
  ...
  it("does not reuse stale service control env during forced reinstall", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["astroclaw", "gateway", "run"],
      environment: {
        ASTROCLAW_STATE_DIR: "/tmp/astroclaw-doctor-manual",
        ASTROCLAW_CONFIG_PATH: "/tmp/astroclaw-doctor-manual/astroclaw.json",
        ASTROCLAW_GATEWAY_TOKEN: "stale-service-token",
    ...
    try {
      ...
      expect(env.ASTROCLAW_STATE_DIR).toBeUndefined();
      expect(env.ASTROCLAW_CONFIG_PATH).toBeUndefined();
