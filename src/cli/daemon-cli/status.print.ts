...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("astroclaw doctor")}" (or "${formatCliCommand("astroclaw doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("astroclaw gateway install --force")}\` from the same --profile / ASTROCLAW_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (gatewayVersion) {
    ...
    if (status.cli?.version && status.cli.version !== gatewayVersion) {
      defaultRuntime.error(
        warnText(
          `Warning: this Astroclaw command is version ${status.cli.version}, but the running Gateway is version ${gatewayVersion}.`,
        ),
      );
      defaultRuntime.error(
        warnText(
          "Check `astroclaw --version`, `which astroclaw`, and `astroclaw gateway status --deep`; if this mismatch is unexpected, update PATH so `astroclaw` points to the version you want, or reinstall the Gateway service from that same Astroclaw install.",
        ),
      );
    }
    spacer();
  ...
  if (
    ...
    defaultRuntime.error(
      errorText(
        `Fix: run ${formatCliCommand("astroclaw gateway restart")} and re-check with ${formatCliCommand("astroclaw gateway status --deep")}.`,
      ),
    );
    spacer();
  }
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.ASTROCLAW_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("astroclaw gateway install")}`),
    );
    spacer();
  }

  if (service.staleUpdateLaunchdJobs?.length) {
    defaultRuntime.error(errorText("Stale Astroclaw updater launchd job(s) detected."));
    ...
    defaultRuntime.error(
      errorText(
        `Fix after confirming no update is running: launchctl remove <label>, then run ${formatCliCommand("astroclaw gateway restart")}.`,
      ),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const unit = resolveGatewaySystemdServiceName(serviceEnv.ASTROCLAW_PROFILE);
      defaultRuntime.error(
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("astroclaw status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.astroclaw.ai/troubleshooting`);
}
