...
describe("proxy cli runtime", () => {
  const envKeys = [
    ...
    "ASTROCLAW_DEBUG_PROXY_CERT_DIR",
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "astroclaw-proxy-cli-runtime-"));
    ...
    process.env.ASTROCLAW_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.ASTROCLAW_DEBUG_PROXY_ENABLED;
    delete process.env.ASTROCLAW_DEBUG_PROXY_SESSION_ID;
  ...
  it("prints actionable disabled proxy config output", async () => {
    ...
    expect(process.stdout.write).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Enable proxy.enabled with proxy.proxyUrl or ASTROCLAW_PROXY_URL, or pass --proxy-url for an explicit one-off validation.\n",
    );
  });
  ...
  it("prints actionable output when proxy config is disabled and missing", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        ...
        source: "disabled",
        errors: [
          "proxy validation requires proxy.enabled=true with proxy.proxyUrl or ASTROCLAW_PROXY_URL, or --proxy-url",
        ],
      },
      checks: [],
    });
  ...
  it("redacts malformed proxy URLs in text output", async () => {
    ...
    expect(process.stdout.write).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Fix proxy.proxyUrl, ASTROCLAW_PROXY_URL, or --proxy-url so it uses a reachable http:// proxy.\n",
    );
  });
  ...
  it("prints proxy validation JSON and sets exit code on failure", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        enabled: true,
        source: "missing",
        errors: ["proxy validation requires proxy.proxyUrl, --proxy-url, or ASTROCLAW_PROXY_URL"],
      },
      checks: [],
    });
    const { runProxyValidateCommand } = await import("./proxy-cli.runtime.js");
      ...
      `${JSON.stringify(
        {
          ...
          config: {
            ...
            source: "missing",
            errors: [
              "proxy validation requires proxy.proxyUrl, --proxy-url, or ASTROCLAW_PROXY_URL",
            ],
          },
          checks: [],
        },
