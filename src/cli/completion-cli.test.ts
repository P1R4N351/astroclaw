...
describe("completion-cli", () => {
  it("generates zsh functions for nested subcommands", () => {
    ...
    expect(script).toContain("_astroclaw_gateway()");
    expect(script).toContain("(status) _astroclaw_gateway_status ;;");
    expect(script).toContain("(restart) _astroclaw_gateway_restart ;;");
  ...
  it("defers zsh registration until compinit is available", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-zsh-completion-"));
    try {
      const scriptPath = path.join(tempDir, "astroclaw.zsh");
      ...
      const result = spawnSync(
        ...
        [
          ...
          `
            ...
            [[ -z "\${_comps[astroclaw]-}" ]] || exit 10
            [[ "\${precmd_functions[(r)_astroclaw_register_completion]}" = "_astroclaw_register_completion" ]] || exit 11
            autoload -Uz compinit
            compinit -C
            _astroclaw_register_completion
            [[ -z "\${precmd_functions[(r)_astroclaw_register_completion]}" ]] || exit 12
            [[ "\${_comps[astroclaw]-}" = "_astroclaw_root_completion" ]]
          `,
        ],
        {
          encoding: "utf8",
  ...
  it("generates PowerShell command paths without the executable prefix", () => {
    ...
    expect(script).not.toContain("if ($commandPath -eq 'astroclaw gateway') {");
  ...
  it("generates fish completions for root and nested command contexts", () => {
    ...
    expect(script).toContain(
      'complete -c astroclaw -n "__fish_use_subcommand" -a "gateway" -d \'Gateway commands\'',
    );
    expect(script).toContain(
      'complete -c astroclaw -n "__fish_seen_subcommand_from gateway" -a "status" -d \'Show gateway status\'',
    );
    expect(script).toContain(
      "complete -c astroclaw -n \"__fish_seen_subcommand_from gateway\" -l force -d 'Force the action'",
    );
  });
});
