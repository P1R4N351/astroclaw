...
${formatHelpExamples([
  ['astroclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['astroclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'astroclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'astroclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['astroclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'astroclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.astroclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.astroclaw.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['astroclaw agents set-identity --agent main --name "Astroclaw" --emoji "🦞"', "Set name + emoji."],
  ["astroclaw agents set-identity --agent main --avatar avatars/astroclaw.png", "Set avatar path."],
  [
    "astroclaw agents set-identity --workspace ~/.astroclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "astroclaw agents set-identity --identity-file ~/.astroclaw/workspace/IDENTITY.md --agent main",
