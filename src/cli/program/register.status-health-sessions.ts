...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["astroclaw status", "Show channel health + session summary."],
          ["astroclaw status --all", "Full diagnosis (read-only)."],
          ["astroclaw status --json", "Machine-readable output."],
          ["astroclaw status --usage", "Show model provider usage/quota snapshots."],
          [
            "astroclaw status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["astroclaw status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.astroclaw.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.astroclaw.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["astroclaw sessions", "List all sessions."],
          ["astroclaw sessions --agent work", "List sessions for one agent."],
          ["astroclaw sessions --all-agents", "Aggregate sessions across agents."],
          ["astroclaw sessions --active 120", "Only last 2 hours."],
          ["astroclaw sessions --limit 25", "Show the newest 25 sessions."],
          ["astroclaw sessions --json", "Machine-readable output."],
          ["astroclaw sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.astroclaw.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["astroclaw sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "astroclaw sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          [
            "astroclaw sessions cleanup --dry-run --fix-dm-scope",
            "Preview stale direct-DM rows after returning dmScope to main.",
          ],
          ["astroclaw sessions cleanup --enforce", "Apply maintenance now."],
          ["astroclaw sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["astroclaw sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "astroclaw sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .astroclaw/trajectory-exports")
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["astroclaw commitments", "List pending inferred follow-ups."],
          ["astroclaw commitments --all", "List all inferred follow-ups."],
          ["astroclaw commitments --agent work", "List one agent's inferred follow-ups."],
          ["astroclaw commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
