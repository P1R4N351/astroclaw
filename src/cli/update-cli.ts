...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Astroclaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["astroclaw update", "Update a source checkout (git)"],
        ["astroclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["astroclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["astroclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["astroclaw update --tag main", "One-off package install from GitHub main"],
        ["astroclaw update --dry-run", "Preview actions without changing anything"],
        ["astroclaw update --no-restart", "Update without restarting the service"],
        ["astroclaw update --json", "Output result as JSON"],
        ["astroclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["astroclaw update wizard", "Interactive update wizard"],
        ["astroclaw --update", "Shorthand for astroclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run astroclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.astroclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .description("Run Astroclaw update finalization after an external core runtime change")
    .option("--json", "Output result as JSON", false)
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["astroclaw update status", "Show channel + version status."],
          ["astroclaw update status --json", "JSON output."],
          ["astroclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.astroclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
