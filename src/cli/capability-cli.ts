...
const LOCAL_MODEL_RUN_SYSTEM_PROMPT = "You are a personal assistant running inside Astroclaw.";
...
function getAuthProfileIdsForProvider(cfg: AstroclawConfig, providerId: string): string[] {
...
export function registerCapabilityCli(program: Command) {
  ...
  const capability = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/infer", "docs.astroclaw.ai/cli/infer")}\n`,
    );
