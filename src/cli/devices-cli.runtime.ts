...
function buildFallbackStateMismatchError(details: ConnectPairingRequiredDetails): Error {
  return new Error(
    [
      ...
      "The running gateway is probably using a different ASTROCLAW_PROFILE or ASTROCLAW_STATE_DIR than this CLI.",
...
function buildExplicitApproveCommand(opts: DevicesRpcOpts, requestId: string): string {
  const args = ["astroclaw", "devices", "approve", requestId];
...
function resolveRequiredDeviceRole(
  ...
  defaultRuntime.error(
    `--device and --role are required. Run ${formatCliCommand("astroclaw devices list")} to choose a paired device.`,
  );
...
export async function runDevicesRemoveCommand(
  ...
  if (!trimmed) {
    defaultRuntime.error(
      `deviceId is required. Run ${formatCliCommand("astroclaw devices list")} to choose a paired device.`,
    );
    defaultRuntime.exit(1);
    return;
  }
...
export async function runDevicesApproveCommand(
  ...
  if (usingImplicitSelection) {
    ...
    switch (approval.kind) {
      ...
      case "re-approval":
        defaultRuntime.log(
          "  Note:   Already paired. Approval-bound device details changed, so Astroclaw created a fresh request instead of silently reusing the old approval.",
        );
        break;
      case "new-pairing":
