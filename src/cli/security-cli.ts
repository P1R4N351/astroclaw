...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["astroclaw security audit", "Run a local security audit."],
          [
            "astroclaw security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["astroclaw security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "astroclaw security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["astroclaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["astroclaw security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.astroclaw.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Astroclaw security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("astroclaw security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("astroclaw security audit --fix")}`));
        if (!fixResult) {
