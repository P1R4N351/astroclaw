...
export type CanonicalInboundMessageHookContext = {
  ...
  // `mediaPath(s)` are files Astroclaw has already staged locally. `mediaUrl(s)`
...
export function toInternalMessageTranscribedContext(
  canonical: CanonicalInboundMessageHookContext,
  cfg: AstroclawConfig,
): MessageTranscribedHookContext & { cfg: AstroclawConfig } {
...
export function toInternalMessagePreprocessedContext(
  canonical: CanonicalInboundMessageHookContext,
  cfg: AstroclawConfig,
): MessagePreprocessedHookContext & { cfg: AstroclawConfig } {
  const shared = toInternalInboundMessageHookContextBase(canonical);
  return {
    ...shared,
    transcript: canonical.transcript,
