...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "astroclaw-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "astroclaw-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "astroclaw-bundled");
      const plugin = makeHookEntry("shared", "astroclaw-plugin");
      const managed = makeHookEntry("shared", "astroclaw-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("astroclaw-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "astroclaw-managed");
      const workspace = makeHookEntry("shared", "astroclaw-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "astroclaw-workspace");
      const second = makeHookEntry("shared", "astroclaw-workspace");
      second.hook.handlerPath = "/tmp/astroclaw-workspace/shared/handler-2.js";
