...
This directory contains hooks that ship with Astroclaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.astroclaw/workspace`)
...
homepage: https://docs.astroclaw.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.astroclaw/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.astroclaw/astroclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'astroclaw.*gateway' && pnpm astroclaw gateway`
3. Enable the hook: `astroclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.astroclaw.ai/automation/hooks
