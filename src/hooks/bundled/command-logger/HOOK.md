...
homepage: https://docs.astroclaw.ai/automation/hooks#command-logger
metadata:
  {
    "astroclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.astroclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.astroclaw/logs/commands.log ~/.astroclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/astroclaw`:
   ```
   /home/username/.astroclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
