...
export function hasConfiguredInternalHooks(config: AstroclawConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: AstroclawConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
