...
export function listAuthProfileStorePaths(config: AstroclawConfig, stateDir: string): string[] {
...
function resolveActiveAgentDir(stateDir: string, env: NodeJS.ProcessEnv = process.env): string {
  const override = env.ASTROCLAW_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
  if (override) {
