...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[astroclaw proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[astroclaw proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`astroclaw proxy coverage\` for details.`,
  );
}
