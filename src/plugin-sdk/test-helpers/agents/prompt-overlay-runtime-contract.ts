...
export function openAiPluginPersonalityConfig(personality: "friendly" | "off"): AstroclawConfig {
  return {
    plugins: {
      entries: {
        openai: {
...
export function sharedGpt5PersonalityConfig(personality: "friendly" | "off"): AstroclawConfig {
  return {
    agents: {
      defaults: {
        promptOverlays: {
...
export function codexPromptOverlayContext(params?: {
  ...
  return {
    ...
    promptMode: "full",
    agentDir: "/tmp/astroclaw-codex-prompt-contract-agent",
    workspaceDir: "/tmp/astroclaw-codex-prompt-contract-workspace",
