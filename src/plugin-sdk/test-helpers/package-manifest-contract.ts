    ...
    if (minHostVersionBaseline) {
      it("declares a parseable minHostVersion floor at or above the baseline", () => {
        ...
        const requirement = parseMinHostVersionRequirement(
          manifest.astroclaw?.install?.minHostVersion ?? null,
        );

        expect(
          requirement,
          `${packagePath} should declare astroclaw.install.minHostVersion`,
        ).not.toBeNull();
        if (!requirement) {
          return;
        }
        ...
        expect(
          ...
          `${packagePath} should require at least Astroclaw ${minHostVersionBaseline}`,
        ).toBe(true);
      });
    }
  });
