...
import { loadSecretFileSync as loadSecretFileSyncFromCore } from "astroclaw/plugin-sdk/core";
import { readFileWithinRoot, writeFileWithinRoot } from "astroclaw/plugin-sdk/file-access-runtime";
import {
  loadSecretFileSync,
  type SecretFileReadResult,
} from "astroclaw/plugin-sdk/secret-file-runtime";
...
describe("plugin SDK fs-safe compatibility exports", () => {
  it("keeps deprecated secret-file result helpers on public SDK subpaths", async () => {
    await withTempDir({ prefix: "astroclaw-sdk-secret-compat-" }, async (root) => {
  ...
  it("keeps deprecated root-bounded read/write helpers on file-access-runtime", async () => {
    await withTempDir({ prefix: "astroclaw-sdk-file-access-compat-" }, async (root) => {
