...
export async function expectPrivateQaLabRuntimeSurfaceLoad(params: {
  ...
  importRuntime: () => Promise<QaRuntimeModule>;
  loadBundledPluginPublicSurfaceModuleSync: SurfaceLoaderMock;
  resolveAstroclawPackageRootSync: SurfaceLoaderMock;
}) {
  const sourceRoot = makePrivateQaSourceRoot(params.tempDirs, "astroclaw-qa-runtime-root-");
  params.resolveAstroclawPackageRootSync.mockReturnValue(sourceRoot);
  ...
  expect(params.loadBundledPluginPublicSurfaceModuleSync).toHaveBeenCalledWith({
    ...
    env: expect.objectContaining({
      ASTROCLAW_ENABLE_PRIVATE_QA_CLI: "1",
      ASTROCLAW_BUNDLED_PLUGINS_DIR: path.join(sourceRoot, "extensions"),
    }),
  });
}
