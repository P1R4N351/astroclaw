...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.ASTROCLAW_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Astroclaw. You can keep running the web gateway without it: \`${formatCliCommand("astroclaw gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
