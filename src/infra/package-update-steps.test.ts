...
function createNpmTarget(globalRoot: string): ResolvedGlobalInstallTarget {
  return {
    ...
    command: "npm",
    globalRoot,
    packageRoot: path.join(globalRoot, "astroclaw"),
  };
}
...
describe("runGlobalPackageUpdateSteps", () => {
  it("installs npm updates into a clean staged prefix before swapping the global package", async () => {
    await withTempDir({ prefix: "astroclaw-package-update-staged-" }, async (base) => {
      ...
      const runStep = vi.fn(
        async ({ name, argv, cwd, timeoutMs }): Promise<PackageUpdateStepResult> => {
          ...
          await writePackageRoot(
            path.join(stagePrefix, "lib", "node_modules", "astroclaw"),
            "2.0.0",
          );
          await fs.mkdir(path.join(stagePrefix, "bin"), { recursive: true });
          await fs.symlink(
            "../lib/node_modules/astroclaw/dist/index.js",
            path.join(stagePrefix, "bin", "astroclaw"),
          );
          return {
            name,
            command: argv.join(" "),
      ...
      await expect(fs.readlink(path.join(prefix, "bin", "astroclaw"))).resolves.toBe(
        "../lib/node_modules/astroclaw/dist/index.js",
      );
    });
  });
  ...
  it("accepts v-prefixed exact npm specs when verifying staged installs", async () => {
    await withTempDir({ prefix: "astroclaw-package-update-v-prefix-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "astroclaw");
      await writePackageRoot(packageRoot, "1.0.0");
      ...
      const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
        ...
        expect(argv).toContain("astroclaw@v2.0.0");
        const prefixIndex = argv.indexOf("--prefix");
        const stagePrefix = argv[prefixIndex + 1];
        if (!stagePrefix) {
          throw new Error("missing staged prefix");
        }
        await writePackageRoot(path.join(stagePrefix, "lib", "node_modules", "astroclaw"), "2.0.0");
        await fs.mkdir(path.join(stagePrefix, "bin"), { recursive: true });
        await fs.symlink(
          "../lib/node_modules/astroclaw/dist/index.js",
          path.join(stagePrefix, "bin", "astroclaw"),
  ...
  it("swaps staged npm package roots through the copy fallback when rename crosses devices", async () => {
    await withTempDir({ prefix: "astroclaw-package-update-exdev-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "astroclaw");
      ...
      const renameSpy = vi
        ...
        .mockImplementation(async (...args: Parameters<typeof fs.rename>) => {
          ...
          if (
            exdevMoves === 0 &&
            fromPath.includes(`${path.sep}.astroclaw-update-stage-`) &&
            path.basename(fromPath) === "astroclaw" &&
  ...
  it("stages pnpm-detected updates through npm when the global root has npm prefix layout", async () => {
    await withTempDir({ prefix: "astroclaw-package-update-pnpm-staged-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "astroclaw");
      ...
      const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
        ...
        expect(argv).toContain("astroclaw@2.0.0");
  ...
  it("keeps Windows pnpm global roots on the pnpm update path", async () => {
    ...
    try {
      await withTempDir({ prefix: "astroclaw-package-update-win32-pnpm-" }, async (base) => {
        ...
        const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
          ...
          expect(argv).toEqual(["pnpm", "add", "-g", "--global-dir", globalDir, "astroclaw@2.0.0"]);
  ...
  it("keeps a successful staged swap when old package cleanup hits a transient Windows native module error", async () => {
    await withTempDir({ prefix: "astroclaw-package-update-staged-cleanup-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "astroclaw");
      await writePackageRoot(packageRoot, "1.0.0");
      ...
      const rmSpy = vi.spyOn(fs, "rm").mockImplementation(async (target, options) => {
        ...
        if (
          targetPath.includes(`${path.sep}.astroclaw-`) &&
          !targetPath.includes(".astroclaw-update-stage-") &&
          !targetPath.includes(".astroclaw-shim-backup-")
        ) {
  ...
  it("does not run post-verify work when staged npm verification fails", async () => {
    await withTempDir({ prefix: "astroclaw-package-update-verify-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "astroclaw");
      await writePackageRoot(packageRoot, "1.0.0");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "astroclaw-package-update-shim-rollback-" }, async (base) => {
        const prefix = path.join(base, "prefix");
        const globalRoot = path.join(prefix, "lib", "node_modules");
        const packageRoot = path.join(globalRoot, "astroclaw");
        const targetShim = path.join(prefix, "bin", "astroclaw");
        await writePackageRoot(packageRoot, "1.0.0");
        ...
        try {
          result = await runGlobalPackageUpdateSteps({
            ...
            runStep: async ({ name, argv, cwd }) => {
              ...
              const stagedShim = path.join(stagePrefix, "bin", "astroclaw");
  ...
  it("cleans the staged npm prefix when the install command throws", async () => {
    await withTempDir({ prefix: "astroclaw-package-update-cleanup-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "astroclaw");
      await writePackageRoot(packageRoot, "1.0.0");
