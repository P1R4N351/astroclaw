...
const ASTROCLAW_DEBUG_PROXY_ENABLED = "ASTROCLAW_DEBUG_PROXY_ENABLED";
...
function isManagedProxyActive(): boolean {
  return process.env["ASTROCLAW_PROXY_ACTIVE"] === "1";
}
...
async function captureGuardedFetchExchange(params: {
  ...
  if (params.capture === false || !isTruthyEnvValue(process.env[ASTROCLAW_DEBUG_PROXY_ENABLED])) {
    return;
  }
