...
const DEFAULT_PACKAGE_NAME = "astroclaw";
...
const PREFLIGHT_TEMP_PREFIX =
  process.platform === "win32" ? "ocu-pf-" : "astroclaw-update-preflight-";
...
const DEV_PREFLIGHT_LINT_ENV: NodeJS.ProcessEnv = {
  ...
  ASTROCLAW_OXLINT_SHARDS_SERIAL: "1",
};
const DEV_PREFLIGHT_LINT_OPT_IN_ENV = "ASTROCLAW_UPDATE_PREFLIGHT_LINT";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["astroclaw", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && (await pathsReferToSameLocation(gitRoot, pkgRoot))) {
    ...
    try {
      ...
      const doctorEntry = path.join(gitRoot, "astroclaw.mjs");
      ...
      const doctorStep = await runStep(
        step("astroclaw doctor", doctorArgv, gitRoot, {
