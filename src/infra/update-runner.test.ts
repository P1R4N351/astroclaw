...
const fixtureRootTracker = createSuiteTempRootTracker({ prefix: "astroclaw-update-" });
...
describe("runGatewayUpdate", () => {
  const preflightPrefixPattern = /(?:astroclaw-update-preflight-|ocu-pf-)/;
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "astroclaw.mjs"), "export {};\n", "utf-8");
  });
  ...
  async function createStableTagRunner(params: {
    ...
    const doctorKey = `${doctorNodePath} ${path.join(tempDir, "astroclaw.mjs")} doctor --non-interactive --fix`;
  ...
  async function setupGitCheckout(options?: { packageManager?: string }) {
    ...
    const pkg: Record<string, string> = { name: "astroclaw", version: "1.0.0" };
  ...
  function createGlobalNpmUpdateRunner(params: {
    ...
    const baseInstallKey = npmGlobalInstallCommand("astroclaw@latest");
    const omitOptionalInstallKey = npmGlobalInstallCommand("astroclaw@latest", ["--omit=optional"]);
  ...
  it("uses pnpm highest resolution mode for update installs", async () => {
    ...
    const { runCommand } = createGitInstallRunner({
      ...
      doctorCommand: `${doctorNodePath} ${path.join(tempDir, "astroclaw.mjs")} doctor --non-interactive --fix`,
  ...
  it("uses pnpm highest resolution mode for dev preflight installs", async () => {
    ...
    const installEnvs: NodeJS.ProcessEnv[] = [];
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(tempDir, "astroclaw.mjs")} doctor --non-interactive --fix`;
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${doctorNodePath} ${path.join(tempDir, "astroclaw.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
  ...
  it("bootstraps pnpm via npm when pnpm and corepack are unavailable", async () => {
    ...
    const { calls, runCommand } = createGitInstallRunner({
      ...
      buildCommand: "pnpm build",
      uiBuildCommand: "pnpm ui:build",
      doctorCommand: `${process.execPath} ${path.join(tempDir, "astroclaw.mjs")} doctor --non-interactive`,
      onCommand: (key, options) => {
        if (key === "pnpm --version") {
          const envPath = options?.env?.PATH ?? options?.env?.Path ?? "";
          if (envPath.includes("astroclaw-update-pnpm-")) {
  ...
  it("uses npm-bootstrapped pnpm for dev preflight when pnpm and corepack are missing", async () => {
    ...
    expect(pnpmEnvPaths.filter((envPath) => envPath.includes("astroclaw-update-pnpm-"))).not.toEqual(
      [],
    );
  });
  ...
  it("runs dev preflight lint in constrained mode when explicitly enabled", async () => {
    ...
    const result = await withEnvAsync({ ASTROCLAW_UPDATE_PREFLIGHT_LINT: "1" }, async () =>
    ...
    expect(lintEnv[0]?.ASTROCLAW_LOCAL_CHECK).toBe("1");
    expect(lintEnv[0]?.ASTROCLAW_LOCAL_CHECK_MODE).toBe("throttled");
    expect(lintEnv[0]?.ASTROCLAW_OXLINT_SHARDS_SERIAL).toBe("1");
  });
  ...
  it("retries windows pnpm git installs with --ignore-scripts for dev updates", async () => {
    ...
    try {
      const runCommand = async (
        ...
        if (key === "pnpm install") {
          if (options?.cwd && /(?:astroclaw-update-preflight-|ocu-pf-)/.test(options.cwd)) {
  ...
  it("falls back to the cloned cwd when git root probing misses a fresh checkout", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(gitRoot, "astroclaw.mjs")} doctor --non-interactive --fix`;
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      ...
      JSON.stringify({ name: "astroclaw", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  async function runNpmGlobalUpdateCase(params: {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async () => {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "astroclaw", version: "2.0.0" }),
          "utf-8",
        );
      },
    });
        ...
        if (normalizedInstallCommand === params.installCommand) {
          const packageRoot =
            process.platform === "win32"
              ? path.join(installPrefix, "node_modules", "astroclaw")
              : path.join(installPrefix, "lib", "node_modules", "astroclaw");
  ...
  it.each([
    {
      ...
      expectedInstallCommand: npmGlobalInstallCommand("astroclaw@latest"),
    },
    {
      title: "uses update channel for global npm installs when tag is omitted",
      expectedInstallCommand: npmGlobalInstallCommand("astroclaw@beta"),
  ...
  it("updates global npm installs from the GitHub main package spec", async () => {
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand: npmGlobalInstallCommand("github:astroclaw/astroclaw#main"),
      tag: "main",
    });

    expect(result.status).toBe("ok");
    expect(result.mode).toBe("npm");
    expect(calls).toContain(npmGlobalInstallCommand("github:astroclaw/astroclaw#main"));
  });
  ...
  it("runs doctor after global npm updates before reporting success", async () => {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
      installCommand: npmGlobalInstallCommand("astroclaw@latest"),
      onInstall: async () => {
    ...
    expect(result.steps.map((step) => step.name)).toContain("astroclaw doctor");
  ...
  it("fails global npm updates when post-update doctor fails", async () => {
    ...
    expect(lastStep?.name).toBe("astroclaw doctor");
  ...
  it("falls back to global npm update when git is missing from PATH", async () => {
    ...
    expect(result.status).toBe("ok");
    expect(result.mode).toBe("npm");
    expect(calls).toContain(npmGlobalInstallCommand("astroclaw@latest"));
  });

  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "astroclaw");
    const staleDir = path.join(nodeModules, ".astroclaw-stale");
  ...
  it("fails global npm update when the installed version misses the requested correction", async () => {
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand: npmGlobalInstallCommand("astroclaw@2026.3.23-2"),
      tag: "2026.3.23-2",
    });
    ...
    expect(calls).toContain(npmGlobalInstallCommand("astroclaw@2026.3.23-2"));
  });

  it("fails global npm update when bundled runtime sidecars are missing after install", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    const expectedInstallCommand = npmGlobalInstallCommand("astroclaw@latest");
    ...
    const { runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async (options) => {
        ...
        if (options?.installPrefix) {
          ...
          await fs.writeFile(path.join(binDir, "astroclaw"), "#!/bin/sh\n", "utf-8");
        }
      },
    });
  ...
  it("uses ASTROCLAW_UPDATE_PACKAGE_SPEC for global package updates", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    const expectedInstallCommand = npmGlobalInstallCommand(
      "http://10.211.55.2:8138/astroclaw-next.tgz",
    );
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
  ...
  it("updates global bun installs when detected", async () => {
    ...
    await withEnvAsync({ BUN_INSTALL: bunInstall }, async () => {
      ...
      const { calls, runCommand } = createGlobalInstallHarness({
        pkgRoot,
        installCommand: "bun add -g astroclaw@latest",
        onInstall: async () => {
          await writeGlobalPackageVersion(pkgRoot);
        },
      });
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls).toContain("bun add -g astroclaw@latest");
    });
  });

  it("rejects git roots that are not a astroclaw checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-astroclaw-root");
    expect(calls.filter((call) => call.includes("status --porcelain"))).toEqual([]);
  });

  it("fails with a clear reason when astroclaw.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "astroclaw.mjs"), { force: true });
    ...
    expect(result.steps.at(-1)?.name).toBe("astroclaw doctor entry");
  });
