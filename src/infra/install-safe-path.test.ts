...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@astroclaw/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@astroclaw/matrix", id: "matrix", expected: true },
    { packageName: "@astroclaw/matrix", id: "@astroclaw/matrix", expected: true },
    { packageName: "@astroclaw/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@astroclaw/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@astroclaw__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "astroclaw-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "astroclaw-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "astroclaw-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "astroclaw-install-safe-" }, async (parentDir) => {
