...
const stateDir = "/opt/astroclaw/state";
...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/astroclaw/state/sessions/../config.jsonl` resolves to
    // `/opt/astroclaw/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("normalizes Windows-style separators before anchor checks", () => {
    const winStateDir = "C:\\astroclaw\\state";
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/astroclaw/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });
