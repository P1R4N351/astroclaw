...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__astroclaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "astroclaw-root");
...
function setPackageRoot(root: string, name = "astroclaw") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./astroclaw-root.js").resolveAstroclawPackageRootSync,
  asyncResolver: typeof import("./astroclaw-root.js").resolveAstroclawPackageRoot,
  opts: Parameters<typeof import("./astroclaw-root.js").resolveAstroclawPackageRootSync>[0],
...
vi.mock("./astroclaw-root.fs.runtime.js", () => ({
  astroClawRootFsSync: mockFsModule(),
  astroClawRootFs: mockFsPromisesModule(),
}));

describe("resolveAstroclawPackageRoot", () => {
  let resolveAstroclawPackageRoot: typeof import("./astroclaw-root.js").resolveAstroclawPackageRoot;
  let resolveAstroclawPackageRootSync: typeof import("./astroclaw-root.js").resolveAstroclawPackageRootSync;
  let clearAstroclawPackageRootCaches: typeof import("./astroclaw-root.js").__testing.clearAstroclawPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveAstroclawPackageRoot,
      resolveAstroclawPackageRootSync,
      __testing: { clearAstroclawPackageRootCaches },
    } = await import("./astroclaw-root.js"));
  });

  beforeEach(() => {
    clearAstroclawPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "astroclaw");
        const pkgRoot = path.join(project, "node_modules", "astroclaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "astroclaw");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "astroclaw.mjs")));
    ...
    {
      name: "falls through from a non-astroclaw moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-astroclaw");
    ...
    {
      name: "returns null for non-astroclaw package roots",
      setup: () => {
        const pkgRoot = fx("not-astroclaw");
        setPackageRoot(pkgRoot, "not-astroclaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "astroclaw.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "astroclaw");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
