...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: AstroclawConfig) => void;
};
...
function resolveHeartbeatAgents(cfg: AstroclawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
function resolveHeartbeatPromptRaw(cfg: AstroclawConfig, heartbeat?: HeartbeatConfig) {
  return heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt;
}

export function resolveHeartbeatPrompt(cfg: AstroclawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(resolveHeartbeatPromptRaw(cfg, heartbeat));
}

function resolveHeartbeatResponseToolPrompt(cfg: AstroclawConfig, heartbeat?: HeartbeatConfig) {
...
function usesCodexHarness(params: {
  ...
  const runtimeId =
    ...
    normalizeHeartbeatRuntimeId(process.env.ASTROCLAW_AGENT_RUNTIME) ||
...
function resolveHeartbeatAckMaxChars(cfg: AstroclawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: AstroclawConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: AstroclawConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: AstroclawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
