...
import { readRootJsonObjectSync } from "@astroclaw/fs-safe/json";
...
export function installedPackageNeedsAstroclawPeerLinkRepair(dir: string): boolean {
  ...
  try {
    fsSync.statSync(path.join(dir, "node_modules", "astroclaw"));
    return false;
  } catch (error) {
