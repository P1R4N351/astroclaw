 ...
 * returning control to the caller (typically `triggerAstroclawRestart` →
...
function parsePidsFromLsofOutput(stdout: string, spawnTimeoutMs: number): number[] {
  ...
  for (const entry of parseLsofEntries(stdout)) {
    ...
    if (entry.cmd && normalizeLowercaseStringOrEmpty(entry.cmd).includes("astroclaw")) {
      pids.push(entry.pid);
      continue;
    }
...
/**
 * Windows: find listening PIDs on the port, then verify each is an astroclaw
 ...
 * Returns only PIDs that belong to astroclaw gateway processes (not the current process).
 */
export function findGatewayPidsOnPortSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only astroclaw gateway processes.
