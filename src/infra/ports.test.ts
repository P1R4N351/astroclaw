...
describe("ports helpers", () => {
  ...
  it("prints an Astroclaw-specific hint when port details look like another Astroclaw instance", async () => {
    const runtime = {
      error: vi.fn(),
      log: vi.fn(),
      exit: vi.fn() as unknown as (code: number) => never,
    ...
    await handlePortError(
      new PortInUseError(18789, "node dist/index.js astroclaw gateway"),
      18789,
      "gateway start",
      runtime,
    ).catch(() => {});
    ...
    const messages = runtime.error.mock.calls.map((call) => stripAnsi(String(call[0] ?? "")));
    expect(messages.join("\n")).toContain("another Astroclaw instance is already running");
  });
});
...
describeUnix("inspectPortUsage", () => {
  ...
  it("falls back to ss when lsof is unavailable", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/astroclaw/dist/index.js gateway --port 18789\n",
            stderr: "",
            code: 0,
          };
        }
    ...
    try {
      ...
      expect(result.listeners[0]?.commandLine).toContain("astroclaw");
...
describe("inspectPortUsage on Windows", () => {
  it("uses PowerShell process command lines to classify Astroclaw listeners", async () => {
    setPlatform("win32");
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      const [command] = argv;
      if (command === "netstat") {
      ...
      if (command === "powershell") {
        return {
          stdout:
            '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\me\\AppData\\Roaming\\npm\\node_modules\\astroclaw\\dist\\index.js gateway run\r\n',
          stderr: "",
          code: 0,
        };
      }
  ...
  it("falls back to wmic when PowerShell cannot read the command line", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "wmic") {
        return {
          stdout: "CommandLine=node.exe C:\\astroclaw\\dist\\index.js gateway run\r\n",
          stderr: "",
          code: 0,
        };
      }
