...
const EXTENSION_MANIFEST = bundledPluginFile("demo", "astroclaw.plugin.json");
...
const DIST_RUNTIME_EXTENSION_MANIFEST = "dist-runtime/extensions/demo/astroclaw.plugin.json";
...
const DIST_ASTROCLAW_ALIAS_PACKAGE = "dist/extensions/node_modules/astroclaw/package.json";
const DIST_ASTROCLAW_ALIAS_PLUGIN_SDK_INDEX =
  "dist/extensions/node_modules/astroclaw/plugin-sdk/index.js";
...
const DIST_EXTENSION_MANIFEST = bundledDistPluginFile("demo", "astroclaw.plugin.json");
...
const BASE_PROJECT_FILES = {
  [ROOT_TSCONFIG]: "{}\n",
  [ROOT_PACKAGE]: '{"name":"astroclaw-test"}\n',
...
async function writeRuntimePostBuildScaffold(tmp: string): Promise<void> {
  ...
  await writeProjectFiles(tmp, {
    ...
    [DIST_ASTROCLAW_ALIAS_PACKAGE]:
      '{"name":"astroclaw","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js"}}\n',
  });
...
function statusCommandSpawn() {
  return [process.execPath, "astroclaw.mjs", "status"];
}

function gatewayCallStatusCommandSpawn() {
  return [process.execPath, "astroclaw.mjs", "gateway", "call", "status", "--json"];
}
...
describe("run-node script", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
        ...
        expect(nodeCalls).toEqual([
          ...
          [process.execPath, "astroclaw.mjs", "--version"],
        ]);
      });
    },
  );
    ...
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[], options?: unknown) => {
        ...
        return createPipedExitedProcess({
          stdout: args[0] === "astroclaw.mjs" ? "child stdout\n" : "",
          stderr: args[0] === "astroclaw.mjs" ? "child stderr\n" : "",
        });
      };
      const mutedStream = {
        write: () => true,
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          ASTROCLAW_FORCE_BUILD: "1",
          ASTROCLAW_RUNNER_LOG: "1",
          ASTROCLAW_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr: mutedStream,
        stdout: mutedStream,
      ...
      await expect(fs.readFile(outputPath, "utf-8")).resolves.toContain("[astroclaw]");
      expect(spawnCalls.at(-1)?.args).toEqual(["astroclaw.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.ASTROCLAW_RUN_NODE_OUTPUT_LOG).toBe(outputPath);
    ...
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[]) =>
        createPipedExitedProcess({
          stderr: args[0] === "astroclaw.mjs" ? childStderr : "",
        });
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          ASTROCLAW_RUNNER_LOG: "0",
          ASTROCLAW_RUN_NODE_FILTER_SYNC_IO_STDERR: "1",
          ASTROCLAW_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr,
  ...
  it("adds Node CPU profiling flags to the launched Astroclaw child when requested", async () => {
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          ASTROCLAW_RUNNER_LOG: "0",
          ASTROCLAW_RUN_NODE_CPU_PROF_DIR: ".artifacts/profiles",
        },
        spawn,
        spawnSync,
        execPath: process.execPath,
      ...
      expect(childArgs[2]).toMatch(
        /^--cpu-prof-name=astroclaw-status-4242-\d{4}-\d{2}-\d{2}T.*\.cpuprofile$/,
      );
      expect(childArgs.slice(3)).toEqual(["astroclaw.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.ASTROCLAW_RUN_NODE_CPU_PROF_DIR).toBe(profileDir);
      expect(fsSync.existsSync(profileDir)).toBe(true);
    });
  });

  it("adds Node sync I/O tracing flag to the launched Astroclaw child when requested", async () => {
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
      ...
      expect(exitCode).toBe(0);
      expect(spawnCalls.at(-1)).toEqual(["--trace-sync-io", "astroclaw.mjs", "gateway", "--force"]);
    });
  });
  ...
  it("does not mutate Matrix QA args when no generic output log is requested", async () => {
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      ...
      expect(exitCode).toBe(0);
      const childArgs = spawnCalls.at(-1)?.args ?? [];
      expect(childArgs).toEqual(["astroclaw.mjs", "qa", "matrix"]);
      expect(spawnCalls.at(-1)?.env.ASTROCLAW_RUN_NODE_OUTPUT_LOG).toBeUndefined();
    });
  });
  ...
  it("passes the synthesized private QA env into runtime postbuild staging", async () => {
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      ...
      expect(postBuildParams?.env?.ASTROCLAW_BUILD_PRIVATE_QA).toBe("1");
      expect(postBuildParams?.env?.ASTROCLAW_ENABLE_PRIVATE_QA_CLI).toBe("1");
      expect(postBuildParams?.env?.ASTROCLAW_DISABLE_BUNDLED_PLUGINS).toBe("0");
    });
  });
  ...
  it("preserves an explicit bundled plugin disable flag for QA runs", async () => {
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      ...
      const exitCode = await runQaCommand({
        ...
        spawnSync,
        runRuntimePostBuild,
        env: { ASTROCLAW_DISABLE_BUNDLED_PLUGINS: "1" },
      });
      ...
      const postBuildParams = firstMockCall(runRuntimePostBuild)?.[0] as
        | { cwd?: string; env?: Record<string, string | undefined> }
        | undefined;
      expect(postBuildParams?.env?.ASTROCLAW_DISABLE_BUNDLED_PLUGINS).toBe("1");
    });
  });
    ...
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      ...
      const requirement = resolveBuildRequirement(
        createBuildRequirementDeps(tmp, {
          env: { ASTROCLAW_BUILD_PRIVATE_QA: "1" },
          gitHead: "abc123\n",
          gitStatus: "",
        }),
      );
  ...
  it("reruns runtime postbuild in watch mode when required outputs are missing with no runtime stamp", async () => {
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      ...
      await fs.rm(resolvePath(tmp, DIST_ASTROCLAW_ALIAS_PACKAGE));
  ...
  it("reruns runtime postbuild for dirty extension package metadata in watch mode", async () => {
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [EXTENSION_PACKAGE]: '{"astroclaw":{"extensions":["./index.ts"]}}\n',
    ...
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      ...
      const runs = Promise.all([
        runStatusCommand({
          ...
          spawnSync,
          env: {
            ASTROCLAW_RUN_NODE_BUILD_LOCK_POLL_MS: "1",
          },
          runRuntimePostBuild,
        }),
        runStatusCommand({
  ...
  it("forwards wrapper SIGTERM to the active astroclaw child and returns 143", async () => {
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
      ...
      expect(spawnCall?.[1]).toEqual(["astroclaw.mjs", "status"]);
    ...
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [EXTENSION_INDEX]: "export default {};\n",
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"name":"demo","astroclaw":{"extensions":["./index.ts"]}}\n',
          [ROOT_TSDOWN]: "export default {};\n",
          [DIST_EXTENSION_INDEX]: "export default {};\n",
          [DIST_EXTENSION_PACKAGE]: '{"name":"demo","astroclaw":{"extensions":["./stale.js"]}}\n',
        },
    ...
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"astroclaw":{"extensions":["./src/index.ts","./src/extra.ts"]}}\n',
    ...
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_SRC]: "export default {};\n",
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"astroclaw":{"extensions":["./src/index.ts"]}}\n',
        },
        buildPaths: [
          ROOT_SRC,
          EXTENSION_SRC,
    ...
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_EXTENSION_PACKAGE]: '{"astroclaw":{"extensions":["./src/index.js"]}}\n',
          [DIST_EXTENSION_RUNTIME_SRC]: "export default {};\n",
          [DIST_RUNTIME_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_RUNTIME_EXTENSION_PACKAGE]: '{"astroclaw":{"extensions":["./src/index.js"]}}\n',
          [RUNTIME_POSTBUILD_STAMP]: '{"head":"abc123"}\n',
        },
        buildPaths: [
          ROOT_SRC,
    ...
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_EXTENSION_INDEX]: "export default {};\n",
          [DIST_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_EXTENSION_PACKAGE]: '{"astroclaw":{"extensions":["./index.js"]}}\n',
          [DIST_RUNTIME_EXTENSION_INDEX]: "export default {};\n",
          [DIST_RUNTIME_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_RUNTIME_EXTENSION_PACKAGE]: '{"astroclaw":{"extensions":["./index.js"]}}\n',
          [RUNTIME_POSTBUILD_STAMP]: '{"head":"abc123"}\n',
        },
        buildPaths: [
          ROOT_SRC,
  ...
  it("does not require Astroclaw SDK alias outputs when dist extensions are absent", async () => {
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
  ...
  it("reports missing Astroclaw SDK alias outputs when runtime stamps match HEAD", async () => {
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          ...
          [DIST_ASTROCLAW_ALIAS_PACKAGE]:
            '{"name":"astroclaw","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js"}}\n',
          [DIST_ASTROCLAW_ALIAS_PLUGIN_SDK_INDEX]:
      ...
      await fs.rm(resolvePath(tmp, DIST_ASTROCLAW_ALIAS_PLUGIN_SDK_INDEX));
    ...
    await withTempDir({ prefix: "astroclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [DIFFS_PACKAGE]:
            '{"astroclaw":{"build":{"staticAssets":[{"source":"./assets/viewer-runtime.js","output":"assets/viewer-runtime.js"}]}}}\n',
