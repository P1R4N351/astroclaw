...
async function createStagedNpmInstall(
  ...
  const prefix = await fs.mkdtemp(path.join(targetLayout.globalRoot, ".astroclaw-update-stage-"));
...
async function replaceNpmBinShims(params: {
  ...
  const names = new Set([params.packageName, "astroclaw"]);
  ...
  const backup: NpmBinShimBackup = {
    backupDir: await fs.mkdtemp(
      path.join(params.targetLayout.globalRoot, ".astroclaw-shim-backup-"),
    ),
...
async function swapStagedNpmInstall(params: {
  ...
  const backupRoot = path.join(targetLayout.globalRoot, `.astroclaw-${process.pid}-${Date.now()}`);
