  ...
  try {
    return await new Promise<ProbeApnsHttp2ReachabilityViaProxyResult>((resolve, reject) => {
      ...
      const request = session.request({
        ...
        authorization: "bearer intentionally.invalid.astroclaw.proxy.validation",
        "apns-topic": "ai.astroclaw.ios",
      ...
      request.end(JSON.stringify({ aps: { alert: "Astroclaw APNs proxy validation" } }));
    });
  } finally {
