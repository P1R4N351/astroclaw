...
const STAGED_NPM_PROJECT_CONFIG_PREFIX = ".astroclaw-install-hidden-npmrc-";
...
export async function installPackageDir(params: {
  ...
  try {
    ...
    stageDir = await fs.mkdtemp(path.join(installBaseRealPath, ".astroclaw-install-stage-"));
  ...
  if (params.mode === "update" && (await pathExists(canonicalTargetDir))) {
    const backupRoot = path.join(installBaseRealPath, ".astroclaw-install-backups");
