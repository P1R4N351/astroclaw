...
import { POSIX_ASTROCLAW_TMP_DIR, resolvePreferredAstroclawTmpDir } from "./tmp-astroclaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredAstroclawTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `astroclaw-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredAstroclawTmpDir({
    ...
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_ASTROCLAW_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredAstroclawTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredAstroclawTmpDir", () => {
  it("prefers /tmp/astroclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_ASTROCLAW_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/astroclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_ASTROCLAW_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(tmpdir).not.toHaveBeenCalled();
  ...
  it.each([
    {
      name: "falls back to os.tmpdir()/astroclaw when /tmp/astroclaw is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/astroclaw when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/astroclaw exists but is not writable",
    ...
    {
      name: "falls back when /tmp/astroclaw is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/astroclaw is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/astroclaw is group/other writable",
  ...
  it("repairs existing /tmp/astroclaw permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_ASTROCLAW_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_ASTROCLAW_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_ASTROCLAW_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(
      `[astroclaw] tightened permissions on temp dir: ${POSIX_ASTROCLAW_TMP_DIR}`,
    );
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/astroclaw after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_ASTROCLAW_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Astroclaw temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "astroclaw");
  ...
  it("repairs existing fallback directory when permissions are too broad", () => {
    ...
    expect(chmodSync).toHaveBeenCalledWith(fallbackPath, 0o700);
    expect(warn).toHaveBeenCalledWith(
      `[astroclaw] tightened permissions on temp dir: ${fallbackPath}`,
    );
  });

  it("uses /tmp/astroclaw when another process tightened permissions before repair", () => {
  ...
  it("uses /tmp/astroclaw when chmod loses a concurrent repair race", () => {
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_ASTROCLAW_TMP_DIR && mode === 0o700) {
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredAstroclawTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_ASTROCLAW_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback Astroclaw temp dir/);
  });
  ...
  it("skips the POSIX preferred path on Windows even when /tmp is accessible (#60713)", () => {
    ...
    // create it), the previous code path returned `/tmp/astroclaw` and routed
    // log files / TTS temp files there instead of `%TEMP%\astroclaw`. The
    // platform: "win32" branch must skip the POSIX path entirely.
    const winFallback = path.win32.join("C:\\Users\\u\\AppData\\Local\\Temp", "astroclaw-501");
    const accessSync = vi.fn();
    const lstatSync = vi.fn((target: string) => {
      if (target === POSIX_ASTROCLAW_TMP_DIR || target === winFallback) {
        return secureDirStat();
      }
      throw nodeErrorWithCode("ENOENT");
    });
    ...
    const result = resolvePreferredAstroclawTmpDir({
      platform: "win32",
      accessSync,
      lstatSync,
      mkdirSync,
    ...
    expect(result).not.toBe(POSIX_ASTROCLAW_TMP_DIR);
    expect(tmpdir).toHaveBeenCalled();
  });
  ...
  it("still uses the POSIX preferred path on non-Windows platforms when available", () => {
    ...
    expect(result).toBe(POSIX_ASTROCLAW_TMP_DIR);
  });
});
