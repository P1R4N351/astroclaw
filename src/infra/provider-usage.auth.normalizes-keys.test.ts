...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "astroclaw-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".astroclaw");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".astroclaw", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      ASTROCLAW_STATE_DIR: path.join(home, ".astroclaw"),
      ...env,
    };
  ...
  async function writeConfig(home: string, config: Record<string, unknown>) {
    ...
    await fs.writeFile(
      path.join(stateDir, "astroclaw.json"),
