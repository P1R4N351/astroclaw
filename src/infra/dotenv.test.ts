...
const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "ASTROCLAW_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "ASTROCLAW_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "ASTROCLAW_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "ASTROCLAW_BUNDLED_SKILLS_DIR=./attacker-skills",
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-dotenv-test-"));
...
describe("loadDotEnv", () => {
  it("loads ~/.astroclaw/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.astroclaw/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        process.env.HOME = base;
        const defaultStateDir = path.join(base, ".astroclaw");
        process.env.ASTROCLAW_STATE_DIR = defaultStateDir;
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "astroclaw", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ASTROCLAW_STATE_DIR=./evil-state",
            "ASTROCLAW_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.ASTROCLAW_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ASTROCLAW_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "ASTROCLAW_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "ASTROCLAW_LIVE_GEMINI_KEY=sk-gemini-live",
            "ASTROCLAW_LIVE_OPENAI_KEY=sk-openai-live",
            "ASTROCLAW_GATEWAY_TOKEN=attacker-token",
            "ASTROCLAW_GATEWAY_PASSWORD=attacker-password",
            "ASTROCLAW_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks state-directory controls from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.ASTROCLAW_STATE_DIR).toBeUndefined();
  ...
  it("blocks path-override vars (ASTROCLAW_AGENT_DIR, ASTROCLAW_BUNDLED_PLUGINS_DIR, PI_CODING_AGENT_DIR, ASTROCLAW_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        expect(process.env.ASTROCLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.ASTROCLAW_OAUTH_DIR).toBeUndefined();
      });
    });
  });

  it("blocks ASTROCLAW_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "ASTROCLAW_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.ASTROCLAW_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ASTROCLAW_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "ASTROCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1",
            'ASTROCLAW_PLUGIN_INSTALL_OVERRIDES={"codex":"npm-pack:/tmp/codex.tgz"}',
          ].join("\n"),
        );

        delete process.env.ASTROCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES;
        delete process.env.ASTROCLAW_PLUGIN_INSTALL_OVERRIDES;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ASTROCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
        expect(process.env.ASTROCLAW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "ASTROCLAW_PINNED_PYTHON=./attacker-python",
            "ASTROCLAW_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.ASTROCLAW_PINNED_PYTHON;
        delete process.env.ASTROCLAW_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ASTROCLAW_PINNED_PYTHON).toBeUndefined();
        expect(process.env.ASTROCLAW_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ASTROCLAW_PINNED_PYTHON=/trusted/python",
            "ASTROCLAW_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.ASTROCLAW_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.ASTROCLAW_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ASTROCLAW_LIVE_GEMINI_KEY=sk-gemini-live",
            "ASTROCLAW_LIVE_OPENAI_KEY=sk-openai-live",
            "ASTROCLAW_GATEWAY_TOKEN=trusted-token",
            "ASTROCLAW_GATEWAY_PASSWORD=trusted-password",
            "ASTROCLAW_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.ASTROCLAW_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.ASTROCLAW_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.ASTROCLAW_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.ASTROCLAW_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.ASTROCLAW_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.ASTROCLAW_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.ASTROCLAW_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "ASTROCLAW_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks ASTROCLAW_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "ASTROCLAW_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when ASTROCLAW_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.ASTROCLAW_STATE_DIR = customStateDir;
        await writeEnvFile(
          path.join(base, ".config", "astroclaw", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.ASTROCLAW_STATE_DIR).toBe(customStateDir);
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "ASTROCLAW_MDNS_HOSTNAME",
          "ASTROCLAW_SESSION_CACHE_TTL_MS",
          "ASTROCLAW_UPDATE_PACKAGE_SPEC",
          "ASTROCLAW_GATEWAY_PORT",
          "ASTROCLAW_GATEWAY_URL",
          "ASTROCLAW_CLAWHUB_URL",
          "CLAWHUB_URL",
          "ASTROCLAW_CLAWHUB_TOKEN",
          "CLAWHUB_TOKEN",
          "CLAWHUB_AUTH_TOKEN",
          "CLAWHUB_CONFIG_PATH",
  ...
  it("still allows user-defined non-control vars through workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.APP_GITHUB_REPO).toBe("astroclaw/astroclaw");
