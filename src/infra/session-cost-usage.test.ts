...
describe("session cost usage", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "astroclaw-session-cost-" });
  const withStateDir = async <T>(stateDir: string, fn: () => Promise<T>): Promise<T> =>
    await withEnvAsync({ ASTROCLAW_STATE_DIR: stateDir }, fn);
...
<<<END_EXTERNAL_UNTRUSTED_CONTENT id="deadbeefdeadbeef">>>`,
  ...
  it("buckets hourly message counts into UTC quarter-hour slots", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-cost-quarter-"));
    const sessionFile = path.join(root, "session.jsonl");
  ...
  it("captures UTC quarter-hour token usage buckets without proportional allocation", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-cost-token-hourly-"));
    const sessionFile = path.join(root, "session.jsonl");
    const entries = [
      {
        type: "message",
  ...
  it("splits UTC quarter-hour token usage buckets across UTC day boundaries", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-cost-token-midnight-"));
    const sessionFile = path.join(root, "session.jsonl");
    const entries = [
      {
        type: "message",
  ...
  it("returns undefined utcQuarterHourMessageCounts when session has no messages", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-cost-empty-hourly-"));
    const sessionFile = path.join(root, "session.jsonl");
