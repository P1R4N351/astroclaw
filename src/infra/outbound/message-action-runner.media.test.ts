...
describe("runMessageAction media behavior", () => {
  ...
  describe("reply hydration", () => {
    ...
    // params as send. Before astroclaw#79864 the runner only hydrated
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Astroclaw tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideAstroclaw = path.join(os.tmpdir(), "outside-astroclaw", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
