...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.ASTROCLAW_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-current-bindings-"));
    process.env.ASTROCLAW_STATE_DIR = testStateDir;
