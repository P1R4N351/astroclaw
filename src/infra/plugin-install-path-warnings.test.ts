...
const MATRIX_REPO_INSTALL_COMMAND = `astroclaw plugins install ${repoInstallSpec("matrix")}`;
...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    const issue = await detectPluginInstallPathIssue({
      ...
      install: {
        source: "path",
        sourcePath: "/tmp/astroclaw-matrix-missing",
        installPath: "/tmp/astroclaw-matrix-missing",
      },
    });
    ...
    expect(issue).toEqual({
      kind: "missing-path",
      pluginId: "matrix",
      path: "/tmp/astroclaw-matrix-missing",
    });
    expect(
      formatPluginInstallPathIssue({
        issue: issue!,
      ...
      "Matrix is installed from a custom path that no longer exists: /tmp/astroclaw-matrix-missing",
      'Reinstall with "astroclaw plugins install @astroclaw/matrix".',
  ...
  it("uses the second candidate path when the first one is stale", async () => {
    const { issue, pluginPath } = await detectMatrixCustomPathIssue("/tmp/astroclaw-matrix-missing");
    expect(issue).toEqual({
      kind: "custom-path",
      pluginId: "matrix",
      path: pluginPath,
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<astroclaw plugins install @astroclaw/matrix>" when you want to return to the standard Matrix plugin.',
