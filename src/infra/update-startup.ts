...
import { resolveAstroclawPackageRoot } from "./astroclaw-root.js";
...
function resolveAutoUpdatePolicy(cfg: AstroclawConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: AstroclawConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.ASTROCLAW_NO_AUTO_UPDATE);
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("astroclaw update")}`,
      );
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by ASTROCLAW_NO_AUTO_UPDATE", {
