...
const GATEWAY_DEVICE_ID_HEADER = "x-astroclaw-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-astroclaw-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-astroclaw-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.ASTROCLAW_APNS_RELAY_BASE_URL);
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.ASTROCLAW_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require ASTROCLAW_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.ASTROCLAW_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
