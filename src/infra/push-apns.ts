...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open Astroclaw to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "astroclaw.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.ASTROCLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.ASTROCLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
  ...
  const inlineKeyRaw =
    normalizeNonEmptyString(env.ASTROCLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.ASTROCLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.ASTROCLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set ASTROCLAW_APNS_PRIVATE_KEY_P8 or ASTROCLAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading ASTROCLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
