...
vi.mock("../channels/plugins/bundled.js", () => {
  ...
  function resolveChatAppAccountId(cfg: AstroclawConfig): string {
  ...
  return {
    ...
    listBundledChannelLegacyStateMigrationDetectors: vi.fn(() => [
      ...
      ({ cfg, env }: { cfg: AstroclawConfig; env: NodeJS.ProcessEnv }) => {
        const root = env.ASTROCLAW_STATE_DIR;
        if (!root) {
          return [];
        }
...
const createTempDir = () => tempDirs.make("astroclaw-state-migrations-test-");
