...
import { compareAstroclawReleaseVersions } from "./npm-registry-spec.js";
...
export async function fetchNpmPackageTargetStatus(params: {
  ...
  try {
    const res = await fetchWithTimeout(
      `https://registry.npmjs.org/astroclaw/${encodeURIComponent(target)}`,
      {},
...
export function compareSemverStrings(a: string | null, b: string | null): number | null {
  if (a && b) {
    const astroClawReleaseCmp = compareAstroclawReleaseVersions(a, b);
    if (astroClawReleaseCmp != null) {
      return astroClawReleaseCmp;
    }
  }
