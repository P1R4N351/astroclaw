...
// Keep aligned with @astroclaw/plugin-package-contract ExternalPluginCompatibility.
...
export type ClawHubPackageReadiness = {
  ready?: boolean | null;
  readyForAstroclaw?: boolean | null;
...
function normalizeBaseUrl(baseUrl?: string): string {
  const envValue =
    normalizeOptionalString(process.env.ASTROCLAW_CLAWHUB_URL) ||
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    normalizeOptionalString(process.env.ASTROCLAW_CLAWHUB_CONFIG_PATH) ||
...
export async function resolveClawHubAuthToken(): Promise<string | undefined> {
  const envToken =
    normalizeOptionalString(process.env.ASTROCLAW_CLAWHUB_TOKEN) ||
...
const ASTROCLAW_CALVER_STABLE_CORRECTION_PATTERN = /^[vV]?(\d{4}\.\d{1,2}\.\d{1,2})-\d+$/;

function normalizeCalVerCorrectionForPluginApi(pluginApiVersion: string): string {
  const match = ASTROCLAW_CALVER_STABLE_CORRECTION_PATTERN.exec(pluginApiVersion.trim());
...
export async function downloadClawHubPackageArchive(params: {
  ...
  if (params.artifact === "clawpack") {
    ...
    const target = await createTempDownloadTarget({
      prefix: "astroclaw-clawhub-clawpack",
  ...
  const sha256Hex = formatSha256Hex(bytes);
  const target = await createTempDownloadTarget({
    prefix: "astroclaw-clawhub-package",
...
export async function downloadClawHubSkillArchive(params: {
  ...
  const sha256Hex = formatSha256Hex(bytes);
  const target = await createTempDownloadTarget({
    prefix: "astroclaw-clawhub-skill",
