...
describe("isGatewayArgv", () => {
  ...
  it("matches known entrypoints across slash and case variants", () => {
    expect(isGatewayArgv(["NODE", "C:\\Astroclaw\\DIST\\ENTRY.JS", "gateway"])).toBe(true);
    expect(isGatewayArgv(["bun", "/srv/astroclaw/scripts/run-node.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["node", "/srv/astroclaw/astroclaw.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/astroclaw/src/entry.ts", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/astroclaw/src/index.ts", "gateway"])).toBe(true);
  });

  it("matches the astroclaw executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\astroclaw.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/astroclaw-gateway", "gateway"])).toBe(false);
    expect(
      isGatewayArgv(["/usr/local/bin/astroclaw-gateway", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
    expect(
      isGatewayArgv(["C:\\bin\\astroclaw-gateway.EXE", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
  ...
  it("rejects unknown gateway argv even when the token is present", () => {
    expect(isGatewayArgv(["node", "/srv/astroclaw/custom.js", "gateway"])).toBe(false);
