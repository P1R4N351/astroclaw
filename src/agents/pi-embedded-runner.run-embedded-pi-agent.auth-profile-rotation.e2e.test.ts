...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureAstroclawModelsJson: vi.fn(async () => ({ wrote: false })),
    };
  });
};
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): AstroclawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): AstroclawConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  try {
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `astroclaw-auth-rotation-${Date.now()}.log`),
    });
