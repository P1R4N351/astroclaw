...
async function prepareSubagentSessionContext(params: {
  ...
  try {
    const forked = (await updateSubagentSessionStore(childTarget.storePath, async (store) => {
      ...
      if (!fork) {
        throw new Error(
          'context="fork" requested but Astroclaw could not fork the requester transcript.',
        );
      }
      pruneLegacyStoreKeys({
        store,
    ...
    if (params.contextMode === "fork") {
      if (!parentEntry || !forked) {
        ...
        return {
          status: "error",
          error: 'context="fork" requested but Astroclaw could not prepare forked context.',
        };
      }
      return {
        status: "ok",
