...
function policyDeniesTool(policy: { deny?: string[] } | undefined, toolName: string): boolean {
  return (
    ...
    listIncludesTool(policy?.deny, "group:astroclaw")
  );
}

function hasExplicitBrowserIntent(cfg: AstroclawConfig): boolean {
...
export function resolveEffectiveToolInventory(
  ...
  const effectiveTools = createAstroclawCodingTools({
    agentId,
