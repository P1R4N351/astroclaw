...
import {
  ...
  createAstroclawTransportStreamFnForModel,
...
describe("openai transport stream", () => {
  ...
  it("summarizes model payload tools with full names when requested", () => {
    const previous = process.env.ASTROCLAW_DEBUG_MODEL_PAYLOAD;
    process.env.ASTROCLAW_DEBUG_MODEL_PAYLOAD = "tools";
    try {
      expect(
      ...
      if (previous === undefined) {
        delete process.env.ASTROCLAW_DEBUG_MODEL_PAYLOAD;
      } else {
        process.env.ASTROCLAW_DEBUG_MODEL_PAYLOAD = previous;
      }
    }
  });
  ...
  it("redacts full model payload debug summaries", () => {
    const previous = process.env.ASTROCLAW_DEBUG_MODEL_PAYLOAD;
    process.env.ASTROCLAW_DEBUG_MODEL_PAYLOAD = "full-redacted";
    try {
  ...
  it("enforces the code mode responses tool surface before requests leave Astroclaw", () => {
    const payload = {
      tools: [
        { type: "function", name: "exec" },
  ...
  it("adds Astroclaw attribution to native OpenAI transport headers and protects it from pi", () => {
  ...
  it("adds Astroclaw attribution to native OpenAI Codex transport headers", () => {
    vi.stubEnv("ASTROCLAW_VERSION", "2026.3.22");
    const headers = __testing.buildOpenAIClientHeaders(
      {
        id: "gpt-5.4-codex",
  ...
  it("prepares a custom simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("astroclaw-openai-responses-transport");
    expectRecordFields(prepared, {
      api: "astroclaw-openai-responses-transport",
      provider: "openai",
      id: "gpt-5.4",
    });
  ...
  it("prepares an Anthropic simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("astroclaw-anthropic-messages-transport");
    expectRecordFields(prepared, {
      api: "astroclaw-anthropic-messages-transport",
      provider: "anthropic",
      id: "claude-sonnet-4-6",
    });
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
  ...
  it("reports the Google simple-completion api alias without loading provider runtime", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe(
      "astroclaw-google-generative-ai-transport",
    );
  });
  ...
  it("uses top-level instructions for Codex responses and preserves prompt cache identity", () => {
    const params = buildOpenAIResponsesParams(
      ...
      {
        astroclaw_session_id: "session-123",
        astroclaw_turn_id: "turn-123",
      },
  ...
  it("sanitizes Codex responses params after payload hooks mutate them without stripping cache identity", () => {
    const payload = {
      ...
      metadata: { astroclaw_session_id: "session-123" },
  ...
  it("adds native OpenAI turn metadata on direct Responses routes", () => {
    const params = buildOpenAIResponsesParams(
      ...
      { sessionId: "session-123" } as never,
      {
        astroclaw_session_id: "session-123",
        astroclaw_turn_id: "turn-123",
        astroclaw_turn_attempt: "1",
        astroclaw_transport: "stream",
      },
