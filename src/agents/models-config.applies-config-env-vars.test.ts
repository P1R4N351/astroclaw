...
const TEST_ENV_VAR = "ASTROCLAW_MODELS_CONFIG_TEST_ENV";
...
async function resolveProvidersForConfigEnvTest(params: {
  ...
  return await resolveProvidersForModelsJsonWithDeps(
    {
      cfg: params.cfg,
      agentDir: "/tmp/astroclaw-models-config-env-vars-test",
      env,
    },
    {
...
function createConfigEnvVarsConfig(): AstroclawConfig {
  return {
    models: { providers: {} },
    env: {
      vars: {
...
async function resolveProvidersAndCaptureDiscoveryEnv(cfg: AstroclawConfig) {
...
describe("models-config", () => {
  ...
  it("threads workspace scope into implicit provider discovery", async () => {
    ...
    expect(observedWorkspaceDir).toBe("/tmp/astroclaw-workspace");
  });
  ...
  it("threads plugin metadata snapshots through models.json planning", async () => {
    ...
    await planAstroclawModelsJsonWithDeps(
      {
        cfg: { models: { providers: {} } },
        agentDir: "/tmp/astroclaw-models-config-env-vars-test",
        env: {},
