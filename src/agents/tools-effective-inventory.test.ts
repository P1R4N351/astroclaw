...
import type { createAstroclawCodingTools } from "./pi-tools.js";
...
const effectiveInventoryState = vi.hoisted(() => ({
  ...
  createToolsMock: vi.fn<typeof createAstroclawCodingTools>(
    (_options) =>
      [
        mockTool({ name: "exec", label: "Exec", description: "Run shell commands" }),
        mockTool({ name: "docs_lookup", label: "Docs Lookup", description: "Search docs" }),
...
vi.mock("./pi-tools.js", () => ({
  createAstroclawCodingTools: (options?: Parameters<typeof createAstroclawCodingTools>[0]) =>
...
async function loadHarness(options?: {
  ...
  effectiveInventoryState.createToolsMock =
    ...
    vi.fn<typeof createAstroclawCodingTools>((_options) => effectiveInventoryState.tools);
  return {
...
describe("resolveEffectiveToolInventory", () => {
  ...
  beforeEach(() => {
    ...
    effectiveInventoryState.createToolsMock = vi.fn<typeof createAstroclawCodingTools>(
  ...
  it("passes resolved model compat into effective tool creation", async () => {
    const createToolsMock = vi.fn<typeof createAstroclawCodingTools>(() => [
      mockTool({ name: "exec", label: "Exec", description: "Run shell commands" }),
    ]);
    const { resolveEffectiveToolInventory } = await loadHarness({
      createToolsMock,
