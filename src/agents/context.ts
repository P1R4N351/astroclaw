...
function isLikelyAstroclawCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = normalizeLowercaseStringOrEmpty(path.basename(argv[1] ?? ""));
  return (
    entryBasename === "astroclaw" ||
    entryBasename === "astroclaw.mjs" ||
...
export function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Astroclaw CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureAstroclawModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyAstroclawCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): AstroclawConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureAstroclawModelsJson(cfg);
    } catch {
