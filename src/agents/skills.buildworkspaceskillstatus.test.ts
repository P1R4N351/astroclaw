...
async function createTempWorkspaceDir() {
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-skill-status-"));
...
function makeEntry(params: {
  ...
  return {
    skill: createFixtureSkill({
      ...
      filePath,
      baseDir,
      source: params.source ?? "astroclaw-workspace",
    }),
    frontmatter: {},
    metadata: {
...
describe("buildWorkspaceSkillStatus", () => {
  ...
  it("does not mark an overridden workspace skill as bundled by bundled name alone", async () => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-bundled-"));
    ...
    await withEnvAsync({ ASTROCLAW_BUNDLED_SKILLS_DIR: bundledDir }, async () => {
      const report = buildWorkspaceSkillStatus("/tmp/ws", {
        entries: [
          makeEntry({
            name: "peekaboo",
      ...
      expect(skill.source).toBe("astroclaw-workspace");
