...
describe("processGatewayAllowlist", () => {
  ...
  it("requires approval for security audit suppression edits unless yolo mode is active", async () => {
    ...
    const result = await runGatewayAllowlist({
      command: "astroclaw config set security.audit.suppressions '[]'",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("does not require suppression edit approval for read-only suppression inspection", async () => {
    evaluateShellAllowlistMock.mockReturnValue({
      ...
      allowlistSatisfied: true,
      segments: [
        { resolution: null, argv: ["astroclaw", "config", "get", "security.audit.suppressions"] },
      ],
      segmentAllowlistEntries: [],
    });
    resolveExecHostApprovalContextMock.mockReturnValue({
    ...
    await runGatewayAllowlist({
      command: "astroclaw config get security.audit.suppressions",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("does not require suppression edit approval for profile-scoped read-only inspection", async () => {
    evaluateShellAllowlistMock.mockReturnValue({
      ...
      segments: [
        {
          resolution: null,
          argv: ["astroclaw", "--profile", "rescue", "config", "get", "security.audit.suppressions"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    await runGatewayAllowlist({
      command: "astroclaw --profile rescue config get security.audit.suppressions",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("requires suppression edit approval when a mutating segment follows read-only inspection", async () => {
    evaluateShellAllowlistMock.mockReturnValue({
      ...
      segments: [
        ...
        {
          resolution: null,
          argv: ["astroclaw", "config", "set", "security.audit.suppressions", "[]"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    const result = await runGatewayAllowlist({
      command:
        "astroclaw config get security.audit.suppressions; astroclaw config set security.audit.suppressions '[]'",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("requires suppression edit approval when a heredoc patch follows read-only inspection", async () => {
    evaluateShellAllowlistMock.mockReturnValue({
      ...
      segments: [
        {
          resolution: null,
          argv: ["astroclaw", "config", "get", "security.audit.suppressions"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    analyzeShellCommandMock.mockReturnValueOnce({
      ...
      segments: [
        {
          raw: "astroclaw config get security.audit.suppressions",
          resolution: null,
          argv: ["astroclaw", "config", "get", "security.audit.suppressions"],
        },
        {
          raw: "astroclaw config patch --stdin <<'EOF'",
          resolution: null,
          argv: ["astroclaw", "config", "patch", "--stdin"],
        },
      ],
    });
    resolveExecHostApprovalContextMock.mockReturnValue({
    ...
    const result = await runGatewayAllowlist({
      command: `astroclaw config get security.audit.suppressions; astroclaw config patch --stdin <<'EOF'
  ...
  it("formats diagnostics approvals as direct pasteable followups", async () => {
    ...
    const outcome = {
      ...
      aggregated: JSON.stringify({
        path: "/tmp/astroclaw-diagnostics.zip",
        bytes: 1234,
        manifest: {
          generatedAt: "2026-04-28T20:58:29.311Z",
          astroclawVersion: "2026.4.27",
          contents: [
    ...
    expect(followupText).toContain("Path: /tmp/astroclaw-diagnostics.zip");
