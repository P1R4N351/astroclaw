...
import { resolveAstroclawMetadata, resolveSkillInvocationPolicy } from "./skills/frontmatter.js";
...
metadata: {"astroclaw":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
const workspaceSuite = createFixtureSuite("astroclaw-skills-install-");
...
async function withWorkspaceCase(
  ...
  const envSnapshot = captureEnv(["ASTROCLAW_STATE_DIR"]);
  try {
...
describe("installSkill code safety scanning", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("ASTROCLAW_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an Astroclaw-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["ASTROCLAW_STATE_DIR", "ASTROCLAW_CONFIG_PATH"]);
    try {
      process.env.ASTROCLAW_STATE_DIR = "/tmp/untrusted-state";
      process.env.ASTROCLAW_CONFIG_PATH = "/tmp/untrusted-config/astroclaw.json";
  ...
  it("surfaces plugin scanner findings from before_install", async () => {
    ...
    await withWorkspaceCase(async ({ workspaceDir }) => {
      ...
      expect(payload?.origin).toBe("astroclaw-workspace");
