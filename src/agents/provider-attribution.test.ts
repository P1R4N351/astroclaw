...
describe("provider attribution", () => {
  it("resolves the canonical Astroclaw product and runtime version", () => {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { ASTROCLAW_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
    ...
    expect(resolveProviderAttributionHeaders("openai", { ASTROCLAW_VERSION: "2026.3.22" })).toEqual({
      originator: "astroclaw",
      version: "2026.3.22",
      "User-Agent": "astroclaw/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { ASTROCLAW_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ ASTROCLAW_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
