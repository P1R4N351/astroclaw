...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "ASTROCLAW_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { ASTROCLAW_LIVE_MODEL_IMAGE_PROBE: "false" },
        "ASTROCLAW_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { ASTROCLAW_LIVE_MODEL_IMAGE_PROBE: "1" },
        "ASTROCLAW_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
