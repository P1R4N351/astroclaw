...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("astroclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("astroclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("astroclaw-sbx-");
