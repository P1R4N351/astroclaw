...
async function createAgentDir(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-pi-auth-storage-"));
}
...
describe("discoverAuthStorage", () => {
  ...
  it("preserves legacy auth.json when auth store is forced read-only", async () => {
    await withAgentDir(async (agentDir) => {
      const previous = process.env.ASTROCLAW_AUTH_STORE_READONLY;
      ...
      try {
        ...
        if (previous === undefined) {
          ...
          process.env.ASTROCLAW_AUTH_STORE_READONLY = previous;
        }
      }
    });
  });
  ...
  it("includes env-backed provider auth when no auth profile exists", () => {
    ...
    const previousDisableBundledPlugins = process.env.ASTROCLAW_DISABLE_BUNDLED_PLUGINS;
