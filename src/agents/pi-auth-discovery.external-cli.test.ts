...
describe("resolvePiCredentialsForDiscovery external CLI scoping", () => {
  ...
  it("threads scoped external CLI discovery into writable auth store loading", () => {
    ...
    resolvePiCredentialsForDiscovery("/tmp/astroclaw-agent", {
      config: cfg,
      env: {},
      externalCli,
    });

    expect(storeMocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/astroclaw-agent", {
  ...
  it("preserves scoped external CLI discovery for read-only auth store loading", () => {
    ...
    expect(storeMocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/astroclaw-agent", {
      allowKeychainPrompt: false,
      config: cfg,
      externalCli,
      readOnly: true,
