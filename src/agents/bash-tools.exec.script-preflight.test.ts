...
import { __setFsSafeTestHooksForTest } from "@astroclaw/fs-safe/test-hooks";
...
const parseAstroclawChannelsLoginShellCommand = __testing.parseAstroclawChannelsLoginShellCommand;
...
async function expectSymlinkSwapDuringPreflightToAvoidErrors(params: {
  ...
  await withTempDir("astroclaw-exec-preflight-open-race-", async (parent) => {
...
describe("exec interactive Astroclaw channel login guard", () => {
  it("recognizes direct and package-runner channel login commands before execution", () => {
    expect(
      parseAstroclawChannelsLoginShellCommand("astroclaw channels login --channel whatsapp"),
    ).toBe(true);
    expect(
      parseAstroclawChannelsLoginShellCommand(
        "pnpm exec astroclaw channels login --channel whatsapp --verbose",
      ),
    ).toBe(true);
    expect(parseAstroclawChannelsLoginShellCommand("astroclaw channels status --deep")).toBe(false);
  });
  ...
  it("blocks interactive channel login commands from exec", async () => {
    ...
    await expect(
      tool.execute("call-astroclaw-channel-login", {
        command: "astroclaw channels login --channel whatsapp --verbose",
      }),
    ).rejects.toThrow(/exec cannot run interactive Astroclaw channel login commands/);
    await expect(
      tool.execute("call-wrapped-astroclaw-channel-login", {
        command: "sudo -u astroclaw bash -lc 'astroclaw channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive Astroclaw channel login commands/);
    await expect(
      tool.execute("call-clustered-sudo-channel-login", {
        command: "sudo -EH bash -lc 'astroclaw channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive Astroclaw channel login commands/);
    await expect(
      tool.execute("call-deep-env-channel-login", {
        command: "env env env env env env astroclaw channels login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive Astroclaw channel login commands/);
    await expect(
      tool.execute("call-env-s-trailing-channel-login", {
        command: "env -S 'astroclaw channels' login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive Astroclaw channel login commands/);
  });
});
...
describeNonWin("exec script preflight", () => {
  it("blocks shell env var injection tokens in python scripts before execution", async () => {
    await withTempDir("astroclaw-exec-preflight-", async (tmp) => {
  ...
  it("skips preflight file reads for script paths outside the workdir", async () => {
    await withTempDir("astroclaw-exec-preflight-parent-", async (parent) => {
      const outsidePath = path.join(parent, "outside.js");
      const workdir = path.join(parent, "workdir");
      await fs.mkdir(workdir, { recursive: true });
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("astroclaw-exec-preflight-nonblock-", async (tmp) => {
...
describeWin("exec script preflight on windows path syntax", () => {
  it("preserves windows-style python relative path separators during script extraction", async () => {
    await withTempDir("astroclaw-exec-preflight-win-", async (tmp) => {
      await fs.writeFile(path.join(tmp, "bad.py"), "payload = $DM_JSON", "utf-8");
...
describe("exec interpreter heuristics ReDoS guard", () => {
  it("does not hang on long commands with VAR=value assignments and whitespace-heavy text", async () => {
    ...
    const command = `ACCESS_TOKEN=$(__astroclaw_missing_redos_guard__)\ncat > /tmp/out.html << 'EOF'\n${htmlBlock}\nEOF`;
