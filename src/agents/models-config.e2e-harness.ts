...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousAstroclawAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousAstroclawAgentDir = process.env.ASTROCLAW_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousAstroclawAgentDir === undefined) {
      delete process.env.ASTROCLAW_AGENT_DIR;
    } else {
      process.env.ASTROCLAW_AGENT_DIR = previousAstroclawAgentDir;
    }
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "ASTROCLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export const CUSTOM_PROXY_MODELS_CONFIG: AstroclawConfig = {
  models: {
    providers: {
      "custom-proxy": {
