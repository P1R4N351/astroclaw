...
let createAstroclawCodingTools: typeof import("./pi-tools.js").createAstroclawCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      ASTROCLAW_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "astroclaw-test-no-bundled-extensions"),
    },
    async () => {
      ({ createAstroclawCodingTools } = await import("./pi-tools.js"));
    },
  );
});
...
async function withSafeBinsExecTool(
  ...
  try {
    await withEnvAsync(
      {
        ASTROCLAW_SHELL_ENV_TIMEOUT_MS: "1",
        PATH: "/usr/bin:/bin",
        SHELL: "/bin/sh",
      },
      async () => {
...
describe("createAstroclawCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "astroclaw-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "astroclaw-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "astroclaw-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "astroclaw-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "astroclaw-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "astroclaw-safe-bins-grep-",
        safeBins: ["grep"],
