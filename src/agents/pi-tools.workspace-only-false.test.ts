...
describe("FS tools with workspaceOnly=false", () => {
  ...
  const toolsFor = (workspaceOnly: boolean | undefined): AnyAgentTool[] => {
    const read = createAstroclawReadTool(createReadTool(workspaceDir) as unknown as AnyAgentTool);
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-test-"));
  ...
  it("restricts memory-triggered writes to append-only canonical memory files", async () => {
    ...
    const tools = [
      createAstroclawReadTool(createReadTool(workspaceDir) as unknown as AnyAgentTool),
