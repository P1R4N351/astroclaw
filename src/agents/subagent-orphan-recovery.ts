 ...
 *
 * @see https://github.com/astroclaw/astroclaw/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /astroclaw\.json|astroclaw gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumeResult = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify astroclaw.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
