...
interface EmbeddedGatewayRuntime {
  resolveSessionAgentId: (opts: { sessionKey: string; config: AstroclawConfig }) => string;
  ...
  resolveEffectiveChatHistoryMaxChars: (cfg: AstroclawConfig) => number;
  ...
  loadCombinedSessionStoreForGateway: (cfg: AstroclawConfig) => {
    storePath: string;
    store: unknown;
  };
