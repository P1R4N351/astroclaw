...
export function createCronTool(opts?: CronToolOptions, deps?: CronToolDeps): AnyAgentTool {
  ...
  return {
    label: "Cron",
    name: "cron",
    ownerOnly: isAstroclawOwnerOnlyCoreToolName("cron"),
...
Main-session cron jobs enqueue system events for heartbeat handling. Isolated cron jobs create background task runs that appear in \`astroclaw tasks\`.
    ...
    execute: async (_toolCallId, args) => {
      ...
      switch (action) {
        ...
        case "add": {
          ...
          // See: https://github.com/astroclaw/astroclaw/issues/11310
