...
import { isAstroclawOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `astroclaw security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as AstroclawConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as AstroclawConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isAstroclawOwnerOnlyCoreToolName("gateway"),
    description:
