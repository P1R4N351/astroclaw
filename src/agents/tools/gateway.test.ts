...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    astroclaw: process.env.ASTROCLAW_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.astroclaw === undefined) {
      delete process.env.ASTROCLAW_GATEWAY_TOKEN;
    } else {
      process.env.ASTROCLAW_GATEWAY_TOKEN = envSnapshot.astroclaw;
    }
  });
  ...
  it("uses ASTROCLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.ASTROCLAW_GATEWAY_TOKEN = "local-env-token";
    mocks.configState.value = {
      gateway: {
