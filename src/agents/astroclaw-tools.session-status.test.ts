...
const buildStatusMessageMock = vi.hoisted(() =>
  vi.fn((_params?: unknown) => "Astroclaw\n🧠 Model: GPT-5.4"),
);
...
describe("session_status tool", () => {
  ...
  it("returns a status card for the current session", async () => {
    ...
    expect(details.statusText).toContain("Astroclaw");
  ...
  it("passes spawned workspace to session_status auth labels", async () => {
    resetSessionStore({
      "agent:main:spawned": {
        ...
        updatedAt: 10,
        spawnedWorkspaceDir: "/tmp/astroclaw-spawned-workspace",
    ...
    expectRecordFields(mockCallArg(buildStatusMessageMock), {
      workspaceDir: "/tmp/astroclaw-spawned-workspace",
    });
  });
  ...
  it("treats the TUI client label as the current requester session", async () => {
    ...
    const result = await tool.execute("call-tui-label", { sessionKey: "astroclaw-tui" });
    const details = result.details as { ok?: boolean; sessionKey?: string };
    expect(details.ok).toBe(true);
    expect(details.sessionKey).toBe("agent:main:main");
  });
