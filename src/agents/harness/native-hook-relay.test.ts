...
describe("native hook relay registry", () => {
  it("registers a short-lived relay and builds hidden CLI commands", () => {
    const relay = registerNativeHookRelay({
      ...
      ttlMs: 10_000,
      command: {
        executable: "/opt/Open Claw/astroclaw.mjs",
    ...
    expect(relay.commandForEvent("pre_tool_use")).toBe(
      "/usr/local/bin/node '/opt/Open Claw/astroclaw.mjs' hooks relay --provider codex --relay-id " +
  ...
  it("uses the Codex no-op output when no Astroclaw hook decides", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      sessionId: "session-1",
      runId: "run-1",
  ...
  it("maps Codex PreToolUse to Astroclaw before_tool_call and blocks before execution", async () => {
  ...
  it("passes config to trusted policies for native pre-tool session extension reads", async () => {
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "astroclaw-native-relay-policy-"));
  ...
  it("uses the Codex cwd when deriving apply_patch paths for PreToolUse", async () => {
    ...
    const cwd = path.join("/tmp", "astroclaw-native-hook-cwd");
  ...
  it("maps Codex PostToolUse to Astroclaw after_tool_call observation", async () => {
  ...
  it("maps Codex MCP PreToolUse to Astroclaw before_tool_call and can block", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_input: {
          entities: [{ name: "Astroclaw", entityType: "project", observations: ["test"] }],
        },
      },
    });
  ...
  it("lets security-style plugins block native MCP calls by scanning tool params", async () => {
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_input: {
          command: "rm -rf /tmp/astroclaw-important-state",
        },
      },
    });
  ...
  it("maps Codex MCP PostToolUse to Astroclaw after_tool_call observation", async () => {
    const afterToolCall = vi.fn();
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "after_tool_call", handler: afterToolCall }]),
    );
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_response: {
          content: [{ type: "text", text: '{ "name": "astroclaw" }' }],
  ...
  it("routes Codex MCP PermissionRequest payloads through Astroclaw approval policy", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      agentId: "agent-1",
      sessionId: "session-1",
  ...
  it("defers PermissionRequest when Astroclaw approval does not decide", async () => {
  ...
  it("does not reuse pending PermissionRequest approvals when a tool call id is reused with different input", async () => {
    ...
    const second = invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_name: "Bash",
        tool_use_id: "reused-call-id",
        tool_input: { command: "rm -rf /tmp/astroclaw-important-state" },
      },
    });
  ...
  it("uses the Codex hook relay command shape", () => {
    expect(
      buildNativeHookRelayCommand({
        ...
        relayId: "relay-1",
        event: "permission_request",
        executable: "astroclaw",
      }),
    ).toBe(
      "astroclaw hooks relay --provider codex --relay-id relay-1 --event permission_request --timeout 5000",
    );
  });
});
