...
describe("runCliAgent bundle MCP e2e", () => {
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-cli-bundle-mcp-"));
      process.env.HOME = tempHome;
      process.env.USERPROFILE = tempHome;
      delete process.env.ASTROCLAW_HOME;
      delete process.env.ASTROCLAW_STATE_DIR;
      process.env.ASTROCLAW_DISABLE_PERSISTED_PLUGIN_REGISTRY = "1";
      ...
      const pluginRoot = path.join(tempHome, ".astroclaw", "extensions", "bundle-probe");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-cli-live-cleanup-"));
      process.env.HOME = tempHome;
      process.env.USERPROFILE = tempHome;
      delete process.env.ASTROCLAW_HOME;
      delete process.env.ASTROCLAW_STATE_DIR;
