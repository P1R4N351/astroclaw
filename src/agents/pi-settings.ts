...
export function resolveCompactionReserveTokensFloor(cfg?: AstroclawConfig): number {
...
export function resolveEffectiveCompactionMode(cfg?: AstroclawConfig): AgentCompactionMode {
 ...
 * before the provider call) when Astroclaw or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (astroclaw#75799).
