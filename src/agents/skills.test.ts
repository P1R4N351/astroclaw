...
const fixtureSuite = createFixtureSuite("astroclaw-skills-suite-");
let tempHome: TempHomeEnv | null = null;
let skillsHomeEnv: SkillsHomeEnvSnapshot | null = null;
const pluginEnvSnapshot = captureEnv(["ASTROCLAW_DISABLE_BUNDLED_PLUGINS"]);
...
function rawSkillApiKeyRefConfig(skillName: string): AstroclawConfig {
  return {
    skills: {
      entries: {
        [skillName]: {
...
function resolvedSkillApiKeyConfig(skillName: string, apiKey: string): AstroclawConfig {
  return {
    skills: {
      entries: {
        [skillName]: {
...
beforeAll(async () => {
  ...
  tempHome = await createTempHomeEnv("astroclaw-skills-home-");
  skillsHomeEnv = setMockSkillsHomeEnv(tempHome.home);
  await fs.mkdir(path.join(tempHome.home, ".astroclaw", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("buildWorkspaceSkillCommandSpecs", () => {
  ...
  it("includes enabled Claude bundle markdown commands as native Astroclaw slash commands", async () => {
    const workspaceDir = await makeWorkspace();
    const config = {
      plugins: {
        entries: {
    ...
    const pluginRoot = path.join(workspaceDir, ".astroclaw", "extensions", "compound-bundle");
