...
function makeFallbacksOnlyCfg(): AstroclawConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): AstroclawConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "astroclaw-auth-suite-mock");
...
// https://github.com/astroclaw/astroclaw/issues/23440
...
describe("runWithModelFallback", () => {
  ...
  it("sanitizes model identifiers in model_not_found warnings", async () => {
    const warnLogs = createWarnLogCapture("astroclaw-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
