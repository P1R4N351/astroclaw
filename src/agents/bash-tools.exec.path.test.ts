...
vi.mock("../process/supervisor/index.js", () => ({
  getProcessSupervisor: () => ({
    spawn: async (input: {
      ...
      const env = input.env ?? {};
      if (command.includes("ASTROCLAW_SHELL")) {
        input.onStdout?.(env.ASTROCLAW_SHELL ?? "");
...
describe("exec PATH login shell merge", () => {
  ...
  it("sets ASTROCLAW_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-astroclaw-shell", {
      command: 'printf "%s" "${ASTROCLAW_SHELL:-}"',
      yieldMs: FOREGROUND_TEST_YIELD_MS,
    });
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/astroclaw-ssl-keys.log";
    try {
      const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
      const result = await tool.execute("call1", {
      ...
      expect(output).not.toContain("/tmp/astroclaw-ssl-keys.log");
    } finally {
  ...
  it.each([
    ...
    "exec -a astroclaw /approve abc123 deny",
    ...
    "sudo ASTROCLAW_APPROVE=1 /approve abc123 allow-once",
    "sudo -uroot bash -lc '/approve abc123 allow-once'",
    "sudo -u root ASTROCLAW_APPROVE=1 bash -lc '/approve abc123 allow-once'",
