...
type CatalogSource = "astroclaw" | "mcp" | "client";
...
function buildModelScriptSource(code) {
  return "(async (astroclaw, console) => {\n" + code + "\n})(astroclaw, console)";
}
...
async function runModelCode(code, timeoutMs) {
  ...
  Object.defineProperties(sandbox, {
    ...
    astroclaw: { value: controller.astroclaw, enumerable: true },
  });
...
const SESSION_CATALOGS_KEY = Symbol.for("astroclaw.toolSearch.sessionCatalogs");
...
function readToolSearchConfig(config?: AstroclawConfig): Record<string, unknown> {
...
export function resolveToolSearchConfig(config?: AstroclawConfig): ToolSearchConfig {
...
function classifyTool(tool: CatalogTool): { source: CatalogSource; sourceName?: string } {
  ...
  if (pluginId) {
    return { source: "astroclaw", sourceName: pluginId };
  }
  return { source: "astroclaw", sourceName: "core" };
}
  ...
  return [
    {
      ...
      description:
        "Run JavaScript in an isolated Node subprocess with astroclaw.tools.search, astroclaw.tools.describe, and astroclaw.tools.call for large tool catalogs.",
      parameters: Type.Object({
        code: Type.String({
          description:
            "JavaScript body for an async function. Use return to return the final value. The astroclaw.tools bridge is available.",
        }),
      }),
      execute: async (
        toolCallId: string,
    ...
    },
    {
      name: TOOL_CALL_RAW_TOOL_NAME,
      label: "Tool Call",
      description: "Call a selected Tool Search catalog entry through Astroclaw.",
      parameters: Type.Object({
        id: Type.String({ description: "Tool search result id or tool name." }),
        args: Type.Optional(
