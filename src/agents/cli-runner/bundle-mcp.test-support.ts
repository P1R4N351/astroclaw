...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    envSnapshot = captureEnv(["ASTROCLAW_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("astroclaw-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("astroclaw-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("astroclaw-cli-bundle-mcp-bundled-");
    process.env.ASTROCLAW_BUNDLED_PLUGINS_DIR = emptyBundledDir;
...
function createEnabledBundleProbeConfig(): AstroclawConfig {
  return {
    plugins: {
      entries: {
