...
const CLI_BACKEND_PRESERVE_ENV = "ASTROCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.ASTROCLAW_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.ASTROCLAW_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.ASTROCLAW_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.ASTROCLAW_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.ASTROCLAW_MCP_MESSAGE_CHANNEL || "<empty>"}`,
    `senderIsOwner=${childEnv.ASTROCLAW_MCP_SENDER_IS_OWNER || "<empty>"}`,
  ].join(" ");
}
...
export async function executePreparedCliRun(
  ...
  const basePrompt = cliSessionIdToUse
    ? params.prompt
    : (context.astroClawHistoryPrompt ?? params.prompt);
      ...
      try {
        cliBackendLog.info(
          buildCliExecLogLine({
            ...
            hasHistoryPrompt: Boolean(context.astroClawHistoryPrompt),
          }),
        );
        const logOutputText =
        ...
        if (logOutputText) {
          ...
          if (
            env.ASTROCLAW_MCP_TOKEN ||
            env.ASTROCLAW_MCP_SESSION_KEY ||
            env.ASTROCLAW_MCP_SENDER_IS_OWNER
          ) {
