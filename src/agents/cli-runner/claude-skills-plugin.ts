...
const ASTROCLAW_CLAUDE_PLUGIN_NAME = "astroclaw-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredAstroclawTmpDir(), "astroclaw-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, ASTROCLAW_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: ASTROCLAW_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Astroclaw skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
