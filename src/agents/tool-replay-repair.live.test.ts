...
const TARGET_MODEL_REFS = parseTargetModelRefs(
  process.env.ASTROCLAW_LIVE_TOOL_REPLAY_REPAIR_MODELS ?? DEFAULT_TARGET_MODEL_REFS,
);
...
function parseTargetModelRefs(raw: string | undefined): TargetModelRef[] {
  ...
  for (const item of (raw ?? "").split(",")) {
    ...
    if (!provider?.trim() || !modelId) {
      throw new Error(
        `Invalid ASTROCLAW_LIVE_TOOL_REPLAY_REPAIR_MODELS entry: ${JSON.stringify(ref)}`,
      );
    }
