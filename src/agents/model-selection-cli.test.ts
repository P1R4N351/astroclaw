...
describe("isCliProvider", () => {
  ...
  it("returns true for setup-registered cli backends", () => {
    expect(isCliProvider("claude-cli", {} as AstroclawConfig)).toBe(true);
  });

  it("accepts the anthropic-cli auth-choice id as a Claude CLI provider alias", () => {
    expect(isCliProvider("anthropic-cli", {} as AstroclawConfig)).toBe(true);
  });

  it("returns false for provider ids", () => {
    expect(isCliProvider("example-cli", {} as AstroclawConfig)).toBe(false);
  });
});
