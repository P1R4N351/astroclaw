...
function resolveAttachmentLimits(config: AstroclawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".astroclaw", "attachments");
  const relDir = path.posix.join(".astroclaw", "attachments", attachmentId);
