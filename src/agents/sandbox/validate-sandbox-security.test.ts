...
describe("getBlockedBindReason", () => {
  ...
  it("still blocks OS-home credential paths when ASTROCLAW_HOME points elsewhere", () => {
    vi.stubEnv("HOME", "/home/tester");
  ...
  it("blocks canonical OS-home aliases for credential paths", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "astroclaw-home-"));
...
describe("validateBindMounts", () => {
  ...
  it("allows legitimate project directory mounts", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "astroclaw-sbx-safe-"));
    expect(
      validateBindMounts([
  ...
  it("allows drive-absolute Windows bind sources", () => {
    expect(validateBindMounts(["D:/data/astroclaw/src:/src:ro"])).toBeUndefined();
    expect(validateBindMounts(["D:\\data\\astroclaw\\output:/output:rw"])).toBeUndefined();
  });

  it("compares Windows allowed roots case-insensitively", () => {
    expect(
      validateBindMounts(["d:/DATA/Astroclaw/src:/src:ro"], {
        allowedSourceRoots: ["D:/data/astroclaw"],
      }),
    ).toBeUndefined();
  ...
  it("blocks symlink escapes into blocked directories", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "astroclaw-sbx-"));
  ...
  it("blocks symlink-parent escapes into blocked paths when leaf does not exist", () => {
    ...
    const missingLeaf = join(link, "astroclaw-not-created");
    expect(() =>
  ...
  it("blocks bind sources outside allowed roots when allowlist is configured", () => {
    const allowedRoot = mkdtempSync(join(tmpdir(), "astroclaw-sbx-allowed-root-"));
    const externalRoot = mkdtempSync(join(tmpdir(), "astroclaw-sbx-external-"));
    expect(() =>
  ...
  it("allows bind sources in allowed roots when allowlist is configured", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "astroclaw-sbx-allowed-"));
    expect(
  ...
  it("blocks reserved container target paths by default", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "astroclaw-sbx-reserved-default-"));
    expect(() =>
  ...
  it("allows reserved container target paths with explicit dangerous override", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "astroclaw-sbx-reserved-"));
    expect(
...
describe("validateApparmorProfile", () => {
  it("allows named profile/undefined", () => {
    expect(validateApparmorProfile("astroclaw-sandbox")).toBeUndefined();
...
describe("validateSandboxSecurity", () => {
  it("passes with safe config", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "astroclaw-sbx-safe-config-"));
    expect(
      validateSandboxSecurity({
