...
describe("runEmbeddedPiAgent incomplete-turn safety", () => {
  ...
  it("does not strict-agentic retry casual Discord status chatter", async () => {
    ...
    const result = await runEmbeddedPiAgent({
      ...overflowBaseRunParams,
      prompt:
        "made a bunch of improvements to the student's source code (astroclaw) this weekend, along with a few other maintainers. hopefully he will be more proactive now",
      provider: "openai-codex",
      model: "gpt-5.4",
