...
type AstroclawSimpleStreamOptions = SimpleStreamOptions & {
...
function isCodeModeEnabled(config?: AstroclawConfig): boolean {
...
export function createCodexNativeWebSearchWrapper(
  baseStreamFn: StreamFn | undefined,
  params: { config?: AstroclawConfig; agentDir?: string },
): StreamFn {
  const underlying = baseStreamFn ?? streamSimple;
  return (model, context, options) => {
    if (isCodeModeEnabled(params.config) && hasCodeModeVisibleTools(context)) {
      ...
      const originalOnPayload = options?.onPayload;
      const codeModeOptions: AstroclawSimpleStreamOptions = {
        ...options,
