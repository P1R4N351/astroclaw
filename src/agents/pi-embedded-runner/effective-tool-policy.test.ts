...
describe("applyFinalEffectiveToolPolicy", () => {
  ...
  it("filters bundled tools through inherited subagent allowlists", () => {
    ...
    const storePath = path.join(os.tmpdir(), `astroclaw-bundled-inherited-allow-${agentId}.json`);
    fs.writeFileSync(
      storePath,
      JSON.stringify(
        {
  ...
  it("honors configured plugin allow entries alongside inherited bundled tool allows", () => {
    ...
    const sessionKey = `agent:${agentId}:subagent:limited`;
    const storePath = path.join(os.tmpdir(), `astroclaw-bundled-plugin-allow-${agentId}.json`);
    fs.writeFileSync(
      storePath,
      JSON.stringify(
        {
