...
vi.mock("../model-suppression.js", () => {
  ...
  // extensions/qwen/astroclaw.plugin.json and src/plugins/manifest-model-suppression.ts.
...
describe("resolveModel", () => {
  ...
  it("invalidates PI discovery stores when inherited default auth changes", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-model-cache-"));
  ...
  it("invalidates PI discovery stores when implicit main auth changes without config", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-model-cache-state-"));
    vi.stubEnv("ASTROCLAW_STATE_DIR", rootDir);
