...
import type { AstroclawCodingToolConstructionPlan } from "../../pi-tools.js";
...
// Names here must be emitted directly by createAstroclawTools(). Catalog entries
...
const ASTROCLAW_TOOL_FACTORY_NAMES = new Set([
  "agents_list",
  "canvas",
  "cron",
  "gateway",
...
const ALL_CODING_TOOL_CONSTRUCTION_PLAN: AstroclawCodingToolConstructionPlan = {
...
const NO_CODING_TOOL_CONSTRUCTION_PLAN: AstroclawCodingToolConstructionPlan = {
...
function cloneCodingToolConstructionPlan(
  plan: AstroclawCodingToolConstructionPlan,
): AstroclawCodingToolConstructionPlan {
  return { ...plan };
}
...
function isKnownLocalCodingToolName(normalized: string): boolean {
  return (
    ...
    ASTROCLAW_TOOL_FACTORY_NAMES.has(normalized)
  );
}
...
function resolveCodingToolConstructionPlanForAllowlist(
  ...
  const includeAstroclawTools = normalized.some((name) => ASTROCLAW_TOOL_FACTORY_NAMES.has(name));
...
export function resolveEmbeddedAttemptToolConstructionPlan(params: {
  ...
  codingToolConstructionPlan: AstroclawCodingToolConstructionPlan;
} {
  ...
  const includeCoreTools =
    ...
    codingToolConstructionPlan.includeAstroclawTools;
