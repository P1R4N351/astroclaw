...
describe("buildEmbeddedRunPayloads tool-error warnings", () => {
  ...
  it("does not replay raw-looking accumulated tool output when final answer text is available", () => {
    const payloads = buildPayloads({
      assistantTexts: [
        "/root/astroclaw/src/gateway/protocol/schema/protocol-schemas.ts:181:  PluginControlUiDescriptorSchema,",
  ...
  it("strips NO_REPLY text but keeps voice media directives", () => {
    const payloads = buildPayloads({
      assistantTexts: ["NO_REPLY\nMEDIA:/tmp/astroclaw/tts-a/voice-a.opus\n[[audio_as_voice]]"],
    });

    expect(payloads).toHaveLength(1);
    expect(payloads[0]?.mediaUrl).toBe("/tmp/astroclaw/tts-a/voice-a.opus");
    expect(payloads[0]?.mediaUrls).toEqual(["/tmp/astroclaw/tts-a/voice-a.opus"]);
