...
import { resolveAstroclawReferencePaths } from "../../docs-path.js";
  ...
  try {
    ...
    const toolsRaw = !shouldConstructTools
      ...
      : (() => {
          ...
          corePluginToolStages.mark("attempt:create-astroclaw-coding-tools");
    ...
    const astroClawReferences = await resolveAstroclawReferencePaths({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop Astroclaw
      // compaction overrides applied in createPreparedEmbeddedPiSettingsManager — same
      // rehydration also restores Pi's auto-compaction (astroclaw#75799), so re-apply
      // both guards.
      applyPiCompactionSettingsFromConfig({
        settingsManager,
        cfg: params.config,
      ...
      // exact Astroclaw-managed registrations so custom tools survive startup and
      ...
      if (isRawModelRun) {
        ...
        // normal Astroclaw agent/tool prompt when `session.prompt()` starts.
      ...
      try {
        ...
        // double-compaction. See: https://github.com/astroclaw/astroclaw/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("astroclaw:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/astroclaw/astroclaw/issues/8643
