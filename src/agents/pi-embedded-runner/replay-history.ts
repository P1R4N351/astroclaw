...
// `provider:"astroclaw"` assistant entries written by the channel-delivery
...
const TRANSCRIPT_ONLY_ASTROCLAW_MODELS = new Set<string>(["delivery-mirror", "gateway-injected"]);
...
function isTranscriptOnlyAstroclawAssistant(message: AgentMessage): boolean {
  ...
  return (
    provider === "astroclaw" &&
    typeof model === "string" &&
    TRANSCRIPT_ONLY_ASTROCLAW_MODELS.has(model)
  );
}
...
export function normalizeAssistantReplayContent(messages: AgentMessage[]): AgentMessage[] {
  ...
  for (const message of messages) {
    ...
    if (isTranscriptOnlyAstroclawAssistant(message)) {
