...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("astroclaw-bundled-");
  const workspaceDir = await createTempDir("astroclaw-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: AstroclawConfig) {
