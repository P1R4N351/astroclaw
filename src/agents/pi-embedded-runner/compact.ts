...
import { createAstroclawCodingTools, resolveProcessToolScopeKey } from "../pi-tools.js";
...
async function compactEmbeddedPiSessionDirectOnce(
  ...
  await ensureAstroclawModelsJson(params.config, agentDir, {
    workspaceDir: resolvedWorkspace,
  });
  ...
  try {
    ...
    const toolsRaw = createAstroclawCodingTools({
      exec: {
    ...
    const filteredBundledTools = applyFinalEffectiveToolPolicy({
      ...
      // createAstroclawCodingTools(...) also omit it, so both paths resolve
    ...
    try {
      ...
      // exact Astroclaw-managed registrations so custom tools survive startup.
