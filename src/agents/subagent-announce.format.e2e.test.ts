...
let configOverride: AstroclawConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: AstroclawConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/astroclaw/astroclaw/issues/31298
    previousFastTestEnv = process.env.ASTROCLAW_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.ASTROCLAW_TEST_FAST = previousFastTestEnv;
  });
  ...
  beforeEach(() => {
    vi.useRealTimers();
    // ASTROCLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Astroclaw runtime context (internal):");
