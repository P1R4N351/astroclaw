...
function resolveLiveProviderCatalogTimeoutMs(env: NodeJS.ProcessEnv): number | null {
  const live =
    env.ASTROCLAW_LIVE_TEST === "1" || env.ASTROCLAW_LIVE_GATEWAY === "1" || env.LIVE === "1";
  if (!live) {
    return null;
  }
  const raw = env.ASTROCLAW_LIVE_PROVIDER_DISCOVERY_TIMEOUT_MS?.trim();
  if (!raw) {
    return 15_000;
  }
...
function resolveProviderDiscoveryFilter(params: {
  ...
  const testRaw = env.ASTROCLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS?.trim();
  if (testRaw) {
    const ids = testRaw
      .split(",")
  ...
  const rawValues = [
    env.ASTROCLAW_LIVE_PROVIDERS?.trim(),
    env.ASTROCLAW_LIVE_GATEWAY_PROVIDERS?.trim(),
...
function buildPluginCatalogConfig(ctx: ImplicitProviderContext): AstroclawConfig {
...
): Promise<NonNullable<AstroclawConfig["models"]>["providers"]> {
