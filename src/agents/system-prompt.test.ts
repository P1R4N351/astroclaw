...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes an Astroclaw control section", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/astroclaw",
    });

    expect(prompt).toContain("## Astroclaw Control");
    expect(prompt).toContain("prefer `gateway` tool");
    expect(prompt).toContain("CLI lifecycle only on explicit user request");
    expect(prompt).toContain("astroclaw gateway status|restart|start|stop");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    ...
    expect(prompt).toContain(
      '[embed url="/__astroclaw__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- agents_list: List Astroclaw agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/astroclaw/docs");
    expect(prompt).toContain("Astroclaw behavior/config/architecture: read local docs first.");
  });
  ...
  it("includes docs guidance when docsPath is provided", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/astroclaw/docs");
    expect(prompt).toContain("Source: /tmp/astroclaw");
    expect(prompt).toContain("Astroclaw behavior/config/architecture: read local docs first.");
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("Docs: https://docs.astroclaw.ai");
    expect(prompt).toContain("Source: https://github.com/astroclaw/astroclaw");
  ...
  it("includes bootstrap instructions in system prompt when bootstrap is pending", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/astroclaw",
      bootstrapMode: "full",
      contextFiles: [{ path: "/tmp/astroclaw/BOOTSTRAP.md", content: "Ask who I am." }],
    });
    ...
    expect(prompt).toContain("## /tmp/astroclaw/BOOTSTRAP.md");
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## Astroclaw Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/astroclaw. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildAgentBootstrapSystemPromptSupplement", () => {
  it("can render bootstrap guidance without duplicating Project Context", () => {
    ...
    expect(sections).not.toContain("## /tmp/astroclaw/BOOTSTRAP.md");
  ...
  it("adds pending bootstrap guidance and BOOTSTRAP.md contents for override prompts", () => {
    ...
    expect(supplement).toContain("## /tmp/astroclaw/BOOTSTRAP.md");
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`astroclaw ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for Astroclaw subagents");
