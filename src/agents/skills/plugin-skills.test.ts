    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "astroclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "astroclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "astroclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("astroclaw-");
  const acpxRoot = await tempDirs.make("astroclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("astroclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("astroclaw-");
  const pluginRoot = await tempDirs.make("astroclaw-plugin-");
  const outsideDir = await tempDirs.make("astroclaw-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("astroclaw-");
    const pluginRoot = await tempDirs.make("astroclaw-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("astroclaw-");
    const pluginRoot = await tempDirs.make("astroclaw-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
