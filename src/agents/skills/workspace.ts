...
import { resolveAstroclawMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
function resolveSkillsLimits(config?: AstroclawConfig, agentId?: string): ResolvedSkillsLimits {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "astroclaw-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "astroclaw-bundled") {
    return {
...
function resolveAllowedSymlinkTargetRealPaths(config?: AstroclawConfig): string[] {
...
function loadSkillEntries(
  ...
  const loadSkills = (params: { dir: string; source: string }): LoadedSkillRecord[] => {
    ...
    // skill directories (e.g. ~/.astroclaw/skills/coze/koze-retrieval/SKILL.md).
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values())
    ...
    .map((record) => {
      ...
      return {
        skill,
        frontmatter,
        metadata: resolveAstroclawMetadata(frontmatter),
        invocation,
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`astroclaw skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`astroclaw skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
