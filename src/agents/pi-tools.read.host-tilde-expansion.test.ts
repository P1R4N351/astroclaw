...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("astroclaw-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("astroclaw-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("astroclaw-tilde-test-mkdir-");
  ...
  it("ignores ASTROCLAW_HOME for write operations", async () => {
    const astroclawHome = await createTempDir("astroclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("astroclaw-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("ASTROCLAW_HOME", astroclawHome);

    createHostWorkspaceWriteTool(astroclawHome, { workspaceOnly: false });
    ...
    await expectMissingPath(fs.access(path.join(astroclawHome, path.basename(testFile))));
  });

  it("ignores ASTROCLAW_HOME for mkdir operations", async () => {
    const astroclawHome = await createTempDir("astroclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("astroclaw-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expectMissingPath(fs.access(path.join(astroclawHome, path.basename(newDir))));
  });

  it("ignores ASTROCLAW_HOME for readFile operations", async () => {
    const astroclawHome = await createTempDir("astroclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("astroclaw-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(astroclawHome, { workspaceOnly: false });
    const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores ASTROCLAW_HOME for access operations", async () => {
    const astroclawHome = await createTempDir("astroclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("astroclaw-tilde-test-edit-");
