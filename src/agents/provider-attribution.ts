import { listAstroclawPluginManifestMetadata } from "../plugins/manifest-metadata-scan.js";
import {
...
const ASTROCLAW_ATTRIBUTION_PRODUCT = "Astroclaw";
const ASTROCLAW_ATTRIBUTION_ORIGINATOR = "astroclaw";
...
function formatAstroclawUserAgent(version: string): string {
  return `${ASTROCLAW_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
function collectManifestProviderEndpoints(): ManifestProviderEndpointCacheEntry[] {
  const entries: ManifestProviderEndpointCacheEntry[] = [];
  for (const { manifest } of listAstroclawPluginManifestMetadata()) {
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: ASTROCLAW_ATTRIBUTION_PRODUCT,
...
function buildOpenRouterAttributionPolicy(
  ...
  return {
    ...
    reviewNote: "Documented app attribution headers. Verified in Astroclaw runtime wrapper.",
    ...identity,
    headers: {
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: ASTROCLAW_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatAstroclawUserAgent(identity.version),
    },
  };
}
