...
describe("path-resolve helpers (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-path-direct-"));
  ...
  it("resolveAuthStorePath falls back to the default agent dir when agentDir is omitted", () => {
    ...
    // ASTROCLAW_STATE_DIR set to our tempdir, the resolved path must live under it.
  ...
  it("resolveAuthStorePathForDisplay preserves a tilde-rooted path unchanged", () => {
    ...
    const tildeAgentDir = "~fake-astroclaw-no-expand";
...
describe("ensureAuthStoreFile (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-path-ensure-"));
    process.env.ASTROCLAW_STATE_DIR = stateDir;
  });
