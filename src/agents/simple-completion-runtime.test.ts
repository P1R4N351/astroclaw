...
describe("prepareSimpleCompletionModel", () => {
  ...
  it("applies provider runtime auth before storing simple-completion credentials", async () => {
    ...
    expect(runtimeAuthInput.workspaceDir).toBe("/tmp/astroclaw-agent");
...
describe("completeWithPreparedSimpleCompletionModel", () => {
  it("prepares provider-owned stream APIs before running a completion", async () => {
    ...
    const preparedModel = {
      ...model,
      api: "astroclaw-ollama-simple-test",
    };
    const cfg = {
  ...
  it("normalizes Astroclaw-only thinking levels before using pi-ai simple completion", async () => {
    const model = {
      provider: "openai",
      id: "gpt-5.4",
      name: "gpt-5.4",
