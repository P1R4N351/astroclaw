...
type HomeEnvSnapshot = {
  ...
  ASTROCLAW_STATE_DIR: string | undefined;
  ASTROCLAW_AGENT_DIR: string | undefined;
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".astroclaw", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.ASTROCLAW_STATE_DIR = path.join(home, ".astroclaw");
    process.env.ASTROCLAW_AGENT_DIR = path.join(home, ".astroclaw", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".astroclaw", "agent");
