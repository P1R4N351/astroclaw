...
const acpMocks = vi.hoisted(() => ({
  ...
  readAcpSessionEntry: vi.fn<(params: { sessionKey: string; cfg?: AstroclawConfig }) => unknown>(
    () => null,
  ),
...
const ttsMocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveTtsConfig: vi.fn((_cfg: AstroclawConfig) => ({ mode: "final" })),
  };
});
...
vi.mock("../../tts/tts.js", () => ({
  ...
  resolveTtsConfig: (cfg: AstroclawConfig) => ttsMocks.resolveTtsConfig(cfg),
}));
...
vi.mock("./dispatch-acp-session.runtime.js", () => ({
  readAcpSessionEntry: (params: { sessionKey: string; cfg?: AstroclawConfig }) =>
    acpMocks.readAcpSessionEntry(params),
}));
vi.mock("../../tts/tts-config.js", () => ({
  normalizeTtsAutoMode: (value: unknown) => ttsMocks.normalizeTtsAutoMode(value),
  resolveConfiguredTtsMode: (cfg: AstroclawConfig) => ttsMocks.resolveTtsConfig(cfg).mode,
  shouldCleanTtsDirectiveText: () => true,
  shouldAttemptTtsPayload: () => true,
}));

const noAbortResult = { handled: false, aborted: false } as const;
const emptyConfig = {} as AstroclawConfig;
...
} as const satisfies AstroclawConfig;
...
function createMockAcpSessionManager() {
  return {
    resolveSession: (params: { cfg: AstroclawConfig; sessionKey: string }) => {
...
describe("dispatchReplyFromConfig", () => {
  ...
  it("retargets reply_dispatch to a bound generic ACP session before model fallback", async () => {
    ...
    acpMocks.readAcpSessionEntry.mockImplementation(
      (params: { sessionKey: string; cfg?: AstroclawConfig }) =>
  ...
  it("normalizes accumulated block TTS-only media before final delivery", async () => {
    ...
    replyMediaPathMocks.createReplyMediaPathNormalizer.mockReturnValue(
      async (payload: ReplyPayload) => ({
        ...payload,
        mediaUrl: "/tmp/astroclaw-media/normalized-tts.ogg",
        mediaUrls: ["/tmp/astroclaw-media/normalized-tts.ogg"],
      }),
    );
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
    ...
    const finalPayload = firstFinalReplyPayload(dispatcher);
    expect(finalPayload?.mediaUrl).toBe("/tmp/astroclaw-media/normalized-tts.ogg");
    expect(finalPayload?.mediaUrls).toStrictEqual(["/tmp/astroclaw-media/normalized-tts.ogg"]);
  ...
  it("does not mark duplicate inbound returns as tool-only when message is unavailable", async () => {
    setNoAbort();
    const cfg = { tools: { allow: ["read"] } } as AstroclawConfig;
    const ctx = buildTestCtx({
      Provider: "telegram",
      Surface: "telegram",
      ChatType: "channel",
  ...
  it("emits diagnostics when enabled", async () => {
    setNoAbort();
    const cfg = { diagnostics: { enabled: true } } as AstroclawConfig;
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
      Provider: "slack",
      Surface: "slack",
  ...
  it("routes plugin-owned bindings to the owning plugin before generic inbound claim broadcast", async () => {
    ...
    hookMocks.registry.plugins = [{ id: "astroclaw-codex-app-server", status: "loaded" }];
    hookMocks.runner.runInboundClaimForPluginOutcome.mockResolvedValue({
      status: "handled",
      result: { handled: true },
    });
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      ...
      metadata: {
        ...
        pluginRoot: "/Users/huntharo/github/astroclaw-app-server",
        data: {
    ...
    const inboundClaimCall = hookMocks.runner.runInboundClaimForPluginOutcome.mock
      ...
      | undefined;
    expect(inboundClaimCall?.[0]).toBe("astroclaw-codex-app-server");
  ...
  it("falls back to Astroclaw once per startup when a bound plugin is missing", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    const replyResolver = vi.fn(async () => ({ text: "astroclaw fallback" }) satisfies ReplyPayload);
  ...
  it("falls back to Astroclaw when the bound plugin is loaded but has no inbound_claim handler", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
  ...
  it("releases inbound dedupe when dispatch fails before completion", async () => {
    ...
    const replyResolver = vi
      .fn<
        (_ctx: MsgContext, _opts?: GetReplyOptions, _cfg?: AstroclawConfig) => Promise<ReplyPayload>
      >()
  ...
  it("passes the loaded config plus configOverride patch to replyResolver when provided", async () => {
    ...
    let receivedCfg: AstroclawConfig | undefined;
    const replyResolver = async (
      _ctx: MsgContext,
      _opts?: GetReplyOptions,
      cfgArg?: AstroclawConfig,
    ) => {
      receivedCfg = cfgArg;
      return { text: "hi" } satisfies ReplyPayload;
    };
  ...
  it("passes the already loaded config to replyResolver when configOverride is not provided", async () => {
    setNoAbort();
    const cfg = { agents: { defaults: { userTimezone: "UTC" } } } as AstroclawConfig;
    const dispatcher = createDispatcher();
...
describe("sendPolicy deny — suppress delivery, not processing (#53328)", () => {
  ...
  it("suppresses tool result delivery when sendPolicy is deny", async () => {
    ...
    const replyResolver = vi.fn(
      async (_ctx: MsgContext, opts?: GetReplyOptions, _cfg?: AstroclawConfig) => {
  ...
  it("falls back to automatic group/channel delivery when the message tool is unavailable", async () => {
    ...
    const result = await dispatchReplyFromConfig({
      ...
      cfg: { tools: { allow: ["read"] } } as AstroclawConfig,
      dispatcher,
      replyResolver,
    });
