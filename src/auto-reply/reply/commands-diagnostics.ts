...
import { buildCurrentAstroclawCliCommand } from "./commands-astroclaw-cli.js";
import {
...
const DIAGNOSTICS_DOCS_URL = "https://docs.astroclaw.ai/gateway/diagnostics";
const GATEWAY_DIAGNOSTICS_EXPORT_JSON_LABEL = "astroclaw gateway diagnostics export --json";
...
function buildGatewayDiagnosticsExportJsonCommand(): string {
  return buildCurrentAstroclawCliCommand(["gateway", "diagnostics", "export", "--json"]);
}
...
function isCodexDiagnosticsUnavailableText(text: string | undefined): boolean {
  return (
    text?.startsWith("No Codex thread is attached to this Astroclaw session yet.") === true ||
    text?.startsWith(
      "Cannot send Codex diagnostics because this command did not include an Astroclaw session file.",
    ) === true
  );
}
