...
import { scheduleGatewaySigusr1Restart, triggerAstroclawRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Astroclaw in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  let sentinelPath: string | null = null;
  ...
  const restartMethod = triggerAstroclawRestart();
  if (!restartMethod.ok) {
    await removeRestartSentinelFile(sentinelPath);
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Astroclaw via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
