...
export function resolveAcpInstallCommandHint(cfg: AstroclawConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `astroclaw plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^astroclaw plugins install /u, "");
    ...
    return "astroclaw plugins install acpx";
  }
