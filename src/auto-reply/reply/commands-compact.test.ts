...
function requireResolveSessionAgentIdCall(index = 0) {
  ...
  )?.[0] as { sessionKey?: string; config?: AstroclawConfig } | undefined;
  if (!call) {
...
function requireResolveAgentDirCall(index = 0) {
  const call = resolveAgentDirMock.mock.calls[index] as [AstroclawConfig, string] | undefined;
  if (!call) {
...
describe("handleCompactCommand", () => {
  beforeEach(() => {
    ...
    resolveAgentDirMock.mockImplementation(
      (_cfg: unknown, agentId: string) => `/tmp/workspace/.astroclaw/agents/${agentId}/agent`,
    );
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    ...
    const result = await handleCompactCommand(
      {
        ...buildCompactParams("/compact", {
          commands: { text: true },
          channels: { whatsapp: { allowFrom: ["*"] } },
          session: { store: "/tmp/astroclaw-session-store.json" },
        } as AstroclawConfig),
        ctx: {
          Provider: "whatsapp",
          Surface: "whatsapp",
        ...
        agentDir: "/tmp/astroclaw-agent-compact",
        sessionEntry: {
          sessionId: "session-1",
          updatedAt: Date.now(),
          groupId: "group-1",
    ...
    expect(call.agentDir).toBe("/tmp/astroclaw-agent-compact");
  });
