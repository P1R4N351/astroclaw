...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop astroclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@astroclaw_bot", { botUsername: "astroclaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@astroclaw_bot", { botUsername: "astroclaw_bot" })).toBe(true);
