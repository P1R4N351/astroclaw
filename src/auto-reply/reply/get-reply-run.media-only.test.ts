...
describe("runPreparedReply media-only handling", () => {
  ...
  it("runs room events as contextual events instead of direct user prompts", async () => {
    ...
    const call = requireLastRunReplyAgentCall();
    expect(call?.commandBody).toBe("[Astroclaw room event]");
    expect(call?.transcriptCommandBody).toBe("");
    expect(call?.followupRun.prompt).toBe("[Astroclaw room event]");
    ...
    expect(call?.followupRun.currentInboundContext?.text).toContain("[Astroclaw room event]");
    expect(call?.followupRun.currentInboundContext?.text).toContain(
  ...
  it("keeps heartbeat prompts out of visible transcript prompt", async () => {
    ...
    expect(call?.transcriptCommandBody).toBe("[Astroclaw heartbeat poll]");
    expect(call?.followupRun.transcriptPrompt).toBe("[Astroclaw heartbeat poll]");
  });
  ...
  it.each([
    ...
    async (commandText, startupAction) => {
      ...
      expect(call?.transcriptCommandBody).toBe(`[Astroclaw session ${startupAction}]`);
      expect(call?.followupRun.transcriptPrompt).toBe(`[Astroclaw session ${startupAction}]`);
