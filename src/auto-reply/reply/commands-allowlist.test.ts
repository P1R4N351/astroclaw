...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-allowlist-config-"));
  const configPath = path.join(dir, "astroclaw.json");
  const previous = process.env.ASTROCLAW_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.ASTROCLAW_CONFIG_PATH = previous;
    }
        ...
        run: async () => {
          await withTempConfigPath(
            ...
            async (configPath) => {
              ...
              const written = await readJsonFile<AstroclawConfig>(configPath);
