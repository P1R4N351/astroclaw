...
import { getRuntimeConfig, type AstroclawConfig } from "../config/config.js";
...
const ASTROCLAW_CLI_WRAPPER_PATH = fileURLToPath(new URL("../../astroclaw.mjs", import.meta.url));
const ASTROCLAW_RUN_NODE_SCRIPT_PATH = fileURLToPath(
  new URL("../../scripts/run-node.mjs", import.meta.url),
);
const ASTROCLAW_DIST_ENTRY_JS_PATH = fileURLToPath(new URL("../../dist/entry.js", import.meta.url));
const ASTROCLAW_DIST_ENTRY_MJS_PATH = fileURLToPath(
...
export function resolveLocalAuthCliInvocation(params?: {
  ...
  const hasDistEntry =
    params?.hasDistEntry ??
    (existsSync(ASTROCLAW_DIST_ENTRY_JS_PATH) || existsSync(ASTROCLAW_DIST_ENTRY_MJS_PATH));
  const hasRunNodeScript = params?.hasRunNodeScript ?? existsSync(ASTROCLAW_RUN_NODE_SCRIPT_PATH);
  const command = params?.execPath ?? process.execPath;
  const wrapperPath = params?.wrapperPath ?? ASTROCLAW_CLI_WRAPPER_PATH;
  const runNodePath = params?.runNodePath ?? ASTROCLAW_RUN_NODE_SCRIPT_PATH;
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run astroclaw devices list",
      pairingHint:
        "Pairing required. Run `astroclaw devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export async function runTui(opts: RunTuiOptions): Promise<TuiResult> {
  ...
  const previousConsoleSubsystemFilter = isLocalMode
    ...
    : null;
  if (isLocalMode) {
    setConsoleSubsystemFilter(["__astroclaw_tui_quiet__"]);
  }
  ...
  const updateHeader = () => {
    ...
    const title = opts.title ?? "astroclaw tui";
    header.setText(
      theme.header(
  ...
  const forceExit = () => {
    try {
      process.stderr.write("astroclaw tui forcing exit\n");
    } catch {
    ...
    void drainAndStopTuiSafely(tui)
      .catch((err) => {
        if (!isTuiTerminalLossError(err)) {
          try {
            process.stderr.write(`astroclaw tui shutdown failed: ${String(err)}\n`);
          } catch {
