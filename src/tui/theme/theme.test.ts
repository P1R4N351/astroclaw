...
describe("light background detection", () => {
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      ASTROCLAW_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when ASTROCLAW_THEME=light", async () => {
    const mod = await importThemeWithEnv({ ASTROCLAW_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when ASTROCLAW_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ ASTROCLAW_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats ASTROCLAW_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ ASTROCLAW_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("ASTROCLAW_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      ASTROCLAW_THEME: "dark",
      COLORFGBG: "0;15",
    });
    expect(mod.lightMode).toBe(false);
  });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ ASTROCLAW_THEME: "light" });
    const darkMod = await importThemeWithEnv({ ASTROCLAW_THEME: "dark" });
