...
import { replaceConfigFile, type AstroclawConfig } from "../config/config.js";
...
import { ASTROCLAW_WRAPPER_ENV_KEY } from "../daemon/program-args.js";
...
function buildGatewayServiceRepairEnv(
  ...
  const wrapperPath = command?.environment?.[ASTROCLAW_WRAPPER_ENV_KEY]?.trim();
  if (!wrapperPath || Object.hasOwn(process.env, ASTROCLAW_WRAPPER_ENV_KEY)) {
    return process.env;
  }
  return {
    ...process.env,
    [ASTROCLAW_WRAPPER_ENV_KEY]: wrapperPath,
  };
}
...
function resolveGatewayServiceWrapperPath(
  ...
  return normalizeOptionalString(command?.environment?.[ASTROCLAW_WRAPPER_ENV_KEY]) ?? null;
}
...
function resolveSystemdUnitNameFromServicePath(sourcePath: string | undefined): string {
  ...
  return base.endsWith(".service") ? base : "astroclaw-gateway.service";
}
...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (serviceWrapperPath) {
    note(`Gateway service invokes ${ASTROCLAW_WRAPPER_ENV_KEY}: ${serviceWrapperPath}`, "Gateway");
  }
  ...
  const sourceCheckoutWarning = serviceLayout?.entrypointSourceCheckout
    ? [
        ...
        "Run `astroclaw doctor --fix` from the intended package install, or reinstall the gateway service with `astroclaw gateway install --force`.",
      ].join("\n")
    : null;
  ...
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      ...
      message:
        "Gateway service ASTROCLAW_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
  ...
  if (serviceRewriteBlocked) {
    note(
      "Gateway service is running; leaving supervisor metadata unchanged. Stop the service first or use `astroclaw gateway install --force` when you want to replace the active launcher.",
      "Gateway service config",
    );
    return;
  }
  ...
  if (
    ...
    note(
      "Update-mode doctor detected gateway service drift but left the live systemd unit unchanged. Review the service file and run `astroclaw gateway install --force` when you want Astroclaw to replace operator-owned systemd directives.",
      "Gateway service config",
    );
    return;
  }
  ...
  if (!repair) {
    if (!emittedSourceCheckoutWarning) {
      note(
        "Run `astroclaw gateway install --force` when you want to replace the gateway service definition.",
        "Gateway service config",
      );
    }
    return;
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing Astroclaw gateway next.");
      }
    }
  }
