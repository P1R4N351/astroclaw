...
const hasUsableCustomProviderApiKey = vi.hoisted(() =>
  vi.fn<(_cfg?: AstroclawConfig, _provider?: string, _env?: NodeJS.ProcessEnv) => boolean>(
    () => false,
  ),
);
...
describe("promptDefaultModel", () => {
  it("adds runtime-route hints for canonical and legacy OpenAI Codex models", async () => {
    ...
    await promptDefaultModel({
      config: { agents: { defaults: {} } } as AstroclawConfig,
      prompter,
      allowKeep: false,
      includeManual: false,
  ...
  it("supports configuring vLLM during setup", async () => {
    ...
    const prompter = makePrompter({ select });
    const config = { agents: { defaults: {} } } as AstroclawConfig;
