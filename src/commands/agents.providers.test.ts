...
describe("buildProviderStatusIndex", () => {
  ...
  it("prefers inspectAccount for read-only status surfaces", async () => {
    ...
    const map = await buildProviderStatusIndex({} as AstroclawConfig);
  ...
  it("records accounts that throw during read-only resolution as not configured", async () => {
    ...
    await expect(buildProviderStatusIndex({} as AstroclawConfig)).resolves.toEqual(
      new Map([
        [
          "quietchat:default",
          {
  ...
  it("rethrows unexpected read-only account resolution errors", async () => {
    ...
    await expect(buildProviderStatusIndex({} as AstroclawConfig)).rejects.toThrow("plugin crash");
  });
});
