...
function printGatewayNotReadyHints(runtime: RuntimeEnv, reason: string): void {
  runtime.log(reason);
  runtime.log("Run `astroclaw gateway status --deep` for details.");
  runtime.log("Run `astroclaw gateway start` to start a managed gateway.");
  runtime.log("Run `astroclaw gateway run` for a foreground gateway.");
}
...
export async function ensureGatewayReadyForOperation(
  ...
  const prompt = shouldInstall
    ? `Gateway is not installed. Install and start it now so Astroclaw can ${options.operation}?`
    : `Gateway is not running. Start it now so Astroclaw can ${options.operation}?`;
