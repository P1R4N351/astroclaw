...
function buildLegacyRealtimeTalkCompat(
  talk: Record<string, unknown>,
  normalizedTalk: NonNullable<AstroclawConfig["talk"]>,
): Record<string, unknown> | undefined {
  ...
  return normalizeTalkSection({ realtime: compat } as AstroclawConfig["talk"])?.realtime;
}
...
export function normalizeLegacyTalkConfig(cfg: AstroclawConfig, changes: string[]): AstroclawConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as AstroclawConfig["talk"]) ?? {};
