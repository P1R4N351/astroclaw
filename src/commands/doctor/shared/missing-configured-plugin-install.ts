...
function collectConfiguredPluginIds(cfg: AstroclawConfig, env?: NodeJS.ProcessEnv): Set<string> {
...
function collectBlockedPluginIds(cfg: AstroclawConfig): Set<string> {
  const ids = new Set<string>();
...
function collectConfiguredChannelIds(cfg: AstroclawConfig, env?: NodeJS.ProcessEnv): Set<string> {
  const ids = new Set<string>();
...
async function repairMissingPluginInstalls(params: {
  ...
  if (isUpdatePackageSwapInProgress(env)) {
    ...
    for (const pluginId of updateDeferredPluginIds) {
      ...
      changes.push(
        `Skipped package-manager repair for configured plugin "${pluginId}" during package update; rerun "astroclaw doctor --fix" after the update completes.`,
      );
    }
  }
