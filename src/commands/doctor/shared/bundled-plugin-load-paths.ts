...
function resolveBundledWorkspaceDir(cfg: AstroclawConfig): string | undefined {
...
export function collectBundledPluginLoadPathWarnings(params: {
  ...
  const lines = params.hits.map(
    (hit) =>
      `- ${hit.pathLabel}: bundled plugin path "${hit.fromPath}" still aliases ${hit.pluginId}; Astroclaw loads the packaged bundled plugin from "${hit.toPath}".`,
  );
