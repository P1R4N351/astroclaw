...
vi.mock("./stale-plugin-config.js", () => ({
  scanStalePluginConfig: (cfg: {
    ...
    if (cfg.channels?.["astroclaw-weixin"]) {
      hits.push({ id: "astroclaw-weixin", surface: "channel" });
    }
    return hits.filter(
...
describe("doctor preview warnings", () => {
  ...
  it("includes stale plugin config warnings", async () => {
    ...
    expect(warning).toContain('Run "astroclaw doctor --fix"');
  ...
  it("includes stale channel config warnings without plugin config", async () => {
    ...
    expectSingleWarningContaining(warnings, "channels.astroclaw-weixin: dangling channel config");
  });

  it("includes bundled plugin load path migration warnings", async () => {
    const packageRoot = path.resolve("app-node-modules", "astroclaw");
  ...
  it("includes stale OAuth profile shadow warnings", async () => {
    staleOAuthShadowState.warnings = [
      '- ~/.astroclaw/agents/telegram/agent/auth-profiles.json has stale OAuth auth profile openai-codex:default. Run "astroclaw doctor --fix".',
    ];
  ...
  it("warns but skips auto-removal when plugin discovery has errors", async () => {
    ...
    expect(warning).toContain('rerun "astroclaw doctor --fix"');
  });
