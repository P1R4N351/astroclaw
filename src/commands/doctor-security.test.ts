...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    ...
    pluginRegistry.list = [];
    prevToken = process.env.ASTROCLAW_GATEWAY_TOKEN;
    prevPassword = process.env.ASTROCLAW_GATEWAY_PASSWORD;
    prevHome = process.env.HOME;
    prevServiceKind = process.env.ASTROCLAW_SERVICE_KIND;
    delete process.env.ASTROCLAW_GATEWAY_TOKEN;
    delete process.env.ASTROCLAW_GATEWAY_PASSWORD;
    delete process.env.ASTROCLAW_SERVICE_KIND;
  });
  ...
  afterEach(() => {
    ...
    if (prevPassword === undefined) {
      ...
      process.env.ASTROCLAW_GATEWAY_PASSWORD = prevPassword;
    }
    ...
    if (prevServiceKind === undefined) {
      ...
      process.env.ASTROCLAW_SERVICE_KIND = prevServiceKind;
    }
  });
  ...
  async function withExecApprovalsFile(
    ...
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-doctor-security-"));
    process.env.HOME = home;
    await fs.mkdir(path.join(home, ".astroclaw"), { recursive: true });
    await fs.writeFile(
      path.join(home, ".astroclaw", "exec-approvals.json"),
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as AstroclawConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.ASTROCLAW_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as AstroclawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("warns when ASTROCLAW_GATEWAY_TOKEN env conflicts with gateway.auth.token config (#74271)", async () => {
    process.env.ASTROCLAW_GATEWAY_TOKEN = "env-token-123";
    const cfg = {
      gateway: {
        auth: {
    ...
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("ASTROCLAW_GATEWAY_TOKEN conflicts with gateway.auth.token");
    expect(message).toContain("Direct local Gateway clients commonly prefer the env token");
    expect(message).toContain("~/.astroclaw/.env");
  });

  it("does not warn when only env token is set without config token", async () => {
    process.env.ASTROCLAW_GATEWAY_TOKEN = "env-token-only";
    const cfg = { gateway: { bind: "lan" } } as AstroclawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).not.toContain("ASTROCLAW_GATEWAY_TOKEN overrides");
  });
  ...
  it("does not warn inside the managed gateway service credential context", async () => {
    ...
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).not.toContain("ASTROCLAW_GATEWAY_TOKEN conflicts");
  });

  it("does not warn when config token uses ASTROCLAW_GATEWAY_TOKEN SecretRef", async () => {
    process.env.ASTROCLAW_GATEWAY_TOKEN = "env-token-123";
    const cfg = {
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as AstroclawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats unset bind as loopback for host-side doctor checks", async () => {
    const cfg = { gateway: {} } as AstroclawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("No channel security warnings detected");
    expect(message).not.toContain("Gateway bound");
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as AstroclawConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("astroclaw approvals get --gateway");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    await noteSecurityWarnings({} as AstroclawConfig);
    ...
    expect(message).toContain("Run: astroclaw security audit --deep");
  });
