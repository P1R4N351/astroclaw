...
export async function channelsResolveCommand(opts: ChannelsResolveOptions, runtime: RuntimeEnv) {
  ...
  if (entries.length === 0) {
    throw new Error(
      `At least one entry is required. Example: ${formatCliCommand("astroclaw channels resolve --channel discord <name-or-id>")}.`,
    );
  }
  ...
  if (explicitChannel && resolvedExplicit?.catalogEntry && !resolvedExplicit.plugin) {
    throw new Error(
      `Channel plugin "${resolvedExplicit.catalogEntry.id}" is not installed. Run ${formatCliCommand(`astroclaw channels add --channel ${resolvedExplicit.catalogEntry.id}`)} first.`,
    );
  }
