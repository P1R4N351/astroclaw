...
export async function dashboardCommand(
  ...
  if (resolvedToken.unresolvedRefReason) {
    ...
    runtime.log(
      "Set ASTROCLAW_GATEWAY_TOKEN in this shell or resolve your secret provider, then rerun `astroclaw dashboard`.",
    );
  }
  ...
  if (opened) {
    runtime.log("Opened in your browser. Keep that tab to control Astroclaw.");
  ...
  if (fallbackToManualAuth) {
    runtime.log(
      "Token auto-auth not delivered. Append your gateway token (from ASTROCLAW_GATEWAY_TOKEN or gateway.auth.token) as a URL fragment with key `token` to authenticate.",
    );
  }
}
