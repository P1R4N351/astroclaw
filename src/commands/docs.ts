...
const SEARCH_TOOL = "https://docs.astroclaw.ai/mcp.search_open_claw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.astroclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('astroclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.astroclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('astroclaw docs "your query"')}`);
    }
    return;
  }
