...
const mocks = vi.hoisted(() => ({
  ...
  applyPrimaryModel: vi.fn((cfg: AstroclawConfig, model: string) => ({
    ...cfg,
    agents: {
      ...cfg.agents,
      defaults: {
  ...
  applyModelAllowlist: vi.fn(
    (cfg: AstroclawConfig, models: string[], opts: { scopeKeys?: string[] } = {}) => {
  ...
  applyModelFallbacksFromSelection: vi.fn(
    (cfg: AstroclawConfig, selection: string[], opts: { scopeKeys?: string[] } = {}) => {
      const defaults = cfg.agents?.defaults;
