...
const mocks = vi.hoisted(() => ({
  ...
  resolveAstroclawWrapperPath: vi.fn(),
}));
...
vi.mock("../daemon/program-args.js", () => ({
  ...
  resolveAstroclawWrapperPath: mocks.resolveAstroclawWrapperPath,
}));
...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    expect(resolveGatewayDevMode(["node", "/Users/me/astroclaw/src/cli/index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\astroclaw\\src\\cli\\index.ts"])).toBe(
      true,
    );
    expect(resolveGatewayDevMode(["node", "/Users/me/astroclaw/dist/cli/index.js"])).toBe(false);
  });
});
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { ASTROCLAW_PORT: "3000" },
  } = params;
  const workingDirectory = Object.hasOwn(params, "workingDirectory")
    ? params.workingDirectory
    : "/Users/me";
  mocks.resolvePreferredNodePath.mockResolvedValue("/opt/node");
  mocks.resolveAstroclawWrapperPath.mockImplementation(async (value: string | undefined) =>
...
describe("buildGatewayInstallPlan", () => {
  // Prevent tests from reading the developer's real ~/.astroclaw/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ ASTROCLAW_PORT: "3000" });
  ...
  it("uses the state dir as the default macOS launchd working directory", async () => {
    ...
    expect(plan.workingDirectory).toBe(path.join(isolatedHome, ".astroclaw"));
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
  ...
  it("passes ASTROCLAW_WRAPPER through program args and managed service env", async () => {
    const wrapperPath = path.resolve("/usr/local/bin/astroclaw-doppler");
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        ASTROCLAW_PORT: "3000",
    ...
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
    expect(
      firstMockArg(mocks.buildServiceEnvironment, "buildServiceEnvironment").env?.ASTROCLAW_WRAPPER,
    ).toBe(wrapperPath);
    expect(plan.environment.ASTROCLAW_WRAPPER).toBe(wrapperPath);
  });
  ...
  it("tracks safe config env keys without embedding literal values", async () => {
    ...
    expect(plan.environment.ASTROCLAW_PORT).toBe("3000");
    expect(plan.environment.ASTROCLAW_SERVICE_MANAGED_ENV_KEYS).toBe(
  ...
  it("includes env SecretRef values from config into the service environment", async () => {
    ...
    expect(plan.environment.ASTROCLAW_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("allows safe inherited passEnv names while blocking dangerous exec SecretRef env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        BASH_ENV: "/tmp/astroclaw-test-bashenv",
        XDG_CONFIG_HOME: "/tmp/astroclaw-test-xdg-home",
  ...
  it("does not embed gateway auth SecretRef values into the service environment", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        ASTROCLAW_GATEWAY_TOKEN: "gateway-test-token",
      }),
      port: 3000,
      runtime: "node",
      config: {
    ...
    expect(plan.environment.ASTROCLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environment.ASTROCLAW_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("does not inline config env SecretRef values already backed by state-dir dotenv", async () => {
    await writeStateDirDotEnv("DISCORD_BOT_TOKEN=discord-dotenv-token\n", {
      stateDir: path.join(isolatedHome, ".astroclaw"),
    });
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        ASTROCLAW_PORT: "3000",
    ...
    expect(plan.environment.ASTROCLAW_SERVICE_MANAGED_ENV_KEYS).toBe("DISCORD_BOT_TOKEN");
  });
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("tracks .env vars with config while preserving service precedence", async () => {
    await writeStateDirDotEnv(
      ...
      {
        stateDir: path.join(tmpDir, ".astroclaw"),
      },
    );
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
  ...
  it("retains managed .env values for macOS LaunchAgent env files", async () => {
    ...
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        HOME: "/from-service",
        ASTROCLAW_LAUNCHD_LABEL: "ai.astroclaw.gateway",
        ASTROCLAW_PORT: "3000",
      },
    });
  ...
  it("works when .env file does not exist", async () => {
    mockNodeGatewayPlanFixture({ serviceEnvironment: { ASTROCLAW_PORT: "3000" } });
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.ASTROCLAW_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        ASTROCLAW_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
  ...
  it("drops legacy inline env values when the key is now managed by .env", async () => {
    ...
    expect(plan.environment.ASTROCLAW_SERVICE_MANAGED_ENV_KEYS).toBe("TAVILY_API_KEY");
  ...
  it("keeps source metadata for EnvironmentFile-backed preserved vars", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        CUSTOM_TOOL_HOME: "/Users/test/.custom-tool",
        ASTROCLAW_GATEWAY_TOKEN: "old-token",
      },
    ...
    expect(plan.environment.ASTROCLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environmentValueSources?.ASTROCLAW_GATEWAY_TOKEN).toBeUndefined();
  });
  ...
  it("does not embed auth-profile env refs when the key is already durable", async () => {
    ...
    expect(plan.environment.ASTROCLAW_SERVICE_MANAGED_ENV_KEYS).toBe("OPENAI_API_KEY");
  });
});
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:astroclaw|astroclaw)( --profile isolated)? gateway install/,
    );
  });
});
