...
function resolveSandboxDockerImage(cfg: AstroclawConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.docker?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_IMAGE;
}

function resolveSandboxBackend(cfg: AstroclawConfig): string {
  const backend = cfg.agents?.defaults?.sandbox?.backend?.trim();
  return backend || "docker";
}

function resolveSandboxBrowserImage(cfg: AstroclawConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.browser?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_BROWSER_IMAGE;
}

function updateSandboxDockerImage(cfg: AstroclawConfig, image: string): AstroclawConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
function updateSandboxBrowserImage(cfg: AstroclawConfig, image: string): AstroclawConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
export async function maybeRepairSandboxImages(
  ...
  if (!dockerAvailable) {
    const lines = [
      ...
      "- Disable sandbox mode: astroclaw config set agents.defaults.sandbox.mode off",
    ];
    note(lines.join("\n"), "Sandbox");
    return cfg;
  }
  ...
  if (!prompter.shouldRepair) {
    note(
      [
        ...
        `Run ${formatCliCommand("astroclaw doctor --fix")} to migrate them to sharded registry files.`,
      ].join("\n"),
      "Sandbox",
    );
    return;
...
export function noteSandboxScopeWarnings(cfg: AstroclawConfig) {
