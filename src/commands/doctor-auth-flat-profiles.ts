...
function listAuthProfileRepairCandidates(cfg: AstroclawConfig): AuthProfileRepairCandidate[] {
...
function ensureConfigAuthProfiles(config: AstroclawConfig): Record<string, AuthProfileConfig> {
...
export async function maybeRepairLegacyFlatAuthProfileStores(params: {
  ...
  const noteLines = [
    ...
    ...awsSdkMarkerStores.map(
      (entry) =>
        `- ${shortenHomePath(entry.authPath)} contains aws-sdk profile markers that belong in astroclaw.json auth.profiles.`,
    ),
  ];
  if (legacyStores.length > 0) {
    noteLines.push(
      `- The gateway expects the canonical version/profiles store; ${formatCliCommand("astroclaw doctor --fix")} rewrites this legacy shape with a backup.`,
    );
  }
  if (awsSdkMarkerStores.length > 0) {
    noteLines.push(
      `- AWS SDK profile markers are routing metadata, not stored credentials; ${formatCliCommand("astroclaw doctor --fix")} moves them to config with a backup.`,
    );
  }
  note(noteLines.join("\n"), "Auth profiles");
