...
function hasExplicitMemorySearchConfig(cfg: AstroclawConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: AstroclawConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: AstroclawConfig, agentId: string) => { store: { path: string } } | null;
