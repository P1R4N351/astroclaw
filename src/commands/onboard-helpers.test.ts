...
describe("handleReset", () => {
  it("uses active profile paths for destructive reset targets", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-reset-profile-"));
    const profileStateDir = path.join(homeDir, ".astroclaw-work");
    const defaultStateDir = path.join(homeDir, ".astroclaw");
    const profileConfigPath = path.join(profileStateDir, "astroclaw.json");
    ...
    vi.stubEnv("ASTROCLAW_HOME", homeDir);
    vi.stubEnv("ASTROCLAW_PROFILE", "work");
    vi.stubEnv("ASTROCLAW_STATE_DIR", profileStateDir);
    vi.stubEnv("ASTROCLAW_CONFIG_PATH", profileConfigPath);
