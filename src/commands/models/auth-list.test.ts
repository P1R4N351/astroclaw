...
vi.mock("../../agents/agent-scope.js", () => ({
  resolveAgentDir: (_cfg: AstroclawConfig, agentId: string) => `/tmp/astroclaw/agents/${agentId}`,
...
describe("modelsAuthListCommand", () => {
  beforeEach(() => {
    mocks.loadModelsConfig.mockReset().mockResolvedValue({} as AstroclawConfig);
    ...
    expect(runtime.jsonPayloads).toStrictEqual([
      {
        ...
        agentId: "coder",
        authStatePath: "/tmp/astroclaw/agents/coder/auth-state.json",
        profiles: [
          {
    ...
    expect(runtime.jsonPayloads).toStrictEqual([
      {
        ...
        agentId: "main",
        authStatePath: "/tmp/astroclaw/agents/main/auth-state.json",
        profiles: [
          {
  ...
  it("prints an empty profile list without failing", async () => {
    ...
    expect(runtime.logs).toEqual([
      "Agent: main",
      "Auth state file: /tmp/astroclaw/agents/main/auth-state.json",
      "Profiles: (none)",
    ]);
  });
});
