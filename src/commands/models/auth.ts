...
function resolveRequestedProviderOrThrow(
  ...
  throw new Error(
    `Unknown provider "${requested}". Loaded providers: ${availableText}. Verify plugins via \`${formatCliCommand("astroclaw plugins list --json")}\`.`,
  );
}
...
export async function modelsAuthSetupTokenCommand(
  ...
  if (!process.stdin.isTTY) {
    throw new Error(
      `setup-token requires an interactive TTY. In automation, use ${formatCliCommand("astroclaw models auth paste-token --provider <provider>")} instead.`,
    );
  }
  ...
  if (tokenProviders.length === 0) {
    throw new Error(
      `No provider token-auth plugins found. Install one via \`${formatCliCommand("astroclaw plugins install")}\`.`,
    );
  }
  ...
  if (!provider) {
    throw new Error(
      `No token-capable provider is available. Run ${formatCliCommand("astroclaw plugins list")} to verify provider plugins are installed.`,
    );
  }
...
export async function modelsAuthPasteTokenCommand(
  ...
  if (!rawProvider) {
    throw new Error(
      `Missing --provider. Run ${formatCliCommand("astroclaw models status")} or ${formatCliCommand("astroclaw plugins list")} to choose a provider.`,
    );
  }
  ...
  if (provider === "anthropic") {
    runtime.log("Anthropic setup-token auth is supported in Astroclaw.");
    runtime.log("Astroclaw prefers Claude CLI reuse when it is available on the host.");
    runtime.log("Anthropic staff told us this Astroclaw path is allowed again.");
  }
}
...
export async function modelsAuthAddCommand(opts: { agent?: string }, runtime: RuntimeEnv) {
  ...
  if (providerPlugin) {
    ...
    if (methodId !== "paste") {
      ...
      if (!method) {
        throw new Error(
          `Unknown token auth method "${methodId}". Run ${formatCliCommand("astroclaw models auth login --provider " + providerPlugin.id)} to choose interactively.`,
        );
      }
      await runProviderAuthMethod({
        config,
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  if (!process.stdin.isTTY) {
    throw new Error(
      `models auth login requires an interactive TTY. In automation, use ${formatCliCommand("astroclaw models auth paste-token --provider <provider>")} when token auth is available.`,
    );
  }
  ...
  if (authProviders.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("astroclaw plugins install")}\`.`,
    );
  }
  ...
  if (!selectedProvider) {
    throw new Error(
      `Unknown provider. Run ${formatCliCommand("astroclaw models status")} or ${formatCliCommand("astroclaw plugins list")} to see available provider plugins.`,
    );
  }
  ...
  if (!chosenMethod) {
    throw new Error(
      `Unknown auth method. Run ${formatCliCommand("astroclaw models auth login --provider " + selectedProvider.id)} without --method to choose interactively.`,
    );
  }
