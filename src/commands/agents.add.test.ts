...
describe("agents add command", () => {
  ...
  it("copies only portable auth profiles when seeding a new agent store", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-agents-add-auth-copy-"));
    try {
  ...
  it("copies portable Codex OAuth profiles without inline token material", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-agents-add-oauth-copy-"));
