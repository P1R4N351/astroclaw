...
import {
  ASTROCLAW_WRAPPER_ENV_KEY,
  resolveGatewayProgramArguments,
  resolveAstroclawWrapperPath,
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperPath = await resolveAstroclawWrapperPath(
    params.wrapperPath ?? params.env[ASTROCLAW_WRAPPER_ENV_KEY],
  );
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [ASTROCLAW_WRAPPER_ENV_KEY]: wrapperPath }
    : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.ASTROCLAW_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("astroclaw gateway install")}\` after fixing the error.`;
}
