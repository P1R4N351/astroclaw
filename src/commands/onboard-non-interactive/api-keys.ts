...
export async function resolveNonInteractiveApiKey(params: {
  ...
  params.runtime.error(
    `Missing ${params.flagName} (or ${params.envVar} in env${profileHint}). Export ${params.envVar}, pass ${params.flagName}, or run ${formatCliCommand("astroclaw onboard")} for interactive setup.`,
  );
  params.runtime.exit(1);
  return null;
}
