...
const SAMPLE_SECRET_REF = {
  source: "env" as const,
  provider: "default",
  id: "ASTROCLAW_GATEWAY_TOKEN_REF",
};

function createTokenConfig(token: unknown): AstroclawConfig {
  return {
...
describe("applyNonInteractiveGatewayConfig token resolution chain", () => {
  const originalEnvToken = process.env.ASTROCLAW_GATEWAY_TOKEN;
  ...
  afterEach(() => {
    if (originalEnvToken === undefined) {
      ...
      process.env.ASTROCLAW_GATEWAY_TOKEN = originalEnvToken;
    }
  ...
  it("prefers existing plaintext token over ambient ASTROCLAW_GATEWAY_TOKEN on re-onboard", () => {
    // A stale shell/launchd ASTROCLAW_GATEWAY_TOKEN must not rotate a
    // persisted token — that would break already-paired clients.
    process.env.ASTROCLAW_GATEWAY_TOKEN = "stale-env-token";
    const nextConfig = createTokenConfig("existing-user-token");
  ...
  it("uses ASTROCLAW_GATEWAY_TOKEN to fill an empty config on first-run", () => {
  ...
  it("preserves an existing SecretRef even when ambient ASTROCLAW_GATEWAY_TOKEN is set", () => {
  ...
  it("overrides an existing SecretRef when --gateway-token-ref-env is provided", () => {
    const newRefId = "ASTROCLAW_GATEWAY_TOKEN_NEW_REF";
  ...
  it("fails when --gateway-token-ref-env points to a missing env var", () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      'Environment variable "MISSING_GATEWAY_TOKEN_ENV" is missing or empty. Export it first, then rerun astroclaw onboard --non-interactive.',
    );
