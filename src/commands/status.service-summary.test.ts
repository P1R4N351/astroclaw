...
describe("readServiceStatusSummary", () => {
  it("marks Astroclaw-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["astroclaw", "gateway", "run"] })),
    ...
    expect(summary.managedByAstroclaw).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByAstroclaw).toBe(false);
  ...
  it("passes command environment to runtime and loaded checks", async () => {
    const isLoaded = vi.fn(async ({ env }: GatewayServiceEnvArgs) => {
      return env?.ASTROCLAW_GATEWAY_PORT === "18789";
    });
    const readRuntime = vi.fn(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.ASTROCLAW_GATEWAY_PORT === "18789" ? ("running" as const) : ("unknown" as const),
    }));
    ...
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({
          programArguments: ["astroclaw", "gateway", "run", "--port", "18789"],
    ...
    expect(loadedArgs?.env?.ASTROCLAW_GATEWAY_PORT).toBe("18789");
    const runtimeEnv = requireMockArg(readRuntime, "readRuntime") as NodeJS.ProcessEnv;
    expect(runtimeEnv?.ASTROCLAW_GATEWAY_PORT).toBe("18789");
    expect(summary.installed).toBe(true);
  ...
  it("includes service layout diagnostics and flags source checkout entrypoints", async () => {
    await withTempDir({ prefix: "astroclaw-status-service-layout-" }, async (root) => {
      ...
      const serviceFile = path.join(root, "astroclaw-gateway.service");
      ...
      expect(layout.sourcePathReal).toBe(path.join(realRoot, "astroclaw-gateway.service"));
