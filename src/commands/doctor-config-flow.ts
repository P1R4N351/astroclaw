...
function collectConfiguredChannelIds(cfg: AstroclawConfig): string[] {
  const channels =
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: AstroclawConfig = baseCfg;
  let candidate = structuredClone(baseCfg);
  let pendingChanges = false;
  let fixHints: string[] = [];
  const doctorFixCommand = formatCliCommand("astroclaw doctor --fix");
  ...
  if (hasLegacyInternalHookHandlers(snapshot.parsed)) {
    note(
      [
        ...
        "- astroclaw doctor --fix does not rewrite this shape automatically.",
      ].join("\n"),
      "Legacy config keys detected",
    );
  }
