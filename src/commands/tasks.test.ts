...
import type { AstroclawTestState } from "../test-utils/astroclaw-test-state.js";
...
async function withTaskCommandStateDir(
  run: (state: AstroclawTestState) => Promise<void>,
): Promise<void> {
  await withAstroclawTestState(
    { layout: "state-only", prefix: "astroclaw-tasks-command-" },
    async (state) => {
      resetTaskRegistryDeliveryRuntimeForTests();
