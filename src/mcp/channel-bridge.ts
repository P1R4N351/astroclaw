...
export class AstroclawChannelBridge {
  ...
  constructor(
    private readonly cfg: AstroclawConfig,
  ...
  async start(): Promise<void> {
    ...
    this.gateway = new GatewayClientCtor({
      ...
      clientDisplayName: "Astroclaw MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`astroclaw mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `astroclaw mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
