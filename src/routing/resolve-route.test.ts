...
const resolveRoute = (
  params: Omit<Parameters<typeof resolveAgentRoute>[0], "cfg"> & { cfg?: AstroclawConfig },
) =>
  resolveAgentRoute({
    cfg: params.cfg ?? {},
    ...params,
...
describe("resolveAgentRoute", () => {
  ...
  const expectRouteResolutionCase = (params: {
    routeParams: Omit<Parameters<typeof resolveRoute>[0], "cfg"> & { cfg: AstroclawConfig };
    expected: ResolvedRouteExpectation;
  }) => {
  ...
  test.each([
    ...
    },
    {
      ...
      cfg: {
        agents: {
          list: [{ id: "home", default: true, workspace: "~/astroclaw-home" }],
        },
      } satisfies AstroclawConfig,
      channel: "whatsapp" as const,
      accountId: "biz",
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<AstroclawConfig["bindings"]>[number];
