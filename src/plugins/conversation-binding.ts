...
const APPROVALS_PATH = "~/.astroclaw/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "astroclaw-app-server:thread:",
  "astroclaw-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("astroclaw.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("astroclaw.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Astroclaw instead. If this started after an update, run "astroclaw doctor --fix"; otherwise reinstall or enable the plugin.${buildDetachHintSuffix(binding.detachHint)}`;
}
