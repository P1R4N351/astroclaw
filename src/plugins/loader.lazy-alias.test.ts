...
describe("native plugin alias preparation", () => {
  ...
  it.each(["cjs", "mjs"])(
    ...
    (extension) => {
      ...
      const entry = writeFile(
        ...
        extension === "cjs"
          ? 'module.exports = require("@astroclaw/plugin-sdk/used");'
          : 'export { value } from "@astroclaw/plugin-sdk/used";',
      );
      const read = vi.spyOn(fs, "readFileSync");
      const load = createPluginModuleLoader({ devSourceRoot: f.root });
  ...
  it("does not prepare aliases for unrelated requests or unregistered parents", () => {
    ...
    expect(() => createRequire(outside.entry).resolve("@astroclaw/plugin-sdk/used")).toThrow();
    expect(read.mock.calls.filter(([target]) => target === f.unused)).toEqual([]);
  });
  ...
  it.each([false, true])(
    ...
    (resolveFirst) => {
      ...
      if (resolveFirst) {
        expect(requirePlugin.resolve("@astroclaw/plugin-sdk/used")).toBe(a.used);
      }
      ...
      fs.rmSync(b.unused);
      installOpenClawPluginSdkNativeResolver({ pluginModulePath: pluginEntry });
      expect(requirePlugin.resolve("@astroclaw/plugin-sdk/used")).toBe(b.used);
      expect(() => requirePlugin.resolve("@astroclaw/plugin-sdk/unused")).toThrow();
      argv.mockRestore();
    },
  );
  ...
  it.each(["argv", "cwd", "module-url"])(
    ...
    (hint) => {
      ...
      const entry = writeFile(
        external,
        "index.ts",
        'import { value } from "@astroclaw/plugin-sdk/used"; export const marker: string = value;',
      );
  ...
  it("captures private QA denial before late use even if ambient authorization changes", () => {
    ...
    expect(() => metadata.load("@astroclaw/plugin-sdk/qa-runtime")).toThrow();
    installOpenClawPluginSdkNativeResolver({ pluginModulePath: f.entry, devSourceRoot: f.root });
    expect(metadata.load("@astroclaw/plugin-sdk/qa-runtime")).toMatchObject({ privateValue: true });
  });
  ...
  it("does not reuse a bundled private alias grant for an external plugin", () => {
    ...
    installOpenClawPluginSdkNativeResolver({ pluginModulePath: f.entry, devSourceRoot: f.root });
    expect(createRequire(f.entry)("@astroclaw/plugin-sdk/demoted-helper")).toMatchObject({
      privateValue: true,
    });
    installOpenClawPluginSdkNativeResolver({ pluginModulePath: external, devSourceRoot: f.root });
    expect(() => createRequire(external).resolve("@astroclaw/plugin-sdk/demoted-helper")).toThrow();
  });
  ...
  it.each([false, true])(
    ...
    (authorized) => {
      ...
      const resolve = () =>
        createRequire(entry).resolve("@astroclaw/plugin-sdk/ssrf-runtime-internal");
      if (authorized) {
    ...
    const entry = writeFile(
      ...
      api.registerCli(({ program }) => program.command("late").action(async () => {
        const results = await Promise.allSettled([
          ...
          import("@astroclaw/plugin-sdk/unused.js"),
        ]);
  ...
  it("keeps deferred aliases in their owner and acquires changed facts only in a new operation", () => {
    ...
    withPluginCache(createPluginCache(), () => {
      installOpenClawPluginSdkNativeResolver({ pluginModulePath: f.entry, devSourceRoot: f.root });
      expect(() => createRequire(f.entry).resolve("@astroclaw/plugin-sdk/unused")).toThrow();
    });
  });
