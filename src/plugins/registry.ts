...
import type {
  ...
  AstroclawPluginHttpRouteParams,
...
function isOfficialCodexPluginRecord(
  ...
  if (record.packageName === "@astroclaw/codex") {
    return true;
  }
  const sourcePath = path
    ...
    .split(path.sep)
    .join("/");
  return sourcePath.includes("/node_modules/@astroclaw/codex");
}
...
const ACTIVE_PLUGIN_HOOK_REGISTRATIONS_KEY = Symbol.for("astroclaw.activePluginHookRegistrations");
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerCodexAppServerExtensionFactory = (
    record: PluginRecord,
    factory: Parameters<AstroclawPluginApi["registerCodexAppServerExtensionFactory"]>[0],
  ) => {
  ...
  const registerAgentToolResultMiddleware = (
    record: PluginRecord,
    handler: Parameters<AstroclawPluginApi["registerAgentToolResultMiddleware"]>[0],
    options: Parameters<AstroclawPluginApi["registerAgentToolResultMiddleware"]>[1],
  ) => {
    if (record.origin !== "bundled") {
      pushDiagnostic({
        level: "error",
  ...
  const registerTool = (
    ...
    const factory: AstroclawPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: AstroclawPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: AstroclawPluginHookOptions | undefined,
    config: AstroclawPluginApi["config"],
  ...
  const registerHttpRoute = (record: PluginRecord, params: AstroclawPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: AstroclawPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const registrationCapabilities = resolvePluginRegistrationCapabilities(mode);
    const normalized =
      typeof (registration as AstroclawPluginChannelRegistration).plugin === "object"
        ? (registration as AstroclawPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    const descriptors = (opts?.descriptors ?? [])
      ...
      .filter(
        (descriptor): descriptor is AstroclawPluginCliCommandDescriptor => descriptor !== null,
      );
    const commands = [
      ...(opts?.commands ?? []),
  ...
  const registerReload = (record: PluginRecord, registration: AstroclawPluginReloadRegistration) => {
    const normalize = (values?: string[]) =>
      (values ?? []).map((value) => value.trim()).filter(Boolean);
    const normalized: AstroclawPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: AstroclawPluginNodeHostCommand,
  ) => {
  ...
  const registerNodeInvokePolicy = (
    record: PluginRecord,
    policy: AstroclawPluginNodeInvokePolicy,
  ...
  const registerService = (record: PluginRecord, service: AstroclawPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: AstroclawPluginCommandDefinition) => {
      ...
      handlers: {
        ...(registrationCapabilities.capabilityHandlers
          ? {
              ...
              registerCompactionProvider: (
                provider: Parameters<AstroclawPluginApi["registerCompactionProvider"]>[0],
              ) => {
                const id = normalizeOptionalString(
                  (
                    provider as Partial<
                      Parameters<AstroclawPluginApi["registerCompactionProvider"]>[0]
                    > | null
                  )?.id,
                );
                if (!id) {
              ...
              enqueueNextTurnInjection: (injection) => {
                ...
                return enqueuePluginNextTurnInjection({
                  cfg: registryParams.runtime.config.current() as AstroclawConfig,
                  pluginId: record.id,
                  pluginName: record.name,
                  injection,
                });
              ...
              sendSessionAttachment: async (attachment) => {
                ...
                try {
                  ...
                  const runtimeConfig =
                    (registryParams.runtime.config?.current?.() as AstroclawConfig | undefined) ??
                    params.config;
