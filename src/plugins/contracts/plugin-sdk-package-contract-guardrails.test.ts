...
const PLUGIN_SDK_SUBPATH_PATTERN = /astroclaw\/plugin-sdk\/([a-z0-9][a-z0-9-]*)\b/g;
...
function collectDeprecatedExtensionSdkImports(): Array<{ file: string; specifier: string }> {
  const leaks: Array<{ file: string; specifier: string }> = [];
  const importPatterns = [
    /\b(?:import|export)\b[\s\S]*?\bfrom\s*["'](astroclaw\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']/g,
    /\bimport\s*\(\s*["'](astroclaw\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']\s*\)/g,
    /\bvi\.(?:mock|doMock)\s*\(\s*["'](astroclaw\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']/g,
  ];
  for (const file of collectExtensionFiles(resolve(REPO_ROOT, "extensions"))) {
    const repoRelativePath = toRepoRelativePath(file);
    const source = fs.readFileSync(file, "utf8");
...
function collectDeprecatedTestBarrelImports(): Array<{ file: string; specifier: string }> {
  const leaks: Array<{ file: string; specifier: string }> = [];
  const importPatterns = [
    /\b(?:import|export)\b[\s\S]*?\bfrom\s*["'](astroclaw\/plugin-sdk\/(?:testing|test-utils))["']/g,
    /\bimport\s*\(\s*["'](astroclaw\/plugin-sdk\/(?:testing|test-utils))["']\s*\)/g,
    /\bvi\.(?:mock|doMock)\s*\(\s*["'](astroclaw\/plugin-sdk\/(?:testing|test-utils))["']/g,
  ];
...
function collectCrossOwnerReservedSdkImports(): Array<{
  ...
  for (const file of collectExtensionFiles(resolve(REPO_ROOT, "extensions"))) {
    ...
    for (const match of source.matchAll(importPattern)) {
      ...
      leaks.push({
        file: repoRelativePath,
        specifier: `astroclaw/plugin-sdk/${subpath}`,
        owner,
      });
    }
  }
    ...
    for (const importPattern of importPatterns) {
      for (const match of source.matchAll(importPattern)) {
        ...
        if (subpath && subpaths.has(subpath)) {
          imports.add(`${repoRelativePath}: astroclaw/plugin-sdk/${subpath}`);
        }
      }
    }
  }
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it("keeps curated public plugin-sdk references on exported built subpaths", () => {
    ...
    for (const reference of collectPluginSdkSubpathReferences()) {
      ...
      failures.push(
        `${reference.file} references astroclaw/plugin-sdk/${reference.subpath}, but ${reference.subpath} is missing from ${missingFrom.join(" and ")}`,
      );
    }
  ...
  it("keeps Matrix dependencies local to the Matrix plugin", () => {
    ...
    expect(rootRuntimeDeps.has("@astroclaw/plugin-package-contract")).toBe(false);
  });
