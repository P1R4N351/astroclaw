...
describe("resolveRuntimePluginRegistry", () => {
  ...
  it("keeps the full workspace registry warm when scoped cron registries churn", () => {
    ...
    try {
      ...
      const fullRegistry = loadAstroclawPlugins(loadOptions);

      loadAstroclawPlugins({ ...loadOptions, onlyPluginIds: ["alpha"] });
      loadAstroclawPlugins({ ...loadOptions, onlyPluginIds: ["bravo"] });
...
describe("loadAstroclawPlugins active runtime clearing", () => {
  it("clears plugin-owned global providers before activating a new registry", () => {
    ...
    loadAstroclawPlugins({ onlyPluginIds: [] });
...
describe("clearPluginRegistryLoadCache", () => {
  ...
  it("invalidates full-workspace load snapshots", () => {
    ...
    const registry = loadAstroclawPlugins(loadOptions);

    clearPluginRegistryLoadCache();

    expect(loadAstroclawPlugins(loadOptions)).not.toBe(registry);
  });
});
