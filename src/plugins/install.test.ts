...
vi.mock("../infra/astroclaw-root.js", () => ({
  resolveAstroclawPackageRootSync: vi.fn(),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("astroclaw-plugin-install");
...
function setPluginMinHostVersion(pluginDir: string, minHostVersion: string) {
  const packageJsonPath = path.join(pluginDir, "package.json");
  const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
    astroclaw?: { install?: Record<string, unknown> };
  };
  manifest.astroclaw = {
    ...manifest.astroclaw,
    install: {
      ...manifest.astroclaw?.install,
      minHostVersion,
    },
  };
  fs.writeFileSync(packageJsonPath, JSON.stringify(manifest), "utf-8");
...
async function ensureDynamicArchiveTemplate(params: {
  ...
  if (params.writePluginManifest !== false) {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "astroclaw.plugin.json"),
      JSON.stringify({
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@astroclaw/cognee-astroclaw",
      version: "0.0.1",
      astroclaw: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "astroclaw.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs package archive runtime dependencies", async () => {
    ...
    expect(commandOptions.cwd).toContain(".astroclaw-install-stage-");
  });
  ...
  it("installs scoped archives, rejects duplicate installs, and allows updates", async () => {
    ...
    expectSuccessfulArchiveInstall({ result: first, stateDir, pluginId: "@astroclaw/voice-call" });
  ...
  it("rejects native plugin zip archives without astroclaw.plugin.json", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package missing valid astroclaw.plugin.json");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_PLUGIN_MANIFEST);
    }
    expect(fs.existsSync(resolvePluginInstallDir("@astroclaw/zipper", extensionsDir))).toBe(false);
  });
  ...
  it("rejects packages without astroclaw.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@astroclaw/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("astroclaw.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_ASTROCLAW_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when astroclaw.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@astroclaw/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing astroclaw.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_ASTROCLAW_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without astroclaw.extensions");
  });

  it("rejects package installs when astroclaw.extensions entries escape the package", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_ASTROCLAW_EXTENSIONS);
  ...
  it("allows missing TypeScript source entries when an inferred built runtime entry exists", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        astroclaw: { extensions: ["./src/index.ts"] },
      }),
    );
    fs.writeFileSync(path.join(pluginDir, "dist", "index.js"), "export {};\n");
  ...
  it("rejects package installs when astroclaw.extensions contains a blank entry", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
    ...
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_ASTROCLAW_EXTENSIONS);
      expect(result.error).toContain("astroclaw.extensions[1]");
  ...
  it("rejects package installs when runtimeExtensions contains a blank entry", async () => {
    ...
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_ASTROCLAW_EXTENSIONS);
      expect(result.error).toContain("astroclaw.runtimeExtensions[0]");
      expect(result.error).toContain("non-empty string");
    }
  });
  ...
  it("rejects package installs when an extension entry is a symlink escape", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        astroclaw: { extensions: ["./linked/escape.js"] },
      }),
    );
  ...
  it("rejects package installs when an extension entry is a hardlinked alias", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        astroclaw: { extensions: ["./escape.js"] },
      }),
    );
  ...
  it("still scans declared package entrypoints when they live under test-looking paths", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        astroclaw: { extensions: ["tests/runtime.test.js"] },
      }),
    );
    fs.mkdirSync(path.join(pluginDir, "tests"), { recursive: true });
    fs.writeFileSync(
  ...
  it.runIf(process.platform !== "win32")(
    "allows package installs when node_modules/astroclaw points at the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-astroclaw");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"astroclaw"}\n');
      vi.mocked(resolveAstroclawPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "astroclaw-peer-plugin");

      const nodeModulesDir = path.join(pluginDir, "node_modules");
      fs.mkdirSync(nodeModulesDir, { recursive: true });
      fs.symlinkSync(hostRoot, path.join(nodeModulesDir, "astroclaw"), "junction");
  ...
  it.runIf(process.platform !== "win32")(
    "allows package installs when node_modules/.bin/astroclaw points inside the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-astroclaw");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"astroclaw"}\n');
      const hostBin = path.join(hostRoot, "astroclaw.mjs");
      fs.writeFileSync(hostBin, "#!/usr/bin/env node\n");
      vi.mocked(resolveAstroclawPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "astroclaw-bin-peer-plugin");

      const binDir = path.join(pluginDir, "node_modules", ".bin");
      fs.mkdirSync(binDir, { recursive: true });
      fs.symlinkSync(hostBin, path.join(binDir, "astroclaw"), "file");
  ...
  it.runIf(process.platform !== "win32")(
    "fails package installs when node_modules/astroclaw points outside the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-astroclaw");
      const spoofedRoot = path.join(tmpDir, "spoofed-astroclaw");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.mkdirSync(spoofedRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"astroclaw"}\n');
      fs.writeFileSync(path.join(spoofedRoot, "package.json"), '{"name":"astroclaw"}\n');
      vi.mocked(resolveAstroclawPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "spoofed-astroclaw-peer-plugin");

      const nodeModulesDir = path.join(pluginDir, "node_modules");
      fs.mkdirSync(nodeModulesDir, { recursive: true });
      fs.symlinkSync(spoofedRoot, path.join(nodeModulesDir, "astroclaw"), "junction");
      ...
      if (!result.ok) {
        expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED);
        expect(result.error).toContain("node_modules/astroclaw");
      }
    },
  );

  it.runIf(process.platform !== "win32")(
    "fails package installs for nested or non-exact astroclaw node_modules symlinks",
    async () => {
      const cases = [
        {
          pluginName: "nested-astroclaw-peer-plugin",
          relativePath: path.join("node_modules", "vendor", "node_modules", "astroclaw"),
        },
        {
          pluginName: "uppercase-astroclaw-peer-plugin",
          relativePath: path.join("node_modules", "Astroclaw"),
        },
        {
          pluginName: "trailing-space-astroclaw-peer-plugin",
          relativePath: path.join("node_modules", "astroclaw "),
        },
      ] as const;
  ...
  it("fails package installs when manifest traversal exceeds the directory cap", async () => {
    vi.stubEnv("ASTROCLAW_INSTALL_SCAN_MAX_DIRECTORIES", "4");
  ...
  it("fails package installs when manifest traversal exceeds the depth cap", async () => {
    vi.stubEnv("ASTROCLAW_INSTALL_SCAN_MAX_DEPTH", "2");
  ...
  it("does not flag the real qa-matrix plugin as dangerous install code", async () => {
    ...
    vi.mocked(resolveAstroclawPackageRootSync).mockReturnValue(process.cwd());
    ...
    const scanResult = await installSecurityScan.scanPackageInstallSource({
      ...
      pluginId: "qa-matrix",
      packageName: "@astroclaw/qa-matrix",
  ...
  it("scans extension entry files in hidden directories", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        astroclaw: { extensions: [".hidden/index.js"] },
      }),
    );
    fs.writeFileSync(
...
describe("installPluginFromDir", () => {
  ...
  it("preserves local package manifests without dependency surgery", async () => {
    ...
    expect(manifest.devDependencies?.astroclaw).toBe("workspace:*");
  ...
  it.each([
    {
      ...
      expectedMessageIncludes: ["requires Astroclaw >=2026.3.22, but this host is 2026.3.21"],
    },
    {
      name: "rejects plugins with invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedCode: PLUGIN_INSTALL_ERROR_CODE.INVALID_MIN_HOST_VERSION,
      expectedMessageIncludes: ["invalid package.json astroclaw.install.minHostVersion"],
    },
    {
  ...
  it("uses astroclaw.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@astroclaw/cognee-astroclaw"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedPluginId: "@astroclaw/test-plugin",
      install: (pluginDir: string, extensionsDir: string) =>
        installPluginFromDir({
          dirPath: pluginDir,
          extensionsDir,
...
describe("linkAstroclawPeerDependencies (via installPluginFromDir)", () => {
  const resolveRootMock = vi.mocked(resolveAstroclawPackageRootSync);
  ...
  it("creates a node_modules/astroclaw symlink when peerDependencies declares astroclaw", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    const run = vi.mocked(runCommandWithTimeout);
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { astroclaw: "*" });
    ...
    const symlinkPath = path.join(result.targetDir, "node_modules", "astroclaw");
  ...
  it("keeps the astroclaw peer symlink when a local plugin already has dependencies", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { astroclaw: "*" }, { "is-number": "7.0.0" });
  ...
  it("does not create a symlink when peerDependencies is empty", async () => {
    ...
    const symlinkPath = path.join(nodeModulesDir, "astroclaw");
  ...
  it("is idempotent - re-installing replaces an existing symlink without error", async () => {
    ...
    const symlinkPath = path.join(second.targetDir, "node_modules", "astroclaw");
    expect(fs.lstatSync(symlinkPath).isSymbolicLink()).toBe(true);
  });

  it("rejects when resolveAstroclawPackageRootSync returns null", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    ...
    expectWarningIncludes(warnings, "Could not locate astroclaw package root");
  });
});
