...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `astroclaw qa`, for example `matrix`. */
  commandName: string;
...
// package.json "astroclaw" metadata (used for setup/catalog)
...
export type AstroclawPackageStartup = {
  /**
...
export type AstroclawPackageSetupFeatures = {
  configPromotion?: boolean;
  legacyStateMigrations?: boolean;
  legacySessionSurfaces?: boolean;
};

export type AstroclawPackageManifest = {
  extensions?: string[];
  runtimeExtensions?: string[];
  setupEntry?: string;
  runtimeSetupEntry?: string;
  setupFeatures?: AstroclawPackageSetupFeatures;
  plugin?: {
    id?: string;
    label?: string;
  };
  ...
  startup?: AstroclawPackageStartup;
};
...
} & Partial<Record<ManifestKey, AstroclawPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): AstroclawPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
...
export function resolvePackageExtensionEntries(
  ...
  const rawAstroclaw = manifest?.[MANIFEST_KEY] as unknown;
  if (rawAstroclaw === undefined || rawAstroclaw === null) {
    return { status: "missing", entries: [] };
  }
  if (!isRecord(rawAstroclaw)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json astroclaw must be an object",
    };
  }
  const raw = rawAstroclaw.extensions;
  ...
  if (!Array.isArray(raw)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json astroclaw.extensions must be an array",
    };
  }
  ...
  for (const [index, entry] of raw.entries()) {
    ...
    if (!normalized) {
      return {
        status: "invalid",
        entries: [],
        error: `package.json astroclaw.extensions[${index}] must be a non-empty string`,
      };
    }
