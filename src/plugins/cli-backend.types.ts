...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before Astroclaw consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Astroclaw should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume Astroclaw's bundle
   ...
   *
   * Use this when the backend should consume a canonical Astroclaw auth profile
   ...
   * `profile-only` treats the selected Astroclaw auth profile as the sole auth
   ...
   * as static config, such as mapping Astroclaw thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside Astroclaw's tool
