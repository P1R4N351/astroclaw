...
import {
  ...
  installedPackageNeedsAstroclawPeerLinkRepair,
...
import { linkAstroclawPeerDependencies } from "./plugin-peer-link.js";
...
function isBundledVersionNewer(bundledVersion: string, installedVersion: string): boolean {
  const releaseCmp = compareAstroclawReleaseVersions(bundledVersion, installedVersion);
  if (releaseCmp !== null) {
    return releaseCmp > 0;
  }
...
function migratePluginConfigId(cfg: AstroclawConfig, fromId: string, toId: string): AstroclawConfig {
  if (fromId === toId) {
    return cfg;
  }
...
function resetDisabledPluginSlots(
  slots: NonNullable<AstroclawConfig["plugins"]>["slots"] | undefined,
  pluginId: string,
): NonNullable<AstroclawConfig["plugins"]>["slots"] | undefined {
  if (!slots) {
    return slots;
  }
  let next = slots;
...
function disablePluginConfigEntry(config: AstroclawConfig, pluginId: string): AstroclawConfig {
...
async function repairAstroclawPeerLinksForNpmInstalls(params: {
  config: AstroclawConfig;
  logger: PluginUpdateLogger;
}): Promise<boolean> {
  let repaired = false;
  for (const [pluginId, record] of Object.entries(params.config.plugins?.installs ?? {})) {
    ...
    try {
      ...
      params.logger.warn?.(
        `Could not repair astroclaw peer link for "${pluginId}" due to invalid install path: ${String(err)}`,
      );
      continue;
    }

    if (!installedPackageNeedsAstroclawPeerLinkRepair(installPath)) {
      continue;
    }
    ...
    try {
      ...
      if (peerLinkRepair.skipped > 0) {
        params.logger.warn?.(
          `Could not repair astroclaw peer link for "${pluginId}" at ${installPath}: ${warnings.join("; ") || "peer link repair was skipped"}`,
        );
        continue;
      }
      repaired = !installedPackageNeedsAstroclawPeerLinkRepair(installPath) || repaired;
    } catch (err) {
      params.logger.warn?.(
        `Could not repair astroclaw peer link for "${pluginId}" at ${installPath}: ${String(err)}`,
      );
    }
  }
  return repaired;
...
export async function updateNpmInstalledPlugins(params: {
  ...
  const recordFailure = (pluginId: string, message: string) => {
    if (params.disableOnFailure && !params.dryRun) {
      const disabledMessage =
        `Disabled "${pluginId}" after plugin update failure; Astroclaw will continue without it. ` +
        message;
    ...
    if (!params.dryRun && record.source === "npm" && currentVersion) {
      ...
      if (metadataResult.ok) {
        if (
          ...
          !installedPackageNeedsAstroclawPeerLinkRepair(installPath) &&
  ...
  if (ranNpmInstaller) {
    changed =
      (await repairAstroclawPeerLinksForNpmInstalls({
        config: next,
        logger,
      })) || changed;
  }
