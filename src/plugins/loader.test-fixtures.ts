...
export type PluginLoadConfig = NonNullable<Parameters<typeof loadAstroclawPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadAstroclawPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "astroclaw-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.ASTROCLAW_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.ASTROCLAW_DISABLE_BUNDLED_PLUGINS;
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".astroclaw", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ ASTROCLAW_STATE_DIR: stateDir, ...params.env }, () =>
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.ASTROCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.ASTROCLAW_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.ASTROCLAW_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
