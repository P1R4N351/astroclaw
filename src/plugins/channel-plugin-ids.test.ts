...
function withManifestLoadPaths<T extends { id: string }>(
  ...
  return {
    ...
    manifestPath: `/tmp/plugins/${plugin.id}/astroclaw.plugin.json`,
    skills: [],
    hooks: [],
    ...plugin,
  };
...
describe("resolveGatewayStartupPluginIds", () => {
  beforeEach(() => {
    listPotentialConfiguredChannelIds.mockReset().mockImplementation((config: AstroclawConfig) => {
    ...
    listPotentialConfiguredChannelPresenceSignals
      .mockReset()
      .mockImplementation((config: AstroclawConfig) => {
        return listPotentialConfiguredChannelIds(config).map((channelId: string) => ({
          channelId,
          source: "config",
        }));
      });
    hasPotentialConfiguredChannels.mockReset().mockImplementation((config: AstroclawConfig) => {
      if (Object.prototype.hasOwnProperty.call(config, "channels")) {
  ...
  it("does not treat persisted auth alone as gateway startup intent", () => {
    ...
    expectStartupPluginIdsCase({
      ...
      env: createPluginPlanningTestEnv({
        ASTROCLAW_STATE_DIR: "/tmp/astroclaw-with-persisted-demo-channel",
      }),
      expected: ["browser", "memory-core"],
    });
  });
  ...
  it("ignores env runtime overrides during startup planning", () => {
    expectStartupPluginIdsCase({
      ...
      env: { ASTROCLAW_AGENT_RUNTIME: "codex" },
      expected: ["demo-channel", "browser", "memory-core"],
    });
  });
