...
const mocks = vi.hoisted(() => ({
  ...
  withBundledPluginAllowlistCompat: vi.fn(
    ({ config, pluginIds }: { config?: AstroclawConfig; pluginIds: string[] }) =>
      ({
        ...config,
        plugins: {
          ...config?.plugins,
...
function expectManifestRegistryLoad(index: number, config: AstroclawConfig | Record<string, never>) {
...
function expectBundledCompatLoadPath(params: {
  cfg: AstroclawConfig;
  allowlistCompat: AstroclawConfig;
  enablementCompat: {
    plugins: {
      allow?: string[];
...
function createCompatChainConfig() {
  const cfg = { plugins: { allow: ["custom-plugin"] } } as AstroclawConfig;
...
describe("resolvePluginCapabilityProviders", () => {
  ...
  it("merges enabled generation providers missing from the active registry", () => {
    ...
    const providers = resolvePluginCapabilityProviders({
      key: "imageGenerationProviders",
      cfg: { plugins: { allow: ["fal", "xai"] } } as AstroclawConfig,
    });
  ...
  it("keeps active capability providers when cfg has no explicit plugin config", () => {
    ...
    const providers = resolvePluginCapabilityProviders({
      key: "speechProviders",
      cfg: { messages: { tts: { provider: "acme" } } } as AstroclawConfig,
    });
  ...
  it("loads requested realtime voice providers missing from active registry", () => {
    ...
    const provider = resolvePluginCapabilityProvider({
      ...
      providerId: "google",
      cfg: { plugins: { allow: ["openai", "google"] } } as AstroclawConfig,
    });
  ...
  it("does not resolve non-speech capability providers when plugins are globally disabled", () => {
    const cfg = { plugins: { enabled: false, allow: ["custom-plugin"] } } as AstroclawConfig;
    const active = createEmptyPluginRegistry();
    active.mediaUnderstandingProviders.push({
      pluginId: "openai",
      pluginName: "openai",
  ...
  it("scopes media capability snapshot loads to manifest-derived bundled owners", () => {
    const cfg = { plugins: { allow: ["openai", "minimax"] } } as AstroclawConfig;
    mocks.loadPluginManifestRegistry.mockReturnValue({
      plugins: [
        {
          id: "openai",
  ...
  it("does not unscoped-load media generation capabilities without bundled owners", () => {
    const cfg = { plugins: { allow: ["openai"] } } as AstroclawConfig;
    mocks.loadPluginManifestRegistry.mockReturnValue({
      plugins: [
        {
          id: "openai",
