...
function hasPackagePluginMetadata(manifest: Record<string, unknown>): boolean {
  const astroclaw = manifest.astroclaw;
  if (!isRecord(astroclaw)) {
    return false;
  }
  const extensions = astroclaw.extensions;
...
function readManifestPluginId(packageDir: string): string | undefined {
  const manifest = readJsonObjectFileSync(path.join(packageDir, "astroclaw.plugin.json"));
