...
const ORIGINAL_ENV = {
  ...
  ASTROCLAW_BUNDLED_PLUGINS_DIR: process.env.ASTROCLAW_BUNDLED_PLUGINS_DIR,
} as const;

const tempDirs: string[] = [];

function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-model-id-normalization-"));
  tempDirs.push(dir);
  return dir;
}
...
function createCurrentSnapshot(params: {
  ...
  const index: InstalledPluginIndex = {
    ...
    plugins: [
      {
        pluginId: "normalizer",
        manifestPath: `/tmp/normalizer-${params.manifestHash}/astroclaw.plugin.json`,
...
describe("manifest model id normalization", () => {
  ...
  it("reflects manifest edits and state-dir changes on the next lookup", () => {
    ...
    process.env.ASTROCLAW_STATE_DIR = stateDirA;
    process.env.ASTROCLAW_HOME = undefined;
    process.env.ASTROCLAW_DISABLE_BUNDLED_PLUGINS = "1";
    process.env.ASTROCLAW_BUNDLED_PLUGINS_DIR = undefined;
    ...
    process.env.ASTROCLAW_STATE_DIR = stateDirB;
  ...
  it("reuses manifest metadata while file fingerprints are unchanged", () => {
    ...
    expect(listAstroclawPluginManifestMetadata(process.env)).toHaveLength(1);
    expect(listAstroclawPluginManifestMetadata(process.env)).toHaveLength(1);
