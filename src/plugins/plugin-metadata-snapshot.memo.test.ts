...
function tempStateDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-metadata-memo-"));
  tempDirs.push(dir);
  return dir;
}
...
function writePersistedIndex(params: {
  ...
  const manifestPath = params.manifestPath ?? path.join(pluginDir, "astroclaw.plugin.json");
...
function writeRecoverableNpmPlugin(params: {
  ...
  writeJson(path.join(packageDir, "astroclaw.plugin.json"), { id: params.pluginId });
}
...
function makeIndex(
  ...
  const manifestPath = options.manifestPath ?? path.join(rootDir, "astroclaw.plugin.json");
  return {
    version: 1,
    hostContractVersion: "test",
    compatRegistryVersion: "test",
...
function makeManifestRegistry(pluginId = "demo"): PluginManifestRegistry {
  const plugin: PluginManifestRecord = {
    ...
    manifestPath: `/plugins/${pluginId}/astroclaw.plugin.json`,
    origin: "global",
  };
...
describe("loadPluginMetadataSnapshot process memo", () => {
  ...
  it("reuses the expanded freshness fingerprint on hot cache hits", () => {
    const stateDir = tempStateDir();
    const manifestPath = path.join(stateDir, "extensions", "demo", "astroclaw.plugin.json");
  ...
  it.each([
    ["manifest", "astroclaw.plugin.json", "manifestPath"],
  ...
  it("does not fingerprint persisted plugin paths outside the plugin root", () => {
    const stateDir = tempStateDir();
    const outsideManifestPath = path.join(stateDir, "outside", "astroclaw.plugin.json");
