...
type LoadAstroclawPlugins = typeof import("./loader.js").loadAstroclawPlugins;
...
const loadAstroclawPluginsMock = vi.fn<LoadAstroclawPlugins>();
...
function expectLastSetupRegistryCall(params: {
  ...
  const call = getLastMockCallArg(loadAstroclawPluginsMock, "Astroclaw plugin setup loader");
...
function getLastSetupLoadedPluginConfig() {
  const call = expectRecordFields(
    getLastMockCallArg(loadAstroclawPluginsMock, "Astroclaw plugin setup loader"),
    {},
  );
...
function createAutoEnabledProviderConfig() {
  const rawConfig: AstroclawConfig = {
    plugins: {},
  };
  const autoEnabledConfig: AstroclawConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadAstroclawPlugins: (...args: Parameters<LoadAstroclawPlugins>) =>
        loadAstroclawPluginsMock(...args),
