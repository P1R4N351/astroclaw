...
describe("security audit exec surface findings", () => {
  // Redirect the Astroclaw home (ASTROCLAW_HOME wins over HOME/USERPROFILE in
  // `resolveRawHomeDir`) to a per-test tempdir so `saveExecApprovals` never
  // touches the real `~/.astroclaw/exec-approvals.json` on the host running
  // the suite.
  ...
  beforeEach(async () => {
    ...
    // ASTROCLAW_HOME takes precedence over HOME/USERPROFILE in resolveRawHomeDir,
    // so all three must point at the tempdir to neutralize whichever the host
    // happens to have set.
    process.env.ASTROCLAW_HOME = tempDir;
