...
import type { AstroclawConfig, ConfigFileSnapshot } from "../config/config.js";
...
async function listSandboxBrowserContainers(
  ...
  try {
    const result = await execDockerRawFn(
      ["ps", "-a", "--filter", "label=astroclaw.sandboxBrowser=1", "--format", "{{.Names}}"],
      { allowFailure: true },
    );
    if (result.code !== 0) {
      return null;
  ...
  try {
    const result = await params.execDockerRawFn(
      [
        "inspect",
        "-f",
        '{{ index .Config.Labels "astroclaw.configHash" }}\t{{ index .Config.Labels "astroclaw.browserConfigEpoch" }}',
...
export async function collectSandboxBrowserHashLabelFindings(params?: {
  ...
  if (missingHash.length > 0) {
    findings.push({
      ...
      remediation: `${formatCliCommand("astroclaw sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
  ...
  if (staleEpoch.length > 0) {
    findings.push({
      ...
      detail:
        ...
        `Expected astroclaw.browserConfigEpoch=${browserHashEpoch}.`,
      remediation: `${formatCliCommand("astroclaw sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
  ...
  if (nonLoopbackPublished.length > 0) {
    findings.push({
      ...
      remediation:
        `${formatCliCommand("astroclaw sandbox recreate --browser --all")} (add --force to skip prompt), ` +
  ...
  const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
    ...
    onReadError: (err) => {
      findings.push({
        ...
        remediation:
          "Check file permissions and plugin layout, then rerun `astroclaw security audit --deep`.",
      });
    },
  });
  ...
  for (const pluginName of pluginDirs) {
    ...
    try {
      ...
      findings.push({
        ...
        detail:
          ...
          "The extension entrypoint list is unavailable. Deep scan will cover the plugin directory but may miss entries declared via `astroclaw.extensions`.",
        remediation:
          "Inspect the plugin package.json for syntax errors. If the plugin is untrusted, remove it from your Astroclaw extensions state directory.",
      });
    ...
    if (escapedEntries.length > 0) {
      findings.push({
        ...
        remediation:
          "Update the plugin manifest so all astroclaw.extensions entries stay inside the plugin directory.",
      });
    }
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your Astroclaw extensions state directory.",
      });
...
export async function collectInstalledSkillsCodeSafetyFindings(params: {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const entry of entries) {
      if (resolveSkillSource(entry.skill) === "astroclaw-bundled") {
        continue;
      }
      ...
      const summary = await getCodeSafetySummary({
        ...
        findings.push({
          ...
          remediation:
            "Check file permissions and skill layout, then rerun `astroclaw security audit --deep`.",
        });
        return null;
      });
      if (!summary) {
