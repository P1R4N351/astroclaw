...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("astroclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("astroclaw security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set ASTROCLAW_GATEWAY_TOKEN/ASTROCLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("astroclaw security audit --deep")}".`,
    });
  }
  return findings;
}
