...
describe("bundled root-aware plugin lookups", () => {
  it("reads bundled channel ids from the active bundled root without re-importing", async () => {
    const rootA = makeBundledRoot("astroclaw-bundled-ids-a-");
    const rootB = makeBundledRoot("astroclaw-bundled-ids-b-");

    vi.doMock("../../plugins/channel-catalog-registry.js", () => ({
      listChannelCatalogEntries: (params?: { env?: NodeJS.ProcessEnv }) => {
        const activeRoot = params?.env?.ASTROCLAW_BUNDLED_PLUGINS_DIR;
  ...
  it("reads bootstrap plugins from the active bundled root without re-importing", async () => {
    const rootA = makeBundledRoot("astroclaw-bootstrap-a-");
    const rootB = makeBundledRoot("astroclaw-bootstrap-b-");

    vi.doMock("./bundled-ids.js", () => ({
      listBundledChannelPluginIdsForRoot: () => {
        if (process.env.ASTROCLAW_BUNDLED_PLUGINS_DIR === rootA.pluginsDir) {
          return ["alpha"];
        }
        if (process.env.ASTROCLAW_BUNDLED_PLUGINS_DIR === rootB.pluginsDir) {
          return ["beta"];
        }
        return [];
      },
    ...
    vi.doMock("./bundled.js", () => ({
      ...
      getBundledChannelSetupPlugin: (id: string) => {
        const suffix = resolveMockRootSuffix({
          activeRoot: process.env.ASTROCLAW_BUNDLED_PLUGINS_DIR,
  ...
  it("retries bootstrap plugin loading after an error", async () => {
    const root = makeBundledRoot("astroclaw-bootstrap-plugin-throw-");

    vi.doMock("./bundled-ids.js", () => ({
      listBundledChannelPluginIdsForRoot: () =>
        process.env.ASTROCLAW_BUNDLED_PLUGINS_DIR === root.pluginsDir ? ["alpha"] : [],
    }));
    ...
    process.env.ASTROCLAW_BUNDLED_PLUGINS_DIR = root.pluginsDir;
    expect(bootstrapRegistry.listBootstrapChannelPluginIds()).toEqual(["alpha"]);
  ...
  it("keeps plugin loading independent from bootstrap secrets loading errors", async () => {
    const root = makeBundledRoot("astroclaw-bootstrap-secrets-throw-");
