...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: AstroclawConfig, policy: ChannelAccessPolicy) => AstroclawConfig;
  resolveAllowlist: (params: { cfg: AstroclawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: AstroclawConfig; resolved: TResolved }) => AstroclawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: AstroclawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: AstroclawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: AstroclawConfig, policy: ChannelAccessPolicy): AstroclawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: AstroclawConfig, policy: ChannelAccessPolicy): AstroclawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: AstroclawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: AstroclawConfig; resolved: string[] }) => {
      calls.push("apply");
