...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-log-tail-redaction-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("readConfiguredLogTail redaction", () => {
  it("redacts raw auth headers before returning log lines", async () => {
    ...
    const logFile = path.join(dir, "astroclaw.log");
    const configFile = path.join(dir, "astroclaw.json");
    const basicSecret = "c2VjcmV0OnBhc3M=";
    const astroClawToken = "supersecretgatewaytoken1234567890";
    ...
    await fs.writeFile(
      ...
      [
        ...
        `X-Astroclaw-Token: ${astroClawToken}`,
        `x-pomerium-jwt-assertion: ${pomeriumJwt}`,
        "normal diagnostic line",
      ].join("\n"),
      "utf8",
    );
    process.env.ASTROCLAW_CONFIG_PATH = configFile;
    ...
    expect(text).toContain("X-Astroclaw-Token: supers…7890");
    ...
    expect(text).not.toContain(astroClawToken);
