...
function writeConfig(source: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-logging-config-"));
  tempDirs.push(dir);
...
describe("readLoggingConfig", () => {
  ...
  it("skips mutating config loads for config schema", () => {
    process.argv = ["node", "astroclaw", "config", "schema"];
  ...
  it("supports JSON5 comments and trailing commas", () => {
    writeConfig(`{
      // users commonly keep comments in astroclaw.json
      logging: {
        consoleLevel: "warn",
      },
    }`);
  ...
  it("returns undefined for missing or malformed config files", () => {
    process.env.ASTROCLAW_CONFIG_PATH = path.join(os.tmpdir(), "astroclaw-missing-config.json");
    expect(readLoggingConfig()).toBeUndefined();
