...
describe("diagnostic memory", () => {
  ...
  it("resolves session store paths only for enabled critical bundle writes", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-memory-pressure-lazy-"));
  ...
  it("can disable critical pressure bundle writes", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-memory-pressure-disabled-"));
    const resolveSessionStorePaths = vi.fn(() => []);
    try {
      startDiagnosticStabilityRecorder();
  ...
  it("leaves critical pressure bundle writes off by default", () => {
    const stateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "astroclaw-memory-pressure-default-off-"),
    );
    const resolveSessionStorePaths = vi.fn(() => []);
    try {
      startDiagnosticStabilityRecorder();
  ...
  it("writes a stability bundle when critical pressure is emitted", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-memory-pressure-"));
    const customRoot = fs.mkdtempSync(path.join(os.tmpdir(), "astroclaw-memory-custom-sessions-"));
    try {
