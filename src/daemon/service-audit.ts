...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds ASTROCLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `astroclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service ASTROCLAW_GATEWAY_TOKEN does not match gateway.auth.token in astroclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.ASTROCLAW_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.ASTROCLAW_GATEWAY_TOKEN);
}
