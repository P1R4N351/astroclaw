...
function collectGatewayServiceStartRepairIssues(
  ...
  const serviceVersion = command.environment?.ASTROCLAW_SERVICE_VERSION?.trim();
  if (serviceVersion && serviceVersion !== VERSION) {
    issues.push({
      code: "version-mismatch",
      message: `service was installed by Astroclaw ${serviceVersion}, current CLI is ${VERSION}`,
    });
  }
