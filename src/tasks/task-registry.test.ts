...
function configureTaskRegistryMaintenanceRuntimeForTest(params: {
  ...
  setTaskRegistryMaintenanceRuntimeForTests({
    ...
    resolveCronStorePath: () => "/tmp/astroclaw-test-cron/jobs.json",
...
function createAcpSessionStoreEntry(params: {
  ...
  return {
    cfg: {} as never,
    storePath: "/tmp/astroclaw-test-sessions.json",
...
async function withTaskRegistryTempDir<T>(
  ...
  return await withTempDir({ prefix: "astroclaw-task-registry-" }, async (root) => {
...
describe("task-registry", () => {
  ...
  it("does not mark codex-native subagent tasks lost when they have no Astroclaw child session", async () => {
    await withTaskRegistryTempDir(async (root) => {
      process.env.ASTROCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      const now = Date.now();
  ...
  it("does not route codex-native task cancellation through Astroclaw subagent sessions", async () => {
    await withTaskRegistryTempDir(async (root) => {
      process.env.ASTROCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      const task = createTaskRecord({
