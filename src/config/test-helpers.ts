...
import { resetConfigRuntimeState, type AstroclawConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        ASTROCLAW_MPM_CATALOG_PATHS: undefined,
        ASTROCLAW_LOAD_SHELL_ENV: undefined,
        ASTROCLAW_DEFER_SHELL_ENV_FALLBACK: undefined,
        ASTROCLAW_SHELL_ENV_TIMEOUT_MS: undefined,
...
export async function writeAstroclawConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.ASTROCLAW_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected ASTROCLAW_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeAstroclawConfig(home, config);
