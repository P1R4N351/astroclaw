...
describe("session path safety", () => {
  ...
  it("resolves transcript path inside an explicit sessions dir", () => {
    const sessionsDir = "/tmp/astroclaw/agents/main/sessions";
  ...
  it("falls back to derived path when sessionFile is outside known agent sessions dirs", () => {
    ...
    const resolved = resolveSessionFilePath(
      "sess-1",
      { sessionFile: "/tmp/astroclaw/agents/work/not-sessions/abc-123.jsonl" },
      { sessionsDir },
    );
  ...
  it("accepts symlink-alias session paths that resolve under the sessions dir", () => {
    ...
    withTempDirSync({ prefix: "astroclaw-symlink-session-" }, (tmpDir) => {
  ...
  it("falls back when sessionFile is a symlink that escapes sessions dir", () => {
    ...
    withTempDirSync({ prefix: "astroclaw-symlink-escape-" }, (tmpDir) => {
...
describe("session lifecycle timestamps", () => {
  it("falls back to the JSONL session header for legacy session start time", async () => {
    const dir = await fsPromises.mkdtemp("/tmp/astroclaw-lifecycle-test-");
    try {
...
describe("session store writer queue", () => {
  const writerFixtureRootTracker = createSuiteTempRootTracker({ prefix: "astroclaw-writer-test-" });
