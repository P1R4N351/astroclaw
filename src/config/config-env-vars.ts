...
function collectConfigEnvVarsByTarget(cfg?: AstroclawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: AstroclawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: AstroclawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.ASTROCLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
