...
import type { AstroclawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
export type ConfigWriteResult = { persistedHash: string; persistedConfig: AstroclawConfig };
export type ConfigWriteOptions = {
   ...
   */
  explicitSetValueSource?: AstroclawConfig;
  /**
...
function coerceConfig(value: unknown): AstroclawConfig {
...
function stampConfigVersion(cfg: AstroclawConfig): AstroclawConfig {
  return stampConfigWriteMetadata(cfg);
}

function warnIfConfigFromFuture(cfg: AstroclawConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    ...
    logger.warn(
      [
        `Your Astroclaw config was written by version ${touched}, but this command is running ${VERSION}.`,
        "Check: `astroclaw --version`, `which astroclaw`, and `astroclaw gateway status --deep`.",
        "If unexpected, update PATH so `astroclaw` points to the version you want, or reinstall the Gateway service from that same Astroclaw install.",
      ].join("\n"),
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as AstroclawConfig, env);
  }
...
export function createConfigIO(
  ...
  function finalizeLoadedRuntimeConfig(cfg: AstroclawConfig): AstroclawConfig {
  ...
  function ensureShippedPluginInstallConfigRecordsMigratedForWrite(
    ...
    try {
      ...
      throw new Error(
        `Config write blocked: shipped plugins.installs records in ${configPath} could not be migrated into the plugin index. Fix state directory permissions or run astroclaw plugins registry --refresh, then retry. ${formatErrorMessage(
          err,
        )}`,
        { cause: err },
      );
  ...
  function loadConfig(): AstroclawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(
        validationConfigRaw as AstroclawConfig,
        {
          env: deps.env,
          homedir: deps.homedir,
        },
      ...
      const loadValidationPluginMetadataSnapshot = (config: AstroclawConfig) => {
  ...
  async function readConfigFileSnapshotInternal(): Promise<ReadConfigFileSnapshotInternalResult> {
    ...
    let fallbackSourceConfig: AstroclawConfig = {};
  ...
  async function readBestEffortConfig(): Promise<AstroclawConfig> {
    const result = await readConfigFileSnapshotInternal();
  ...
  async function readSourceConfigBestEffort(): Promise<AstroclawConfig> {
    maybeLoadDotEnvForConfig(deps.env);
    const exists = deps.fs.existsSync(configPath);
    if (!exists) {
      return {};
  ...
  async function writeConfigFile(
  ...
  ): Promise<{ persistedHash: string; persistedConfig: AstroclawConfig }> {
    ...
    persistCandidate = applyUnsetPathsForWrite(persistCandidate as AstroclawConfig, unsetPaths);
    ...
    let cfgToWrite = persistCandidate as AstroclawConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as AstroclawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.ASTROCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      if (!isVerbose() && deps.env.ASTROCLAW_CONFIG_OVERWRITE_LOG !== "1" && !shouldLogInVitest) {
        return;
      }
      deps.logger.warn(
        formatConfigOverwriteLogMessage({
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.ASTROCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
...
// module scope. `ASTROCLAW_CONFIG_PATH` (and friends) are expected to work even
...
function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: AstroclawConfig;
...
export function projectConfigOntoRuntimeSourceSnapshot(config: AstroclawConfig): AstroclawConfig {
...
export function loadConfig(): AstroclawConfig {
  ...
  // reparsing astroclaw.json on hot code paths.
  return loadPinnedRuntimeConfig(() => createConfigIO().loadConfig());
}

export function getRuntimeConfig(): AstroclawConfig {
  return loadConfig();
}

export async function readBestEffortConfig(): Promise<AstroclawConfig> {
  return await createConfigIO().readBestEffortConfig();
}

export async function readSourceConfigBestEffort(): Promise<AstroclawConfig> {
...
export async function writeConfigFile(
  ...
  let canonicalSourceConfig: AstroclawConfig = nextCfg;
  try {
