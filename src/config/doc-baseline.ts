...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.ASTROCLAW_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveAstroclawPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    ASTROCLAW_STATE_DIR: path.join(os.tmpdir(), "astroclaw-config-doc-baseline-state"),
