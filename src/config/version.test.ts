...
import {
  compareAstroclawVersions,
  isSameAstroclawStableFamily,
  parseAstroclawVersion,
  shouldWarnOnTouchedVersion,
} from "./version.js";

describe("parseAstroclawVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseAstroclawVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseAstroclawVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseAstroclawVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseAstroclawVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseAstroclawVersion("2026.3")).toBeNull();
    expect(parseAstroclawVersion("latest")).toBeNull();
  });
});

describe("compareAstroclawVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareAstroclawVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareAstroclawVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareAstroclawVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareAstroclawVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareAstroclawVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareAstroclawVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});

describe("isSameAstroclawStableFamily", () => {
  it("treats same-base stable and correction versions as one family", () => {
    expect(isSameAstroclawStableFamily("2026.3.23", "2026.3.23-1")).toBe(true);
    expect(isSameAstroclawStableFamily("2026.3.23-1", "2026.3.23-2")).toBe(true);
    expect(isSameAstroclawStableFamily("2026.3.23", "2026.3.24")).toBe(false);
    expect(isSameAstroclawStableFamily("2026.3.23-beta.1", "2026.3.23")).toBe(false);
  });
});
