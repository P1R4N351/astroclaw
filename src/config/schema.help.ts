...
export const FIELD_HELP: Record<string, string> = {
  meta: "Metadata fields automatically maintained by Astroclaw to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
  "meta.lastTouchedVersion": "Auto-set when Astroclaw writes the config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for Astroclaw. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  wizard:
  ...
  "wizard.lastRunVersion":
    "Astroclaw version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version setup changes.",
  ...
  update:
    "Update-channel and startup-check behavior for keeping Astroclaw runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  ...
  "gateway.handshakeTimeoutMs":
    "Pre-auth Gateway WebSocket handshake timeout in milliseconds. Use higher values on loaded or low-powered hosts where local clients can connect during startup warmup. ASTROCLAW_HANDSHAKE_TIMEOUT_MS still takes precedence.",
  ...
  "gateway.tailscale.preserveFunnel":
    "When mode='serve' and an externally configured Tailscale Funnel route already covers the gateway port, skip re-applying tailscale serve on startup. Lets operators keep Funnel exposure managed outside Astroclaw without losing it across gateway restarts.",
  "gateway.remote":
  ...
  "talk.realtime.instructions":
    "Additional system instructions appended to Astroclaw's built-in realtime Talk prompt. Use this for voice style, tone, and other provider-facing realtime behavior while keeping agent-consult guidance intact.",
  ...
  "agents.list[].runtime":
    "Optional runtime descriptor for this agent. Use embedded for default Astroclaw execution or acp for external ACP harness defaults.",
  "agents.list[].runtime.type":
    'Runtime type for this agent: "embedded" (default Astroclaw runtime) or "acp" (ACP harness defaults).',
  ...
  "agents.list[].runtime.acp.agent":
    "Optional ACP harness agent id to use for this Astroclaw agent (for example codex, claude, cursor, gemini, astroclaw).",
  ...
  "browser.profiles.*.driver":
    'Per-profile browser driver mode. Use "astroclaw" (or legacy "clawd") for CDP-based profiles, or use "existing-session" for Chrome DevTools MCP attachment on the selected host or browser node.',
  ...
  "discovery.wideArea.domain":
    "Optional unicast DNS-SD domain for wide-area discovery, such as astroclaw.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
  "discovery.mdns":
  ...
  "tools.toolSearch":
    "Compact large Astroclaw, MCP, and client tool catalogs behind one search/call surface. Set to true for the default code bridge or use the object form to choose the structured fallback.",
  "tools.toolSearch.enabled":
    "Enables Tool Search. When on, Astroclaw hides large tool catalogs behind `tool_search_code` or structured search/describe/call tools during PI runs.",
  ...
  "tools.codeMode":
    "Generic Astroclaw code mode. When enabled, agent runs expose only `exec` and `wait` to the model and hide normal tools behind a QuickJS-WASI catalog bridge.",
  "tools.codeMode.enabled":
    "Enables generic code mode. Default is off. When explicitly enabled, Astroclaw fails closed if the runtime is unavailable instead of exposing the full tool list.",
  ...
  "agents.defaults.sandbox.browser.network":
    "Docker network for sandbox browser containers (default: astroclaw-sandbox-browser). Avoid bridge if you need stricter isolation.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /astroclaw).",
  ...
  "gateway.controlUi.allowExternalEmbedUrls":
    "DANGEROUS toggle that allows hosted embeds to load absolute external http(s) URLs. Keep this off unless your Control UI intentionally embeds trusted third-party pages; hosted /__astroclaw__/canvas and /__astroclaw__/a2ui documents do not need it.",
  ...
  "gateway.nodes.allowCommands":
    "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `astroclaw security audit`.",
  ...
  "nodeHost.browserProxy.allowProfiles":
    "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, Astroclaw enforces least-privilege profile access and blocks persistent profile create/delete through the proxy.",
  media:
  ...
  "audio.transcription.command":
    'Executable + args used to transcribe audio (first token must be a safe binary/path), for example `["whisper-cli", "--model", "small", "{{MediaPath}}"]`. Deprecated `{input}` placeholders are migrated to `{{MediaPath}}` by `astroclaw doctor --fix`.',
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $ASTROCLAW_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "skills.load.extraDirs":
    "Additional shared skill roots to scan at lowest precedence. Use this for sibling repos or shared skill packs that should be available without copying them into the Astroclaw workspace.",
  ...
  "models.providers.*.injectNumCtxForOpenAICompat":
    "Controls whether Astroclaw injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
  ...
  "models.providers.*.agentRuntime":
    "Optional low-level agent runtime policy for this provider. Use provider/model runtime policy instead of agent-wide runtime pins; omitted/default lets Astroclaw choose the runtime for the selected provider.",
  ...
  "models.providers.*.localService":
    "Optional on-demand local model server process for this provider. Astroclaw probes healthUrl, starts the command when needed, waits for readiness, and then sends the model request.",
  ...
  "models.providers.*.localService.healthUrl":
    "Readiness URL probed before model requests. If omitted, Astroclaw uses the provider baseUrl with /models appended.",
  ...
  "models.providers.*.localService.idleStopMs":
    "Milliseconds to keep an Astroclaw-started local model server alive after the last request finishes. Set 0 to keep it alive until Astroclaw exits.",
  ...
  "agents.defaults.experimental.localModelLean":
    "Experimental local-model prompt trim. When enabled, Astroclaw drops heavyweight default tools like browser, cron, and message for weaker or smaller local-model backends.",
  ...
  "agents.defaults.memorySearch.store.path":
    "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.astroclaw/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
  ...
  "memory.backend":
    'Selects the global memory engine: "builtin" uses Astroclaw memory internals, while "qmd" uses the QMD sidecar pipeline. Keep "builtin" unless you intentionally operate QMD.',
  "memory.citations":
  ...
  "agents.defaults.agentRuntime":
    "Legacy whole-agent runtime policy. It is ignored by runtime selection; configure runtime policy on a provider or model instead. Run astroclaw doctor --fix to remove stale values.",
  ...
  "agents.defaults.embeddedHarness":
    "Legacy whole-agent embedded harness input. Run astroclaw doctor --fix to remove it and use provider/model runtime policy where needed.",
  ...
  "agents.list.*.agentRuntime":
    "Legacy per-agent runtime policy. It is ignored by runtime selection; configure provider/model runtime policy instead. Run astroclaw doctor --fix to remove stale values.",
  ...
  "agents.list.*.embeddedHarness":
    "Legacy per-agent embedded harness input. Run astroclaw doctor --fix to remove it and use provider/model runtime policy where needed.",
  ...
  "agents.defaults.compaction.midTurnPrecheck":
    "Optional Pi tool-loop precheck that detects context pressure after a tool result is appended and before the next model call. When enabled, Astroclaw reuses existing precheck recovery to truncate tool results or compact before retrying.",
  ...
  "agents.defaults.embeddedPi":
    "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in Astroclaw sessions.",
  ...
  "commands.mcp":
    "Allow /mcp chat command to manage Astroclaw MCP server config under mcp.servers (default: false).",
  "commands.plugins":
  ...
  mcp: "Global MCP server definitions managed by Astroclaw. Embedded Pi and other runtime adapters can consume these servers without storing them inside Pi-owned project settings.",
  "mcp.servers":
    "Named MCP server definitions. Astroclaw stores them in its own config and runtime adapters decide which transports are supported at execution time.",
  "mcp.servers.*.codex":
    "Astroclaw projection metadata for Codex app-server threads only. It does not affect ACP sessions or generic Codex harness config. Omit this block to keep the server available to every Codex app-server agent with Codex's default MCP approval behavior.",
  "mcp.servers.*.codex.agents":
    "Optional non-empty Astroclaw agent ids that should receive this MCP server in Codex app-server thread config. Empty, blank, or invalid lists fail closed; when omitted, the server is projected for all Codex app-server agents.",
  ...
  "mcp.servers.*.codex.default_tools_approval_mode":
    "Codex-native spelling for the same per-server MCP tool approval mode. Prefer defaultToolsApprovalMode in Astroclaw config.",
  ...
  "session.maintenance.rotateBytes":
    'Deprecated and ignored. Do not use for `sessions.json` growth control; Astroclaw no longer creates automatic rotation backups, and "astroclaw doctor --fix" removes this key.',
  ...
  hooks:
    "Inbound webhook automation surface for mapping external events into wake or agent actions in Astroclaw. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
  "hooks.enabled":
