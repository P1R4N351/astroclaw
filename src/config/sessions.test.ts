...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ASTROCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-astroclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-astroclaw#general");
  });
  ...
  it("derives session transcripts dir from ASTROCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ASTROCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when ASTROCLAW_STATE_DIR differs from stored paths", () => {
