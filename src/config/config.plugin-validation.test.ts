...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "astroclaw.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      ASTROCLAW_STATE_DIR: path.join(suiteHome, ".astroclaw"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "astroclaw-config-plugin-validation-"));
  ...
  it("reports catalog install hints for missing configured official external plugins", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin not installed: brave — install the official external plugin with: astroclaw plugins install @astroclaw/brave-plugin";
  ...
  it("warns instead of failing when an official external memory slot plugin is not installed", () => {
    ...
    expect(res.ok).toBe(true);
    const slotMessage =
      "plugin not installed: memory-lancedb — gateway will run without persistent memory until installed; install the official external plugin with: astroclaw plugins install @astroclaw/memory-lancedb";
    const entryMessage =
      "plugin not installed: memory-lancedb — install the official external plugin with: astroclaw plugins install @astroclaw/memory-lancedb";
  ...
  it("warns instead of failing for stale channel config backed by missing plugin refs", () => {
    ...
    expect(res.warnings).toContainEqual({
      ...
      message:
        "unknown channel id: missing-chat (stale channel plugin config ignored; run astroclaw doctor --fix to remove stale config, or install the plugin)",
    });
    expect(res.warnings).toContainEqual({
      path: "plugins.allow",
      message:
    ...
    expect(res.warnings ?? []).toEqual([
      {
        path: "plugins.allow",
        message:
          "plugin not installed: discord — install the official external plugin with: astroclaw plugins install @astroclaw/discord",
      },
    ]);
  });

  it("uses persisted installed-plugin records as stale channel evidence", async () => {
    const installedPluginIndexPath = path.join(suiteHome, ".astroclaw", "plugins", "installs.json");
    ...
    try {
      ...
      expect(res.warnings).toContainEqual({
        ...
        message:
          "unknown channel id: missing-sms (stale channel plugin config ignored; run astroclaw doctor --fix to remove stale config, or install the plugin)",
      });
    } finally {
  ...
  it("does not fail validation for the implicit default memory slot when plugins config is explicit", () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          ASTROCLAW_BUNDLED_PLUGINS_DIR: path.join(suiteHome, "missing-bundled-plugins"),
        },
      },
    );
    expect(res.ok).toBe(true);
  ...
  it("surfaces invalid Codex native plugin marketplaces as config diagnostics", () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          ASTROCLAW_BUNDLED_PLUGINS_DIR: path.join(process.cwd(), "extensions"),
        },
      },
    );
