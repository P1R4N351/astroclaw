...
type AstroclawMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_ASTROCLAW_TRANSPORT: Record<string, AstroclawMcpHttpTransport | "stdio"> = {
...
export function resolveAstroclawMcpTransportAlias(
  value: unknown,
): AstroclawMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_ASTROCLAW_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_ASTROCLAW_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveAstroclawMcpTransportAlias(next.type);
